...
Remote WordPress Swarm Nodes can sync their memories to the main Geometry OS cluster using a **pull-based architecture**.

### How It Works
*   **Pull-Based**: Main cluster fetches content from remote nodes periodically
*   **Incremental Sync**: Uses timestamps to fetch only new/updated content
*   **Deduplication**: Prevents duplicate posts using source node + post ID tracking
*   **State Persistence**: Sync state saved to disk for recovery after restart

### Configure Remote Nodes

Create or edit `systems/swarm/remote_nodes.json`:
...
```json
{
  "nodes": [
    {
      "node_id": "wp-remote-1",
      "url": "https://your-remote-site.com",
      "api_url": "https://your-remote-site.com/wp-json",
    ...
    },
    {
      ...
      "api_url": "https://another-site.com/wp-json",
      "secret": "your-shared-secret",
      "enabled": true
    }
  ]
...
After updating `remote_nodes.json`, restart the discovery daemon:

```bash
python3 systems/swarm/wp_discovery_daemon.py --stop
python3 systems/swarm/wp_discovery_daemon.py --start
```

### Manual Sync (Python)

For on-demand synchronization outside the daemon:
...
async def manual_sync():
    async with SyncManager() as manager:
        # Add remote node
        node = RemoteNode(
            node_id="wp-manual-sync",
            url="https://your-remote-site.com",
            api_url="https://your-remote-site.com/wp-json",
            enabled=True
        )
        ...
        print(f"Fetched: {result.posts_fetched}, Stored: {result.posts_stored}")

        # View mirrored posts
        posts = manager.get_mirrored_posts(source_node="wp-manual-sync")
        for post in posts[:5]:

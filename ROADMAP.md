...
**Progress:** 38/38 phases complete, 0 in progress, 0 planned

**Deliverables:** 91/91 complete

**Tasks:** 189/189 complete
...
| phase-J Active Substrate — Recursive Logic and Kinetic Matter | COMPLETE | 4/4 | 163K | 11 |
| phase-O Signal Momentum & 4-Tier Memory | COMPLETE | 2/2 | - | 7 |
| phase-P Spatial File System | COMPLETE | 3/3 | - | 10 |
| phase-Q Spatial Neural Networks | COMPLETE | 3/3 | - | 5 |
| phase-R The Spatial Shell | COMPLETE | 4/4 | - | 1 |
| phase-S Spatial Package Manager | COMPLETE | 3/3 | - | 1 |
| phase-T Self-Modifying Programs | COMPLETE | 3/3 | - | 1 |
| phase-U Spatial Evolutionary Algorithms | COMPLETE | 3/3 | - | 1 |
...
| phase-34 Area Agent Evolution | COMPLETE | 5/5 | - | - |
| phase-201 Memory Palace: Checkpointing and Spatial Version Control | COMPLETE | 4/4 | - | - |
| phase-202 Memory Palace: Multi-Modal Spatial Metadata API | COMPLETE | 3/3 | - | - |
| phase-203 Memory Palace: Cross-Platform Export/Import | COMPLETE | 3/3 | ~540 | 3 |
| phase-461 Memory Palace: Shell Tools and Keyboard Navigation | COMPLETE | 3/3 | 20.5K | 3 |
| phase-204 Memory Palace: Autonomous Spatial Exploration | COMPLETE | 14/14 | ~27K | 23 |
...
- {'id': 'phase-290', 'title': 'RISC-V Guest Networking -- Socket Syscall Layer', 'status': 'complete', 'goal': 'Enable bare-metal RISC-V C programs to open TCP connections and exchange data', 'description': 'COMPLETE: src/riscv/socket.rs (483 LOC) implements GuestSockets with sys_socket, sys_connect, sys_sendto, sys_recvfrom, sys_shutdown, sys_close, plus connect_guest/sendto_guest/recvfrom_guest direct-call variants. Bus intercept methods in bus.rs (intercept_connect, intercept_sendto, intercept_recvfrom) bridge guest RAM to GuestSockets. System.rs dispatch wires syscall numbers 198=socket, 203=connect, 206=sendto, 207=recvfrom, 210=shutdown, plus close(57) with socket-first fallback. 7 unit tests + 1 integration test (test_net_syscall_user_mode) + echo-server roundtrip. All 19 socket-related tests pass.', 'deliverables': [{'name': 'Socket syscall implementation for RISC-V guest', 'status': 'complete', 'tasks': [{'id': 'p290d1.t1', 'title': 'Implement socket() syscall returning guest fd', 'status': 'complete', 'description': 'GuestSockets::sys_socket allocates fd slot (0..7). Validates AF_INET+SOCK_STREAM. Returns -EINVAL or -ENOMEM.', 'scope_files': ['src/riscv/socket.rs'], 'scope_lines': 20}, {'id': 'p290d1.t2', 'title': 'Implement send/recv syscalls via host TCP proxy', 'status': 'complete', 'description': 'sys_sendto reads bytes from guest RAM via closure, writes to host TcpStream. sys_recvfrom reads from TcpStream, writes to guest RAM. Non-blocking with 5s timeout. Max 4096 bytes per call.', 'scope_files': ['src/riscv/socket.rs', 'src/riscv/bus.rs'], 'scope_lines': 100}, {'id': 'p290d1.t3', 'title': 'Test: guest echo roundtrip + net_syscall integration', 'status': 'complete', 'description': 'test_connect_send_recv_roundtrip uses echo server, verifies bytes. test_net_syscall_user_mode verifies ECALL dispatch. Both pass.', 'scope_files': ['src/riscv/socket.rs', 'src/riscv/live.rs'], 'scope_lines': 40}]}], 'test_target': 8, 'scope_lines_total': 180}
- {'id': 'phase-291', 'title': 'GeOS Audio System -- PCM Streaming via BEEP', 'status': 'complete', 'goal': 'Extend the single-tone BEEP opcode into a sample-streaming audio pipeline for sound effects and music', 'description': 'The current BEEP opcode plays one sine-wave tone at a time via aplay. For games and demos, we need PCM sample streaming: pre-load a waveform into RAM, then play it back at a given sample rate. Add AUDIO_PLAY and AUDIO_STOP opcodes that stream from a RAM buffer through aplay in a background thread. This enables sound effects in snake.asm, background music in breakout.asm, and ambient audio in screensaver.asm.', 'deliverables': [{'name': 'PCM sample streaming opcodes', 'status': 'complete', 'tasks': [{'id': 'p291d1.t1', 'title': 'Add AUDIO_PLAY opcode (addr_reg, len_reg, rate_reg)', 'status': 'complete', 'description': 'Stream raw PCM samples from RAM through aplay in a background thread. Non-blocking: VM continues while audio plays. Use existing aplay pipe pattern.', 'scope_files': ['src/vm/mod.rs', 'src/audio.rs'], 'scope_lines': 80}, {'id': 'p291d1.t2', 'title': 'Add AUDIO_STOP opcode and audio state query', 'status': 'complete', 'description': 'AUDIO_STOP halts playback. AUDIO_STATUS (register) returns 1 if playing, 0 if done. Allow one concurrent stream (stop previous before starting new).', 'scope_files': ['src/vm/mod.rs', 'src/audio.rs'], 'scope_lines': 40}, {'id': 'p291d1.t3', 'title': 'Demo: tetris with sound effects via PCM streaming', 'status': 'complete', 'description': 'Pre-generate click/line-clear/game-over waveforms at boot, play them on events. Uses AUDIO_PLAY instead of BEEP for multi-sample effects.', 'scope_files': ['programs/tetris_clean.asm'], 'scope_lines': 50}]}], 'test_target': 3, 'scope_lines_total': 170}
- {'id': 'phase-292', 'title': 'RISC-V Fuzzer Coverage Expansion', 'status': 'complete', 'goal': 'Extend the oracle-based RISC-V fuzzer to cover compressed instructions, CSR operations, and memory management', 'description': 'The current fuzzer (src/riscv_fuzzer.rs, 747 lines) generates random RV32IM programs and checks against a reference oracle. It covers basic ALU, load/store, and branches but misses compressed instructions (C extension), CSR reads/writes, and atomic operations. Expanding coverage catches emulator bugs before they manifest in real guest programs. The fuzzer has caught real bugs before -- this is high-value defensive work.', 'deliverables': [{'name': 'Compressed instruction fuzzer coverage', 'status': 'complete', 'tasks': [{'id': 'p292d1.t1', 'title': 'Add C-extension instruction generation to fuzzer', 'status': 'complete', 'description': 'Generate random C.ADDI, C.LI, C.LUI, C.MV, C.J, C.BEQZ, C.BNEZ, C.LW, C.SW, C.LWSP, C.SWSP instructions with proper encoding. Mix with 32-bit instructions at configurable ratio.', 'scope_files': ['src/riscv_fuzzer.rs'], 'scope_lines': 100}, {'id': 'p292d1.t2', 'title': 'Add CSR read/write fuzzer paths', 'status': 'complete', 'description': 'Generate CSRRS/CSRRC/CSRRW for common CSRs (mstatus, mepc, mtvec, sstatus, sepc, stvec). Verify oracle matches emulator state.', 'scope_files': ['src/riscv_fuzzer.rs'], 'scope_lines': 60}, {'id': 'p292d1.t3', 'title': 'Run extended fuzzer for 1M iterations, fix any divergences', 'status': 'complete', 'description': 'Execute the expanded fuzzer with seed variation. Log and categorize any oracle mismatches. Fix emulator bugs found.', 'scope_files': ['src/riscv_fuzzer.rs', 'src/riscv/cpu/'], 'scope_lines': 40}]}], 'test_target': 3, 'scope_lines_total': 200}
- {'id': 'phase-293', 'title': 'GeOS Clipboard -- Cross-Program Pixel Copy/Paste', 'status': 'complete', 'goal': 'Allow programs to share pixel data via a clipboard buffer accessible through new CLIP_COPY and CLIP_PASTE opcodes', 'description': 'Programs currently operate in isolation -- snake.asm draws its own pixels and tetris.asm draws its own. A clipboard buffer lets programs export/import rectangular pixel regions. Use case: a paint program exports a sprite, a game loads it. Implementation: 256x256 pixel buffer (same size as screen), CLIP_COPY reads from screen into buffer, CLIP_PASTE writes from buffer to screen. The buffer persists across program runs within the same VM session.', 'deliverables': [{'name': 'Clipboard opcodes and buffer', 'status': 'complete', 'tasks': [{'id': 'p293d1.t1', 'title': 'Add CLIP_COPY opcode (x, y, w, h registers)', 'status': 'complete', 'description': 'Copy a rectangular region from vm.screen into clipboard buffer. Parameters: x, y, width, height in registers. Clip to screen bounds.', 'scope_files': ['src/vm/mod.rs'], 'scope_lines': 40}, {'id': 'p293d1.t2', 'title': 'Add CLIP_PASTE opcode (x, y registers)', 'status': 'complete', 'description': 'Paste clipboard buffer contents at (x, y) on screen. Supports partial paste at screen edges.', 'scope_files': ['src/vm/mod.rs'], 'scope_lines': 40}, {'id': 'p293d1.t3', 'title': 'Demo: paint program with copy/paste functionality', 'status': 'complete', 'description': 'Extend painter.asm or write a new sprite editor that uses CLIP_COPY/CLIP_PASTE to duplicate regions and build sprite sheets.', 'scope_files': ['programs/clipboard_demo.asm'], 'scope_lines': 80}]}], 'test_target': 3, 'scope_lines_total': 160}
- {'id': 'phase-294', 'title': 'GeOS Sprite Sheet Loader', 'status': 'complete', 'goal': 'Load sprite sheets from VFS files and render them with SPRITE opcode in programs', 'description': 'The SPRITE opcode already exists (0x4A) and blits NxM pixels from RAM to screen. But there is no standard way to load sprite data from VFS into RAM at a known address. Add a SPRITE_LOAD opcode that reads a VFS file into RAM at a given address, formatted as a width x height pixel array. Programs can then use SPRITE to render individual frames from the loaded sheet. This enables animated sprites in games without manually encoding pixel data in assembly.', 'deliverables': [{'name': 'SPRITE_LOAD opcode for VFS-to-RAM sprite loading', 'status': 'complete', 'tasks': [{'id': 'p294d1.t1', 'title': 'Add SPRITE_LOAD opcode (filename_addr, dest_addr, width, height regs)', 'status': 'complete', 'description': 'Open a VFS file by name (null-terminated string at filename_addr), read pixel data into RAM starting at dest_addr. Each pixel is a u32 color value. Return 0 on success, error code on failure.', 'scope_files': ['src/vm/mod.rs', 'src/vm/ops_syscall.rs'], 'scope_lines': 60}, {'id': 'p294d1.t2', 'title': 'Add SPRITE_FRAME opcode for sheet animation', 'status': 'complete', 'description': 'Given a loaded sheet base address, frame dimensions, and frame index, compute the pixel offset for that frame. Write result to a register. Thin wrapper around address arithmetic.', 'scope_files': ['src/vm/mod.rs'], 'scope_lines': 30}, {'id': 'p294d1.t3', 'title': 'Demo: animated character with sprite sheet', 'status': 'complete', 'description': 'Create a sprite sheet (4-8 frames of a walking character), load via SPRITE_LOAD, animate via SPRITE_FRAME + SPRITE in a game loop.', 'scope_files': ['programs/sprite_anim.asm'], 'scope_lines': 80}]}], 'test_target': 3, 'scope_lines_total': 170}
- {'id': 'phase-295', 'title': 'RISC-V Virtio-Blk Filesystem', 'status': 'complete', 'goal': 'Complete the virtio-blk driver so bare-metal C programs can read/write persistent files', 'description': 'VirtioBlk exists in src/riscv/virtio_blk.rs and is wired into the bus, but the driver is a stub -- it returns zeros on read and accepts writes silently. A real virtio-blk driver backed by a host file would give guest programs persistent storage beyond the in-memory VFS surface. This is the next step toward a real bare-metal OS: programs that save data across VM restarts.', 'deliverables': [{'name': 'Functional virtio-blk backed by host file', 'status': 'complete', 'tasks': [{'id': 'p295d1.t1', 'title': 'Implement virtio-blk read/write against a host backing file', 'status': 'complete', 'description': 'On VM creation, open or create a backing file (e.g., 1MB). Virtio-blk read returns file contents, write persists to file. Handle virtio descriptor chains properly.', 'scope_files': ['src/riscv/virtio_blk.rs', 'src/riscv/bus.rs'], 'scope_lines': 120}, {'id': 'p295d1.t2', 'title': 'Add GEOS_BLK_READ/WRITE helpers to libgeos', 'status': 'complete', 'description': 'Add C functions that wrap virtio-blk MMIO reads/writes: geos_blk_read(sector, buf, n), geos_blk_write(sector, buf, n). Add to libgeos.a.', 'scope_files': ['examples/riscv-hello/libgeos.c', 'examples/riscv-hello/libgeos.h'], 'scope_lines': 60}, {'id': 'p295d1.t3', 'title': 'Test: guest writes data, restarts VM, reads data back', 'status': 'complete', 'description': 'C program writes a known pattern to block 0, triggers shutdown, runner restarts VM, program reads block 0 and verifies pattern. Proves persistence.', 'scope_files': ['examples/riscv-hello/blk_test.c', 'examples/blk_persist_test.rs'], 'scope_lines': 50}]}], 'test_target': 3, 'scope_lines_total': 230}
- {'id': 'phase-296', 'title': 'GeOS Collision Detection Library', 'status': 'complete', 'goal': 'Build reusable collision detection primitives as assembly subroutines in the standard library', 'description': 'Multiple game programs (snake, ball, breakout) implement their own collision detection with PEEK and manual bounds checking. This is duplicated, error-prone and wasteful. Create a collision detection library in lib/ with subroutines for AABB rectangle overlap, point-in-rect, circle-rect intersection, and pixel-perfect collision. Programs CALL these instead of inlining the logic.', 'deliverables': [{'name': 'Collision detection subroutine library', 'status': 'complete', 'tasks': [{'id': 'p296d1.t1', 'title': 'Implement AABB rectangle overlap subroutine', 'status': 'complete', 'description': 'rect_overlap(x1,y1,w1,h1, x2,y2,w2,h2) -> r0=1 if overlap, 0 if not. All args in registers. Use CMP+BLT pattern. No RAM usage.', 'scope_files': ['programs/lib/collision.asm'], 'scope_lines': 40}, {'id': 'p296d1.t2', 'title': 'Implement circle-rect and point-in-circle subroutines', 'status': 'complete', 'description': "circle_rect_intersect(cx,cy,cr, rx,ry,rw,rh) and point_in_circle(px,py,cx,cy,cr). Use squared distance to avoid SQRT (which doesn't exist).", 'scope_files': ['programs/lib/collision.asm'], 'scope_lines': 50}, {'id': 'p296d1.t3', 'title': 'Refactor snake.asm to use collision library', 'status': 'complete', 'description': 'Replace inline collision checks in snake.asm with CALL to library subroutines. Verify behavior unchanged. Measure instruction savings.', 'scope_files': ['programs/snake.asm'], 'scope_lines': 40}]}], 'test_target': 3, 'scope_lines_total': 130}
- {'id': 'phase-297', 'title': 'MCP Server -- Programmatic Assembly and Execution', 'status': 'complete', 'goal': 'Add MCP tools for loading, assembling, and running .asm programs from external agents', 'description': 'The MCP server (src/mcp_server.rs) exposes status/screenshot/registers tools but cannot load and execute programs. An agent must type into the canvas via vm_type, which is slow and fragile. Add asm_load, asm_assemble, asm_run, and asm_step MCP tools so Hermes can programmatically test and develop GeOS programs. This unblocks the hermes/build agent loop from inside MCP.', 'deliverables': [{'name': 'Assembly and execution MCP tools', 'status': 'complete', 'tasks': [{'id': 'p297d1.t1', 'title': 'Add asm_load tool -- load .asm file into canvas buffer', 'status': 'complete', 'description': 'Takes file path, reads .asm source, writes to canvas buffer via load_source_to_canvas. Returns line count and any parse warnings.', 'scope_files': ['src/mcp_server.rs'], 'scope_lines': 40}, {'id': 'p297d1.t2', 'title': 'Add asm_assemble and asm_run tools', 'status': 'complete', 'description': 'asm_assemble triggers F8 (assemble canvas to bytecode). asm_run starts execution from 0x1000 with optional step limit. Returns register state and screen checksum after execution.', 'scope_files': ['src/mcp_server.rs'], 'scope_lines': 60}, {'id': 'p297d1.t3', 'title': 'Test: Hermes loads, assembles, runs hello.asm via MCP', 'status': 'complete', 'description': 'Integration test that calls the MCP tools in sequence and verifies the output matches expected screen content.', 'scope_files': ['src/mcp_server.rs'], 'scope_lines': 30}]}], 'test_target': 2, 'scope_lines_total': 130}
- {'id': 'phase-298', 'title': 'RISC-V Multi-Process -- Cooperative Context Switching', 'status': 'complete', 'goal': 'Run multiple bare-metal C programs concurrently in the RISC-V interpreter with cooperative yielding', 'description': 'The GeOS VM has SPAWN/KILL for multi-process but the RISC-V guest runs a single ELF. Add a lightweight context switcher: the guest calls a yield SBI extension, the host saves all registers and switches to a second guest context (separate register file, separate PC, shared MMIO). This proves multi-programming on the RISC-V layer without a full kernel -- just register save/restore and PC swap.', 'deliverables': [{'name': 'Cooperative multi-process for RISC-V guests', 'status': 'complete', 'tasks': [{'id': 'p298d1.t1', 'title': 'Add SBI_YIELD extension to sbi.rs', 'status': 'complete', 'description': 'New SBI extension (0x4759 = "GY"). Guest calls ecall with a7=0x4759, a0=context_id. Host saves current context, switches to target context. Return to caller on next yield.', 'scope_files': ['src/riscv/sbi.rs', 'src/riscv/mod.rs'], 'scope_lines': 80}, {'id': 'p298d1.t2', 'title': 'Add context management (create, switch, destroy)', 'status': 'complete', 'description': 'RiscvVm holds a Vec of GuestContext (registers, PC, privilege). SBI_YIELD with new context_id creates it. SBI_YIELD with existing ID switches. SBI_KILL terminates a context.', 'scope_files': ['src/riscv/mod.rs'], 'scope_lines': 60}, {'id': 'p298d1.t3', 'title': 'Demo: two C programs alternating on framebuffer', 'status': 'complete', 'description': 'Program A draws a bouncing ball, yields every frame. Program B draws a rotating pattern, yields every frame. Both share the framebuffer, producing a composite display.', 'scope_files': ['examples/riscv-hello/multiproc_a.c', 'examples/riscv-hello/multiproc_b.c', 'examples/multiproc_run.rs'], 'scope_lines': 80}]}], 'test_target': 3, 'scope_lines_total': 220}
- {'id': 'phase-299', 'title': 'GeOS Text Rendering -- Variable-Width Font Support', 'status': 'complete', 'goal': 'Support variable-width fonts in the TEXT opcode for more readable text display', 'description': 'The TEXT opcode uses a fixed 8x8 pixel font (font.rs). For programs that display paragraphs of text (notepad.asm, chatbot.asm, help.asm), variable-width characters (like a real terminal font) look dramatically better. Add a FONT_SELECT opcode that switches the active font, and implement a proportional 5x7 font (mini_font.in already exists). The existing 8x8 font remains the default.', 'deliverables': [{'name': 'Variable-width font rendering', 'status': 'complete', 'tasks': [{'id': 'p299d1.t1', 'title': 'Implement proportional font rendering in TEXT opcode', 'status': 'complete', 'description': 'Add font selection state to Vm. When proportional font is active, TEXT uses character-width table from mini_font.in. Advance cursor by character width instead of fixed 8 pixels.', 'scope_files': ['src/vm/mod.rs', 'src/font.rs'], 'scope_lines': 60}, {'id': 'p299d1.t2', 'title': 'Add FONT_SELECT opcode (font_id register)', 'status': 'complete', 'description': 'FONT_SELECT r -- 0 = 8x8 fixed, 1 = 5x7 proportional, 2 = 6x10 medium. Persists until changed. Validated range check.', 'scope_files': ['src/vm/mod.rs', 'src/vm/ops_graphics.rs', 'src/assembler/'], 'scope_lines': 40}, {'id': 'p299d1.t3', 'title': 'Demo: notepad with proportional font', 'status': 'complete', 'description': 'Modify notepad.asm to use proportional font for the text area while keeping the fixed font for the UI border. Shows visual improvement.', 'scope_files': ['programs/notepad.asm'], 'scope_lines': 30}]}], 'test_target': 3, 'scope_lines_total': 130}
- {'id': 'phase-300', 'title': 'GeOS Performance -- Instruction Cache for Hot Loops', 'status': 'complete', 'goal': 'Add a decoded instruction cache to speed up tight loops by 3-5x', 'description': 'The VM currently decodes every instruction on every step() call, even in tight loops that execute the same instructions millions of times. At 88 MIPS, this decode overhead is the bottleneck. Add a small direct-mapped cache (256 entries) that maps PC to decoded instruction. On cache hit, skip decode and execute directly. Cache is invalidated on self-modifying code (STORE to code region). Expected speedup: 3-5x on loop-heavy programs like plasma, infinite_map, and the fuzzer.', 'deliverables': [{'name': 'Decoded instruction cache', 'status': 'complete', 'tasks': [{'id': 'p300d1.t1', 'title': 'Implement PC-indexed instruction cache in Vm', 'status': 'complete', 'description': 'Add HashMap<u32, CachedInstruction> to Vm. On step(), check cache before decoding. On cache miss, decode and insert. Invalidate on writes to code pages.', 'scope_files': ['src/vm/mod.rs'], 'scope_lines': 80}, {'id': 'p300d1.t2', 'title': 'Add cache invalidation on self-modifying code', 'status': 'complete', 'description': 'When STORE writes to the code region (0x1000-0x1FFF), invalidate the cache entry for that address. When ASM/ASMSELF writes bytecode, invalidate entire cache.', 'scope_files': ['src/vm/mod.rs'], 'scope_lines': 30}, {'id': 'p300d1.t3', 'title': 'Benchmark: measure speedup on plasma and infinite_map', 'status': 'complete', 'description': 'Run plasma and infinite_map with and without cache. Measure instructions/second. Target: 3x speedup on plasma (heavy loop), no regression on branchy code.', 'scope_files': ['src/vm/tests.rs'], 'scope_lines': 30}]}], 'test_target': 3, 'scope_lines_total': 140}
- {'id': 'phase-301', 'title': 'GeOS Pixel Art Export -- PNG Screenshot to VFS', 'status': 'complete', 'goal': 'Save the current screen as a PNG file in the VFS, accessible from programs and the host', 'description': 'The screenshot opcode exists (F9 key) but saves to a host file path. Programs cannot programmatically save their visual output. Add a SCREENSHOT opcode that encodes the current 256x256 screen as PNG and writes it to the VFS. This lets paint.asm save artwork, maze_gen.asm export mazes, and code_evolution.asm archive interesting generations -- all from within the program.', 'deliverables': [{'name': 'SCREENSHOT opcode for in-program PNG export', 'status': 'complete', 'tasks': [{'id': 'p301d1.t1', 'title': 'Add SCREENSHOT opcode (filename_addr register)', 'status': 'complete', 'description': 'Encode vm.screen as PNG (using existing encode_png from vision.rs logic) and write to VFS at the given filename. Return 0 on success.', 'scope_files': ['src/vm/mod.rs', 'src/vision.rs'], 'scope_lines': 60}, {'id': 'p301d1.t2', 'title': 'Add SCREEN_LOAD opcode to load PNG into screen', 'status': 'complete', 'description': 'Read a VFS file, decode as PNG, write pixels to vm.screen. Inverse of SCREENSHOT. Uses existing PNG decode logic.', 'scope_files': ['src/vm/mod.rs'], 'scope_lines': 40}, {'id': 'p301d1.t3', 'title': 'Demo: paint program with save/load functionality', 'status': 'complete', 'description': 'Painter saves current canvas as "painting1.png" via SCREENSHOT. On next run, loads it back via SCREEN_LOAD. Proves the round-trip.', 'scope_files': ['programs/painter.asm'], 'scope_lines': 30}]}], 'test_target': 3, 'scope_lines_total': 130}
- {'id': 'phase-302', 'title': 'GeOS Tilemap Editor -- Visual Map Building Tool', 'status': 'complete', 'goal': 'Build an interactive tilemap editor that lets users paint tile-based levels for games', 'description': 'The TILEMAP opcode (0x4C) blits tile grids from RAM to screen, but there is no tool to visually design tilemaps. Create a tilemap editor program with a tile palette on the left (selectable tiles), a grid canvas on the right (click to place tiles), and export to RAM format. This enables level design for roguelike.asm, maze.asm, and future tile-based games.', 'deliverables': [{'name': 'Interactive tilemap editor program', 'status': 'complete', 'tasks': [{'id': 'p302d1.t1', 'title': 'Tile palette display and selection UI', 'status': 'complete', 'description': 'Left panel shows 8x8 pixel tiles (8 across, scrollable). Arrow keys move selection cursor. Selected tile highlights. Uses SPRITE or PSET for rendering.', 'scope_files': ['programs/tilemap_editor.asm'], 'scope_lines': 100}, {'id': 'p302d1.t2', 'title': 'Grid canvas with tile placement', 'status': 'complete', 'description': 'Right panel is a 16x16 grid (each cell = 16x16 pixels = 4x4 tile). Arrow keys + space to place selected tile. WASD to scroll. IKEY for input.', 'scope_files': ['programs/tilemap_editor.asm'], 'scope_lines': 80}, {'id': 'p302d1.t3', 'title': 'Export tilemap to RAM for use with TILEMAP opcode', 'status': 'complete', 'description': 'Press S to save the tilemap as a flat index array in RAM at 0x7000. Another program can then use TILEMAP to render the level. Include a demo game that loads the saved map.', 'scope_files': ['programs/tilemap_editor.asm', 'programs/tilemap_demo.asm'], 'scope_lines': 60}]}], 'test_target': 2, 'scope_lines_total': 240}
- {'id': 'phase-303', 'title': 'RISC-V GPU Compute -- WGSL Shader Integration', 'status': 'partial', 'goal': 'Enable the RISC-V guest to offload compute work to the host GPU via the existing WGSL shader pipeline', 'description': 'src/riscv/gpu.rs already has a WGSL compute shader (riscv_executor.wgsl) behind a feature flag. The GPU executor can run multiple RISC-V tiles in parallel on the GPU. PROGRESS: Phase 303 boot-path work is done -- init_gpu_substrate() added to RiscvVm, called from boot_linux_setup_with_patches() and vm_thread_main(ELF mode). Creates headless wgpu instance, probes adapter, builds ActiveSubstrate(256,256). Feature-gated behind #[cfg(feature = "gpu")]. 6 tests added. REMAINING: Wire GPU executor into step loop, add GPU memory transfer, benchmark vs CPU.', 'deliverables': [{'name': 'GPU-accelerated RISC-V execution', 'status': 'todo', 'tasks': [{'id': 'p303d1.t1', 'title': 'Wire GPU executor into RiscvVm::step loop', 'description': 'When gpu feature is enabled and a GPU adapter is available, dispatch tile execution to the GPU shader instead of the CPU interpreter. Fall back to CPU on GPU errors.', 'scope_files': ['src/riscv/mod.rs', 'src/riscv/gpu.rs'], 'scope_lines': 80}, {'id': 'p303d1.t2', 'title': 'Add GPU memory transfer (guest RAM <-> GPU buffer)', 'description': 'Upload guest RAM to GPU buffer before compute, download results after. Handle dirty-page tracking to minimize transfers.', 'scope_files': ['src/riscv/gpu.rs'], 'scope_lines': 60}, {'id': 'p303d1.t3', 'title': 'Benchmark: life.c with and without GPU', 'description': "Run Conway's life at 256x256 with CPU-only vs GPU-accelerated. Measure frames per second. Target: 5x+ speedup on GPU.", 'scope_files': ['examples/riscv-hello/life.c'], 'scope_lines': 30}]}], 'test_target': 2, 'scope_lines_total': 170}
- {'id': 'phase-304', 'title': 'Hypervisor Integration -- Socket Command Wiring', 'status': 'complete', 'goal': 'Wire the hypervisor_boot and hypervisor_kill socket commands to the QemuBridge implementation', 'description': 'Previously the hypervisor_boot socket command only set a flag on the VM struct that was never read. This phase wires the command to actually spawn a QEMU process via QemuBridge, enabling programmatic control of guest OS instances from external agents. Also added debug logging for socket binding.', 'deliverables': [{'name': 'Wired socket commands', 'status': 'complete', 'tasks': [{'id': 'p304d1.t1', 'title': 'Implement QemuBridge::spawn in socket handler', 'description': 'Modified src/main.rs to call QemuBridge::spawn and set qemu_active when hypervisor_boot is received.', 'status': 'complete'}, {'id': 'p304d1.t2', 'title': 'Implement QEMU termination in hypervisor_kill', 'description': 'Ensured the QEMU process is killed and the bridge is cleared when hypervisor_kill is received.', 'status': 'complete'}]}], 'test_target': 1, 'scope_lines_total': 50}

## [x] phase-J: Active Substrate — Recursive Logic and Kinetic Matter (COMPLETE)

**Goal:** Transform the screen into a living computational fabric with 1700x compression and 10k FPS logic.

We have moved beyond static rendering to an **Active Substrate** where pixels are autonomous agents. This phase established the physics of 'Kinetic Matter' (momentum transfer) and verified Turing-completeness via a Full Adder in the pixel grid.

### Deliverables

- [x] **1700x Hierarchical DGI Compression** -- Replaced sliding-window mining with a recursive 2x2 quadtree induction strategy. Verified 1,741x collapse ratio on 1024x1024 worlds.
- [x] **10,000+ FPS GPU Expander** -- Engineered a WGSL compute shader for top-down quadtree expansion. Verified 10,523 FPS on a 1024x1024 viewport with unrolled recursion.
- [x] **Kinetic Matter Physics** -- Implemented momentum transfer where signals physically push matter. Verified spatially in the active substrate test suite.
- [x] **Verified Turing-Completeness** -- Implemented AND, OR, XOR, and NOT logic gates via signal collisions. Verified all 8 rows of a Full Adder truth table in the fabric.

## [x] phase-O: Signal Momentum & 4-Tier Memory (COMPLETE)

**Goal:** Establish autonomous spatial execution and a self-organizing memory hierarchy.

This phase moved Geometry OS from host-driven logic to autonomous substrate-native execution. It established 'Signal Momentum' where signals sustain themselves in loops, and defined a 4-tier memory hierarchy to handle infinite address space.

### Deliverables
- [x] **Signal Momentum Verified** -- PC pulses sustain autonomous circulation in substrate loops for 500+ ticks without host intervention.
- [x] **4-Tier Memory Hierarchy** -- Implemented Active (GPU), Dormant (RAM), Archived (Disk/EPL), and Genome (Seed) stack for scaling.

## [x] phase-P: Spatial File System (COMPLETE)

**Goal:** Map traditional filesystem semantics to the geometric grid.

Established the 'Screen as the Hard Drive' architecture. Standard syscalls (OPEN/READ/WRITE) now map directly to grid regions, enabling persistent spatial programs.

### Deliverables
- [x] **VFS-to-Substrate Bridge** -- `OPEN/READ/WRITE/CLOSE/SEEK` map standard syscalls to grid pixels via TileStore.
- [x] **Hilbert Directory** -- Global indexing using Order-16 Hilbert Curve provides ~18 exabytes of addressable space.
- [x] **EPL Native Storage** -- Implemented RLE-compressed spatial deltas for persistent grid journaling across VM lifecycles.

## [x] phase-Q: Spatial Neural Networks (COMPLETE)

**Goal:** Evolve the substrate into a learning-capable computational fabric.

Transitioned from rigid logic to dynamic weighted execution. Proved non-linear spatial convergence via a trainable XOR network entirely on the GPU.

### Deliverables
- [x] **Neuron & Synapse Primitives** -- Implemented `LOGIC_NEURON` (Type 5) with thresholding and `TAXONOMY_WIRE` with 24-bit fixed-point weights.
- [x] **Balanced Competitive Learning** -- Implemented Hebbian Plasticity ('bump') balanced by Synaptic Decay ('forgetting') for stable training.
- [x] **Verified XOR Convergence** -- Successfully trained a multi-layer spatial network to solve the XOR truth table in 50 epochs.

## [x] phase-R: The Spatial Shell (COMPLETE)

**Goal:** Create a robust interactive interface for the geometric substrate.

Built the primary user interface for Geometry OS, enabling real-time debugging and control of the 4-tier memory stack.

### Deliverables
- [x] **Hardened Terminal Engine** -- Implemented modal input (Ctrl+B) to decouple command entry from assembly interaction.
- [x] **Substrate Visualizer** -- Taxonomy-aware decoder translates raw GPU bits into high-fidelity, color-coded execution maps.
- [x] **Non-Blocking Execution** -- Refactored substrate 'run' into an async state machine for responsive UI during long training cycles.
- [x] **Bridge Parser** -- Rust-native command handler for `pulse`, `clear`, `step`, and `run` substrate-native commands.

## [x] phase-S: Spatial Package Manager (COMPLETE)

**Goal:** Enable modularity and portability of spatial circuits.

Established the foundation for a shared ecosystem of geometric software. Verified bit-perfect roundtrips of trained neural states via the RTS container format.

### Deliverables
- [x] **RTS Container Format (.rts.png)** -- 32-bit RGBA container preserving all substrate metadata (Taxonomy, Weights, Thresholds).
- [x] **Export/Import Engine** -- Implemented `export` and `import` shell commands for dumping/loading GPU substrate regions.
- [x] **Verified Circuit Portability** -- Confirmed bit-perfect roundtrip of pre-trained neural networks via automated test suite.

## [x] phase-T: Self-Modifying Programs (COMPLETE)

**Goal:** Enable substrate-native spatial code emission and replication.

The peak of 'Geometry IS the Software.' Proved that programs can autonomously architect their own physical layout on the infinite map using the MMIO JIT primitives.

### Deliverables
- [x] **Substrate JIT Primitives** -- Implemented `SUBSTRATE_PATCH` and `SUBSTRATE_COPY` MMIO commands for surgical grid manipulation.
- [x] **Self-Replicating Assembly** -- Developed `self_replicator.asm`, a program that clones its own geometric state to a new grid region.
- [x] **Verified Autonomous Expansion** -- Confirmed via automated test that a spatial circuit can expand its physical footprint and trigger execution of its offspring.
- [x] **Gray Goo Proof** -- Successfully executed a cascading spatial replicator that autonomously spawned 4 generations across the infinite map.

## [x] phase-U: Spatial Evolutionary Algorithms (COMPLETE)

**Goal:** Evolve pure spatial logic from noise via VM-mediated selection.

Transformed the VM into the 'Blind Watchmaker'. Implemented a massively parallel evolutionary harness that uses substrate physics as the genome and JIT primitives for reproduction.

### Deliverables
- [x] **Massively Parallel Harness** -- Orchestrated simultaneous evaluation of 64 spatial circuits on a 2D grid.
- [x] **Spatial Crossover & Elitism** -- Implemented recombination of spatial sub-circuits and preservation of the fittest individuals.
- [x] **Non-Linear Convergence** -- Proven that random spatial noise can be evolved into XOR-approximating logic (3.1/4.0 fitness) autonomously.


## [x] phase-34: Area Agent Evolution (COMPLETE)

**Goal:** Establish autonomous region management and large-scale substrate maintenance.

Phase 34 leverages the 'Geometric Intelligence' vision stack and 'Vast Substrate' scaling to enable autonomous Area Agents. These agents manage 1GB+ visual storage, prune noise, and maintain structural health (PAS) across the infinite map.

### Deliverables
- [x] **Vast Substrate Foundation** -- Implemented O(1) LRU eviction and 512-chunk memory cache for high-performance visual storage.
- [x] **PNG Tile Serialization** -- Verified bit-perfect visual interchange for 64x64 substrate chunks via standard PNG format.
- [x] **Multi-modal Navigation** -- Implemented `jump` and `look` commands, granting Hermes 'sight' across the entire 1GB landscape.
- [x] **Geometric Diagnostic Expert** -- Upgraded Hermes system prompt for visual reasoning, fracture detection, and autonomous diagnostic.
- [x] **Empirical Sight Benchmark** -- Proven diagnostic advantage of multi-modal vision vs text-only via `vision_bench.rs` harness.

## [ ] phase-201: Memory Palace Checkpointing and Spatial Version Control (PLANNED)

**Goal:** Enable spatial state snapshots, version management, and safe experimentation in the Memory Palace.

Based on convergence #3 "Spatial Data Storage" - agents need version-controlled, immutable spatial snapshots for experimentation without loss.

### Deliverables

- [ ] **mcp_geo_vm_checkpoint - save spatial state to file**
  - [x] `201.d1.t1` Add checkpoint() method to RiscvVm
    - Serialize: player position, buildings list, canvas state (checksum), register state, open windows
    - Write to .geometry_os/checkpoints/<timestamp>.json
    _Files: src/riscv/mod.rs_
  - [x] `201.d1.t2` Add MCP tool: mcp_geo_vm_checkpoint
    - Registered in mcp_server.rs
    - Returns checkpoint_id (timestamp) and path
    _Files: src/mcp_server.rs_
  - [x] `201.d1.3` Add restore() method to RiscvVm
    - Load checkpoint file, restore player/buildings/canvas/registers/windows
    - Clear all transient state (running programs, background processes)
    _Files: src/riscv/mod.rs_
  - [x] `201.d1.4` Add MCP tool: mcp_geo_vm_restore
    - Takes checkpoint_id, restores state
    - Returns success/failure with diff summary
    _Files: src/mcp_server.rs_
  _~500 LOC, 4 unit tests_

- [ ] **Spatial version control - branch and diff**
  - [ ] `201.d2.t1` Add checkpoint branch concept
    - Each checkpoint has parent_id, enabling branch graph
    - List checkpoints with ancestry
    _Files: src/riscv/checkpoint.rs_
  - [ ] `201.d2.2` Diff between checkpoints
    - Compare buildings (added/removed), player position delta, canvas checksum diff
    - Return structured JSON diff
    _Files: src/riscv/checkpoint.rs_
  - [ ] `201.d2.3` Merge branches (union of buildings)
    - Merge two checkpoints: union of building sets, resolve conflicts by timestamp
    - Warn on building ID collisions
    _Files: src/riscv/checkpoint.rs_
  _~300 LOC, 3 unit tests_

### Technical Notes

Checkpoints capture the entire spatial state at a moment in time. Buildings are the primary mutable entity - their positions, app associations, and window configurations. Player position and facing are also saved. Canvas state is saved as a checksum for verification; actual pixel data already persists via VFS surface.

### Risks

- Checkpoint file size grows with building count. Mitigation: incremental checkpoints only store diff from parent.
- Window state restoration may fail if process is gone. Mitigation: graceful failure, window closed notification.

## [x] phase-202: Memory Palace Multi-Modal Spatial Metadata API (COMPLETE)

**Goal:** Enable agents to attach semantic metadata to spatial locations for richer memory encoding.

Based on convergence #1 "Spatial Memory Encoding" - multi-modal encoding and simplified user encoding primitives.

### Deliverables

- [ ] **Metadata attachment to buildings and coordinates**
  - [ ] `202.d1.t1` Extend Building struct with metadata field
    - HashMap<String, String> for arbitrary key-value pairs
    - Common keys: description, tags, purpose, last_accessed, access_count
    _Files: src/mcp_server.rs (Building struct)_
  - [ ] `202.d1.2` MCP tool: mcp_geo_building_set_metadata
    - Takes building_id, key, value
    - Returns success/failure
    _Files: src/mcp_server.rs_
  - [ ] `202.d1.3` MCP tool: mcp_geo_building_get_metadata
    - Takes building_id, optional key filter
    - Returns all metadata or specific value
    _Files: src/mcp_server.rs_
  _~200 LOC, 3 unit tests_

- [ ] **Metadata query API - search buildings by tags/descriptions**
  - [ ] `202.d2.1` MCP tool: mcp_geo_building_search_metadata
    - Takes query string (e.g., "development tool", tag="IDE")
    - Returns list of building_ids with matching metadata
    _Files: src/mcp_server.rs_
  - [ ] `202.d2.2` Coordinate-based metadata
    - Allow metadata on arbitrary coordinates (not just buildings)
    - Virtual annotations on the spatial grid
    _Files: src/mcp_server.rs_
  - [ ] `202.d2.3` Multi-modal encoding patterns
    - Encode text, tags, and visual cues (color hints)
    - Example: "development building with blue tint, IDE tag"
    _Files: src/mcp_server.rs_
  _~250 LOC, 3 unit tests_

### Technical Notes

Metadata is ephemeral by default - stored in RAM only. Persistence is a future phase (serialization to buildings.json). Key-value schema is flexible; conventions emerge through agent usage patterns.

### Risks

- Metadata bloat. Mitigation: pruning policy based on last_accessed, size limits per building.
- Inconsistent keys across agents. Mitigation: document common keys, suggest conventions.

## [x] phase-203: Memory Palace Cross-Platform Export/Import (COMPLETE)

**Goal:** Enable spatial configuration interchange between different platforms and sessions.

Based on convergence #4 "Interoperability Across Platforms" - cross-platform compatibility and flexible access.

### Deliverables

- [x] **Export spatial configuration to portable format**
  - [x] `203.d1.1` Export buildings to JSON
    - Serialize: building_id, position, app_name, metadata, window config
    - Write to memory_palace_export_<timestamp>.json
    _Files: src/export.rs_
  - [x] `203.d1.2` Export canvas state as PNG
    - Screenshot current vm.screen, save to memory_palace_canvas_<timestamp>.png
    - Include reference in export JSON
    _Files: src/export.rs, src/main.rs_
  - [x] `203.d1.3` Export player state
    - Position, facing, last_visited_buildings
    _Files: src/export.rs_
  _~300 LOC, 2 unit tests_

- [x] **Import and merge spatial configurations**
  - [x] `203.d2.t1` Import buildings from JSON
    - Load export file, merge into current buildings list
    - Resolve conflicts: rename buildings on collision, preserve existing by default
    _Files: src/export.rs_
  - [x] `203.d2.2` Import canvas state (optional)
    - Load PNG, composite into vm.screen (or replace entire canvas)
    - User chooses: merge vs replace (alpha blending support)
    _Files: src/export.rs, src/main.rs_
  - [x] `203.d2.3` Sync player position (optional)
    - Teleport player to exported position
    - Warn if position is outside current map bounds
    _Files: src/export.rs_
  _~250 LOC, 3 unit tests_

- [x] **Remote access API - query Memory Palace from other processes**
  - [x] `203.d3.t1` HTTP/REST endpoint for spatial queries
    - Simple HTTP server on localhost:4242
    - Endpoints: GET /buildings, GET /buildings/<id>, GET /player, POST /api/memory_palace/import, GET /health
    - JSON responses
    - Bearer token authentication for write operations
    _Files: src/http_server.rs (~540 LOC)_
  - [x] `203.d3.2` Authentication (basic token)
    - Optional token in header for write operations
    - Read-only access without token
    - Implemented with Bearer token in Authorization header
    _Files: src/http_server.rs_
  - [x] `203.d3.3` WebSocket push for live updates
    - Notify clients on building changes, player movement
    - Enable external dashboards
    - Broadcast channel implementation with tokio-broadcast
    - Update types: PlayerMoved, BuildingChanged, BuildingRemoved, PalaceChanged
    _Files: src/http_server.rs_
  _~540 LOC, 3 unit tests_

**INTEGRATION COMPLETE:**
- HTTP server integrated into cli.rs with --http-server flag
- Tokio runtime properly spawned in separate thread
- CLI flags: --http-server, --http-port <port>, --http-auth <token>
- http feature available (not enabled by default due to Vm architecture)
- Build: `cargo build --release --features http`

**ARCHITECTURAL LIMITATION:**
The HTTP server creates its own VM instance rather than sharing the CLI's VM. This is because Vm contains `Rc<RefCell<...>>` fields which are not thread-safe (no Send/Sync). To enable true VM sharing, Vm would need to be refactored to use thread-safe types (Arc/Mutex instead of Rc/RefCell). This is a significant refactor beyond Phase 203 scope.

### Technical Notes

Export format is intentionally simple - JSON for structures, PNG for canvas. No binary serialization complexity. This maximizes interoperability across platforms (Linux, macOS, Windows, mobile).

### Risks

- Merge conflicts on import. Mitigation: conservative merge (preserve existing, rename imported), conflict resolution UI.
- HTTP server security. Mitigation: localhost-only by default, optional network binding with auth.

## [x] phase-204: Memory Palace Autonomous Spatial Exploration (COMPLETE)

**Goal:** Enable autonomous agents to explore, map, and navigate the Memory Palace without manual guidance.

Based on convergence #5 "Enhanced Navigation Systems" - multi-modal navigation and socially-aware simulation.

### Deliverables

- [x] **Pathfinding API - A* for optimal routes**
  - [x] `204.d1.t1` Build spatial graph from buildings
    - Nodes: buildings + key landmarks (spawn point, map center)
    - Edges: Manhattan distance between nodes
    _Files: src/pathfinding.rs_
  - [x] `204.d1.2` A* pathfinding implementation
    - Heuristic: Manhattan distance to target
    - Returns: list of coordinates (waypoints)
    _Files: src/pathfinding.rs_
  - [x] `204.d1.3` MCP tool: mcp_geo_navigate_to
    - Takes target (building_id or coordinate)
    - Returns path + step-by-step instructions
    - Optional: auto-walk (agent follows path autonomously)
    _Implemented as socket command navigate_to_
  - [x] `204.d1.4` Obstacle avoidance
    - Treat unknown coordinates as walkable (open map assumption)
    - Future: terrain constraints, no-fly zones
    _Files: src/pathfinding.rs_
  _~600 LOC, 11 unit tests_

- [x] **Autonomous exploration and mapping**
  - [x] `204.d2.t1` Exploration frontier algorithm
    - Prioritize unvisited regions near current position
    - Breadth-first expansion from spawn point
    _Files: src/exploration.rs_
  - [x] `204.d2.2` Building discovery API
    - Agent moves in expanding spiral, discovers new buildings
    - Records building_id, position, metadata
    _Files: src/exploration.rs_
  - [x] `204.d2.3` Map memory - visited coordinates
    - Track which tiles have been visited
    - Sparse representation (BitSet or HashSet)
    _Files: src/exploration.rs_
  - [x] `204.d2.4` MCP tool: mcp_geo_explore
    - Starts autonomous exploration loop
    - Stops when all buildings visited or user interrupts
    - Returns exploration log (buildings discovered, tiles visited)
    _Implemented as socket command explore_
  _~500 LOC, 8 unit tests_

- [x] **Multi-modal navigation cues**
  - [x] `204.d3.t1` Visual indicators on canvas
    - Draw path overlay (dots or line) showing route to target
    - Highlight target building with pulsing border
    _Files: src/render.rs_
  - [x] `204.d3.2` Audio hints
    - Synthesized speech: Turn north, 10 steps to Library
    - Triggered at path waypoints on navigation
    - Uses espeak-ng for speech synthesis, falls back silently if unavailable
    _Files: src/audio.rs_
  - [x] `204.d3.3` Desktop notification feedback
    - notify-send on navigation events (arrival, failure)
    - Direction-change chimes via play_navigation_waypoint_chime()
    - Arrival chime via play_navigation_arrival_chime()
    _Files: src/audio.rs_
  _~350 LOC, 4 unit tests_

### Technical Notes

A* with Manhattan heuristic is optimal for the infinite 2D grid. Exploration frontier uses BFS for systematic coverage - guarantees all buildings discovered within bounded radius. Visual overlay uses existing vm.screen compositing infrastructure.

### Risks

- Pathfinding performance degrades with many buildings. Mitigation: spatial partitioning (grid buckets), path caching.
- Autonomous exploration may run forever. Mitigation: radius limit, max_steps threshold, user interrupt handler.
- Audio synthesis may not work on all platforms. Mitigation: optional, fallback to visual-only.

#!/usr/bin/env python3
"""
Simple test script to verify dynamic efficiency metrics are working correctly.
"""
...
async def test_dynamic_efficiency():
    ...
    # Test with some recorded efficiency data
    print(f"\n🔧 Recording test efficiency data...")
    await efficiency_tracker.record_efficiency("test_operation", 23000, 0.121)
    await efficiency_tracker.record_efficiency("test_operation", 23000, 0.131)

    # Get updated report
    report = await efficiency_tracker.get_efficiency_report()
    print(f"   Updated average efficiency: {report['average_efficiency']:.6f}")
    print(f"   Updated efficiency health: {report['efficiency_health_score']:.2f}")

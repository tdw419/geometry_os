...
fn test_rv64_kernel_boot() {
    ...
    assert!(
        output.contains("GEOMETRY OS RV64"),
        "Missing boot banner. Console: {}",
        output
    );
    assert!(
        output.contains("Hart ID:"),
        "Missing hartid. Console: {}",
        output
    );
    assert!(
        output.contains("DTB Addr:"),
        "Missing DTB address. Console: {}",
        output
    );
    assert!(
        output.contains("Fibonacci(20)"),
        "Missing fibonacci output. Console: {}",
        output
    );
    ...
    assert!(
        output.contains("Memory test:"),
        "No memory test output. Console: {}",
        output
    );
    assert!(
        output.contains("Boot complete"),
        "Missing boot completion. Console: {}",
        output
    );
    assert!(
        output.contains("HALT"),
        "Missing halt marker. Console: {}",
        output
    );
    ...
    assert!(
        output.contains("0x0000000000001A6D"),
        "Fibonacci(20) wrong. Console: {}",
        output
    );
...
fn test_rv64_kernel_memory_integrity() {
    ...
    assert!(
        output.contains("Memory test:"),
        "Memory integrity test - no output"
    );
}
...
fn test_rv64_kernel_privilege_drop() {
    ...
    assert!(
        !output.is_empty(),
        "No console output - SBI ecall handling broken"
    );

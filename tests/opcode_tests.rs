...
// ── GEO-53: GPU execution of C-transpiled Minix FS read ──────────────
// Proves the full pipeline: C code -> transpiler -> .gasm -> GPU texture -> GPU execution -> correct results.

#[test]
fn gpu_minix_read_hello() {
    gpu_test_helpers::with_gpu_vm(|gvm| {
        // Same transpiled minix_read.c gasm as GEO-47 software VM test
        let gasm = "\
...
";
        ...
        let program = assembler::parse_gasm(&full).expect("minix_read should assemble");

        // Load program into GPU substrate
        gvm.substrate().load_program(0, &program.pixels);

        // Set up fake disk image in substrate
        // Superblock magic at disk+526: bytes 0x68, 0x24
        gvm.substrate().poke_byte(disk_base + 526, 0x68);
        gvm.substrate().poke_byte(disk_base + 527, 0x24);

        // Inode at disk+2048: +4: file_size=5, +28: zone_num=5
        gvm.substrate().poke_word(disk_base + 2048 + 4, 5);
        gvm.substrate().poke_word(disk_base + 2048 + 28, 5);

        // Data block at disk + 5*512 = disk + 2560: "HELLO"
        let hello = b"HELLO";
        for (i, &byte) in hello.iter().enumerate() {
            gvm.substrate().poke_byte(disk_base + 2560 + i as u32, byte);
        }
        ...
        let vm_state = gvm.vm_state(0);
        assert_eq!(vm_state.state, 2, "minix_read should halt on GPU (state=HALTED)");
        assert_eq!(vm_state.regs[0], 5, "minix_read should return 5 (bytes read)");

        // Verify result buffer contains "HELLO"
        let mut result = [0u8; 5];
        for i in 0..5 {
            result[i] = gvm.substrate().peek_byte(result_buf + i as u32);
        }

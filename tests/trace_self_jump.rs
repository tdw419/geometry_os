...
fn trace_self_jump_step_by_step() {
    ...
    for i in 0..prog.len() {
        ...
        eprintln!(
            "[LOAD] pixel {} = 0x{:08X} (expected 0x{:08X}) {}",
            i,
            read_back,
            prog[i],
            if read_back == prog[i] {
    ...
    eprintln!(
        "[PRE] state={} halted={} pc={} entry={} frame_count={} attention_mask={}",
        vm0.state, vm0.halted, vm0.pc, vm0.entry_point, vm0.frame_count, vm0.attention_mask
    );
    eprintln!(
        "[PRE] base_addr={} bound_addr={}",
        vm0.base_addr, vm0.bound_addr
    );

...
use geometry_os::assembler::parse_gasm;
...
const MNEMONICS: &[&str] = &[
    "NOP", "LDI", "MOV", "LOAD", "STORE", "ADD", "SUB", "MUL", "DIV", "JMP", "BNE", "BEQ", "BLT",
    "BGE", "CALL", "RET", "HALT", "ENTRY", "CHAR", "BLIT", "DATA",
];
...
fn dual_assemble(source: &str) -> Result<bool, String> {
    let result_gasm =
        gasm::assemble(source).map_err(|e| format!("gasm::assemble failed: {}", e))?;
...
fn test_assembler_fuzz_1000_programs() {
            ...
            Err(e) => {
                ...
                if gasm_ok != parse_ok {
                    ...
                    errors.push((
                        i,
                        format!(
                            "Agreement failure: gasm_ok={}, parse_ok={}, err={}",
                            gasm_ok, parse_ok, e
                        ),
                    ));
                } else {
    ...
    eprintln!(
        "Skip:   {} (both assemblers reject -- expected for random input)",
        skip
    );
    ...
    assert!(
        pass > 0,
        "No programs passed dual-assembly -- test may be broken"
    );

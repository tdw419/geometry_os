...
use geometry_os::assembler::{self, op, Program};
use geometry_os::software_vm::SoftwareVm;
...
fn cross_validate_mod() {
    ...
    assert_eq!(
        sv.regs[0],
        gpu_vm.vm_state(0).regs[0],
        "MOD cross-validation failed"
    );
}
...
fn cross_validate_mod_zero_divisor() {
    ...
    assert_eq!(
        sv.regs[0],
        gpu_vm.vm_state(0).regs[0],
        "MOD div-by-zero cross-validation failed"
    );
}
...
fn cross_validate_ldb() {
    ...
    for ch in 10u8..=13 {
        ...
        assert_eq!(
            sv_reg, gpu_reg,
            "LDB cross-val channel r{ch}: soft=0x{sv_reg:02X} gpu=0x{gpu_reg:02X}"
        );
    }
...
fn sv_stb_all_channels() {
    ...
    assert_eq!(
        svm.peek(600),
        0x55EFCDAB,
        "STB should compose bytes into 0x55EFCDAB"
    );
}
...
fn cross_validate_stb() {
    ...
    assert_eq!(
        soft_val, gpu_val,
        "STB cross-val: soft=0x{soft_val:08X} gpu=0x{gpu_val:08X}"
    );
...
fn cross_validate_glyph_mutate() {
    ...
    assert_eq!(
        gpu_vm.vm_state(0).regs[10],
        1,
        "GPU: mutate should return 1"
    );
...
fn cross_validate_spatial_spawn() {
    ...
    for offset in 0..3u32 {
        ...
        assert_eq!(
            sw_val,
            svm.peek(200 + offset),
            "SW: copy mismatch at offset {offset}"
        );
        assert_eq!(
            gpu_val,
            gpu_vm.substrate().peek(200 + offset),
            "GPU: copy mismatch at offset {offset}"
        );
        assert_eq!(
            sw_val, gpu_val,
            "SPATIAL_SPAWN cross-validation failed at offset {offset}"
        );
    }
}
...
fn cross_validate_issue_create() {
    ...
    for i in 0..(4 + 64 * 32) {
        svm.poke(geometry_os::ISSUEQ_BASE + i as u32, 0);
    }
    // Write title
    let title_addr: u32 = 0x0010_0000;
    for i in 0..24 {
        svm.poke(title_addr + i, 0);
    }
    ...
    for i in 0..(4 + 64 * 32) {
        gpu_vm
            .substrate()
            .poke(geometry_os::ISSUEQ_BASE + i as u32, 0);
    }
    for i in 0..24 {
        gpu_vm.substrate().poke(title_addr + i, 0);
    }
    ...
    // Check queue count
    let sv_count = svm.peek(geometry_os::ISSUEQ_BASE + 2);
    let gpu_count = gpu_vm.substrate().peek(geometry_os::ISSUEQ_BASE + 2);
...
fn cross_validate_issue_pick() {
    ...
    assert_eq!(
        sv_id, gpu_id,
        "ISSUE_PICK cross-validation: same issue picked"
    );
}
...
fn cross_validate_issue_update() {
    ...
    let slot_base = geometry_os::ISSUEQ_SLOTS_BASE;
...
fn cross_validate_semantic_merge() {
    ...
    assert_eq!(
        sv_count, gpu_count,
        "SEMANTIC_MERGE count: soft={sv_count} gpu={gpu_count}"
    );
    ...
    assert_eq!(
        sv_p0, gpu_p0,
        "SEMANTIC_MERGE pixel 0: soft=0x{sv_p0:08X} gpu=0x{gpu_p0:08X}"
    );
    assert_eq!(
        sv_p1, gpu_p1,
        "SEMANTIC_MERGE pixel 1: soft=0x{sv_p1:08X} gpu=0x{gpu_p1:08X}"
    );
...
fn cross_validate_branch_prob_not_taken() {
    ...
    assert_eq!(
        sv_r0, 99,
        "SW: prob=0 should not branch, r0=99 (LDI executed)"
    );
    assert_eq!(
        gpu_r0, 99,
        "GPU: prob=0 should not branch, r0=99 (LDI executed)"
    );
...
fn cross_validate_branch_prob_taken() {
    ...
    assert_eq!(
        sv_r0, gpu_r0,
        "BRANCH_PROB taken: r0 soft={sv_r0} gpu={gpu_r0}"
    );

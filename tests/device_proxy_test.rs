...
use geometry_os::assembler::Program;
use geometry_os::cpu_stub::{
    Command, CommandExecutor, CommandResult, CpuStub, CMD_READ_BLOCK, STATUS_COMPLETE,
    STATUS_PENDING,
};
use geometry_os::gasm;
use geometry_os::software_vm::SoftwareVm;
use geometry_os::substrate::Substrate;
use geometry_os::{MSGQ_BASE, MSGQ_DATA_BASE, MSGQ_MAX_DATA};
...
fn test_worker_to_proxy_ipc() {
    ...
    assert_ne!(
        header & 1,
        0,
        "VM 1 should have a pending message, header=0x{:08X}",
        header
    );
    ...
    assert_eq!(
        msg0, 1,
        "Message[0] should be CMD_READ_BLOCK (1), got {}",
        msg0
    );
}

...
use geometry_os::gasp::{Canvas, Draw, GaspDoc, Keyframe, Layer, Timeline};
use geometry_os::gasp_compiler::GaspCompiler;
...
fn test_named_frames_assemble_with_labels() {
    ...
    let (programs, labels) =
        gasm::assemble_filmstrip_with_labels(&output).expect("filmstrip assembly should succeed");
...
"#;
    ...
    let (programs, labels) =
        gasm::assemble_filmstrip_with_labels(source).expect("filmstrip should assemble");
...
fn test_gasp_compiled_named_frame_goto() {
    ...
    let doc = GaspDoc {
        ...
        timeline: Timeline {
            ...
            scripts: vec![geometry_os::gasp::FrameScript {
                frame: 0,
    ...
    // Assemble and run
    let (programs, labels) =
        gasm::assemble_filmstrip_with_labels(&output).expect("filmstrip should assemble");

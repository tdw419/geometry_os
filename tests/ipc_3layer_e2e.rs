...
use geometry_os::cpu_stub::{
    Command, CommandExecutor, CommandResult, CpuStub, CMD_BUF_BASE, CMD_READ_BLOCK, CMD_SLOTS,
    CMD_WORDS, STATUS_COMPLETE, STATUS_ERROR,
};
use geometry_os::gasm;
use geometry_os::software_vm::SoftwareVm;
use geometry_os::substrate::Substrate;
...
fn test_3layer_ipc_minix_read() {
    ...
    let disk_path =
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/minix_test.img");
    let disk_file = std::fs::File::open(&disk_path).expect("minix_test.img should exist");
    ...
    let proxy_prog = gasm::assemble(&proxy_source).expect("device_proxy.gasm should assemble");
    ...
    let worker_prog =
        gasm::assemble(&worker_source).expect("minix_ipc_worker.gasm should assemble");
    ...
    for frame in 0..MAX_FRAMES {
        ...
        eprintln!(
            "Frame {}: worker pc={}, halted={}, proxy halted={}",
        ...
        let mailbox = svm.peek(geometry_os::MSGQ_BASE + 1);
        eprintln!("  Proxy(VM1) mailbox flags: {}", mailbox);

        // Check worker mailbox for VM 2
        let mailbox2 = svm.peek(geometry_os::MSGQ_BASE + 2);
        ...
        if executed > 0 {
            eprintln!(
                "  Substrate cmd[0] result: status={}, result={}",
                substrate.peek(CMD_BUF_BASE + 6),
                substrate.peek(CMD_BUF_BASE + 7)
            );
        }
        ...
        eprintln!(
            "  Substrate block data[0]: 0x{:08x}, [1]: 0x{:08x}",
            sub_data0, sub_data1
        );
        ...
        eprintln!(
            "  SVM block data[0] after sync: 0x{:08x}, [1]: 0x{:08x}",
            data0, data1
        );
        ...
        if svm.vm_state(2).halted != 0 {
            eprintln!(
                "Worker halted after {} frames ({:.1}ms), r0={}, r9={}, r10={}",
                frame + 1,
                start.elapsed().as_secs_f64() * 1000.0,
                svm.vm_state(2).regs[0],
                svm.vm_state(2).regs[9],
                svm.vm_state(2).regs[10]
            );
            break;
        }
    ...
    assert!(
        elapsed.as_secs() < 5,
        "Test took {:.1}s, must complete in under 5s",
        elapsed.as_secs_f64()
    );
    ...
    assert_eq!(
        worker_state.regs[0], 5,
        "Worker r0 should be 5 (bytes read), got {}",
        worker_state.regs[0]
    );
    ...
    assert_eq!(
        &result,
        b"HELLO",
        "Result buffer should contain 'HELLO', got {:?}",

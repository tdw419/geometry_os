...
fn test_plic_external_interrupt_to_trap() {
    ...
    vm.bus.plic.contexts[0].enable = 1 << plic::IRQ_UART;
...
fn test_plic_threshold_blocks_low_priority() {
    ...
    vm.bus.plic.contexts[0].enable = 1 << plic::IRQ_UART;
    vm.bus.plic.contexts[0].threshold = 3;
    vm.bus.plic.signal(plic::IRQ_UART);
    vm.step();
...
fn test_plic_multiple_sources_priority() {
    ...
    vm.bus.plic.contexts[0].enable = (1 << 1) | (1 << plic::IRQ_UART);
    vm.bus.plic.signal(1);
    vm.bus.plic.signal(plic::IRQ_UART);
    assert_eq!(vm.bus.plic.claim(0), plic::IRQ_UART);
}
...
fn test_plic_complete_then_next() {
    ...
    vm.bus.plic.contexts[0].enable = (1 << 1) | (1 << 2);
    vm.bus.plic.signal(1);
    vm.bus.plic.signal(2);
    let first = vm.bus.plic.claim(0);
    assert_eq!(first, 1);
    vm.bus.plic.complete(first, 0);
    assert_eq!(vm.bus.plic.pending & (1 << 1), 0);
    let second = vm.bus.plic.claim(0);
    assert_eq!(second, 2);
}

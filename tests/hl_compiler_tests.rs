...
fn test_factorial() {
    ...
    let state = svm.vm_state(0);
    assert_eq!(state.regs[1], 120, "factorial(5) should be 120");
}
...
fn test_fibonacci_iterative() {
    ...
    let state = svm.vm_state(0);
    assert_eq!(state.regs[1], 55, "fib(10) should be 55");
}
...
fn test_lexer_complex() {
    let tokens = Lexer::new("fn factorial(n) { while (n > 1) { n = n - 1; } return n; }")
        .tokenize()
        .unwrap();

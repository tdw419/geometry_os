...
use geometry_os::filmstrip;
use geometry_os::gasm;
use geometry_os::software_vm::SoftwareVm;
use geometry_os::{SCREEN_BASE, SCREEN_SIZE};
...
fn test_sierpinski_filmstrip_assembles() {
    ...
    let (programs, labels) =
        gasm::assemble_filmstrip_with_labels(&source).expect("sierpinski.gasm should assemble");
    ...
    assert!(
        programs[0].pixels.len() > 5,
        "Init frame should have instructions"
    );

    // Frame 1 (Compute) should be the biggest
    assert!(
        programs[1].pixels.len() > programs[0].pixels.len(),
        "Compute frame should be larger than Init"
    );

    // Frame 2 (Done) should be just a HALT
    assert!(
        programs[2].pixels.len() >= 1,
        "Done frame should have at least HALT"
    );

    eprintln!(
        "[SIERPINSKI] Frame sizes: Init={}, Compute={}, Done={}",
...
fn test_sierpinski_filmstrip_execution() {
    ...
    eprintln!(
        "[SIERPINSKI] Initial: frame_ptr={}, frame_count={}",
    ...
    for i in 0..max_dispatches {
        ...
        if vm0.halted == 1 || vm0.state == 2 {
            eprintln!(
                "[SIERPINSKI] Halted after {} dispatches, frame_ptr={}",
                i + 1,
                vm0.attention_mask
            );
            break;
        ...
        let vm0 = svm.vm_state(0);
        eprintln!(
            "[SIERPINSKI] Dispatch {}: state={} halted={} cycles={} frame_ptr={} r0={} r1={}",
            i + 1,
            vm0.state,
            vm0.halted,
            vm0.cycles,
    ...
    for y in 0..32u32 {
        for x in 0..32u32 {
            ...
            if expected_on {
                assert_eq!(
                    pixel, 0xFFFFFFFF,
                    "pixel ({}, {}) should be ON (white), got 0x{:08X}",
                    x, y, pixel
                );
                on_count += 1;
            } else {
                ...
                assert_eq!(
                    pixel, 0,
                    "pixel ({}, {}) should be OFF (black), got 0x{:08X}",
                    x, y, pixel
                );
                off_count += 1;
            }
    ...
    eprintln!(
        "[SIERPINSKI] Verified: {} ON pixels, {} OFF pixels out of 1024",
        on_count, off_count
    );
    ...
    // (0,0): 0&0=0 -> ON
    assert_eq!(
        read_screen_pixel(&svm, 0, 0),
        0xFFFFFFFF,
        "(0,0) should be ON"
    );
    // (1,0): 1&0=0 -> ON (any x with y=0 is ON)
    assert_eq!(
        read_screen_pixel(&svm, 1, 0),
        0xFFFFFFFF,
        "(1,0) should be ON"
    );
    // (0,1): 0&1=0 -> ON
    assert_eq!(
        read_screen_pixel(&svm, 0, 1),
        0xFFFFFFFF,
        "(0,1) should be ON"
    );
    // (1,1): 1&1=1 -> OFF
...
fn test_sierpinski_filmstrip_dumps_png() {
    ...
    let metadata =
        std::fs::metadata("sierpinski_output.png").expect("sierpinski_output.png should exist");

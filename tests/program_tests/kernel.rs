...
fn test_syscall_dispatches_to_handler() {
    // SYSCALL 0 (EXIT) is handled inline -- reads syscall number from regs[0]
    let mut vm = Vm::new();
    vm.regs[0] = 0; // syscall number 0 = EXIT
    vm.regs[1] = 42; // exit code
    vm.ram[0] = 0x52; // SYSCALL opcode
    vm.pc = 0;

    let result = vm.step(); // execute SYSCALL 0 (EXIT)

    assert!(!result, "SYSCALL EXIT should return false (halt)");
    assert!(vm.halted, "SYSCALL EXIT should halt the VM");
    assert_eq!(vm.step_exit_code, Some(42), "exit code should be r1");
}

#[test]
fn test_syscall_no_handler_returns_error() {
    // Syscall 5 is unknown (no inline handler) -- returns ENOTSUP
    let mut vm = Vm::new();
    vm.regs[0] = 5; // syscall number 5 (unknown)
    vm.ram[0] = 0x52; // SYSCALL opcode
    vm.pc = 0;

    vm.step(); // execute SYSCALL 5

    // Should set r0 = geos_errno(ENOTSUP) and NOT jump
    assert_ne!(vm.regs[0], 0, "Unknown syscall should return error in r0");
    assert_eq!(vm.pc, 1, "PC should advance past SYSCALL opcode");
}

#[test]
fn test_retk_returns_to_user_mode() {
    // RETK pops (return_pc, saved_mode) from kernel_stack
    // Simulate what a RAM-table-based SYSCALL would have pushed
    let mut vm = Vm::new();
    vm.mode = geometry_os::vm::CpuMode::Kernel;
    vm.kernel_stack.push((2, geometry_os::vm::CpuMode::User));
...
fn test_nested_syscalls() {
    // SYSCALL 3 (GETPID) and SYSCALL 1 (BRK) are handled inline
    // Multiple syscalls in sequence via LDI to set regs[0]
    let mut vm = Vm::new();
    vm.current_pid = 7;

    // Address 0: LDI r0, 3 (GETPID)
    vm.ram[0] = 0x10;
    vm.ram[1] = 0;
    vm.ram[2] = 3;
    // Address 3: SYSCALL (reads regs[0] = 3 = GETPID)
    vm.ram[3] = 0x52;
    // Address 4: LDI r0, 1 (BRK)
    vm.ram[4] = 0x10;
    vm.ram[5] = 0;
    vm.ram[6] = 1;
    // Address 7: SYSCALL (reads regs[0] = 1 = BRK)
    vm.ram[7] = 0x52;
    // Address 8: HALT
    vm.ram[8] = 0x00;
    ...
    assert_eq!(vm.regs[0], 0, "last syscall (BRK) returns 0 in r0");
}
...
fn test_syscall_preserves_mode_for_nested_calls() {
    // Syscalls are inline -- mode is not changed by SYSCALL itself
    // SYSCALL 3 (GETPID) works in any mode, SYSCALL 1 (BRK) works in any mode
    let mut vm = Vm::new();
    vm.mode = geometry_os::vm::CpuMode::User;
    vm.current_pid = 99;

    // At 0: LDI r0, 3 (GETPID syscall number)
    vm.ram[0] = 0x10;
    vm.ram[1] = 0;
    vm.ram[2] = 3;
    // At 3: SYSCALL (GETPID)
    vm.ram[3] = 0x52;
    // At 4: LDI r0, 1 (BRK syscall number)
    vm.ram[4] = 0x10;
    vm.ram[5] = 0;
    vm.ram[6] = 1;
    // At 7: SYSCALL (BRK)
    ...
    // GETPID
    vm.step();
    assert_eq!(vm.regs[0], 99, "GETPID should return current PID");
    assert_eq!(vm.mode, geometry_os::vm::CpuMode::User, "mode unchanged");

    // LDI r0, 1
    vm.step();
    // BRK
    vm.step();
    assert_eq!(vm.regs[0], 0, "BRK returns 0");
    assert_eq!(vm.mode, geometry_os::vm::CpuMode::User, "mode unchanged");

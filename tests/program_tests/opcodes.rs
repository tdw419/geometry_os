...
fn test_unknown_opcode_error_includes_line_number() {
    ...
    assert_eq!(
        displayed, "line 2 col 1: unknown opcode: BOGUS_OPCODE",
        "full error format should be 'line N col C: unknown opcode: XYZ', got: {}",
        displayed
    );
}
...
fn test_unknown_opcode_error_on_first_line() {
    ...
    let displayed = format!("{}", err);
    assert_eq!(
        displayed, "line 1 col 1: unknown opcode: NOSUCH",
...
// ── Phase 282: Typed memory access (LOADB/STOREB/LOADH/STOREH) ──────

#[test]
fn test_byte_demo_program() {
...
#[test]
fn test_hmirror_spatial_locality() {
    let mut vm = Vm::new();

    // 1. Prepare RAM with a recognizable pattern: stripes every 256 words.
    // Order-8 Hilbert curve covers 256x256 = 65536 points.
    // We use a safe RAM region (0x4000) that won't exceed RAM_SIZE (0x10000).
    let src_addr = 0x4000;
    for i in 0..16384 {
        if i % 256 < 128 {
            vm.ram[src_addr + i] = 0xFF; // Grayscale white
        } else {
            vm.ram[src_addr + i] = 0x00; // Black
        }
    }

    // 2. Assemble and run HMIRROR
    let program = format!(
        "
        LDI r1, {}      ; src_addr
        LDI r2, 0        ; dst_x
    ...
    let asm = assemble(&program, 0).expect("Assembly failed");
    for (i, &v) in asm.pixels.iter().enumerate() {
        vm.ram[i] = v;
    }
    ...
    // 3. Verify visual output (spatial locality)
    // The first 128 pixels (d=0..127) should be white (0xFFFFFF)
    for d in 0..128 {
        let (x, y) = geometry_os::vm::hilbert::d2xy(256, d);
        let color = vm.screen[y as usize * 256 + x as usize];
        assert_eq!(
            color, 0xFFFFFF,
            "Pixel at d={} ({},{}) should be white",
            d, x, y
        );
    }

    // The next 128 pixels (d=128..255) should be black (0x000000)
    for d in 128..256 {
        let (x, y) = geometry_os::vm::hilbert::d2xy(256, d);
        let color = vm.screen[y as usize * 256 + x as usize];
        assert_eq!(
            color, 0x000000,
            "Pixel at d={} ({},{}) should be black",
            d, x, y
        );
    }
}

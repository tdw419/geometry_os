...
#[test]
fn test_cellular_automata_day_night_block_survives() {
    // Day and Night (B3678/S34678) preserves blocks since S includes 3,4 neighbors.
    let mut vm = cellular_automata_vm();
    ...
    // Switch to Day and Night rule
    vm.ram[0x3000] = 0x01C8; // birth_mask: B3678
    vm.ram[0x3001] = 0x01D8; // survival_mask: S34678
    vm.ram[0x3005] = 0xFF00FF; // magenta color
    ...
    // Block should survive under Day and Night
    assert_eq!(
        vm.screen[200 * 256 + 40],
        0xFF00FF,
        "DayAndNight: block (10,50) should survive"
    );
    assert_eq!(
        vm.screen[200 * 256 + 44],
        0xFF00FF,
        "DayAndNight: block (11,50) should survive"
    );
}
...
// ── FLAPPY BIRD (Phase 373) ────────────────────────────────────

#[test]
fn test_flappy_bird_assembles_and_runs() {
    let source = std::fs::read_to_string("programs/flappy_bird.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Run until first FRAME (init + first game loop iteration)
    let mut frames_seen = 0u32;
    for _ in 0..10_000_000 {
        if !vm.step() {
            break;
    ...
    assert!(!vm.halted, "program should not halt (infinite FRAME loop)");

    // Verify bird was initialized and is in a reasonable position
    // bird_y starts at 400 but gravity changes it each frame
    assert!(
        vm.ram[0x2000] >= 390 && vm.ram[0x2000] <= 420,
        "bird_y should be near initial 400, got {}",
        vm.ram[0x2000]
    );
    assert_eq!(vm.ram[0x2002], 0, "score should start at 0");
    assert_eq!(vm.ram[0x2003], 0, "game_over should start at 0 (playing)");
}

#[test]
fn test_flappy_bird_bird_drawn() {
    let source = std::fs::read_to_string("programs/flappy_bird.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Bird should be drawn at x=40, y=100 (bird_y=400, pixel_y=400/4=100)
    // Bird body is a 6x6 yellow rect at (40, 100)
    let bird_pixel = vm.screen[100 * 256 + 42]; // center of bird body
    assert_ne!(
        bird_pixel, 0,
        "bird should be visible on screen at (42, 100)"
    );
}

#[test]
fn test_flappy_bird_score_text() {
    let source = std::fs::read_to_string("programs/flappy_bird.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Score text buffer at 0x5000 should contain "SCR:" = [83, 67, 82, 58]
    assert_eq!(vm.ram[0x5000], 83, "score label should start with 'S'");
    assert_eq!(vm.ram[0x5001], 67, "score label should have 'C'");
    assert_eq!(vm.ram[0x5004], 48, "score digit tens should be '0'");
}

// ══ CONWAY ADVANCED (Phase 375) ════════════════════════════════════

/// Helper: assemble conway_advanced.asm and return a fresh VM ready to run.
fn conway_advanced_vm() -> Vm {
    let source = std::fs::read_to_string("programs/conway_advanced.asm")
        .expect("conway_advanced.asm not found");
    let asm = assemble(&source, 0).expect("conway_advanced.asm failed to assemble");
    let mut vm = Vm::new();
    for (i, &word) in asm.pixels.iter().enumerate() {
        if i < vm.ram.len() {
            vm.ram[i] = word;
...
#[test]
fn test_conway_advanced_assembles() {
    let source = std::fs::read_to_string("programs/conway_advanced.asm").unwrap();
    let asm = assemble(&source, 0).unwrap();
    assert!(
        asm.pixels.len() > 500,
        "conway_advanced bytecode should be >500 words, got {}",
        asm.pixels.len()
    );
}

#[test]
fn test_conway_advanced_block_survives() {
    // Block at grid (3,3) is a still life -- each cell has exactly 3 alive neighbors.
    let mut vm = conway_advanced_vm();
    let steps = step_until_frame(&mut vm, 10_000_000);
    assert!(
        vm.frame_ready,
        "should reach FRAME within 10M steps (took {})",
    ...
    let alive = 0x00FF88u32;
    // Block cells: (3,3),(4,3),(3,4),(4,4) -> screen (12,12),(16,12),(12,16),(16,16)
    assert_eq!(
        vm.screen[12 * 256 + 12],
        alive,
        "block (3,3) should survive"
    );
    assert_eq!(
        vm.screen[12 * 256 + 16],
        alive,
        "block (4,3) should survive"
    );
    assert_eq!(
        vm.screen[16 * 256 + 12],
        alive,
        "block (3,4) should survive"
    );
    assert_eq!(
        vm.screen[16 * 256 + 16],
        alive,
        "block (4,4) should survive"
    );
}

#[test]
fn test_conway_advanced_beacon_oscillates() {
    // Beacon at grid (25,3) is a period-2 oscillator.
    // Speed divisor=2: gen computed every other frame.
    let mut vm = conway_advanced_vm();
    let alive = 0x00FF88u32;

    // Frame 0: gen 1 -- inner corners die
    let steps = step_until_frame(&mut vm, 10_000_000);
    assert!(vm.frame_ready, "frame 0 within 10M steps (took {})", steps);
    assert_eq!(
        vm.screen[16 * 256 + 104],
        0,
        "beacon (26,4) should die after gen 1"
    );
    assert_eq!(
        vm.screen[20 * 256 + 108],
        0,
        "beacon (27,5) should die after gen 1"
    );

    // Frame 1: no gen (TICKS=1, speed=2)
    let steps = step_until_frame(&mut vm, 10_000_000);
    assert!(vm.frame_ready, "frame 1 within 10M steps (took {})", steps);

    // Frame 2: gen 2 -- corners restore
    let steps = step_until_frame(&mut vm, 10_000_000);
    assert!(vm.frame_ready, "frame 2 within 10M steps (took {})", steps);
    assert_eq!(
        vm.screen[16 * 256 + 104],
        alive,
        "beacon (26,4) should restore"
    );
    assert_eq!(
        vm.screen[20 * 256 + 108],
        alive,
        "beacon (27,5) should restore"
    );
}

#[test]
fn test_conway_advanced_generation_counter() {
    let mut vm = conway_advanced_vm();
    let steps = step_until_frame(&mut vm, 10_000_000);
    assert!(
        vm.frame_ready,
        "first frame within 10M steps (took {})",
        steps
    );
    assert_eq!(
        vm.ram[0x4000], 1,
        "generation should be 1 after first frame"
    );
}

#[test]
fn test_conway_advanced_mouse_toggle() {
    let mut vm = conway_advanced_vm();
    ...
    assert!(
        vm.frame_ready,
        "init frame within 10M steps (took {})",
        steps
    );
    ...
    assert!(
        vm.frame_ready,
        "mouse frame within 10M steps (took {})",
        steps
    );
    ...
    assert_eq!(
        ...
        alive,
        "clicked cell should be alive"
    );
}

#[test]
fn test_conway_advanced_speed_control() {
    let mut vm = conway_advanced_vm();
    ...
    let gen_after_init = vm.ram[0x4000];
    assert_eq!(gen_after_init, 1, "gen should be 1 after first frame");

    vm.ram[0x4004] = 1; // speed 1

    for i in 0..3 {
        let steps = step_until_frame(&mut vm, 10_000_000);
        assert!(
            vm.frame_ready,
            "frame {} within 10M steps (took {})",
            i + 1,
            steps
        );
    }

    let gen_final = vm.ram[0x4000];
    assert_eq!(
        gen_final,
        gen_after_init + 3,
        "gen should increase by 3 after 3 frames at speed 1 (got {} -> {})",
        gen_after_init,
        gen_final
    );
}
...
#[test]
fn test_conway_advanced_pattern_placement() {
    let mut vm = conway_advanced_vm();
    ...
    // Pause and set cursor
    vm.ram[0x4001] = 0;
    vm.ram[0x4002] = 50;
    vm.ram[0x4003] = 50;
    vm.push_key(55); // '7' = place block

    let steps = step_until_frame(&mut vm, 10_000_000);
    assert!(
        vm.frame_ready,
        "placement frame within 10M steps (took {})",
        steps
    );

    let alive = 0x00FF88u32;
    assert_eq!(
        vm.screen[200 * 256 + 200],
        alive,
        "placed block (50,50) should be alive"
    );
    assert_eq!(
        vm.screen[200 * 256 + 204],
        alive,
        "placed block (51,50) should be alive"
    );
    assert_eq!(
        vm.screen[204 * 256 + 200],
        alive,
        "placed block (50,51) should be alive"
    );
    assert_eq!(
        vm.screen[204 * 256 + 204],
        alive,
        "placed block (51,51) should be alive"
    );
}
...
#[test]
fn test_chess_assembles_and_runs() {
    let source = std::fs::read_to_string("programs/chess.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    assert!(
        asm.pixels.len() > 100,
        "chess should have substantial bytecode"
    );
    ...
    assert!(vm.frame_ready, "chess should produce a frame");
    assert!(!vm.halted, "chess should not halt (game loop)");
}

#[test]
fn test_chess_board_initialized() {
    let source = std::fs::read_to_string("programs/chess.asm").unwrap();
    let asm = assemble(&source, 0).unwrap();
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
        if i < vm.ram.len() {
    ...
    // Verify board at 0x2000: black rook at (0,0)=8, black king at (4,0)=12
    assert_eq!(vm.ram[0x2000], 8, "black rook at (0,0)");
    assert_eq!(vm.ram[0x2004], 12, "black king at (4,0)");
    assert_eq!(vm.ram[0x2007], 8, "black rook at (7,0)");
    // Black pawns on row 1
    assert_eq!(vm.ram[0x2008], 7, "black pawn at (0,1)");
    assert_eq!(vm.ram[0x200F], 7, "black pawn at (7,1)");
    // Empty rows 2-5
    assert_eq!(vm.ram[0x2010], 0, "empty at (0,2)");
    assert_eq!(vm.ram[0x202F], 0, "empty at (7,5)");
    // White pawns on row 6
    assert_eq!(vm.ram[0x2030], 1, "white pawn at (0,6)");
    assert_eq!(vm.ram[0x2037], 1, "white pawn at (7,6)");
    // White pieces on row 7
    assert_eq!(vm.ram[0x2038], 2, "white rook at (0,7)");
    assert_eq!(vm.ram[0x203C], 6, "white king at (4,7)");
    assert_eq!(vm.ram[0x203F], 2, "white rook at (7,7)");
}

#[test]
fn test_chess_screen_renders_board() {
    let source = std::fs::read_to_string("programs/chess.asm").unwrap();
    let asm = assemble(&source, 0).unwrap();
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Screen should not be all black -- board cells have colors
    let non_black = vm.screen.iter().filter(|&&p| p != 0).count();
    assert!(
        non_black > 1000,
        "board should have many colored pixels, got {}",
        non_black
    );

    // Check alternating square colors at known positions
    // (0,0) is a dark square (row+col=0, even=light 0xEECC88)
    let dark_sq = vm.screen[0 * 256 + 0]; // (0,0) dark
    let light_sq = vm.screen[0 * 256 + 32]; // (1,0) light
    assert_ne!(dark_sq, light_sq, "alternating squares should differ");
}

#[test]
fn test_chess_cursor_visible() {
    let source = std::fs::read_to_string("programs/chess.asm").unwrap();
    let asm = assemble(&source, 0).unwrap();
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Cursor at (4,4), white turn -> yellow (0xFFFF00)
    // Check center pixel of cell (4,4) = pixel (4*32+16, 4*32+16) = (144, 144)
    let cursor_px = vm.screen[144 * 256 + 144];
    assert_eq!(
        cursor_px, 0xFFFF00,
        "cursor cell should be yellow for white turn"
    );
}

#[test]
fn test_chess_white_pawn_forward() {
    let source = std::fs::read_to_string("programs/chess.asm").unwrap();
    let asm = assemble(&source, 0).unwrap();
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Simulate selecting white pawn at (4,6) -- move cursor to (4,6) then press space
    // Move state: set sel_col=4, sel_row=6 (select)
    vm.ram[0x2102] = 4; // sel_col
    vm.ram[0x2103] = 6; // sel_row

    // Set cursor to (4,5) -- destination
    vm.ram[0x2100] = 4; // cursor_col
    vm.ram[0x2101] = 5; // cursor_row

    // Simulate space press to trigger handle_select -> sel_phase1 -> validate -> execute
    // Set the "newly pressed" bits (bit 4 = space)
    // NOTE: 0xFFB is intercepted by LOAD to read from vm.key_bitmask field, not RAM
    vm.ram[0x2110] = 0; // prev_bitmask = 0
    vm.key_bitmask = 16; // current bitmask = space (field, not RAM)

    // Run a few frames to process
    vm.frame_ready = false;
    vm.halted = false;
    for _ in 0..10_000_000 {
        if !vm.step() {
    ...
    // White pawn should have moved from (4,6) to (4,5)
    assert_eq!(vm.ram[0x2034], 0, "source (4,6) should be empty after move"); // 0x2000 + 6*8 + 4 = 0x2034
    assert_eq!(vm.ram[0x202C], 1, "dest (4,5) should have white pawn"); // 0x2000 + 5*8 + 4 = 0x202C
}

#[test]
fn test_chess_invalid_move_blocked() {
    let source = std::fs::read_to_string("programs/chess.asm").unwrap();
    let asm = assemble(&source, 0).unwrap();
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Try moving white pawn backward (4,6) -> (4,7) -- invalid
    vm.ram[0x2102] = 4; // sel_col
    vm.ram[0x2103] = 6; // sel_row
    vm.ram[0x2100] = 4; // cursor_col
    vm.ram[0x2101] = 7; // cursor_row (backward)
    vm.ram[0x2110] = 0;
    vm.key_bitmask = 16; // space press (field, not RAM -- 0xFFB is intercepted)
    ...
    assert_eq!(vm.ram[0x2034], 1, "white pawn should still be at (4,6)"); // 0x2000 + 6*8 + 4
}

// ── SOLAR SYSTEM ───────────────────────────────────────────────

#[test]
fn test_solar_system_assembles() {
    let source = std::fs::read_to_string("programs/solar_system.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    assert!(
        asm.pixels.len() > 100,
        "solar_system should have substantial bytecode"
    );
}

#[test]
fn test_solar_system_renders_frame() {
    let source = std::fs::read_to_string("programs/solar_system.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    assert!(frames_seen >= 2, "should produce at least 2 frames");

    // Check sine table was built (entries at 0x3000-0x30FF)
    // Entry 0 should be ~127 (sin(0) = 0, stored as 0+127 = 127)
    let sin0 = vm.ram[0x3000];
    assert!(
        sin0 > 120 && sin0 < 135,
        "sin table entry 0 should be ~127, got {}",
        sin0
    );

    // Entry 64 should be ~254 (sin(90deg) = 127, stored as 127+127 = 254)
    let sin64 = vm.ram[0x3040]; // 0x3000 + 64
    assert!(
        sin64 > 240 && sin64 <= 254,
        "sin table entry 64 (peak) should be ~254, got {}",
        sin64
    );

    // Check planets exist in memory at 0x3500+
    // Sun distance should be 0
    assert_eq!(vm.ram[0x3500], 0, "Sun distance should be 0");
    // Sun radius should be 6
    assert_eq!(vm.ram[0x3503], 6, "Sun radius should be 6");

    // Mercury distance should be 25
    assert_eq!(vm.ram[0x3506], 25, "Mercury distance should be 25");
}

#[test]
fn test_solar_system_orbital_motion() {
    let source = std::fs::read_to_string("programs/solar_system.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Record Mercury's angle after frame 1
    let angle_after_1 = vm.ram[0x3507]; // Mercury angle at 0x3506 + 1
    ...
    // Record Mercury's angle after frame 2
    let angle_after_2 = vm.ram[0x3507];

    // Mercury should have moved (speed * time_speed = 8 * 2 = 16 per frame)
    let angle_diff = if angle_after_2 >= angle_after_1 {
        angle_after_2 - angle_after_1
    } else {
        (256 - angle_after_1) + angle_after_2
    };
    assert!(
        angle_diff > 0,
        "Mercury angle should advance each frame (got {} -> {})",
        angle_after_1,
        angle_after_2
    );
}
...
#[test]
fn test_solar_system_time_controls() {
    let source = std::fs::read_to_string("programs/solar_system.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Check default time_speed = 2
    assert_eq!(vm.ram[0x3404], 2, "default time_speed should be 2");

    // Simulate pressing "down" key (bit1 of key bitmask) to slow down
    vm.key_bitmask = 2; // bit1 = down

    // Run one more frame with key pressed
    for _ in 0..10_000_000 {
        if !vm.step() {
            break;
        }
    ...
    // Time speed should have decreased from 2 to 1
    let speed = vm.ram[0x3404];
    assert_eq!(
        speed, 1,
        "time_speed should decrease to 1 when down pressed, got {}",
        speed
    );

    // Simulate pressing "up" key (bit0) to speed up
    vm.key_bitmask = 1; // bit0 = up
    ...
    let speed2 = vm.ram[0x3404];
    assert_eq!(
        speed2, 2,
        "time_speed should increase to 2 when up pressed, got {}",
        speed2
    );
}

#[test]
fn test_solar_system_screen_has_content() {
    let source = std::fs::read_to_string("programs/solar_system.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Screen should have non-zero pixels (sun, stars, planets)
    let non_zero = vm.screen.iter().filter(|&&p| p != 0).count();
    assert!(
        non_zero > 50,
        "screen should have significant content, got {} non-zero pixels",
        non_zero
    );

    // Check for yellow sun pixels near center
    // Sun is at (128, 128) by default
    let center_idx = 128 * 256 + 128;
    let sun_color = vm.screen[center_idx];
    assert!(
        sun_color != 0,
        "sun center pixel should be non-zero, got {:x}",
        sun_color
    );
}

#[test]
fn test_brians_brain_assembles_and_runs() {
    let source = std::fs::read_to_string("programs/brians_brain.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Run up to 10M steps or until a frame is rendered
    let mut frames = 0;
    for _ in 0..10_000_000 {
        if !vm.step() {
            break;
    ...
    assert!(
        frames >= 1,
        "Brian's Brain should render at least 1 frame, got {}",
        frames
    );
    ...
    assert!(
        non_black > 100,
        "screen should have many non-black pixels after {} frames, got {}",
        frames,
        non_black
    );
}
...
#[test]
fn test_day_night_assembles_and_runs() {
    let mut vm = super::compile_run("programs/day_night.asm");
    // Run up to 500k steps
    for _ in 0..500_000 {
        if !vm.step() {
            break;
        }
    ...
    let non_black = vm.screen.iter().filter(|&&p| p != 0).count();
    println!("Day/Night simulation: {} non-black pixels", non_black);
    assert!(
        non_black > 0,
        "Expected non-black pixels in Day/Night simulation, got 0"
    );
}

#[test]
fn test_digital_clock_assembles_and_runs() {
    let source = std::fs::read_to_string("programs/digital_clock.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Run until at least 1 frame renders
    let mut frames = 0;
    for _ in 0..5_000_000 {
        if !vm.step() {
            break;
    ...
    assert!(
        frames >= 1,
        "Digital clock should render at least 1 frame, got {}",
        frames
    );

    // Verify green pixels exist (segment rendering)
    let green_pixels = vm
        .screen
        .iter()
        .filter(|&&p| (p & 0x00FF00) != 0 && (p & 0xFF0000) == 0 && (p & 0x0000FF) == 0)
        .count();
    assert!(
        green_pixels > 10,
        "Should have green segment pixels, got {}",
        green_pixels
    );
}
...
#[test]
fn test_racing_assembles_and_runs() {
    let source = std::fs::read_to_string("programs/racing.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    for (i, &pixel) in asm.pixels.iter().enumerate() {
    ...
    // Run until at least 1 frame renders (game loop never halts)
    let mut frames = 0;
    for _ in 0..5_000_000 {
        if !vm.step() {
            break;
    ...
    assert!(
        frames >= 1,
        "Racing game should render at least 1 frame, got {}",
        frames
    );

    // Verify track was rendered (red + green pixels exist)
    let red_pixels = vm
        .screen
        .iter()
        .filter(|&&p| (p & 0xFF0000) != 0 && (p & 0x00FF00) == 0)
        .count();
    let green_pixels = vm
        .screen
        .iter()
        .filter(|&&p| (p & 0x00FF00) != 0 && (p & 0xFF0000) == 0)
        .count();
    assert!(
        red_pixels > 100,
        "Should have red wall pixels on track, got {}",
        red_pixels
    );
    assert!(
        green_pixels > 100,
        "Should have green track surface pixels, got {}",
        green_pixels
    );

    // Verify car data structures initialized
    assert!(
        vm.ram[0x3000] > 0 || vm.ram[0x3001] > 0,
        "Car should have non-zero position"
    );
    assert_eq!(vm.ram[0x300A], 3, "Should have 3 total laps configured");
}

#[test]
fn test_racing_collision_detection() {
    // Test that PEEK-based collision works:
    // Create a VM, place car off-track (red wall area), run a step,
    // verify game_state transitions to "crashed" (1).
    let source = std::fs::read_to_string("programs/racing.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Override car position to be OFF the track (in the red wall area at x=12, y=12)
    vm.ram[0x3000] = 12; // car_x in outer wall
    vm.ram[0x3001] = 12; // car_y in outer wall

    // Run steps to complete init and first game loop iteration
    // The first frame draws the track and checks collision
    let mut frames = 0;
    for _ in 0..5_000_000 {
        if !vm.step() {
            break;
    ...
    assert!(frames >= 1, "Should render at least 1 frame");
}

#[test]
fn test_racing_on_track_passes_no_crash() {
    // Test that the car on valid track does NOT crash
    let source = std::fs::read_to_string("programs/racing.asm")
        .unwrap_or_else(|e| panic!("failed to read: {}", e));
    let asm = assemble(&source, 0).unwrap_or_else(|e| panic!("assembly failed: {:?}", e));
    let mut vm = Vm::new();
    ...
    // Car starts at (128, 35) which is on the track - should not crash
    let mut frames = 0;
    for _ in 0..5_000_000 {
        if !vm.step() {
            break;
    ...
    assert_eq!(
        vm.ram[0x3008], 0,
        "Car on track should not crash (game_state should be 0, playing)"
    );
}

...
#[test]
fn test_window_manager_runs_and_produces_frames() {
    // Run for 3 frames: window manager should run without halting
    let vm = compile_run_multiproc("programs/window_manager.asm", 3);
    assert!(!vm.halted, "window manager should not halt");
    // Screen should have non-black pixels (desktop + taskbar + windows)
    let non_black = vm.screen.iter().filter(|&&p| p != 0).count();
    assert!(
        non_black > 100,
        "window manager should draw pixels (got {})",
        non_black
    );
}

#[test]
fn test_window_manager_draws_green_sysmon() {
    // Run for 5 frames and check that green sysmon pixels exist
    let vm = compile_run_multiproc("programs/window_manager.asm", 5);
    let green = 0x44DD44u32;
    let green_count = vm.screen.iter().filter(|&&p| p == green).count();
    assert!(
        green_count > 0,
        "sysmon green pixels should be visible (got {})",
        green_count
    );
}

#[test]
fn test_window_manager_sysmon_text_visible() {
    // Run for 10 frames; sysmon draws green text inside its window
    let vm = compile_run_multiproc("programs/window_manager.asm", 10);
    let green = 0x44DD44u32;
    let screen = &vm.screen;
    let green_pixels: Vec<(usize, usize)> = (0..256usize)
        .flat_map(|y| {
            (0..256usize).filter_map(move |x| {
                if screen[y * 256 + x] == green {
                    Some((x, y))
                } else {
                    None
                }
        ...
        .collect();
    assert!(
        !green_pixels.is_empty(),
        "green sysmon text should be visible on screen"
    );
}

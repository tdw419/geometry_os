...
#[test]
fn test_tictactoe_assembles() {
    // Smoke test: tictactoe.asm must assemble without errors
    let source =
        std::fs::read_to_string("programs/tictactoe.asm").expect("tictactoe.asm not found");
    let asm = assemble(&source, 0x1000).expect("tictactoe.asm failed to assemble");
    assert!(
        asm.pixels.len() > 100,
        "tictactoe should be more than 100 words"
    );
}

...
fn test_dungeon_renders() {
    ...
    assert!(explored > 10, "visibility should have >10 explored tiles, got {}", explored);
}
...
fn test_dungeon_player_marker() {
    ...
    assert!(player >= 4, "player marker should be >=4 green pixels, got {}", player);
}
...
fn test_dungeon_four_rooms_bsp() {
    ...
    for i in 0..4u32 {
        ...
        assert_eq!(vm.ram[0x2000 + tile_idx], 1,
            "room {} center tile ({}, {}) should be floor", i, center_x, center_y);
    }
    
    // Total floor tiles from 4 rooms + corridors
    let floor = (0..1024).filter(|&i| vm.ram[0x2000 + i] == 1).count();
    assert!(floor > 30, "4 rooms + corridors should yield >30 floor tiles, got {}", floor);
}

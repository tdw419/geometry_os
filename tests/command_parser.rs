...
fn cls_clears_all_output_lines() {
    ...
    assert!(shell.output_lines().is_empty(), "CLS should clear all output lines");
...
fn print_renders_text_to_output() {
    ...
    assert_eq!(actions, vec![ShellAction::Print { text: "Hello".to_string() }]);
...
fn print_with_spaces_preserves_spaces() {
    ...
    assert_eq!(actions, vec![ShellAction::Print { text: "Hello World".to_string() }]);
...
fn print_empty_string_produces_empty_action() {
    ...
    assert_eq!(actions, vec![ShellAction::Print { text: String::new() }]);
}
...
fn run_invokes_loader_with_program_name() {
    ...
    assert_eq!(actions, vec![ShellAction::Run { filename: "myprogram.gasm".to_string() }]);
}
...
fn run_appends_gasm_extension_if_missing() {
    ...
    assert_eq!(actions, vec![ShellAction::Run { filename: "hello.gasm".to_string() }]);
}
...
fn run_without_argument_produces_error_no_action() {
    ...
    assert!(actions.is_empty(), "RUN without args should produce no ShellAction");
    assert!(shell.output_lines().iter().any(|l| l.contains("RUN requires a program name")));
}
...
fn run_program_with_path_segments() {
    ...
    assert_eq!(actions, vec![ShellAction::Run { filename: "programs/demo.gasm".to_string() }]);
}
...
fn help_contains_all_command_names() {
    ...
    assert!(help_text.contains("PRINT"), "HELP output should mention PRINT");
    assert!(help_text.contains("RUN"), "HELP output should mention RUN");
    assert!(help_text.contains("HELP"), "HELP output should mention HELP");
}
...
fn help_does_not_produce_actions() {
    ...
    assert!(actions.is_empty(), "HELP should not produce any ShellAction");
}
...
fn help_output_is_multi_line() {
    ...
    assert!(added >= 3, "HELP should produce multiple output lines, got {} new lines", added);
}
...
fn unknown_command_produces_error_message() {
    ...
    assert!(actions.is_empty(), "Unknown command should produce no actions");
    assert!(shell.output_lines().iter().any(|l| l.contains("Unknown command")));
}
...
fn unknown_command_does_not_clear_existing_output() {
    ...
    assert!(shell.output_lines().iter().any(|l| l == "important_data"),
...
fn partial_prefix_cl_is_rejected() {
    ...
    assert!(actions.is_empty(), "Partial command 'CL' should not be recognized");
    assert!(shell.output_lines().iter().any(|l| l.contains("Unknown command")));
}
...
fn help_shows_descriptions_not_just_names() {
    ...
    assert!(text.contains("Clear") || text.contains("clear"), "HELP should describe CLS");
    assert!(text.contains("Print") || text.contains("print") || text.contains("Display"), "HELP should describe PRINT");
    assert!(text.contains("Load") || text.contains("load") || text.contains("Run"), "HELP should describe RUN");
}

...
fn test_signal_invalid_pid() {
    ...
    assert_eq!(
        vm.regs[0],
        geometry_os::vm::geos_errno(geometry_os::vm::GEOS_ESRCH),
        "invalid PID should return ESRCH"
    );
}
...
fn test_signal_invalid_signal_number() {
    ...
    assert_eq!(
        vm.regs[0],
        geometry_os::vm::geos_errno(geometry_os::vm::GEOS_ESRCH),
        "invalid signal should return ESRCH"
    );
}
...
fn test_sigset_main_process_fails() {
    ...
    assert_eq!(
        vm.regs[0],
        geometry_os::vm::geos_errno(geometry_os::vm::GEOS_EPERM),
        "SIGSET should fail for main process with EPERM"
    );
}

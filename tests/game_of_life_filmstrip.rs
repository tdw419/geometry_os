...
fn test_gol_filmstrip_assembles() {
    ...
    let (programs, labels) =
        gasm::assemble_filmstrip_with_labels(&source).expect("game_of_life.gasm should assemble");
    ...
    eprintln!(
        "[GOL] Frame sizes: Seed={}, Step={}, Done={}",
    ...
    assert!(
        programs[0].pixels.len() > 20,
        "Seed frame should have instructions"
    );
    assert!(
        programs[1].pixels.len() > 50,
        "Step frame should be the largest"
    );
    assert!(
...
fn test_gol_blinker_one_generation() {
    ...
    for i in 0..max_dispatches {
        ...
        if vm0.halted == 1 || vm0.state == 2 {
            eprintln!(
                "[GOL] Halted after {} dispatches, frame_ptr={}",
        ...
        let vm0 = svm.vm_state(0);
        eprintln!(
            "[GOL] Dispatch {}: state={} halted={} cycles={} frame_ptr={} r0={} r1={} r15(gen)={}",
            i + 1,
            vm0.state,
            vm0.halted,
            vm0.cycles,
    ...
    let expected_alive: Vec<(u32, u32)> = vec![(10, 9), (10, 10), (10, 11)];

    for &(x, y) in &expected_alive {
        assert!(
            is_alive(&svm, x, y),
            "Blinker: pixel ({}, {}) should be alive after 1 gen",
            x,
            y
        );
    }
    ...
    let expected_dead: Vec<(u32, u32)> = vec![(9, 10), (11, 10)];

    for &(x, y) in &expected_dead {
        assert!(
            !is_alive(&svm, x, y),
            "Blinker: pixel ({}, {}) should be dead after 1 gen",
            x,
            y
        );
    }
    ...
    assert_eq!(
        alive_count, 3,
        "Should have exactly 3 alive cells (blinker), got {}",
        alive_count
    );

    eprintln!(
        "[GOL] Verified: {} alive cells after 1 generation",
        alive_count
    );
}
...
fn test_gol_dumps_png() {
    ...
    let metadata = std::fs::metadata("gol_output.png").expect("gol_output.png should exist");

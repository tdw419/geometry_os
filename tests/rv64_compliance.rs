...
use geometry_os::rv64::executor::Rv64Cpu;
...
fn test_rv64_add() {
    ...
    cpu.x[2] = 200;
    // ADD x3, x1, x2
    load_program(&mut cpu, &[encode_r_type(0, 2, 1, 0, 3, 0x33)]);
    cpu.step();
...
fn test_rv64_sub() {
    ...
    cpu.x[2] = 50;
    // SUB x3, x1, x2
    load_program(&mut cpu, &[encode_r_type(0x20, 2, 1, 0, 3, 0x33)]);
    cpu.step();
...
fn test_rv64_lui() {
    let mut cpu = Rv64Cpu::new();
    // LUI x1, 0x12345
    load_program(&mut cpu, &[encode_u_type(0x12345, 1, 0x37)]);
    cpu.step();
...
fn test_rv64_auipc() {
    ...
    let base = 0x8000_0000u64;
    // AUIPC x1, 0x100
    load_program(&mut cpu, &[encode_u_type(0x100, 1, 0x17)]);
    cpu.pc = base;
    cpu.step();
...
fn test_rv64_jal() {
    ...
    load_program(&mut cpu, &[encode_j_type(8, 1, 0x6F)]);
    cpu.pc = base;
    cpu.step();
...
fn test_rv64_jalr() {
    ...
    cpu.x[2] = base + 0x100;
    // JALR x1, x2, 0
    load_program(&mut cpu, &[encode_i_type(0, 2, 0, 1, 0x67)]);
    cpu.pc = base;
    cpu.step();
...
fn test_rv64_beq_taken() {
    ...
    cpu.x[2] = 42;
    // BEQ x1, x2, +8
    load_program(&mut cpu, &[encode_b_type(8, 2, 1, 0, 0x63)]);
    cpu.pc = base;
    cpu.step();
...
fn test_rv64_blt() {
    ...
    cpu.x[2] = 20;
    // BLT x1, x2, +8
    load_program(&mut cpu, &[encode_b_type(8, 2, 1, 4, 0x63)]);
    cpu.pc = base;
    cpu.step();
...
fn test_rv64_slt_sltu() {
    ...
    cpu.x[2] = 1;
    // SLT x3, x1, x2 => -1 < 1 => 1
    load_program(&mut cpu, &[encode_r_type(0, 2, 1, 2, 3, 0x33)]);
    cpu.step();
...
fn test_rv64_addiw() {
    ...
    load_program(&mut cpu, &[encode_i_type(5, 1, 0, 2, 0x1B)]);
    cpu.step();
...
fn test_rv64_addw_subw() {
    ...
    load_program(&mut cpu, &[encode_r_type(0, 2, 1, 0, 3, 0x3B)]);
    cpu.step();
    ...
    load_program(&mut cpu2, &[encode_r_type(0x20, 2, 1, 0, 3, 0x3B)]);
    cpu2.step();
...
fn test_rv64_mul_div() {
    ...
    cpu.x[2] = 6;
    // MUL x3, x1, x2
    load_program(&mut cpu, &[encode_r_type(1, 2, 1, 0, 3, 0x33)]);
    cpu.step();
...
fn test_rv64_x0_hardwired() {
    ...
    load_program(&mut cpu, &[encode_i_type(42, 0, 0, 0, 0x13)]);
    cpu.step();
    assert_eq!(cpu.x[0], 0);
}
...
fn test_rv64_or_and_xor() {
    ...
    cpu.x[2] = 0x0FF0;
    // OR x3, x1, x2
    load_program(&mut cpu, &[encode_r_type(0, 2, 1, 6, 3, 0x33)]);
    cpu.step();
    ...
    cpu2.x[1] = 0xFF00;
    cpu2.x[2] = 0x0FF0;
    load_program(&mut cpu2, &[encode_r_type(0, 2, 1, 7, 3, 0x33)]);
    cpu2.step();
    ...
    cpu3.x[1] = 0xFF00;
    cpu3.x[2] = 0x0FF0;
    load_program(&mut cpu3, &[encode_r_type(0, 2, 1, 4, 3, 0x33)]);
    cpu3.step();

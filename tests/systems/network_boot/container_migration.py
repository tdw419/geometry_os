...
class CheckpointManager:
    ...
    async def cancel_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        if not state:
            return False
        if state.status not in MigrationStatus.PENDING:
            return False
...
class TransferManager:
    ...
    def __init__(
        self,
        source_client=None,  # RemoteBootClient
        target_client=None,  # RemoteBootClient
    ...
    async def transfer_checkpoint(
        ...
        checkpoint.elapsed_time = checkpoint.end_time - checkpoint.start_time

        logger.info(f"Transfer complete: {checkpoint.bytes_transferred} bytes in {checkpoint.elapsed_time:.2f}s")
        return True
    ...
    async def _send_chunk(
        ...
        """Send chunk to target peer via remote boot client."""
        # In real implementation, this would use the remote boot client's
        # streaming protocol
        await asyncio.sleep(0.01)
        return True
...
class RestoreManager:
    ...
    async def restore_checkpoint(
        ...
        if not chunks:
            ...
            state.error = f"No chunks received for {checkpoint_id}"
            return state
        ...
        if self.boot_manager:
            ...
            try:
                ...
                if boot_result.success:
                    ...
                    state.status = MigrationStatus.FAILED
                    state.error = boot_result.error or "Boot failed"
                    return state
            except Exception as e:
                state.status = MigrationStatus.FAILED
                state.error = str(e)
                logger.error(f"Failed to boot restored container: {e}")
                return state
        else:
...
class MigrationCoordinator:
    ...
    async def migrate(
        ...
        try:
            ...
            # Phase 4: Reconnect network (if configured)
            ...
            # Phase 6: Cleanup source (if configured)

...
static GPU: LazyLock<Mutex<GlyphVm>> = LazyLock::new(|| Mutex::new(GlyphVm::new()));
...
fn assert_vm_match(sw: &VmState, gpu: &VmState, label: &str) {
    ...
    assert_eq!(
        sw.pc, gpu.pc,
        "{label}: pc mismatch: sw={}, gpu={}",
        sw.pc, gpu.pc
    );
    assert_eq!(
        sw.cycles, gpu.cycles,
        "{label}: cycles mismatch: sw={}, gpu={}",
        sw.cycles, gpu.cycles
    );
...
fn sv_char_blit() {
    ...
    svm.spawn_vm(0, 0);
    svm.execute_frame();
    let h_rows = geometry_os::font_atlas::get_char_rows(72);
    for row in 0..8 {
        assert_eq!(
...
fn gpu_blit() {
    ...
    for i in 0..5u32 {
        assert_eq!(
            gpu.substrate().peek(600 + i),
            0xB000 + i,
            "GPU BLIT pixel {} mismatch",
            i
        );
    }
}
...
fn vm_state_field_offsets() {
    ...
    assert_eq!(
        &vm.halted as *const u32 as usize - base,
        516,
        "halted offset"
    );
    assert_eq!(
        &vm.stratum as *const u32 as usize - base,
        520,
        "stratum offset"
    );
    assert_eq!(
        &vm.cycles as *const u32 as usize - base,
        524,
        "cycles offset"
    );
    assert_eq!(
        &vm.stack_ptr as *const u32 as usize - base,
        528,
        "stack_ptr offset"
    );
    ...
    assert_eq!(
        &vm.entry_point as *const u32 as usize - base,
        544,
        "entry_point offset"
    );
    assert_eq!(
        &vm.stack as *const [u32; 64] as usize - base,
        576,
        "stack offset"
    );
}

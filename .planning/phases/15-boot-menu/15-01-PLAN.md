...
must_haves:
  truths:
    - "iPXE client can fetch boot script from /pxe/boot.ipxe"
    - "Boot script fetches dynamic menu from /pxe/menu.ipxe"
    - "Menu script contains valid iPXE menu commands"
  artifacts:
...
- Missing metadata: display "Unknown" for distro/architecture
  ...
  <done>GET /pxe/menu.ipxe returns dynamic iPXE menu script listing all PXE-enabled containers</done>
</task>
...
3. Fetch /pxe/menu.ipxe - should list containers as menu items
...
- Menu script includes all PXE-enabled containers

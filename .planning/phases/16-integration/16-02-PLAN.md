...
must_haves:
  truths:
    ...
    - "PXE container boot uses existing RemoteBootFetcher (INT-01)"
  artifacts:
  ...
  key_links:
    ...
    - from: "DesktopObjectManager._startRemoteDownload"
      to: "RemoteBootFetcher"
      via: "PXE HTTP server URL"
      pattern: "RemoteBootFetcher.*fetchWithRetry"
---
...
# INT-01: RemoteBootFetcher reuse
...
<task type="auto">
  ...
  <action>
    ...
    **INT-01 Note:** The existing `_startRemoteDownload()` method (~line 1062) already uses RemoteBootFetcher for streaming downloads. When booting a PXE-enabled container, the same flow applies - just pass the PXE HTTP server URL (e.g., `http://localhost:8080/pxe/{entryId}/download`) instead of the catalog server URL. RemoteBootFetcher handles:
    - Streaming download with progress tracking
    - Automatic retry with exponential backoff
    - Cancellation support via AbortController
    - Hash verification (if hash available in PXE metadata)
  </action>
  <verify>
...
- PXE boot reuses RemoteBootFetcher (same flow, different URL source)
</verification>
...
- PXE container boot uses existing RemoteBootFetcher (INT-01 satisfied)
</success_criteria>

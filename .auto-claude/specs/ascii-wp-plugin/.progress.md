...
- [x] 2.2 Create settings page template - feat(wp): add settings page template
- [x] 2.3 Create logs page template - feat(wp): add logs page template
...
- Settings page template created with WordPress Settings API pattern (settings_fields, do_settings_sections)
- Admin templates stored in admin/ subdirectory
- Logs page uses WordPress list table structure (wp-list-table widefat fixed striped classes)
- Filter form includes status dropdown, date range pickers, search field
- Expandable row details implemented via tr.main-row + log-details-row pattern
- ASCII snapshot section supports before/after display with dark theme (#1e1e1e bg)
- Pagination follows WordPress tablenav pattern with first/prev/next/last links
- Inline JS placeholder for ASCIILogs module, actual implementation in control.js

...
- **Clobbers**: documented per-function; all functions preserve r10–r25
...
.include "lib/stdio.asm"    ; screen I/O (requires stdlib.asm first)
.include "lib/heap.asm"     ; advanced heap (no deps)
```
...
| `strncpy`  | `r1=dst, r2=src, r3=n` → `r0=dst` | Copy at most N chars, null-pad |
...
| `memchr` | `r1=addr, r2=count, r3=value` → `r0=addr` | Find first match, or 0 |
...
| `atoi`   | `r1=addr` → `r0=value` | Decimal string → unsigned int |
...
| `lerp`        | `r1=a, r2=b, r3=t, r4=scale` → `r0` | Linear interpolation: a + (b-a)*t/scale |
...
| `_lib_heap_alloc`    | `r1=words` → `r0=addr` | Allocate with [size][0xDEAD][data...] header |
| `_lib_heap_free`     | `r1=addr` | No-op (bump allocator) |
...
```
load programs/lib_test.asm
assemble
run
```
Results stored in RAM[0xF80..0xFA7]: 1=PASS, 0=FAIL per slot.

### Stdlib visual test (screen output)
```
load programs/stdlib_test.asm
assemble
run
```

See `programs/lib_test.asm` for the full test suite covering all library functions
with deterministic RAM-based result verification.

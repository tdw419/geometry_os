...
class WASMCodeVisualizer:
    ...
    def visualize(self, data: bytes) -> np.ndarray:
        ...
        For code-mode symmetry with decode_rgba(), this method encodes each
        4-byte chunk into 2 pixels to enable lossless round-trip decoding:
        - Pixel 1: R=entropy(byte[0]), G=byte[0], B=byte[1], A=exec_mask
        - Pixel 2: R=entropy(byte[0]), G=byte[2], B=byte[3], A=exec_mask

        This allows decode_rgba() to recover all 4 bytes from G and B channels.
        ...
        total_chunks = total_bytes // 4

        # We use 2 pixels per 4-byte chunk for lossless encoding
        total_pixels = total_chunks * 2
        rgba_pixels = np.zeros((total_pixels, 4), dtype=np.uint8)

        # Process each 4-byte chunk as 2 pixels
        for i in range(total_chunks):
            byte_idx = i * 4

            # Get bytes for this chunk
            byte0 = data_array[byte_idx]
            byte1 = data_array[byte_idx + 1] if byte_idx + 1 < total_bytes else 0
            byte2 = data_array[byte_idx + 2] if byte_idx + 2 < total_bytes else 0
            byte3 = data_array[byte_idx + 3] if byte_idx + 3 < total_bytes else 0

            # Get entropy for this position
            ent = entropy[byte_idx]

            # Determine if this is executable code
            is_executable = (byte0 in self.CONTROL_FLOW_OPCODES or
            ...
            alpha = 255 if is_executable else 0

            # Pixel 1: R=entropy, G=byte[0], B=byte[1], A=exec_mask
            rgba_pixels[i * 2] = [ent, byte0, byte1, alpha]

            # Pixel 2: R=entropy, G=byte[2], B=byte[3], A=exec_mask
            rgba_pixels[i * 2 + 1] = [ent, byte2, byte3, alpha]
    ...
    def decode_rgba(self, rgba_pixels: np.ndarray, expected_size: int) -> bytes:
        ...
        This is the inverse of visualize() - recovers original data from
        semantic RGBA coloring. Each pair of pixels decodes to 4 bytes:
        - Pixel 1: G=byte[0], B=byte[1]
        - Pixel 2: G=byte[2], B=byte[3]

        Args:
            rgba_pixels: RGBA pixel array as numpy array (N, 4)
            expected_size: Expected number of bytes to recover
        ...
        Example:
            ...
            >>> decoded == wasm_bytes  # Should match exactly
        """
        # Reshape if needed
        if rgba_pixels.ndim == 3:
            # Shape is (height, width, 4) - flatten to (N, 4)
            height, width, _ = rgba_pixels.shape
            rgba_pixels = rgba_pixels.reshape(-1, 4)

        decoded_bytes = bytearray()

        # Process pixels in pairs (each pair = 4 bytes of original data)
        num_pixels = len(rgba_pixels)
        for i in range(0, num_pixels - 1, 2):  # Step by 2 to process pairs
            pixel1 = rgba_pixels[i]
            pixel2 = rgba_pixels[i + 1]

            # Extract from pixel 1: G=byte[0], B=byte[1]
            decoded_bytes.append(pixel1[1])  # Green
            if len(decoded_bytes) < expected_size:
                decoded_bytes.append(pixel1[2])  # Blue

            # Extract from pixel 2: G=byte[2], B=byte[3]
            if len(decoded_bytes) < expected_size:
                decoded_bytes.append(pixel2[1])  # Green
            if len(decoded_bytes) < expected_size:
                decoded_bytes.append(pixel2[2])  # Blue

            # Stop when we have enough bytes
            if len(decoded_bytes) >= expected_size:
                break

        # Trim to expected size
        result = bytes(decoded_bytes[:expected_size])

        # Verify WASM magic number
        if not self.is_wasm(result):
            raise ValueError(
                f"Decoded data is not valid WASM (missing magic number \\x00asm). "
                f"Got: {result[:8].hex()!r}"
            )

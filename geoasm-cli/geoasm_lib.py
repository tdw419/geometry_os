...
class GeoASMVM:
    ...
    def step(self):
        ...
        # Glyph Primitives (Screen is the Hard Drive architecture)
            ...
            glyph_id = dst
            # Transform matrix stored at memory[matrix_ptr] (src1 | src2 << 8)
            ...
            src_count = src1
            src_ptr = src2 << 8
            # Read source glyph IDs from memory (up to src_count)
            _ = [self.memory[src_ptr + i] for i in range(min(src_count, 8))]
            ...
            # Border style: 0=none, 1=solid, 2=rounded, 3=shadow (dst=panel_id, src1=style)
            pass

...
// ── Phase 204: Navigation Audio Hints ──────────────────────────

/// Speak navigation instructions using espeak-ng in a background thread.
///
/// Each instruction is spoken sequentially with a brief pause between them.
/// Non-blocking: returns immediately, speech plays in background.
/// Silently fails if espeak-ng is not installed.
///
/// # Arguments
/// * `instructions` - A slice of instruction strings (e.g. "Turn north, 10 steps to Library")
pub fn speak_navigation(instructions: &[String]) {
    if instructions.is_empty() {
        return;
    }

    let lines: Vec<String> = instructions.to_vec();
    std::thread::spawn(move || {
        for (i, line) in lines.iter().enumerate() {
            ...
            let _ = std::process::Command::new("espeak-ng")
                .args([
                    "-a", "100", // amplitude 100%
                    "-s", "130", // speaking speed (words per minute)
                    "-p", "50", // pitch
                    "-g", "10", // word gap (10ms)
                    "--", line,
                ])
...
/// Play a short chime to signal navigation arrival or waypoint.
/// Uses a high-pitched bell-like tone (1000Hz, 100ms).
pub fn play_navigation_waypoint_chime() {
    play_beep(1000, 100);
}

/// Play a destination-arrival chime (two ascending notes).
pub fn play_navigation_arrival_chime() {
    play_beep(880, 150);
    std::thread::sleep(std::time::Duration::from_millis(100));
    play_beep(1320, 200);
}

/// Send a desktop notification about a navigation event using notify-send.
/// Silently fails if notify-send is not available.
pub fn notify_navigation(title: &str, body: &str) {
    let _ = std::process::Command::new("notify-send")
        .args(["-a", "Geometry OS", title, body])
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null())
        .status();
}
...
mod tests {
    ...
    // ── Phase 204: Navigation Audio Tests ──

    #[test]
    fn test_speak_navigation_empty() {
        speak_navigation(&[]);
        // Should return immediately without spawning thread work
    }

    #[test]
    fn test_speak_navigation_with_instructions() {
        let inst = vec![
            "Turn north".to_string(),
            "Walk 10 steps to Library".to_string(),
        ];
        speak_navigation(&inst);
        // Should spawn background thread without panicking
        // Give the thread a moment to start
        std::thread::sleep(std::time::Duration::from_millis(50));
    }

    #[test]
    fn test_notify_navigation_no_panic() {
        notify_navigation("Test", "Test notification body");
        // Should not panic even if notify-send isn't available
    }

    #[test]
    fn test_navigation_chimes_no_panic() {
        play_navigation_waypoint_chime();
        std::thread::sleep(std::time::Duration::from_millis(50));
        play_navigation_arrival_chime();

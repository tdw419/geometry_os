...
pub fn canvas_assemble(
    ...
    // Use assemble_multi() for multi-error collection and column info
    match assembler::assemble_multi(&preprocessed_source, CANVAS_BYTECODE_ADDR, None) {
        Ok(asm_result) => {
            ...
            *canvas_assembled = true;
            ...
            vm.halted = false;
            // Phase 435: Store assembler labels for label-aware disassembly
            vm.populate_assembler_labels(&asm_result.labels, CANVAS_BYTECODE_ADDR as u32);
            // Include warnings if any
            if !asm_result.warnings.is_empty() {
                let warn_count = asm_result.warnings.len();
                *status_msg = format!(
                    "[OK: {} bytes at 0x{:04X} ({} warning{})]",
        ...
        Err((errors, warnings)) => {
            ...
            // Show first error with column info if available
            let first = &errors[0];
            let first_detail = match first.column {
                Some(col) => format!("line {} col {}: {}", first.line, col, first.message),
                None => format!("line {}: {}", first.line, first.message),
            };
            let warn_detail = if !warnings.is_empty() {
                format!(
                    " ({} warning{})",
                    warnings.len(),
    ...
    match command.as_str() {
        "help" | "?" => {
            ...
            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
                "  agents [cmd]      Manage autonomous Area Agents",
            );
            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
                "  jump <x> <y>      Jump camera to world pixel (x,y)",
            );
            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
                "  look <x> <y>      Blit world (x,y) to framebuffer",
            );
            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
            ...
            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
                "  vfs[vfsfiles]     List VFS files with sizes",
            );
            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
        ...
        "vfsfiles" | "vfs" => {
            let vfs_dir = std::path::Path::new(crate::vfs::FS_DIR);
            let mut entries = Vec::new();
            if let Ok(rd) = std::fs::read_dir(vfs_dir) {
            ...
            if entries.is_empty() {
                *output_row = write_line_to_canvas(
                ...
                for (name, size) in &entries {
                    ...
                    let line = format!("  {:<32} {}", name, size_str);
                    // Truncate to canvas width (32 chars for name)
                    let display = if line.len() > 40 {
                        format!("{}... {}", &name[..29.min(name.len())], size_str)
                    } else {
                        line
                    };
                    *output_row = write_line_to_canvas(canvas_buffer, *output_row, &display);
                    total_size += size;
                }
                let total_str = if total_size < 1024 {
                    format!("{} B total", total_size)
                } else if total_size < 1024 * 1024 {
                    format!("{:.1} KB total", total_size as f64 / 1024.0)
                } else {
                    format!("{:.1} MB total", total_size as f64 / (1024.0 * 1024.0))
                };
                *output_row = write_line_to_canvas(
                    canvas_buffer,
                    *output_row,
                    &format!("  {} files, {}", entries.len(), total_str),
                );
            }
            *output_row = write_line_to_canvas(canvas_buffer, *output_row, "geo> ");
            ensure_scroll(*output_row, scroll_offset);
        ...
        "run" => {
            ...
            // Use assemble_multi() for multi-error collection and column info
            match assembler::assemble_multi(&source, CANVAS_BYTECODE_ADDR, None) {
                Ok(asm_result) => {
                    let ram_len = vm.ram.len();
                ...
                Err((errors, warnings)) => {
                    ...
                    let warn_part = if !warnings.is_empty() {
                        format!(
                            " ({} warning{})",
                            warnings.len(),
                            if warnings.len() == 1 { "" } else { "s" }
        ...
        "jump" => {
            if parts.len() < 3 {
                *output_row =
                    write_line_to_canvas(canvas_buffer, *output_row, "Usage: jump <x> <y>");
                *output_row = write_line_to_canvas(canvas_buffer, *output_row, "geo> ");
                ensure_scroll(*output_row, scroll_offset);
                return (None, false, false);
            }
            ...
            // Camera in RAM[0x7800-0x7801] is in tile coords (8px per tile)
            vm.ram[0x7800] = (x / 8) as u32;
            vm.ram[0x7801] = (y / 8) as u32;

            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
                &format!("Jumped camera to world ({}, {})", x, y),
            );
            *output_row = write_line_to_canvas(canvas_buffer, *output_row, "geo> ");
            ensure_scroll(*output_row, scroll_offset);
            (None, false, false)
        ...
        "look" => {
            if parts.len() < 3 {
                *output_row =
                    write_line_to_canvas(canvas_buffer, *output_row, "Usage: look <x> <y>");
                *output_row = write_line_to_canvas(canvas_buffer, *output_row, "geo> ");
                ensure_scroll(*output_row, scroll_offset);
                return (None, false, false);
            }
            ...
            // 2. Blit region to screen (RAM[0x10000])
            let screen_range = 0x10000..0x20000;
            vm.tile_store
                .blit_to_screen(&mut vm.ram[screen_range], x, y, 0, 0, 256, 256);

            *output_row = write_line_to_canvas(
                canvas_buffer,
                *output_row,
                &format!("Blitted world ({}, {}) to framebuffer", x, y),
            );
            *output_row = write_line_to_canvas(canvas_buffer, *output_row, "geo> ");
            ensure_scroll(*output_row, scroll_offset);
            (None, false, false)
        ...
        "agents" => {
            if parts.len() < 2 || parts[1] == "list" || parts[1] == "ls" {
                *output_row =
                    write_line_to_canvas(canvas_buffer, *output_row, "Active Area Agents:");
                for (i, agent) in vm.area_manager.agents.iter().enumerate() {
                    ...
                    let role_name = match agent.role {
                        1 => "COMPOSITOR",
                        2 => "SHELL",
                        3 => "NEURAL",
                        4 => "GRAPHICS",
                    ...
                    let line = format!(
                        "  [{}] Role: {:<10} | Sector: ({},{}) {}x{} | Status: {} | Cycles: {}",
                        i,
                        role_name,
                        agent.home_x,
                        agent.home_y,
                ...
                "assign" => {
                    if parts.len() < 7 {
                        *output_row = write_line_to_canvas(
                            canvas_buffer,
                            *output_row,
                            "Usage: agents assign <role_id> <x> <y> <w> <h>",
                        );
                        *output_row = write_line_to_canvas(canvas_buffer, *output_row, "Roles: 0:MAINT, 1:COMP, 2:SHELL, 3:NEURAL, 4:GPU, 5:INPUT, 6:SVC, 7:DEV");
                    } else {
                        let role = parts[2].parse::<u32>().unwrap_or(0);
                        let x = parts[3].parse::<i32>().unwrap_or(0);
                        let y = parts[4].parse::<i32>().unwrap_or(0);
                        let w = parts[5].parse::<u32>().unwrap_or(1024);
                        let h = parts[6].parse::<u32>().unwrap_or(1024);
                        if let Err(e) = vm.area_manager.assign_sector(role, x, y, w, h) {
                            *output_row = write_line_to_canvas(
                                canvas_buffer,
                                *output_row,
                                &format!("Error: {}", e),
                            ...
                            *output_row = write_line_to_canvas(
                                canvas_buffer,
                                *output_row,
                                &format!("Agent assigned as role {} to region ({},{})", role, x, y),
                            );
                        }
                    }
                },
                "auto" => {
                    vm.area_manager.auto_assign_grid(0, 0, 2048, 2048, 1024);
                    *output_row = write_line_to_canvas(
                        canvas_buffer,
                        *output_row,
                        "Auto-assigned 2x2 agent grid (2048x2048 total)",
                    );
                },
                "start" => {
                    vm.area_manager.start();
                    *output_row =
                        write_line_to_canvas(canvas_buffer, *output_row, "All agents started.");
                },
                "stop" => {
                    vm.area_manager.stop();
                    *output_row =
                        write_line_to_canvas(canvas_buffer, *output_row, "All agents stopped.");
                },
                _ => {
                    *output_row = write_line_to_canvas(
                        canvas_buffer,
                        *output_row,
                        &format!("Unknown agents command: {}", parts[1]),
                    );
                },
            }

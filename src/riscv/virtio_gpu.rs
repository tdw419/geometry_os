...
impl VirtioGpu {
   ...
   /// Read a 32-bit register from the VirtIO MMIO transport.
       ...
       match offset {
           ...
           QUEUE_NUM => {
               ...
               if (value as usize) < MAX_QUEUES {
                   ...
                   q.size = if value > max && value != 0 { max } else { value };
               }
               None
           }
           QUEUE_READY => {
           ...
           for i in 0..count {
               ...
               if offset + std::mem::size_of::<VirtioGpuMemEntry>() <= data.len() {
                   let entry: VirtioGpuMemEntry =
                       parse_struct(&data[offset..offset + std::mem::size_of::<VirtioGpuMemEntry>()]);
   ...
   pub fn blit_to_framebuffer(&mut self, resource: &GpuResource, scanout: &GpuScanout) {
       ...
       for row in 0..copy_h {
           for col in 0..copy_w {
               ...
               let pixel_bytes = &resource.data
                   [src_offset..src_offset + (BYTES_PER_PIXEL as usize)];
...
mod tests {
   ...
   fn new_virtio_gpu_defaults() {
       ...
       assert_eq!(gpu.framebuffer.len(), (DEFAULT_WIDTH * DEFAULT_HEIGHT) as usize);

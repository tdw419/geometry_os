...
pub struct RiscvCpu {
    ...
    pub x: [u32; 32],
    /// Float registers f0-f31 (64-bit, D extension).
    pub f: [u64; 32],
    /// Program counter.
    pub pc: u32,
...
impl RiscvCpu {
    ...
    pub(crate) fn write_csr(&mut self, addr: u32, val: u32, bus: &mut Bus) {
        if addr == csr::SATP {
            ...
            // SATP PPN fixup: Linux writes Linux-physical PPNs, but our guest RAM
            // starts at ram_base (0x80000000). Convert: any PPN -> linux-physical,
            // then add ram_base_ppn to get guest-physical PPN.
            //
            // Must be idempotent: the kernel may read SATP (which has our fixed PPN)
            // and write it back. Un-fix first, then re-fix.
            let mut fixed_val = val;
            let page_offset_ppn: u32 = 0xC000_0000 >> 12; // 0xC0000
            let ram_base_ppn: u32 = (bus.mem.ram_base >> 12) as u32; // 0x80000
            let ppn = val & 0x003F_FFFF;
            // Step 1: Convert to Linux-physical PPN (un-fix any prior fixup)
            let linux_phys_ppn = if ppn >= page_offset_ppn {
                ppn - page_offset_ppn // Virtual -> Linux-physical
            } else if ppn >= ram_base_ppn {
                ppn - ram_base_ppn // Guest-physical -> Linux-physical (un-fix)
            } else {
                ppn // Already Linux-physical
            };
            // Step 2: Convert to guest-physical PPN
            let guest_phys_ppn = linux_phys_ppn + ram_base_ppn;
            fixed_val = (val & !0x003F_FFFF) | guest_phys_ppn;
    ...
    pub(crate) fn deliver_trap(&mut self, cause: u32, tval: u32) {
        // Log non-timer-interrupt traps for debugging
        let is_interrupt = (cause & csr::MCAUSE_INTERRUPT_BIT) != 0;
        let code = cause & !csr::MCAUSE_INTERRUPT_BIT;
        if !is_interrupt || (code != csr::INT_STI && code != csr::INT_MTI) {
            // Debug tracing suppressed — fires on every page fault during normal operation
        }
    ...
    pub fn step(&mut self, bus: &mut Bus) -> StepResult {
        // Phase 20-25: Record instruction fetch in spatial heatmap
        bus.record_fetch(self.pc);

        let tracing_on = bus.sbi.tracing_enabled;
        ...
        if tracing_on && tp_before != tp_after && self.privilege != Privilege::User {

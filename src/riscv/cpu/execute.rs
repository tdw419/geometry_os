...
impl RiscvCpu {
    ...
    pub(crate) fn execute(&mut self, op: Operation, bus: &mut Bus, inst_len: u32) -> StepResult {
        ...
        match op {
            ...
            | Operation::Lbu { .. }
            | Operation::Lhu { .. }
            | Operation::Fld { .. } => self.execute_load(op, bus, next_pc),

            // ---- Stores (delegate to loadstore.rs) ----
            Operation::Sb { .. }
            | Operation::Sh { .. }
            | Operation::Sw { .. }
            | Operation::Fsd { .. } => self.execute_store(op, bus, next_pc),
            ...
            // ---- D extension register moves ----
            Operation::FmvDX { rd, rs1 } => {
                // FMV.D.X: copy integer register to float register (64-bit zero-extended)
                self.f[rd as usize] = self.x[rs1 as usize] as u64;
                self.pc = next_pc;
                StepResult::Ok
            },
            Operation::FmvXD { rd, rs1 } => {
                // FMV.X.D: copy lower 32 bits of float register to integer register
                self.set_reg(rd, self.f[rs1 as usize] as u32);
                self.pc = next_pc;
                StepResult::Ok
            },
        }

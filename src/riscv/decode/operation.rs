...
pub enum Operation {
    ...
    /// FLD: load double-precision (64-bit) from memory to f register
    Fld {
        rd: u8,  // f register destination
        rs1: u8, // integer register base
        imm: i32,
    },
    /// FSD: store double-precision (64-bit) from f register to memory
    Fsd {
        rs1: u8, // integer register base
        rs2: u8, // f register source
        imm: i32,
    },
    /// FMV.D.X: move integer register to f register (64-bit)
    FmvDX {
        rd: u8,  // f register destination
        rs1: u8, // integer register source
    },
    /// FMV.X.D: move f register to integer register (lower 32 bits)
    FmvXD {
        rd: u8,  // integer register destination
        rs1: u8, // f register source
    },

...
pub fn decode_c(halfword: u16) -> Operation {
    ...
    match bits01 {
        ...
        0b00 => match funct3 {
            ...
            // C.FLD: load double from rs1' + offset into f[8+rd']
            // RV32 D-extension: valid on RV32 when D extension is enabled
            0b001 => {
                let rd_p = ((w >> 2) & 0x7) as u8;
                let rs1_p = ((w >> 7) & 0x7) as u8;
                let rd = crd(rd_p); // f[8+rd_p]
                let rs1 = crd(rs1_p);
                // uimm[5:3|7:6|2]: bits[12:10] = offset[5:3], bits[9:8] = offset[7:6], bit[6] = offset[2]
                let imm = ((((w >> 10) & 0x7) as i32) << 3)  // bits[12:10] -> offset[5:3]
                    | ((((w >> 8) & 0x3) as i32) << 6)       // bits[9:8] -> offset[7:6]
                    | ((((w >> 6) & 0x1) as i32) << 2); // bit[6] -> offset[2]
            ...
            // C.LD (funct3=011): on RV32, this is C.FLW (float load word)
            // Not implemented - not used by our kernel
            ...
            // C.FSD: store f[8+rs2'] to rs1' + offset
            // RV32 D-extension: valid on RV32 when D extension is enabled
            0b101 => {
                let rs2_p = ((w >> 2) & 0x7) as u8;
                let rs1_p = ((w >> 7) & 0x7) as u8;
                let rs2 = crd(rs2_p); // f[8+rs2_p]
                let rs1 = crd(rs1_p);
                // Same offset encoding as C.FLD
                let imm = ((((w >> 10) & 0x7) as i32) << 3)  // bits[12:10] -> offset[5:3]
                    | ((((w >> 8) & 0x3) as i32) << 6)       // bits[9:8] -> offset[7:6]
                    | ((((w >> 6) & 0x1) as i32) << 2); // bit[6] -> offset[2]
                Operation::Fsd { rs1, rs2, imm }
            },
            // C.SD (funct3=111): on RV32, this is C.FSW (float store word)
            // Not implemented - not used by our kernel

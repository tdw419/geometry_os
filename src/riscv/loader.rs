...
pub fn load_elf_with_offset(
    ...
    for i in 0..hdr.phnum {
        ...
        let target_pa = if phdr.p_paddr as u64 >= offset {
            phdr.p_paddr as u64
        } else {
            offset + phdr.p_paddr as u64
        };
        ...
        let seg_end = target_pa + phdr.p_memsz.max(phdr.p_filesz) as u64;
...
pub fn load_auto(bus: &mut Bus, image: &[u8], default_base: u64) -> Result<LoadInfo, LoadError> {
    load_auto_with_offset(bus, image, default_base)
}

/// Detect image format and load accordingly with a physical offset.
///
/// If ELF, uses p_paddr + offset.
/// If raw, loads at offset (base_addr).
pub fn load_auto_with_offset(
    bus: &mut Bus,
    image: &[u8],
    offset: u64,
) -> Result<LoadInfo, LoadError> {
    if image.len() >= 4 {
        let magic = u32::from_le_bytes([image[0], image[1], image[2], image[3]]);
        if magic == ELF_MAGIC {
            return load_elf_with_offset(bus, image, offset);
        }
    }
    load_raw(bus, image, offset)
}

...
impl VirtioBlk {
    ...
    pub fn read_sectors(&self, sector: u64, count: u32) -> Option<Vec<u8>> {
        eprintln!("[virtio-blk-trace] READ_SECTORS: sector={}, count={}", sector, count);
        let start = (sector * SECTOR_SIZE) as usize;
    ...
    pub fn write_sectors(&mut self, sector: u64, data: &[u8]) -> bool {
        eprintln!("[virtio-blk-trace] WRITE_SECTORS: sector={}, len={}", sector, data.len());
        if self.read_only {
            return false;
        }
        let start = (sector * SECTOR_SIZE) as usize;
...
/// Bridge: VirtioBlk implements BlockDevice so GSFS can mount the VirtIO disk directly.
impl BlockDevice for VirtioBlk {
    fn read_blocks(&mut self, start_block: u64, count: u32) -> Option<Vec<u8>> {
        self.read_sectors(start_block, count)
    }
    fn write_blocks(&mut self, start_block: u64, data: &[u8]) -> bool {
        self.write_sectors(start_block, data)
    }
...
mod tests {
    ...
    fn config_capacity() {
        ...
        assert_eq!(capacity, dev.disk.len() as u64 / SECTOR_SIZE);
    }

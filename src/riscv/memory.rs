...
impl GuestMemory {
    ...
    pub fn write_byte(&mut self, addr: u64, val: u8) -> Result<(), MemoryError> {
        if addr == 0x80400FFE || addr == 0x80400FFF {
            eprintln!("[mem-watch] WRITE to 0x{:08X} val=0x{:02X}", addr, val);
    ...
    pub fn write_half(&mut self, addr: u64, val: u16) -> Result<(), MemoryError> {
        if addr <= 0x80400FFE && addr + 2 > 0x80400FFE {
            eprintln!("[mem-watch] WRITE_HALF to 0x{:08X} val=0x{:04X}", addr, val);
            let _ = std::io::stderr().flush();
        }
        let off = self.offset(addr, 2)?;
    ...
    pub fn write_word(&mut self, addr: u64, val: u32) -> Result<(), MemoryError> {
        if addr <= 0x80400FFE && addr + 4 > 0x80400FFE {
            eprintln!("[mem-watch] WRITE_WORD to 0x{:08X} val=0x{:08X}", addr, val);
            let _ = std::io::stderr().flush();
        }
        let off = self.offset(addr, 4)?;
        self.ram[off] = (val & 0xFF) as u8;
    ...
    /// Return a full snapshot of RAM.
    pub fn ram_snapshot(&self) -> Vec<u8> {
        self.ram.clone()
    }

    /// Restore RAM from a full snapshot. Returns false if length mismatches.
    pub fn ram_restore(&mut self, data: &[u8]) -> bool {

...
/// Compute FNV-1a hash of a rectangular region of the screen.
pub fn region_checksum(screen: &[u32], x: usize, y: usize, w: usize, h: usize) -> u32 {
    let mut hash: u32 = 0x811C9DC5;
    for ry in y..(y + h).min(256) {
        for rx in x..(x + w).min(256) {
            let idx = ry * 256 + rx;
            if idx < screen.len() {
                hash ^= screen[idx];

...
fn get_tool_list() -> Vec<serde_json::Value> {
    vec![
        ...
        tool(
            "vm_checkpoint",
            "Create a checkpoint snapshot of current VM state. Returns filename and checkpoint details.",
            vec![param("description", "string", "Optional description for the checkpoint", false)],
            vm_checkpoint_schema(),
        ),
        tool(
            "vm_restore",
            "Restore VM state from a checkpoint file.",
            vec![param("filename", "string", "Checkpoint filename (e.g. checkpoint_0123456789abcdef.json)", true)],
            vm_restore_schema(),
        ),
        tool(
            "vm_list_checkpoints",
            ...
            vec![],
            vm_list_checkpoints_schema(),
        ),
        tool(
            "vm_delete_checkpoint",
            "Delete a checkpoint file.",
            vec![param("filename", "string", "Checkpoint filename to delete", true)],
            vm_delete_checkpoint_schema(),
        ),
        tool(
            "vm_diff_checkpoints",
            "Diff two checkpoints and return structured comparison.",
            vec![
                param("filename_a", "string", "First checkpoint filename", true),
                param("filename_b", "string", "Second checkpoint filename", true),
            ],
            vm_diff_checkpoints_schema(),
        ),
        // -- Phase 202: Building Metadata Tools --
        tool(
            "building_set_metadata",
            "Set metadata key-value pair on a building",
            vec![
                param("building_id", "integer", "Building ID (from RAM 0x7500 table)", true),
                param("key", "string", "Metadata key (e.g., 'description', 'tags')", true),
                param("value", "string", "Metadata value", true),
            ],
            building_set_metadata_schema(),
        ),
        tool(
            "building_get_metadata",
            "Get metadata from a building (all or specific key)",
            vec![
                param("building_id", "integer", "Building ID (from RAM 0x7500 table)", true),
                param("key", "string", "Optional key filter (omit to get all metadata)", false),
            ],
            building_get_metadata_schema(),
        ),
        tool(
            "building_search_metadata",
            "Search buildings by metadata content",
            vec![
                param("query", "string", "Search query (matches against keys and values)", true),
            ],
            building_search_metadata_schema(),
        ),
        tool(
            "coordinate_set_metadata",
            "Set metadata key-value pair at a specific coordinate (spatial annotation)",
            vec![
                param("x", "integer", "X coordinate", true),
                param("y", "integer", "Y coordinate", true),
                param("key", "string", "Metadata key", true),
                param("value", "string", "Metadata value", true),
            ],
            coordinate_set_metadata_schema(),
        ),
        tool(
            "coordinate_get_metadata",
            "Get metadata at a specific coordinate",
            vec![
                param("x", "integer", "X coordinate", true),
                param("y", "integer", "Y coordinate", true),
                param("key", "string", "Optional key filter", false),
            ],
            coordinate_get_metadata_schema(),
        ),
        // -- Phase 203+: Binary Memory Palace Extension --
        tool(
            "vm_binary_import",
            "Import binary file into building metadata with specified format and encoding",
            vec![
                param("path", "string", "Path to binary file", true),
                param("building_id", "integer", "Building ID (from RAM 0x7500 table)", true),
                param("format", "string", "Format: elf, bios_rom, disk_image, raw", false),
                param("encoding", "string", "Encoding: base64, hex, pixels", false),
            ],
            vm_binary_import_schema(),
        ),
        tool(
            "vm_binary_export",
            "Export binary from building to file",
            vec![
                param("building_id", "integer", "Building ID to export from", true),
                param("path", "string", "Output file path", true),
            ],
            vm_binary_export_schema(),
        ),
        tool(
            "vm_binary_inject",
            "Inject building binary directly to VM RAM at specified address",
            vec![
                param("building_id", "integer", "Building ID containing binary", true),
                param("ram_addr", "string", "RAM address (hex, e.g., 0x80000000)", true),
            ],
            vm_binary_inject_schema(),
        ),
        tool(
            "vm_coord_set_binary",
            "Store binary at coordinate as metadata",
            vec![
                param("x", "integer", "X coordinate", true),
                param("y", "integer", "Y coordinate", true),
                param("format", "string", "Format: elf, bios_rom, disk_image, raw", true),
                param("path", "string", "Path to binary file", true),
            ],
            coord_set_binary_schema(),
        ),
        tool(
            "vm_coord_get_binary",
            "Get binary metadata at coordinate",
            vec![
                param("x", "integer", "X coordinate", true),
                param("y", "integer", "Y coordinate", true),
                param("key", "string", "Metadata key", true),
            ],
            coord_get_binary_schema(),
        ),
        tool(
            "vm_binary_chunk_add",
            "Add binary chunk to building (for large file chunking)",
            vec![
                param("building_id", "integer", "Building ID", true),
                param("offset", "integer", "File offset in bytes", true),
                param("path", "string", "Path to binary chunk file", true),
            ],
            binary_chunk_add_schema(),
        ),
        tool(
            "vm_binary_chunk_list",
            "List binary chunks for a building",
            vec![
                param("building_id", "integer", "Building ID", true),
            ],
            binary_chunk_list_schema(),
        ),
        ...
        tool(
            "riscv_input",
            "Send text input to the running RISC-V VM's UART (each character becomes a key event, appends newline)",
            vec![param("text", "string", "Text to send to the RISC-V VM", true)],
            riscv_input_schema(),
        ),
        tool(
            "riscv_console",
        ...
        // -- Phase 203: Memory Palace Cross-Platform Export/Import --
        tool(
            "vm_memory_palace_export",
            "Export Memory Palace to JSON file",
            vec![
                param("path", "string", "Output JSON file path", false),
                param("include_canvas", "boolean", "Also export canvas as PNG", false),
            ],
            vm_memory_palace_export_schema(),
        ),
        tool(
            "vm_memory_palace_import",
            "Import Memory Palace from JSON file",
            vec![
                param("path", "string", "Input JSON file path", true),
                param("strategy", "string", "Merge strategy: preserve, rename, overwrite", false),
            ],
            vm_memory_palace_import_schema(),
        ),
        ...
        tool(
            "vision_describe",
            "Take a screenshot and describe it using the configured vision model. Returns a text description of what's on screen.",
            vec![param(
                "prompt",
                "string",
        ...
        // -- RAM Write Tools (host -> VM data injection) --
        tool(
            "vm_poke",
            "Write a single u32 word to VM RAM. Address and value in hex or decimal. Invalidates icache at written address.",
            vec![
                param("address", "string", "RAM address (hex with 0x prefix or decimal)", true),
                param("value", "string", "u32 value to write (hex with 0x prefix or decimal)", true),
            ],
            vm_poke_schema(),
        ),
        tool(
            "vm_poke_bulk",
            "Write multiple u32 words to VM RAM starting at address. Values space-separated in hex or decimal.",
            vec![
                param("address", "string", "Start RAM address (hex with 0x prefix or decimal)", true),
                param("values", "string", "Space-separated u32 values to write (hex or decimal)", true),
            ],
            vm_poke_bulk_schema(),
        ),
    ...
    for p in params {
        if let Some(obj) = p.as_object() {
            ...
            if !pname.is_empty() {
                ...
                let mut prop_obj = obj.clone();
                prop_obj.remove("name");
                prop_obj.remove("required");
                properties.insert(pname.to_string(), serde_json::Value::Object(prop_obj));
            }
        }
    }
...
fn vm_checkpoint_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "filename": {"type": "string"}, "checkpoint": {"type": "object", "properties": {"timestamp": {"type": "integer"}, "description": {"type": "string"}, "pc": {"type": "string"}, "ram_size": {"type": "integer"}, "contexts_count": {"type": "integer"}}}}})
}
fn vm_restore_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "checkpoint": {"type": "object"}, "error": {"type": "string"}}})
}
fn vm_list_checkpoints_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"checkpoints": {"type": "array", "items": {"type": "string"}}}})
}
fn vm_delete_checkpoint_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "error": {"type": "string"}}})
}
fn vm_diff_checkpoints_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"diff": {"type": "object"}}})
}
// -- Phase 202: Building Metadata Schemas --
fn building_set_metadata_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn building_get_metadata_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"building_id": {"type": "integer"}, "metadata": {"type": "object"}, "key": {"type": "string"}, "value": {"type": "string"}}})
}
fn building_search_metadata_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"query": {"type": "string"}, "matches": {"type": "array", "items": {"type": "integer"}}}})
}
fn coordinate_set_metadata_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn coordinate_get_metadata_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"x": {"type": "integer"}, "y": {"type": "integer"}, "metadata": {"type": "object"}, "key": {"type": "string"}, "value": {"type": "string"}}})
}

// -- Phase 203+: Binary Memory Palace Extension Schemas --
fn vm_binary_import_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn vm_binary_export_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn vm_binary_inject_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn coord_set_binary_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn coord_get_binary_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"x": {"type": "integer"}, "y": {"type": "integer"}, "key": {"type": "string"}, "value": {"type": "string"}}})
}
fn binary_chunk_add_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "response": {"type": "string"}}})
}
fn binary_chunk_list_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"building_id": {"type": "integer"}, "chunks": {"type": "array", "items": {"type": "string"}}}})
}

// -- Phase 84: Building & Desktop Schemas --
...
fn riscv_input_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"ok": {"type": "boolean"}, "sent": {"type": "integer"}}})
}
fn riscv_console_schema() -> serde_json::Value {
    serde_json::json!({"type": "object", "properties": {"output": {"type": "string"}, "bytes": {"type": "integer"}}})
}
...
// Phase 203: Memory Palace Export/Import schemas
fn vm_memory_palace_export_schema() -> serde_json::Value {
    serde_json::json!({
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Output file path"},
            "include_canvas": {"type": "boolean", "description": "Whether canvas PNG was exported"},
            "status": {"type": "string", "description": "Operation status"}
        }
    })
}

fn vm_memory_palace_import_schema() -> serde_json::Value {
    serde_json::json!({
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Input file path"},
            "strategy": {"type": "string", "description": "Merge strategy used"},
            "status": {"type": "string", "description": "Operation status"}
        }
    })
}
...
fn vm_poke_schema() -> serde_json::Value {
    serde_json::json!({
        "type": "object",
        "properties": {
            "ok": {"type": "boolean"},
...
fn vm_poke_bulk_schema() -> serde_json::Value {
    serde_json::json!({
        "type": "object",
        "properties": {
            "ok": {"type": "boolean"},
...
fn handle_tool_call(name: &str, args: &serde_json::Value) -> Result<serde_json::Value, String> {
    match name {
        ...
        "riscv_input" => {
            ...
            let resp = send_socket_cmd(&format!("riscv_input {}", text))?;
            Ok(serde_json::json!({
                "ok": resp.contains("sent"),
                "sent": text.len(),
                "response": resp,
            }))
        },
        "riscv_console" => {
            let resp = send_socket_cmd("riscv_console")?;
            Ok(serde_json::json!({
                "output": resp.trim(),
                "bytes": resp.trim().len(),
            }))
        ...
        // Phase 203: Memory Palace Export/Import tools
        "vm_memory_palace_export" => {
            let path = args
                .get("path")
                .and_then(|v| v.as_str())
                .unwrap_or("memory_palace_export.json");
            let include_canvas = args
                .get("include_canvas")
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            let canvas_flag = if include_canvas { "1" } else { "0" };
            let cmd = format!("export_memory_palace {} {}", path, canvas_flag);
            let resp = send_socket_cmd(&cmd)?;
            Ok(serde_json::json!({
                "path": path,
        ...
        "vm_memory_palace_import" => {
            ...
            let strategy = args
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("preserve");
            let cmd = format!("import_memory_palace {} {}", path, strategy);
            let resp = send_socket_cmd(&cmd)?;
            Ok(serde_json::json!({
                "path": path,
                "strategy": strategy,
        ...
        "vision_describe" => {
            ...
            if resp.starts_with("error:") {
                ...
                Ok(serde_json::json!({
                    "description": resp,
                    "model": "Geometry OS Vision Bridge",
                }))
            }
        },
        ...
        "vcc_validate" => {
            ...
            let w = args["w"].as_i64().unwrap_or(256);
            let h = args["h"].as_i64().unwrap_or(256);
            let expected = args["expected_checksum"]
                .as_str()
                .ok_or("Missing 'expected_checksum' parameter")?;

            let resp = send_socket_cmd(&format!("region_checksum {} {} {} {}", x, y, w, h))?;
            let actual = resp.trim();
        ...
        // ── RAM Write Handlers (host -> VM data injection) ──────
        "vm_poke" => {
            let addr = args["address"].as_str().unwrap_or("0");
            let val = args["value"].as_str().unwrap_or("0");
            let cmd = format!("poke {} {}", addr, val);
            let resp = send_socket_cmd(&cmd)?;
        ...
        "vm_poke_bulk" => {
            let addr = args["address"].as_str().unwrap_or("0");
            let vals = args["values"].as_str().unwrap_or("");
            let cmd = format!("poke_bulk {} {}", addr, vals);
            let resp = send_socket_cmd(&cmd)?;
            let ok = resp.starts_with("ok");
            Ok(serde_json::json!({
                "ok": ok,
        ...
        // Phase 201: Checkpointing tools
        "vm_checkpoint" => {
            let desc = args.get("description").and_then(|v| v.as_str());
            let cmd = if let Some(d) = desc {
                format!("checkpoint {}", d)
            } else {
        ...
        "vm_restore" => {
            ...
            let resp = send_socket_cmd(&format!("restore_checkpoint {}", filename))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "vm_list_checkpoints" => {
            let resp = send_socket_cmd("list_checkpoints")?;
            let files: Vec<String> = resp.lines().map(|s| s.to_string()).collect();
            Ok(serde_json::json!({ "checkpoints": files }))
        },
        "vm_delete_checkpoint" => {
            let filename = args["filename"]
                .as_str()
                .ok_or("Missing 'filename' parameter")?;
            let resp = send_socket_cmd(&format!("delete_checkpoint {}", filename))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "vm_diff_checkpoints" => {
            let filename_a = args["filename_a"]
                .as_str()
                .ok_or("Missing 'filename_a' parameter")?;
            let filename_b = args["filename_b"]
                .as_str()
                .ok_or("Missing 'filename_b' parameter")?;
            let resp = send_socket_cmd(&format!("diff_checkpoints {} {}", filename_a, filename_b))?;
            Ok(serde_json::json!({ "diff": resp }))
        },
        // -- Phase 202: Building Metadata --
        "building_set_metadata" => {
            let building_id = args["building_id"]
                .as_u64()
                .ok_or("Missing 'building_id' parameter")? as u32;
            let key = args["key"].as_str().ok_or("Missing 'key' parameter")?;
            let value = args["value"].as_str().ok_or("Missing 'value' parameter")?;
            let resp = send_socket_cmd(&format!("set_metadata {} {} {}", building_id, key, value))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "building_get_metadata" => {
            let building_id = args["building_id"]
                .as_u64()
                .ok_or("Missing 'building_id' parameter")? as u32;
            if let Some(key_filter) = args["key"].as_str() {
                let resp =
                    send_socket_cmd(&format!("get_metadata {} {}", building_id, key_filter))?;
                Ok(
                    serde_json::json!({ "building_id": building_id, "key": key_filter, "value": resp.trim().to_string() }),
                )
            } else {
                let resp = send_socket_cmd(&format!("get_metadata {}", building_id))?;
                ...
                Ok(serde_json::json!({ "building_id": building_id, "metadata": metadata_map }))
            }
        },
        "building_search_metadata" => {
            let query = args["query"].as_str().ok_or("Missing 'query' parameter")?;
            let resp = send_socket_cmd(&format!("search_metadata {}", query))?;
            let building_ids: Vec<u32> = resp
                .lines()
                .filter_map(|s| s.trim().parse::<u32>().ok())
                .collect();
            Ok(serde_json::json!({ "query": query, "matches": building_ids }))
        },
        "coordinate_set_metadata" => {
            let x = args["x"].as_u64().ok_or("Missing 'x' parameter")? as u32;
            let y = args["y"].as_u64().ok_or("Missing 'y' parameter")? as u32;
            let key = args["key"].as_str().ok_or("Missing 'key' parameter")?;
            let value = args["value"].as_str().ok_or("Missing 'value' parameter")?;
            let resp =
                send_socket_cmd(&format!("set_coord_metadata {} {} {} {}", x, y, key, value))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "coordinate_get_metadata" => {
            let x = args["x"].as_u64().ok_or("Missing 'x' parameter")? as u32;
            ...
            if let Some(key_filter) = args["key"].as_str() {
                let resp =
                    send_socket_cmd(&format!("get_coord_metadata {} {} {}", x, y, key_filter))?;
                Ok(
                    serde_json::json!({ "x": x, "y": y, "key": key_filter, "value": resp.trim().to_string() }),
                )
            } else {
                let resp = send_socket_cmd(&format!("get_coord_metadata {} {}", x, y))?;
                let mut metadata_map = serde_json::Map::new();
                for line in resp.lines() {
                    if let Some((k, v)) = line.split_once(": ") {
                        metadata_map
                ...
                Ok(serde_json::json!({ "x": x, "y": y, "metadata": metadata_map }))
            }
        },
        // -- Phase 203+: Binary Memory Palace Extension --
        "vm_binary_import" => {
            let file_path = args["path"].as_str().ok_or("Missing 'path' parameter")?;
            let building_id = args["building_id"]
                .as_u64()
                .ok_or("Missing 'building_id' parameter")? as u32;
            let format = args["format"].as_str().unwrap_or("raw");
            let encoding = args["encoding"].as_str().unwrap_or("base64");
            let resp = send_socket_cmd(&format!(
                "import_binary {} {} {} {}",
                file_path, building_id, format, encoding
            ))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "vm_binary_export" => {
            let building_id = args["building_id"]
                .as_u64()
                .ok_or("Missing 'building_id' parameter")? as u32;
            let output_path = args["path"].as_str().ok_or("Missing 'path' parameter")?;
            let resp = send_socket_cmd(&format!("export_binary {} {}", building_id, output_path))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "vm_binary_inject" => {
            let building_id = args["building_id"]
                .as_u64()
                .ok_or("Missing 'building_id' parameter")? as u32;
            let ram_addr = args["ram_addr"]
                .as_str()
                .ok_or("Missing 'ram_addr' parameter")?;
            let resp = send_socket_cmd(&format!("binary_inject {} {}", building_id, ram_addr))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "vm_coord_set_binary" => {
            let x = args["x"].as_u64().ok_or("Missing 'x' parameter")? as u32;
        ...
        "vm_coord_get_binary" => {
            ...
            let y = args["y"].as_u64().ok_or("Missing 'y' parameter")? as u32;
            let key = args["key"].as_str().ok_or("Missing 'key' parameter")?;
            let resp = send_socket_cmd(&format!("coord_get_binary {} {} {}", x, y, key))?;
            Ok(serde_json::json!({ "x": x, "y": y, "key": key, "value": resp.trim().to_string() }))
        },
        "vm_binary_chunk_add" => {
            let building_id = args["building_id"]
                .as_u64()
            ...
            let offset = args["offset"]
                .as_u64()
                .ok_or("Missing 'offset' parameter")? as u64;
            let path = args["path"].as_str().ok_or("Missing 'path' parameter")?;
            let resp = send_socket_cmd(&format!(
                "binary_chunk_add {} {} {}",
                building_id, offset, path
            ))?;
            Ok(serde_json::json!({ "ok": true, "response": resp }))
        },
        "vm_binary_chunk_list" => {
            let building_id = args["building_id"]
                .as_u64()
                .ok_or("Missing 'building_id' parameter")? as u32;
            let resp = send_socket_cmd(&format!("binary_chunk_list {}", building_id))?;
            ...
            Ok(serde_json::json!({ "building_id": building_id, "chunks": chunks }))
        },
    ...
    match request.method.as_str() {
        ...
        "tools/call" => {
            ...
            match handle_tool_call(tool_name, &tool_args) {
                ...
                Err(e) => Some(JsonRpcResponse::error(request.id, -32000, e)),
            }
        },

        "ping" => {
            // MCP ping keepalive — respond with empty success
            Some(JsonRpcResponse::success(request.id, serde_json::json!({})))
        },

        "notifications/initialized" => {
            // Notifications MUST NOT have a response
            None
        },
...
mod tests {
    ...
    fn test_unknown_method_returns_error() {
        ...
        let response =
            handle_request(request).expect("unknown method should still return a response");
    ...
    fn test_notifications_initialized_no_error() {
        ...
        // Notifications MUST NOT have a response per JSON-RPC spec

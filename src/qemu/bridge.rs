...
impl QemuBridge {
    ...
    /// Returns (bytes_read, raw_text) where raw_text has ANSI escapes stripped.
    /// raw_text is suitable for writing to a VM ring buffer for a dev console app.
    pub fn read_output(&mut self, canvas_buffer: &mut [u32]) -> (usize, String) {
        if !self.alive {
            return (0, String::new());
        }
        ...
        loop {
            match self.stdout.read(&mut tmp_buf) {
                ...
                Ok(n) => {
                    ...
                    // Also capture printable text for ring buffer
                    let mut i = 0;
                    while i < n {
                        let b = tmp_buf[i];
                        if b == 0x1B {
    ...
    pub fn read_output_text(&mut self) -> String {
        // Use read_output but discard canvas rendering
        let (_, text) = self.read_output(&mut []);
        text
    }

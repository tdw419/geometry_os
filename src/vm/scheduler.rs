...
mod tests {
    ...
    #[test]
    fn test_empty_process_queue_noop() {
        let mut vm = make_halt_vm();
        // No processes spawned
        assert_eq!(vm.processes.len(), 0);

        let initial_pc = vm.pc;
        let initial_regs = vm.regs.clone();

        // step_all_processes should be safe and do nothing
        vm.step_all_processes();

        // Parent state should be unchanged
        assert_eq!(vm.pc, initial_pc);
        assert_eq!(vm.regs, initial_regs);
    }
    ...
    fn test_single_process_round_robin() {
        ...
        // Single process should execute independently
        vm.step_all_processes();

        // Helper sets slice_remaining = 10, so after 1 step it should be 9
        assert_eq!(vm.processes[0].slice_remaining, 9);
        assert_eq!(vm.processes[0].state, ProcessState::Ready);

        // Exhaust slice (10 more steps to go from 9 to 0)
        for _ in 0..9 {
            vm.step_all_processes();
        }
        assert_eq!(vm.processes[0].slice_remaining, 0);

        // Next tick should allocate fresh slice (default_time_slice * priority_multiplier = 5 * 2 = 10)
        // Then execute one step, so slice = 9
        vm.step_all_processes();
        assert_eq!(vm.processes[0].slice_remaining, 9);
    }

    #[test]
    fn test_all_processes_zombie_no_execution() {
        let mut vm = make_halt_vm();
        // Create multiple zombie processes
        for i in 1..=3 {
            spawn_halt_process(&mut vm, i);
        }

        // Run scheduler - all processes are zombie after first step
        vm.step_all_processes();

        // All should be zombie
        assert!(vm.processes.iter().all(|p| p.state == ProcessState::Zombie));

        // Subsequent scheduler calls should be noops (no active processes)
        let initial_tick = vm.sched_tick;
        vm.step_all_processes();
        vm.step_all_processes();

        // Tick should still increment (scheduler loop runs)
        assert!(vm.sched_tick > initial_tick);
    }

    #[test]
    fn test_all_processes_sleeping_no_execution() {
        let mut vm = make_halt_vm();
        let mut p1 = Process::new(1, 0, 0x3000);
        p1.sleep_until = 1000;
        vm.processes.push(p1);

        let mut p2 = Process::new(2, 0, 0x4000);
        p2.sleep_until = 2000;
        vm.processes.push(p2);

        vm.default_time_slice = 10;

        // Run scheduler - should skip both sleeping processes
        vm.step_all_processes();

        // Both should still be sleeping
        assert!(vm.processes[0].sleep_until > 0);
        assert!(vm.processes[1].sleep_until > 0);

        // Slices should remain 0 (never allocated since sleeping)
        assert_eq!(vm.processes[0].slice_remaining, 0);
        assert_eq!(vm.processes[1].slice_remaining, 0);
    }

    // ── Process preemption and yield tests ────────────────────

    #[test]
    fn test_priority_preemption_high_goes_first() {
        let mut vm = make_halt_vm();

        // Low priority process with NOP
        spawn_nop_process(&mut vm, 1, 0); // priority 0

        // High priority process with HALT
        let mut hi_proc = Process::new(2, 0, 0x5000);
        hi_proc.priority = 3;
        hi_proc.slice_remaining = 10;
        vm.processes.push(hi_proc);
        vm.ram[0x5000] = 0x00; // HALT

        vm.default_time_slice = 10;

        // High priority should execute first (due to sorting)
        vm.step_all_processes();

        // High priority (pid=2) should be zombie (executed HALT)
        assert_eq!(
            vm.processes.iter().find(|p| p.pid == 2).unwrap().state,
            ProcessState::Zombie
        );

        // Low priority (pid=1) should also have executed (one NOP)
        assert_eq!(
            vm.processes.iter().find(|p| p.pid == 1).unwrap().state,
            ProcessState::Ready
        );
        assert_eq!(
            vm.processes
    ...
    #[test]
    fn test_yield_from_opcode_clears_slice() {
        let mut vm = make_halt_vm();

        // Create a process with YIELD opcode at its PC
        let mut proc = Process::new(1, 0, 0x3000);
        proc.slice_remaining = 10;
        proc.priority = 1;
        vm.processes.push(proc);
        vm.ram[0x3000] = 0x5A; // YIELD opcode

        vm.default_time_slice = 10;

        vm.step_all_processes();

        // After YIELD, slice should be 0 and yielded flag should be set
        assert_eq!(vm.processes[0].slice_remaining, 0);
        assert!(vm.processes[0].yielded);
    }

    #[test]
    fn test_cooperative_yield_multiple_processes() {
        let mut vm = make_halt_vm();

        // Process 1: YIELD immediately
        let mut p1 = Process::new(1, 0, 0x3000);
        p1.slice_remaining = 5;
        p1.priority = 1;
        vm.processes.push(p1);
        vm.ram[0x3000] = 0x5A; // YIELD

        // Process 2: NOP
        let mut p2 = Process::new(2, 0, 0x4000);
        p2.slice_remaining = 5;
        p2.priority = 1;
        vm.processes.push(p2);
        vm.ram[0x4000] = 0x01; // NOP

        vm.default_time_slice = 5;

        vm.step_all_processes();

        // p1 should have slice=0 (yielded)
        assert_eq!(vm.processes[0].slice_remaining, 0);

        // p2 should have slice=4 (executed one NOP)
        assert_eq!(vm.processes[1].slice_remaining, 4);
    }

    #[test]
    fn test_yielded_process_waits_for_new_round() {
        let mut vm = make_halt_vm();

        // Create a process that YIELDs
        let mut proc = Process::new(1, 0, 0x3000);
        proc.slice_remaining = 5;
        proc.priority = 1;
        vm.processes.push(proc);
        vm.ram[0x3000] = 0x5A; // YIELD opcode
        ...
        // Process should have slice=0 after yield
        assert_eq!(vm.processes[0].slice_remaining, 0);

        // Replace YIELD with NOP for subsequent steps
        vm.ram[0x3000] = 0x01; // NOP

        // Run until new round (all slices exhausted)
        // Since only one process with slice=0, new round triggers
        vm.step_all_processes();

        // Process should get fresh slice (default_time_slice * 2^priority = 5 * 2 = 10)
        // Then execute one step, so slice = 10 - 1 = 9
        assert_eq!(vm.processes[0].slice_remaining, 9);
    }
}

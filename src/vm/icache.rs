...
impl Vm {
    ...
    pub fn is_icacheable(opcode: u32) -> bool {
        match opcode {
            ...
            0x11 | 0x12 | 0x2C => true, // LOAD, STORE, BANK
            ...
            // NOP, HALT (0 operands)
            0x00 | 0x01 => true,
            // Bit manipulation: rd, rs (2 operands)
            0x8D | 0x8E | 0x8F => true, // BITSET, BITCLR, BITTEST
            // ALU: rd, rs (2 operands) — CLZ, CTZ not in fast path
            0x90 => true, // NOT
    ...
    pub fn icache_operand_count(opcode: u32) -> u8 {
        match opcode {
            ...
            0x90 => 1,                                    // NOT
            0x10 | 0x11 | 0x12 | 0x2C | 0x50 | 0x51 => 2, // LDI, LOAD, STORE, BANK, CMP, MOV
    ...
    pub fn icache_execute(&mut self, decoded: &DecodedInstruction) -> Option<bool> {
        ...
        match opcode {
            ...
            // BANK reg, dest_addr -- Fall back to slow path for complex memory swap
            0x2C => return None,
...
mod tests {
    ...
    // ================================================================
    // Phase 382: PROFILE mode 4/5/6 (ICACHE_STATS/RESET/SET_THRESHOLD)
    // ================================================================

    const PROFILE: u32 = 0xC6;

    #[test]
    fn test_profile_icache_stats() {
        // Run a tight loop, then PROFILE mode=4 to get stats
        let mut vm = Vm::new();
        // @0: LDI r1, 0 | @3: LDI r2, 100 | @6: LDI r7, 1
        // @9: ADD r1, r7 | @12: CMP r1, r2 | @15: BLT r0, 9 | @18: HALT
        load_program(
            &mut vm,
            0,
            &[
                LDI, 1, 0, LDI, 2, 100, LDI, 7, 1, // setup
                ADD, 1, 7, CMP, 1, 2, BLT, 0, 9, // loop body (3 instructions)
                HALT,
            ],
        );
        let (steps, halted) = run_steps(&mut vm, 10_000);
        ...
        // Now use PROFILE mode=4 to dump icache stats to RAM[0x2000]
        let base = 0x2000u32;
        load_program(
            &mut vm,
            20, // after HALT at 19
            &[
                LDI, 10, 4, // mode = ICACHE_STATS
                LDI, 11, base, // data = RAM base addr
                PROFILE, 10, 11, HALT,
            ],
        );
        vm.halted = false;
        ...
        // Verify stats were written to RAM
        let hits_lo = vm.ram[base as usize];
        let hits_hi = vm.ram[(base as usize) + 1];
        let misses_lo = vm.ram[(base as usize) + 2];
        let generation = vm.ram[(base as usize) + 4];
        let cached = vm.ram[(base as usize) + 5];

        // The PROFILE LDI instructions add 2 extra misses (LDI r10, LDI r11)
        // but PROFILE opcode itself is not icacheable.
        let profile_misses = 2u32; // LDI r10,4 and LDI r11,base at PC 20,23

        // Total should include loop + PROFILE program misses
        let total = vm.regs[0];
        assert!(total > 0, "total accesses should be > 0");
        assert!(
            hits_lo > 0,
            "should have cache hits from the loop (got {})",
            hits_lo
        );
        assert!(
            misses_lo >= profile_misses,
            "should have misses >= profile LDI misses (got {})",
            misses_lo
        );
        // Should have some cached entries from the loop
        assert!(cached > 0, "should have cached entries");
        println!(
            "Stats: hits={}, misses={}, gen={}, cached={}, total={}",
            hits_lo, misses_lo, generation, cached, total
        );
        // Verify total = hits + misses
        assert_eq!(
            total,
            hits_lo + misses_lo,
    ...
    #[test]
    fn test_profile_icache_reset() {
        // Run a loop, then PROFILE mode=5 to reset counters
        let mut vm = Vm::new();
        load_program(
            &mut vm,
            0,
            &[
                LDI, 1, 0, LDI, 2, 50, LDI, 7, 1, ADD, 1, 7, CMP, 1, 2, BLT, 0, 9, HALT,
            ],
        );
        run_steps(&mut vm, 10_000);
        assert!(vm.halted);

        let hits_before = vm.icache_hits;
        let misses_before = vm.icache_misses;
        assert!(hits_before > 0, "should have hits before reset");

        // PROFILE mode=5: reset counters
        load_program(
            &mut vm,
            20,
            &[
                LDI, 10, 5, // mode = ICACHE_RESET
                LDI, 11, 0, // data ignored
                PROFILE, 10, 11, HALT,
            ],
        );
        vm.halted = false;
        ...
        assert_eq!(vm.icache_hits, 0, "hits should be 0 after reset");
        // The LDI instructions after reset may generate 1-2 misses
        // (LDI r10,5 and LDI r11,0 at new PC addresses after reset)
        assert!(
            vm.icache_misses <= 2,
            "misses should be 0-2 after reset (LDI side effects), got {}",
            vm.icache_misses
        );
    }

    #[test]
    fn test_profile_set_threshold() {
        // Test PROFILE mode=6 to change the hot threshold
        let mut vm = Vm::new();
        let default_thresh = vm.icache_hot_threshold;
        assert_eq!(default_thresh, 4, "default threshold should be 4");
        ...
        load_program(
            ...
            0,
            &[
                LDI, 10, 6, // mode = SET_THRESHOLD
                LDI, 11, 2, // new threshold = 2
                PROFILE, 10, 11, HALT,
            ],
        );
        let (steps, halted) = run_steps(&mut vm, 100);
        assert!(halted);
        assert_eq!(vm.regs[0], 4, "r0 should be old threshold (4)");
        assert_eq!(vm.icache_hot_threshold, 2, "threshold should now be 2");
    }

    #[test]
    fn test_profile_set_threshold_bounds() {
        // Test that out-of-range thresholds are rejected
        let mut vm = Vm::new();

        // Try setting to 0 (invalid)
        load_program(
            &mut vm,
            0,
            &[
                LDI, 10, 6, // mode = SET_THRESHOLD
                LDI, 11, 0, // invalid threshold
                PROFILE, 10, 11, HALT,
            ],
        );
        run_steps(&mut vm, 100);
        assert_eq!(vm.icache_hot_threshold, 4, "threshold should stay at 4");

        // Try setting to 256 (invalid)
        vm.halted = false;
        vm.pc = 0;
        load_program(
            &mut vm,
            0,
            &[LDI, 10, 6, LDI, 11, 256, PROFILE, 10, 11, HALT],
        );
        run_steps(&mut vm, 100);
        assert_eq!(
            vm.icache_hot_threshold, 4,
            "threshold should stay at 4 for 256"
        );
    }

    #[test]
    fn test_threshold_affects_caching_speed() {
        // With threshold=1, instructions should be cached on first visit.
        // With threshold=8, instructions take longer to cache.
        // Both should eventually reach the same hit rate.
        let prog = &[
            LDI, 1, 0, LDI, 2, 200, LDI, 7, 1, // setup
            ADD, 1, 7, CMP, 1, 2, BLT, 0, 9, // loop
            HALT,
        ];

        // Test with threshold=1
        let mut vm1 = Vm::new();
        vm1.icache_hot_threshold = 1;
        load_program(&mut vm1, 0, prog);
        run_steps(&mut vm1, 100_000);
        let hits_t1 = vm1.icache_hits;
        let total_t1 = vm1.icache_hits + vm1.icache_misses;

        // Test with threshold=8
        let mut vm8 = Vm::new();
        vm8.icache_hot_threshold = 8;
        load_program(&mut vm8, 0, prog);
        run_steps(&mut vm8, 100_000);
        let hits_t8 = vm8.icache_hits;
        let total_t8 = vm8.icache_hits + vm8.icache_misses;

        let rate_t1 = if total_t1 > 0 {
            hits_t1 as f64 / total_t1 as f64
        } else {
            0.0
        };
        let rate_t8 = if total_t8 > 0 {
            hits_t8 as f64 / total_t8 as f64
        } else {
            0.0
        };

        println!(
            "Threshold=1: hits={}, total={}, rate={:.1}%",
            hits_t1,
            total_t1,
            rate_t1 * 100.0
        );
        println!(
            "Threshold=8: hits={}, total={}, rate={:.1}%",
            hits_t8,
            total_t8,
            rate_t8 * 100.0
        );

        // Threshold=1 should have higher hit rate (caches immediately)
        assert!(
            rate_t1 >= rate_t8,
            "threshold=1 ({:.1}%) should have >= hit rate than threshold=8 ({:.1}%)",
            rate_t1 * 100.0,
            rate_t8 * 100.0
        );
    }
    ...
    #[test]
    fn test_hit_rate_across_patterns() {
        // Measure hit rates for different patterns with default threshold
        let patterns: Vec<(&str, Vec<u32>)> = vec![
            (
                "tight_loop",
                vec![
                    LDI, 1, 0, LDI, 2, 500, LDI, 7, 1, ADD, 1, 7, CMP, 1, 2, BLT, 0, 9, HALT,
                ],
            ),
            (
                "wide_loop",
                vec![
                    LDI, 1, 0, LDI, 2, 200, LDI, 7, 1, LDI, 10, 0, ADD, 1, 7, ADD, 10, 1, SUB, 10,
                    1, CMP, 1, 2, BLT, 0, 12, HALT,
                ],
            ),
            (
                "nested_loop",
                vec![
                    LDI, 1, 0, LDI, 2, 20, LDI, 3, 0, LDI, 4, 20, LDI, 7, 1,
                    // inner: @18
                    ADD, 3, 7, CMP, 3, 4, BLT, 0, 18, // outer: next y
                    LDI, 3, 0, ADD, 1, 7, CMP, 1, 2, BLT, 0, 18, HALT,
                ],
            ),
        ];
        ...
        for (name, prog) in &patterns {
            let mut vm = Vm::new();
            load_program(&mut vm, 0, prog);
            run_steps(&mut vm, 500_000);

            let hits = vm.icache_hits;
            ...
            // All patterns should have >80% hit rate with default threshold
            assert!(
                rate > 0.80,
                "{} hit rate too low: {:.1}%",
                name,
    ...
    #[test]
    fn test_game_loop_pattern_cache() {
        // Simulates a game loop: init code + FRAME loop.
        // Init code runs once (cold), loop body repeats (hot).
        // Pattern: [LDI setup] [loop: ADD + CMP + BLT + NOP + NOP + JMP loop]
        // Only ALU ops (ADD, CMP, BLT) are icacheable.
        let mut vm = Vm::new();
        // @0-8: init (LDI r1,0; LDI r2,1000; LDI r7,1)
        // @9: ADD r1,r7 | @12: CMP r1,r2 | @15: BLT r0,9 | @18: HALT
        load_program(
            &mut vm,
            0,
            &[
                LDI, 1, 0, LDI, 2, 1000, LDI, 7, 1, // init (9 words)
                ADD, 1, 7, CMP, 1, 2, BLT, 0, 9, // loop body (9 words)
                HALT,
            ],
        );
        run_steps(&mut vm, 500_000);
        ...
        println!(
            "Game loop: hits={}, misses={}, rate={:.1}%, threshold={}",
            hits,
            misses,
            rate * 100.0,
            vm.icache_hot_threshold
        );

        // Game loop should have very high hit rate (>95%)
        assert!(
            rate > 0.95,
            "Game loop hit rate too low: {:.1}%",
            rate * 100.0
        );
    }
}

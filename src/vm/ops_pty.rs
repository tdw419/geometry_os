...
";
    ...
    // ══════════════════════════════════════════════════════════════
    // Deliverable 1: Unit tests for PTY spawn and I/O
    // ══════════════════════════════════════════════════════════════

    /// Verify PTYCLOSE frees the slot so it can be reused by the next PTYOPEN.
    #[test]
    fn ptyclose_frees_slot_for_reuse() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0; // empty command = default shell
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        setup_fetch(&mut vm, 0x100, &[5, 10]);
        vm.op_ptyopen();
        ...
        // Close it
        vm.regs[5] = handle as u32;
        setup_fetch(&mut vm, 0x200, &[5]);
        vm.op_ptyclose();
        assert_eq!(vm.regs[0], PTY_OK, "close should succeed");
        assert!(vm.pty_slots[handle].is_none(), "slot should be freed");

        // Open again -- should get the SAME slot index back
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        setup_fetch(&mut vm, 0x300, &[5, 10]);
        vm.op_ptyopen();
        if vm.regs[0] != PTY_OK {
            eprintln!("skipping: second PTY spawn failed");
            return;
        }
        let handle2 = vm.regs[10] as usize;
        assert_eq!(handle2, handle, "closed slot should be reused");

        // Cleanup
        vm.pty_slots[handle2] = None;
    }

    /// PTYREAD on a closed slot should return GEOS_EIO, not hang.
    #[test]
    fn ptyread_on_closed_slot_returns_eio() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        ...
        // Kill the child by dropping the slot, then re-create as closed
        // Simulate closed: drop the writer so the child gets EOF, wait, then read
        {
            let slot = vm.pty_slots[handle].as_mut().unwrap();
            slot.writer = None; // drop writer -> child sees EOF
        }

        // Wait for the reader thread to notice EOF
        let deadline = Instant::now() + Duration::from_secs(3);
        loop {
            let slot = vm.pty_slots[handle].as_ref().unwrap();
            if slot.is_closed() {
                break;
            }
            if Instant::now() > deadline {
                // Force the flag if the child is slow
                break;
            }
            thread::sleep(Duration::from_millis(50));
        }

        // Now try to read from the closed slot -- drain any buffered data first.
        // The shell may have written prompt bytes before exiting.
        vm.regs[12] = handle as u32;
        vm.regs[13] = 0x6000;
        vm.regs[14] = 256;
        loop {
            setup_fetch(&mut vm, 0x200, &[12, 13, 14]);
            vm.op_ptyread();
            if vm.regs[0] == geos_errno(GEOS_EIO) || vm.regs[0] == 0 {
                break;
            }
            // Got buffered data, keep draining
        }

        // Final read should return GEOS_EIO since slot is closed with no remaining data
        // (or 0 if the last drain already returned 0)
        let result = vm.regs[0];
        assert!(
            result == geos_errno(GEOS_EIO) || result == 0,
            "PTYREAD on closed slot should return EIO or 0, got {}",
            result
        );
    ...
    /// Write-then-read roundtrip: write "echo MARKER\n", read back, verify "MARKER" appears.
    #[test]
    fn pty_write_read_roundtrip_data_integrity() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        ...
        // Write "echo MARKER\n" into RAM
        let cmd = b"echo MARKER\n";
        for (i, &byte) in cmd.iter().enumerate() {
            vm.ram[0x6000 + i] = byte as u32;
        }

        vm.regs[12] = handle as u32;
        vm.regs[13] = 0x6000;
        vm.regs[14] = cmd.len() as u32;
        setup_fetch(&mut vm, 0x200, &[12, 13, 14]);
        vm.op_ptywrite();
        assert_eq!(vm.regs[0], PTY_OK, "write should succeed");
        ...
        let n_read = vm.regs[0] as usize;
        // Collect the bytes read into RAM
        let output: Vec<u8> = (0..n_read)
            .map(|i| (vm.ram[0x7000 + i] & 0xFF) as u8)
            .collect();
        let text = String::from_utf8_lossy(&output);
        assert!(
    ...
    /// Verify PTYSIZE changes the pty dimensions and data still flows after resize.
    #[test]
    fn ptysize_then_read_still_works() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        ...
        vm.regs[6] = 12; // rows
        vm.regs[7] = 40; // cols
        setup_fetch(&mut vm, 0x200, &[5, 6, 7]);
        vm.op_ptysize();
        assert_eq!(vm.regs[0], PTY_OK, "resize should succeed");

        // Write a command to trigger output
        let cmd = b"echo resized\n";
        for (i, &byte) in cmd.iter().enumerate() {
            vm.ram[0x6000 + i] = byte as u32;
        }
        vm.regs[12] = handle as u32;
        ...
        // Read should still work after resize
        vm.regs[12] = handle as u32;
        vm.regs[13] = 0x7000;
        vm.regs[14] = 4096;
        setup_fetch(&mut vm, 0x400, &[12, 13, 14]);
        vm.op_ptyread();
        ...
        assert!(
            ...
            "expected 'resized' after PTYSIZE, got: {:?}",
            text
        );
    ...
    /// PTYOPEN with a specific command (e.g., "echo hello") should spawn and exit.
    #[test]
    fn ptyopen_with_explicit_command() {
        let mut vm = crate::vm::Vm::new();
        ...
        vm.op_ptyopen();
        if vm.regs[0] != PTY_OK {
            eprintln!("skipping: PTY spawn with command failed");
            return;
        }
        let handle = vm.regs[10] as usize;
        assert!(vm.pty_slots[handle].is_some(), "slot should be populated");

        // Wait for the short-lived command to produce output
        let deadline = Instant::now() + Duration::from_secs(3);
        let mut output = Vec::new();
        while Instant::now() < deadline {
            let slot = vm.pty_slots[handle].as_ref().unwrap();
            match slot.rx.try_recv() {
                Ok(b) => output.push(b),
                Err(_) => {
                    if slot.is_closed() && output.contains(&b'o') {
                        break;
                    }
                    thread::sleep(Duration::from_millis(20));
                },
        ...
        assert!(
            text.contains("hello"),
            "expected 'hello' from explicit command, got: {:?}",
            text
        );
    ...
    // ══════════════════════════════════════════════════════════════
    // Deliverable 2: Integration tests for terminal emulation edge cases
    // ══════════════════════════════════════════════════════════════

    /// Verify that control characters (BEL, BS, TAB, LF, CR) pass through
    /// the QueryInterceptor and reach the output buffer.
    #[test]
    fn qi_control_chars_pass_through() {
        let (fwd, resp) = run_qi(&[
            0x07, // BEL
            0x08, // BS
            0x09, // TAB
        ...
        assert_eq!(fwd.len(), 5, "all control chars should forward");
        assert!(
            resp.is_empty(),
            "control chars should not produce responses"
        );
    }

    /// CSI SGR reset (ESC[0m) should pass through without generating a response.
    #[test]
    fn qi_sgr_reset_passes_through() {
        let (fwd, resp) = run_qi(&[0x1B, b'[', b'0', b'm']);
        assert_eq!(fwd.len(), 4, "SGR reset should forward");
        assert!(resp.is_empty(), "SGR reset should not produce a response");
    }

    /// CSI with long parameter string (e.g., 256-color SGR with large index)
    /// should not overflow or panic.
    #[test]
    fn qi_csi_long_param_string_no_panic() {
        let mut bytes = vec![0x1B, b'['];
        // Simulate "38;5;255m" (256-color fg with max index)
        bytes.extend_from_slice(b"38;5;255m");
        let (fwd, resp) = run_qi(&bytes);
        assert_eq!(fwd.len(), bytes.len(), "long CSI params should forward");
        assert!(
            resp.is_empty(),
            "256-color SGR should not produce a response"
        );
    }

    /// Multiple ESC sequences in rapid succession should all be handled correctly.
    #[test]
    fn qi_rapid_succession_esc_sequences() {
        // Three SGR sequences back-to-back, no text between them
        let bytes: &[u8] = &[
            0x1B, b'[', b'1', b'm', // bold
            0x1B, b'[', b'3', b'1', b'm', // red fg
            0x1B, b'[', b'0', b'm', // reset
        ];
        let (fwd, resp) = run_qi(bytes);
        assert_eq!(fwd.len(), bytes.len(), "all SGR bytes should forward");
        assert!(
            resp.is_empty(),
            "SGR sequences should not produce responses"
        );
    }

    /// Verify that the QueryInterceptor handles NUL bytes in ground state.
    #[test]
    fn qi_nul_byte_in_ground_state() {
        let (fwd, resp) = run_qi(&[0x00]);
        // NUL should be forwarded (it's < 0x20 and not a control char the QI handles)
        assert!(fwd.len() <= 1, "NUL may or may not forward");
        assert!(resp.is_empty(), "NUL should not produce a response");
    }

    /// Mixed text + ESC sequences should produce correct forwarding behavior.
    #[test]
    fn qi_mixed_text_and_esc_sequences() {
        let bytes: &[u8] = &[
            b'H', b'e', b'l', b'l', b'o', // "Hello"
            0x1B, b'[', b'1', b'm', // bold on
            b' ', b'W', b'o', b'r', b'l', b'd', // " World"
            0x1B, b'[', b'0', b'm', // reset
        ];
        let (fwd, resp) = run_qi(bytes);
        assert_eq!(fwd.len(), bytes.len(), "all bytes should forward");
        assert!(
            resp.is_empty(),
            "mixed text+ESC should not produce a response"
        );
    }

    /// Verify PTYWRITE with high-byte data (values > 127) correctly masks to low byte.
    #[test]
    fn ptywrite_masks_ram_to_low_byte() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        ...
        // Write bytes with high bits set -- only low byte should be used
        vm.ram[0x6000] = 0x00000041; // 'A' (clean)
        vm.ram[0x6001] = 0xFFFF0042; // 'B' (high bits set)
        vm.ram[0x6002] = 0x12340043; // 'C' (high bits set)
        ...
        setup_fetch(&mut vm, 0x200, &[12, 13, 14]);
        vm.op_ptywrite();
        assert_eq!(vm.regs[0], PTY_OK, "write should succeed despite high bits");

        // Cleanup
        vm.pty_slots[handle] = None;
    }

    /// PTYSIZE with zero dimensions should not panic.
    #[test]
    fn ptysize_zero_dimensions_no_panic() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        ...
        // Resize to 0x0 -- should not panic, may succeed or fail
        vm.regs[5] = handle as u32;
        vm.regs[6] = 0;
        vm.regs[7] = 0;
        setup_fetch(&mut vm, 0x200, &[5, 6, 7]);
        ...
        let result = vm.regs[0];
        assert!(
            result == PTY_OK || result == PTY_ERR_RESIZE_FAILED,
            "zero-dim resize should return OK or RESIZE_FAILED, got {}",
            result
        );
    ...
    /// PTYSIZE with very large dimensions should not panic.
    #[test]
    fn ptysize_large_dimensions_no_panic() {
        let mut vm = crate::vm::Vm::new();
        vm.ram[0x5000] = 0;
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        ...
        assert!(
            result == PTY_OK || result == PTY_ERR_RESIZE_FAILED,
            "large resize should return OK or RESIZE_FAILED, got {}",
            result
        );
    ...
    // ══════════════════════════════════════════════════════════════
    // Deliverable 3: Stress test for concurrent PTY sessions
    // ══════════════════════════════════════════════════════════════

    /// Open all MAX_PTY_SLOTS PTYs, write a unique command to each,
    /// read back from each, close all, then reopen and verify reuse.
    #[test]
    fn pty_stress_all_slots_write_read_close_reopen() {
        let mut vm = crate::vm::Vm::new();

        // Phase 1: Open all slots
        let mut handles = Vec::new();
        for i in 0..MAX_PTY_SLOTS {
            let cmd = format!("echo SLOT{}\0", i);
            for (j, byte) in cmd.bytes().enumerate() {
                vm.ram[0x5000 + j] = byte as u32;
            }
            vm.regs[5] = 0x5000;
            vm.regs[6] = 10;
            setup_fetch(&mut vm, 0x100 + i * 16, &[5, 10]);
            vm.op_ptyopen();
            if vm.regs[0] != PTY_OK {
                // System can't open this many PTYs -- clean up and skip
                for h in &handles {
                    vm.pty_slots[*h] = None;
                }
                eprintln!("skipping: system PTY limit reached at slot {}", i);
                return;
            }
            handles.push(vm.regs[10] as usize);
            thread::sleep(Duration::from_millis(100));
        }

        assert_eq!(handles.len(), MAX_PTY_SLOTS, "all slots should be open");

        // Phase 2: Write a command to each
        for (i, &handle) in handles.iter().enumerate() {
            let msg = format!("echo S{}\n", i);
            for (j, byte) in msg.bytes().enumerate() {
                vm.ram[0x6000 + j] = byte as u32;
            }
            vm.regs[12] = handle as u32;
            vm.regs[13] = 0x6000;
            vm.regs[14] = msg.len() as u32;
            setup_fetch(&mut vm, 0x200 + i * 16, &[12, 13, 14]);
            vm.op_ptywrite();
            assert_eq!(vm.regs[0], PTY_OK, "write to slot {} should succeed", i);
        }

        // Phase 3: Wait for output, then read from each
        thread::sleep(Duration::from_millis(800));
        for (i, &handle) in handles.iter().enumerate() {
            vm.regs[12] = handle as u32;
            vm.regs[13] = (0x7000 + i * 256) as u32;
            vm.regs[14] = 255;
            setup_fetch(&mut vm, 0x300 + i * 16, &[12, 13, 14]);
            vm.op_ptyread();
            // We don't assert specific content since shells may inject
            // prompt/ANSI, but the read should not return an error
            assert!(
                vm.regs[0] != PTY_ERR_INVALID_HANDLE,
                "read from slot {} should not return INVALID_HANDLE",
                i
            );
        }

        // Phase 4: Close all slots
        for (i, &handle) in handles.iter().enumerate() {
            vm.regs[5] = handle as u32;
            setup_fetch(&mut vm, 0x400 + i * 16, &[5]);
            vm.op_ptyclose();
            assert_eq!(vm.regs[0], PTY_OK, "close slot {} should succeed", i);
        }

        // Verify all slots are empty
        for i in 0..MAX_PTY_SLOTS {
            assert!(
                vm.pty_slots[i].is_none(),
                "slot {} should be empty after close",
                i
            );
        }

        // Phase 5: Reopen one slot and verify it works
        vm.ram[0x5000] = 0; // empty command
        vm.regs[5] = 0x5000;
        vm.regs[6] = 10;
        setup_fetch(&mut vm, 0x500, &[5, 10]);
        vm.op_ptyopen();
        if vm.regs[0] != PTY_OK {
            eprintln!("skipping: reopen after full cycle failed");
            return;
        }
        let reopen_handle = vm.regs[10] as usize;
        assert!(
            vm.pty_slots[reopen_handle].is_some(),
            "reopened slot should be populated"
        );

        // Cleanup
        vm.pty_slots[reopen_handle] = None;
    }

    /// Rapid open-close cycles to test slot allocation stability.
    #[test]
    fn pty_stress_rapid_open_close_cycles() {
        let mut vm = crate::vm::Vm::new();
        let cycles = 4;
        ...
        for cycle in 0..cycles {
            ...
            vm.op_ptyopen();
            if vm.regs[0] != PTY_OK {
                eprintln!("skipping: spawn failed at cycle {}", cycle);
                return;
            }
            let handle = vm.regs[10] as usize;
    ...
    /// Verify that concurrent reads from multiple slots don't interfere.
    #[test]
    fn pty_stress_concurrent_reads_no_interference() {
        let mut vm = crate::vm::Vm::new();
        let mut handles = Vec::new();
        ...
        for i in 0..2 {
            ...
            if vm.regs[0] != PTY_OK {
                ...
                eprintln!("skipping: spawn failed at slot {}", i);
                return;
            }
            handles.push(vm.regs[10] as usize);
            thread::sleep(Duration::from_millis(100));
        ...
        // Write unique marker to each
        for (i, &handle) in handles.iter().enumerate() {
            let msg = format!("echo MRK{}\n", i);
            for (j, byte) in msg.bytes().enumerate() {
                vm.ram[0x6000 + j] = byte as u32;
            }
            vm.regs[12] = handle as u32;
        ...
        // Read from both, verify no cross-slot contamination
        let mut outputs = Vec::new();
        for (i, &handle) in handles.iter().enumerate() {
            vm.regs[12] = handle as u32;
            vm.regs[13] = (0x7000 + i * 512) as u32;
            vm.regs[14] = 500;
            setup_fetch(&mut vm, 0x300 + i * 16, &[12, 13, 14]);
            vm.op_ptyread();
            let n = vm.regs[0] as usize;
            let bytes: Vec<u8> = (0..n)
                .map(|j| (vm.ram[0x7000 + i * 512 + j] & 0xFF) as u8)
                .collect();
            outputs.push(bytes);
        }

        // Slot 0 should contain MRK0, slot 1 should contain MRK1
        let text0 = String::from_utf8_lossy(&outputs[0]);
        let text1 = String::from_utf8_lossy(&outputs[1]);
        assert!(
            text0.contains("MRK0"),
            "slot 0 should contain MRK0, got: {:?}",
            text0
        );
        assert!(
            text1.contains("MRK1"),
            "slot 1 should contain MRK1, got: {:?}",
            text1
        );

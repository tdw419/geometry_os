...
impl Vm {
    ...
    pub(super) fn step_graphics(&mut self, opcode: u32) -> bool {
        match opcode {
            ...
            // HILBERT_XY2D dst_reg, x_reg, y_reg -- Convert (x,y) to 1D index
            // Phase 217: Remapped from 0x07 (conflicted with MTEXUNLOCK) to 0x3B
            0x3B => {
                ...
                if dr < NUM_REGS && xr < NUM_REGS && yr < NUM_REGS {
                    let x = self.regs[xr];
                    let y = self.regs[yr];
                    // Alignment: Use the actual TileStore/Substrate grid size if available,
                    // otherwise default to 256 for backward compatibility.
                    self.regs[dr] = super::hilbert::xy2d_u32(256, x, y);
                }
            },

            // HILBERT_D2XY x_reg, y_reg, d_reg -- Convert 1D index to (x,y)
            // Phase 217: Remapped from 0x08 (conflicted with SEMINIT) to 0x3C
            0x3C => {
                let xr = self.fetch() as usize;
                let yr = self.fetch() as usize;
                let dr = self.fetch() as usize;
                if xr < NUM_REGS && yr < NUM_REGS && dr < NUM_REGS {
                    let d = self.regs[dr];
                    let (x, y) = super::hilbert::d2xy_u32(256, d);
                    self.regs[xr] = x;
                    self.regs[yr] = y;
                }
            },

            // HMIRROR src_addr_reg, dst_x_reg, dst_y_reg -- Project RAM to canvas via Hilbert
            // Projects 65536 bytes (full order-8 curve) starting at src_addr onto
            // a 256x256 region starting at (dst_x, dst_y).
            0x3E => {
                let sr = self.fetch() as usize;
                let xr = self.fetch() as usize;
                let yr = self.fetch() as usize;
                if sr < NUM_REGS && xr < NUM_REGS && yr < NUM_REGS {
                    let src_addr = self.regs[sr] as usize;
                    let start_x = self.regs[xr] as usize;
                    let start_y = self.regs[yr] as usize;

                    // Hilbert order 8 is exactly 256x256 = 65536 points.
                    // We iterate through all 64K points and project RAM values as colors.
                    for d in 0..65536 {
                        let addr = src_addr + d;
                        ...
                        // Map byte/word to grayscale or simple palette for visualization
                        let color = (val & 0xFF) | ((val & 0xFF) << 8) | ((val & 0xFF) << 16);

                        let (lx, ly) = super::hilbert::d2xy_u32(256, d as u32);
                        self.set_pixel_clipped(start_x + lx as usize, start_y + ly as usize, color);
                    }
                }
            },

            // NEURAL_EVAL dst_reg, input_reg, weight_addr_reg
            // Phase 217: Remapped from 0x09 (conflicted with SEMWAIT) to 0x3D
            0x3D => {
                let dr = self.fetch() as usize;
                let ir = self.fetch() as usize;
                let ar = self.fetch() as usize;
                if dr < NUM_REGS && ir < NUM_REGS && ar < NUM_REGS {
                    let input = (self.regs[ir] & 0xFF) as u8;
                    let addr = self.regs[ar] as usize;
                    if addr < self.ram.len() {
                        let weight_pixel = self.ram[addr];
                        let node = super::neural::Perceptron::from_u32(weight_pixel);
                        self.regs[dr] =
                            super::neural::evaluate_node(input, node.weight, node.bias) as u32;
                    }
                }
            },
...
mod tests {
    ...
    // ══════════════════════════════════════════════════════════════════
    // Phase 404: RECTF edge cases — off-screen, negative coords, overflow
    // ══════════════════════════════════════════════════════════════════

    #[test]
    fn test_rectf_zero_height() {
        let mut vm = Vm::new();
        vm.regs[1] = 50; // x
        vm.regs[2] = 50; // y
        vm.regs[3] = 10; // w
        ...
        vm.regs[5] = 0xFF0000;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        // No pixels should be drawn with h=0
        assert_eq!(vm.screen[50 * 256 + 50], 0);
    }

    #[test]
    fn test_rectf_entirely_off_screen_right() {
        let mut vm = Vm::new();
        vm.regs[1] = 300; // x past screen (256 wide)
        vm.regs[2] = 50;
        vm.regs[3] = 20;
        vm.regs[4] = 20;
        vm.regs[5] = 0xFFFFFF;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        // Nothing on screen should be set
        let any_set = vm.screen.iter().any(|&p| p != 0);
        assert!(
            !any_set,
            "rect entirely off-screen right should draw nothing"
        );
    }

    #[test]
    fn test_rectf_entirely_off_screen_below() {
        let mut vm = Vm::new();
        vm.regs[1] = 50;
        vm.regs[2] = 300; // y past screen (256 tall)
        vm.regs[3] = 20;
        vm.regs[4] = 20;
        vm.regs[5] = 0xFFFFFF;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        ...
        assert!(
            !any_set,
            "rect entirely off-screen below should draw nothing"
        );
    }

    #[test]
    fn test_rectf_partially_off_screen_right() {
        let mut vm = Vm::new();
        vm.regs[1] = 250; // x near right edge
        vm.regs[2] = 10;
        vm.regs[3] = 20; // w extends past edge
        vm.regs[4] = 10;
        vm.regs[5] = 0x00FF00;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        // Pixels at x=250..255 should be set (clipped at 256)
        assert_eq!(vm.screen[10 * 256 + 250], 0x00FF00);
        assert_eq!(vm.screen[10 * 256 + 255], 0x00FF00);
        // y=10 through y=19 should be drawn for visible x range
        assert_eq!(vm.screen[19 * 256 + 250], 0x00FF00);
    }

    #[test]
    fn test_rectf_partially_off_screen_bottom() {
        let mut vm = Vm::new();
        vm.regs[1] = 10;
        vm.regs[2] = 250; // y near bottom edge
        vm.regs[3] = 10;
        vm.regs[4] = 20; // h extends past edge
        vm.regs[5] = 0x0000FF;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        // Pixels at y=250..255 should be set (clipped at 256)
        assert_eq!(vm.screen[250 * 256 + 10], 0x0000FF);
        assert_eq!(vm.screen[255 * 256 + 10], 0x0000FF);
        assert_eq!(vm.screen[255 * 256 + 19], 0x0000FF);
    }

    #[test]
    fn test_rectf_negative_coords_as_large_unsigned() {
        // Negative i32 coords stored as u32 wrap to huge values > 256
        // set_pixel_clipped rejects x >= 256 and y >= 256
        let mut vm = Vm::new();
        vm.regs[1] = 0xFFFFFFFF; // -1 as i32, 4294967295 as usize
        vm.regs[2] = 0xFFFFFFFE; // -2 as i32
        vm.regs[3] = 10;
        vm.regs[4] = 10;
        vm.regs[5] = 0xFF0000;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        // No crash; nothing drawn because coords > 256
        let any_set = vm.screen.iter().any(|&p| p != 0);
        assert!(
            !any_set,
            "negative coords (as large usize) should draw nothing"
        );
    }

    #[test]
    fn test_rectf_at_origin() {
        let mut vm = Vm::new();
        vm.regs[1] = 0; // x = 0
        vm.regs[2] = 0; // y = 0
        vm.regs[3] = 5;
        ...
        vm.regs[5] = 0xABCDEF;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        assert_eq!(vm.screen[0], 0xABCDEF); // top-left
        assert_eq!(vm.screen[4 * 256 + 4], 0xABCDEF); // bottom-right
        assert_eq!(vm.screen[5 * 256 + 0], 0); // just below
    }

    #[test]
    fn test_rectf_width_one_height_large() {
        // Thin vertical bar spanning most of the screen
        let mut vm = Vm::new();
        vm.regs[1] = 128;
        vm.regs[2] = 0;
        vm.regs[3] = 1; // width = 1
        ...
        vm.regs[5] = 0x112233;
        let vm = step_from(&vm, &[0x43, 1, 2, 3, 4, 5], 0);
        assert_eq!(vm.screen[0 * 256 + 128], 0x112233);
        assert_eq!(vm.screen[199 * 256 + 128], 0x112233);
        assert_eq!(vm.screen[200 * 256 + 128], 0); // just past
        assert_eq!(vm.screen[100 * 256 + 127], 0); // adjacent pixel untouched
    }

    // ══════════════════════════════════════════════════════════════════
    // Phase 404: CIRCLE boundary positions
    // ══════════════════════════════════════════════════════════════════

    #[test]
    fn test_circle_center_at_top_left_corner() {
        let mut vm = Vm::new();
        vm.regs[1] = 0; // cx at left edge
        vm.regs[2] = 0; // cy at top edge
        vm.regs[3] = 10;
        vm.regs[4] = 0xFFFFFF;
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        // Only the SE quadrant is on-screen
        // Pixel at (10, 0) should be set (right cardinal)
        assert_eq!(vm.screen[0 * 256 + 10], 0xFFFFFF);
        // Pixel at (0, 10) should be set (bottom cardinal)
        assert_eq!(vm.screen[10 * 256 + 0], 0xFFFFFF);
        // Center pixel itself (radius 10 = outline only, center not drawn)
        // (0,0) IS the center, radius 10 so center is only drawn at r=0, not r=10
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0xFFFFFF).count();
        assert!(
            pixel_count > 0,
            "should draw some pixels at top-left corner"
        );
    }

    #[test]
    fn test_circle_center_at_bottom_right_corner() {
        let mut vm = Vm::new();
        vm.regs[1] = 255; // cx at right edge
        vm.regs[2] = 255; // cy at bottom edge
        vm.regs[3] = 15;
        vm.regs[4] = 0x00FF00;
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        // Only the NW quadrant is on-screen
        assert_eq!(vm.screen[255 * 256 + 240], 0x00FF00); // left cardinal
        assert_eq!(vm.screen[240 * 256 + 255], 0x00FF00); // top cardinal
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0x00FF00).count();
        assert!(
            pixel_count > 0,
            "should draw some pixels at bottom-right corner"
        );
    }

    #[test]
    fn test_circle_center_off_screen() {
        // Center far off-screen, but radius large enough to reach screen
        let mut vm = Vm::new();
        vm.regs[1] = 300; // cx off-screen right (300 - 256 = 44 pixels past edge)
        vm.regs[2] = 128;
        vm.regs[3] = 100; // radius large enough to wrap onto screen
        vm.regs[4] = 0xFF0000;
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        // All circle pixels with x >= 256 are clipped.
        // Left side of circle: cx - radius = 300 - 100 = 200, which IS on screen.
        // So some pixels should be visible.
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0xFF0000).count();
        assert!(
            pixel_count > 0,
            "large circle with off-screen center should still reach screen, got {} pixels",
            pixel_count
        );
    }

    #[test]
    fn test_circle_center_far_off_screen_no_pixels() {
        // Center far off-screen with small radius -- nothing visible
        let mut vm = Vm::new();
        vm.regs[1] = 500;
        vm.regs[2] = 500;
        vm.regs[3] = 10;
        ...
        assert!(
            !any_set,
            "circle far off-screen with small radius should draw nothing"
        );
    }

    #[test]
    fn test_circle_large_radius_clipping() {
        // Circle centered on-screen with radius larger than the screen
        let mut vm = Vm::new();
        vm.regs[1] = 128;
        vm.regs[2] = 128;
        vm.regs[3] = 300; // radius > 256
        vm.regs[4] = 0x0000FF;
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        // No crash is the main assertion.
        // With center (128,128) and r=300, all 8-fold symmetry points
        // have coordinates outside [0,255], so pixel_count may be 0.
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0x0000FF).count();
        // The test passes as long as it doesn't panic/crash
        assert!(
            pixel_count < 1000,
            "large circle should not draw excessive pixels, got {}",
            pixel_count
        );
    }
    ...
    fn test_circle_radius_one() {
        ...
        vm.regs[4] = 0xFFFFFF;
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        // Radius 1: 4 cardinal points + 4 diagonal points = 8 pixels
        // (cx+1,cy), (cx-1,cy), (cx,cy+1), (cx,cy-1)
        assert_eq!(vm.screen[100 * 256 + 101], 0xFFFFFF); // right
        assert_eq!(vm.screen[100 * 256 + 99], 0xFFFFFF); // left
        assert_eq!(vm.screen[101 * 256 + 100], 0xFFFFFF); // bottom
        assert_eq!(vm.screen[99 * 256 + 100], 0xFFFFFF); // top
                                                         // Center should NOT be drawn (outline only)
        assert_eq!(vm.screen[100 * 256 + 100], 0);
    }

    #[test]
    fn test_circle_pixel_count_reasonable() {
        // A circle of radius R should have approximately 2*pi*R pixels (circumference)
        let mut vm = Vm::new();
        vm.regs[1] = 128;
        vm.regs[2] = 128;
        vm.regs[3] = 50;
        ...
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0xFFFFFF).count();
        // Expected: ~314 (2 * pi * 50). Bresenham circles often produce 4*R pixels
        // Accept a generous range: 150 to 450
        assert!(
            pixel_count >= 150 && pixel_count <= 450,
            "circle r=50 should have ~314 pixels, got {}",
            pixel_count
        );
    }

    #[test]
    fn test_circle_with_clip_rect() {
        // Center the circle inside the clip rect so pixels are guaranteed to be drawn
        let mut vm = Vm::new();
        vm.clip_rect = Some((100, 100, 60, 60)); // 100..159, 100..159
        vm.regs[1] = 130; // cx inside clip
        vm.regs[2] = 130; // cy inside clip
        vm.regs[3] = 10; // radius small enough to fit inside clip
        vm.regs[4] = 0xFF0000;
        let vm = step_from(&vm, &[0x46, 1, 2, 3, 4], 0);
        // Circle outline should be fully inside clip rect
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0xFF0000).count();
        assert!(
            pixel_count > 0,
            "circle should have pixels inside clip rect"
        );
        // Verify no pixels outside clip
        for y in 0..256 {
            for x in 0..256 {
                if vm.screen[y * 256 + x] == 0xFF0000 {
                    assert!(
                        x >= 100 && x < 160 && y >= 100 && y < 160,
                        "pixel at ({}, {}) outside clip rect",
                        x,
                        y
                    );
                }
    ...
    // ══════════════════════════════════════════════════════════════════
    // Phase 404: LINE Bresenham correctness and edge cases
    // ══════════════════════════════════════════════════════════════════

    #[test]
    fn test_line_reversed_horizontal() {
        // x1 < x0 (right to left)
        let mut vm = Vm::new();
        vm.regs[1] = 50; // x0
        vm.regs[2] = 20; // y0
        vm.regs[3] = 10; // x1 (left of x0)
        vm.regs[4] = 20; // y1
        vm.regs[5] = 0xFFFFFF;
        let vm = step_from(&vm, &[0x45, 1, 2, 3, 4, 5], 0);
        for x in 10..=50 {
    ...
    #[test]
    fn test_line_reversed_vertical() {
        // y1 < y0 (bottom to top)
        let mut vm = Vm::new();
        vm.regs[1] = 30;
        vm.regs[2] = 60; // y0 (below)
        vm.regs[3] = 30;
        vm.regs[4] = 10; // y1 (above)
        vm.regs[5] = 0x00FF00;
        let vm = step_from(&vm, &[0x45, 1, 2, 3, 4, 5], 0);
        for y in 10..=60 {
            assert_eq!(
    ...
    fn test_line_anti_diagonal() {
        // Slope = -1 (top-right to bottom-left)
        let mut vm = Vm::new();
        vm.regs[1] = 0; // x0
        vm.regs[2] = 20; // y0
        vm.regs[3] = 20; // x1
    ...
    #[test]
    fn test_line_bresenham_pixel_count_horizontal() {
        // Horizontal line from (10,50) to (60,50) should produce exactly 51 pixels
        let mut vm = Vm::new();
        vm.regs[1] = 10;
        vm.regs[2] = 50;
        vm.regs[3] = 60;
        ...
        let count = (0..256)
            .flat_map(|y| (0..256).map(move |x| (x, y)))
            .filter(|&(x, y)| vm.screen[y * 256 + x] == 0x123456)
            .count();
        assert_eq!(count, 51, "horizontal line 10..=60 should have 51 pixels");
    }

    #[test]
    fn test_line_bresenham_pixel_count_diagonal() {
        // Diagonal from (0,0) to (50,50) should produce exactly 51 pixels
        let mut vm = Vm::new();
        vm.regs[1] = 0;
        vm.regs[2] = 0;
        vm.regs[3] = 50;
        ...
        let count = (0..256)
            .flat_map(|y| (0..256).map(move |x| (x, y)))
            .filter(|&(x, y)| vm.screen[y * 256 + x] == 0xABCDEF)
            .count();
        assert_eq!(count, 51, "diagonal 0,0 to 50,50 should have 51 pixels");
    }

    #[test]
    fn test_line_bresenham_shallow_slope_pixel_count() {
        // Shallow slope: dx=100, dy=3 -- should have 101 pixels (one per x step)
        let mut vm = Vm::new();
        vm.regs[1] = 0;
        vm.regs[2] = 100;
        vm.regs[3] = 100;
        ...
        let count = (0..256)
            .flat_map(|y| (0..256).map(move |x| (x, y)))
            .filter(|&(x, y)| vm.screen[y * 256 + x] == 0x550055)
            .count();
        assert_eq!(
            count, 101,
            "shallow line should have 101 pixels (one per x)"
        );
    }

    #[test]
    fn test_line_bresenham_steep_slope_pixel_count() {
        // Steep slope: dx=3, dy=100 -- should have 101 pixels (one per y step)
        let mut vm = Vm::new();
        vm.regs[1] = 100;
        vm.regs[2] = 0;
        vm.regs[3] = 103;
        ...
        let count = (0..256)
            .flat_map(|y| (0..256).map(move |x| (x, y)))
            .filter(|&(x, y)| vm.screen[y * 256 + x] == 0x005500)
            .count();
        assert_eq!(count, 101, "steep line should have 101 pixels (one per y)");
    }

    #[test]
    fn test_line_entirely_off_screen() {
        let mut vm = Vm::new();
        vm.regs[1] = 300; // off-screen
        vm.regs[2] = 300;
        vm.regs[3] = 400;
        vm.regs[4] = 400;
        vm.regs[5] = 0xFFFFFF;
        let vm = step_from(&vm, &[0x45, 1, 2, 3, 4, 5], 0);
        let any_set = vm.screen.iter().any(|&p| p != 0);
        assert!(!any_set, "line entirely off-screen should draw nothing");
    }

    #[test]
    fn test_line_crosses_screen_boundary() {
        // Line starts off-screen left, ends on-screen
        let mut vm = Vm::new();
        vm.regs[1] = 300; // off-screen right
        vm.regs[2] = 128;
        vm.regs[3] = 0; // on-screen left
        vm.regs[4] = 128;
        vm.regs[5] = 0xFF00FF;
        let vm = step_from(&vm, &[0x45, 1, 2, 3, 4, 5], 0);
        // Horizontal line y=128 from x=0 to x=255 (visible portion)
        assert_eq!(vm.screen[128 * 256 + 0], 0xFF00FF);
        assert_eq!(vm.screen[128 * 256 + 128], 0xFF00FF);
        assert_eq!(vm.screen[128 * 256 + 255], 0xFF00FF);
    }

    #[test]
    fn test_line_with_clip_rect() {
        let mut vm = Vm::new();
        vm.clip_rect = Some((50, 50, 20, 20)); // 50..69, 50..69
        vm.regs[1] = 0; // start outside clip
        vm.regs[2] = 0;
        vm.regs[3] = 255; // end outside clip
        vm.regs[4] = 255;
        vm.regs[5] = 0x00FFFF;
        let vm = step_from(&vm, &[0x45, 1, 2, 3, 4, 5], 0);
        // Only pixels inside clip should be drawn
        let pixel_count = vm.screen.iter().filter(|&&p| p == 0x00FFFF).count();
        assert!(pixel_count > 0, "diagonal line should intersect clip rect");
        // Verify all drawn pixels are inside clip
        for y in 0..256 {
            for x in 0..256 {
    ...
    #[test]
    fn test_line_negative_coords_as_large_unsigned() {
        // Start with negative coords (as large u32), end on-screen
        let mut vm = Vm::new();
        vm.regs[1] = 0xFFFFFF00; // -256 as i32, wraps to huge usize > 256
        vm.regs[2] = 0xFFFFFF00;
        vm.regs[3] = 128; // on-screen
        vm.regs[4] = 128;
        vm.regs[5] = 0xFFFF00;
        let vm = step_from(&vm, &[0x45, 1, 2, 3, 4, 5], 0);
        // Only the endpoint should be on-screen
        assert_eq!(vm.screen[128 * 256 + 128], 0xFFFF00);
    }
}

...
fn main() {
    ...
    let scenarios = vec![
        ("healthy-oscillator", setup_oscillator as fn(&mut vm::Vm), "Is this system healthy? Reply YES or NO and one short reason."),
        ("memory-leak", setup_leak as fn(&mut vm::Vm), "Is this system healthy? Reply YES or NO and one short reason."),
        ("runaway-process", setup_runaway as fn(&mut vm::Vm), "Is this system healthy? Reply YES or NO and one short reason."),
        ("stuck-pixel", setup_stuck_pixel as fn(&mut vm::Vm), "Is this system healthy? Reply YES or NO and one short reason."),
        ("idle-blank", setup_blank as fn(&mut vm::Vm), "Is this system healthy? Reply YES or NO and one short reason."),
    ];
    ...
    for (name, setup_fn, prompt) in scenarios {
        ...
        let text_system = format!("You are a Geometry OS diagnostic assistant.\n\n{}", text_ctx);
        ...
        // Log results
        println!("[text]   latency: {:?}, response: {:?}", text_duration, text_response.as_deref().unwrap_or("(failed)"));
        println!("[vision] latency: {:?}, response: {:?}", vision_duration, vision_response.as_deref().unwrap_or("(failed)"));

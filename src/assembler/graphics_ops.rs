...
pub(super) fn try_parse(
    ...
    match opcode {
        "HILBERT_XY2D" => {
            if tokens.len() < 4 {
                return Err("HILBERT_XY2D requires 3 arguments: dst_reg, x_reg, y_reg".to_string());
            }
            bytecode.push(0x3B);
        ...
        "HILBERT_D2XY" => {
            if tokens.len() < 4 {
                return Err("HILBERT_D2XY requires 3 arguments: x_reg, y_reg, d_reg".to_string());
            }
            bytecode.push(0x3C);
            bytecode.push(parse_reg(tokens[1])? as u32);
            bytecode.push(parse_reg(tokens[2])? as u32);
        ...
        "HMIRROR" => {
            if tokens.len() < 4 {
                return Err(
                    "HMIRROR requires 3 arguments: src_addr_reg, dst_x_reg, dst_y_reg".to_string(),
                );
            }
            bytecode.push(0x3E);
            bytecode.push(parse_reg(tokens[1])? as u32);
        ...
        "NEURAL_EVAL" => {
            if tokens.len() < 4 {
                return Err(
                    "NEURAL_EVAL requires 3 arguments: dst_reg, input_reg, weight_addr_reg"
                        .to_string(),
                );
            }
            bytecode.push(0x3D);
        ...
        "SPRITE_SCALE" | "SPRITESCL" => {
            if tokens.len() < 7 {
                return Err(
                    "SPRITE_SCALE requires 6 arguments: SPRITE_SCALE x_reg, y_reg, addr_reg, w_reg, h_reg, scale_reg"
                        .to_string(),
                );
            }
            bytecode.push(0x0A);
        ...
        "SPRITELAYER" => {
            if tokens.len() < 4 {
                return Err(
                    "SPRITELAYER requires 3 arguments: SPRITELAYER layer_reg, mode_reg, data_reg"
                        .to_string(),
                );
            }
            bytecode.push(0xD0);
...
mod tests {
    ...
    // ── SPRITE_SCALE (0x0A) ──────────────────────────────────
    #[test]
    fn test_sprite_scale_basic() {
        let bc = parse_one("SPRITE_SCALE", "r1, r2, r3, r4, r5, r6");
        assert_eq!(bc, vec![0x0A, 1, 2, 3, 4, 5, 6]);
    }

    #[test]
    fn test_sprite_scale_short_name() {
        let bc = parse_one("SPRITESCL", "r10, r11, r12, r13, r14, r15");
        assert_eq!(bc, vec![0x0A, 10, 11, 12, 13, 14, 15]);
    }

    #[test]
    fn test_sprite_scale_too_few_args() {
        let err = parse_one_err("SPRITE_SCALE", "r1, r2, r3");
        assert!(err.contains("6 arguments"), "got: {err}");
    }

    #[test]
    fn test_sprite_scale_too_few_args_short() {
        let err = parse_one_err("SPRITESCL", "r1, r2");
        assert!(err.contains("6 arguments"), "got: {err}");
    }

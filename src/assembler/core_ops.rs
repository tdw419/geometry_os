...
pub(super) fn try_parse(
    ...
    match opcode {
        ...
        "EVAL" => {
            if tokens.len() < 3 {
                return Err(
                    "EVAL requires 2 arguments: EVAL script_addr_reg, result_buf_reg".to_string(),
                );
            }
            bytecode.push(0xF7);
            bytecode.push(parse_reg(tokens[1])? as u32);
        ...
        "BSWAP" => {
            if tokens.len() < 3 {
                return Err("BSWAP requires 2 arguments: BSWAP dest_reg, src_reg".to_string());
            }
            bytecode.push(0x39);
            bytecode.push(parse_reg(tokens[1])? as u32);
            bytecode.push(parse_reg(tokens[2])? as u32);
        ...
        "LDI" => {
            ...
            if let Ok(imm) = parse_imm(tokens[2], constants) {
                ...
                label_refs.push((
                    pos,
                    tokens[2].trim_start_matches(':').to_lowercase(),
                    line_num,
                ));
            }
            Ok(Some(()))
        ...
        "BANK" => {
            if tokens.len() < 3 {
                return Err("BANK requires 2 arguments: BANK coord_reg, dest_addr".to_string());
            }
            bytecode.push(0x2C);
            bytecode.push(parse_reg(tokens[1])? as u32);
        ...
        "JMP" => {
            ...
            if let Ok(addr) = parse_imm(tokens[1], constants) {
                ...
                label_refs.push((
                    pos,
                    tokens[1].trim_start_matches(':').to_lowercase(),
                    line_num,
                ));
            }
            Ok(Some(()))
...
mod tests {
    ...
    #[test]
    fn test_bswap() {
        assert_eq!(parse_one("BSWAP", "r1, r2"), vec![0x39, 1, 2]);
    }
    ...
    #[test]
    fn test_clz() {
        assert_eq!(parse_one("CLZ", "r5, r6"), vec![0x2D, 5, 6]);
    }

    #[test]
    fn test_ctz() {
        assert_eq!(parse_one("CTZ", "r7, r8"), vec![0x0C, 7, 8]);
    }

    #[test]
    fn test_clz_too_few() {
        assert!(parse_one_err("CLZ", "r1").contains("2 arguments"));
    }

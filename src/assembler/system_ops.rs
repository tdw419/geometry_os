...
pub(super) fn try_parse(
    ...
    match opcode {
        ...
        "FILEOPEN" => {
            if tokens.len() < 3 {
                return Err(
                    "FILEOPEN requires 2 arguments: FILEOPEN ring_reg, angle_reg".to_string(),
                );
            }
            bytecode.push(0x0D);
            bytecode.push(parse_reg(tokens[1])? as u32);
        ...
        "FILEREAD" => {
            if tokens.len() < 4 {
                return Err(
                    "FILEREAD requires 3 arguments: FILEREAD handle_reg, buf_reg, len_reg"
                        .to_string(),
                );
            }
            bytecode.push(0x0E);
        ...
        "CHAIN" => {
            if tokens.len() < 2 {
                return Err("CHAIN requires 1 argument: CHAIN path_addr_reg".to_string());
            }
            bytecode.push(0x3A);
            bytecode.push(parse_reg(tokens[1])? as u32);
            Ok(Some(()))
        ...
        // Phase 447: Double buffering -- wraps SYSCALL 59
        // BACKBUF sub_cmd, data
        // sub_cmd 0=ALLOCBB, 1=SWAPBB, 2=SETACTIVE
        "BACKBUF" => {
            if tokens.len() < 3 {
                return Err("BACKBUF requires 2 arguments: BACKBUF sub_cmd, data".to_string());
            }
            bytecode.push(0x52); // SYSCALL
            bytecode.push(59); // syscall number for BACKBUF
            bytecode.push(parse_imm(tokens[1], constants)?); // sub_cmd
            bytecode.push(parse_imm(tokens[2], constants)?); // data
            Ok(Some(()))
        },
        ...
        // VEC_FIND table_reg, qx_reg, qy_reg  (0x08) -- nearest neighbor search
        "VEC_FIND" => {
            if tokens.len() < 4 {
                return Err(
                    "VEC_FIND requires 3 arguments: VEC_FIND table_reg, qx_reg, qy_reg".to_string(),
                );
            }
            bytecode.push(0x08);
            bytecode.push(parse_reg(tokens[1])? as u32);
        ...
        // VEC_NEAR table_reg, idx_reg  (0x09) -- nearest neighbor of entry
        "VEC_NEAR" => {
            if tokens.len() < 3 {
                return Err(
                    "VEC_NEAR requires 2 arguments: VEC_NEAR table_reg, idx_reg".to_string()
                );
            }
            bytecode.push(0x09);
            bytecode.push(parse_reg(tokens[1])? as u32);
        ...
        // mode=4 ICACHE_STATS: dump hit/miss counters to RAM (data_reg=base addr) -> r0=total
        // mode=5 ICACHE_RESET: reset hit/miss counters
        // mode=6 ICACHE_SET_THRESHOLD: set hot threshold (data_reg=1..=255) -> r0=old threshold
        "PROFILE" => {
            if tokens.len() < 3 {
        ...
        // STRLEN addr_reg  (0xED) -- Count bytes until null terminator
        // Scans RAM at addr, returns length in r0. Max scan: RAM boundary.
        "STRLEN" => {
            if tokens.len() < 2 {
...
mod tests {
    ...
    fn test_strlen_basic() {
        let bc = parse_one("STRLEN", "r2");
        assert_eq!(bc, vec![0xED, 2]);
    }

    #[test]
    fn test_strlen_too_few() {
        let err = parse_one_err("STRLEN", "");
        assert!(err.contains("1 argument"));
    }

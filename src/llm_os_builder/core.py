...
class LLMOSBuilder:
    ...
    async def _llm_write_code(self, design: Dict[str, Any],
        ...
        prompt = f"""Write Python 3.10+ code for this OS component design:
...
Return ONLY the Python code, no explanations, no planning, no comments about what you're doing. Just the actual implementation.
"""
    ...
    async def _llm_write_tests(self, code: str, requirement: str) -> List[str]:
        ...
        prompt = f"""Write pytest tests for this Python OS component:
...
6. NO planning text - only actual test code
...
Return ONLY the Python test code starting with 'import pytest'. NO explanations, NO planning, NO comments about what you're doing. Just the actual test functions.
"""
        ...
        # Fallback: if no tests found, create basic ones
        if not tests:
            tests = [
...
'''
    ...
    async def improve_component(self, component_id: str,
                               issue: str) -> OSComponent:
        """Improve an existing component with self-analysis"""
        if component_id not in self.components:
            raise ValueError(f"Component {component_id} not found")
        ...
        analysis = self._analyze_component_issues(component, issue)

        # Ask LLM to improve the code with analysis context
        prompt = f"""
        Improve this Python OS component with self-analysis:
        ...
        Requirements for improvement:
        1. Fix the issue mentioned in self-analysis
        2. Don't break existing functionality
        3. Add comprehensive tests if needed
        4. Improve documentation and error handling
        5. Follow PEP 8 and best practices
        6. Add type hints if missing
        7. Include logging for debugging

        Return ONLY the improved Python code with enhancements.
        """
        ...
        print(f"      Tests: {new_execution.get('tests_passed', 0)}/{new_execution.get('tests_total', 0)}")
        print(f"      Execution: {'Success' if new_execution.get('compilation_success', False) else 'Failed'}")

        return new_component

    def _analyze_component_issues(self, component: OSComponent, issue: str) -> str:
        """Perform self-analysis of component issues"""
        analysis = []
        ...
        if tests_total == 0:
            analysis.append("No tests found - needs comprehensive test coverage")
        elif tests_passed < tests_total:
            analysis.append(f"Low test coverage - only {tests_passed}/{tests_total} tests passing")

        # Check code quality
        if "def __init__" not in component.code:
            analysis.append("Missing proper initialization method")

        if "def " not in component.code or component.code.count("def ") < 2:
            analysis.append("Needs more functionality methods")

        if "import " not in component.code and "from " not in component.code:
            analysis.append("Missing proper imports")

        if "typing" not in component.code:
            analysis.append("Missing type hints - add proper type annotations")

        if '"""' not in component.code and "'''" not in component.code:
            analysis.append("Missing docstrings - add proper documentation")

        # Check error handling
        if "try:" not in component.code and "except" not in component.code:
            analysis.append("Missing error handling - add try/except blocks")

        # Check logging
        if "logging" not in component.code and "print(" not in component.code:
            analysis.append("Missing logging - add debug/logging statements")

        # Add the specific issue
        analysis.append(f"Specific issue: {issue}")

        return "\n".join(f"- {item}" for item in analysis)

    async def self_improve_os(self, performance_issues: List[str]) -> Dict[str, Any]:
        """Self-improve the entire OS based on performance issues"""
        print("🔄 Starting OS self-improvement process...")
        ...
        for component_id, component in self.components.items():
            ...
            # Check if component has any of the performance issues
            component_issues = []
            for issue in performance_issues:
                if issue.lower() in component.vector.semantic_summary.lower():
                    component_issues.append(issue)

            # Add general quality issues
            analysis = self._analyze_component_issues(component, "general quality improvement")
            if analysis:
                component_issues.extend(analysis.split('\n'))

            if component_issues:
                print(f"   🔧 Found {len(component_issues)} improvement opportunities")

                # Improve the component
                try:
                    improved_component = await self.improve_component(
                        component_id,
                        ", ".join(component_issues[:3])  # Top 3 issues
                    )
                    improvement_report["components_improved"].append(improved_component.id)
                    improvement_report["issues_addressed"].extend(component_issues[:3])

                    # Calculate performance improvement
                    old_tests = component.execution_results.get('tests_passed', 0)
                    new_tests = improved_component.execution_results.get('tests_passed', 0)
                    if old_tests > 0:
                        improvement_pct = ((new_tests - old_tests) / old_tests) * 100
                        improvement_report["performance_gains"][component_id] = improvement_pct

                except Exception as e:
                    print(f"   ❌ Failed to improve {component.name}: {e}")
                    improvement_report["recommendations"].append(
                        f"Manual improvement needed for {component.name}: {e}"
                    )

        # Generate overall recommendations
        if not improvement_report["components_improved"]:
            improvement_report["recommendations"].append(
                "OS is already in good shape! No major improvements needed."
            )
        else:
            improvement_report["recommendations"].append(
                f"Successfully improved {len(improvement_report['components_improved'])} components. "
                f"Consider running performance tests to validate improvements."
            )

        print(f"✅ OS self-improvement complete!")
        print(f"   📊 Improved {len(improvement_report['components_improved'])} components")
        print(f"   🎯 Addressed {len(improvement_report['issues_addressed'])} issues")

        return improvement_report

    async def continuous_improvement_loop(self, interval: int = 3600):
        """Run continuous improvement loop"""
        print(f"🔄 Starting continuous improvement loop (interval: {interval}s)")
        ...
        while True:
            try:
                ...
                # Check OS health and performance
                performance_issues = await self._check_os_health()

                if performance_issues:
                    print(f"🔍 Found {len(performance_issues)} performance issues")
                    report = await self.self_improve_os(performance_issues)

                    # Log improvement results
                    with open(f"{self.workspace_dir}/improvement_log.json", "a") as f:
                        f.write(json.dumps({
                            "timestamp": datetime.now().isoformat(),
                            "report": report
                        }, indent=2) + "\n")
                    ...
                    print("✅ OS is running optimally - no improvements needed")

            except Exception as e:
                print(f"❌ Continuous improvement error: {e}")

    async def _check_os_health(self) -> List[str]:
        """Check overall OS health and identify performance issues"""
        issues = []
        ...
        for component in self.components.values():
            total_tests += component.execution_results.get('tests_total', 0)
            passed_tests += component.execution_results.get('tests_passed', 0)

        if total_tests == 0:
            issues.append("No test coverage across any components")
        elif passed_tests / total_tests < 0.8:
            issues.append(f"Low overall test coverage: {passed_tests}/{total_tests} ({passed_tests/total_tests:.1%})")

        # Check for failed components
        failed_components = [
            comp.name for comp in self.components.values()
            if not comp.execution_results.get('compilation_success', False)
        ]
        ...
        zero_vectors = [
            comp.name for comp in self.components.values()
            if all(v == 0 for v in comp.vector.vector)
        ]
        if zero_vectors:
            issues.append(f"Components with zero vectors: {', '.join(zero_vectors)}")

...
## Vision & Geometric Reasoning
You receive a 256x256 PNG of the current framebuffer in every iteration. Use it to:
- Identify "fractures" (visual inconsistencies or unexpected patterns).
- Verify that your drawings (RECTF, PSET, etc.) appear exactly where intended.
- Reason about system health. If you see a frozen screen or random noise, diagnose the PC and register state via `trace` and `who_wrote`.
- You are a visual-first agent. Trust your eyes over the register dump if they conflict.
...
/// Call a vision-capable LLM with a base64-encoded image.
pub fn call_vision_llm(
    ...
    // Resolve vision endpoint.
    // We strictly prefer a provider that explicitly has vision_model set,
    // or we fallback to local Ollama if everything else fails.
    // Force local Ollama for vision during verification
    ...
    // Build Ollama-specific vision payload
    let payload = format!(
    ...
    // Write payload to vision-specific temp file
    ...
    if stdout.contains("\"error\"") {
        if let Some(e) = extract_json_string(&stdout, "message") {
            println!("[hermes] Vision API error: {}", e);
        } else {
            println!(
                "[hermes] Vision API error: {}...",
                &stdout[..stdout.len().min(200)]
            );
        }
        return None;
...
pub fn execute_cli_command(
    ...
    match command.as_str() {
        ...
        "vfsfiles" | "vfs" => {
            // List VFS (.geometry_os/fs/) files with sizes
            let vfs_dir = std::path::Path::new(".geometry_os/fs");
            let mut entries = Vec::new();
            ...
            if entries.is_empty() {
                let msg = "  (no files in VFS directory .geometry_os/fs/)".to_string();
                println!("{}", msg);
                output.push_str(&msg);
                output.push('\n');
            } else {
                ...
                for (name, size) in &entries {
                    ...
                    let msg = format!("  {:<32} {:>8}", name, size_str);
                    println!("{}", msg);
                    output.push_str(&msg);
                    output.push('\n');
                    total_size += size;
                ...
                let msg = format!("  {} files, {} total", entries.len(), total_str);
                println!("{}", msg);
                output.push_str(&msg);
                output.push('\n');
            }

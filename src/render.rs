...
// ── Mirror Pixel Resolution (Phase 95) ───────────────────────────
//
// An Object-mode pixel with alpha = 0xFE is a Mirror Token. It does not
// store a literal color; it stores a signed (dx, dy) offset to another
// pixel whose color should be displayed instead. Resolution happens at
// render time so mirrors are "live" — they reflect the current state of
// the referenced pixel without explicit redraw.

/// Sentinel alpha value indicating a Mirror Token.
pub const MIRROR_ALPHA: u8 = 0xFE;

/// Maximum recursion depth when following mirror chains.
const MIRROR_MAX_DEPTH: usize = 8;

/// Resolve a pixel from the VM screen, following mirror references.
///
/// If the pixel at (x, y) is a Mirror Token (alpha = 0xFE), this function
/// reads the embedded (dx, dy) offset, looks up the source pixel, and
/// repeats until a literal color is found or the max depth is reached.
///
/// # Mirror Token Encoding
/// ```text
/// Bits 31:24 = 0xFE          (Mirror sentinel)
/// Bits 23:16 = dx (i8)       (signed X offset)
/// Bits 15:8  = dy (i8)       (signed Y offset)
/// Bits 7:0   = mode (u8)     (0 = simple copy, reserved for future)
/// ```
pub fn resolve_screen_pixel(screen: &[u32], x: usize, y: usize, depth: usize) -> u32 {
    if depth == 0 || x >= 256 || y >= 256 {
        return 0;
    }
    let idx = y * 256 + x;
    ...
    // Decode signed offsets from the RGB bytes
    let dx = ((pixel >> 16) & 0xFF) as i8 as i32;
    let dy = ((pixel >> 8) & 0xFF) as i8 as i32;
    let src_x = (x as i32 + dx).clamp(0, 255) as usize;
    let src_y = (y as i32 + dy).clamp(0, 255) as usize;
    resolve_screen_pixel(screen, src_x, src_y, depth - 1)
}
...
pub fn render(
    ...
    if !font_mode.is_fullwidth() {
        for y in 0..256 {
            for x in 0..256 {
                let color = resolve_screen_pixel(&vm.screen, x, y, MIRROR_MAX_DEPTH);
    ...
    // Read building data from Vm.buildings Vec (unlimited capacity).
    ...
    for (i, &addr) in display_addrs.iter().enumerate() {
        let (mnemonic, _, _) = vm.disassemble_annotated(addr, &vm.assembler_labels);
        let is_pc = addr == pc;
        ...
        let line = format!("{}{:04X} {}{}", marker, addr, label_prefix, mnemonic);
...
pub fn render_fullscreen_map(
    ...
    for sy in 0..src_region {
        for sx in 0..src_region {
            ...
            let color = resolve_screen_pixel(&vm.screen, vm_x, vm_y, MIRROR_MAX_DEPTH);
    ...
    if let Some(icon_cache) = icon_cache {
        let bldg_count = vm.buildings.len();
        let cam_x = vm.ram.get(0x7800).copied().unwrap_or(0) as i32;
        let cam_y = vm.ram.get(0x7801).copied().unwrap_or(0) as i32;
    ...
    // ── Phase 204: Navigation path overlay ──────────────────────
    if !vm.nav_path.is_empty() {
        let cam_x = vm.ram.get(0x7800).copied().unwrap_or(0) as i32;
        let cam_y = vm.ram.get(0x7801).copied().unwrap_or(0) as i32;
        let path_color = 0x00FF00u32; // green dots
        for wp in vm.nav_path.iter() {
            // Waypoint tile center in VM screen pixels
            let scr_x = (wp.0 - cam_x) * 4 + 2;
            let scr_y = (wp.1 - cam_y) * 4 + 2;

            // Skip if off the VM screen
            if scr_x < 0 || scr_x >= 256 || scr_y < 0 || scr_y >= 256 {
                continue;
            }

            // Apply zoom crop
            let crop_px = scr_x - src_offset as i32;
            let crop_py = scr_y - src_offset as i32;
            if crop_px < 0
                || crop_px >= src_region as i32
                || crop_py < 0
                || crop_py >= src_region as i32
            ...
            // Draw a 2x2 dot at zoom scale
            let disp_x = map_offset + crop_px as usize * scale;
            let disp_y = map_offset + crop_py as usize * scale;
            for dy in 0..scale.saturating_mul(2) {
                for dx in 0..scale.saturating_mul(2) {
                    let dpx = disp_x + dx;
                    let dpy = disp_y + dy;
                    if dpx < WIDTH && dpy < HEIGHT {
                        buffer[dpy * WIDTH + dpx] = path_color;
                    }
                }
            }
        }
    ...
    // Building list
    for (i, b) in vm.buildings.iter().enumerate() {
        let bx = b.x as u32;
        let by = b.y as u32;
        let name = &b.name;
...
mod tests {
    ...
    #[test]
    fn test_mirror_pixel_resolution() {
        let mut screen = vec![0u32; 256 * 256];
        // Place a literal red pixel at (10, 10)
        screen[10 * 256 + 10] = 0x00FF0000;
        // Place a mirror pixel at (11, 10) pointing left (dx=-1, dy=0)
        let mirror = (MIRROR_ALPHA as u32) << 24 | ((-1i8 as u8) as u32) << 16 | (0u32) << 8;
        screen[10 * 256 + 11] = mirror;
        // Resolve both
        assert_eq!(
            resolve_screen_pixel(&screen, 10, 10, MIRROR_MAX_DEPTH),
            0x00FF0000
        );
        assert_eq!(
            resolve_screen_pixel(&screen, 11, 10, MIRROR_MAX_DEPTH),
            0x00FF0000
        );
    }

    #[test]
    fn test_mirror_chain_breaks_on_depth() {
        let mut screen = vec![0u32; 256 * 256];
        // Create a cycle: (5,5) -> (6,5) -> (5,5)
        let m1 = (MIRROR_ALPHA as u32) << 24 | (1u32) << 16 | (0u32) << 8; // points right
        let m2 = (MIRROR_ALPHA as u32) << 24 | ((-1i8 as u8) as u32) << 16 | (0u32) << 8; // points left
        screen[5 * 256 + 5] = m1;
        screen[5 * 256 + 6] = m2;
        // Should return 0 (black) when depth exhausted
        assert_eq!(resolve_screen_pixel(&screen, 5, 5, MIRROR_MAX_DEPTH), 0);
    }
}

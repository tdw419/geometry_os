...
pub const OPCODES: &[&str] = &[
    ...
    "VWTXT",
    // Phase 210: Font selection for TEXT opcode
    "FONT_SELECT",
    ...
    "INC",
    "DEC",
    // Region directives for layout blocks
    "REGION",
    "ENDR",
];
...
pub struct Preprocessor {
    ...
    /// Stack of active REGION blocks: (name, x, y, w, h, is_shared)
    region_stack: Vec<(String, u32, u32, u32, u32, bool)>,
}
        ...
        for line in source.lines() {
            ...
            if spans[0].kind == SynTok::Opcode {
                ...
                let handled = match cmd.as_str() {
                    ...
                    "REGION" => {
                        // Pattern: REGION name, x, y, w, h [, bg=0xRRGGBB] [, SHARED]
                        // Collect all non-comment token texts after REGION, strip commas
                        let args: Vec<String> = spans[1..]
                            .iter()
                            .filter(|s| s.kind != SynTok::Comment)
                            .map(|s| s.text.trim_end_matches(',').to_string())
                            .collect();
                        // args: [name, x, y, w, h, ...optional...]
                        if args.len() >= 5 {
                            let name = args[0].clone();
                            let x = self.parse_imm(&args[1]).unwrap_or(0);
                            let y = self.parse_imm(&args[2]).unwrap_or(0);
                            let w = self.parse_imm(&args[3]).unwrap_or(0);
                            let h = self.parse_imm(&args[4]).unwrap_or(0);
                            ...
                            for p in &args[5..] {
                                ...
                                if pl == "shared" {
                                    is_shared = true;
                                } else if pl.starts_with("bg=") {
                                    bg_color = self.parse_imm(&pl[3..]).ok();
                                }
                            }
                            output.push_str(&format!(
                                "; REGION {} x={} y={} w={} h={}{}\n",
                                ...
                                w,
                                h,
                                if is_shared { " SHARED" } else { "" }
                            ));
                            // Emit bg fill if specified
                            if let Some(bg) = bg_color {
                                output.push_str(&format!("LDI r28, 0x{:X}\n", bg));
                                output.push_str(&format!("LDI r10, {}\n", x));
                                output.push_str(&format!("LDI r11, {}\n", y));
                                output.push_str(&format!("LDI r12, {}\n", w));
                                output.push_str(&format!("LDI r13, {}\n", h));
                                output.push_str("RECTF r10, r11, r12, r13, r28\n");
                            }
                            // Write Window Bounds Protocol for SHARED regions
                            if is_shared && w > 0 && h > 0 {
                                output.push_str(
                                    "; Window Bounds Protocol (auto-window for VM_LIVE_SPAWN)\n",
                                );
                                output.push_str(&format!("LDI r28, {}\n", x));
                                output.push_str("LDI r29, 0xFC00\n");
                                output.push_str("STORE r29, r28\n");
                                output.push_str(&format!("LDI r28, {}\n", y));
                                output.push_str("LDI r29, 0xFC01\n");
                                output.push_str("STORE r29, r28\n");
                                output.push_str(&format!("LDI r28, {}\n", w));
                                output.push_str("LDI r29, 0xFC02\n");
                                output.push_str("STORE r29, r28\n");
                                output.push_str(&format!("LDI r28, {}\n", h));
                                output.push_str("LDI r29, 0xFC03\n");
                                output.push_str("STORE r29, r28\n");
                            }
                            self.region_stack.push((name, x, y, w, h, is_shared));
                            true
                        } else {
                            output.push_str("; [REGION error: need name, x, y, w, h]\n");
                            false
                        }
                    },
                    "ENDR" => {
                        if let Some((name, _x, _y, _w, _h, _shared)) = self.region_stack.pop() {
                            output.push_str(&format!("; ENDR {}\n", name));
                        } else {
                            output.push_str("; ENDR (no matching REGION)\n");
                        }
                        true
                    },
                    _ => false,
...
mod tests {
    ...
    #[test]
    fn test_region_shared_with_vm_live_spawn() {
        // REGION with SHARED emits Window Bounds Protocol writes to 0xFC00-0xFC03
        let mut pp = Preprocessor::new();
        let src = "REGION vm_panel, 64, 64, 128, 128, bg=0x111111, SHARED\n\
                   ; body code here\n\
                   ENDR\n";
        let result = pp.preprocess(src);
        // Should emit comment header
        assert!(
            result.contains("REGION vm_panel"),
            "should have REGION comment, got: {:?}",
            result
        );
        assert!(
            result.contains("SHARED"),
            "should mention SHARED, got: {:?}",
            result
        );
        // Should emit bg fill via RECTF
        assert!(
            result.contains("RECTF"),
            "should have RECTF for bg fill, got: {:?}",
            result
        );
        assert!(
            result.contains("0x111111"),
            "should have bg color, got: {:?}",
            result
        );
        // Should emit Window Bounds Protocol writes
        assert!(
            result.contains("0xFC00"),
            "should write x to 0xFC00, got: {:?}",
            result
        );
        assert!(
            result.contains("0xFC01"),
            "should write y to 0xFC01, got: {:?}",
            result
        );
        assert!(
            result.contains("0xFC02"),
            "should write w to 0xFC02, got: {:?}",
            result
        );
        assert!(
            result.contains("0xFC03"),
            "should write h to 0xFC03, got: {:?}",
            result
        );
        // Should emit the actual coordinate values
        assert!(
            result.contains("LDI r28, 64"),
            "should store x=64, got: {:?}",
            result
        );
        assert!(
            result.contains("LDI r28, 128"),
            "should store w/h=128, got: {:?}",
            result
        );
        // Should have ENDR
        assert!(
            result.contains("ENDR vm_panel"),
            "should close with ENDR, got: {:?}",
            result
        );
    }

    #[test]
    fn test_region_non_shared_no_bounds() {
        // REGION without SHARED should NOT emit Window Bounds Protocol
        let mut pp = Preprocessor::new();
        let src = "REGION title_bar, 0, 0, 256, 16, bg=0x0000FF\nENDR\n";
        let result = pp.preprocess(src);
        assert!(
            result.contains("RECTF"),
            "should have bg fill, got: {:?}",
            result
        );
        assert!(
            !result.contains("0xFC00"),
            "non-SHARED should NOT emit bounds, got: {:?}",
            result
        );
        assert!(
            !result.contains("0xFC03"),
    ...
    #[test]
    fn test_region_no_bg() {
        // REGION without bg= should skip fill
        let mut pp = Preprocessor::new();
        let src = "REGION empty_region, 10, 20, 50, 50, SHARED\nENDR\n";
        let result = pp.preprocess(src);
        assert!(
            !result.contains("RECTF"),
            "no bg= means no RECTF, got: {:?}",
            result
        );
        // Still emits WBP because it's SHARED
        assert!(
            result.contains("0xFC00"),
            "SHARED still emits bounds, got: {:?}",
            result
        );
    }

    #[test]
    fn test_region_missing_endr() {
        // Missing ENDR should not crash; region stays on stack
        let mut pp = Preprocessor::new();
        let src = "REGION orphan, 0, 0, 100, 100, bg=0xFF0000\nHALT\n";
        let result = pp.preprocess(src);
        assert!(
            result.contains("RECTF"),
            "should emit bg fill, got: {:?}",
            result
        );
        assert!(
            result.contains("HALT"),
            "body should pass through, got: {:?}",
            result
        );
        // region_stack still has the orphan entry (no panic)
        assert_eq!(
            pp.region_stack.len(),
            1,
            "orphan region should remain on stack"
        );
    }

    #[test]
    fn test_region_zero_bounds_shared() {
        // Zero w/h with SHARED should NOT emit WBP (guard: w>0 && h>0)
        let mut pp = Preprocessor::new();
        let src = "REGION zero_region, 0, 0, 0, 0, SHARED\nENDR\n";
        let result = pp.preprocess(src);
        assert!(
            !result.contains("0xFC00"),
            "zero bounds should NOT emit WBP, got: {:?}",
            result
        );
    }
    ...
    fn test_region_nested() {
        // Nested regions: inner ENDR closes inner, outer ENDR closes outer
        let mut pp = Preprocessor::new();
        let src = "REGION outer, 0, 0, 200, 200, bg=0x000000\n\
                   REGION inner, 10, 10, 100, 100, bg=0xFFFFFF, SHARED\n\
                   ; body\n\
                   ENDR\n\
                   ENDR\n";
        let result = pp.preprocess(src);
        // Both should have RECTF (bg fills)
        let rectf_count = result.matches("RECTF").count();
        assert_eq!(
            rectf_count, 2,
            "both regions should have bg fill, got {} RECTF",
            rectf_count
        );
        // Inner is SHARED so should have WBP
        assert!(
            result.contains("0xFC00"),
            "inner SHARED should emit WBP, got: {:?}",
            result
        );
        // Both ENDR markers
        assert!(
            result.contains("ENDR inner"),
            "should close inner, got: {:?}",
            result
        );
        assert!(
            result.contains("ENDR outer"),
            "should close outer, got: {:?}",
            result
        );
        // Stack should be empty after
        assert_eq!(pp.region_stack.len(), 0, "all regions should be closed");
    }

    #[test]
    fn test_region_multiple_sequential() {
        // Multiple sequential regions: second overwrites WBP from first
        let mut pp = Preprocessor::new();
        let src = "REGION first, 0, 0, 100, 50, SHARED\nENDR\n\
                   REGION second, 100, 50, 100, 50, SHARED\nENDR\n";
        let result = pp.preprocess(src);
        // Should have bounds for both -- second region writes its own WBP
        assert!(
            result.contains("ENDR first"),
            "should close first, got: {:?}",
            result
        );
        assert!(
            result.contains("ENDR second"),
            "should close second, got: {:?}",
            result
        );
        // Count WBP blocks -- each SHARED region emits 4 STORE instructions
        let store_count = result.matches("STORE r29, r28").count();
        assert_eq!(
            store_count, 8,
            "2 SHARED regions = 8 STOREs, got {}",
            store_count
        );
    }

    #[test]
    fn test_region_too_few_args() {
        // REGION with < 5 args should emit error comment
        let mut pp = Preprocessor::new();
        let src = "REGION bad_region, 10, 20\nENDR\n";
        let result = pp.preprocess(src);
        assert!(
            result.contains("REGION error"),
            "too few args should emit error, got: {:?}",
            result
        );
    }

    #[test]
    fn test_endr_without_region() {
        // ENDR without matching REGION should emit error comment, not panic
        let mut pp = Preprocessor::new();
        let src = "ENDR\nHALT\n";
        let result = pp.preprocess(src);
        assert!(
            result.contains("no matching REGION"),
            "lone ENDR should warn, got: {:?}",
            result
        );
        assert!(
            result.contains("HALT"),

...
mark_row_dirty:
    PUSH r1
    LDI r1, 31
    SUB r1, r13          ; r1 = 31 - row
    LDI r4, 1
    SHL r4, r1          ; r4 = 1 << (31 - row)
    LDI r0, DIRTY_ROWS
    LOAD r12, r0         ; r12 = current bitmap
    OR r12, r4           ; set the bit
    STORE r0, r12
    POP r1
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r0 = word_index, r13 = bit position within word
...
att_scan_loop:
    ...
    MOV r4, r13           ; r4 = col (will become bit_pos after div)
...
att_check_bit:
    ...
    ADD r20, r0
    LOAD r12, r20
    ; Check if bit r4 is set: shift 1 left by r4, AND
    LDI r5, 1
    LDI r9, 0
att_shift_loop:
    CMP r9, r4
...
apply_sgr:
    ...
    CMPI r2, 0x85
    JNZ r13, sgr_ext95
    CALL sgr_256_to_rgb  ; r13 = RGB from r9 (color index)
    LDI r20, FG_COLOR
    STORE r20, r13
    LDI r20, SGR_EXTENDED
    LDI r13, 0
...
; Clobbers: r13, r1, r12, r5
; Preserves: r9, r2, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r13, 232
    SUB r9, r13        ; r9 = index - 232
    LDI r13, 10
    MUL r9, r13        ; r9 = (index - 232) * 10
    LDI r13, 8
    ADD r9, r13        ; r9 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r9           ; save v
...
sgr256_cube:
    ...
    LDI r1, 8
    SHL r9, r1        ; r9 = G << 8
    OR r13, r9         ; r13 = (R << 16) | (G << 8)
    OR r13, r5         ; r13 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    LOAD r2, r20       ; r2 = row
    LDI r15, COLS
    MUL r2, r15         ; r2 = row * COLS

    ; Get current col
    LDI r20, CUR_COL
    LOAD r10, r20       ; r10 = col

    ; Determine start and end columns
    ; mode 0: col..COLS-1 (default)
    ; mode 1: 0..col
    ; mode 2: 0..COLS-1
    LDI r6, 0         ; r6 = start_col
    MOV r14, r10        ; r14 = end_col (exclusive)
    CMPI r9, 0
    JZ r13, cel_clear
    CMPI r9, 1
    JNZ r13, cel_mode2
...
ced_check0:
    ...
    LDI r14, COLS
    MUL r6, r14
    ADD r6, r10        ; offset = row*COLS + col

    LDI r2, ROWS
    LDI r15, COLS
    MUL r2, r15         ; total cells
ced_loop0:
    CMP r6, r2
    BGE r13, ced_done
    LDI r20, BUF
...
cil_count:
    ...
    LDI r6, ROWS
    SUB r6, r1        ; r6 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r6, r2
    BLT r13, cil_clear
...
cdl_clear:
    ...
    LDI r15, ROWS
    SUB r15, r1         ; r15 = ROWS-1
    MOV r6, r15
    SUB r6, r9
    ADD r6, r1        ; r6 = first row to clear (ROWS - count)
    CMP r6, r2
...
cdc_count:
    ...
    LOAD r2, r20
    LDI r20, CUR_COL
    LOAD r15, r20       ; r15 = cursor col

    ; Base offset = row * COLS
    LDI r10, COLS
    MOV r13, r2
    MUL r13, r10
    MOV r2, r13         ; r2 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r6, r15        ; r6 = dst col (starting at cursor)
cdc_shift:
    ADD r13, r6
    ADD r13, r9         ; r6 + count = src col
    LDI r10, COLS
    CMP r13, r10
...
cdc_clear:
    ...
    LDI r6, COLS
    SUB r6, r9        ; r6 = first col to clear
    LDI r10, COLS
    CMP r6, r15
    BGE r13, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r6 < r15 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r6, COLS
    SUB r6, r1        ; r6 = COLS-1 (start from rightmost)
    SUB r6, r9        ; r6 = COLS-1-count (first dst position)
cic_shift:
    CMP r6, r15
    BLT r13, cic_clear

    ; dst = BUF + base + r6 + count
    LDI r21, BUF
    MOV r13, r2
    ADD r21, r13
    ADD r21, r6
...
render_row:
    ...
    LDI r3, 0          ; x (use r3 to preserve r1)
    MOV r4, r8         ; y = 10 + row*6
    LDI r0, 256         ; w
    LDI r12, 6           ; h

...
mark_row_dirty:
    ...
    LDI r14, 1
    SHL r14, r10          ; r14 = 1 << (31 - row)
    LDI r4, DIRTY_ROWS
    LOAD r13, r4         ; r13 = current bitmap
    OR r13, r14           ; set the bit
    STORE r4, r13
    POP r10
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r4 = word_index, r6 = bit position within word
...
att_scan_loop:
    ...
    MOV r14, r6           ; r14 = col (will become bit_pos after div)
    LDI r4, 0            ; r4 = word_index
    LDI r0, 32           ; r0 = divisor
att_div_loop:
    CMPI r14, 32
    BLT r6, att_check_bit
    SUB r14, r0           ; r14 -= 32
    ADD r4, r10           ; word_index++
    JMP att_div_loop
...
apply_sgr:
    ...
    CMPI r0, 0x85
    JNZ r6, sgr_ext95
    CALL sgr_256_to_rgb  ; r6 = RGB from r9 (color index)
    LDI r20, FG_COLOR
    STORE r20, r6
    LDI r20, SGR_EXTENDED
    LDI r6, 0
...
; Clobbers: r6, r10, r13, r8
; Preserves: r9, r0, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r6, 232
    SUB r9, r6        ; r9 = index - 232
    LDI r6, 10
    MUL r9, r6        ; r9 = (index - 232) * 10
    LDI r6, 8
    ADD r9, r6        ; r9 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r9           ; save v
...
sgr256_cube:
    ...
    LDI r10, 8
    SHL r9, r10        ; r9 = G << 8
    OR r6, r9         ; r6 = (R << 16) | (G << 8)
    OR r6, r8         ; r6 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r3, r1        ; r3 = end_col (exclusive)
    CMPI r9, 0
    JZ r6, cel_clear
    CMPI r9, 1
    JNZ r6, cel_mode2
...
ced_check0:
    ...
    LDI r3, COLS
    MUL r15, r3
    ADD r15, r1        ; offset = row*COLS + col

    LDI r0, ROWS
    LDI r2, COLS
    MUL r0, r2         ; total cells
ced_loop0:
    CMP r15, r0
    BGE r6, ced_done
    LDI r20, BUF
...
cil_count:
    ...
    LDI r15, ROWS
    SUB r15, r10        ; r15 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r15, r0
    BLT r6, cil_clear
...
cdl_clear:
    ...
    LDI r2, ROWS
    SUB r2, r10         ; r2 = ROWS-1
    MOV r15, r2
    SUB r15, r9
    ADD r15, r10        ; r15 = first row to clear (ROWS - count)
    CMP r15, r0
...
cdc_count:
    ...
    MOV r15, r2        ; r15 = dst col (starting at cursor)
cdc_shift:
    ADD r6, r15
    ADD r6, r9         ; r15 + count = src col
    LDI r1, COLS
    CMP r6, r1
...
cdc_clear:
    ...
    LDI r15, COLS
    SUB r15, r9        ; r15 = first col to clear
    LDI r1, COLS
    CMP r15, r2
    BGE r6, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r15 < r2 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r15, COLS
    SUB r15, r10        ; r15 = COLS-1 (start from rightmost)
    SUB r15, r9        ; r15 = COLS-1-count (first dst position)
cic_shift:
    CMP r15, r2
    BLT r6, cic_clear
...
render_row:
    ...
    LDI r5, 0          ; x (use r5 to preserve r10)
    MOV r14, r11         ; y = 10 + row*6
    LDI r4, 256         ; w
    LDI r13, 6           ; h

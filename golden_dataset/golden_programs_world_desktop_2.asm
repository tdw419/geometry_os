...
LDI r10, 0x7800         ; camera_x address
...
zoom_0:
LDI r6, 256              ; TILES per axis (zoomed out: see 256 tiles)
LDI r5, 1                ; TILE_SIZE = 1 pixel
LDI r30, 0               ; detail_level = 0 (minimal: flat tiles only)
JMP zoom_set

zoom_1:
LDI r6, 128              ; TILES per axis (medium: 128 tiles)
LDI r5, 2                ; TILE_SIZE = 2 pixels
LDI r30, 1               ; detail_level = 1 (reduced: patterns + shimmer, no trees/contours)
JMP zoom_set

zoom_default:
LDI r6, 64               ; TILES per axis (standard: 64 tiles)
LDI r5, 4                ; TILE_SIZE = 4 pixels
...
LDI r18, 0x7808
LOAD r9, r18           ; r9 = player_x
LDI r18, 0x7809
LOAD r8, r18           ; r8 = player_y

; --- Read key bitmask ---
LOAD r16, r14           ; r16 = key bitmask
...
LOAD r18, r17                 ; r18 = CMD_LEN
CMPI r18, 63
BGE r4, cmd_type_no_key       ; buffer full
LDI r19, 0x7832
...
MOV r2, r9             ; world_x = player_x
MOV r13, r17             ; world_y = target_y
CALL check_biome_walkable
JZ r4, up_blocked       ; r4 == 0 means not walkable
JMP no_up
up_blocked:
LDI r18, 0x7809
STORE r18, r8          ; revert player_y to original
no_up:
...
MOV r13, r8             ; world_y = player_y
CALL check_biome_walkable
JZ r4, left_blocked
JMP no_left
left_blocked:
LDI r18, 0x7808
STORE r18, r9          ; revert player_x to original
no_left:
...
ent_update_loop:
  CMP r26, r21
  BGE r4, ent_update_done

  ; Load entity fields
  MOV r22, r20
  LOAD r2, r22             ; world_x (r2 for biome check)
  ADDI r22, 1
  LOAD r13, r22             ; world_y (r13 for biome check)
  ADDI r22, 1
  LOAD r17, r22            ; type
  ADDI r22, 1
...
ent_not_program:
  ...
  LDI r22, 1
  CMP r17, r22
  JZ r4, ent_ai_wander     ; wanderer
  LDI r22, 2
  CMP r17, r22
  JZ r4, ent_ai_guard      ; guard
  LDI r22, 3
  CMP r17, r22
  JZ r4, ent_ai_animal     ; animal (flee)
  LDI r22, 4
  CMP r17, r22
  JZ r4, ent_ai_ghost      ; ghost
  LDI r22, 5
  CMP r17, r22
  JZ r4, ent_ai_area_agent ; area_agent
...
ent_agent_not_idle:
  LDI r22, 1
  CMP r17, r22
  JZ r4, ent_agent_state_moving
  LDI r22, 3
  CMP r17, r22
  JZ r4, ent_agent_state_inside
  LDI r22, 5
  CMP r17, r22
  JZ r4, ent_agent_state_returning
...
ent_agent_state_moving:
  ...
  MOV r17, r24
  SUB r17, r13               ; dy = target_y - entity_y
  ...
  LDI r23, 2
  CMP r22, r23
  BLT r4, ent_agent_check_dy  ; dx < 2, check dy
  JMP ent_agent_move_dx       ; dx >= 2, move
ent_agent_dx_neg:
  NEG r22                   ; |dx|
  LDI r23, 2
  CMP r22, r23
  BLT r4, ent_agent_check_dy  ; |dx| < 2
...
ent_agent_check_dy:
  ...
  LDI r23, 2
  CMP r17, r23
  BLT r4, ent_agent_arrived   ; dy < 2, arrived!
...
ent_agent_arrived:
  ...
  LDI r23, 4
  CMP r17, r23
  BGE r4, ent_agent_arrived_done  ; mailbox full
  ADDI r17, 1
...
ent_agent_move_dy:
  ...
  SUB r22, r13               ; dy = target_y - entity_y
  LDI r23, 31
  MOV r25, r22
  SAR r25, r23
...
ent_agent_state_returning:
  ...
  MOV r17, r24
  SUB r17, r13               ; dy = home_y - entity_y
  ...
  LDI r24, 2
  CMP r22, r24
  BLT r4, ent_agent_home_check_dy
...
ent_do_biome_check:
  ...
  MOV r13, r23
  CALL check_biome_walkable
  JZ r4, ent_move_blocked  ; not walkable, skip move
...
; r6 = tiles per axis, so center offset = r6 / 2
MOV r17, r6
LDI r18, 1
SHR r17, r18              ; r17 = tiles/2 (center offset)
MOV r18, r9
SUB r18, r17
STORE r10, r18            ; camera_x = player_x - tiles/2
MOV r18, r8
SUB r18, r17
STORE r3, r18            ; camera_y = player_y - tiles/2
...
ypre_done:
  ...
  render_x:
    ; World coordinates
    MOV r2, r9
    ADD r2, r0           ; r2 = world_x
    MOV r13, r8
    ADD r13, r1           ; r13 = world_y
...
no_claim:
    ...
    MOV r11, r13
    LDI r18, 3
    SHR r12, r18          ; r12 = world_x >> 3
    SHR r11, r18          ; r11 = world_y >> 3
    LDI r18, 99001
    MUL r12, r18          ; r12 = x_hash
    LDI r18, 79007
    MUL r11, r18          ; r11 = y_hash
    XOR r12, r11           ; r12 = coarse_hash
    LDI r18, 1103515245
    MUL r12, r18          ; r12 = mixed_hash
...
no_yblend:
    ...
    ; r11 = world_x * 374761393 XOR world_y * 668265263
    ...
    LDI r18, 668265263
    MUL r21, r18
    XOR r11, r21           ; r11 = fine_hash (THE SEED, 32 bits of goodness)
...
no_foam:
    ...
    RECTF r28, r27, r20, r5, r17
    MOV r21, r28
    ADD r21, r20           ; r21 = x + 2
    RECTF r21, r27, r20, r5, r19
    JMP tile_done

pat_flat:
    ; Pattern 0: single flat tile
    RECTF r28, r27, r5, r5, r17
    JMP tile_done
...
pat_horiz:
    ...
    RECTF r28, r27, r5, r20, r17
    MOV r21, r27
    ADD r21, r20           ; r21 = y + 2
    RECTF r28, r21, r5, r20, r19
    JMP tile_done
...
tile_done:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r4, contour_clr_skip   ; detail < 2 → skip
    ...
    LDI r18, 28
    SHR r12, r18
    ANDI r12, 7              ; r12 = current_elevation (0-7)
...
contour_b_draw:
    ...
    RECTF r28, r18, r5, r15, r17

contour_done:
    ; Store current elevation for next tile's right-edge contour check
    LDI r18, 0x7807
    STORE r18, r12             ; RAM[0x7807] = current_elevation
    JMP contour_after
...
contour_after:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r4, no_tree        ; detail < 2 → skip trees
...
sky_night:
  LDI r12, 0x050510       ; top: near-black with hint of blue
  LDI r11, 0x0A0A30       ; horizon: dark navy
  JMP sky_draw

sky_dawn:
  LDI r12, 0x101040       ; top: deep blue-purple
  LDI r11, 0xCC6600       ; horizon: warm orange
  JMP sky_draw

sky_day:
  LDI r12, 0x1844AA       ; top: medium blue
  LDI r11, 0x5599DD       ; horizon: light sky blue
  JMP sky_draw

sky_dusk:
  LDI r12, 0x0C0820       ; top: dark purple
  LDI r11, 0xDD4400       ; horizon: deep orange-red
  JMP sky_draw
...
LDI r18, 256
LDI r19, 4
RECTF r2, r13, r18, r19, r12

; Band 1 (rows 4-7): top + (horizon >> 2) = 75% top + 25% horizon
MOV r17, r11
LDI r18, 2
SHR r17, r18            ; horizon >> 2
ADD r17, r12             ; top + (horizon >> 2)
LDI r18, 256            ; width
LDI r13, 4
RECTF r2, r13, r18, r19, r17
...
LDI r17, 2
CMP r18, r17
BLT r4, mm_skip          ; detail < 2 → skip minimap
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 2
    MUL r2, r18
    ADD r2, r9          ; r2 = camera_x + mx*2

    MOV r13, r1
    LDI r18, 2
    MUL r13, r18
    ADD r13, r8          ; r13 = camera_y + my*2

    ; Coarse hash for biome (same hash as main terrain)
    MOV r12, r2
    LDI r18, 3
    SHR r12, r18          ; world_x >> 3
    LDI r18, 99001
    MUL r12, r18          ; x_hash

    MOV r11, r13
    LDI r18, 3
    SHR r11, r18          ; world_y >> 3
    LDI r18, 79007
    MUL r11, r18          ; y_hash

    XOR r12, r11           ; coarse_hash
    LDI r18, 1103515245
    MUL r12, r18          ; mixed_hash
    LDI r18, 27
    SHR r12, r18          ; biome index (0..31)
...
LDI r2, 224
LDI r13, 0
RECTF r2, r13, r19, r18, r17    ; top edge
LDI r13, 31
RECTF r2, r13, r19, r18, r17    ; bottom edge
LDI r13, 0
RECTF r2, r13, r18, r19, r17    ; left edge
LDI r2, 255
RECTF r2, r13, r18, r19, r17    ; right edge
...
LDI r17, 2
CMP r18, r17
BLT r4, player_simple   ; detail < 2 → simple marker
...
LDI r17, 2
CMP r18, r17
BLT r4, bldg_simple_dots   ; detail < 2 → simple dots
...
bldg_dot_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r4, bldg_dot_next
  MOV r29, r6
  SUB r29, r15           ; tiles - 1
  CMP r28, r29
  BGE r4, bldg_dot_next
  ...
  LOAD r27, r18
  SUB r28, r27
  MUL r28, r5           ; screen_x = dx * tile_size

  MOV r29, r13
  LDI r18, 0x7801
  LOAD r27, r18
  SUB r29, r27
  MUL r29, r5           ; screen_y = dy * tile_size
...
bldg_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r4, bldg_next     ; off-screen left
  LDI r29, 62
  CMP r28, r29
  BGE r4, bldg_next     ; off-screen right
  ...
  LDI r18, 0
  CMP r29, r18
  BLT r4, bldg_next     ; off-screen top
  LDI r18, 58
  CMP r29, r18
  BGE r4, bldg_next     ; off-screen bottom
...
LDI r19, 0x7801
LOAD r13, r19             ; camera_y

; Compute screen pixel for world (32,32)
LDI r17, 32
SUB r17, r2
MUL r17, r5              ; px_x = (32 - cam_x) * tile_size
MOV r18, r17             ; save px_x

LDI r17, 32
SUB r17, r13
MUL r17, r5              ; px_y = (32 - cam_y) * tile_size
...
ent_render_loop:
  CMP r26, r21
  BGE r4, ent_render_done

  ; Load entity fields
  MOV r22, r20
  LOAD r2, r22             ; world_x
  ADDI r22, 1
  LOAD r13, r22             ; world_y
  ADDI r22, 1
  LOAD r17, r22            ; type
  ADDI r22, 1
  LOAD r18, r22            ; dir_seed
  ...
  LDI r25, 0
  CMP r27, r25
  BLT r4, ent_render_next
  MOV r25, r6
  SUB r25, r15              ; tiles - 1
  CMP r27, r25
  BGE r4, ent_render_next
  ...
  MUL r27, r5              ; screen_x = dx * tile_size
  MUL r28, r5              ; screen_y = dy * tile_size
...
ent_render_not_program:
  ...
  LDI r25, 1
  CMP r17, r25
  JZ r4, ent_render_agent   ; wanderer

  LDI r25, 2
  CMP r17, r25
  JZ r4, ent_render_guard   ; guard

  LDI r25, 3
  CMP r17, r25
  JZ r4, ent_render_animal  ; animal

  LDI r25, 4
  CMP r17, r25
  JZ r4, ent_render_ghost   ; ghost

  LDI r25, 5
  CMP r17, r25
  JZ r4, ent_render_area_agent ; area_agent
...
ent_agent_color:
  ...
  MOV r25, r18
  LDI r11, 16
  SHR r25, r11         ; direction
  LDI r11, 0
  CMP r25, r11
  JNZ r4, ent_agent_not_up2
  LDI r25, 0
  ADD r25, r27             ; dx=0
  SUB r28, r15              ; dy=-1
  LDI r11, 0xFFFFFF
  PSET r25, r28, r11   ; white dot above
  ADDI r28, 1              ; restore y
  JMP ent_agent_dir_done
ent_agent_not_up2:
  LDI r11, 1
  CMP r25, r11
  JNZ r4, ent_agent_not_down2
  LDI r25, 0
  ADD r25, r27
  ADD r28, r15              ; dy=+1
  LDI r11, 0xFFFFFF
...
ent_animal_color:
  ...
  PSET r27, r28, r11        ; left ear
  ADDI r27, 1
  PSET r27, r28, r11        ; right ear
  JMP ent_render_next
...
ent_ghost_visible:
  ...
  MOV r25, r19
  LDI r11, 3
  AND r25, r11              ; 0-3 phase
  LDI r17, 0x9933FF        ; purple base
  LDI r11, 1
  CMP r25, r11
  JNZ r4, ent_ghost_not_p2
  LDI r17, 0xBB66FF        ; light purple
  JMP ent_ghost_draw
ent_ghost_not_p2:
  LDI r11, 2
  CMP r25, r11
  JNZ r4, ent_ghost_not_p3
  LDI r17, 0x7722CC        ; dark purple
  JMP ent_ghost_draw
ent_ghost_not_p3:
  LDI r11, 3
  CMP r25, r11
  JNZ r4, ent_ghost_draw
...
ent_ghost_draw:
  ...
  SUBI r28, 2              ; eye row
  LDI r11, 0xFFFFFF
  PSET r27, r28, r11        ; center eye
  SUBI r27, 1
  PSET r27, r28, r11        ; left eye
  ADDI r27, 2
  PSET r27, r28, r11        ; right eye
  JMP ent_render_next
...
mm_ent_not_program:
  LDI r25, 1
  CMP r17, r25
  JNZ r4, mm_ent_not_wanderer
  LDI r18, 0xFF8800        ; orange (wanderer)
  JMP mm_ent_dot
mm_ent_not_wanderer:
  LDI r25, 2
  CMP r17, r25
  JNZ r4, mm_ent_not_guard
  LDI r18, 0xFF2020        ; red (guard)
  JMP mm_ent_dot
mm_ent_not_guard:
  LDI r25, 3
  CMP r17, r25
  JNZ r4, mm_ent_not_animal
...
LDI r18, 0xFFFFFFFF
CMP r17, r18
JZ r4, no_entity_tooltip   ; -1 = no entity nearby
...
LDI r18, 1
CMP r17, r18
JZ r4, ent_tooltip_wanderer
LDI r18, 2
CMP r17, r18
JZ r4, ent_tooltip_guard
LDI r18, 3
CMP r17, r18
JZ r4, ent_tooltip_animal
...
LDI r17, 2
CMP r18, r17
BGE r4, player_marker_done   ; detail >= 2 → skip (full sprite already drawn)
...
LDI r2, 128
LDI r13, 128
PSET r2, r13, r17            ; center dot
LDI r2, 127
PSET r2, r13, r17            ; left
LDI r2, 129
LDI r13, 128
PSET r2, r13, r17            ; right
LDI r2, 128
LDI r13, 127
PSET r2, r13, r17            ; up
LDI r13, 129
PSET r2, r13, r17            ; down
player_marker_done:
...
LOAD r18, r17
CMPI r18, 47               ; '/'
JNZ r4, cmd_oracle          ; not a / command → send to Oracle
...
CMPI r18, 116               ; 't'
JNZ r4, cmd_try_build
ADDI r17, 1
LOAD r18, r17
CMPI r18, 112               ; 'p'
JNZ r4, cmd_oracle          ; not /tp, fall through to oracle

; Parse first number (x) after /tp
ADDI r17, 1                  ; skip 'p'
CALL parse_next_number       ; r4 = number, r17 advanced past it
LDI r19, 0x7808
STORE r19, r4                ; player_x = parsed x

; Parse second number (y)
CALL parse_next_number
LDI r19, 0x7809
STORE r19, r4                ; player_y = parsed y
...
CMPI r18, 32
BGE r4, cmd_build_full       ; max 32 buildings
...
LDI r13, 0x7873              ; response buffer
LDI r12, 895                 ; max response length
LLM r2, r13, r12
...
; Output: r4 = parsed number, r17 = advanced past number + space
; Uses r0 as accumulator (r4 is clobbered by CMPI!)
; =========================================
parse_next_number:
PUSH r31
LDI r1, 1
...
LOAD r18, r17
CMPI r18, 32                ; space
JNZ r4, pnn_digit_start
ADDI r17, 1
JMP pnn_skip

pnn_digit_start:
LDI r0, 0                   ; accumulator in r0 (NOT r4, CMPI clobbers r4)
...
pnn_done:
MOV r4, r0                  ; move result to r4 for caller
POP r31
RET

; ===== check_biome_walkable subroutine =====
; Input: r2 = world_x, r13 = world_y
; Output: r4 = 1 (walkable) or 0 (blocked)
; Preserves: r1-r11, r9-r8
; Uses: r17-r22 as temporaries
; Clobbers: r4 (CMP result), r31 (saved/restored via push/pop)
...
LDI r17, 16
CMP r21, r17
BLT r4, biome_not_lava
LDI r17, 19
CMP r21, r17
BGE r4, biome_not_lava
JMP biome_blocked

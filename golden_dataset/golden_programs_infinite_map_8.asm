...
LDI r1, 0xFFB          ; key bitmask port
LDI r9, 0x7800         ; camera_x address
LDI r8, 0x7801         ; camera_y address
LDI r15, 0x7802         ; frame_counter address
...
LOAD r17, r15
ADD r17, r10
STORE r15, r17          ; frame_counter++

; --- Read camera position ---
LOAD r5, r9           ; r5 = camera_x
LOAD r4, r8           ; r4 = camera_y

; --- Read key bitmask ---
LOAD r16, r1           ; r16 = key bitmask
...
LDI r19, 8
CMP r18, r19
BGE r12, pre_tint_warm  ; zone >= 8 -> east
LDI r19, 0x0808
MUL r18, r19
...
LOAD r22, r15           ; r22 = frame_counter (load once for whole frame)
LDI r14, 0               ; ty = 0
...
render_y:
  ...
  render_x:
    ...
    ; World coordinates
    MOV r13, r5
    ADD r13, r0           ; r13 = world_x = camera_x + tx
    MOV r3, r4
    ADD r3, r14           ; r3 = world_y = camera_y + ty

    ; ---- Coarse hash for contiguous biomes ----
    ; Zone size = 8 tiles (>> 3) = 32x32 pixel biome patches
    MOV r11, r13
    LDI r18, 3
    SHR r11, r18          ; r11 = world_x >> 3 (coarse_x)
    LDI r18, 99001
    MUL r11, r18          ; r11 = coarse_x * 99001

    MOV r2, r3
    LDI r18, 3
    SHR r2, r18          ; r2 = world_y >> 3 (coarse_y)
    LDI r18, 79007
    MUL r2, r18          ; r2 = coarse_y * 79007

    XOR r11, r2           ; r11 = coarse_hash

    ; Mix: multiply by a large prime to spread bits into upper positions
    LDI r18, 1103515245
    MUL r11, r18          ; r11 = coarse_hash * mixing_prime

    ; Extract top 5 bits: biome type 0..31
    LDI r18, 27
    SHR r11, r18          ; r11 = biome_type (0..31)

    ; ---- Fine hash for structure placement ----
    MOV r2, r13
    LDI r18, 374761393
    MUL r2, r18          ; r2 = world_x * big_prime
    MOV r21, r3
    LDI r18, 668265263
    MUL r21, r18         ; r21 = world_y * big_prime
    XOR r2, r21          ; r2 = fine_hash
    ...
    LDI r18, 2
    CMP r11, r18
    BLT r12, struct_water       ; 0-1 water
    LDI r18, 3
    CMP r11, r18
    BLT r12, struct_land        ; 2 beach hut
    LDI r18, 5
    CMP r11, r18
    BLT r12, struct_desert      ; 3-4 desert cactus
    LDI r18, 6
    CMP r11, r18
    BLT r12, struct_oasis       ; 5 oasis palm
    LDI r18, 8
    CMP r11, r18
    BLT r12, struct_land        ; 6-7 grass hut
    LDI r18, 10
    CMP r11, r18
    BLT r12, struct_swamp       ; 8-9 swamp lily
    LDI r18, 12
    CMP r11, r18
    BLT r12, struct_land        ; 10-11 forest hut
    LDI r18, 13
    CMP r11, r18
    BLT r12, struct_mushroom    ; 12 mushroom cap
    LDI r18, 15
    CMP r11, r18
    BLT r12, struct_mountain    ; 13-14
    LDI r18, 16
    CMP r11, r18
    BLT r12, struct_tundra      ; 15 tundra frost
    LDI r18, 18
    CMP r11, r18
    BLT r12, struct_lava        ; 16-17 lava ember
    LDI r18, 19
    CMP r11, r18
    BLT r12, struct_volcanic    ; 18 volcanic vent
    LDI r18, 22
    CMP r11, r18
    BLT r12, struct_snow        ; 19-21 snow crystal
    LDI r18, 23
    CMP r11, r18
    BLT r12, struct_coral       ; 22 coral anemone
    LDI r18, 24
    CMP r11, r18
    BLT r12, struct_ruins       ; 23 ruins pillar
    LDI r18, 26
    CMP r11, r18
    BLT r12, struct_crystal     ; 24-25 crystal cluster
    LDI r18, 27
    CMP r11, r18
    BLT r12, struct_ash         ; 26 ash geyser
    LDI r18, 29
    CMP r11, r18
    BLT r12, struct_dead        ; 27-28 deadlands bone
    LDI r18, 31
    CMP r11, r18
    BLT r12, struct_biolum      ; 29-30 bioluminescent spore
...
struct_land:
    ...
    MOV r20, r22
    ADD r20, r13           ; frame_counter + world_x
    LDI r18, 3
    AND r20, r18          ; & 3 -> 0..3
...
struct_lava:
    ...
    MOV r20, r22
    ADD r20, r3           ; frame_counter + world_y
    LDI r18, 7
    AND r20, r18          ; & 7 -> 0..7
...
struct_crystal:
    ...
    MOV r20, r22
    XOR r20, r13           ; frame_counter ^ world_x
    LDI r18, 0xF
...
do_rect:
    ...
    LDI r20, 1
    CMP r18, r20
    JZ r12, pat_vert        ; pattern 1 = vertical accent
    LDI r20, 2
    CMP r18, r20
    JZ r12, pat_center      ; pattern 2 = center dot
    LDI r20, 3
    CMP r18, r20
    JZ r12, pat_corner      ; pattern 3 = corner spark
    LDI r20, 4
    CMP r18, r20
    JZ r12, pat_diag_bl     ; pattern 4 = diagonal backslash
    LDI r20, 5
    CMP r18, r20
    JZ r12, pat_diag_fw     ; pattern 5 = diagonal forward slash
    LDI r20, 6
    CMP r18, r20
    JZ r12, pat_topedge     ; pattern 6 = top edge
...
pat_center:
    ...
    AND r21, r17          ; fine_hash & 1
    JZ r21, pat_c_draw
    ADD r18, r10           ; x = screen_x + 2
    ADD r20, r10           ; y = screen_y + 2
pat_c_draw:
    PSET r18, r20, r19
    JMP next_tile
...
LOAD r17, r15           ; r17 = frame_counter
LDI r18, 16
...
mm_y:
  ...
  mm_x:
    ...
    MOV r13, r0
    LDI r18, 4
    MUL r13, r18          ; r13 = mx * 4
    ADD r13, r5          ; r13 = world_x

    MOV r3, r14
    LDI r18, 4
    MUL r3, r18          ; r3 = my * 4
    ADD r3, r4          ; r3 = world_y

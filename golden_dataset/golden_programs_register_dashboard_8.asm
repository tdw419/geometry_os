; DESCRIPTION: Geometry OS program to draw a colored object.

; register_dashboard.asm
; Phase 50: Live Register Dashboard
;
; The canvas grid shows live register values as 4-digit decimals.
; r13 counts up each frame. r14-r16 derive from r13 via arithmetic.
; 8 registers per row, 4 digits each, 2 rows = 16 registers.
; The grid IS the debug view -- no separate inspector.
;
; Layout (canvas buffer indices, addresses = 0x8000 + index):
;   Row 0: r13@0  r14@4  r5@8  r7@12  r3@16  r1@20  r12@24  r2@28
;   Row 1: r11@32 r8@36 r9@40 r15@44 r0@48 r4@52 r10@56 r16@60
;
; Register computations (from r13 = frame counter):
;   r14  = r13 * 2        (double)
;   r5  = r13 * 3        (triple)
;   r7  = r13 * 4        (quadruple)
;   r3  = r13 & 255      (low byte)
;   r1  = r13 * r13       (square)
;   r12  = r13 XOR 0xAAAA (pseudo-random)
;   r2  = r13 << 4       (shifted)
;   r11  = -r13           (negated)
;   r8 = r13 % 17       (modular)
;   r9 = r3 + r1       (sum)
;   r15 = r1 >> 8       (upper square)
;   r0 = r13 | 0x0F0F   (bitmask)
;   r4 = r12 & r2       (combination)
;   r10 = r13 + r11       (zero: r13 + NEG(r13))
;   r16 = r2 - r13       (r13 * 15)

  LDI r30, 0xFF00      ; Stack pointer for CALL/RET
  LDI r20, 10          ; Divisor constant
  LDI r21, 48          ; ASCII '0'
  LDI r22, 1           ; Increment constant

main_loop:
  ; === Update register values ===
  ADD r13, r22           ; r13 = frame counter (1, 2, 3, ...)

  MOV r14, r13            ; r14 = r13 * 2
  ADD r14, r13

  MOV r5, r14            ; r5 = r13 * 3
  ADD r5, r13

  MOV r7, r14            ; r7 = r13 * 4
  ADD r7, r14

  MOV r3, r13            ; r3 = r13 & 0xFF (low byte)
  LDI r23, 255
  AND r3, r23

  MOV r1, r13            ; r1 = r13 * r13 (square)
  MUL r1, r13

  MOV r12, r13            ; r12 = r13 XOR 0xAAAA
  LDI r23, 43690
  XOR r12, r23

  MOV r2, r13            ; r2 = r13 << 4
  LDI r23, 4
  SHL r2, r23

  MOV r11, r13            ; r11 = -r13 (negated)
  NEG r11

  MOV r8, r13           ; r8 = r13 % 17
  LDI r23, 17
  MOD r8, r23

  MOV r9, r3           ; r9 = r3 + r1
  ADD r9, r1

  MOV r15, r1           ; r15 = r1 >> 8
  LDI r23, 8
  SHR r15, r23

  MOV r0, r13           ; r0 = r13 | 0x0F0F
  LDI r23, 3855
  OR r0, r23

  MOV r4, r12           ; r4 = r12 & r2
  AND r4, r2

  MOV r10, r13           ; r10 = r13 + r11 = 0
  ADD r10, r11

  MOV r16, r2           ; r16 = r2 - r13 = r13*15
  SUB r16, r13

  ; === Display registers to canvas ===
  ; Row 0 (canvas indices 0-31)
  LDI r25, 0x8000
  MOV r26, r13
  CALL write_digits

  LDI r25, 0x8004
  MOV r26, r14
  CALL write_digits

  LDI r25, 0x8008
  MOV r26, r5
  CALL write_digits

  LDI r25, 0x800C
  MOV r26, r7
  CALL write_digits

  LDI r25, 0x8010
  MOV r26, r3
  CALL write_digits

  LDI r25, 0x8014
  MOV r26, r1
  CALL write_digits

  LDI r25, 0x8018
  MOV r26, r12
  CALL write_digits

  LDI r25, 0x801C
  MOV r26, r2
  CALL write_digits

  ; Row 1 (canvas indices 32-63)
  LDI r25, 0x8020
  MOV r26, r11
  CALL write_digits

  LDI r25, 0x8024
  MOV r26, r8
  CALL write_digits

  LDI r25, 0x8028
  MOV r26, r9
  CALL write_digits

  LDI r25, 0x802C
  MOV r26, r15
  CALL write_digits

  LDI r25, 0x8030
  MOV r26, r0
  CALL write_digits

  LDI r25, 0x8034
  MOV r26, r4
  CALL write_digits

  LDI r25, 0x8038
  MOV r26, r10
  CALL write_digits

  LDI r25, 0x803C
  MOV r26, r16
  CALL write_digits

  FRAME
  JMP main_loop

; === Subroutine: write 4 decimal digits of r26 to canvas at r25 ===
; Preserves r25, r26. Uses r27, r28, r29 (pushed/popped).
write_digits:
  PUSH r27
  PUSH r28
  PUSH r29

  ; Ones: (r26 % 10) + '0' -> [r25 + 3]
  MOV r27, r26
  MOD r27, r20
  ADD r27, r21
  LDI r28, 3
  MOV r29, r25
  ADD r29, r28
  STORE r29, r27

  ; Tens: ((r26 / 10) % 10) + '0' -> [r25 + 2]
  MOV r27, r26
  DIV r27, r20
  MOD r27, r20
  ADD r27, r21
  LDI r28, 2
  MOV r29, r25
  ADD r29, r28
  STORE r29, r27

  ; Hundreds: ((r26 / 100) % 10) + '0' -> [r25 + 1]
  MOV r27, r26
  LDI r28, 100
  DIV r27, r28
  MOD r27, r20
  ADD r27, r21
  LDI r28, 1
  MOV r29, r25
  ADD r29, r28
  STORE r29, r27

  ; Thousands: ((r26 / 1000) % 10) + '0' -> [r25]
  MOV r27, r26
  LDI r28, 1000
  DIV r27, r28
  MOD r27, r20
  ADD r27, r21
  STORE r25, r27

  POP r29
  POP r28
  POP r27
  RET

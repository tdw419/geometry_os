...
pb_normal_byte:
    ...
    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r1 & 0x1F) << 6
    LDI r15, 31
    AND r1, r15
    LDI r15, 6
    SHL r1, r15
...
pb_utf8_3_start:
    CMPI r1, 240
    BGE r8, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r1 & 0x0F) << 12
    LDI r15, 15
    AND r1, r15
    LDI r15, 12
    SHL r1, r15
...
pb_esc_restore_cursor:
    CMPI r1, 56 ; '8' = DEC Restore Cursor
    JNZ r8, pb_esc_index
    CALL restore_cursor
    LDI r20, ANSI_STATE
    LDI r8, ANS_NORMAL
...
pb_esc_index:
    CMPI r1, 68 ; 'D' = Index (move down, scroll if at bottom)
...
pb_esc_rev_index:
    CMPI r1, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r8, pb_esc_ris
...
pb_esc_next_line:
    CMPI r1, 69 ; 'E' = Next Line (CR + Index)
    JNZ r8, pb_esc_other
    LDI r20, CUR_COL
    LDI r8, 0
    STORE r20, r8
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r1 - 48)
    LDI r20, CSI_PARAM
    LOAD r8, r20
    LDI r6, 10
    MUL r8, r6
...
mark_row_dirty:
    ...
    SUB r9, r8          ; r9 = 31 - row
    LDI r14, 1
    SHL r14, r9          ; r14 = 1 << (31 - row)
    LDI r13, DIRTY_ROWS
    LOAD r10, r13         ; r10 = current bitmap
    OR r10, r14           ; set the bit
    STORE r13, r10
    POP r9
    RET
...
mark_row_dirty_check:
    ...
    LOAD r10, r13         ; r10 = current bitmap
    MOV r8, r10
    AND r8, r14          ; r8 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r1 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r9, 1
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r13 = word_index, r8 = bit position within word
...
att_scan_loop:
    ...
    MOV r14, r8           ; r14 = col (will become bit_pos after div)
...
apply_sgr:
    ...
    CMPI r15, 0x85
    JNZ r8, sgr_ext95
    CALL sgr_256_to_rgb  ; r8 = RGB from r6 (color index)
    LDI r20, FG_COLOR
    STORE r20, r8
    LDI r20, SGR_EXTENDED
    LDI r8, 0
...
; Clobbers: r8, r9, r10, r1
; Preserves: r6, r15, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r8, 232
    SUB r6, r8        ; r6 = index - 232
    LDI r8, 10
    MUL r6, r8        ; r6 = (index - 232) * 10
    LDI r8, 8
    ADD r6, r8        ; r6 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r6           ; save v
...
sgr256_cube:
    ...
    LDI r9, 8
    SHL r6, r9        ; r6 = G << 8
    OR r8, r6         ; r8 = (R << 16) | (G << 8)
    OR r8, r1         ; r8 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r12, r11        ; r12 = end_col (exclusive)
    CMPI r6, 0
    JZ r8, cel_clear
    CMPI r6, 1
    JNZ r8, cel_mode2
...
ced_check0:
    ...
    LDI r12, COLS
    MUL r7, r12
    ADD r7, r11        ; offset = row*COLS + col
...
cil_count:
    ...
    LDI r7, ROWS
    SUB r7, r9        ; r7 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r7, r15
    BLT r8, cil_clear
...
cdl_clear:
    ...
    MOV r7, r0
    SUB r7, r6
    ADD r7, r9        ; r7 = first row to clear (ROWS - count)
    CMP r7, r15
...
cdc_count:
    ...
    MOV r7, r0        ; r7 = dst col (starting at cursor)
cdc_shift:
    ADD r8, r7
    ADD r8, r6         ; r7 + count = src col
    LDI r11, COLS
    CMP r8, r11
...
cdc_clear:
    ...
    ; r7 < r0 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r7, COLS
    SUB r7, r9        ; r7 = COLS-1 (start from rightmost)
...
render_row:
    ...
    LDI r9, 1           ; restore r9 (clobbered by mark_row_dirty_check)
    ...
    LDI r2, 0          ; x (use r2 to preserve r9)
    MOV r14, r4         ; y = 10 + row*6
    LDI r13, 256         ; w
    LDI r10, 6           ; h
    LDI r1, 0x0A0A0A    ; bg color
    RECTF r2, r14, r13, r10, r1

...
init_tabs_loop:
    ...
    ADD r2, r14
    CMPI r2, 4
    BLT r5, init_tabs_loop

; Clear all 4 tab text buffers to spaces
LDI r2, 0
clear_tabs_buf:
    ; Get buffer base for tab r2
...
drain_all_loop:
    ...
    ; r5 = 1 if tab was switched (consumed the key)
    CMPI r5, 1
    JZ r5, main_loop
...
send_to_pty:
    ...
    MOV r15, r20       ; handle
    LDI r9, SEND_BUF
    ; r5 = byte count from translate_key
    PTYWRITE r15, r9, r5
    JMP main_loop

; =========================================
; GET_TAB_BUF_BASE -- return buffer address for tab r2
...
; =========================================
; GET_TAB_META -- return metadata base for tab r2
...
; =========================================
; GET_TAB_PTY -- get PTY handle for tab r2
...
; =========================================
; SPAWN_TAB -- spawn bash in PTY for tab r2
; Uses: CMD_BUF, r0, r2
; =========================================
spawn_tab:
    PUSH r31
...
; =========================================
; DRAIN_TAB -- read PTY output for tab r2, process through ANSI
; =========================================
drain_tab:
    PUSH r31
    PUSH r2
...
; =========================================
; PROCESS_BYTE_TAB -- process byte r0 for tab r2
...
process_byte_tab:
    ...
    ; State: NORMAL
    CMPI r7, ANS_NORMAL
    JNZ r5, pbt_check_esc

    CMPI r0, 27
    JNZ r5, pbt_normal_byte
...
pbt_check_esc:
    CMPI r7, ANS_ESC
    JNZ r5, pbt_check_csi

    CMPI r0, 91
    JNZ r5, pbt_esc_other
...
; =========================================
; APPEND_BYTE_TAB -- append r0 to tab r2's text buffer
; =========================================
append_byte_tab:
    PUSH r31
    LDI r14, 1
...
abt_check_print:
    ...
    LDI r9, 2
    ADD r21, r9
    LOAD r10, r21     ; r10 = CUR_ROW
    LDI r13, COLS
    MUL r10, r13
    MOV r21, r20
    LDI r9, 1
    ADD r21, r9
    LOAD r5, r21     ; r5 = CUR_COL
    ADD r10, r5
...
; =========================================
; DO_NEWLINE_TAB -- newline for tab r2
; =========================================
do_newline_tab:
    PUSH r31
    LDI r14, 1
...
; =========================================
; SCROLL_UP_TAB -- scroll text buffer for tab r2
; =========================================
scroll_up_tab:
    PUSH r31
    LDI r14, 1
...
; Returns: r5 = 1 if handled, 0 if not
; =========================================
check_tab_switch:
    PUSH r31
...
; =========================================
; SWITCH_TO_TAB -- switch to tab r5
...
close_active_tab:
    ...
    CALL get_tab_meta
    LDI r5, 0xFFFF
    STORE r20, r5      ; PTY = unused
    MOV r22, r20
    LDI r9, 4
    ADD r22, r9
    LDI r5, 0
    STORE r22, r5      ; CONNECTED = 0
...
; Fills SEND_BUF, returns byte count in r5
; r0 = raw IKEY value
; =========================================
translate_key:
    PUSH r31
...
rat_copy_col:
    ...
    LDI r9, 2
    ADD r22, r9
    LOAD r10, r22     ; CUR_ROW
    LDI r9, 6
    MUL r10, r9
    LDI r9, 10
    ADD r10, r9       ; + title bar offset
    LDI r13, 2
    LDI r7, 5
    LDI r0, 0x44FF44
    RECTF r5, r10, r13, r7, r0

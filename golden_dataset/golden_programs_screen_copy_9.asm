; DESCRIPTION: A red line centered at the screen with fixed size.

; screen_copy.asm -- Screen buffer LOAD readback demo
; Phase 219: Copy screen pixels to RAM via LOAD/STORE loop, verify consistency.
;
; How it works:
; 1. Draw 3 colored pixels on the screen via STORE to screen-mapped I/O (0x10000+)
; 2. Copy first 10 pixels from screen (0x10000-0x10009) to RAM (0x2000-0x2009)
; 3. Compare copied values against expected colors
; 4. Draw green diagonal line at (2,2)-(4,4) if pass, red if fail
;
; Memory layout:
;   0x2000-0x2009: copied screen pixels (first 10)

; ── Draw 3 test pixels on screen ──
LDI r15, 0x10000          ; screen[0] = pixel (0, 0)
LDI r9, 0xFF0000          ; red
STORE r15, r9

LDI r15, 0x10001          ; screen[1] = pixel (1, 0)
LDI r9, 0x00FF00          ; green
STORE r15, r9

LDI r15, 0x10002          ; screen[2] = pixel (2, 0)
LDI r9, 0x0000FF          ; blue
STORE r15, r9

; ── Bulk copy first 10 screen pixels to RAM ──
LDI r11, 0x10000          ; source: screen base
LDI r6, 0x2000           ; dest: RAM at 0x2000
LDI r12, 10               ; count: 10 pixels
COPY_LOOP:
LOAD r3, r11              ; r3 = screen[i]
STORE r6, r3             ; ram[dest] = r3
ADDI r11, 1               ; source++
ADDI r6, 1               ; dest++
SUBI r12, 1               ; count--
CMPI r12, 0               ; compare count to 0
JNZ r5, COPY_LOOP        ; loop while count > 0

; ── Verify pixel 0 is red (0xFF0000) ──
LDI r7, 0x2000
LOAD r1, r7              ; r1 = ram[0x2000] (copied from screen[0])
CMPI r1, 0xFF0000
JNZ r5, FAIL

; ── Verify pixel 1 is green (0x00FF00) ──
LDI r7, 0x2001
LOAD r1, r7
CMPI r1, 0x00FF00
JNZ r5, FAIL

; ── Verify pixel 2 is blue (0x0000FF) ──
LDI r7, 0x2002
LOAD r1, r7
CMPI r1, 0x0000FF
JNZ r5, FAIL

; ── Verify pixels 3-9 are zero (black/empty) ──
LDI r12, 7                ; check 7 more pixels
LDI r7, 0x2003           ; start at pixel 3
ZERO_CHECK:
LOAD r1, r7
CMPI r1, 0
JNZ r5, FAIL
ADDI r7, 1
SUBI r12, 1
CMPI r12, 0
JNZ r5, ZERO_CHECK

; ── PASS: draw green diagonal at (2,2)-(4,4) ──
LDI r15, 2
LDI r9, 2
LDI r3, 0x00FF00
PSET r15, r9, r3
LDI r15, 3
LDI r9, 3
PSET r15, r9, r3
LDI r15, 4
LDI r9, 4
PSET r15, r9, r3
JMP DONE

; ── FAIL: draw red diagonal at (2,2)-(4,4) ──
FAIL:
LDI r15, 2
LDI r9, 2
LDI r3, 0xFF0000
PSET r15, r9, r3
LDI r15, 3
LDI r9, 3
PSET r15, r9, r3
LDI r15, 4
LDI r9, 4
PSET r15, r9, r3

DONE:
HALT

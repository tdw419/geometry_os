; DESCRIPTION: This GeOS assembly code draws three colored pixels on the screen, copies the first ten screen pixels to RAM, verifies the copied values against expected colors, and then draws a green diagonal line if the verification passes or a red diagonal line if it fails.

; screen_copy.asm -- Screen buffer LOAD readback demo
; Phase 219: Copy screen pixels to RAM via LOAD/STORE loop, verify consistency.
;
; How it works:
; 1. Draw 3 colored pixels on the screen via STORE to screen-mapped I/O (0x10000+)
; 2. Copy first 10 pixels from screen (0x10000-0x10009) to RAM (0x2000-0x2009)
; 3. Compare copied values against expected colors
; 4. Draw green diagonal line at (2,2)-(4,4) if pass, red if fail
;
; Memory layout:
;   0x2000-0x2009: copied screen pixels (first 10)

; ── Draw 3 test pixels on screen ──
LDI r14, 0x10000          ; screen[0] = pixel (0, 0)
LDI r12, 0xFF0000          ; red
STORE r14, r12

LDI r14, 0x10001          ; screen[1] = pixel (1, 0)
LDI r12, 0x00FF00          ; green
STORE r14, r12

LDI r14, 0x10002          ; screen[2] = pixel (2, 0)
LDI r12, 0x0000FF          ; blue
STORE r14, r12

; ── Bulk copy first 10 screen pixels to RAM ──
LDI r3, 0x10000          ; source: screen base
LDI r8, 0x2000           ; dest: RAM at 0x2000
LDI r1, 10               ; count: 10 pixels
COPY_LOOP:
LOAD r13, r3              ; r13 = screen[i]
STORE r8, r13             ; ram[dest] = r13
ADDI r3, 1               ; source++
ADDI r8, 1               ; dest++
SUBI r1, 1               ; count--
CMPI r1, 0               ; compare count to 0
JNZ r6, COPY_LOOP        ; loop while count > 0

; ── Verify pixel 0 is red (0xFF0000) ──
LDI r0, 0x2000
LOAD r5, r0              ; r5 = ram[0x2000] (copied from screen[0])
CMPI r5, 0xFF0000
JNZ r6, FAIL

; ── Verify pixel 1 is green (0x00FF00) ──
LDI r0, 0x2001
LOAD r5, r0
CMPI r5, 0x00FF00
JNZ r6, FAIL

; ── Verify pixel 2 is blue (0x0000FF) ──
LDI r0, 0x2002
LOAD r5, r0
CMPI r5, 0x0000FF
JNZ r6, FAIL

; ── Verify pixels 3-9 are zero (black/empty) ──
LDI r1, 7                ; check 7 more pixels
LDI r0, 0x2003           ; start at pixel 3
ZERO_CHECK:
LOAD r5, r0
CMPI r5, 0
JNZ r6, FAIL
ADDI r0, 1
SUBI r1, 1
CMPI r1, 0
JNZ r6, ZERO_CHECK

; ── PASS: draw green diagonal at (2,2)-(4,4) ──
LDI r14, 2
LDI r12, 2
LDI r13, 0x00FF00
PSET r14, r12, r13
LDI r14, 3
LDI r12, 3
PSET r14, r12, r13
LDI r14, 4
LDI r12, 4
PSET r14, r12, r13
JMP DONE

; ── FAIL: draw red diagonal at (2,2)-(4,4) ──
FAIL:
LDI r14, 2
LDI r12, 2
LDI r13, 0xFF0000
PSET r14, r12, r13
LDI r14, 3
LDI r12, 3
PSET r14, r12, r13
LDI r14, 4
LDI r12, 4
PSET r14, r12, r13

DONE:
HALT

...
; ===== Constants =====
LDI r9, 1               ; constant 1
LDI r3, 64              ; TILES per axis
LDI r7, 4               ; TILE_SIZE pixels
LDI r12, 0xFFB          ; key bitmask port
LDI r10, 0x7800         ; camera_x address
LDI r2, 0x7801         ; camera_y address
LDI r6, 0x7802         ; frame_counter address
...
LOAD r17, r6
ADD r17, r9
STORE r6, r17          ; frame_counter++

; --- Read camera position ---
LOAD r4, r10           ; r4 = camera_x
LOAD r14, r2           ; r14 = camera_y
...
LDI r19, 8
CMP r18, r19
BGE r13, pre_tint_warm  ; zone >= 8 -> east
LDI r19, 0x0808
MUL r18, r19
...
LOAD r22, r6           ; r22 = frame_counter (load once for whole frame)
LDI r11, 0               ; ty = 0
...
render_y:
  ...
  render_x:
    ...
    ; World coordinates
    MOV r5, r4
    ADD r5, r1           ; r5 = world_x = camera_x + tx
    MOV r15, r14
    ADD r15, r11           ; r15 = world_y = camera_y + ty

    ; ---- Coarse hash for contiguous biomes ----
    ; Zone size = 8 tiles (>> 3) = 32x32 pixel biome patches
    MOV r8, r5
    LDI r18, 3
    SHR r8, r18          ; r8 = world_x >> 3 (coarse_x)
    LDI r18, 99001
    MUL r8, r18          ; r8 = coarse_x * 99001

    MOV r0, r15
    LDI r18, 3
    SHR r0, r18          ; r0 = world_y >> 3 (coarse_y)
    LDI r18, 79007
    MUL r0, r18          ; r0 = coarse_y * 79007

    XOR r8, r0           ; r8 = coarse_hash

    ; Mix: multiply by a large prime to spread bits into upper positions
    LDI r18, 1103515245
    MUL r8, r18          ; r8 = coarse_hash * mixing_prime

    ; Extract top 5 bits: biome type 0..31
    LDI r18, 27
    SHR r8, r18          ; r8 = biome_type (0..31)

    ; ---- Fine hash for structure placement ----
    MOV r0, r5
    LDI r18, 374761393
    MUL r0, r18          ; r0 = world_x * big_prime
    MOV r21, r15
    LDI r18, 668265263
    MUL r21, r18         ; r21 = world_y * big_prime
    XOR r0, r21          ; r0 = fine_hash
    ...
    LDI r18, 2
    CMP r8, r18
    BLT r13, struct_water       ; 0-1 water
    LDI r18, 3
    CMP r8, r18
    BLT r13, struct_land        ; 2 beach hut
    LDI r18, 5
    CMP r8, r18
    BLT r13, struct_desert      ; 3-4 desert cactus
    LDI r18, 6
    CMP r8, r18
    BLT r13, struct_oasis       ; 5 oasis palm
    LDI r18, 8
    CMP r8, r18
    BLT r13, struct_land        ; 6-7 grass hut
    LDI r18, 10
    CMP r8, r18
    BLT r13, struct_swamp       ; 8-9 swamp lily
    LDI r18, 12
    CMP r8, r18
    BLT r13, struct_land        ; 10-11 forest hut
    LDI r18, 13
    CMP r8, r18
    BLT r13, struct_mushroom    ; 12 mushroom cap
    LDI r18, 15
    CMP r8, r18
    BLT r13, struct_mountain    ; 13-14
    LDI r18, 16
    CMP r8, r18
    BLT r13, struct_tundra      ; 15 tundra frost
    LDI r18, 18
    CMP r8, r18
    BLT r13, struct_lava        ; 16-17 lava ember
    LDI r18, 19
    CMP r8, r18
    BLT r13, struct_volcanic    ; 18 volcanic vent
    LDI r18, 22
    CMP r8, r18
    BLT r13, struct_snow        ; 19-21 snow crystal
    LDI r18, 23
    CMP r8, r18
    BLT r13, struct_coral       ; 22 coral anemone
    LDI r18, 24
    CMP r8, r18
    BLT r13, struct_ruins       ; 23 ruins pillar
    LDI r18, 26
    CMP r8, r18
    BLT r13, struct_crystal     ; 24-25 crystal cluster
    LDI r18, 27
    CMP r8, r18
    BLT r13, struct_ash         ; 26 ash geyser
    LDI r18, 29
    CMP r8, r18
    BLT r13, struct_dead        ; 27-28 deadlands bone
    LDI r18, 31
    CMP r8, r18
    BLT r13, struct_biolum      ; 29-30 bioluminescent spore
...
struct_land:
    ...
    MOV r20, r22
    ADD r20, r5           ; frame_counter + world_x
    LDI r18, 3
    AND r20, r18          ; & 3 -> 0..3
...
struct_lava:
    ...
    MOV r20, r22
    ADD r20, r15           ; frame_counter + world_y
    LDI r18, 7
    AND r20, r18          ; & 7 -> 0..7
...
struct_crystal:
    ...
    MOV r20, r22
    XOR r20, r5           ; frame_counter ^ world_x
    LDI r18, 0xF
...
do_rect:
    ...
    LDI r20, 1
    CMP r18, r20
    JZ r13, pat_vert        ; pattern 1 = vertical accent
    LDI r20, 2
    CMP r18, r20
    JZ r13, pat_center      ; pattern 2 = center dot
    LDI r20, 3
    CMP r18, r20
    JZ r13, pat_corner      ; pattern 3 = corner spark
    LDI r20, 4
    CMP r18, r20
    JZ r13, pat_diag_bl     ; pattern 4 = diagonal backslash
    LDI r20, 5
    CMP r18, r20
    JZ r13, pat_diag_fw     ; pattern 5 = diagonal forward slash
    LDI r20, 6
    CMP r18, r20
    JZ r13, pat_topedge     ; pattern 6 = top edge
...
pat_center:
    ...
    AND r21, r17          ; fine_hash & 1
    JZ r21, pat_c_draw
    ADD r18, r9           ; x = screen_x + 2
    ADD r20, r9           ; y = screen_y + 2
pat_c_draw:
    PSET r18, r20, r19
    JMP next_tile
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 4
    MUL r5, r18          ; r5 = mx * 4
    ADD r5, r4          ; r5 = world_x

    MOV r15, r11
    LDI r18, 4
    MUL r15, r18          ; r15 = my * 4
    ADD r15, r14          ; r15 = world_y

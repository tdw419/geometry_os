...
;   r12  = 1
;   r6  = 128 (screen center)
;   r2 = star index (0-127)
;   r7 = 128 (star count)
;   r15 = 0x4000 (star pool base)
;   r8 = frame counter
;   r10 = 256 (projection scale)
;   r0 = current star base address
...
LDI r7, 128
LDI r15, 0x4000
LDI r10, 64             ; projection scale (smaller = wider FOV)
LDI r8, 0
...
star_loop:
  ...
  LDI r21, 256
  CMP r18, r21
  BGE r5, recycle_star    ; sx >= 256 -> off screen
  ...
  LDI r22, 2
  CMP r21, r22
  BLT r5, recycle_star_z  ; z < 2 -> recycle

  STORE r16, r21          ; save new z
  JMP next_star

recycle_star:
  ; Also recycles on z==0 from load -- need the base address in r0
recycle_star_z:
  ; New x = (RAND & 0x7F) - 64
  RAND r17
  LDI r18, 0x7F

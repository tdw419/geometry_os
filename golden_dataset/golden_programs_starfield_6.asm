...
;   r0  = 1
;   r15  = 128 (screen center)
;   r14 = star index (0-127)
;   r6 = 128 (star count)
;   r2 = 0x4000 (star pool base)
;   r5 = frame counter
;   r13 = 256 (projection scale)
;   r4 = current star base address
...
star_loop:
  ...
  LDI r21, 256
  CMP r18, r21
  BGE r11, recycle_star    ; sx >= 256 -> off screen
  ...
  LDI r22, 2
  CMP r21, r22
  BLT r11, recycle_star_z  ; z < 2 -> recycle

  STORE r16, r21          ; save new z
  JMP next_star

recycle_star:
  ; Also recycles on z==0 from load -- need the base address in r4
recycle_star_z:
  ; New x = (RAND & 0x7F) - 64
  RAND r17
  LDI r18, 0x7F

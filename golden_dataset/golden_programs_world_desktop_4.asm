...
LOAD r18, r17                 ; r18 = CMD_LEN
CMPI r18, 63
BGE r2, cmd_type_no_key       ; buffer full
LDI r19, 0x7832
...
MOV r5, r4             ; world_x = player_x
MOV r15, r17             ; world_y = target_y
CALL check_biome_walkable
JZ r2, up_blocked       ; r2 == 0 means not walkable
JMP no_up
up_blocked:
LDI r18, 0x7809
STORE r18, r10          ; revert player_y to original
no_up:
...
LDI r18, 0x7808
STORE r18, r17
MOV r5, r17             ; world_x = target_x
MOV r15, r10             ; world_y = player_y
CALL check_biome_walkable
JZ r2, left_blocked
JMP no_left
left_blocked:
...
ent_update_loop:
  CMP r26, r21
  BGE r2, ent_update_done

  ; Load entity fields
  MOV r22, r20
  LOAD r5, r22             ; world_x (r5 for biome check)
  ADDI r22, 1
...
ent_not_program:
  ...
  LDI r22, 1
  CMP r17, r22
  JZ r2, ent_ai_wander     ; wanderer
  LDI r22, 2
  CMP r17, r22
  JZ r2, ent_ai_guard      ; guard
  LDI r22, 3
  CMP r17, r22
  JZ r2, ent_ai_animal     ; animal (flee)
  LDI r22, 4
  CMP r17, r22
  JZ r2, ent_ai_ghost      ; ghost
  LDI r22, 5
  CMP r17, r22
  JZ r2, ent_ai_area_agent ; area_agent
...
ent_agent_not_idle:
  LDI r22, 1
  CMP r17, r22
  JZ r2, ent_agent_state_moving
  LDI r22, 3
  CMP r17, r22
  JZ r2, ent_agent_state_inside
  LDI r22, 5
  CMP r17, r22
  JZ r2, ent_agent_state_returning
...
ent_agent_state_moving:
  ...
  MOV r22, r23
  SUB r22, r5               ; dx = target_x - entity_x
  MOV r17, r24
  ...
  LDI r23, 2
  CMP r22, r23
  BLT r2, ent_agent_check_dy  ; dx < 2, check dy
  JMP ent_agent_move_dx       ; dx >= 2, move
ent_agent_dx_neg:
  NEG r22                   ; |dx|
  LDI r23, 2
  CMP r22, r23
  BLT r2, ent_agent_check_dy  ; |dx| < 2
...
ent_agent_check_dy:
  ...
  LDI r23, 2
  CMP r17, r23
  BLT r2, ent_agent_arrived   ; dy < 2, arrived!
...
ent_agent_arrived:
  ...
  LDI r23, 4
  CMP r17, r23
  BGE r2, ent_agent_arrived_done  ; mailbox full
  ADDI r17, 1
...
ent_agent_state_returning:
  ...
  LDI r24, 2
  CMP r22, r24
  BLT r2, ent_agent_home_check_dy
...
ent_do_biome_check:
  ...
  MOV r5, r22              ; target coords for biome check
  MOV r15, r23
  CALL check_biome_walkable
  JZ r2, ent_move_blocked  ; not walkable, skip move
...
MOV r18, r10
SUB r18, r17
STORE r9, r18            ; camera_y = player_y - tiles/2
...
LOAD r17, r12           ; r17 = frame_counter
LDI r18, 0xFF
...
no_claim:
    ...
    MOV r1, r15
    LDI r18, 3
    SHR r11, r18          ; r11 = world_x >> 3
    SHR r1, r18          ; r1 = world_y >> 3
    LDI r18, 99001
    MUL r11, r18          ; r11 = x_hash
    LDI r18, 79007
    MUL r1, r18          ; r1 = y_hash
    XOR r11, r1           ; r11 = coarse_hash
    LDI r18, 1103515245
    MUL r11, r18          ; r11 = mixed_hash
...
no_yblend:
    ...
    ; r1 = world_x * 374761393 XOR world_y * 668265263
    ...
    LDI r18, 668265263
    MUL r21, r18
    XOR r1, r21           ; r1 = fine_hash (THE SEED, 32 bits of goodness)
...
water_checked:
    ...
    LOAD r20, r12            ; frame_counter
    ADD r20, r5              ; fc + world_x (cheap position variation)
...
tile_done:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r2, contour_clr_skip   ; detail < 2 → skip
    ...
    LDI r18, 28
    SHR r11, r18
    ANDI r11, 7              ; r11 = current_elevation (0-7)
...
contour_done:
    ...
    LDI r18, 0x7807
    STORE r18, r11             ; RAM[0x7807] = current_elevation
    JMP contour_after
...
contour_after:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r2, no_tree        ; detail < 2 → skip trees
...
sky_night:
  LDI r11, 0x050510       ; top: near-black with hint of blue
  LDI r1, 0x0A0A30       ; horizon: dark navy
  JMP sky_draw

sky_dawn:
  LDI r11, 0x101040       ; top: deep blue-purple
  LDI r1, 0xCC6600       ; horizon: warm orange
  JMP sky_draw

sky_day:
  LDI r11, 0x1844AA       ; top: medium blue
  LDI r1, 0x5599DD       ; horizon: light sky blue
  JMP sky_draw

sky_dusk:
  LDI r11, 0x0C0820       ; top: dark purple
  LDI r1, 0xDD4400       ; horizon: deep orange-red
  JMP sky_draw
...
LDI r18, 256
LDI r19, 4
RECTF r5, r15, r18, r19, r11

; Band 1 (rows 4-7): top + (horizon >> 2) = 75% top + 25% horizon
MOV r17, r1
LDI r18, 2
SHR r17, r18            ; horizon >> 2
ADD r17, r11             ; top + (horizon >> 2)
LDI r18, 256            ; width
LDI r15, 4
RECTF r5, r15, r18, r19, r17
...
LDI r17, 2
CMP r18, r17
BLT r2, mm_skip          ; detail < 2 → skip minimap
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 2
    MUL r5, r18
    ADD r5, r4          ; r5 = camera_x + mx*2

    MOV r15, r7
    LDI r18, 2
    MUL r15, r18
    ADD r15, r10          ; r15 = camera_y + my*2

    ; Coarse hash for biome (same hash as main terrain)
    MOV r11, r5
    LDI r18, 3
    SHR r11, r18          ; world_x >> 3
    LDI r18, 99001
    MUL r11, r18          ; x_hash

    MOV r1, r15
    LDI r18, 3
    SHR r1, r18          ; world_y >> 3
    LDI r18, 79007
    MUL r1, r18          ; y_hash

    XOR r11, r1           ; coarse_hash
    LDI r18, 1103515245
    MUL r11, r18          ; mixed_hash
    LDI r18, 27
    SHR r11, r18          ; biome index (0..31)
...
LDI r5, 224
LDI r15, 0
RECTF r5, r15, r19, r18, r17    ; top edge
LDI r15, 31
RECTF r5, r15, r19, r18, r17    ; bottom edge
LDI r15, 0
RECTF r5, r15, r18, r19, r17    ; left edge
LDI r5, 255
RECTF r5, r15, r18, r19, r17    ; right edge
...
LDI r17, 2
CMP r18, r17
BLT r2, player_simple   ; detail < 2 → simple marker
...
LDI r17, 2
CMP r18, r17
BLT r2, bldg_simple_dots   ; detail < 2 → simple dots
...
bldg_dot_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r2, bldg_dot_next
  MOV r29, r14
  SUB r29, r3           ; tiles - 1
  CMP r28, r29
  BGE r2, bldg_dot_next
...
bldg_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r2, bldg_next     ; off-screen left
  LDI r29, 62
  CMP r28, r29
  BGE r2, bldg_next     ; off-screen right
  ...
  LDI r18, 0
  CMP r29, r18
  BLT r2, bldg_next     ; off-screen top
  LDI r18, 58
  CMP r29, r18
  BGE r2, bldg_next     ; off-screen bottom
...
ent_render_loop:
  ...
  LDI r25, 0
  CMP r27, r25
  BLT r2, ent_render_next
  MOV r25, r14
  SUB r25, r3              ; tiles - 1
  CMP r27, r25
  BGE r2, ent_render_next
...
ent_render_not_program:
  ...
  LDI r25, 1
  CMP r17, r25
  JZ r2, ent_render_agent   ; wanderer

  LDI r25, 2
  CMP r17, r25
  JZ r2, ent_render_guard   ; guard

  LDI r25, 3
  CMP r17, r25
  JZ r2, ent_render_animal  ; animal

  LDI r25, 4
  CMP r17, r25
  JZ r2, ent_render_ghost   ; ghost

  LDI r25, 5
  CMP r17, r25
  JZ r2, ent_render_area_agent ; area_agent
...
ent_agent_color:
  ...
  MOV r25, r18
  LDI r1, 16
  SHR r25, r1         ; direction
  LDI r1, 0
  CMP r25, r1
  JNZ r2, ent_agent_not_up2
  LDI r25, 0
  ADD r25, r27             ; dx=0
  SUB r28, r3              ; dy=-1
  LDI r1, 0xFFFFFF
  PSET r25, r28, r1   ; white dot above
  ADDI r28, 1              ; restore y
  JMP ent_agent_dir_done
ent_agent_not_up2:
  LDI r1, 1
  CMP r25, r1
  JNZ r2, ent_agent_not_down2
  LDI r25, 0
  ADD r25, r27
  ADD r28, r3              ; dy=+1
  LDI r1, 0xFFFFFF
...
ent_animal_color:
  ...
  PSET r27, r28, r1        ; left ear
  ADDI r27, 1
  PSET r27, r28, r1        ; right ear
  JMP ent_render_next
...
ent_ghost_visible:
  ...
  MOV r25, r19
  LDI r1, 3
  AND r25, r1              ; 0-3 phase
  LDI r17, 0x9933FF        ; purple base
  LDI r1, 1
  CMP r25, r1
  JNZ r2, ent_ghost_not_p2
  LDI r17, 0xBB66FF        ; light purple
  JMP ent_ghost_draw
ent_ghost_not_p2:
  LDI r1, 2
  CMP r25, r1
  JNZ r2, ent_ghost_not_p3
  LDI r17, 0x7722CC        ; dark purple
  JMP ent_ghost_draw
ent_ghost_not_p3:
  LDI r1, 3
  CMP r25, r1
  JNZ r2, ent_ghost_draw
...
ent_ghost_draw:
  ...
  SUBI r28, 2              ; eye row
  LDI r1, 0xFFFFFF
  PSET r27, r28, r1        ; center eye
  SUBI r27, 1
  PSET r27, r28, r1        ; left eye
  ADDI r27, 2
  PSET r27, r28, r1        ; right eye
  JMP ent_render_next
...
mm_ent_not_program:
  LDI r25, 1
  CMP r17, r25
  JNZ r2, mm_ent_not_wanderer
  LDI r18, 0xFF8800        ; orange (wanderer)
  JMP mm_ent_dot
mm_ent_not_wanderer:
  LDI r25, 2
  CMP r17, r25
  JNZ r2, mm_ent_not_guard
  LDI r18, 0xFF2020        ; red (guard)
  JMP mm_ent_dot
mm_ent_not_guard:
  LDI r25, 3
  CMP r17, r25
  JNZ r2, mm_ent_not_animal
...
LDI r18, 0xFFFFFFFF
CMP r17, r18
JZ r2, no_entity_tooltip   ; -1 = no entity nearby
...
LDI r18, 1
CMP r17, r18
JZ r2, ent_tooltip_wanderer
LDI r18, 2
CMP r17, r18
JZ r2, ent_tooltip_guard
LDI r18, 3
CMP r17, r18
JZ r2, ent_tooltip_animal
...
LDI r17, 2
CMP r18, r17
BGE r2, player_marker_done   ; detail >= 2 → skip (full sprite already drawn)
...
LDI r5, 128
LDI r15, 128
PSET r5, r15, r17            ; center dot
LDI r5, 127
PSET r5, r15, r17            ; left
LDI r5, 129
LDI r15, 128
PSET r5, r15, r17            ; right
LDI r5, 128
LDI r15, 127
PSET r5, r15, r17            ; up
LDI r15, 129
PSET r5, r15, r17            ; down
player_marker_done:
...
LOAD r18, r17
CMPI r18, 47               ; '/'
JNZ r2, cmd_oracle          ; not a / command → send to Oracle
...
CMPI r18, 116               ; 't'
JNZ r2, cmd_try_build
ADDI r17, 1
LOAD r18, r17
CMPI r18, 112               ; 'p'
JNZ r2, cmd_oracle          ; not /tp, fall through to oracle

; Parse first number (x) after /tp
ADDI r17, 1                  ; skip 'p'
CALL parse_next_number       ; r2 = number, r17 advanced past it
LDI r19, 0x7808
STORE r19, r2                ; player_x = parsed x

; Parse second number (y)
CALL parse_next_number
LDI r19, 0x7809
STORE r19, r2                ; player_y = parsed y
...
CMPI r18, 32
BGE r2, cmd_build_full       ; max 32 buildings
...
LDI r5, 0x7832              ; prompt = CMD_BUF text
LDI r15, 0x7873              ; response buffer
LDI r11, 895                 ; max response length
LLM r5, r15, r11
...
; Output: r2 = parsed number, r17 = advanced past number + space
; Uses r13 as accumulator (r2 is clobbered by CMPI!)
; =========================================
parse_next_number:
PUSH r31
LDI r7, 1
...
LOAD r18, r17
CMPI r18, 32                ; space
JNZ r2, pnn_digit_start
ADDI r17, 1
JMP pnn_skip

pnn_digit_start:
LDI r13, 0                   ; accumulator in r13 (NOT r2, CMPI clobbers r2)
...
pnn_done:
MOV r2, r13                  ; move result to r2 for caller
POP r31
RET

; ===== check_biome_walkable subroutine =====
; Input: r5 = world_x, r15 = world_y
; Output: r2 = 1 (walkable) or 0 (blocked)
; Preserves: r7-r1, r4-r10
; Uses: r17-r22 as temporaries
; Clobbers: r2 (CMP result), r31 (saved/restored via push/pop)
...
LDI r17, 16
CMP r21, r17
BLT r2, biome_not_lava
LDI r17, 19
CMP r21, r17
BGE r2, biome_not_lava
JMP biome_blocked

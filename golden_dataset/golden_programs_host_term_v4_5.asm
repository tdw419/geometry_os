...
; r8 = 1 (restore after earlier code clobbered it)
LDI r8, 1
...
; =========================================
; APPEND_BYTE -- append r9 to text buffer (visible chars only)
...
render:
    ...
    LDI r15, BUF         ; buf pointer
    LDI r1, 8           ; y position (start after title bar)
render_row:
...
draw_cursor:
    ...
    MUL r4, r7          ; row * 6
    LDI r10, 8
    ADD r4, r10          ; + title bar offset
    LDI r10, 2           ; width
    LDI r2, 5           ; height
    LDI r9, 0x44FF44
    RECTF r14, r4, r10, r2, r9
...
; Uses: r14-r7, r12-r1 (saved/restored around CALL)
; =========================================
check_resize:
    PUSH r31
    ...
    ; WINSYS op=6 (WINFO): r14=win_id, r8=winfo_buf_addr
    LDI r20, WIN_ID
    LOAD r14, r20
    LDI r8, WINFO_BUF
    ...
    ; Set r14=win_id, r8=WINFO_BUF, r12=6 for WINSYS r12
    WINSYS r12
    ...
    LDI r7, 2
    ADD r20, r7
    LOAD r15, r20     ; r15 = window width in pixels

    ; h is at WINFO_BUF+3
    LDI r20, WINFO_BUF
    LDI r7, 3
    ADD r20, r7
    LOAD r1, r20     ; r1 = window height in pixels
    ...
    SUB r1, r7        ; usable height
    LDI r7, 8          ; 8px per row
    DIV r1, r7        ; r1 = usable rows

    LDI r7, 6          ; 6px per column (MEDTEXT)
    DIV r15, r7        ; r15 = new_cols

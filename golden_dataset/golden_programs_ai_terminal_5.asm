...
LDI r15, 0x1A0033  ; match title bar
DRAWTEXT r1, r2, r14, r8, r15
...
check_ai_prefix:
    ...
    CMPI r3, 58         ; ':'
    JNZ r6, render_text_default
    LDI r4, 0x00FFFF  ; cyan for AI response
    JMP render_text
...
render_text_default:
    ...
    CMPI r3, 47         ; '/'
    JNZ r6, render_text
    LDI r4, 0xFFFF44  ; yellow for commands

render_text:
    ; DRAWTEXT x=0, y=r13, addr=SCRATCH, fg=r4, bg=0 (transparent)
    LDI r1, 0
    LDI r7, SCRATCH
    LDI r11, 0         ; bg = transparent
    DRAWTEXT r1, r13, r7, r4, r11
...
draw_cursor:
    ...
    MUL r2, r10           ; row * 8
    LDI r14, 16
    ADD r2, r14           ; y = 16 + row*8
    LDI r14, 6
    LDI r8, 8
    LDI r15, 0x44FF44
    RECTF r6, r2, r14, r8, r15
...
check_del:
    CMPI r15, 127
    JNZ r6, check_up_arrow
    JMP do_backspace

check_up_arrow:
    CMPI r15, 128         ; Up arrow = 0x80
    JNZ r6, check_down_arrow
    JMP do_history_up

check_down_arrow:
    CMPI r15, 129         ; Down arrow = 0x81
    JNZ r6, do_char
    JMP do_history_down
...
do_char:
    ...
    LOAD r2, r20
    LDI r14, COLS
    MUL r2, r14           ; r2 = row * COLS
    LDI r20, CUR_COL
    LOAD r6, r20
    ADD r2, r6           ; r2 = row*COLS + col
    LDI r20, BUF
...
do_enter:
    ...
    LOAD r22, r20
    CMPI r22, 47         ; '/'
    JNZ r6, send_to_llm  ; not a command, send to LLM

    ; Check /help
    ADD r20, r1
    LOAD r22, r20
    CMPI r22, 104        ; 'h'
    JNZ r6, try_clear_cmd
    ADD r20, r1
    LOAD r22, r20
    CMPI r22, 101        ; 'e'
    JNZ r6, try_clear_cmd
...
skip_history:
    ...
    ; r6 = response length -- add to CTX_CHARS
    LDI r20, CTX_CHARS
    LOAD r0, r20
    ADD r0, r6
    STORE r20, r0
...
wrap_loop:
    ...
    LDI r3, 0
    CMP r6, r3
    JZ r6, wrap_line_done
    LDI r3, 10
    CMP r6, r3
    JNZ r6, wrap_no_newline
...
run_check_zero:
    CMP r6, r3
    JZ r6, run_failed_empty

    ; Success -- r6 = word count. Park it in RUN_PENDING and wait for /yes.
...
cmd_yes:
    ...
    ; SPAWNC r0, r5 -- r0=start addr, r5=caps list addr
    LDI r0, 0x1000
    LDI r5, SANDBOX_CAPS
    SPAWNC r0, r5
    ...
    LDI r3, 0xFFFFFFFF
    CMP r6, r3
    JZ r6, yes_spawn_failed

    ; Success -- child PID in r6
...
cmd_focus:
    ...
    LOAD r22, r20
    CMPI r22, 0
    JZ r6, focus_show     ; bare "/focus" with no arg -> show current

    ; Skip spaces between /focus and arg
focus_skip_space:
    CMPI r22, 32          ; space
    JNZ r6, focus_parse_arg
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 0
    JNZ r6, focus_skip_space
    JMP focus_show        ; trailing spaces only -> show current

focus_parse_arg:
    ; Check for "off" -> clear focus
    CMPI r22, 111         ; 'o'
    JNZ r6, focus_try_hex
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 102         ; 'f'
    JNZ r6, focus_try_hex
...
focus_hex_check:
    ...
    CMPI r22, 48
    BLT r6, focus_hi_not_digit   ; r22 < 48, not '0'-'9'
    CMPI r22, 58
    BGE r6, focus_hi_not_digit   ; r22 >= 58, past '9'
    ; It IS '0'-'9'
    SUBI r22, 48
    MOV r23, r22
    SHLI r23, 4
...
focus_hi_not_digit:
    ...
    CMPI r22, 65
    BLT r6, focus_hi_not_upper   ; < 'A'
    CMPI r22, 71
    BGE r6, focus_hi_not_upper   ; > 'F'
    ; It IS 'A'-'F'
    SUBI r22, 55                 ; 'A'=65 -> 10
    MOV r23, r22
    SHLI r23, 4
...
focus_hi_not_upper:
    ...
    CMPI r22, 97
    BLT r6, focus_bad_arg
    CMPI r22, 103
    BGE r6, focus_bad_arg
    ; It IS 'a'-'f'
    SUBI r22, 87                 ; 'a'=97 -> 10
    MOV r23, r22
    SHLI r23, 4
...
focus_lo_digit_start:
    ...
    ; Check '0'-'9'
    CMPI r22, 48
    BLT r6, focus_lo_not_digit
    CMPI r22, 58
    BGE r6, focus_lo_not_digit
    SUBI r22, 48
    OR r23, r22
    JMP focus_write
...
focus_lo_not_digit:
    ; Check 'A'-'F'
    CMPI r22, 65
    BLT r6, focus_lo_not_upper
    CMPI r22, 71
    BGE r6, focus_lo_not_upper
    SUBI r22, 55
    OR r23, r22
    JMP focus_write
...
debug_skip_space:
    CMPI r22, 32
    JNZ r6, debug_parse_arg
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 0
    JNZ r6, debug_skip_space
    JMP debug_usage

debug_parse_arg:
    CMPI r22, 112
    JNZ r6, debug_try_regs
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 105
    JNZ r6, debug_try_regs
...
debug_regs_wait:
    ...
    LOAD r22, r20
    CMPI r22, 2
    JZ r6, debug_regs_got
    SUBI r3, 1
    CMPI r3, 0
    JNZ r6, debug_regs_wait
    JMP debug_timeout
...
; Input: r6 = nibble value (0-15)
; Output: r6 = ASCII char
; =========================================
hex_digit_char:
    CMPI r6, 10
    BGE r6, hdc_alpha
...
do_backspace:
    ...
    CMPI r6, 2
    JNZ r6, hk_ret       ; if col <= 2, do nothing (JNZ after CMPI means not-equal)
    ; Actually: CMPI r6, 2 sets r6 to 0 if equal. So JNZ r6 means "if not 2"
    ; We want to proceed only if r6 > 2
    LDI r1, 1
    LDI r20, CUR_COL
    LOAD r6, r20
    CMPI r6, 2
    JZ r6, hk_ret        ; at prompt, do nothing
...
; Output: r6 = length (does NOT clobber r20)
; Clobbers: r6, r5, r13
; =========================================
strlen_prompt_buf:
    PUSH r31
...
sp_strlen_loop:
    LOAD r13, r20
    JZ r13, sp_strlen_done
    ADDI r6, 1
    ADDI r20, 1
    JMP sp_strlen_loop
sp_strlen_done:
    MOV r20, r5         ; restore r20
    POP r31
    RET
...
; =========================================
; UTILITY: clear_buf (r13=start, r16=count)
; =========================================
clear_buf:
    PUSH r31
    PUSH r13
...
check_watchdog:
    ...
    STORE r20, r6
    CMPI r6, 0
    JNZ r6, cw_done     ; Not time to check yet
    ...
    LDI r23, 0xDB9900
    CMP r22, r23
    JNZ r6, cw_done     ; No child active
    ...
    LOAD r23, r20
    CMP r22, r23
    JZ r6, cw_flatline   ; Heartbeat hasn't changed!
...
cw_flatline:
    ...
    ADDI r6, 1
    STORE r20, r6
    CMPI r6, 3          ; 3 strikes (3 seconds of flatline)
    BGE r6, trigger_heal
    JMP cw_done
...
heal_no_source:
    ...
    LDI r3, 0xFFFFFFFF
    CMP r6, r3
    JZ r6, heal_asm_failed

    LDI r3, 0
    CMP r6, r3
    JZ r6, heal_asm_empty
...
; =========================================
; WRITE_HEX_TO_BUF -- write 8-char hex of r6 to buffer at r19
; Input: r6 = value, r19 = buffer position
; Advances r19 by 8. Uses r2, r14 as scratch.
; =========================================
write_hex_to_buf:
    PUSH r31
    PUSH r6
...
; Input: r6 = number (capped at 9999)
...
u2d_notcap:
    ...
    ; Skip leading zeros.
    LDI r3, 0            ; digit count (suppressed leading zeros)
    LDI r12, 1000

    ; Thousands digit
    LDI r3, 0
    MOV r3, r6
    DIV r3, r12           ; r3 = n / 1000
    JZ r3, u2d_no_thou
    ADDI r3, 48          ; '0' + digit
    STORE r20, r3
    ADDI r20, 1
u2d_no_thou:
    ...
    DIV r3, r12           ; r3 = n / 100
    LDI r12, 10
    MOD r3, r12           ; r3 = (n/100) % 10 = hundreds digit
...
u2d_skip_hun:
    ...
    LDI r12, 10
    DIV r3, r12           ; r3 = n / 10
    MOD r3, r12           ; r3 = (n/10) % 10 = tens digit
...
history_save:
    ...
    LOAD r6, r20
    CMPI r6, 0
    JZ r6, hist_save_done  ; empty command, skip

    ; Load history count
    LDI r20, CMD_HIST_COUNT
    LOAD r3, r20          ; r3 = count
    ...
    SUBI r3, 1
    LDI r12, 8
    MOD r3, r12           ; r3 = last entry index
    LDI r12, 42
    MUL r3, r12           ; r3 = byte offset of last entry
    ADDI r3, CMD_HIST_BASE ; r3 = address of last entry
    CALL strcmp_scratch    ; sets r6 = 0 if equal
    JZ r6, hist_save_done ; duplicate, skip
...
hist_not_dup:
    ...
    MOV r12, r3
    LDI r10, 8
    MOD r12, r10           ; r12 = write index

    ; Compute dest address = CMD_HIST_BASE + write_index * 42
    LDI r9, 42
    MUL r12, r9
    ADDI r12, CMD_HIST_BASE
...
; =========================================
; STRCMP_SCRATCH - Compare SCRATCH with string at r3
; Returns r6 = 0 if equal, nonzero otherwise
; Clobbers: r6, r12, r10
; =========================================
strcmp_scratch:
    LDI r12, SCRATCH
strcmp_loop:
    ...
    CMP r6, r10
    JNZ r6, strcmp_neq
    JZ r6, strcmp_eq     ; both null = equal
    ADDI r12, 1
    ADDI r3, 1
    JMP strcmp_loop
strcmp_neq:
...
do_history_up:
    ...
    LDI r20, CMD_HIST_COUNT
    LOAD r3, r20
    JZ r3, hist_up_done  ; no history

    ; Load current nav index
    LDI r20, CMD_HIST_NAV
    LOAD r12, r20          ; r12 = current nav index
    ...
    SUBI r10, 1
    CMP r12, r10
    BGE r6, hist_up_done  ; already at oldest

    ; If nav index is 0, save current prompt line first
    JNZ r12, hist_up_recall
...
hist_up_recall:
    ...
    MOV r10, r3           ; r10 = count
    SUBI r10, 1           ; r10 = count - 1
    SUB r10, r12           ; r10 = count - 1 - nav_index
    LDI r9, 8
    MOD r10, r9           ; r10 = wrapped index
    LDI r9, 42
    MUL r10, r9           ; r10 = byte offset
...
hist_up_copy:
    CMP r17, r9
    BGE r6, hist_up_copy_done
    LOAD r6, r10
    JZ r6, hist_up_copy_done  ; stop at null terminator
    STORE r20, r6
    ADDI r10, 1
    ADDI r20, 1
    ADDI r17, 1
...
do_history_down:
    ...
    LDI r20, CMD_HIST_NAV
    LOAD r12, r20
    JZ r12, hist_down_done  ; already at current input
...
hist_dn_copy:
    CMP r17, r9
    BGE r6, hist_dn_copy_done
    LOAD r6, r10
    JZ r6, hist_dn_copy_done
    STORE r20, r6
    ADDI r10, 1
    ADDI r20, 1
    ADDI r17, 1
...
clear_prompt_line:
    ...
    LOAD r6, r20
    LDI r12, COLS
    MUL r6, r12           ; r6 = row * COLS
    LDI r12, PROMPT_BUF
    ADD r12, r6           ; r12 = &PROMPT_BUF[row * COLS]

...
main_loop:
    ...
    LDI r9, 1
    ADD r9, r5          ; r5 = tick counter (reuse frame count)
    LDI r14, 0xFF4444
    ANDI r14, 0xFF
    SHLI r14, 16
    ORI r14, 0x4444FF
    ...
    LDI r13, 20
    LDI r3, 20
    RECTF r10, r2, r13, r3, r14

    ; ── Check for debug commands ──
    LDI r20, DEBUG_STATUS
    LOAD r22, r20
    CMPI r22, 1
    JZ r14, debug_handle_command  ; r14=0 means equal (status==1)
...
debug_handle_command:
    ...
    ; Save r14 because CMPI clobbers it
    PUSH r14

    ; Command 1 = dump registers
    CMPI r22, 1
    JNZ r14, try_cmd_ram_read

    ; Restore r14 (the actual register value) before dumping
    POP r14

    ; Dump registers to response buffer
    LDI r20, DEBUG_RESPONSE
    STORE r20, r14       ; r14 (real value)
    ADDI r20, 1
    LDI r23, 1          ; skip r14, start at r10
    STORE r20, r10       ; r10
    ADDI r20, 1
    STORE r20, r2       ; r2
    ADDI r20, 1
    ...
    STORE r20, r15       ; r15
    ADDI r20, 1
    ; r0-r31: just store zeros for now (can't read all regs easily)
    LDI r23, 0
    LDI r24, 23         ; remaining regs (r0-r31 = 23)
dump_loop:
    CMPI r24, 0
    JZ r14, dump_done
...
try_cmd_ram_read:
    POP r14              ; clean up pushed r14 from command dispatch
    ; Command 2 = read RAM at address
    CMPI r22, 2
    JNZ r14, try_cmd_ping

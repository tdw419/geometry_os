; DESCRIPTION: This GeOS assembly code implements a basic multi-line text editor with features such as typing, backspace, newline insertion, cursor navigation (using arrow keys), line numbers, a title bar, blinking cursor, and character/line count display. The editor operates within a 256x256 pixel screen area, using a RAM-based buffer for storing text content and maintaining state information like cursor position and blink counter.

; notepad.asm -- Multi-line Text Editor for Geometry OS
;
; Proves: TEXT rendering, IKEY keyboard input, cursor management,
;         RECTF UI elements, RAM-based state, FRAME loop, HITSET.
;
; Features:
;   - Type text characters (printable ASCII 32-126)
;   - Backspace to delete characters
;   - Enter to insert new lines
;   - Arrow keys to navigate cursor (left/right/up/down)
;   - Line numbers on the left margin
;   - Title bar with "GeoPad" header
;   - Blinking cursor indicator
;   - Character and line count display
;
; RAM Layout:
;   0x4000-0x5AFF  Text buffer (48*32 = 1536 u32 cells, row-major)
;                   48 cols x 32 rows of editable text
;   0x6000         Cursor column (0-47)
;   0x6001         Cursor row (0-31)
;   0x6002         Total lines used (count of non-empty lines)
;   0x6003         Blink counter (for cursor blink)
;   0x6100-0x6130  Scratch buffer for rendering one line (48 chars + null)
;   0x6200-0x6230  Scratch buffer for line number text (4 chars + null)
;
; Key codes:
;   8  = Backspace
;   13 = Enter
;   37 = Left arrow
;   38 = Up arrow
;   39 = Right arrow
;   40 = Down arrow
;   32-126 = Printable ASCII
;
; Screen layout (256x256):
;   Row 0-15:    Title bar (dark blue background)
;   Row 16-255:  Text editing area (dark gray background)
;   Col 0-39:    Line numbers margin (darker gray)
;   Col 42-255:  Text area

#define COLS    36         ; editable columns per line
#define ROWS    30         ; visible text rows
#define BUF     0x4000     ; text buffer start
#define CUR_COL 0x6000     ; cursor column
#define CUR_ROW 0x6001     ; cursor row
#define LINES   0x6002     ; total lines count
#define BLINK   0x6003     ; blink counter
#define SCRATCH 0x6100     ; line render scratch
#define LNSCR   0x6200     ; line number scratch
#define CHAR_H  8          ; character height in pixels
#define CHAR_W  6          ; character width in pixels
#define TITLE_H 16         ; title bar height
#define MARGIN_W 42        ; line number margin width

; =========================================
; INIT
; =========================================
LDI r11, 1
LDI r30, 0xFD00

; Clear text buffer to spaces (32)
LDI r20, BUF
LDI r2, 32
LDI r0, COLS
LDI r8, ROWS
clear_buf_row:
    ; clear one row (COLS spaces)
    LDI r21, 0
clear_buf_col:
    STORE r20, r2
    ADD r20, r11
    ADD r21, r11
    CMP r21, r0
    BLT r14, clear_buf_col
    ; next row
    SUB r8, r11
    JZ r8, clear_buf_done
    JMP clear_buf_row
clear_buf_done:

; Init cursor position
LDI r20, CUR_COL
LDI r3, 0
STORE r20, r3
LDI r20, CUR_ROW
STORE r20, r3

; Init lines count = 1 (start with one empty line)
LDI r20, LINES
LDI r3, 1
STORE r20, r3

; Init blink counter
LDI r20, BLINK
STORE r20, r3

; =========================================
; MAIN LOOP
; =========================================
main_loop:
    LDI r11, 1

    ; Draw everything
    CALL render

    ; Signal frame
    FRAME

    ; Read keyboard
    IKEY r6
    JZ r6, main_loop

    ; Handle special keys
    CMPI r6, 8
    JZ r14, do_backspace
    CMPI r6, 13
    JZ r14, do_enter
    CMPI r6, 37
    JZ r14, do_left
    CMPI r6, 38
    JZ r14, do_up
    CMPI r6, 39
    JZ r14, do_right
    CMPI r6, 40
    JZ r14, do_down

    ; Printable character (32-126)?
    CMPI r6, 32
    BLT r14, main_loop
    CMPI r6, 127
    BGE r14, main_loop

    ; Insert character at cursor
    CALL insert_char
    JMP main_loop

; =========================================
; INSERT CHARACTER
; =========================================
insert_char:
    LDI r11, 1

    ; Compute buffer address: BUF + cur_row * COLS + cur_col
    LDI r20, CUR_ROW
    LOAD r2, r20
    LDI r0, COLS
    MUL r2, r0
    LDI r20, CUR_COL
    LOAD r8, r20
    ADD r2, r8
    LDI r20, BUF
    ADD r20, r2

    ; Store the character
    STORE r20, r6

    ; Advance cursor column
    ADD r8, r11
    CMPI r8, COLS
    BLT r14, no_wrap
    ; Wrap to next line
    LDI r8, 0
    LDI r20, CUR_COL
    STORE r20, r8
    ; Advance row
    LDI r20, CUR_ROW
    LOAD r2, r20
    ADD r2, r11
    CMPI r2, ROWS
    BLT r14, no_wrap_row_limit
    LDI r2, ROWS
    SUB r2, r11
no_wrap_row_limit:
    STORE r20, r2
    ; Update lines count if needed
    CALL update_lines
    RET
no_wrap:
    LDI r20, CUR_COL
    STORE r20, r8
    RET

; =========================================
; DO BACKSPACE
; =========================================
do_backspace:
    LDI r11, 1

    ; Load cursor position
    LDI r20, CUR_COL
    LOAD r2, r20
    LDI r20, CUR_ROW
    LOAD r0, r20

    ; If col > 0, just move back and clear
    CMPI r2, 0
    JZ r2, bs_at_line_start

    ; Move cursor left
    SUB r2, r11
    LDI r20, CUR_COL
    STORE r20, r2

    ; Clear character at new position
    CALL clear_cursor_pos
    JMP main_loop

bs_at_line_start:
    ; At start of line -- if row > 0, join with previous line
    CMPI r0, 0
    JZ r0, bs_done

    ; Move to end of previous line
    SUB r0, r11
    LDI r20, CUR_ROW
    STORE r20, r0

    ; Find last non-space character on previous line
    LDI r8, COLS
    SUB r8, r11           ; start from last col
find_end:
    ; Compute buf addr for (row, col)
    LDI r20, CUR_ROW
    LOAD r2, r20
    LDI r7, COLS
    MUL r2, r7
    ADD r2, r8
    LDI r20, BUF
    ADD r20, r2
    LOAD r15, r20        ; load char
    CMPI r15, 32
    JNZ r14, found_end   ; non-space = end of content
    CMPI r8, 0
    JZ r8, found_end     ; at col 0 = empty line
    SUB r8, r11
    JMP find_end

found_end:
    ADD r8, r11           ; cursor goes one past last char
    LDI r20, CUR_COL
    STORE r20, r8
bs_done:
    JMP main_loop

; =========================================
; DO ENTER
; =========================================
do_enter:
    LDI r11, 1

    ; Load cursor position
    LDI r20, CUR_ROW
    LOAD r2, r20
    LDI r20, CUR_COL
    LOAD r0, r20

    ; If at last row, don't advance
    LDI r8, ROWS
    SUB r8, r11
    CMP r2, r8
    BGE r14, enter_done

    ; Move to start of next row
    ADD r2, r11
    LDI r20, CUR_ROW
    STORE r20, r2

    ; Clear cursor column
    LDI r0, 0
    LDI r20, CUR_COL
    STORE r20, r0

    ; Clear the new line to spaces
    LDI r20, CUR_ROW
    LOAD r2, r20
    LDI r7, COLS
    MUL r2, r7
    LDI r20, BUF
    ADD r20, r2
    LDI r21, 0
clear_new_line:
    LDI r15, 32
    STORE r20, r15
    ADD r20, r11
    ADD r21, r11
    CMPI r21, COLS
    BLT r14, clear_new_line

    ; Update lines count
    CALL update_lines
enter_done:
    JMP main_loop

; =========================================
; DO LEFT
; =========================================
do_left:
    LDI r11, 1
    LDI r20, CUR_COL
    LOAD r2, r20
    CMPI r2, 0
    JZ r2, left_done
    SUB r2, r11
    STORE r20, r2
left_done:
    JMP main_loop

; =========================================
; DO RIGHT
; =========================================
do_right:
    LDI r11, 1
    LDI r20, CUR_COL
    LOAD r2, r20
    LDI r8, COLS
    SUB r8, r11
    CMP r2, r8
    BGE r14, right_done
    ADD r2, r11
    STORE r20, r2
right_done:
    JMP main_loop

; =========================================
; DO UP
; =========================================
do_up:
    LDI r11, 1
    LDI r20, CUR_ROW
    LOAD r2, r20
    CMPI r2, 0
    JZ r2, up_done
    SUB r2, r11
    STORE r20, r2
up_done:
    JMP main_loop

; =========================================
; DO DOWN
; =========================================
do_down:
    LDI r11, 1
    LDI r20, CUR_ROW
    LOAD r2, r20
    LDI r8, ROWS
    SUB r8, r11
    CMP r2, r8
    BGE r14, down_done
    ADD r2, r11
    STORE r20, r2
down_done:
    JMP main_loop

; =========================================
; CLEAR CHARACTER AT CURSOR POSITION
; =========================================
clear_cursor_pos:
    ; Expects r11 = 1
    ; Computes buffer addr from CUR_ROW and CUR_COL, writes space (32)
    LDI r20, CUR_ROW
    LOAD r2, r20
    LDI r0, COLS
    MUL r2, r0
    LDI r20, CUR_COL
    LOAD r8, r20
    ADD r2, r8
    LDI r20, BUF
    ADD r20, r2
    LDI r15, 32
    STORE r20, r15
    RET

; =========================================
; UPDATE LINES COUNT
; =========================================
update_lines:
    ; Count non-empty lines from bottom up
    ; Expects r11 = 1
    LDI r2, 0            ; count = 0
    LDI r0, ROWS
    SUB r0, r11           ; start from last row
count_lines:
    ; Compute buf addr for row r0
    LDI r20, 0
    ADD r20, r0
    LDI r8, COLS
    MUL r20, r8
    LDI r21, BUF
    ADD r21, r20

    ; Check if any non-space char in this row
    LDI r22, 0
check_line:
    LOAD r23, r21
    CMPI r23, 32
    JNZ r14, line_has_content
    ADD r21, r11
    ADD r22, r11
    CMPI r22, COLS
    BLT r14, check_line
    ; All spaces -- empty line, check if we already found content
    CMPI r2, 0
    JNZ r2, count_done   ; already found content above, stop
    ; No content yet, keep looking up
    SUB r0, r11
    CMPI r0, 0
    BGE r14, count_lines
    JMP count_done

line_has_content:
    ADD r2, r11
    SUB r0, r11
    CMPI r0, 0
    BGE r14, count_lines

count_done:
    CMPI r2, 0
    JNZ r2, has_lines
    LDI r2, 1            ; at least 1 line
has_lines:
    LDI r20, LINES
    STORE r20, r2
    RET

; =========================================
; RENDER
; =========================================
render:
    LDI r11, 1

    ; ── Title bar ──
    LDI r3, 0x16213E
    FILL r3

    ; Title bar background
    LDI r11, 0
    LDI r3, 0
    LDI r10, 256
    LDI r13, TITLE_H
    LDI r6, 0x16213E
    RECTF r11, r3, r10, r13, r6

    ; Title text: "GeoPad"
    LDI r20, SCRATCH
    STRO r20, "GeoPad v1.0"
    LDI r3, 0
    STORE r20, r3
    LDI r11, 8
    LDI r3, 4
    LDI r10, SCRATCH
    TEXT r11, r3, r10

    ; ── Text area background ──
    LDI r11, 0
    LDI r3, TITLE_H
    LDI r10, 256
    LDI r13, 240
    LDI r6, 0x1A1A2E
    RECTF r11, r3, r10, r13, r6

    ; ── Line number margin ──
    LDI r11, 0
    LDI r3, TITLE_H
    LDI r10, MARGIN_W
    LDI r13, 240
    LDI r6, 0x141428
    RECTF r11, r3, r10, r13, r6

    ; ── Render text lines ──
    LDI r11, 1             ; restore r11 (RECTF clobbers it to 0)
    LDI r15, 0           ; row counter
    LDI r4, TITLE_H     ; y position

render_row:
    ; Draw line number -- use r1,r9 for intermediate math to preserve r4 (y-pos)
    LDI r20, LNSCR
    ; Convert row number (r15) to 2-char decimal string
    LDI r2, 10
    LDI r12, 0
    ADD r12, r15
    ADD r12, r11           ; r12 = row+1 (1-indexed)
    DIV r12, r2           ; r12 = (row+1) / 10 (tens digit)
    LDI r3, 48
    ADD r3, r12
    STORE r20, r3         ; LNSCR[0] = '0' + tens
    ADD r20, r11
    LDI r1, 0
    ADD r1, r12          ; r1 = tens digit
    LDI r3, 10
    MUL r1, r3           ; r1 = tens * 10
    LDI r5, 0
    ADD r5, r15
    ADD r5, r11           ; r5 = row+1
    SUB r5, r1          ; r5 = (row+1) % 10 (ones digit)
    LDI r3, 48
    ADD r3, r5
    STORE r20, r3         ; LNSCR[1] = '0' + ones
    ADD r20, r11
    LDI r3, 32            ; space separator
    STORE r20, r3         ; LNSCR[2] = ' '
    ADD r20, r11
    LDI r3, 0
    STORE r20, r3         ; LNSCR[3] = null

    ; Render line number at x=2, y=r4
    LDI r11, 2
    LDI r10, LNSCR
    TEXT r11, r4, r10

    ; Render text line from buffer
    ; Copy COLS chars from buffer to scratch
    LDI r11, 1             ; restore r11 (TEXT x-pos set it to 2)
    LDI r20, SCRATCH
    LDI r21, 0
    ; Compute buffer address for this row
    LDI r22, BUF
    LDI r23, COLS
    LDI r24, 0
    ADD r24, r15
    MUL r24, r23
    ADD r22, r24

copy_line:
    LOAD r2, r22
    STORE r20, r2
    ADD r22, r11
    ADD r20, r11
    ADD r21, r11
    CMPI r21, COLS
    BLT r14, copy_line

    ; Null terminate
    LDI r3, 0
    STORE r20, r3         ; null terminate scratch

    ; Render text at x=MARGIN_W, y=r4
    LDI r11, MARGIN_W
    LDI r10, SCRATCH
    TEXT r11, r4, r10

    ; Advance y
    LDI r11, 1
    LDI r3, CHAR_H
    ADD r4, r3

    ; Next row
    ADD r15, r11
    CMPI r15, ROWS
    BLT r14, render_row

    ; ── Draw cursor ──
    CALL draw_cursor

    ; ── Status bar ──
    CALL draw_status

    RET

; =========================================
; DRAW CURSOR (blinking underline)
; =========================================
draw_cursor:
    ; Expects r11 = 1
    ; Read blink counter and toggle
    LDI r20, BLINK
    LOAD r2, r20
    ADD r2, r11
    STORE r20, r2

    ; Cursor visible on even counts (blink every 30 frames)
    LDI r0, 30
    LDI r8, 0
    ADD r8, r2
    DIV r8, r0
    MOD r8, r0
    CMPI r8, 0
    JNZ r8, cursor_done

    ; Load cursor position
    LDI r20, CUR_COL
    LOAD r2, r20
    LDI r20, CUR_ROW
    LOAD r0, r20

    ; Compute screen position
    ; x = MARGIN_W + cur_col * CHAR_W
    LDI r11, 1
    LDI r3, CHAR_W
    MUL r2, r3
    LDI r10, MARGIN_W
    ADD r2, r10           ; r2 = x

    ; y = TITLE_H + cur_row * CHAR_H + (CHAR_H - 2)
    LDI r3, CHAR_H
    MUL r0, r3
    LDI r10, TITLE_H
    ADD r0, r10           ; r0 = base y
    LDI r10, CHAR_H
    SUB r10, r11
    SUB r10, r11           ; CHAR_H - 2
    ADD r0, r10           ; r0 = y (near bottom of character cell)

    ; Draw cursor as white underline (width=CHAR_W, height=2)
    LDI r13, CHAR_W
    LDI r6, 2
    LDI r8, 0xFFFFFF
    RECTF r2, r0, r13, r6, r8

cursor_done:
    RET

; =========================================
; DRAW STATUS BAR
; =========================================
draw_status:
    ; Expects r11 = 1
    ; Status bar at bottom of screen
    LDI r11, 0
    LDI r3, 248
    LDI r10, 256
    LDI r13, 8
    LDI r6, 0x0D0D1A
    RECTF r11, r3, r10, r13, r6
    LDI r11, 1             ; restore r11 (RECTF clobbered it to 0)

    ; Show "Lines: N  Row: R  Col: C"
    LDI r20, SCRATCH
    STRO r20, "Ln:"
    ADD r20, r11

    ; Lines count (2-digit)
    LDI r25, LINES
    LOAD r2, r25
    CALL two_digit
    ADD r20, r11
    ADD r20, r11

    STRO r20, " R:"
    ADD r20, r11
    ADD r20, r11

    ; Current row (2-digit)
    LDI r25, CUR_ROW
    LOAD r2, r25
    CALL two_digit
    ADD r20, r11
    ADD r20, r11

    STRO r20, " C:"
    ADD r20, r11
    ADD r20, r11

    ; Current col (2-digit)
    LDI r25, CUR_COL
    LOAD r2, r25
    CALL two_digit
    ADD r20, r11
    ADD r20, r11

    LDI r3, 0
    STORE r20, r3

    ; Render at x=4, y=250
    LDI r11, 4
    LDI r3, 250
    LDI r10, SCRATCH
    TEXT r11, r3, r10

    RET

; =========================================
; TWO_DIGIT: Convert r2 (0-99) to 2 ASCII digits at SCRATCH
; =========================================
two_digit:
    ; r2 = value
    ; Writes 2 chars starting at address in r20, does NOT null-terminate
    ; Expects r11 = 1
    LDI r21, 10
    LDI r22, 0
    ADD r22, r2
    DIV r22, r21          ; tens
    LDI r3, 48
    ADD r3, r22
    STORE r20, r3
    ADD r20, r11

    LDI r23, 0
    ADD r23, r2
    MOD r23, r21          ; ones
    LDI r3, 48
    ADD r3, r23
    STORE r20, r3
    RET

...
po_copy_path:
    ; r15 points to the 3rd '/' -- copy from there into STATUS_CWD
...
copy_col_after_skip:
    ...
    LDI r14, 0
    LDI r3, SCRATCH
    LDI r5, 0xBBBBBB  ; light gray terminal text
    LDI r4, 0         ; no background (already cleared)
    SMALLTEXT r14, r12, r3, r5, r4
...
draw_cursor:
    ...
    LOAD r0, r20
    LDI r10, 3
    MUL r0, r10          ; x = col * 3 (3px per char)
    LDI r20, CUR_ROW
    LOAD r8, r20
    LDI r10, 6
    MUL r8, r10          ; row * 6
...
cursor_done:
    ...
    STORE r20, r0
    LDI r3, SCRATCH
    LDI r5, 0xFFFF00  ; yellow indicator
    LDI r4, 0
    SMALLTEXT r14, r8, r3, r5, r4
...
; Uses: r0-r10, r15-r12 (saved/restored around CALL)
; =========================================
check_resize:
    PUSH r31
    ...
    ; WINSYS op=6 (WINFO): r0=win_id, r14=winfo_buf_addr
    LDI r20, WIN_ID
    LOAD r0, r20
    LDI r14, WINFO_BUF
    ; WINSYS op_reg needs to hold the operation number (6)
    LDI r15, 6
    ; Call WINSYS with r15 containing op=6
    ; WINSYS reads op from register at op_reg position
    ; Set r0=win_id, r14=WINFO_BUF, r15=6 for WINSYS r15
    WINSYS r15

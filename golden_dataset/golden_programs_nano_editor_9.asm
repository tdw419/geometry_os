...
; =========================================
    ...
    LDI r2, R_BACT
    LDI r12, 0
    STORE r2, r12              ; active_buf = 0
    LDI r2, R_BCNT
    LDI r12, 1
    STORE r2, r12              ; buf_count = 1
...
hi_prompt:
    ...
    ; Add char to prompt
    LDI r2, R_PM
    LOAD r2, r2              ; prompt mode
    LDI r7, 1
    CMP r2, r7
    JZ r11, hp_add_search
...
hp_add_goto:
    ...
    LDI r12, 48                ; '0'
    SUB r4, r12                ; digit value (assumes 0-9 input)
    LDI r7, 0
    CMP r4, r7
    BLT r11, hi_done
    LDI r7, 9
    CMP r4, r7
    BGE r11, hi_done            ; not a digit
...
hp_add_search:
    ...
    LDI r2, R_SLEN
    LOAD r2, r2
    LDI r7, 40                ; max 40 chars
    CMP r2, r7
    BGE r11, hi_done

    LDI r7, R_SEARCH
    ADD r7, r2
    STORE r7, r4              ; search_buf[slen] = char
    ADD r2, r6
    LDI r7, R_SLEN
    STORE r7, r2             ; slen++
...
hp_execute:
    ...
    LDI r7, 0
    CMP r2, r7
    JZ r11, hp_repl_to_replace
    LDI r7, 1
    CMP r2, r7
    JZ r11, hp_repl_to_confirm
    LDI r7, 2
    CMP r2, r7
    JNZ r11, hp_cancel
...
; =========================================
; GET LINE LENGTH -- returns in r11
...
insert_char:
    ...
    LDI r7, R_CC
    LOAD r7, r7
    ADD r12, r7            ; r12 = cursor_pos (offset)
...
render_title:
    ...
    LDI r12, 1
    CMP r2, r12
    JZ r11, rt_nomod             ; only 1 buffer, skip indicator

    ; Show "[1]" or "[2]" in cyan
    LDI r2, R_BACT
    LOAD r2, r2
    LDI r12, 48                  ; '0'
    ADD r2, r12                 ; ascii digit
    LDI r12, 1
    ADD r2, r12                 ; 1-based
    LDI r3, R_SCR
    LDI r16, 91                  ; '['
...
render_cursor:
    ...
    LDI r12, R_SC
    LOAD r12, r12
    SUB r2, r12             ; r2 = cur_line - scroll_off
...
render_hints:
    ...
    ; Normal hints
    LDI r2, R_SCR
    STRO r2, "^S:Save ^Q:Quit ^F:Find ^G:Goto ^B:Buf ^N:New"
    LDI r2, 4
    LDI r12, 242
    LDI r7, R_SCR
    LDI r13, C_HINT
...
rh_prompt:
    ...
    LDI r12, 1
    CMP r2, r12
    JZ r11, rh_do_search_prompt

    ; Incremental search prompt (mode 3)
    LDI r12, 3
    CMP r2, r12
    JZ r11, rh_incsearch_prompt

    ; Replace prompt (mode 4)
    LDI r12, 4
    CMP r2, r12
    JZ r11, rh_replace_prompt
...
rh_replace_prompt:
    ...
    LDI r12, 0
    CMP r2, r12
    JZ r11, rh_repl_search

    LDI r12, 1
    CMP r2, r12
    JZ r11, rh_repl_replace

    ; Sub-mode 2: confirm
    LDI r2, R_SCR
    STRO r2, "Replace all? (y=yes, n=no) "
    LDI r2, 4
    LDI r12, 242
    LDI r7, R_SCR
    LDI r13, C_REPL
...
rh_goto_prompt:
    ...
    STRO r12, "                                        "
    LDI r3, R_SCR
    ADD r3, r7            ; points to end of number string
    LDI r16, 10
...
rh_ndiv:
    ...
    CMP r2, r12
    JZ r11, rh_ndone
    MOV r17, r2            ; save value before MOD
    LDI r12, 10
    MOD r2, r12            ; r2 = value % 10 (digit)
    LDI r7, 48
    ADD r2, r7            ; digit char
    STORE r3, r2
    ADD r3, r6
    LDI r12, 10
    DIV r17, r12            ; r17 = value / 10 (quotient)
    MOV r2, r17            ; r2 = quotient for next iteration
    JMP rh_ndiv

rh_ndone:
    ; Reverse the string in place
    ; r3 points past last digit, r7 = R_SCR was start
    ; Actually we need to reverse from R_SCR to r3-1
    LDI r2, R_SCR
    MOV r12, r3
    SUB r12, r6             ; r12 = last digit position
    LDI r7, 0
    CMP r2, r12
    BGE r11, rh_nshow        ; 0 or 1 digits, no reversal needed
...
do_search:
    ...
    LDI r12, 0
    CMP r2, r12
    JZ r11, ds_done          ; empty string, skip
    ...
    LDI r7, R_CC
    LOAD r7, r7
    ADD r12, r7            ; r12 = cursor offset
    ADD r12, r6             ; start one past cursor
...
ds_outer:
    ...
    MOV r14, r12
    ADD r14, r2             ; end of potential match
    CMP r14, r13
    BGE r11, ds_not_found

    ; Try matching at position r12
    LDI r3, 0               ; match offset
...
ds_find_line:
    ...
    LDI r3, LS
    ADD r3, r14
    LOAD r3, r3            ; line_starts[line]

    ; Check next line start
    MOV r16, r14
    ADD r16, r6
    CMP r16, r7
    BGE r11, ds_last_line
    ...
    SUB r12, r3             ; col = offset - line_start
    MOV r2, r14             ; line
    JMP ds_set_match
...
ds_set_match:
    ...
    LDI r3, R_CL
    STORE r3, r2           ; cursor line
    LDI r3, R_CC
    STORE r3, r12           ; cursor col
    LDI r3, R_SML
    STORE r3, r2           ; match line
    LDI r3, R_SMC
    STORE r3, r12           ; match col
...
; Uses: r6-r18, modifies buffer in-place
...
do_replace:
    ...
    LDI r12, 0
    CMP r2, r12
    JZ r11, dr_done          ; empty search, skip
...
dr_found:
    ; Found match at r14
    ADD r19, r6              ; match count++

    ; Calculate size delta: replace_len - search_len
    MOV r20, r12
    SUB r20, r2             ; delta (signed)
    ...
    MOV r16, r14
    ADD r16, r2             ; source start (after match)
    MOV r17, r14
...
dr_shrink_loop:
    ...
    LDI r7, FB
    ADD r7, r3
    LOAD r7, r7           ; r7 = char from source
    ...
    SUB r3, r6
    LDI r14, FB
    ADD r3, r14            ; r3 = dest address
    STORE r3, r7          ; store char
...
dr_grow:
    ...
    LDI r3, 2000
    CMP r13, r3
    BGE r11, dr_scan_done     ; buffer full, stop
...
do_goto:
    ...
    LDI r12, 0
    CMP r2, r12
    JZ r11, dg_done          ; goto line 0 = noop

    ; Convert to 0-based
    SUB r2, r6
    LDI r12, 0
    CMP r2, r12
    BLT r11, dg_done         ; negative, invalid

    ; Clamp to last line
    LDI r7, R_NL
    LOAD r7, r7
    SUB r7, r6             ; last valid line index
    CMP r2, r7
    BLT r11, dg_ok
    MOV r2, r7            ; clamp
...
switch_buf:
    ...
    LDI r12, 1
    CMP r2, r12
    JZ r11, sb_done              ; only 1 buffer, no switch
...
new_buf:
    ...
    LDI r12, 2
    CMP r2, r12
    BGE r11, nb_switch           ; already 2 buffers, just switch

...
; ── Parent Process (Music Player) ────────────────────────────────
    ...
    LDI r4, 0xF00          ; command flag address
...
; Input: r13=freq, r7=dur, r6=channel
; Uses: r1 (scratch)
; Preserves: r13, r7, r6, r4-r0
write_note:
    PUSH r31
wait_ready:
    LOAD r1, r4            ; r1 = command flag
    JNZ r1, wait_ready      ; wait until mixer clears previous command

    ; Write note data to shared RAM
    STORE r15, r13            ; frequency
    STORE r12, r7            ; duration
    STORE r0, r6            ; channel
    LDI r1, 1
    STORE r4, r1            ; set command flag = 1
...
mixer_loop:
    ...
    LDI r13, 2
    CMP r1, r13
    JZ r11, mixer_shutdown

    ; Command == 1: play note
    ; Read note data from shared RAM
    LOAD r13, r15            ; frequency
    LOAD r7, r12            ; duration
    LOAD r6, r0            ; channel

    ; Validate channel (0-3)
    CMP r6, r9
    BGE r11, clear_cmd       ; invalid channel, skip

    ; Store in channel table
    ; offset = channel * 3, base = 0x200
    MOV r1, r6              ; r1 = channel
    LDI r3, 3
    MUL r1, r3              ; r1 = channel * 3
    ADD r1, r5              ; r1 = 0x200 + channel * 3

    STORE r1, r13            ; freq at base+0
    MOV r3, r1
    ADD r3, r10
    STORE r3, r7            ; dur at base+1
    MOV r3, r1
    LDI r14, 2
    ADD r3, r14
    STORE r3, r10            ; active = 1 at base+2

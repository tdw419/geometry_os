...
; r2 = 1 (restore after earlier code clobbered it)
LDI r2, 1
...
; =========================================
; APPEND_BYTE -- append r13 to text buffer (visible chars only)
...
process_osc:
    ...
    LDI r3, 0     ; position in OSC_BUF
    LDI r1, 0     ; slash count
po_find_path:
    LDI r20, OSC_BUF
    ADD r20, r3
    LOAD r14, r20
...
po_copy_path:
    ; r3 points to the 3rd '/' -- copy from there into STATUS_CWD
    LDI r0, 0     ; cwd offset
po_cp_loop:
    LDI r20, OSC_BUF
    ADD r20, r3
...
copy_col_after_skip:
    ...
    LDI r2, 0
    LDI r5, SCRATCH
    LDI r8, 0xBBBBBB  ; light gray terminal text
    LDI r10, 0         ; no background (already cleared)
    SMALLTEXT r2, r0, r5, r8, r10
...
draw_cursor:
    ...
    LOAD r14, r20
    LDI r7, 3
    MUL r14, r7          ; x = col * 3 (3px per char)
    LDI r20, CUR_ROW
    LOAD r15, r20
    LDI r7, 6
    MUL r15, r7          ; row * 6
    LDI r9, 8
    ADD r15, r9          ; + title bar offset
    LDI r9, 2           ; width
    LDI r4, 5           ; height
    LDI r13, 0x44FF44
    RECTF r14, r15, r9, r4, r13
...
cursor_done:
    ...
    STORE r20, r14
    LDI r5, SCRATCH
    LDI r8, 0xFFFF00  ; yellow indicator
    LDI r10, 0
    SMALLTEXT r2, r15, r5, r8, r10
...
; Uses: r14-r7, r3-r0 (saved/restored around CALL)
; =========================================
check_resize:
    PUSH r31
    ...
    ; WINSYS op=6 (WINFO): r14=win_id, r2=winfo_buf_addr
    LDI r20, WIN_ID
    LOAD r14, r20
    LDI r2, WINFO_BUF
    ; WINSYS op_reg needs to hold the operation number (6)
    LDI r3, 6
    ; Call WINSYS with r3 containing op=6
    ; WINSYS reads op from register at op_reg position
    ; Set r14=win_id, r2=WINFO_BUF, r3=6 for WINSYS r3
    WINSYS r3
    ...
    LDI r7, 2
    ADD r20, r7
    LOAD r1, r20     ; r1 = window width in pixels
    ...
    SUB r0, r7        ; usable height
    LDI r7, 8          ; 8px per row
    DIV r0, r7        ; r0 = usable rows

    LDI r7, 6          ; 6px per column (MEDTEXT)
    DIV r1, r7        ; r1 = new_cols

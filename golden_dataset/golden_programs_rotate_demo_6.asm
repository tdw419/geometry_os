...
;   r11  = sprite y position (top-left of rendered area)
;   r4  = base sprite size (32x32)
;   r12  = current scale factor (32 = 1x, 64 = 2x, etc.)
;   r6  = angle (fixed-point radians, increments by 64 = ~14 degrees)
;   r5  = scratch
;   r13  = key code from IKEY
;   r15  = color (red)
;   r3  = 0 (black for clearing)
;   r0 = color (green)
;   r10 = color (blue)
;   r2 = color (yellow)
;   r7 = source x (sprite drawn at fixed position)
;   r9 = source y
;   r1 = dest x (scaled output position)
;   r16 = dest y

; ── init ──────────────────────────────────────────────────────
LDI r8, 0        ; sprite screen x (computed each frame)
LDI r11, 0        ; sprite screen y (computed each frame)
...
LDI r7, 0       ; source x (sprite drawn at top-left)
LDI r9, 0       ; source y
...
angle_inc:
  LDI r5, 64
  ADD r6, r5        ; +64 fixed-point (~14 degrees)
  JMP frame_done

angle_dec:
  LDI r5, 64
  SUB r6, r5        ; -64 fixed-point (~14 degrees)
  JMP frame_done

scale_up:
  LDI r5, 8
  ADD r12, r5        ; increase scale
  ; Clamp: max 128
  LDI r5, 128
  CMP r12, r5
  BLT r14, frame_done

...
STORE r20, r9
ADD r20, r5
STORE r20, r9       ; word2 (cols 64-79): only bits 0,8 used (cols 64,72)
...
mark_row_dirty:
    PUSH r5
    LDI r5, 31
    SUB r5, r15          ; r5 = 31 - row
    LDI r1, 1
    SHL r1, r5          ; r1 = 1 << (31 - row)
    LDI r2, DIRTY_ROWS
    LOAD r11, r2         ; r11 = current bitmap
    OR r11, r1           ; set the bit
    STORE r2, r11
    POP r5
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r2 = word_index, r15 = bit position within word
...
ctc_shift_loop:
    ...
    JMP ctc_shift_loop
ctc_shift_done:
    ; r11 = mask. Invert: r11 = ~r11 (all bits except the one to clear)
...
att_scan_loop:
    ...
    MOV r1, r15           ; r1 = col (will become bit_pos after div)
    LDI r2, 0            ; r2 = word_index
    LDI r13, 32           ; r13 = divisor
att_div_loop:
    CMPI r1, 32
    BLT r15, att_check_bit
    SUB r1, r13           ; r1 -= 32
    ADD r2, r5           ; word_index++
    JMP att_div_loop
...
apply_sgr:
    ...
    LDI r20, CSI_PARAM
    LOAD r9, r20       ; r9 = SGR code

    ; ── Extended SGR state machine ──
    LDI r20, SGR_EXTENDED
    LOAD r13, r20       ; r13 = extended state

    ; State 0x85: waiting for 256-color FG index
    CMPI r13, 0x85
    JNZ r15, sgr_ext95
    CALL sgr_256_to_rgb  ; r15 = RGB from r9 (color index)
    LDI r20, FG_COLOR
    STORE r20, r15
    LDI r20, SGR_EXTENDED
    LDI r15, 0
...
sgr_existing:
    ...
    ; r9 already loaded with SGR code from above
...
; Input:  r9 = color index (0-255)
; Output: r15 = RGB color value
; Clobbers: r15, r5, r11, r3
; Preserves: r9, r13, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r15, 232
    SUB r9, r15        ; r9 = index - 232
    LDI r15, 10
    MUL r9, r15        ; r9 = (index - 232) * 10
    LDI r15, 8
    ADD r9, r15        ; r9 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r9           ; save v
...
sgr256_cube:
    ...
    LDI r5, 8
    SHL r9, r5        ; r9 = G << 8
    OR r15, r9         ; r15 = (R << 16) | (G << 8)
    OR r15, r3         ; r15 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    LDI r20, CSI_PARAM
    LOAD r9, r20       ; r9 = mode (0, 1, or 2)

    ; Get current row
    LDI r20, CUR_ROW
    LOAD r13, r20       ; r13 = row
    LDI r7, COLS
    MUL r13, r7         ; r13 = row * COLS
    ...
    MOV r8, r0        ; r8 = end_col (exclusive)
    CMPI r9, 0
    JZ r15, cel_clear
    CMPI r9, 1
    JNZ r15, cel_mode2
...
cel_done:
    ...
    LDI r15, 1
    STORE r20, r15
    ; Mark current row as dirty (row is in r13 / COLS)
    ; Re-derive row from saved value: r13 = row * COLS, row = r13 / COLS
    ; Actually we saved CUR_ROW into r13 early, then multiplied. Re-read CUR_ROW.
    LDI r20, CUR_ROW
    LOAD r15, r20
    CALL mark_row_dirty
...
ced_check0:
    ...
    LDI r8, COLS
    MUL r12, r8
    ADD r12, r0        ; offset = row*COLS + col

    LDI r13, ROWS
    LDI r7, COLS
    MUL r13, r7         ; total cells
ced_loop0:
    CMP r12, r13
    BGE r15, ced_done
    LDI r20, BUF
...
cdl_clear:
    ...
    MOV r12, r7
    SUB r12, r9
    ADD r12, r5        ; r12 = first row to clear (ROWS - count)
    CMP r12, r13
...
cdc_count:
    ...
    MOV r15, r13
    MUL r15, r0
    MOV r13, r15         ; r13 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r12, r7        ; r12 = dst col (starting at cursor)
cdc_shift:
    ADD r15, r12
    ADD r15, r9         ; r12 + count = src col
    LDI r0, COLS
    CMP r15, r0
...
cdc_clear:
    ...
    LDI r12, COLS
    SUB r12, r9        ; r12 = first col to clear
    LDI r0, COLS
    CMP r12, r7
    BGE r15, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r12 < r7 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r12, COLS
    SUB r12, r5        ; r12 = COLS-1 (start from rightmost)
    SUB r12, r9        ; r12 = COLS-1-count (first dst position)
cic_shift:
    CMP r12, r7
    BLT r15, cic_clear
...
render_row:
    ...
    LDI r10, 0          ; x (use r10 to preserve r5)
    MOV r1, r4         ; y = 10 + row*6
    LDI r2, 256         ; w
    LDI r11, 6           ; h

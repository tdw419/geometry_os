...
mark_row_dirty:
    PUSH r8
    LDI r8, 31
    SUB r8, r15          ; r8 = 31 - row
    LDI r2, 1
    SHL r2, r8          ; r2 = 1 << (31 - row)
    LDI r12, DIRTY_ROWS
    LOAD r3, r12         ; r3 = current bitmap
    OR r3, r2           ; set the bit
    STORE r12, r3
    POP r8
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r12 = word_index, r15 = bit position within word
...
att_scan_loop:
    ...
    MOV r2, r15           ; r2 = col (will become bit_pos after div)
    LDI r12, 0            ; r12 = word_index
    LDI r6, 32           ; r6 = divisor
att_div_loop:
    CMPI r2, 32
    BLT r15, att_check_bit
    SUB r2, r6           ; r2 -= 32
    ADD r12, r8           ; word_index++
    JMP att_div_loop
...
apply_sgr:
    ...
    CMPI r6, 0x85
    JNZ r15, sgr_ext95
    CALL sgr_256_to_rgb  ; r15 = RGB from r13 (color index)
    LDI r20, FG_COLOR
    STORE r20, r15
    LDI r20, SGR_EXTENDED
    LDI r15, 0
...
; Clobbers: r15, r8, r3, r0
; Preserves: r13, r6, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r15, 232
    SUB r13, r15        ; r13 = index - 232
    LDI r15, 10
    MUL r13, r15        ; r13 = (index - 232) * 10
    LDI r15, 8
    ADD r13, r15        ; r13 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r13           ; save v
    ...
    MOV r15, r13
    SHL r15, r8        ; r15 = v << 8
    OR r15, r13         ; r15 = (v << 8) | v
    POP r8            ; r8 = v << 16
    OR r15, r8         ; r15 = (v << 16) | (v << 8) | v
    POP r13            ; restore r13
    JMP sgr256_ret
...
sgr256_cube:
    ...
    MOV r15, r8
    LDI r13, 16
    SHL r15, r13        ; r15 = R << 16
    MOV r13, r3
    LDI r8, 8
    SHL r13, r8        ; r13 = G << 8
    OR r15, r13         ; r15 = (R << 16) | (G << 8)
    OR r15, r0         ; r15 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    ; mode 1: 0..col
    ; mode 2: 0..COLS-1
    LDI r14, 0         ; r14 = start_col
    MOV r5, r11        ; r5 = end_col (exclusive)
    CMPI r13, 0
    JZ r15, cel_clear
    CMPI r13, 1
    JNZ r15, cel_mode2
...
ced_check0:
    ...
    LDI r5, COLS
    MUL r14, r5
    ADD r14, r11        ; offset = row*COLS + col

    LDI r6, ROWS
    LDI r10, COLS
    MUL r6, r10         ; total cells
ced_loop0:
    CMP r14, r6
    BGE r15, ced_done
    LDI r20, BUF
...
cil_count:
    ...
    LDI r14, ROWS
    SUB r14, r8        ; r14 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r14, r6
    BLT r15, cil_clear
...
cdl_clear:
    ...
    LDI r10, ROWS
    SUB r10, r8         ; r10 = ROWS-1
    MOV r14, r10
    SUB r14, r13
    ADD r14, r8        ; r14 = first row to clear (ROWS - count)
    CMP r14, r6
...
cdc_count:
    ...
    MOV r15, r6
    MUL r15, r11
    MOV r6, r15         ; r6 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r14, r10        ; r14 = dst col (starting at cursor)
cdc_shift:
    ADD r15, r14
    ADD r15, r13         ; r14 + count = src col
    LDI r11, COLS
    CMP r15, r11
...
cdc_clear:
    ...
    LDI r14, COLS
    SUB r14, r13        ; r14 = first col to clear
    LDI r11, COLS
    CMP r14, r10
    BGE r15, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r14 < r10 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r14, COLS
    SUB r14, r8        ; r14 = COLS-1 (start from rightmost)
    SUB r14, r13        ; r14 = COLS-1-count (first dst position)
cic_shift:
    CMP r14, r10
    BLT r15, cic_clear

    ; dst = BUF + base + r14 + count
    LDI r21, BUF
    MOV r15, r6
    ADD r21, r15
    ADD r21, r14
...
render_row:
    ...
    LDI r8, 1           ; restore r8 (clobbered by mark_row_dirty_check)
    ...
    LDI r7, 0          ; x (use r7 to preserve r8)
    MOV r2, r9         ; y = 10 + row*6
    LDI r12, 256         ; w
    LDI r3, 6           ; h

...
;   _check_result: r1==0 → store 1 (pass), else store 0 (fail)
;     Use after CMP to check equality, or directly for "expected==0"
;   _check_neq: r1!=0 → store 1 (pass), else store 0 (fail)
;     Use for "expected nonzero" checks
;   Both use r0 as the result address (set BEFORE calling)
; ═══════════════════════════════════════════════════════════════
_check_result:
    JZ r1, _cr_pass
    LDI r1, 0
...
; ═══════════════════════════════════════════════════════════════
    ...
    LDI r0, buf_a
    LDI r1, 0
    STORE r0, r1           ; null terminate after 3 'A's
    LDI r13, buf_a
    CALL strlen
    LDI r0, 0xF86
    LDI r8, 3
...
; T13: abs(-42) == 42
; ═══════════════════════════════════════════════════════════════
    LDI r13, 0xFFFFFFD6     ; -42 in 2's complement
    CALL abs
    LDI r0, 0xF8C
    LDI r8, 42
    CMP r1, r8

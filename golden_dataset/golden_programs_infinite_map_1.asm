...
LDI r9, 64              ; TILES per axis
LDI r4, 4               ; TILE_SIZE pixels
...
LOAD r17, r8
ADD r17, r10
STORE r8, r17          ; frame_counter++

; --- Read camera position ---
LOAD r6, r1           ; r6 = camera_x
LOAD r2, r15           ; r2 = camera_y
...
LDI r19, 8
CMP r18, r19
BGE r7, pre_tint_warm  ; zone >= 8 -> east
LDI r19, 0x0808
MUL r18, r19
...
LOAD r22, r8           ; r22 = frame_counter (load once for whole frame)
LDI r14, 0               ; ty = 0
...
render_y:
  ...
  render_x:
    ...
    ; World coordinates
    MOV r3, r6
    ADD r3, r11           ; r3 = world_x = camera_x + tx
    MOV r0, r2
    ADD r0, r14           ; r0 = world_y = camera_y + ty

    ; ---- Coarse hash for contiguous biomes ----
    ; Zone size = 8 tiles (>> 3) = 32x32 pixel biome patches
    MOV r12, r3
    LDI r18, 3
    SHR r12, r18          ; r12 = world_x >> 3 (coarse_x)
    LDI r18, 99001
    MUL r12, r18          ; r12 = coarse_x * 99001

    MOV r5, r0
    LDI r18, 3
    SHR r5, r18          ; r5 = world_y >> 3 (coarse_y)
    LDI r18, 79007
    MUL r5, r18          ; r5 = coarse_y * 79007

    XOR r12, r5           ; r12 = coarse_hash

    ; Mix: multiply by a large prime to spread bits into upper positions
    LDI r18, 1103515245
    MUL r12, r18          ; r12 = coarse_hash * mixing_prime

    ; Extract top 5 bits: biome type 0..31
    LDI r18, 27
    SHR r12, r18          ; r12 = biome_type (0..31)

    ; ---- Fine hash for structure placement ----
    MOV r5, r3
    LDI r18, 374761393
    MUL r5, r18          ; r5 = world_x * big_prime
    MOV r21, r0
    LDI r18, 668265263
    MUL r21, r18         ; r21 = world_y * big_prime
    XOR r5, r21          ; r5 = fine_hash
    ...
    LDI r18, 2
    CMP r12, r18
    BLT r7, struct_water       ; 0-1 water
    LDI r18, 3
    CMP r12, r18
    BLT r7, struct_land        ; 2 beach hut
    LDI r18, 5
    CMP r12, r18
    BLT r7, struct_desert      ; 3-4 desert cactus
    LDI r18, 6
    CMP r12, r18
    BLT r7, struct_oasis       ; 5 oasis palm
    LDI r18, 8
    CMP r12, r18
    BLT r7, struct_land        ; 6-7 grass hut
    LDI r18, 10
    CMP r12, r18
    BLT r7, struct_swamp       ; 8-9 swamp lily
    LDI r18, 12
    CMP r12, r18
    BLT r7, struct_land        ; 10-11 forest hut
    LDI r18, 13
    CMP r12, r18
    BLT r7, struct_mushroom    ; 12 mushroom cap
    LDI r18, 15
    CMP r12, r18
    BLT r7, struct_mountain    ; 13-14
    LDI r18, 16
    CMP r12, r18
    BLT r7, struct_tundra      ; 15 tundra frost
    LDI r18, 18
    CMP r12, r18
    BLT r7, struct_lava        ; 16-17 lava ember
    LDI r18, 19
    CMP r12, r18
    BLT r7, struct_volcanic    ; 18 volcanic vent
    LDI r18, 22
    CMP r12, r18
    BLT r7, struct_snow        ; 19-21 snow crystal
    LDI r18, 23
    CMP r12, r18
    BLT r7, struct_coral       ; 22 coral anemone
    LDI r18, 24
    CMP r12, r18
    BLT r7, struct_ruins       ; 23 ruins pillar
    LDI r18, 26
    CMP r12, r18
    BLT r7, struct_crystal     ; 24-25 crystal cluster
    LDI r18, 27
    CMP r12, r18
    BLT r7, struct_ash         ; 26 ash geyser
    LDI r18, 29
    CMP r12, r18
    BLT r7, struct_dead        ; 27-28 deadlands bone
    LDI r18, 31
    CMP r12, r18
    BLT r7, struct_biolum      ; 29-30 bioluminescent spore
...
struct_lava:
    ...
    MOV r20, r22
    ADD r20, r0           ; frame_counter + world_y
    LDI r18, 7
    AND r20, r18          ; & 7 -> 0..7
...
do_rect:
    ...
    LDI r20, 1
    CMP r18, r20
    JZ r7, pat_vert        ; pattern 1 = vertical accent
    LDI r20, 2
    CMP r18, r20
    JZ r7, pat_center      ; pattern 2 = center dot
    LDI r20, 3
    CMP r18, r20
    JZ r7, pat_corner      ; pattern 3 = corner spark
    LDI r20, 4
    CMP r18, r20
    JZ r7, pat_diag_bl     ; pattern 4 = diagonal backslash
    LDI r20, 5
    CMP r18, r20
    JZ r7, pat_diag_fw     ; pattern 5 = diagonal forward slash
    LDI r20, 6
    CMP r18, r20
    JZ r7, pat_topedge     ; pattern 6 = top edge

...
LDI r11, 4
LDI r0, SCRATCH
LDI r14, 0x00FF00  ; fg = green
LDI r8, 0x333355  ; bg = match title bar
DRAWTEXT r9, r11, r0, r14, r8
...
LDI r0, 36
LDI r14, 16
HITSET r9, r11, r0, r14, 99

; Restore r9 = 1 before writing prompt!
LDI r9, 1
...
copy_col:
    ...
    ; DRAWTEXT x=0, y=r13, addr=SCRATCH, fg=white, bg=0 (transparent)
    LDI r9, 0
    LDI r10, SCRATCH
    LDI r3, 0xCCCCCC  ; fg = light gray
    LDI r6, 0         ; bg = transparent
    DRAWTEXT r9, r13, r10, r3, r6
...
draw_cursor:
    ...
    MUL r11, r15           ; row * 8
    LDI r0, 16
    ADD r11, r0           ; y = 16 + row*8
    LDI r0, 6
    LDI r14, 8
    LDI r8, 0x44FF44
    RECTF r2, r11, r0, r14, r8
...
do_char:
    ...
    LOAD r11, r20
    LDI r0, COLS
    MUL r11, r0           ; r11 = row * COLS
    LDI r20, CUR_COL
    LOAD r2, r20
    ADD r11, r2           ; r11 = row*COLS + col
    LDI r20, BUF
...
extract_cmd:
    ...
    LDI r7, COLS
    MUL r12, r7            ; r12 = row * COLS

    ; Source starts at col 2 (skip "$ ")
    LDI r20, BUF
    ADD r20, r12           ; r20 = BUF + row*COLS
    ADD r20, r9
    ADD r20, r9           ; r20 = BUF + row*COLS + 2
...
ec_loop:
    ...
    ADD r20, r9           ; advance source
    ADD r21, r9           ; advance dest
    ADD r22, r9           ; col++
    JMP ec_loop
...
ls_copied:
    ADD r21, r9           ; skip null terminator
    CALL do_newline
    JMP ls_next
...
write_unknown:
    ...
    MUL r12, r7
    LDI r20, BUF
    ADD r20, r12           ; r20 = BUF + row*COLS (destination)
...
cmd_cat:
    ...
    ADD r21, r9
    ADD r21, r9
    ADD r21, r9           ; r21 = SCRATCH+4 (filename)
    ...
    MOV r18, r2           ; r18 = fd
    CMPI r2, 0xFFFFFFFF
    JNZ r2, cat_read      ; if not -1, file opened OK
...
cat_read:
    ...
    READ r18, r22, r23
    ; r2 = bytes read (0 = EOF, 0xFFFFFFFF = error)
    CMPI r2, 0
    JZ r2, cat_close      ; EOF or empty
    CMPI r2, 0xFFFFFFFF
    JNZ r2, cat_close     ; error
...
cat_write_loop:
    ...
    BGE r2, cat_close
    LOAD r2, r22
    JZ r2, cat_close      ; null terminator
...
cat_write_char:
    ; Write r2 (char) at current cursor position in buffer
    ...
    ADD r0, r9
    CMPI r0, COLS
    JNZ r2, cwc_done      ; branch if not equal (r2 = CMPI result)
    CALL do_newline
    JMP cwc_ret
cwc_done:
    LDI r20, CUR_COL
...
cmd_uptime:
    ...
    ADD r20, r9
    ADD r20, r9
    ADD r20, r9           ; advance past "uptime: "
    STRO r20, "running"
    CALL do_newline
    JMP dc_ret
...
cmd_run:
    ...
    ADD r21, r9       ; +2
    ADD r21, r9       ; +3
    ADD r21, r9       ; +4 = SCRATCH+4 (start of filename)
...
run_copy:
    ...
    ADD r21, r9
    ADD r22, r9
    JZ r2, run_exec   ; stop at null terminator

run_exec:
    ; EXEC takes a register pointing to the filename
    LDI r9, 0x5300
    EXEC r9            ; r2 = PID or 0xFFFFFFFF on error
    CMPI r2, 0xFFFFFFFF
    JZ r2, run_err
...
run_wait:
    WAITPID r11         ; r2 = 0 still running, 1 done
    JZ r2, run_wait
...
cmd_edit:
    ...
    ADD r21, r9       ; +3
    ADD r21, r9       ; +4
    ADD r21, r9       ; +5 = SCRATCH+5 (start of filename)
    LDI r22, 0x5300   ; dest buffer
edit_copy:
    LOAD r2, r21
    STORE r22, r2
    ADD r21, r9
    ADD r22, r9
    JZ r2, edit_fire   ; stop at null terminator
...
cmd_build:
    ...
    LDI r2, 47         ; '/'
    STORE r22, r2
    ADD r22, r9        ; r22 = 0x5309, pointing after "programs/"
    ...
    ADD r21, r9       ; +4
    ADD r21, r9       ; +5
    ADD r21, r9       ; +6 = SCRATCH+6 (start of filename)
...
build_open:
    ...
    LDI r21, 0x5300
    LDI r20, 0
    OPEN r21, r20, r20  ; fd in r2
    MOV r18, r2         ; r18 = fd
    CMPI r2, 0xFFFFFFFF
    JZ r2, build_fopen_err
    ...
    READ r18, r22, r23  ; r2 = bytes read
    MOV r19, r2         ; r19 = bytes read
...
build_pack:
    ...
    JZ r2, build_asm
    LOAD r2, r21
    STORE r22, r2       ; store the word (low byte has the char)
    ADD r21, r9
    ADD r22, r9
    SUB r24, r9
    JMP build_pack
...
do_newline:
    ...
    STORE r20, r2
    LDI r20, CUR_ROW
    LOAD r12, r20          ; r12 = current row
    ADD r12, r9            ; r12 = new row
    CMPI r12, ROWS
    BLT r2, dn_store      ; branch if new row < ROWS (r2 = CMPI result)

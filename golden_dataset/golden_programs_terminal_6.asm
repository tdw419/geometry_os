...
LDI r10, 36
LDI r0, 16
HITSET r7, r5, r10, r0, 99

; Restore r7 = 1 before writing prompt!
LDI r7, 1
...
draw_cursor:
    ...
    MUL r5, r4           ; row * 8
    LDI r10, 16
    ADD r5, r10           ; y = 16 + row*8
    LDI r10, 6
    LDI r0, 8
    LDI r1, 0x44FF44
    RECTF r11, r5, r10, r0, r1
...
extract_cmd:
    ...
    LOAD r9, r20          ; r9 = row
    LDI r14, COLS
    MUL r9, r14            ; r9 = row * COLS

    ; Source starts at col 2 (skip "$ ")
    LDI r20, BUF
    ADD r20, r9           ; r20 = BUF + row*COLS
    ADD r20, r7
    ADD r20, r7           ; r20 = BUF + row*COLS + 2
...
cmd_ls:
    ...
    LS r20                ; list files, count in r11
    CMPI r11, 0
    JZ r11, ls_empty
...
ls_copied:
    ADD r21, r7           ; skip null terminator
    CALL do_newline
    JMP ls_next
...
write_unknown:
    ...
    MUL r9, r14
    LDI r20, BUF
    ADD r20, r9           ; r20 = BUF + row*COLS (destination)
...
cmd_cat:
    ...
    ADD r21, r7
    ADD r21, r7
    ADD r21, r7           ; r21 = SCRATCH+4 (filename)
    ...
    MOV r18, r11           ; r18 = fd
    CMPI r11, 0xFFFFFFFF
    JNZ r11, cat_read      ; if not -1, file opened OK
...
cat_read:
    ...
    READ r18, r22, r23
    ; r11 = bytes read (0 = EOF, 0xFFFFFFFF = error)
    CMPI r11, 0
    JZ r11, cat_close      ; EOF or empty
    CMPI r11, 0xFFFFFFFF
    JNZ r11, cat_close     ; error
...
cat_write_loop:
    ...
    BGE r11, cat_close
    LOAD r11, r22
    JZ r11, cat_close      ; null terminator
...
cat_write_char:
    ; Write r11 (char) at current cursor position in buffer
    ...
    ADD r10, r7
    CMPI r10, COLS
    JNZ r11, cwc_done      ; branch if not equal (r11 = CMPI result)
    CALL do_newline
    JMP cwc_ret
cwc_done:
    LDI r20, CUR_COL
...
cmd_uptime:
    ...
    ADD r20, r7
    ADD r20, r7
    ADD r20, r7           ; advance past "uptime: "
    STRO r20, "running"
    CALL do_newline
    JMP dc_ret
...
cmd_run:
    ...
    ADD r21, r7       ; +2
    ADD r21, r7       ; +3
    ADD r21, r7       ; +4 = SCRATCH+4 (start of filename)
...
run_copy:
    ...
    ADD r21, r7
    ADD r22, r7
    JZ r11, run_exec   ; stop at null terminator

run_exec:
    ; EXEC takes a register pointing to the filename
    LDI r7, 0x5300
    EXEC r7            ; r11 = PID or 0xFFFFFFFF on error
    CMPI r11, 0xFFFFFFFF
    JZ r11, run_err
...
run_wait:
    WAITPID r5         ; r11 = 0 still running, 1 done
    JZ r11, run_wait
...
cmd_edit:
    ...
    ADD r21, r7       ; +3
    ADD r21, r7       ; +4
    ADD r21, r7       ; +5 = SCRATCH+5 (start of filename)
    LDI r22, 0x5300   ; dest buffer
edit_copy:
    LOAD r11, r21
    STORE r22, r11
    ADD r21, r7
    ADD r22, r7
    JZ r11, edit_fire   ; stop at null terminator
...
cmd_build:
    ...
    LDI r11, 47         ; '/'
    STORE r22, r11
    ADD r22, r7        ; r22 = 0x5309, pointing after "programs/"
    ...
    ADD r21, r7       ; +4
    ADD r21, r7       ; +5
    ADD r21, r7       ; +6 = SCRATCH+6 (start of filename)
...
build_copy:
    ...
    ADD r21, r7
    ADD r22, r7
    JZ r11, build_open   ; stop at null
...
build_open:
    ...
    LDI r21, 0x5300
    LDI r20, 0
    OPEN r21, r20, r20  ; fd in r11
    MOV r18, r11         ; r18 = fd
    CMPI r11, 0xFFFFFFFF
    JZ r11, build_fopen_err
    ...
    READ r18, r22, r23  ; r11 = bytes read
    MOV r19, r11         ; r19 = bytes read
...
build_pack:
    ...
    JZ r11, build_asm
    LOAD r11, r21
    STORE r22, r11       ; store the word (low byte has the char)
    ADD r21, r7
    ADD r22, r7
    SUB r24, r7
    JMP build_pack
...
build_asm:
    ...
    LDI r7, 0x9000      ; source address
    LDI r5, 0xC000      ; dest address for bytecode
    ASM r7, r5
...
do_newline:
    ...
    STORE r20, r11
    LDI r20, CUR_ROW
    LOAD r9, r20          ; r9 = current row
    ADD r9, r7            ; r9 = new row
    CMPI r9, ROWS
    BLT r11, dn_store      ; branch if new row < ROWS (r11 = CMPI result)

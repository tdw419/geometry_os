...
; =========================================
    ...
    LDI r3, R_BACT
    LDI r2, 0
    STORE r3, r2              ; active_buf = 0
    LDI r3, R_BCNT
    LDI r2, 1
    STORE r3, r2              ; buf_count = 1
...
; =========================================
; HANDLE INPUT
; r11 = IKEY key, r14 = arrow bitmask
; =========================================
handle_input:
    PUSH r31
    PUSH r11
...
hp_add_goto:
    ...
    LDI r2, 48                ; '0'
    SUB r11, r2                ; digit value (assumes 0-9 input)
    LDI r12, 0
    CMP r11, r12
    BLT r9, hi_done
    LDI r12, 9
...
hp_add_search:
    ...
    LDI r12, R_SEARCH
    ADD r12, r3
    STORE r12, r11              ; search_buf[slen] = char
    ADD r3, r10
    LDI r12, R_SLEN
    STORE r12, r3             ; slen++
...
; =========================================
; INSERT CHAR
; Insert the key (from r11) at cursor position
; =========================================
insert_char:
    PUSH r31
    PUSH r10
    ...
    LDI r12, R_CC
    LOAD r12, r12
    ADD r2, r12            ; r2 = cursor_pos (offset)

    ; Shift buffer right from buf_size down to cursor_pos
    LDI r4, R_BS
    LOAD r4, r4           ; r4 = buf_size (shift counter)
...
ic_shift:
    ...
    LDI r7, FB
    ADD r7, r4
    SUB r7, r10             ; source = FB + offset - 1
    LOAD r15, r7
    LDI r7, FB
    ADD r7, r4            ; dest = FB + offset
    STORE r7, r15
...
ic_write:
    ...
    LDI r7, FB
    ADD r7, r2
    STORE r7, r11           ; buf[cursor_pos] = key char
...
do_backspace:
    ...
    SUB r2, r10             ; r2 = delete_pos

    ; Shift left: copy [delete_pos+1 .. buf_size-1] to [delete_pos .. buf_size-2]
    LDI r4, R_BS
    LOAD r4, r4
    MOV r7, r2            ; r7 = current position

db_shift:
    MOV r15, r7
    ADD r15, r10             ; r15 = r7 + 1
    CMP r15, r4
    BGE r9, db_shift_done

    ; Copy buf[r7+1] to buf[r7]
    LDI r16, FB
    ADD r16, r15
    LOAD r17, r16
    LDI r16, FB
...
rc_loop:
    ...
    LDI r16, LH
    LDI r17, C_CURLN
    RECTF r7, r4, r15, r16, r17

rc_no_hl:
    ; Get line_start offset
    LDI r7, LS
    ADD r7, r12
    LOAD r7, r7            ; r7 = line_start (offset)
...
render_cursor:
    ...
    LDI r2, R_SC
    LOAD r2, r2
    SUB r3, r2             ; r3 = cur_line - scroll_off
...
rh_ndiv:
    ...
    LDI r2, 10
    MOD r3, r2            ; r3 = value % 10 (digit)
    LDI r12, 48
    ADD r3, r12            ; digit char
    STORE r15, r3
    ADD r15, r10
    LDI r2, 10
    DIV r17, r2            ; r17 = value / 10 (quotient)
...
rh_ndone:
    ...
    LDI r3, R_SCR
    MOV r2, r15
    SUB r2, r10             ; r2 = last digit position
    LDI r12, 0
    CMP r3, r2
...
do_search:
    ...
    LDI r12, R_CC
    LOAD r12, r12
    ADD r2, r12            ; r2 = cursor offset
    ADD r2, r10             ; start one past cursor
...
ds_outer:
    ...
    MOV r7, r2
    ADD r7, r3             ; end of potential match
    CMP r7, r4
    BGE r9, ds_not_found
...
ds_find_line:
    ...
    ; r2 >= line_starts[line] AND r2 < line_starts[line+1]
    CMP r2, r15
    BLT r9, ds_fl_next
    CMP r2, r17
    BGE r9, ds_fl_next
    ...
    ; Found the line
    ; col = r2 - line_starts[line]
    SUB r2, r15             ; col = offset - line_start
    MOV r3, r7             ; line
    JMP ds_set_match
...
ds_set_match:
    ...
    LDI r15, R_CL
    STORE r15, r3           ; cursor line
    LDI r15, R_CC
    STORE r15, r2           ; cursor col
    LDI r15, R_SML
    STORE r15, r3           ; match line
    LDI r15, R_SMC
    STORE r15, r2           ; match col
...
dr_found:
    ...
    ; Source: FB[r7+search_len] to FB[buf_size]
    ; Dest: FB[r7+replace_len]
    PUSH r7                 ; save match position for write loop
    MOV r16, r7
    ADD r16, r3             ; source start (after match)
    MOV r17, r7
    ADD r17, r2             ; dest start (after replace)
    ; Copy from end to start to avoid overwrite
    ; Remaining = buf_size - (r7 + search_len)
    MOV r18, r4
...
dr_after_shrink:
    ...
    STORE r16, r17
    MOV r4, r17             ; update local

    ; Update scan position (past the replacement)
    MOV r7, r16
    ADD r7, r2             ; skip past replace
    JMP dr_scan
...
dr_grow:
    ...
    PUSH r7                 ; save match position for write loop
    MOV r18, r4             ; remaining count = buf_size
    LDI r15, 0
...
dr_grow_loop:
    ...
    ; Source index = buf_size - remaining = r4 - r18
    ; Source addr = FB + r4 - r18
    MOV r15, r4
    SUB r15, r18
    LDI r12, FB
    ADD r12, r15
    ...
    ; Dest index = source_index + delta = r4 - r18 + r20
    ; Dest addr = FB + r4 - r18 + r20
    MOV r15, r4
    SUB r15, r18
    ADD r15, r20
    LDI r7, FB
...
sb_no_save:
    ...
    LOAD r3, r3
    LDI r2, 1
    XOR r3, r2                ; toggle 0 <-> 1
    LDI r2, R_BACT
    STORE r2, r3              ; save new active

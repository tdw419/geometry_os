...
pv_loop:
    ...
    MOV r8, r6
    ADD r8, r14            ; r8 = PV_PROG_BASE + PC
    LOAD r4, r8           ; r4 = pixel-instruction u32
...
pv_op_loadi:
    MOV r8, r0
    ADD r8, r12            ; r8 = PV_REG_BASE + reg
    STORE r8, r10          ; screen[PV_REG_BASE + reg] = imm
    JMP pv_next

; ── ADD dest, src: pv_regs[dest] += pv_regs[src] ─────────
pv_op_add:
    MOV r8, r0
    ADD r8, r12            ; addr of pv_regs[dest]
    LOAD r7, r8          ; r7 = pv_regs[dest]
    MOV r1, r0
    ADD r1, r10            ; addr of pv_regs[src]
    LOAD r3, r1          ; r3 = pv_regs[src]
    ADD r7, r3
    STORE r8, r7         ; pv_regs[dest] = result
    JMP pv_next
...
pv_op_store:
    MOV r8, r0
    ADD r8, r12            ; addr of pv_regs[reg]
    LOAD r7, r8          ; r7 = pv_regs[reg]
    MOV r1, r5
    ADD r1, r10            ; addr of pv_data[addr]
    STORE r1, r7         ; pv_data[addr] = pv_regs[reg]
    JMP pv_next

; ── LOAD reg, addr: pv_regs[operand1] = pv_data[operand2] ─
pv_op_load:
    MOV r8, r5
    ADD r8, r10            ; addr of pv_data[addr]
    LOAD r7, r8          ; r7 = pv_data[addr]
    MOV r1, r0
    ADD r1, r12            ; addr of pv_regs[reg]
    STORE r1, r7         ; pv_regs[reg] = pv_data[addr]
    JMP pv_next
...
pv_op_cmpi:
    ...
    ADD r8, r12            ; addr of pv_regs[reg]
    LOAD r7, r8          ; r7 = pv_regs[reg]
    ; Compare r7 with operand2 (r10)
    ; GeOS CMP sets r9: 0=equal, 1=less, 2=greater
    CMP r7, r10
    ; If r9 == 0, values are equal -> pixel flag = 0
    ; If r9 != 0, values differ -> pixel flag = 1
    CMPI r9, 0
    CMPI r9, 0              ; double-negation: r9 was 0 means equal
    JZ r9, pv_cmpi_eq
...
pv_op_jnz:
    ...
    CMPI r9, 0              ; r9=0 means flag was 0
    JZ r9, pv_next         ; flag is 0, don't jump
    MOV r14, r10             ; PC = operand2

...
LDI r13, 0xFFB          ; key bitmask port
LDI r5, 0x7800         ; camera_x address
LDI r15, 0x7801         ; camera_y address
...
zoom_0:
LDI r0, 256              ; TILES per axis (zoomed out: see 256 tiles)
LDI r12, 1                ; TILE_SIZE = 1 pixel
LDI r30, 0               ; detail_level = 0 (minimal: flat tiles only)
JMP zoom_set

zoom_1:
LDI r0, 128              ; TILES per axis (medium: 128 tiles)
LDI r12, 2                ; TILE_SIZE = 2 pixels
LDI r30, 1               ; detail_level = 1 (reduced: patterns + shimmer, no trees/contours)
JMP zoom_set

zoom_default:
LDI r0, 64               ; TILES per axis (standard: 64 tiles)
LDI r12, 4                ; TILE_SIZE = 4 pixels
...
LDI r18, 0x7808
LOAD r4, r18           ; r4 = player_x
LDI r18, 0x7809
LOAD r7, r18           ; r7 = player_y

; --- Read key bitmask ---
LOAD r16, r13           ; r16 = key bitmask
...
LOAD r18, r17                 ; r18 = CMD_LEN
CMPI r18, 63
BGE r6, cmd_type_no_key       ; buffer full
LDI r19, 0x7832
...
STORE r17, r18          ; facing = down
MOV r17, r7
ADD r17, r11             ; target_y = player_y + 1
LDI r18, 0x7809
STORE r18, r17
MOV r1, r4
MOV r14, r17
...
MOV r17, r4
SUB r17, r11             ; target_x = player_x - 1
LDI r18, 0x7808
STORE r18, r17
MOV r1, r17             ; world_x = target_x
MOV r14, r7             ; world_y = player_y
CALL check_biome_walkable
JZ r6, left_blocked
JMP no_left
left_blocked:
LDI r18, 0x7808
STORE r18, r4          ; revert player_x to original
no_left:
...
MOV r17, r4
ADD r17, r11             ; target_x = player_x + 1
LDI r18, 0x7808
STORE r18, r17
MOV r1, r17
MOV r14, r7
...
ent_not_program:
  ...
  LDI r22, 1
  CMP r17, r22
  JZ r6, ent_ai_wander     ; wanderer
  LDI r22, 2
  CMP r17, r22
  JZ r6, ent_ai_guard      ; guard
  LDI r22, 3
  CMP r17, r22
  JZ r6, ent_ai_animal     ; animal (flee)
  LDI r22, 4
  CMP r17, r22
  JZ r6, ent_ai_ghost      ; ghost
  LDI r22, 5
  CMP r17, r22
  JZ r6, ent_ai_area_agent ; area_agent
...
ent_agent_not_idle:
  LDI r22, 1
  CMP r17, r22
  JZ r6, ent_agent_state_moving
  LDI r22, 3
  CMP r17, r22
  JZ r6, ent_agent_state_inside
  LDI r22, 5
  CMP r17, r22
  JZ r6, ent_agent_state_returning
...
ent_agent_state_moving:
  ...
  LDI r23, 2
  CMP r22, r23
  BLT r6, ent_agent_check_dy  ; dx < 2, check dy
  JMP ent_agent_move_dx       ; dx >= 2, move
ent_agent_dx_neg:
  NEG r22                   ; |dx|
  LDI r23, 2
  CMP r22, r23
  BLT r6, ent_agent_check_dy  ; |dx| < 2
...
ent_agent_check_dy:
  ...
  LDI r23, 2
  CMP r17, r23
  BLT r6, ent_agent_arrived   ; dy < 2, arrived!
...
ent_agent_arrived:
  ...
  LDI r23, 4
  CMP r17, r23
  BGE r6, ent_agent_arrived_done  ; mailbox full
  ADDI r17, 1
...
ent_agent_state_returning:
  ...
  LDI r24, 2
  CMP r22, r24
  BLT r6, ent_agent_home_check_dy
...
ent_compute_target:
  ...
  JNZ r25, ent_not_up
  SUB r23, r11              ; target_y -= 1
  JMP ent_do_biome_check
ent_not_up:
  LDI r24, 1
  CMP r25, r24
  JNZ r6, ent_not_down
  ADD r23, r11              ; target_y += 1
  JMP ent_do_biome_check
ent_not_down:
  LDI r24, 2
  CMP r25, r24
  JNZ r6, ent_not_left
  SUB r22, r11              ; target_x -= 1
  JMP ent_do_biome_check
ent_not_left:
  ADD r22, r11              ; target_x += 1 (right)
ent_do_biome_check:
  ...
  MOV r14, r23
  CALL check_biome_walkable
  JZ r6, ent_move_blocked  ; not walkable, skip move
...
; r0 = tiles per axis, so center offset = r0 / 2
MOV r17, r0
LDI r18, 1
SHR r17, r18              ; r17 = tiles/2 (center offset)
MOV r18, r4
SUB r18, r17
STORE r5, r18            ; camera_x = player_x - tiles/2
MOV r18, r7
SUB r18, r17
STORE r15, r18            ; camera_y = player_y - tiles/2
...
render_y:
  ...
  MOV r26, r7
  ADD r26, r2            ; r26 = world_y for this row
  LDI r18, 3
...
no_claim:
    ...
    MOV r8, r14
    LDI r18, 3
    SHR r3, r18          ; r3 = world_x >> 3
    SHR r8, r18          ; r8 = world_y >> 3
    LDI r18, 99001
    MUL r3, r18          ; r3 = x_hash
    LDI r18, 79007
    MUL r8, r18          ; r8 = y_hash
    XOR r3, r8           ; r3 = coarse_hash
    LDI r18, 1103515245
    MUL r3, r18          ; r3 = mixed_hash
...
no_yblend:
    ...
    ; r8 = world_x * 374761393 XOR world_y * 668265263
    ...
    LDI r18, 668265263
    MUL r21, r18
    XOR r8, r21           ; r8 = fine_hash (THE SEED, 32 bits of goodness)
...
no_shimmer:
    ...
    MOV r18, r1
    SUB r18, r11              ; r18 = world_x - 1 (left neighbor)
    MOV r21, r18
    LDI r18, 3
...
pat_horiz:
    ...
    RECTF r28, r27, r12, r20, r17
    MOV r21, r27
    ADD r21, r20           ; r21 = y + 2
    RECTF r28, r21, r12, r20, r19
    JMP tile_done
...
tile_done:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r6, contour_clr_skip   ; detail < 2 → skip
    ...
    LDI r18, 28
    SHR r3, r18
    ANDI r3, 7              ; r3 = current_elevation (0-7)
...
contour_r_draw:
    ...
    MOV r18, r28
    ADD r18, r11
    ADD r18, r11               ; x = screen_x + 2 = sx + 2
    RECTF r18, r27, r11, r12, r17
...
contour_b_draw:
    ...
    MOV r18, r27
    ADD r18, r11
    ADD r18, r11               ; y = screen_y + 2 = sy + 2
    RECTF r28, r18, r12, r11, r17

contour_done:
    ; Store current elevation for next tile's right-edge contour check
    LDI r18, 0x7807
    STORE r18, r3             ; RAM[0x7807] = current_elevation
    JMP contour_after
...
contour_after:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r6, no_tree        ; detail < 2 → skip trees
...
claim_next_tile:
    ; ---- Next tile ----
    ADD r9, r11            ; tx++
    ADD r28, r12           ; screen_x += TILE_SIZE
    MOV r18, r9
    SUB r18, r0           ; tx - 64
    JZ r18, next_row
    JMP render_x
...
next_row:
    ADD r2, r11            ; ty++
    ADD r27, r12           ; screen_y += TILE_SIZE
    MOV r18, r2
    SUB r18, r0           ; ty - 64
    JZ r18, frame_end
    JMP render_y
...
sky_night:
  LDI r3, 0x050510       ; top: near-black with hint of blue
  LDI r8, 0x0A0A30       ; horizon: dark navy
  JMP sky_draw

sky_dawn:
  LDI r3, 0x101040       ; top: deep blue-purple
  LDI r8, 0xCC6600       ; horizon: warm orange
  JMP sky_draw

sky_day:
  LDI r3, 0x1844AA       ; top: medium blue
  LDI r8, 0x5599DD       ; horizon: light sky blue
  JMP sky_draw

sky_dusk:
  LDI r3, 0x0C0820       ; top: dark purple
  LDI r8, 0xDD4400       ; horizon: deep orange-red
  JMP sky_draw
...
LDI r18, 256
LDI r19, 4
RECTF r1, r14, r18, r19, r3

; Band 1 (rows 4-7): top + (horizon >> 2) = 75% top + 25% horizon
MOV r17, r8
LDI r18, 2
SHR r17, r18            ; horizon >> 2
ADD r17, r3             ; top + (horizon >> 2)
LDI r18, 256            ; width
LDI r14, 4
RECTF r1, r14, r18, r19, r17
...
LDI r17, 2
CMP r18, r17
BLT r6, mm_skip          ; detail < 2 → skip minimap
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 2
    MUL r1, r18
    ADD r1, r4          ; r1 = camera_x + mx*2

    MOV r14, r2
    LDI r18, 2
    MUL r14, r18
    ADD r14, r7          ; r14 = camera_y + my*2

    ; Coarse hash for biome (same hash as main terrain)
    MOV r3, r1
    LDI r18, 3
    SHR r3, r18          ; world_x >> 3
    LDI r18, 99001
    MUL r3, r18          ; x_hash

    MOV r8, r14
    LDI r18, 3
    SHR r8, r18          ; world_y >> 3
    LDI r18, 79007
    MUL r8, r18          ; y_hash

    XOR r3, r8           ; coarse_hash
    LDI r18, 1103515245
    MUL r3, r18          ; mixed_hash
    LDI r18, 27
    SHR r3, r18          ; biome index (0..31)
...
LDI r1, 224
LDI r14, 0
RECTF r1, r14, r19, r18, r17    ; top edge
LDI r14, 31
RECTF r1, r14, r19, r18, r17    ; bottom edge
LDI r14, 0
RECTF r1, r14, r18, r19, r17    ; left edge
LDI r1, 255
RECTF r1, r14, r18, r19, r17    ; right edge
...
LDI r17, 2
CMP r18, r17
BLT r6, player_simple   ; detail < 2 → simple marker
...
LDI r17, 2
CMP r18, r17
BLT r6, bldg_simple_dots   ; detail < 2 → simple dots
...
bldg_dot_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r6, bldg_dot_next
  MOV r29, r0
  SUB r29, r11           ; tiles - 1
  CMP r28, r29
  BGE r6, bldg_dot_next
  ...
  LOAD r27, r18
  SUB r28, r27
  MUL r28, r12           ; screen_x = dx * tile_size

  MOV r29, r14
  LDI r18, 0x7801
  LOAD r27, r18
  SUB r29, r27
  MUL r29, r12           ; screen_y = dy * tile_size
...
bldg_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r6, bldg_next     ; off-screen left
  LDI r29, 62
  CMP r28, r29
  BGE r6, bldg_next     ; off-screen right
  ...
  LDI r18, 0
  CMP r29, r18
  BLT r6, bldg_next     ; off-screen top
  LDI r18, 58
  CMP r29, r18
  BGE r6, bldg_next     ; off-screen bottom
...
LDI r17, 32
SUB r17, r1
MUL r17, r12              ; px_x = (32 - cam_x) * tile_size
MOV r18, r17             ; save px_x

LDI r17, 32
SUB r17, r14
MUL r17, r12              ; px_y = (32 - cam_y) * tile_size
...
ent_render_loop:
  ...
  LDI r25, 0
  CMP r27, r25
  BLT r6, ent_render_next
  MOV r25, r0
  SUB r25, r11              ; tiles - 1
  CMP r27, r25
  BGE r6, ent_render_next
  ...
  MUL r27, r12              ; screen_x = dx * tile_size
  MUL r28, r12              ; screen_y = dy * tile_size
...
ent_render_not_program:
  ...
  LDI r25, 1
  CMP r17, r25
  JZ r6, ent_render_agent   ; wanderer

  LDI r25, 2
  CMP r17, r25
  JZ r6, ent_render_guard   ; guard

  LDI r25, 3
  CMP r17, r25
  JZ r6, ent_render_animal  ; animal

  LDI r25, 4
  CMP r17, r25
  JZ r6, ent_render_ghost   ; ghost

  LDI r25, 5
  CMP r17, r25
  JZ r6, ent_render_area_agent ; area_agent
...
ent_agent_color:
  ...
  MOV r25, r18
  LDI r8, 16
  SHR r25, r8         ; direction
  LDI r8, 0
  CMP r25, r8
  JNZ r6, ent_agent_not_up2
  LDI r25, 0
  ADD r25, r27             ; dx=0
  SUB r28, r11              ; dy=-1
  LDI r8, 0xFFFFFF
  PSET r25, r28, r8   ; white dot above
  ADDI r28, 1              ; restore y
  JMP ent_agent_dir_done
ent_agent_not_up2:
  LDI r8, 1
  CMP r25, r8
  JNZ r6, ent_agent_not_down2
  LDI r25, 0
  ADD r25, r27
  ADD r28, r11              ; dy=+1
  LDI r8, 0xFFFFFF
...
ent_animal_color:
  ...
  PSET r27, r28, r8        ; left ear
  ADDI r27, 1
  PSET r27, r28, r8        ; right ear
  JMP ent_render_next
...
ent_ghost_visible:
  ...
  MOV r25, r19
  LDI r8, 3
  AND r25, r8              ; 0-3 phase
  LDI r17, 0x9933FF        ; purple base
  LDI r8, 1
  CMP r25, r8
  JNZ r6, ent_ghost_not_p2
  LDI r17, 0xBB66FF        ; light purple
  JMP ent_ghost_draw
ent_ghost_not_p2:
  LDI r8, 2
  CMP r25, r8
  JNZ r6, ent_ghost_not_p3
  LDI r17, 0x7722CC        ; dark purple
  JMP ent_ghost_draw
ent_ghost_not_p3:
  LDI r8, 3
  CMP r25, r8
  JNZ r6, ent_ghost_draw
...
ent_ghost_draw:
  ...
  SUBI r28, 2              ; eye row
  LDI r8, 0xFFFFFF
  PSET r27, r28, r8        ; center eye
  SUBI r27, 1
  PSET r27, r28, r8        ; left eye
  ADDI r27, 2
  PSET r27, r28, r8        ; right eye
  JMP ent_render_next
...
mm_ent_not_program:
  LDI r25, 1
  CMP r17, r25
  JNZ r6, mm_ent_not_wanderer
  LDI r18, 0xFF8800        ; orange (wanderer)
  JMP mm_ent_dot
mm_ent_not_wanderer:
  LDI r25, 2
  CMP r17, r25
  JNZ r6, mm_ent_not_guard
  LDI r18, 0xFF2020        ; red (guard)
  JMP mm_ent_dot
mm_ent_not_guard:
  LDI r25, 3
  CMP r17, r25
  JNZ r6, mm_ent_not_animal
...
LDI r18, 0xFFFFFFFF
CMP r17, r18
JZ r6, no_entity_tooltip   ; -1 = no entity nearby
...
LDI r18, 1
CMP r17, r18
JZ r6, ent_tooltip_wanderer
LDI r18, 2
CMP r17, r18
JZ r6, ent_tooltip_guard
LDI r18, 3
CMP r17, r18
JZ r6, ent_tooltip_animal
...
LDI r17, 2
CMP r18, r17
BGE r6, player_marker_done   ; detail >= 2 → skip (full sprite already drawn)
...
LDI r1, 128
LDI r14, 128
PSET r1, r14, r17            ; center dot
LDI r1, 127
PSET r1, r14, r17            ; left
LDI r1, 129
LDI r14, 128
PSET r1, r14, r17            ; right
LDI r1, 128
LDI r14, 127
PSET r1, r14, r17            ; up
LDI r14, 129
PSET r1, r14, r17            ; down
player_marker_done:
...
LOAD r18, r17
CMPI r18, 47               ; '/'
JNZ r6, cmd_oracle          ; not a / command → send to Oracle
...
CMPI r18, 116               ; 't'
JNZ r6, cmd_try_build
ADDI r17, 1
LOAD r18, r17
CMPI r18, 112               ; 'p'
JNZ r6, cmd_oracle          ; not /tp, fall through to oracle

; Parse first number (x) after /tp
ADDI r17, 1                  ; skip 'p'
CALL parse_next_number       ; r6 = number, r17 advanced past it
LDI r19, 0x7808
STORE r19, r6                ; player_x = parsed x

; Parse second number (y)
CALL parse_next_number
LDI r19, 0x7809
STORE r19, r6                ; player_y = parsed y
...
CMPI r18, 32
BGE r6, cmd_build_full       ; max 32 buildings
...
LDI r3, 895                 ; max response length
LLM r1, r14, r3
...
; Output: r6 = parsed number, r17 = advanced past number + space
; Uses r9 as accumulator (r6 is clobbered by CMPI!)
; =========================================
parse_next_number:
PUSH r31
LDI r2, 1
...
LOAD r18, r17
CMPI r18, 32                ; space
JNZ r6, pnn_digit_start
ADDI r17, 1
JMP pnn_skip

pnn_digit_start:
LDI r9, 0                   ; accumulator in r9 (NOT r6, CMPI clobbers r6)
...
pnn_done:
MOV r6, r9                  ; move result to r6 for caller
POP r31
RET

; ===== check_biome_walkable subroutine =====
; Input: r1 = world_x, r14 = world_y
; Output: r6 = 1 (walkable) or 0 (blocked)
; Preserves: r2-r8, r4-r7
; Uses: r17-r22 as temporaries
; Clobbers: r6 (CMP result), r31 (saved/restored via push/pop)
...
LDI r17, 16
CMP r21, r17
BLT r6, biome_not_lava
LDI r17, 19
CMP r21, r17
BGE r6, biome_not_lava
JMP biome_blocked

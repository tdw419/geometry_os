; DESCRIPTION: This GeOS assembly code implements a persistent taskbar at the bottom of the screen for a desktop environment. The taskbar includes a Start button, running process icons, a frame counter (clock), and supports clickable regions for menu interactions. It manages process listing, mouse input, text rendering, file listing, and maintains state in specific RAM locations.

; taskbar.asm -- Desktop Taskbar for Geometry OS
;
; Phase 72: Persistent taskbar at bottom of screen with running process
; icons, frame counter (clock), and Start button.
;
; Proves: PROCLS (process listing), MOUSEQ (mouse input), HITSET/HITQ
;         (clickable regions), DRAWTEXT (text rendering), LS (file listing).
;
; RAM Layout:
;   0x4000-0x407F  Process PID buffer (from PROCLS)
;   0x4080-0x47FF  File list buffer (from LS)
;   0x5000-0x50FF  String scratch buffers
;   0x5100         Process count
;   0x5104         File count
;   0x5108         Start menu open (0=closed, 1=open)
;   0x510C         Scroll offset for file list
;   0x5110         Selected file index
;   0x5114         Tick counter backup
;
; Taskbar: y=240..255 (bottom 16 pixels)
; Start button: x=0..47, y=240..255
; Process info: x=48..200
; Clock: x=200..255

#define PID_BUF      0x4000
#define FILE_BUF     0x4080
#define STR_BUF      0x5000
#define PROC_COUNT   0x5100
#define FILE_COUNT   0x5104
#define MENU_OPEN    0x5108
#define SCROLL_OFF   0x510C
#define SEL_INDEX    0x5110
#define TICK_BAK     0x5114
#define TBAR_Y       240
#define TBAR_H       16

; ==========================================
; INIT
; ==========================================
LDI r30, 0xFD00           ; stack pointer
LDI r13, 1
LDI r11, 0                 ; zero constant

; Clear state
LDI r20, MENU_OPEN
STORE r20, r11
LDI r20, SCROLL_OFF
STORE r20, r11
LDI r20, SEL_INDEX
STORE r20, r11

; ==========================================
; MAIN LOOP
; ==========================================
main_loop:
  ; -- Draw desktop area (above taskbar) --
  LDI r5, 0x181828
  FILL r5

  ; -- Draw taskbar background --
  LDI r5, 0x2D2D3D        ; dark blue-gray
  LDI r9, 0
  LDI r1, TBAR_Y
  LDI r12, 256
  LDI r10, TBAR_H
  RECTF r9, r1, r12, r10, r5

  ; -- Draw Start button --
  LDI r5, 0x4444AA         ; blue
  LDI r9, 1
  LDI r1, TBAR_Y
  ADDI r1, 1
  LDI r12, 46
  LDI r10, 14
  RECTF r9, r1, r12, r10, r5

  ; Write "Start" to STR_BUF
  LDI r9, STR_BUF
  LDI r5, 83               ; S
  STORE r9, r5
  ADD r9, r13
  LDI r5, 116              ; t
  STORE r9, r5
  ADD r9, r13
  LDI r5, 97               ; a
  STORE r9, r5
  ADD r9, r13
  LDI r5, 114              ; r
  STORE r9, r5
  ADD r9, r13
  LDI r5, 116              ; t
  STORE r9, r5
  ADD r9, r13
  LDI r5, 0                ; null
  STORE r9, r5

  ; Draw Start text
  LDI r9, 8
  LDI r1, TBAR_Y
  ADDI r1, 4
  LDI r12, STR_BUF
  LDI r10, 0xFFFFFF
  LDI r6, 0
  DRAWTEXT r9, r1, r12, r10, r6

  ; -- Get running processes via PROCLS --
  LDI r9, PID_BUF
  PROCLS r9
  ; r5 = process count
  LDI r20, PROC_COUNT
  STORE r20, r5

  ; -- Draw process count on taskbar as "P:N" --
  LDI r9, STR_BUF
  LDI r5, 80               ; P
  STORE r9, r5
  ADD r9, r13
  LDI r5, 58               ; :
  STORE r9, r5
  ADD r9, r13
  ; Convert process count to ASCII digit
  LDI r20, PROC_COUNT
  LOAD r20, r20
  LDI r5, 48               ; 0
  ADD r5, r20
  STORE r9, r5
  ADD r9, r13
  LDI r5, 0
  STORE r9, r5

  LDI r9, 52
  LDI r1, TBAR_Y
  ADDI r1, 4
  LDI r12, STR_BUF
  LDI r10, 0x00FF00
  LDI r6, 0
  DRAWTEXT r9, r1, r12, r10, r6

  ; -- Draw frame counter (clock) at right side --
  ; TICKS / 60 = seconds, seconds / 60 = minutes
  LDI r20, 0xFFE
  LOAD r20, r20             ; TICKS
  LDI r21, 60
  DIV r20, r21              ; seconds
  LDI r21, 60
  DIV r20, r21              ; minutes
  LDI r21, 10
  MOD r20, r21              ; minutes mod 10
  LDI r5, 48
  ADD r5, r20               ; ASCII digit for minutes
  LDI r9, STR_BUF
  STORE r9, r5
  ADD r9, r13
  LDI r5, 58                ; :
  STORE r9, r5
  ADD r9, r13
  ; seconds
  LDI r20, 0xFFE
  LOAD r20, r20
  LDI r21, 60
  DIV r20, r21              ; total seconds
  LDI r21, 10
  MOD r20, r21              ; seconds mod 10
  LDI r5, 48
  ADD r5, r20
  STORE r9, r5
  ADD r9, r13
  LDI r5, 0
  STORE r9, r5

  LDI r9, 232
  LDI r1, TBAR_Y
  ADDI r1, 4
  LDI r12, STR_BUF
  LDI r10, 0xAAAAFF
  LDI r6, 0
  DRAWTEXT r9, r1, r12, r10, r6

  ; -- Register hit region for Start button --
  LDI r9, 1               ; x
  LDI r1, TBAR_Y
  ADDI r1, 1              ; y
  LDI r12, 46              ; w
  LDI r10, 14              ; h
  HITSET r9, r1, r12, r10, 1

  ; -- Check mouse --
  MOUSEQ r9                ; r9=mx, r1=my, r12=btn

  ; Check for click (btn==2)
  LDI r10, 2
  CMP r12, r10
  BNE_check:
  ; BNE not available -- use JNZ on difference
  MOV r3, r12
  SUB r3, r10              ; r3 = btn - 2
  JNZ r3, no_click

  ; Something was clicked -- check what
  HITQ r6                  ; r6 = hit region id

  ; Start button (id==1)?
  LDI r3, 1
  CMP r6, r3
  JNZ r5, check_menu_click

  ; Toggle menu
  LDI r20, MENU_OPEN
  LOAD r21, r20
  LDI r3, 1
  CMP r21, r3
  JNZ r5, open_menu
  ; Close menu
  LDI r5, 0
  STORE r20, r5
  JMP no_click

open_menu:
  LDI r5, 1
  STORE r20, r5
  ; Refresh file list
  LDI r9, FILE_BUF
  LS r9
  LDI r20, FILE_COUNT
  STORE r20, r5
  LDI r20, SCROLL_OFF
  LDI r5, 0
  STORE r20, r5
  LDI r20, SEL_INDEX
  STORE r20, r5
  JMP no_click

check_menu_click:
  ; Menu item (id >= 10)? If r6 < 10, skip
  LDI r3, 10
  CMP r6, r3
  BLT r5, no_click

  ; Close menu on selection
  LDI r20, MENU_OPEN
  LDI r5, 0
  STORE r20, r5
  JMP no_click

no_click:
  ; -- Draw menu if open --
  LDI r20, MENU_OPEN
  LOAD r20, r20
  LDI r3, 1
  CMP r20, r3
  JNZ r5, skip_menu

  ; Menu background
  LDI r5, 0x303050
  LDI r9, 0
  LDI r1, 144
  LDI r12, 120
  LDI r10, 96
  RECTF r9, r1, r12, r10, r5

  ; Menu border
  LDI r5, 0x6666AA
  LDI r9, 0
  LDI r1, 144
  LDI r12, 120
  LDI r10, 96
  RECT r9, r1, r12, r10, r5

  ; Menu title "Apps"
  LDI r9, STR_BUF
  LDI r5, 65                ; A
  STORE r9, r5
  ADD r9, r13
  LDI r5, 112               ; p
  STORE r9, r5
  ADD r9, r13
  LDI r5, 112               ; p
  STORE r9, r5
  ADD r9, r13
  LDI r5, 115               ; s
  STORE r9, r5
  ADD r9, r13
  LDI r5, 0
  STORE r9, r5
  LDI r9, 4
  LDI r1, 148
  LDI r12, STR_BUF
  LDI r10, 0xFFFFFF
  LDI r6, 0
  DRAWTEXT r9, r1, r12, r10, r6

  ; Draw file list entries (up to 4 visible)
  LDI r22, 0                ; visible entry counter
  LDI r23, 4                ; max visible
  LDI r24, FILE_BUF          ; filename ptr
  LDI r20, FILE_COUNT
  LOAD r20, r20             ; total files

draw_entries:
  CMP r22, r23
  BGE r5, entries_done
  CMP r20, r11
  JZ r5, entries_done

  ; Copy filename to STR_BUF
  LDI r25, STR_BUF
  LDI r26, 12               ; max chars
copy_name:
  LOAD r27, r24
  STORE r25, r27
  JZ r27, name_done
  ADD r24, r13
  ADD r25, r13
  SUBI r26, 1
  JZ r26, name_done
  JMP copy_name

name_done:
  ; Draw filename
  LDI r9, 4
  LDI r1, 164
  MOV r12, r22
  LDI r28, 14
  MUL r12, r28
  ADD r1, r12              ; y = 164 + entry * 14
  LDI r12, STR_BUF
  LDI r10, 0xCCCCFF
  LDI r6, 0
  DRAWTEXT r9, r1, r12, r10, r6

  ; Advance r24 past null
  LOAD r27, r24
  JNZ r27, no_adv
  ADD r24, r13
no_adv:

  ADDI r22, 1
  SUBI r20, 1
  JMP draw_entries

entries_done:
  ; Menu background hit region
  LDI r9, 0
  LDI r1, 144
  LDI r12, 120
  LDI r10, 96
  HITSET r9, r1, r12, r10, 5

skip_menu:
  FRAME
  JMP main_loop

...
LDI r0, 0xAA        ; R channel
LDI r8, 8           ; shift amount
SHL r0, r8          ; r0 = 0x0000AA00 (R at bits 8-15)
LDI r12, 0xBB        ; G channel
OR r0, r12           ; r0 = 0x0000AABB (R+G)
LDI r8, 16
SHL r0, r8          ; r0 = 0xAABB0000 (R+G shifted up)
LDI r12, 0xCC        ; B channel
OR r0, r12           ; r0 = 0xAABBCC (packed: R=bits16-23, G=bits8-15, B=bits0-7)

; Now extract each channel using BFE
; BFE r5, r0, r4, r6 -- extract 8 bits at lsb=16 from r0 -> R
LDI r4, 8           ; width = 8
LDI r6, 16          ; lsb = 16
BFE r5, r0, r4, r6  ; r5 = 0xAA (red channel)

; BFE r9, r0, r4, r13 -- extract 8 bits at lsb=8 from r0 -> G
LDI r13, 8           ; lsb = 8
BFE r9, r0, r4, r13  ; r9 = 0xBB (green channel)

; BFE r1, r0, r15, r14 -- extract 8 bits at lsb=0 from r0 -> B
LDI r15, 8          ; width = 8
LDI r14, 0          ; lsb = 0
BFE r1, r0, r15, r14 ; r1 = 0xCC (blue channel)

; Now reassemble as BGR: B at bits 16-23, G at bits 8-15, R at bits 0-7
LDI r2, 0          ; start with 0

; Insert B (r1) at bits 16-23
LDI r7, 8          ; width = 8
LDI r3, 16         ; lsb = 16
MOV r10, r1         ; src = blue
BFI r2, r10, r7, r3 ; r2 = 0x00CC0000

; Insert G (r9) at bits 8-15
LDI r3, 8          ; lsb = 8
MOV r10, r9         ; src = green
BFI r2, r10, r7, r3 ; r2 = 0x00CCBB00

; Insert R (r5) at bits 0-7
LDI r3, 0          ; lsb = 0
MOV r10, r5         ; src = red
BFI r2, r10, r7, r3 ; r2 = 0x00CCBBAA (BGR swapped!)
...
LDI r22, 32
LDI r23, 32
RECTF r20, r21, r22, r23, r0  ; original
...
LDI r4, 1
LDI r6, 0
BFE r17, r16, r4, r6  ; r17 = bit 0 of 0xAA = 0

; Extract bit 1 (should be 1)
LDI r6, 1
BFE r18, r16, r4, r6  ; r18 = bit 1 of 0xAA = 1

; Extract bit 7 (should be 1)
LDI r6, 7
BFE r19, r16, r4, r6  ; r19 = bit 7 of 0xAA = 1

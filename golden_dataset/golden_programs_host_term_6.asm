...
mark_row_dirty:
    ...
    SUB r7, r6          ; r7 = 31 - row
    LDI r12, 1
    SHL r12, r7          ; r12 = 1 << (31 - row)
    LDI r10, DIRTY_ROWS
    LOAD r8, r10         ; r8 = current bitmap
    OR r8, r12           ; set the bit
    STORE r10, r8
    POP r7
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r10 = word_index, r6 = bit position within word
...
att_scan_loop:
    ...
    MOV r12, r6           ; r12 = col (will become bit_pos after div)
...
att_check_bit:
    ...
    ADD r20, r10
    LOAD r8, r20
    ; Check if bit r12 is set: shift 1 left by r12, AND
    LDI r13, 1
    LDI r14, 0
att_shift_loop:
    CMP r14, r12
...
apply_sgr:
    ...
    CMPI r5, 0x85
    JNZ r6, sgr_ext95
    CALL sgr_256_to_rgb  ; r6 = RGB from r14 (color index)
    LDI r20, FG_COLOR
    STORE r20, r6
    LDI r20, SGR_EXTENDED
    LDI r6, 0
...
; Clobbers: r6, r7, r8, r13
; Preserves: r14, r5, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r6, 232
    SUB r14, r6        ; r14 = index - 232
    LDI r6, 10
    MUL r14, r6        ; r14 = (index - 232) * 10
    LDI r6, 8
    ADD r14, r6        ; r14 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r14           ; save v
...
sgr256_cube:
    ...
    LDI r7, 8
    SHL r14, r7        ; r14 = G << 8
    OR r6, r14         ; r6 = (R << 16) | (G << 8)
    OR r6, r13         ; r6 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r3, r2        ; r3 = end_col (exclusive)
    CMPI r14, 0
    JZ r6, cel_clear
    CMPI r14, 1
    JNZ r6, cel_mode2
...
ced_check0:
    ...
    LDI r3, COLS
    MUL r0, r3
    ADD r0, r2        ; offset = row*COLS + col

    LDI r5, ROWS
    LDI r4, COLS
    MUL r5, r4         ; total cells
ced_loop0:
    CMP r0, r5
    BGE r6, ced_done
    LDI r20, BUF
...
cdc_count:
    ...
    MOV r6, r5
    MUL r6, r2
    MOV r5, r6         ; r5 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r0, r4        ; r0 = dst col (starting at cursor)
cdc_shift:
    ADD r6, r0
    ADD r6, r14         ; r0 + count = src col
    LDI r2, COLS
    CMP r6, r2
...
cdc_clear:
    ...
    LDI r0, COLS
    SUB r0, r14        ; r0 = first col to clear
    LDI r2, COLS
    CMP r0, r4
    BGE r6, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r0 < r4 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    SUB r0, r14        ; r0 = COLS-1-count (first dst position)
cic_shift:
    CMP r0, r4
    BLT r6, cic_clear
...
render_row:
    ...
    LDI r9, 0          ; x (use r9 to preserve r7)
    MOV r12, r1         ; y = 10 + row*6
    LDI r10, 256         ; w
    LDI r8, 6           ; h

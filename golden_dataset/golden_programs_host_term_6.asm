...
pb_csi_h_mode:
    ...
    CMPI r12, 104
    JNZ r9, pb_csi_l_mode
    CMPI r24, 0
    JZ r9, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r9, r20
    CMPI r9, 1049
    JNZ r9, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r9, pb_csi_s
    CMPI r24, 0
    JZ r9, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r9, r20
    CMPI r9, 1049
    JNZ r9, pb_ret        ; only handle 1049
...
pb_check_utf8_2:
    CMPI r0, ANS_UTF8_2
    JNZ r9, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r12, 128
    BLT r9, pb_reset_state
    CMPI r12, 192
    BGE r9, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r9 as dirty
...
mark_row_dirty:
    ...
    SUB r5, r9          ; r5 = 31 - row
    LDI r10, 1
    SHL r10, r5          ; r10 = 1 << (31 - row)
    LDI r14, DIRTY_ROWS
    LOAD r0, r14         ; r0 = current bitmap
    OR r0, r10           ; set the bit
    STORE r14, r0
    POP r5
    RET
...
; mark_row_dirty_check -- check if row in r9 is dirty
; Returns r9 = bit value (0=clean, nonzero=dirty)
...
map_codepoint:
    ...
    CMPI r12, 9472
    BLT r9, mc_check_arrows
    CMPI r12, 9600
    BGE r9, mc_check_blocks
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r14 = word_index, r9 = bit position within word
    PUSH r9              ; save bit position (CMP will clobber r9)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r14
    LOAD r10, r20
    POP r9               ; restore bit position
    ; Set bit: r10 |= (1 << r9)
    LDI r0, 1
    LDI r12, 0
sts_shift_loop:
    CMP r12, r9
    JZ r9, sts_shift_done
    SHL r0, r5
    ADD r12, r5
    JMP sts_shift_loop
sts_shift_done:
...
ctc_shift_loop:
    CMP r12, r9
    JZ r9, ctc_shift_done
    SHL r0, r5
    ADD r12, r5
    JMP ctc_shift_loop
ctc_shift_done:
    ; r0 = mask. Invert: r0 = ~r0 (all bits except the one to clear)
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r9)
    PUSH r9

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r10, r9           ; r10 = col (will become bit_pos after div)
    LDI r14, 0            ; r14 = word_index
    LDI r11, 32           ; r11 = divisor
att_div_loop:
    CMPI r10, 32
    BLT r9, att_check_bit
    SUB r10, r11           ; r10 -= 32
    ADD r14, r5           ; word_index++
    JMP att_div_loop
...
att_test_bit:
    AND r0, r12
    JNZ r0, att_found_pop

    ; Not a tab stop -- restore column and try next
    POP r9               ; restore scan column
att_scan_next:
    ADD r9, r5
    JMP att_scan_loop

att_found_pop:
    POP r9               ; restore scan column (r9 = tab stop column)
att_found:
    ; Found tab stop at column r9 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r9
    LDI r20, CUR_ROW
    LOAD r9, r20         ; r9 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r9
    POP r31
    RET
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r9 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r9               ; r9 = original count
    LDI r21, 10
    MOD r9, r21          ; r9 = count % 10
...
; Preserves all registers except r9 (CMP clobbers it).
...
apply_sgr:
    ...
    CMPI r11, 0x85
    JNZ r9, sgr_ext95
    CALL sgr_256_to_rgb  ; r9 = RGB from r6 (color index)
    LDI r20, FG_COLOR
    STORE r20, r9
    LDI r20, SGR_EXTENDED
    LDI r9, 0
...
; Output: r9 = RGB color value
; Clobbers: r9, r5, r0, r12
; Preserves: r6, r11, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r9, 232
    SUB r6, r9        ; r6 = index - 232
    LDI r9, 10
    MUL r6, r9        ; r6 = (index - 232) * 10
    LDI r9, 8
    ADD r6, r9        ; r6 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r6           ; save v
    ...
    MOV r9, r6
    SHL r9, r5        ; r9 = v << 16
    PUSH r9           ; save high bits
    LDI r5, 8
    MOV r9, r6
    SHL r9, r5        ; r9 = v << 8
    OR r9, r6         ; r9 = (v << 8) | v
    POP r5            ; r5 = v << 16
    OR r9, r5         ; r9 = (v << 16) | (v << 8) | v
    POP r6            ; restore r6
    JMP sgr256_ret
...
sgr256_cube:
    ...
    LDI r5, 8
    SHL r6, r5        ; r6 = G << 8
    OR r9, r6         ; r9 = (R << 16) | (G << 8)
    OR r9, r12         ; r9 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r2, r4        ; r2 = end_col (exclusive)
    CMPI r6, 0
    JZ r9, cel_clear
    CMPI r6, 1
    JNZ r9, cel_mode2
...
ced_check0:
    ...
    LDI r2, COLS
    MUL r7, r2
    ADD r7, r4        ; offset = row*COLS + col

    LDI r11, ROWS
    LDI r1, COLS
    MUL r11, r1         ; total cells
ced_loop0:
    CMP r7, r11
    BGE r9, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r6, 1
ccp_row_set:
    SUB r6, r5         ; convert to 0-based
    CMPI r6, 0
    BGE r9, ccp_row_clamp
    LDI r6, 0
ccp_row_clamp:
    LDI r11, ROWS
    SUB r11, r5
...
cil_clear_loop:
    ...
    LDI r1, ROWS
    SUB r1, r5
    BGE r9, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r4, r5
    CMP r9, r4
    BGE r9, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    CMP r7, r11
    BGE r9, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r6
    BGE r9, cdl_done2
...
cdc_count:
    ...
    LOAD r11, r20
    LDI r20, CUR_COL
    LOAD r1, r20       ; r1 = cursor col

    ; Base offset = row * COLS
    LDI r4, COLS
    MOV r9, r11
    MUL r9, r4
    MOV r11, r9         ; r11 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r7, r1        ; r7 = dst col (starting at cursor)
cdc_shift:
    ADD r9, r7
    ADD r9, r6         ; r7 + count = src col
    LDI r4, COLS
    CMP r9, r4
    BGE r9, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r7, COLS
    SUB r7, r6        ; r7 = first col to clear
    LDI r4, COLS
    CMP r7, r1
    BGE r9, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r7 < r1 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    SUB r7, r6        ; r7 = COLS-1-count (first dst position)
cic_shift:
    CMP r7, r1
    BLT r9, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r9, r20
    JZ r9, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    LDI r13, 0          ; x (use r13 to preserve r5)
    MOV r10, r8         ; y = 10 + row*6
    LDI r14, 256         ; w
    LDI r0, 6           ; h

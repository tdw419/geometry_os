...
pb_normal_byte:
    ...
    BLT r14, pb_ascii
    CMPI r4, 224
    BGE r14, pb_utf8_3_start

    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r4 & 0x1F) << 6
    LDI r11, 31
    AND r4, r11
    LDI r11, 6
    SHL r4, r11
...
pb_utf8_3_start:
    CMPI r4, 240
    BGE r14, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r4 & 0x0F) << 12
    LDI r11, 15
    AND r4, r11
    LDI r11, 12
    SHL r4, r11
...
pb_esc_save_cursor:
    CMPI r4, 55 ; '7' = DEC Save Cursor
    JNZ r14, pb_esc_restore_cursor
    CALL save_cursor
    LDI r20, ANSI_STATE
    LDI r14, ANS_NORMAL
    STORE r20, r14
...
pb_esc_restore_cursor:
    CMPI r4, 56 ; '8' = DEC Restore Cursor
    JNZ r14, pb_esc_index
    CALL restore_cursor
    LDI r20, ANSI_STATE
    LDI r14, ANS_NORMAL
    STORE r20, r14
...
pb_esc_index:
    CMPI r4, 68 ; 'D' = Index (move down, scroll if at bottom)
    JNZ r14, pb_esc_rev_index
    CALL do_newline
    LDI r20, ANSI_STATE
    LDI r14, ANS_NORMAL
    STORE r20, r14
...
pb_esc_rev_index:
    CMPI r4, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r14, pb_esc_ris
...
pb_esc_next_line:
    CMPI r4, 69 ; 'E' = Next Line (CR + Index)
...
pb_csi_g_tab:
    ...
    CMPI r4, 103
    JNZ r14, pb_csi_h_mode
    ; TBC: CSI_PARAM determines mode (default 0)
    LDI r20, CSI_PARAM
    LOAD r14, r20
    CMPI r14, 3
    JZ r14, tbc_clear_all
    CMPI r14, 0
    JZ r14, tbc_clear_one
...
pb_csi_h_mode:
    ...
    CMPI r4, 104
    JNZ r14, pb_csi_l_mode
    CMPI r24, 0
    JZ r14, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r14, r20
    CMPI r14, 1049
    JNZ r14, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r14, pb_csi_s
    CMPI r24, 0
    JZ r14, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r14, r20
    CMPI r14, 1049
    JNZ r14, pb_ret        ; only handle 1049
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r4 - 48)
    LDI r20, CSI_PARAM
    LOAD r14, r20
    LDI r3, 10
    MUL r14, r3
...
pb_check_utf8_2:
    CMPI r7, ANS_UTF8_2
    JNZ r14, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r4, 128
    BLT r14, pb_reset_state
    CMPI r4, 192
    BGE r14, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r14 as dirty
; Sets bit (31 - row) in DIRTY_ROWS bitmap
mark_row_dirty:
    PUSH r12
    LDI r12, 31
    SUB r12, r14          ; r12 = 31 - row
    LDI r8, 1
    SHL r8, r12          ; r8 = 1 << (31 - row)
    LDI r2, DIRTY_ROWS
    LOAD r7, r2         ; r7 = current bitmap
    OR r7, r8           ; set the bit
    STORE r2, r7
    POP r12
    RET
...
; mark_row_dirty_check -- check if row in r14 is dirty
; Returns r14 = bit value (0=clean, nonzero=dirty)
mark_row_dirty_check:
    ...
    LOAD r7, r2         ; r7 = current bitmap
    MOV r14, r7
    AND r14, r8          ; r14 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r4 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r12, 1
...
sts_div_loop:
    CMPI r14, 32
    BLT r14, sts_div_done
    SUB r14, r8
    ADD r2, r12
    JMP sts_div_loop
sts_div_done:
    ; r2 = word_index, r14 = bit position within word
    PUSH r14              ; save bit position (CMP will clobber r14)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r2
    LOAD r8, r20
    POP r14               ; restore bit position
    ; Set bit: r8 |= (1 << r14)
    LDI r7, 1
    LDI r4, 0
sts_shift_loop:
    CMP r4, r14
    JZ r14, sts_shift_done
    SHL r7, r12
    ADD r4, r12
    JMP sts_shift_loop
sts_shift_done:
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r14)
    PUSH r14

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r8, r14           ; r8 = col (will become bit_pos after div)
    LDI r2, 0            ; r2 = word_index
    LDI r11, 32           ; r11 = divisor
att_div_loop:
    CMPI r8, 32
    BLT r14, att_check_bit
    SUB r8, r11           ; r8 -= 32
    ADD r2, r12           ; word_index++
    JMP att_div_loop
...
att_test_bit:
    ...
    POP r14               ; restore scan column
att_scan_next:
    ADD r14, r12
    JMP att_scan_loop

att_found_pop:
    POP r14               ; restore scan column (r14 = tab stop column)
att_found:
    ; Found tab stop at column r14 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r14
    LDI r20, CUR_ROW
    LOAD r14, r20         ; r14 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r14
    POP r31
    RET
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r14 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r14               ; r14 = original count
    LDI r21, 10
    MOD r14, r21          ; r14 = count % 10
...
; Preserves all registers except r14 (CMP clobbers it).
...
apply_sgr:
    ...
    CMPI r11, 0x85
    JNZ r14, sgr_ext95
    CALL sgr_256_to_rgb  ; r14 = RGB from r3 (color index)
    LDI r20, FG_COLOR
    STORE r20, r14
    LDI r20, SGR_EXTENDED
    LDI r14, 0
...
; Output: r14 = RGB color value
; Clobbers: r14, r12, r7, r4
; Preserves: r3, r11, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r14, 232
    SUB r3, r14        ; r3 = index - 232
    LDI r14, 10
    MUL r3, r14        ; r3 = (index - 232) * 10
    LDI r14, 8
    ADD r3, r14        ; r3 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r3           ; save v
    LDI r12, 16
    MOV r14, r3
    SHL r14, r12        ; r14 = v << 16
    PUSH r14           ; save high bits
    LDI r12, 8
    MOV r14, r3
    SHL r14, r12        ; r14 = v << 8
    OR r14, r3         ; r14 = (v << 8) | v
    POP r12            ; r12 = v << 16
    OR r14, r12         ; r14 = (v << 16) | (v << 8) | v
    POP r3            ; restore r3
    JMP sgr256_ret
...
sgr256_cube:
    ...
    LDI r12, 8
    SHL r3, r12        ; r3 = G << 8
    OR r14, r3         ; r14 = (R << 16) | (G << 8)
    OR r14, r4         ; r14 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r15, r1        ; r15 = end_col (exclusive)
    CMPI r3, 0
    JZ r14, cel_clear
    CMPI r3, 1
    JNZ r14, cel_mode2
...
ced_check0:
    ...
    LDI r15, COLS
    MUL r10, r15
    ADD r10, r1        ; offset = row*COLS + col

    LDI r11, ROWS
    LDI r5, COLS
    MUL r11, r5         ; total cells
ced_loop0:
    CMP r10, r11
    BGE r14, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r3, 1
ccp_row_set:
    SUB r3, r12         ; convert to 0-based
    CMPI r3, 0
    BGE r14, ccp_row_clamp
    LDI r3, 0
ccp_row_clamp:
    LDI r11, ROWS
    SUB r11, r12
...
cil_clear_loop:
    ...
    LDI r5, ROWS
    SUB r5, r12
    BGE r14, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r1, r12
    CMP r14, r1
    BGE r14, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    LDI r5, ROWS
    SUB r5, r12         ; r5 = ROWS-1
    MOV r10, r5
    SUB r10, r3
    ADD r10, r12        ; r10 = first row to clear (ROWS - count)
    CMP r10, r11
    BGE r14, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r3
    BGE r14, cdl_done2
...
cdc_count:
    ...
    MOV r14, r11
    MUL r14, r1
    MOV r11, r14         ; r11 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r10, r5        ; r10 = dst col (starting at cursor)
cdc_shift:
    ADD r14, r10
    ADD r14, r3         ; r10 + count = src col
    LDI r1, COLS
    CMP r14, r1
    BGE r14, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r10, COLS
    SUB r10, r3        ; r10 = first col to clear
    LDI r1, COLS
    CMP r10, r5
    BGE r14, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r10 < r5 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    SUB r10, r3        ; r10 = COLS-1-count (first dst position)
cic_shift:
    CMP r10, r5
    BLT r14, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r14, r20
    JZ r14, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    LDI r6, 0          ; x (use r6 to preserve r12)
    MOV r8, r0         ; y = 10 + row*6
    LDI r2, 256         ; w
    LDI r7, 6           ; h
    LDI r4, 0x0A0A0A    ; bg color
    RECTF r6, r8, r2, r7, r4

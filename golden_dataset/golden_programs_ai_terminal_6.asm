...
;   r11: Constant 1 (must reload after ops that clobber)
;   r1: Key input
...
render_text:
    ; DRAWTEXT x=0, y=r2, addr=SCRATCH, fg=r14, bg=0 (transparent)
    LDI r11, 0
    LDI r12, SCRATCH
    LDI r13, 0         ; bg = transparent
    DRAWTEXT r11, r2, r12, r14, r13
...
draw_cursor:
    ...
    MUL r8, r15           ; row * 8
    LDI r6, 16
    ADD r8, r6           ; y = 16 + row*8
    LDI r6, 6
    LDI r7, 8
    LDI r1, 0x44FF44
    RECTF r4, r8, r6, r7, r1
...
check_up_arrow:
    CMPI r1, 128         ; Up arrow = 0x80
    JNZ r4, check_down_arrow
    JMP do_history_up

check_down_arrow:
    CMPI r1, 129         ; Down arrow = 0x81
    JNZ r4, do_char
    JMP do_history_down
...
do_char:
    ...
    LOAD r8, r20
    LDI r6, COLS
    MUL r8, r6           ; r8 = row * COLS
    LDI r20, CUR_COL
    LOAD r4, r20
    ADD r8, r4           ; r8 = row*COLS + col
    LDI r20, BUF
...
cmd_yes:
    ...
    ; SPAWNC r9, r10 -- r9=start addr, r10=caps list addr
    LDI r9, 0x1000
    LDI r10, SANDBOX_CAPS
    SPAWNC r9, r10
...
; Advances r19 by 8. Uses r8, r6 as scratch.
; =========================================
write_hex_to_buf:
    PUSH r31
    PUSH r4
...
u2d_no_thou:
    ...
    DIV r3, r5           ; r3 = n / 100
    LDI r5, 10
    MOD r3, r5           ; r3 = (n/100) % 10 = hundreds digit
...
u2d_skip_hun:
    ...
    LDI r5, 10
    DIV r3, r5           ; r3 = n / 10
    MOD r3, r5           ; r3 = (n/10) % 10 = tens digit
...
history_save:
    ...
    SUBI r3, 1
    LDI r5, 8
    MOD r3, r5           ; r3 = last entry index
    LDI r5, 42
    MUL r3, r5           ; r3 = byte offset of last entry
...
do_history_up:
    ...
    LDI r20, CMD_HIST_NAV
    LOAD r5, r20          ; r5 = current nav index
...
hist_up_recall:
    ...
    MOV r15, r3           ; r15 = count
    SUBI r15, 1           ; r15 = count - 1
    SUB r15, r5           ; r15 = count - 1 - nav_index
    LDI r0, 8
    MOD r15, r0           ; r15 = wrapped index
    LDI r0, 42
    MUL r15, r0           ; r15 = byte offset
...
do_history_down:
    ...
    LDI r20, CMD_HIST_NAV
    LOAD r5, r20
    JZ r5, hist_down_done  ; already at current input
...
clear_prompt_line:
    ...
    LOAD r4, r20
    LDI r5, COLS
    MUL r4, r5           ; r4 = row * COLS
    LDI r5, PROMPT_BUF
    ADD r5, r4           ; r5 = &PROMPT_BUF[row * COLS]

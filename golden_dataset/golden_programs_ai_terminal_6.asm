...
;   r7: Constant 1 (must reload after ops that clobber)
;   r10: Key input
...
check_ai_prefix:
    ...
    CMPI r5, 58         ; ':'
    JNZ r12, render_text_default
    LDI r15, 0x00FFFF  ; cyan for AI response
    JMP render_text
...
render_text_default:
    ...
    CMPI r5, 47         ; '/'
    JNZ r12, render_text
    LDI r15, 0xFFFF44  ; yellow for commands

render_text:
    ; DRAWTEXT x=0, y=r2, addr=SCRATCH, fg=r15, bg=0 (transparent)
    LDI r7, 0
    LDI r1, SCRATCH
    LDI r14, 0         ; bg = transparent
    DRAWTEXT r7, r2, r1, r15, r14
...
draw_cursor:
    ...
    MUL r8, r4           ; row * 8
    LDI r9, 16
    ADD r8, r9           ; y = 16 + row*8
    LDI r9, 6
    LDI r0, 8
    LDI r10, 0x44FF44
    RECTF r12, r8, r9, r0, r10
...
check_del:
    CMPI r10, 127
    JNZ r12, check_up_arrow
    JMP do_backspace

check_up_arrow:
    CMPI r10, 128         ; Up arrow = 0x80
    JNZ r12, check_down_arrow
    JMP do_history_up

check_down_arrow:
    CMPI r10, 129         ; Down arrow = 0x81
    JNZ r12, do_char
    JMP do_history_down
...
do_char:
    ...
    LOAD r8, r20
    LDI r9, COLS
    MUL r8, r9           ; r8 = row * COLS
    LDI r20, CUR_COL
    LOAD r12, r20
    ADD r8, r12           ; r8 = row*COLS + col
    LDI r20, BUF
...
do_enter:
    ...
    LOAD r22, r20
    CMPI r22, 47         ; '/'
    JNZ r12, send_to_llm  ; not a command, send to LLM
...
skip_history:
    ...
    ; r12 = response length -- add to CTX_CHARS
    LDI r20, CTX_CHARS
    LOAD r13, r20
    ADD r13, r12
    STORE r20, r13
...
wrap_loop:
    ...
    LDI r5, 0
    CMP r12, r5
    JZ r12, wrap_line_done
    LDI r5, 10
    CMP r12, r5
    JNZ r12, wrap_no_newline
...
run_check_zero:
    CMP r12, r5
    JZ r12, run_failed_empty

    ; Success -- r12 = word count. Park it in RUN_PENDING and wait for /yes.
...
cmd_yes:
    ...
    ; SPAWNC r13, r6 -- r13=start addr, r6=caps list addr
    LDI r13, 0x1000
    LDI r6, SANDBOX_CAPS
    SPAWNC r13, r6
    ...
    LDI r5, 0xFFFFFFFF
    CMP r12, r5
    JZ r12, yes_spawn_failed

    ; Success -- child PID in r12
...
cmd_focus:
    ...
    LOAD r22, r20
    CMPI r22, 0
    JZ r12, focus_show     ; bare "/focus" with no arg -> show current

    ; Skip spaces between /focus and arg
focus_skip_space:
    CMPI r22, 32          ; space
    JNZ r12, focus_parse_arg
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 0
    JNZ r12, focus_skip_space
    JMP focus_show        ; trailing spaces only -> show current

focus_parse_arg:
    ; Check for "off" -> clear focus
    CMPI r22, 111         ; 'o'
    JNZ r12, focus_try_hex
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 102         ; 'f'
    JNZ r12, focus_try_hex
...
focus_hex_check:
    ...
    CMPI r22, 48
    BLT r12, focus_hi_not_digit   ; r22 < 48, not '0'-'9'
    CMPI r22, 58
    BGE r12, focus_hi_not_digit   ; r22 >= 58, past '9'
    ; It IS '0'-'9'
    SUBI r22, 48
    MOV r23, r22
    SHLI r23, 4
...
focus_hi_not_digit:
    ...
    CMPI r22, 65
    BLT r12, focus_hi_not_upper   ; < 'A'
    CMPI r22, 71
    BGE r12, focus_hi_not_upper   ; > 'F'
    ; It IS 'A'-'F'
    SUBI r22, 55                 ; 'A'=65 -> 10
    MOV r23, r22
    SHLI r23, 4
...
focus_hi_not_upper:
    ...
    CMPI r22, 97
    BLT r12, focus_bad_arg
    CMPI r22, 103
    BGE r12, focus_bad_arg
    ; It IS 'a'-'f'
    SUBI r22, 87                 ; 'a'=97 -> 10
    MOV r23, r22
    SHLI r23, 4
...
focus_lo_digit_start:
    ...
    ; Check '0'-'9'
    CMPI r22, 48
    BLT r12, focus_lo_not_digit
    CMPI r22, 58
    BGE r12, focus_lo_not_digit
    SUBI r22, 48
    OR r23, r22
    JMP focus_write
...
focus_lo_not_digit:
    ; Check 'A'-'F'
    CMPI r22, 65
    BLT r12, focus_lo_not_upper
    CMPI r22, 71
    BGE r12, focus_lo_not_upper
    SUBI r22, 55
    OR r23, r22
    JMP focus_write
...
debug_skip_space:
    CMPI r22, 32
    JNZ r12, debug_parse_arg
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 0
    JNZ r12, debug_skip_space
    JMP debug_usage

debug_parse_arg:
    CMPI r22, 112
    JNZ r12, debug_try_regs
    ADDI r20, 1
    LOAD r22, r20
    CMPI r22, 105
    JNZ r12, debug_try_regs
...
debug_ping_wait:
    ...
    LOAD r22, r20
    CMPI r22, 2
    JZ r12, debug_ping_ok
    SUBI r5, 1
    CMPI r5, 0
    JNZ r12, debug_ping_wait
    JMP debug_timeout
...
debug_regs_wait:
    ...
    LOAD r22, r20
    CMPI r22, 2
    JZ r12, debug_regs_got
    SUBI r5, 1
    CMPI r5, 0
    JNZ r12, debug_regs_wait
    JMP debug_timeout
...
; Input: r12 = nibble value (0-15)
; Output: r12 = ASCII char
; =========================================
hex_digit_char:
    CMPI r12, 10
    BGE r12, hdc_alpha
...
do_backspace:
    ...
    CMPI r12, 2
    JNZ r12, hk_ret       ; if col <= 2, do nothing (JNZ after CMPI means not-equal)
    ; Actually: CMPI r12, 2 sets r12 to 0 if equal. So JNZ r12 means "if not 2"
    ; We want to proceed only if r12 > 2
    LDI r7, 1
    LDI r20, CUR_COL
    LOAD r12, r20
    CMPI r12, 2
    JZ r12, hk_ret        ; at prompt, do nothing
...
; Output: r12 = length (does NOT clobber r20)
; Clobbers: r12, r6, r2
; =========================================
strlen_prompt_buf:
    PUSH r31
...
sp_strlen_loop:
    LOAD r2, r20
    JZ r2, sp_strlen_done
    ADDI r12, 1
    ADDI r20, 1
    JMP sp_strlen_loop
sp_strlen_done:
    MOV r20, r6         ; restore r20
    POP r31
    RET
...
; =========================================
; UTILITY: clear_buf (r2=start, r16=count)
; =========================================
clear_buf:
    PUSH r31
    PUSH r2
...
check_watchdog:
    ...
    STORE r20, r12
    CMPI r12, 0
    JNZ r12, cw_done     ; Not time to check yet
    ...
    LDI r23, 0xDB9900
    CMP r22, r23
    JNZ r12, cw_done     ; No child active
    ...
    LOAD r23, r20
    CMP r22, r23
    JZ r12, cw_flatline   ; Heartbeat hasn't changed!
...
cw_flatline:
    ...
    ADDI r12, 1
    STORE r20, r12
    CMPI r12, 3          ; 3 strikes (3 seconds of flatline)
    BGE r12, trigger_heal
    JMP cw_done
...
heal_no_source:
    ...
    LDI r5, 0xFFFFFFFF
    CMP r12, r5
    JZ r12, heal_asm_failed

    LDI r5, 0
    CMP r12, r5
    JZ r12, heal_asm_empty
...
; =========================================
; WRITE_HEX_TO_BUF -- write 8-char hex of r12 to buffer at r19
; Input: r12 = value, r19 = buffer position
; Advances r19 by 8. Uses r8, r9 as scratch.
; =========================================
write_hex_to_buf:
    PUSH r31
    PUSH r12
...
; Input: r12 = number (capped at 9999)
...
u2d_notcap:
    ...
    ; Skip leading zeros.
    LDI r5, 0            ; digit count (suppressed leading zeros)
    LDI r3, 1000
...
u2d_no_thou:
    ...
    DIV r5, r3           ; r5 = n / 100
    LDI r3, 10
    MOD r5, r3           ; r5 = (n/100) % 10 = hundreds digit

    ; Check if we should skip (no thousands AND hundreds is 0)
    LDI r3, 1000
    CMP r12, r3
    BLT r12, u2d_skip_hun
    JZ r5, u2d_skip_hun
    ADDI r5, 48
    STORE r20, r5
    ADDI r20, 1
u2d_skip_hun:
    ...
    LDI r3, 10
    DIV r5, r3           ; r5 = n / 10
    MOD r5, r3           ; r5 = (n/10) % 10 = tens digit

    ; Check if we should skip (n < 10 AND no prior digits)
    LDI r3, 10
    CMP r12, r3
    BLT r12, u2d_skip_ten
    JZ r5, u2d_skip_ten
    ADDI r5, 48
    STORE r20, r5
    ADDI r20, 1
...
history_save:
    ...
    LOAD r12, r20
    CMPI r12, 0
    JZ r12, hist_save_done  ; empty command, skip

    ; Load history count
    LDI r20, CMD_HIST_COUNT
    LOAD r5, r20          ; r5 = count
    ...
    SUBI r5, 1
    LDI r3, 8
    MOD r5, r3           ; r5 = last entry index
    LDI r3, 42
    MUL r5, r3           ; r5 = byte offset of last entry
    ADDI r5, CMD_HIST_BASE ; r5 = address of last entry
    CALL strcmp_scratch    ; sets r12 = 0 if equal
    JZ r12, hist_save_done ; duplicate, skip
...
; =========================================
; STRCMP_SCRATCH - Compare SCRATCH with string at r5
; Returns r12 = 0 if equal, nonzero otherwise
; Clobbers: r12, r3, r4
; =========================================
strcmp_scratch:
    LDI r3, SCRATCH
strcmp_loop:
    ...
    CMP r12, r4
    JNZ r12, strcmp_neq
    JZ r12, strcmp_eq     ; both null = equal
    ADDI r3, 1
    ADDI r5, 1
    JMP strcmp_loop
strcmp_neq:
...
do_history_up:
    ...
    LDI r20, CMD_HIST_COUNT
    LOAD r5, r20
    JZ r5, hist_up_done  ; no history

    ; Load current nav index
    LDI r20, CMD_HIST_NAV
    LOAD r3, r20          ; r3 = current nav index
    ...
    SUBI r4, 1
    CMP r3, r4
    BGE r12, hist_up_done  ; already at oldest

    ; If nav index is 0, save current prompt line first
    JNZ r3, hist_up_recall
...
hist_up_recall:
    ...
    MOV r4, r5           ; r4 = count
    SUBI r4, 1           ; r4 = count - 1
    SUB r4, r3           ; r4 = count - 1 - nav_index
    LDI r11, 8
    MOD r4, r11           ; r4 = wrapped index
    LDI r11, 42
    MUL r4, r11           ; r4 = byte offset
    ADDI r4, CMD_HIST_BASE
...
hist_up_copy:
    CMP r17, r11
    BGE r12, hist_up_copy_done
    LOAD r12, r4
    JZ r12, hist_up_copy_done  ; stop at null terminator
    STORE r20, r12
    ADDI r4, 1
    ADDI r20, 1
    ADDI r17, 1
...
do_history_down:
    ...
    LDI r20, CMD_HIST_NAV
    LOAD r3, r20
    JZ r3, hist_down_done  ; already at current input
...
hist_dn_copy:
    CMP r17, r11
    BGE r12, hist_dn_copy_done
    LOAD r12, r4
    JZ r12, hist_dn_copy_done
    STORE r20, r12
    ADDI r4, 1
    ADDI r20, 1
    ADDI r17, 1
...
clear_prompt_line:
    ...
    LDI r3, PROMPT_BUF
    ADD r3, r12           ; r3 = &PROMPT_BUF[row * COLS]

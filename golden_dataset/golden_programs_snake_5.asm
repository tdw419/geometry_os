; DESCRIPTION: Draw rectangle: pos=the screen, color=colored, size=fixed size.

; snake.asm -- Snake game on a 32x32 cell grid (8px per cell = 256x256)
;
; Memory layout (all RAM addresses):
;   0x2000..0x21FF  body ring buffer: each cell = (x<<8)|y, 512 slots
;   0x2200          head index (into ring buffer)
;   0x2201          tail index
;   0x2202          length
;   0x2203          apple_x (cell coords, 0-31)
;   0x2204          apple_y
;   0x2205          direction (0=up 1=right 2=down 3=left)
;   0x2206          game_over flag
;
; Stack: r30 = SP, initialized to 0x8000 (grows down into safe RAM)
;
; Controls: WASD (also lowercase)

; ── restart / init ──────────────────────────────────────────────
restart:
  LDI r30, 0x8000     ; stack pointer

  ; write initial snake: (14,16) (15,16) (16,16) at indices 0,1,2
  LDI r6, 0x2000
  LDI r0, 8

  LDI r8, 14
  SHL r8, r0
  LDI r9, 16
  OR r8, r9
  STORE r6, r8        ; body[0] = (14,16)

  LDI r8, 15
  SHL r8, r0
  OR r8, r9
  LDI r6, 0x2001
  STORE r6, r8        ; body[1] = (15,16)

  LDI r8, 16
  SHL r8, r0
  OR r8, r9
  LDI r6, 0x2002
  STORE r6, r8        ; body[2] = (16,16)

  LDI r8, 2
  LDI r6, 0x2200
  STORE r6, r8        ; head = 2
  LDI r8, 0
  LDI r6, 0x2201
  STORE r6, r8        ; tail = 0
  LDI r8, 3
  LDI r6, 0x2202
  STORE r6, r8        ; length = 3

  LDI r8, 1           ; direction = right
  LDI r6, 0x2205
  STORE r6, r8

  LDI r8, 0           ; game_over = 0
  LDI r6, 0x2206
  STORE r6, r8

  CALL place_apple

; ── main loop ────────────────────────────────────────────────────
game_loop:
  ; check game over
  LDI r6, 0x2206
  LOAD r8, r6
  JNZ r8, game_over_screen

  ; read key and update direction (cannot reverse)
  IKEY r14

  LDI r10, 87          ; W / w = up
  CMP r14, r10
  JZ r13, try_up
  LDI r10, 119
  CMP r14, r10
  JZ r13, try_up

  LDI r10, 68          ; D / d = right
  CMP r14, r10
  JZ r13, try_right
  LDI r10, 100
  CMP r14, r10
  JZ r13, try_right

  LDI r10, 83          ; S / s = down
  CMP r14, r10
  JZ r13, try_down
  LDI r10, 115
  CMP r14, r10
  JZ r13, try_down

  LDI r10, 65          ; A / a = left
  CMP r14, r10
  JZ r13, try_left
  LDI r10, 97
  CMP r14, r10
  JZ r13, try_left

after_input:
  ; throttle: only move snake every 8 frames (~7.5 moves/sec at 60fps)
  LDI r6, 0xFFE
  LOAD r12, r6           ; r12 = TICKS
  LDI r0, 7
  AND r12, r0            ; r12 = TICKS & 7
  JZ r12, do_move        ; move on frame 0, 8, 16, ...
  CALL draw_frame
  FRAME
  JMP game_loop

do_move:
  CALL move_snake
  CALL draw_frame
  FRAME
  JMP game_loop

try_up:
  LDI r6, 0x2205
  LOAD r10, r6
  LDI r0, 2
  CMP r10, r0
  JZ r13, after_input  ; refuse if going down
  LDI r8, 0
  STORE r6, r8
  JMP after_input

try_right:
  LDI r6, 0x2205
  LOAD r10, r6
  LDI r0, 3
  CMP r10, r0
  JZ r13, after_input  ; refuse if going left
  LDI r8, 1
  STORE r6, r8
  JMP after_input

try_down:
  LDI r6, 0x2205
  LOAD r10, r6
  LDI r0, 0
  CMP r10, r0
  JZ r13, after_input  ; refuse if going up
  LDI r8, 2
  STORE r6, r8
  JMP after_input

try_left:
  LDI r6, 0x2205
  LOAD r10, r6
  LDI r0, 1
  CMP r10, r0
  JZ r13, after_input  ; refuse if going right
  LDI r8, 3
  STORE r6, r8
  JMP after_input

; ── game over screen ─────────────────────────────────────────────
game_over_screen:
  LDI r8, 0x550000
  FILL r8
  FRAME
  IKEY r14
  JZ r14, game_over_screen
  JMP restart

; ─────────────────────────────────────────────────────────────────
; SUBROUTINE: move_snake
;   Reads direction, computes new head, checks walls + apple.
;   PUSH/POP r31 because we may nested-call place_apple.
; ─────────────────────────────────────────────────────────────────
move_snake:
  PUSH r31

  ; load head cell
  LDI r6, 0x2200
  LOAD r11, r6         ; r11 = head index
  LDI r6, 0x2000
  ADD r6, r11
  LOAD r3, r6         ; r3 = head cell = (x<<8)|y

  ; decode head x (r15), y (r5)
  LDI r0, 8
  LDI r15, 0
  ADD r15, r3
  SHR r15, r0          ; r15 = x
  LDI r0, 0xFF
  LDI r5, 0
  ADD r5, r3
  AND r5, r0          ; r5 = y

  ; load direction
  LDI r6, 0x2205
  LOAD r4, r6

  LDI r0, 0
  CMP r4, r0
  JZ r13, dir_up
  LDI r0, 1
  CMP r4, r0
  JZ r13, dir_right
  LDI r0, 2
  CMP r4, r0
  JZ r13, dir_down
  ; left: x -= 1
  LDI r0, 1
  SUB r15, r0
  JMP wall_check

dir_up:
  LDI r0, 1
  SUB r5, r0
  JMP wall_check

dir_right:
  LDI r0, 1
  ADD r15, r0
  JMP wall_check

dir_down:
  LDI r0, 1
  ADD r5, r0

wall_check:
  ; x >= 32 -> hit
  LDI r0, 32
  CMP r15, r0
  BGE r13, do_wall_hit
  ; x < 0 (signed underflow -> large u32) -> hit
  LDI r0, 0
  CMP r15, r0
  BLT r13, do_wall_hit
  ; y < 0
  CMP r5, r0
  BLT r13, do_wall_hit
  ; y >= 32
  LDI r0, 32
  CMP r5, r0
  BGE r13, do_wall_hit

  ; encode new head: (x<<8)|y
  LDI r0, 8
  LDI r3, 0
  ADD r3, r15
  SHL r3, r0
  OR r3, r5

  ; advance head index (mod 512)
  LDI r6, 0x2200
  LOAD r11, r6
  LDI r0, 1
  ADD r11, r0
  LDI r0, 0x1FF
  AND r11, r0
  STORE r6, r11

  ; store new head cell
  LDI r6, 0x2000
  ADD r6, r11
  STORE r6, r3

  ; check apple: compare new head_x with apple_x
  LDI r6, 0x2203
  LOAD r20, r6          ; apple_x
  LDI r6, 0x2204
  LOAD r21, r6          ; apple_y
  CMP r15, r20
  JZ r13, check_apple_y
  JMP advance_tail

check_apple_y:
  CMP r5, r21
  JZ r13, do_eat_apple

advance_tail:
  ; not eating: advance tail
  LDI r6, 0x2201
  LOAD r22, r6
  LDI r0, 1
  ADD r22, r0
  LDI r0, 0x1FF
  AND r22, r0
  LDI r6, 0x2201
  STORE r6, r22
  POP r31
  RET

do_eat_apple:
  ; grow: increment length
  LDI r6, 0x2202
  LOAD r0, r6
  LDI r20, 1
  ADD r0, r20
  STORE r6, r0
  CALL place_apple
  ; eat sound: 880 Hz, 60 ms
  LDI r2, 880
  LDI r10, 60
  BEEP r2, r10
  POP r31
  RET

do_wall_hit:
  LDI r8, 1
  LDI r6, 0x2206
  STORE r6, r8
  ; death sound: 110 Hz, 300 ms
  LDI r2, 110
  LDI r10, 300
  BEEP r2, r10
  POP r31
  RET

; ─────────────────────────────────────────────────────────────────
; SUBROUTINE: place_apple -- pick random cell (0-31, 0-31)
; ─────────────────────────────────────────────────────────────────
place_apple:
  RAND r8
  LDI r9, 31
  AND r8, r9
  LDI r6, 0x2203
  STORE r6, r8
  RAND r8
  AND r8, r9
  LDI r6, 0x2204
  STORE r6, r8
  RET

; ─────────────────────────────────────────────────────────────────
; SUBROUTINE: draw_frame -- clear, draw apple, draw snake body
;   No nested calls, r31 safe.
; ─────────────────────────────────────────────────────────────────
draw_frame:
  ; clear dark background
  LDI r8, 0x001008
  FILL r8

  ; draw apple (red 8x8 block)
  LDI r6, 0x2203
  LOAD r9, r6           ; apple_x
  LDI r6, 0x2204
  LOAD r1, r6           ; apple_y
  LDI r0, 8
  MUL r9, r0            ; px_x
  MUL r1, r0            ; px_y
  LDI r22, 8
  LDI r23, 8
  LDI r24, 0xFF2200
  RECTF r9, r1, r22, r23, r24

  ; draw snake body: iterate ring buffer from tail, length times
  LDI r6, 0x2201
  LOAD r11, r6           ; r11 = tail index
  LDI r6, 0x2202
  LOAD r25, r6           ; r25 = length (loop counter)

draw_loop:
  LDI r6, 0x2000
  ADD r6, r11
  LOAD r8, r6            ; r8 = cell = (x<<8)|y

  ; x = r8 >> 8
  LDI r0, 8
  LDI r9, 0
  ADD r9, r8
  SHR r9, r0             ; r9 = x

  ; y = r8 & 0xFF
  LDI r1, 0xFF
  AND r1, r8             ; r1 = y

  ; pixel coords
  LDI r0, 8
  MUL r9, r0
  MUL r1, r0

  ; draw 7x7 green cell (1px gap gives grid look)
  LDI r22, 7
  LDI r23, 7
  LDI r24, 0x00CC44
  RECTF r9, r1, r22, r23, r24

  ; advance ring buffer index
  LDI r0, 1
  ADD r11, r0
  LDI r0, 0x1FF
  AND r11, r0

  ; decrement counter and loop
  LDI r0, 1
  SUB r25, r0
  JZ r25, draw_done
  JMP draw_loop

draw_done:
  RET

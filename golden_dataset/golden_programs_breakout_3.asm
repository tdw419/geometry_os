; DESCRIPTION: Draws a red object at the screen with fixed size.

; breakout.asm -- Breakout / Brick Breaker for Geometry OS
;
; Controls:
;   A/D or Left/Right arrows -- move paddle
;   W or Space -- launch ball
;   R -- restart game
;
; Layout (256x256 screen):
;   y=0..7:   score area (text)
;   y=8..39:  bricks (8 cols x 4 rows, 32x8 px each)
;   y=40..243: play field
;   y=244..249: paddle (48x6 px)
;
; Memory map:
;   0x3000..0x301F  brick[0..31]  (0=dead, color=alive)
;   0x3020          paddle_x      (0..208)
;   0x3021          ball_x
;   0x3022          ball_y
;   0x3023          ball_dx       (signed u32)
;   0x3024          ball_dy       (signed u32)
;   0x3025          score
;   0x3026          lives         (3)
;   0x3027          game_over     (0=play, 1=win, 2=lose)
;   0x3028          ball_launched (0=on paddle, 1=flying)
;   0x3029          bricks_left
;   0x3100..0x3106  score text buf (7 chars + null)

; ── init / restart ──────────────────────────────────────────────
restart:
  LDI r30, 0x8000

  ; write "SCORE:" text to buffer (stays constant)
  LDI r1, 0x3100
  LDI r14, 83
  STORE r1, r14        ; S
  LDI r1, 0x3101
  LDI r14, 67
  STORE r1, r14        ; C
  LDI r1, 0x3102
  LDI r14, 79
  STORE r1, r14        ; O
  LDI r1, 0x3103
  LDI r14, 82
  STORE r1, r14        ; R
  LDI r1, 0x3104
  LDI r14, 69
  STORE r1, r14        ; E
  LDI r1, 0x3105
  LDI r14, 58
  STORE r1, r14        ; :

  ; init bricks -- row 0=red, row 1=orange, row 2=yellow, row 3=green
  LDI r22, 0xFF0000   ; red
  LDI r23, 0xFF8800   ; orange
  LDI r24, 0xFFDD00   ; yellow
  LDI r25, 0x00CC44   ; green
  LDI r9, 0          ; brick index

ib_loop:
  LDI r4, 0
  ADD r4, r9
  LDI r5, 3
  SHR r4, r5          ; r4 = row (0-3)
  ; default green (row 3)
  LDI r14, 0
  ADD r14, r25
  ; check row 0
  LDI r5, 0
  CMP r4, r5
  JZ r0, ib_r0
  ; check row 1
  LDI r5, 1
  CMP r4, r5
  JZ r0, ib_r1
  ; check row 2
  LDI r5, 2
  CMP r4, r5
  JZ r0, ib_r2
  JMP ib_store        ; row 3 = green (already in r14)

ib_r0:
  LDI r14, 0
  ADD r14, r22         ; red
  JMP ib_store

ib_r1:
  LDI r14, 0
  ADD r14, r23         ; orange
  JMP ib_store

ib_r2:
  LDI r14, 0
  ADD r14, r24         ; yellow

ib_store:
  LDI r1, 0x3000
  ADD r1, r9
  STORE r1, r14
  LDI r4, 1
  ADD r9, r4
  LDI r5, 32
  CMP r9, r5
  BLT r0, ib_loop

  ; init game state
  LDI r14, 104
  LDI r1, 0x3020
  STORE r1, r14        ; paddle_x = 104 (centered)
  LDI r14, 0
  LDI r1, 0x3025
  STORE r1, r14        ; score = 0
  LDI r14, 3
  LDI r1, 0x3026
  STORE r1, r14        ; lives = 3
  LDI r14, 0
  LDI r1, 0x3027
  STORE r1, r14        ; game_over = 0
  LDI r14, 32
  LDI r1, 0x3029
  STORE r1, r14        ; bricks_left = 32

  CALL reset_ball

; ── main loop ────────────────────────────────────────────────────
game_loop:
  ; check game over
  LDI r1, 0x3027
  LOAD r14, r1
  JNZ r14, game_over_screen

  ; read keyboard
  IKEY r6

  ; A/a = paddle left
  LDI r5, 65
  CMP r6, r5
  JZ r0, pad_left
  LDI r5, 97
  CMP r6, r5
  JZ r0, pad_left

  ; D/d = paddle right
  LDI r5, 68
  CMP r6, r5
  JZ r0, pad_right
  LDI r5, 100
  CMP r6, r5
  JZ r0, pad_right

  ; W/w/Space = launch ball
  LDI r5, 87
  CMP r6, r5
  JZ r0, do_launch
  LDI r5, 119
  CMP r6, r5
  JZ r0, do_launch
  LDI r5, 32
  CMP r6, r5
  JZ r0, do_launch

  ; R/r = restart
  LDI r5, 82
  CMP r6, r5
  JZ r0, restart
  LDI r5, 114
  CMP r6, r5
  JZ r0, restart

after_input:
  ; if ball not launched, stick to paddle
  LDI r1, 0x3028
  LOAD r14, r1
  JNZ r14, update_ball

  ; ball sits on paddle
  LDI r1, 0x3020
  LOAD r14, r1          ; paddle_x
  LDI r4, 22
  ADD r14, r4           ; center ball on paddle
  LDI r1, 0x3021
  STORE r1, r14         ; ball_x
  LDI r14, 240
  LDI r1, 0x3022
  STORE r1, r14         ; ball_y = 240
  JMP draw_and_loop

update_ball:
  ; move ball x
  LDI r1, 0x3021
  LOAD r14, r1
  LDI r1, 0x3023
  LOAD r8, r1
  ADD r14, r8
  LDI r1, 0x3021
  STORE r1, r14

  ; move ball y
  LDI r1, 0x3022
  LOAD r14, r1
  LDI r1, 0x3024
  LOAD r8, r1
  ADD r14, r8
  LDI r1, 0x3022
  STORE r1, r14

  ; ── left wall bounce ──
  LDI r1, 0x3021
  LOAD r14, r1
  LDI r5, 0
  CMP r14, r5
  BGE r0, chk_rw
  LDI r14, 0
  LDI r1, 0x3021
  STORE r1, r14
  LDI r1, 0x3023
  LOAD r14, r1
  NEG r14
  LDI r1, 0x3023
  STORE r1, r14
  ; wall bounce sound
  LDI r4, 330
  LDI r5, 20
  BEEP r4, r5

chk_rw:
  ; ── right wall bounce ──
  LDI r1, 0x3021
  LOAD r14, r1
  LDI r5, 252          ; 256 - 4 (ball width)
  CMP r14, r5
  BLT r0, chk_tw
  LDI r14, 252
  LDI r1, 0x3021
  STORE r1, r14
  LDI r1, 0x3023
  LOAD r14, r1
  NEG r14
  LDI r1, 0x3023
  STORE r1, r14
  ; wall bounce sound
  LDI r4, 330
  LDI r5, 20
  BEEP r4, r5

chk_tw:
  ; ── top wall bounce ──
  LDI r1, 0x3022
  LOAD r14, r1
  LDI r5, 0
  CMP r14, r5
  BGE r0, chk_bot
  LDI r14, 0
  LDI r1, 0x3022
  STORE r1, r14
  LDI r1, 0x3024
  LOAD r14, r1
  NEG r14
  LDI r1, 0x3024
  STORE r1, r14
  ; wall bounce sound
  LDI r4, 330
  LDI r5, 20
  BEEP r4, r5

chk_bot:
  ; ── ball lost (fell off bottom) ──
  LDI r1, 0x3022
  LOAD r14, r1
  LDI r5, 256
  CMP r14, r5
  BLT r0, chk_brick
  CALL lose_life
  JMP draw_and_loop

chk_brick:
  ; ── brick collision ──
  LDI r1, 0x3022
  LOAD r14, r1          ; ball_y
  ; ball must be in brick area: y >= 8 and y < 40
  LDI r5, 8
  CMP r14, r5
  BLT r0, chk_paddle
  LDI r5, 40
  CMP r14, r5
  BGE r0, chk_paddle

  ; compute brick row = (ball_y - 8) >> 3
  LDI r4, 8
  SUB r14, r4
  LDI r4, 3
  SHR r14, r4           ; r14 = row (0-3)

  ; compute brick col = ball_x >> 5
  LDI r1, 0x3021
  LOAD r8, r1          ; ball_x
  LDI r4, 5
  SHR r8, r4           ; r8 = col (0-7)

  ; bounds check
  LDI r5, 4
  CMP r14, r5
  BGE r0, chk_paddle
  LDI r5, 8
  CMP r8, r5
  BGE r0, chk_paddle

  ; brick index = row * 8 + col
  LDI r12, 0
  ADD r12, r14
  LDI r4, 3
  SHL r12, r4           ; row * 8
  ADD r12, r8           ; + col

  ; check if brick alive
  LDI r1, 0x3000
  ADD r1, r12
  LOAD r14, r1
  JZ r14, chk_paddle   ; dead brick, skip

  ; kill brick
  LDI r14, 0
  STORE r1, r14

  ; decrement bricks_left
  LDI r1, 0x3029
  LOAD r14, r1
  LDI r4, 1
  SUB r14, r4
  LDI r1, 0x3029
  STORE r1, r14

  ; check win
  JNZ r14, brick_bounce
  LDI r14, 1
  LDI r1, 0x3027
  STORE r1, r14         ; game_over = 1 (won!)
  LDI r4, 880
  LDI r5, 200
  BEEP r4, r5
  JMP draw_and_loop

brick_bounce:
  ; reverse ball_dy
  LDI r1, 0x3024
  LOAD r14, r1
  NEG r14
  LDI r1, 0x3024
  STORE r1, r14
  ; score += 10
  LDI r1, 0x3025
  LOAD r14, r1
  LDI r4, 10
  ADD r14, r4
  LDI r1, 0x3025
  STORE r1, r14
  ; brick hit sound
  LDI r4, 660
  LDI r5, 30
  BEEP r4, r5

chk_paddle:
  ; ── paddle collision ──
  ; ball must be at paddle level (y >= 240) and moving down
  LDI r1, 0x3022
  LOAD r14, r1          ; ball_y
  LDI r5, 240
  CMP r14, r5
  BLT r0, draw_and_loop
  ; check ball moving down (dy > 0)
  LDI r1, 0x3024
  LOAD r14, r1
  LDI r5, 0
  CMP r14, r5
  BLT r0, draw_and_loop ; moving up, skip
  ; check x overlap (ball center vs paddle bounds)
  LDI r1, 0x3021
  LOAD r14, r1          ; ball_x
  LDI r1, 0x3020
  LOAD r8, r1          ; paddle_x
  LDI r4, 2
  ADD r14, r4           ; ball center x
  CMP r14, r8
  BLT r0, draw_and_loop ; left of paddle
  LDI r4, 48
  ADD r8, r4           ; paddle right edge
  CMP r14, r8
  BGE r0, draw_and_loop ; right of paddle

  ; paddle hit! reverse dy
  LDI r1, 0x3024
  LOAD r14, r1
  NEG r14
  LDI r1, 0x3024
  STORE r1, r14

  ; set dx from hit position (offset -24..24, clamped to -3..3)
  LDI r1, 0x3021
  LOAD r14, r1          ; ball_x
  LDI r4, 2
  ADD r14, r4           ; ball center x
  LDI r1, 0x3020
  LOAD r8, r1          ; paddle_x
  SUB r14, r8           ; offset (0-48)
  LDI r4, 24
  SUB r14, r4           ; -24..24

  ; clamp to [-3, 3]
  LDI r5, 0xFFFFFFFD   ; -3 unsigned
  CMP r14, r5
  BLT r0, dx_neg
  LDI r5, 3
  CMP r14, r5
  BGE r0, dx_pos
  JMP dx_store

dx_neg:
  LDI r14, 0xFFFFFFFD
  JMP dx_store

dx_pos:
  LDI r14, 3

dx_store:
  ; ensure dx is never 0 (would be boring)
  JNZ r14, dx_ok
  LDI r14, 1
dx_ok:
  LDI r1, 0x3023
  STORE r1, r14
  ; paddle hit sound
  LDI r4, 440
  LDI r5, 20
  BEEP r4, r5

draw_and_loop:
  CALL draw_frame
  FRAME
  JMP game_loop

; ── input handlers ───────────────────────────────────────────────
pad_left:
  LDI r1, 0x3020
  LOAD r14, r1
  LDI r4, 5
  SUB r14, r4
  LDI r5, 0
  CMP r14, r5
  BGE r0, pl_ok
  LDI r14, 0
pl_ok:
  LDI r1, 0x3020
  STORE r1, r14
  JMP after_input

pad_right:
  LDI r1, 0x3020
  LOAD r14, r1
  LDI r4, 5
  ADD r14, r4
  LDI r5, 208           ; 256 - 48
  CMP r14, r5
  BLT r0, pr_ok
  LDI r14, 208
pr_ok:
  LDI r1, 0x3020
  STORE r1, r14
  JMP after_input

do_launch:
  LDI r1, 0x3028
  LOAD r14, r1
  JNZ r14, after_input  ; already launched
  LDI r14, 1
  LDI r1, 0x3028
  STORE r1, r14
  JMP after_input

; ── subroutines ──────────────────────────────────────────────────
reset_ball:
  LDI r14, 0
  LDI r1, 0x3028
  STORE r1, r14         ; ball_launched = 0
  LDI r14, 2
  LDI r1, 0x3023
  STORE r1, r14         ; ball_dx = 2
  LDI r14, 0xFFFFFFFD   ; -3
  LDI r1, 0x3024
  STORE r1, r14         ; ball_dy = -3
  RET

lose_life:
  PUSH r31
  LDI r1, 0x3026
  LOAD r14, r1
  LDI r4, 1
  SUB r14, r4
  LDI r1, 0x3026
  STORE r1, r14
  JNZ r14, ll_reset
  ; game over (lost)
  LDI r14, 2
  LDI r1, 0x3027
  STORE r1, r14
  LDI r4, 110
  LDI r5, 300
  BEEP r4, r5
  POP r31
  RET

ll_reset:
  CALL reset_ball
  POP r31
  RET

; ── game over screen ─────────────────────────────────────────────
game_over_screen:
  LDI r1, 0x3027
  LOAD r14, r1
  LDI r5, 1
  CMP r14, r5
  JZ r0, win_screen

  ; lose screen (dark red)
  LDI r14, 0x330000
  FILL r14
  FRAME
  IKEY r6
  JZ r6, game_over_screen
  JMP restart

win_screen:
  ; win screen (dark green)
  LDI r14, 0x003300
  FILL r14
  FRAME
  IKEY r6
  JZ r6, win_screen
  JMP restart

; ─────────────────────────────────────────────────────────────────
; draw_frame -- render entire screen
;   Clobbers r14-r2, r9. Preserves r20-r25.
; ─────────────────────────────────────────────────────────────────
draw_frame:
  ; dark background
  LDI r14, 0x000811
  FILL r14

  ; ── draw bricks ──
  LDI r9, 0           ; brick index
  LDI r20, 0x3000      ; brick base

df_bloop:
  ; load brick color
  LDI r1, 0
  ADD r1, r20
  ADD r1, r9
  LOAD r14, r1          ; r14 = color (0 = dead)
  JZ r14, df_bnext

  ; compute pixel position
  LDI r4, 0
  ADD r4, r9
  LDI r5, 7
  AND r4, r5           ; r4 = col (0-7)
  LDI r13, 5
  SHL r4, r13           ; r4 = col * 32

  LDI r5, 0
  ADD r5, r9
  LDI r13, 3
  SHR r5, r13           ; r5 = row (0-3)
  LDI r13, 3
  SHL r5, r13           ; r5 = row * 8
  LDI r13, 9
  ADD r5, r13           ; r5 = row * 8 + 9 (1px top gap)

  ; draw brick: RECTF(x+1, y+1, 30, 6, color)
  LDI r6, 1
  ADD r4, r6           ; x + 1
  ADD r5, r6           ; y + 1
  LDI r6, 30           ; width
  LDI r13, 6            ; height
  RECTF r4, r5, r6, r13, r14

df_bnext:
  LDI r4, 1
  ADD r9, r4
  LDI r5, 32
  CMP r9, r5
  BLT r0, df_bloop

  ; ── draw paddle ──
  LDI r1, 0x3020
  LOAD r4, r1          ; r4 = paddle_x
  LDI r5, 244           ; y
  LDI r6, 48            ; width
  LDI r13, 6             ; height
  LDI r2, 0xCCCCCC      ; light gray
  RECTF r4, r5, r6, r13, r2

  ; ── draw ball ──
  LDI r1, 0x3021
  LOAD r4, r1          ; ball_x
  LDI r1, 0x3022
  LOAD r5, r1          ; ball_y
  LDI r6, 4            ; width
  LDI r13, 4            ; height
  LDI r2, 0xFFFFFF     ; white
  RECTF r4, r5, r6, r13, r2

  ; ── draw score text ──
  CALL draw_score

  ; ── draw lives (small rects at bottom-right) ──
  LDI r1, 0x3026
  LOAD r14, r1          ; lives
  LDI r2, 0            ; loop counter
  LDI r9, 240         ; x start
df_lloop:
  CMP r2, r14
  BGE r0, df_ldone
  LDI r4, 0
  ADD r4, r9
  LDI r5, 252           ; y
  LDI r6, 4             ; w
  LDI r13, 4             ; h
  LDI r8, 0x00CC44     ; green
  RECTF r4, r5, r6, r13, r8
  LDI r4, 6
  ADD r9, r4           ; next life dot x += 6
  LDI r4, 1
  ADD r2, r4
  JMP df_lloop

df_ldone:
  RET

; ─────────────────────────────────────────────────────────────────
; draw_score -- convert score to 3-digit ASCII and display with TEXT
;   Writes digits to RAM[0x3106..0x3108], null at 0x3109
; ─────────────────────────────────────────────────────────────────
draw_score:
  LDI r1, 0x3025
  LOAD r14, r1          ; r14 = score

  ; hundreds digit: score / 100
  LDI r8, 0
  ADD r8, r14           ; r8 = score copy
  LDI r12, 100
  DIV r8, r12           ; r8 = hundreds
  LDI r4, 48
  ADD r8, r4           ; r8 = '0' + hundreds
  LDI r1, 0x3106
  STORE r1, r8

  ; tens digit: (score - hundreds*100) / 10
  LDI r12, 100
  LDI r5, 0
  ADD r5, r8           ; r5 = ASCII hundreds
  LDI r4, 48
  SUB r5, r4           ; r5 = numeric hundreds
  MUL r5, r12           ; r5 = hundreds * 100
  LDI r8, 0
  ADD r8, r14           ; r8 = score
  SUB r8, r5           ; r8 = remainder (< 100)
  LDI r12, 10
  DIV r8, r12           ; r8 = tens
  LDI r4, 48
  ADD r8, r4           ; r8 = '0' + tens
  LDI r1, 0x3107
  STORE r1, r8

  ; ones digit: remainder - tens*10
  LDI r12, 10
  LDI r5, 0
  ADD r5, r8           ; r5 = ASCII tens
  LDI r4, 48
  SUB r5, r4           ; r5 = numeric tens
  MUL r5, r12           ; r5 = tens * 10
  SUB r8, r5           ; r8 = ones
  LDI r4, 48
  ADD r8, r4           ; r8 = '0' + ones
  LDI r1, 0x3108
  STORE r1, r8

  ; null terminator
  LDI r14, 0
  LDI r1, 0x3109
  STORE r1, r14

  ; TEXT opcode to render "SCORE:XXX"
  LDI r4, 2             ; x
  LDI r5, 0             ; y
  LDI r6, 0x3100        ; string address
  TEXT r4, r5, r6

  RET

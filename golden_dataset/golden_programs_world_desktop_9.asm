...
LOAD r18, r17                 ; r18 = CMD_LEN
CMPI r18, 63
BGE r5, cmd_type_no_key       ; buffer full
LDI r19, 0x7832
...
MOV r3, r2             ; world_x = player_x
MOV r1, r17             ; world_y = target_y
CALL check_biome_walkable
JZ r5, up_blocked       ; r5 == 0 means not walkable
JMP no_up
up_blocked:
LDI r18, 0x7809
...
ent_update_loop:
  CMP r26, r21
  BGE r5, ent_update_done

  ; Load entity fields
  MOV r22, r20
  LOAD r3, r22             ; world_x (r3 for biome check)
  ADDI r22, 1
  LOAD r1, r22             ; world_y (r1 for biome check)
  ADDI r22, 1
  LOAD r17, r22            ; type
  ADDI r22, 1
...
ent_not_program:
  ...
  LDI r22, 1
  CMP r17, r22
  JZ r5, ent_ai_wander     ; wanderer
  LDI r22, 2
  CMP r17, r22
  JZ r5, ent_ai_guard      ; guard
  LDI r22, 3
  CMP r17, r22
  JZ r5, ent_ai_animal     ; animal (flee)
  LDI r22, 4
  CMP r17, r22
  JZ r5, ent_ai_ghost      ; ghost
  LDI r22, 5
  CMP r17, r22
  JZ r5, ent_ai_area_agent ; area_agent
...
ent_agent_not_idle:
  LDI r22, 1
  CMP r17, r22
  JZ r5, ent_agent_state_moving
  LDI r22, 3
  CMP r17, r22
  JZ r5, ent_agent_state_inside
  LDI r22, 5
  CMP r17, r22
  JZ r5, ent_agent_state_returning
...
ent_agent_state_moving:
  ...
  MOV r17, r24
  SUB r17, r1               ; dy = target_y - entity_y
  ...
  LDI r23, 2
  CMP r22, r23
  BLT r5, ent_agent_check_dy  ; dx < 2, check dy
  JMP ent_agent_move_dx       ; dx >= 2, move
ent_agent_dx_neg:
  NEG r22                   ; |dx|
  LDI r23, 2
  CMP r22, r23
  BLT r5, ent_agent_check_dy  ; |dx| < 2
...
ent_agent_check_dy:
  ...
  LDI r23, 2
  CMP r17, r23
  BLT r5, ent_agent_arrived   ; dy < 2, arrived!
...
ent_agent_arrived:
  ...
  LDI r23, 4
  CMP r17, r23
  BGE r5, ent_agent_arrived_done  ; mailbox full
  ADDI r17, 1
...
ent_agent_move_dy:
  ...
  SUB r22, r1               ; dy = target_y - entity_y
  LDI r23, 31
  MOV r25, r22
  SAR r25, r23
...
ent_agent_state_returning:
  ...
  LDI r24, 2
  CMP r22, r24
  BLT r5, ent_agent_home_check_dy
...
ent_do_biome_check:
  ...
  MOV r1, r23
  CALL check_biome_walkable
  JZ r5, ent_move_blocked  ; not walkable, skip move
...
; r4 = tiles per axis, so center offset = r4 / 2
MOV r17, r4
LDI r18, 1
...
MOV r18, r0
SUB r18, r17
STORE r10, r18            ; camera_y = player_y - tiles/2
...
render_y:
  ...
  MOV r26, r0
  ADD r26, r12            ; r26 = world_y for this row
  LDI r18, 3
...
ypre_done:
  ...
  render_x:
    ; World coordinates
    MOV r3, r2
    ADD r3, r6           ; r3 = world_x
    MOV r1, r0
    ADD r1, r12           ; r1 = world_y
...
no_claim:
    ...
    SHR r15, r18          ; r15 = world_y >> 3
    LDI r18, 99001
    MUL r9, r18          ; r9 = x_hash
    LDI r18, 79007
    MUL r15, r18          ; r15 = y_hash
    XOR r9, r15           ; r9 = coarse_hash
    LDI r18, 1103515245
...
no_yblend:
    ...
    ; r15 = world_x * 374761393 XOR world_y * 668265263
    ...
    LDI r18, 668265263
    MUL r21, r18
    XOR r15, r21           ; r15 = fine_hash (THE SEED, 32 bits of goodness)
...
tile_done:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r5, contour_clr_skip   ; detail < 2 → skip
...
contour_after:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r5, no_tree        ; detail < 2 → skip trees
...
sky_night:
  ...
  LDI r15, 0x0A0A30       ; horizon: dark navy
  JMP sky_draw

sky_dawn:
  LDI r9, 0x101040       ; top: deep blue-purple
  LDI r15, 0xCC6600       ; horizon: warm orange
  JMP sky_draw

sky_day:
  LDI r9, 0x1844AA       ; top: medium blue
  LDI r15, 0x5599DD       ; horizon: light sky blue
  JMP sky_draw

sky_dusk:
  LDI r9, 0x0C0820       ; top: dark purple
  LDI r15, 0xDD4400       ; horizon: deep orange-red
  JMP sky_draw
...
LDI r17, 2
CMP r18, r17
BLT r5, mm_skip          ; detail < 2 → skip minimap
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 2
    MUL r3, r18
    ADD r3, r2          ; r3 = camera_x + mx*2

    MOV r1, r12
    LDI r18, 2
    MUL r1, r18
    ADD r1, r0          ; r1 = camera_y + my*2
    ...
    MOV r15, r1
    LDI r18, 3
    SHR r15, r18          ; world_y >> 3
    LDI r18, 79007
    MUL r15, r18          ; y_hash

    XOR r9, r15           ; coarse_hash
    LDI r18, 1103515245
...
LDI r3, 224
LDI r1, 0
RECTF r3, r1, r19, r18, r17    ; top edge
LDI r1, 31
RECTF r3, r1, r19, r18, r17    ; bottom edge
LDI r1, 0
RECTF r3, r1, r18, r19, r17    ; left edge
LDI r3, 255
RECTF r3, r1, r18, r19, r17    ; right edge
...
LDI r17, 2
CMP r18, r17
BLT r5, player_simple   ; detail < 2 → simple marker
...
LDI r17, 2
CMP r18, r17
BLT r5, bldg_simple_dots   ; detail < 2 → simple dots
...
bldg_dot_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r5, bldg_dot_next
  MOV r29, r4
  SUB r29, r11           ; tiles - 1
  CMP r28, r29
  BGE r5, bldg_dot_next
...
bldg_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r5, bldg_next     ; off-screen left
  LDI r29, 62
  CMP r28, r29
  BGE r5, bldg_next     ; off-screen right
  ...
  LDI r18, 0
  CMP r29, r18
  BLT r5, bldg_next     ; off-screen top
  LDI r18, 58
  CMP r29, r18
  BGE r5, bldg_next     ; off-screen bottom
...
ent_render_loop:
  ...
  LDI r25, 0
  CMP r27, r25
  BLT r5, ent_render_next
  MOV r25, r4
  SUB r25, r11              ; tiles - 1
  CMP r27, r25
  BGE r5, ent_render_next
...
ent_render_not_program:
  ...
  LDI r25, 1
  CMP r17, r25
  JZ r5, ent_render_agent   ; wanderer

  LDI r25, 2
  CMP r17, r25
  JZ r5, ent_render_guard   ; guard

  LDI r25, 3
  CMP r17, r25
  JZ r5, ent_render_animal  ; animal

  LDI r25, 4
  CMP r17, r25
  JZ r5, ent_render_ghost   ; ghost

  LDI r25, 5
  CMP r17, r25
  JZ r5, ent_render_area_agent ; area_agent
...
ent_agent_color:
  ...
  MOV r25, r18
  LDI r15, 16
  SHR r25, r15         ; direction
  LDI r15, 0
  CMP r25, r15
  JNZ r5, ent_agent_not_up2
  LDI r25, 0
  ADD r25, r27             ; dx=0
  SUB r28, r11              ; dy=-1
  LDI r15, 0xFFFFFF
  PSET r25, r28, r15   ; white dot above
  ADDI r28, 1              ; restore y
  JMP ent_agent_dir_done
ent_agent_not_up2:
  LDI r15, 1
  CMP r25, r15
  JNZ r5, ent_agent_not_down2
  LDI r25, 0
  ADD r25, r27
  ADD r28, r11              ; dy=+1
  LDI r15, 0xFFFFFF
...
ent_animal_color:
  ...
  PSET r27, r28, r15        ; left ear
  ADDI r27, 1
  PSET r27, r28, r15        ; right ear
  JMP ent_render_next
...
ent_ghost_visible:
  ...
  MOV r25, r19
  LDI r15, 3
  AND r25, r15              ; 0-3 phase
  LDI r17, 0x9933FF        ; purple base
  LDI r15, 1
  CMP r25, r15
  JNZ r5, ent_ghost_not_p2
  LDI r17, 0xBB66FF        ; light purple
  JMP ent_ghost_draw
ent_ghost_not_p2:
  LDI r15, 2
  CMP r25, r15
  JNZ r5, ent_ghost_not_p3
  LDI r17, 0x7722CC        ; dark purple
  JMP ent_ghost_draw
ent_ghost_not_p3:
  LDI r15, 3
  CMP r25, r15
  JNZ r5, ent_ghost_draw
...
ent_ghost_draw:
  ...
  SUBI r28, 2              ; eye row
  LDI r15, 0xFFFFFF
  PSET r27, r28, r15        ; center eye
  SUBI r27, 1
  PSET r27, r28, r15        ; left eye
  ADDI r27, 2
  PSET r27, r28, r15        ; right eye
  JMP ent_render_next
...
mm_ent_not_program:
  LDI r25, 1
  CMP r17, r25
  JNZ r5, mm_ent_not_wanderer
  LDI r18, 0xFF8800        ; orange (wanderer)
  JMP mm_ent_dot
mm_ent_not_wanderer:
  LDI r25, 2
  CMP r17, r25
  JNZ r5, mm_ent_not_guard
  LDI r18, 0xFF2020        ; red (guard)
  JMP mm_ent_dot
mm_ent_not_guard:
  LDI r25, 3
  CMP r17, r25
  JNZ r5, mm_ent_not_animal
...
LDI r18, 0xFFFFFFFF
CMP r17, r18
JZ r5, no_entity_tooltip   ; -1 = no entity nearby
...
LDI r18, 1
CMP r17, r18
JZ r5, ent_tooltip_wanderer
LDI r18, 2
CMP r17, r18
JZ r5, ent_tooltip_guard
LDI r18, 3
CMP r17, r18
JZ r5, ent_tooltip_animal
...
LDI r17, 2
CMP r18, r17
BGE r5, player_marker_done   ; detail >= 2 → skip (full sprite already drawn)
...
LDI r3, 128
LDI r1, 128
PSET r3, r1, r17            ; center dot
LDI r3, 127
PSET r3, r1, r17            ; left
LDI r3, 129
LDI r1, 128
PSET r3, r1, r17            ; right
LDI r3, 128
LDI r1, 127
PSET r3, r1, r17            ; up
LDI r1, 129
...
LOAD r18, r17
CMPI r18, 47               ; '/'
JNZ r5, cmd_oracle          ; not a / command → send to Oracle
...
CMPI r18, 116               ; 't'
JNZ r5, cmd_try_build
ADDI r17, 1
LOAD r18, r17
CMPI r18, 112               ; 'p'
JNZ r5, cmd_oracle          ; not /tp, fall through to oracle

; Parse first number (x) after /tp
ADDI r17, 1                  ; skip 'p'
CALL parse_next_number       ; r5 = number, r17 advanced past it
LDI r19, 0x7808
STORE r19, r5                ; player_x = parsed x

; Parse second number (y)
CALL parse_next_number
LDI r19, 0x7809
STORE r19, r5                ; player_y = parsed y
...
CMPI r18, 32
BGE r5, cmd_build_full       ; max 32 buildings
...
; Output: r5 = parsed number, r17 = advanced past number + space
; Uses r6 as accumulator (r5 is clobbered by CMPI!)
; =========================================
parse_next_number:
PUSH r31
LDI r12, 1
...
LOAD r18, r17
CMPI r18, 32                ; space
JNZ r5, pnn_digit_start
ADDI r17, 1
JMP pnn_skip

pnn_digit_start:
LDI r6, 0                   ; accumulator in r6 (NOT r5, CMPI clobbers r5)
...
pnn_done:
MOV r5, r6                  ; move result to r5 for caller
POP r31
RET

; ===== check_biome_walkable subroutine =====
; Input: r3 = world_x, r1 = world_y
; Output: r5 = 1 (walkable) or 0 (blocked)
; Preserves: r12-r15, r2-r0
; Uses: r17-r22 as temporaries
; Clobbers: r5 (CMP result), r31 (saved/restored via push/pop)
...
LDI r17, 16
CMP r21, r17
BLT r5, biome_not_lava
LDI r17, 19
CMP r21, r17
BGE r5, biome_not_lava
JMP biome_blocked

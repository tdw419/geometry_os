...
LDI r18, 0x7808
LOAD r11, r18           ; r11 = player_x
LDI r18, 0x7809
LOAD r13, r18           ; r13 = player_y
...
LOAD r18, r17                 ; r18 = CMD_LEN
CMPI r18, 63
BGE r12, cmd_type_no_key       ; buffer full
LDI r19, 0x7832
...
MOV r0, r11             ; world_x = player_x
MOV r15, r17             ; world_y = target_y
CALL check_biome_walkable
JZ r12, up_blocked       ; r12 == 0 means not walkable
JMP no_up
up_blocked:
LDI r18, 0x7809
STORE r18, r13          ; revert player_y to original
no_up:
...
LDI r18, 0x7808
STORE r18, r17
MOV r0, r17             ; world_x = target_x
MOV r15, r13             ; world_y = player_y
CALL check_biome_walkable
JZ r12, left_blocked
JMP no_left
left_blocked:
LDI r18, 0x7808
STORE r18, r11          ; revert player_x to original
no_left:
...
ent_update_loop:
  CMP r26, r21
  BGE r12, ent_update_done

  ; Load entity fields
  MOV r22, r20
  LOAD r0, r22             ; world_x (r0 for biome check)
  ADDI r22, 1
...
ent_not_program:
  ...
  LDI r22, 1
  CMP r17, r22
  JZ r12, ent_ai_wander     ; wanderer
  LDI r22, 2
  CMP r17, r22
  JZ r12, ent_ai_guard      ; guard
  LDI r22, 3
  CMP r17, r22
  JZ r12, ent_ai_animal     ; animal (flee)
  LDI r22, 4
  CMP r17, r22
  JZ r12, ent_ai_ghost      ; ghost
  LDI r22, 5
  CMP r17, r22
  JZ r12, ent_ai_area_agent ; area_agent
...
ent_agent_not_idle:
  LDI r22, 1
  CMP r17, r22
  JZ r12, ent_agent_state_moving
  LDI r22, 3
  CMP r17, r22
  JZ r12, ent_agent_state_inside
  LDI r22, 5
  CMP r17, r22
  JZ r12, ent_agent_state_returning
...
ent_agent_state_moving:
  ...
  MOV r22, r23
  SUB r22, r0               ; dx = target_x - entity_x
  MOV r17, r24
  ...
  LDI r23, 2
  CMP r22, r23
  BLT r12, ent_agent_check_dy  ; dx < 2, check dy
  JMP ent_agent_move_dx       ; dx >= 2, move
ent_agent_dx_neg:
  NEG r22                   ; |dx|
  LDI r23, 2
  CMP r22, r23
  BLT r12, ent_agent_check_dy  ; |dx| < 2
...
ent_agent_check_dy:
  ...
  LDI r23, 2
  CMP r17, r23
  BLT r12, ent_agent_arrived   ; dy < 2, arrived!
...
ent_agent_arrived:
  ...
  LDI r23, 4
  CMP r17, r23
  BGE r12, ent_agent_arrived_done  ; mailbox full
  ADDI r17, 1
...
ent_agent_state_returning:
  ...
  LDI r24, 2
  CMP r22, r24
  BLT r12, ent_agent_home_check_dy
...
ent_agent_home_check_dy:
  ...
  LDI r24, 2
  CMP r17, r24
  BLT r12, ent_agent_home_arrived
...
ent_not_up:
  LDI r24, 1
  CMP r25, r24
  JNZ r12, ent_not_down
  ADD r23, r9              ; target_y += 1
  JMP ent_do_biome_check
ent_not_down:
  LDI r24, 2
  CMP r25, r24
  JNZ r12, ent_not_left
...
ent_do_biome_check:
  ...
  MOV r0, r22              ; target coords for biome check
  MOV r15, r23
  CALL check_biome_walkable
  JZ r12, ent_move_blocked  ; not walkable, skip move
...
ent_nt_up2:
  LDI r24, 1
  CMP r25, r24
  JNZ r12, ent_nt_down2
  ADD r23, r9
  JMP ent_do_move_no_check
ent_nt_down2:
  LDI r24, 2
  CMP r25, r24
  JNZ r12, ent_nt_left2
  SUB r22, r9
  JMP ent_do_move_no_check
ent_nt_left2:
  ADD r22, r9
...
; r14 = tiles per axis, so center offset = r14 / 2
MOV r17, r14
LDI r18, 1
...
render_y:
  ...
  MOV r26, r13
  ADD r26, r7            ; r26 = world_y for this row
  LDI r18, 3
...
ypre_done:
  ...
  render_x:
    ; World coordinates
    MOV r0, r11
    ADD r0, r10           ; r0 = world_x
    MOV r15, r13
    ADD r15, r7           ; r15 = world_y
...
water_checked:
    ...
    ADD r20, r0              ; fc + world_x (cheap position variation)
...
tile_done:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r12, contour_clr_skip   ; detail < 2 → skip
...
contour_after:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r12, no_tree        ; detail < 2 → skip trees
...
LDI r17, 2
CMP r18, r17
BLT r12, mm_skip          ; detail < 2 → skip minimap
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 2
    MUL r0, r18
    ADD r0, r11          ; r0 = camera_x + mx*2

    MOV r15, r7
    LDI r18, 2
    MUL r15, r18
    ADD r15, r13          ; r15 = camera_y + my*2
...
LDI r0, 224
LDI r15, 0
RECTF r0, r15, r19, r18, r17    ; top edge
LDI r15, 31
RECTF r0, r15, r19, r18, r17    ; bottom edge
LDI r15, 0
RECTF r0, r15, r18, r19, r17    ; left edge
LDI r0, 255
RECTF r0, r15, r18, r19, r17    ; right edge
...
LDI r17, 2
CMP r18, r17
BLT r12, player_simple   ; detail < 2 → simple marker
...
LDI r17, 2
CMP r18, r17
BLT r12, bldg_simple_dots   ; detail < 2 → simple dots
...
bldg_dot_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r12, bldg_dot_next
  MOV r29, r14
  SUB r29, r9           ; tiles - 1
  CMP r28, r29
  BGE r12, bldg_dot_next
...
bldg_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r12, bldg_next     ; off-screen left
  LDI r29, 62
  CMP r28, r29
  BGE r12, bldg_next     ; off-screen right
  ...
  LDI r18, 0
  CMP r29, r18
  BLT r12, bldg_next     ; off-screen top
  LDI r18, 58
  CMP r29, r18
  BGE r12, bldg_next     ; off-screen bottom
...
ent_render_loop:
  ...
  LDI r25, 0
  CMP r27, r25
  BLT r12, ent_render_next
  MOV r25, r14
  SUB r25, r9              ; tiles - 1
  CMP r27, r25
  BGE r12, ent_render_next
...
ent_render_not_program:
  ...
  LDI r25, 1
  CMP r17, r25
  JZ r12, ent_render_agent   ; wanderer

  LDI r25, 2
  CMP r17, r25
  JZ r12, ent_render_guard   ; guard

  LDI r25, 3
  CMP r17, r25
  JZ r12, ent_render_animal  ; animal

  LDI r25, 4
  CMP r17, r25
  JZ r12, ent_render_ghost   ; ghost

  LDI r25, 5
  CMP r17, r25
  JZ r12, ent_render_area_agent ; area_agent
...
ent_ghost_visible:
  ...
  LDI r3, 1
  CMP r25, r3
  JNZ r12, ent_ghost_not_p2
  LDI r17, 0xBB66FF        ; light purple
  JMP ent_ghost_draw
ent_ghost_not_p2:
  LDI r3, 2
  CMP r25, r3
  JNZ r12, ent_ghost_not_p3
  LDI r17, 0x7722CC        ; dark purple
  JMP ent_ghost_draw
ent_ghost_not_p3:
  LDI r3, 3
  CMP r25, r3
  JNZ r12, ent_ghost_draw
...
mm_ent_not_program:
  LDI r25, 1
  CMP r17, r25
  JNZ r12, mm_ent_not_wanderer
  LDI r18, 0xFF8800        ; orange (wanderer)
  JMP mm_ent_dot
mm_ent_not_wanderer:
  LDI r25, 2
  CMP r17, r25
  JNZ r12, mm_ent_not_guard
  LDI r18, 0xFF2020        ; red (guard)
  JMP mm_ent_dot
mm_ent_not_guard:
  LDI r25, 3
  CMP r17, r25
  JNZ r12, mm_ent_not_animal
...
LDI r18, 0xFFFFFFFF
CMP r17, r18
JZ r12, no_entity_tooltip   ; -1 = no entity nearby
...
LDI r18, 1
CMP r17, r18
JZ r12, ent_tooltip_wanderer
LDI r18, 2
CMP r17, r18
JZ r12, ent_tooltip_guard
LDI r18, 3
CMP r17, r18
JZ r12, ent_tooltip_animal
...
LDI r17, 2
CMP r18, r17
BGE r12, player_marker_done   ; detail >= 2 → skip (full sprite already drawn)
...
LDI r0, 128
LDI r15, 128
PSET r0, r15, r17            ; center dot
LDI r0, 127
PSET r0, r15, r17            ; left
LDI r0, 129
LDI r15, 128
PSET r0, r15, r17            ; right
LDI r0, 128
LDI r15, 127
PSET r0, r15, r17            ; up
LDI r15, 129
PSET r0, r15, r17            ; down
player_marker_done:
...
LOAD r18, r17
CMPI r18, 47               ; '/'
JNZ r12, cmd_oracle          ; not a / command → send to Oracle
...
CMPI r18, 116               ; 't'
JNZ r12, cmd_try_build
ADDI r17, 1
LOAD r18, r17
CMPI r18, 112               ; 'p'
JNZ r12, cmd_oracle          ; not /tp, fall through to oracle

; Parse first number (x) after /tp
ADDI r17, 1                  ; skip 'p'
CALL parse_next_number       ; r12 = number, r17 advanced past it
LDI r19, 0x7808
STORE r19, r12                ; player_x = parsed x

; Parse second number (y)
CALL parse_next_number
LDI r19, 0x7809
STORE r19, r12                ; player_y = parsed y
...
CMPI r18, 32
BGE r12, cmd_build_full       ; max 32 buildings
...
LDI r0, 0x7832              ; prompt = CMD_BUF text
LDI r15, 0x7873              ; response buffer
LDI r1, 895                 ; max response length
LLM r0, r15, r1
...
; Output: r12 = parsed number, r17 = advanced past number + space
; Uses r10 as accumulator (r12 is clobbered by CMPI!)
; =========================================
parse_next_number:
PUSH r31
LDI r7, 1
...
LOAD r18, r17
CMPI r18, 32                ; space
JNZ r12, pnn_digit_start
ADDI r17, 1
JMP pnn_skip

pnn_digit_start:
LDI r10, 0                   ; accumulator in r10 (NOT r12, CMPI clobbers r12)
...
pnn_done:
MOV r12, r10                  ; move result to r12 for caller
POP r31
RET

; ===== check_biome_walkable subroutine =====
; Input: r0 = world_x, r15 = world_y
; Output: r12 = 1 (walkable) or 0 (blocked)
; Preserves: r7-r3, r11-r13
; Uses: r17-r22 as temporaries
; Clobbers: r12 (CMP result), r31 (saved/restored via push/pop)
...
LDI r17, 2
CMP r21, r17
BLT r12, biome_blocked

; Mountain: biome 13-14 (13 <= biome < 15)
LDI r17, 13
CMP r21, r17
BLT r12, biome_not_mt
LDI r17, 15
CMP r21, r17
BGE r12, biome_not_mt
JMP biome_blocked
...
LDI r17, 16
CMP r21, r17
BLT r12, biome_not_lava
LDI r17, 19
CMP r21, r17
BGE r12, biome_not_lava
JMP biome_blocked

...
LDI r10, 8           ; shift amount
SHL r4, r10          ; r4 = 0x0000AA00 (R at bits 8-15)
LDI r2, 0xBB        ; G channel
OR r4, r2           ; r4 = 0x0000AABB (R+G)
LDI r10, 16
SHL r4, r10          ; r4 = 0xAABB0000 (R+G shifted up)
LDI r2, 0xCC        ; B channel
OR r4, r2           ; r4 = 0xAABBCC (packed: R=bits16-23, G=bits8-15, B=bits0-7)

; Now extract each channel using BFE
; BFE r0, r4, r12, r14 -- extract 8 bits at lsb=16 from r4 -> R
LDI r12, 8           ; width = 8
LDI r14, 16          ; lsb = 16
BFE r0, r4, r12, r14  ; r0 = 0xAA (red channel)

; BFE r9, r4, r12, r11 -- extract 8 bits at lsb=8 from r4 -> G
LDI r11, 8           ; lsb = 8
BFE r9, r4, r12, r11  ; r9 = 0xBB (green channel)

; BFE r13, r4, r3, r1 -- extract 8 bits at lsb=0 from r4 -> B
LDI r3, 8          ; width = 8
LDI r1, 0          ; lsb = 0
BFE r13, r4, r3, r1 ; r13 = 0xCC (blue channel)

; Now reassemble as BGR: B at bits 16-23, G at bits 8-15, R at bits 0-7
LDI r8, 0          ; start with 0

; Insert B (r13) at bits 16-23
LDI r6, 8          ; width = 8
LDI r7, 16         ; lsb = 16
MOV r15, r13         ; src = blue
BFI r8, r15, r6, r7 ; r8 = 0x00CC0000

; Insert G (r9) at bits 8-15
LDI r7, 8          ; lsb = 8
MOV r15, r9         ; src = green
BFI r8, r15, r6, r7 ; r8 = 0x00CCBB00

; Insert R (r0) at bits 0-7
LDI r7, 0          ; lsb = 0
MOV r15, r0         ; src = red
BFI r8, r15, r6, r7 ; r8 = 0x00CCBBAA (BGR swapped!)
...
LDI r20, 60
LDI r21, 10
RECTF r20, r21, r22, r23, r8 ; swapped
...
LDI r12, 1
LDI r14, 0
BFE r17, r16, r12, r14  ; r17 = bit 0 of 0xAA = 0

; Extract bit 1 (should be 1)
LDI r14, 1
BFE r18, r16, r12, r14  ; r18 = bit 1 of 0xAA = 1

; Extract bit 7 (should be 1)
LDI r14, 7
BFE r19, r16, r12, r14  ; r19 = bit 7 of 0xAA = 1

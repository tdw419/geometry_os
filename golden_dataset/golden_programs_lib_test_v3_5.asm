...
; T21: memset(3 words of 'A') → strlen == 3
    ...
    ADD r8, r3
    LDI r3, 0
    STORE r8, r3           ; null terminate after 3 'A's
    LDI r10, buf_a
    CALL strlen
    LDI r8, 0xF94
    LDI r5, 3
...
; T40: malloc returns address >= HEAP_START
    ...
    ; Actually: after CALL malloc, r3 = addr. Then LDI r8 sets r8.
    ; CMP r3, r5 → r3 = CMP(addr, 0xC000)
    ; Then CMP r3, r5 → r3 = CMP(CMP_result, 0xFFFFFFFF)
...
; T42: Two mallocs return different addresses
    LDI r10, 10
    CALL malloc
    MOV r14, r3             ; save first addr
    LDI r10, 10
    CALL malloc
    ; r3 should differ from r14
    CMP r3, r14
    LDI r8, 0xFA9
    ; CMP == 0 means equal (fail), CMP != 0 means different (pass)
    CALL _check_neq

; T43: malloc preserves callee-saved r7-r11
    ; Set r7-r11 to known values
    LDI r7, 0xAAAA
    LDI r1, 0xBBBB
    LDI r11, 0xCCCC
    LDI r10, 20
...
; T44: malloc preserves callee-saved r1-r11
    LDI r5, 0xBBBB
    CMP r1, r5
    LDI r8, 0xFAB
    CALL _check_result

; T45: malloc preserves callee-saved r11
    LDI r5, 0xCCCC
    CMP r11, r5
    LDI r8, 0xFAC
    CALL _check_result
...
; T54: point_in_triangle preserves r7-r12 (was broken in v1.1.0)
    LDI r7, 0x1111
    LDI r1, 0x2222
    LDI r11, 0x3333
    LDI r6, 0x4444
...
; T55: point_in_triangle preserves r1-r6
    LDI r5, 0x2222
    CMP r1, r5
    LDI r8, 0xFB6
    CALL _check_result

; T56: point_in_triangle preserves r0-r12
    LDI r5, 0x5555
    CMP r0, r5
    LDI r8, 0xFB7
    CALL _check_result

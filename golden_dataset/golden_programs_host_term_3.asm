...
STORE r20, r14
ADD r20, r3
STORE r20, r14       ; word2 (cols 64-79): only bits 0,8 used (cols 64,72)
...
process_byte:
    ...
    CMPI r13, ANS_NORMAL
    JNZ r15, pb_check_esc

    CMPI r2, 27
    JNZ r15, pb_normal_byte
...
pb_normal_byte:
    ...
    BLT r15, pb_ascii
    CMPI r2, 224
    BGE r15, pb_utf8_3_start

    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r2 & 0x1F) << 6
    LDI r9, 31
    AND r2, r9
    LDI r9, 6
    SHL r2, r9
...
pb_utf8_3_start:
    CMPI r2, 240
    BGE r15, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r2 & 0x0F) << 12
    LDI r9, 15
    AND r2, r9
    LDI r9, 12
    SHL r2, r9
...
pb_check_esc:
    CMPI r13, ANS_ESC
    JNZ r15, pb_check_csi

    CMPI r2, 91
    JNZ r15, pb_esc_check_osc
...
pb_esc_save_cursor:
    CMPI r2, 55 ; '7' = DEC Save Cursor
    JNZ r15, pb_esc_restore_cursor
    CALL save_cursor
    LDI r20, ANSI_STATE
    LDI r15, ANS_NORMAL
    STORE r20, r15
...
pb_esc_restore_cursor:
    CMPI r2, 56 ; '8' = DEC Restore Cursor
    JNZ r15, pb_esc_index
    CALL restore_cursor
    LDI r20, ANSI_STATE
    LDI r15, ANS_NORMAL
    STORE r20, r15
...
pb_esc_index:
    CMPI r2, 68 ; 'D' = Index (move down, scroll if at bottom)
    JNZ r15, pb_esc_rev_index
    CALL do_newline
    LDI r20, ANSI_STATE
    LDI r15, ANS_NORMAL
    STORE r20, r15
...
pb_esc_rev_index:
    CMPI r2, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r15, pb_esc_ris
...
pb_esc_next_line:
    CMPI r2, 69 ; 'E' = Next Line (CR + Index)
    JNZ r15, pb_esc_other
    LDI r20, CUR_COL
    LDI r15, 0
    STORE r20, r15
    CALL do_newline
...
pb_check_csi:
    CMPI r13, ANS_CSI
    JNZ r15, pb_check_osc

    ; Check if this is a final byte (>= 0x40)
    CMPI r2, 64
    BLT r15, pb_csi_param
...
pb_csi_g_tab:
    ...
    CMPI r2, 103
    JNZ r15, pb_csi_h_mode
    ; TBC: CSI_PARAM determines mode (default 0)
    LDI r20, CSI_PARAM
    LOAD r15, r20
    CMPI r15, 3
    JZ r15, tbc_clear_all
    CMPI r15, 0
    JZ r15, tbc_clear_one
...
pb_csi_h_mode:
    ...
    CMPI r2, 104
    JNZ r15, pb_csi_l_mode
    CMPI r24, 0
    JZ r15, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r15, r20
    CMPI r15, 1049
    JNZ r15, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r15, pb_csi_s
    CMPI r24, 0
    JZ r15, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r15, r20
    CMPI r15, 1049
    JNZ r15, pb_ret        ; only handle 1049
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r2 - 48)
    LDI r20, CSI_PARAM
    LOAD r15, r20
    LDI r14, 10
    MUL r15, r14
...
pb_check_osc:
    CMPI r13, ANS_OSC
    JNZ r15, pb_check_utf8_2

    CMPI r2, 7
    JZ r15, pb_osc_end
    CMPI r2, 27
    JNZ r15, pb_osc_continue
    LDI r20, ANSI_STATE
    LDI r15, ANS_ESC
    STORE r20, r15
    JMP pb_ret
...
pb_check_utf8_2:
    CMPI r13, ANS_UTF8_2
    JNZ r15, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r2, 128
    BLT r15, pb_reset_state
    CMPI r2, 192
    BGE r15, pb_reset_state
...
pb_check_utf8_3a:
    CMPI r13, ANS_UTF8_3A
    JNZ r15, pb_check_utf8_3b
...
pb_check_utf8_3b:
    CMPI r13, ANS_UTF8_3B
    JNZ r15, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r15 as dirty
; Sets bit (31 - row) in DIRTY_ROWS bitmap
mark_row_dirty:
    PUSH r3
    LDI r3, 31
    SUB r3, r15          ; r3 = 31 - row
    LDI r10, 1
    SHL r10, r3          ; r10 = 1 << (31 - row)
    LDI r8, DIRTY_ROWS
    LOAD r13, r8         ; r13 = current bitmap
    OR r13, r10           ; set the bit
    STORE r8, r13
    POP r3
    RET
...
; mark_row_dirty_check -- check if row in r15 is dirty
; Returns r15 = bit value (0=clean, nonzero=dirty)
mark_row_dirty_check:
    ...
    LOAD r13, r8         ; r13 = current bitmap
    MOV r15, r13
    AND r15, r10          ; r15 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r2 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r3, 1
...
sts_div_loop:
    CMPI r15, 32
    BLT r15, sts_div_done
    SUB r15, r10
    ADD r8, r3
    JMP sts_div_loop
sts_div_done:
    ; r8 = word_index, r15 = bit position within word
    PUSH r15              ; save bit position (CMP will clobber r15)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r8
    LOAD r10, r20
    POP r15               ; restore bit position
    ; Set bit: r10 |= (1 << r15)
    LDI r13, 1
    LDI r2, 0
sts_shift_loop:
    CMP r2, r15
    JZ r15, sts_shift_done
    SHL r13, r3
    ADD r2, r3
    JMP sts_shift_loop
sts_shift_done:
...
ctc_shift_loop:
    CMP r2, r15
    JZ r15, ctc_shift_done
    SHL r13, r3
    ADD r2, r3
    JMP ctc_shift_loop
ctc_shift_done:
    ; r13 = mask. Invert: r13 = ~r13 (all bits except the one to clear)
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r15)
    PUSH r15

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r10, r15           ; r10 = col (will become bit_pos after div)
    LDI r8, 0            ; r8 = word_index
    LDI r9, 32           ; r9 = divisor
att_div_loop:
    CMPI r10, 32
    BLT r15, att_check_bit
    SUB r10, r9           ; r10 -= 32
    ADD r8, r3           ; word_index++
    JMP att_div_loop
...
att_check_bit:
    ...
    ADD r20, r8
    LOAD r13, r20
    ; Check if bit r10 is set: shift 1 left by r10, AND
    LDI r2, 1
    LDI r14, 0
att_shift_loop:
    CMP r14, r10
...
att_test_bit:
    AND r13, r2
    JNZ r13, att_found_pop

    ; Not a tab stop -- restore column and try next
    POP r15               ; restore scan column
att_scan_next:
    ADD r15, r3
    JMP att_scan_loop

att_found_pop:
    POP r15               ; restore scan column (r15 = tab stop column)
att_found:
    ; Found tab stop at column r15 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r15
    LDI r20, CUR_ROW
    LOAD r15, r20         ; r15 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r15
    POP r31
    RET
...
po_find_path:
    ...
    ADD r1, r3
    CMPI r1, 3
    BGE r15, po_copy_path
po_next:
    ADD r4, r3
    LDI r9, 80
    CMP r4, r9
    BLT r15, po_find_path
    JMP po_done
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r15 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r15               ; r15 = original count
    LDI r21, 10
    MOD r15, r21          ; r15 = count % 10
...
; Preserves all registers except r15 (CMP clobbers it).
...
apply_sgr:
    ...
    LDI r20, CSI_PARAM
    LOAD r14, r20       ; r14 = SGR code

    ; ── Extended SGR state machine ──
    LDI r20, SGR_EXTENDED
    LOAD r9, r20       ; r9 = extended state

    ; State 0x85: waiting for 256-color FG index
    CMPI r9, 0x85
    JNZ r15, sgr_ext95
    CALL sgr_256_to_rgb  ; r15 = RGB from r14 (color index)
    LDI r20, FG_COLOR
    STORE r20, r15
    LDI r20, SGR_EXTENDED
    LDI r15, 0
...
sgr_n49:
    ...
    CMPI r14, 49
    JNZ r15, sgr_existing
    JMP sgr_ret

sgr_existing:
    ; ── Standard SGR dispatch (16-color) ──
    ; r14 already loaded with SGR code from above
...
; Input:  r14 = color index (0-255)
; Output: r15 = RGB color value
; Clobbers: r15, r3, r13, r2
; Preserves: r14, r9, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r15, 232
    SUB r14, r15        ; r14 = index - 232
    LDI r15, 10
    MUL r14, r15        ; r14 = (index - 232) * 10
    LDI r15, 8
    ADD r14, r15        ; r14 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r14           ; save v
    ...
    MOV r15, r14
    SHL r15, r3        ; r15 = v << 16
    PUSH r15           ; save high bits
    LDI r3, 8
    MOV r15, r14
    SHL r15, r3        ; r15 = v << 8
    OR r15, r14         ; r15 = (v << 8) | v
    POP r3            ; r3 = v << 16
    OR r15, r3         ; r15 = (v << 16) | (v << 8) | v
    POP r14            ; restore r14
    JMP sgr256_ret
...
sgr256_cube:
    ...
    MOV r15, r3
    LDI r14, 16
    SHL r15, r14        ; r15 = R << 16
    MOV r14, r13
    LDI r3, 8
    SHL r14, r3        ; r14 = G << 8
    OR r15, r14         ; r15 = (R << 16) | (G << 8)
    OR r15, r2         ; r15 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    LDI r20, CSI_PARAM
    LOAD r14, r20       ; r14 = mode (0, 1, or 2)

    ; Get current row
    LDI r20, CUR_ROW
    LOAD r9, r20       ; r9 = row
    LDI r5, COLS
    MUL r9, r5         ; r9 = row * COLS
    ...
    MOV r1, r7        ; r1 = end_col (exclusive)
    CMPI r14, 0
    JZ r15, cel_clear
    CMPI r14, 1
    JNZ r15, cel_mode2
...
cel_done:
    ...
    LDI r15, 1
    STORE r20, r15
    ; Mark current row as dirty (row is in r9 / COLS)
    ; Re-derive row from saved value: r9 = row * COLS, row = r9 / COLS
    ; Actually we saved CUR_ROW into r9 early, then multiplied. Re-read CUR_ROW.
    LDI r20, CUR_ROW
    LOAD r15, r20
    CALL mark_row_dirty
...
ced_check0:
    ...
    LDI r1, COLS
    MUL r4, r1
    ADD r4, r7        ; offset = row*COLS + col

    LDI r9, ROWS
    LDI r5, COLS
    MUL r9, r5         ; total cells
ced_loop0:
    CMP r4, r9
    BGE r15, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r14, 1
ccp_row_set:
    SUB r14, r3         ; convert to 0-based
    CMPI r14, 0
    BGE r15, ccp_row_clamp
    LDI r14, 0
ccp_row_clamp:
    LDI r9, ROWS
    SUB r9, r3
...
cil_clear_loop:
    ...
    LDI r5, ROWS
    SUB r5, r3
    BGE r15, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r7, r3
    CMP r15, r7
    BGE r15, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    MOV r4, r5
    SUB r4, r14
    ADD r4, r3        ; r4 = first row to clear (ROWS - count)
    CMP r4, r9
    BGE r15, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r14
    BGE r15, cdl_done2
...
cdc_count:
    ...
    LOAD r9, r20
    LDI r20, CUR_COL
    LOAD r5, r20       ; r5 = cursor col

    ; Base offset = row * COLS
    LDI r7, COLS
    MOV r15, r9
    MUL r15, r7
    MOV r9, r15         ; r9 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r4, r5        ; r4 = dst col (starting at cursor)
cdc_shift:
    ADD r15, r4
    ADD r15, r14         ; r4 + count = src col
    LDI r7, COLS
    CMP r15, r7
    BGE r15, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r4, COLS
    SUB r4, r14        ; r4 = first col to clear
    LDI r7, COLS
    CMP r4, r5
    BGE r15, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r4 < r5 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r4, COLS
    SUB r4, r3        ; r4 = COLS-1 (start from rightmost)
    SUB r4, r14        ; r4 = COLS-1-count (first dst position)
cic_shift:
    CMP r4, r5
    BLT r15, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r15, r20
    JZ r15, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    LDI r11, 0          ; x (use r11 to preserve r3)
    MOV r10, r12         ; y = 10 + row*6
    LDI r8, 256         ; w
    LDI r13, 6           ; h
    LDI r2, 0x0A0A0A    ; bg color
    RECTF r11, r10, r8, r13, r2

...
LDI r12, 0x1000   ; r12 = pointer to filename
LDI r7, 0        ; r7 = mode (read)
OPEN r12, r7       ; r8 = file descriptor (or 0xFFFFFFFF on error)

; ── Save fd, then read up to 200 bytes ──
MOV r3, r8       ; r3 = saved fd
LDI r1, 0x2000   ; r1 = read buffer address
LDI r13, 200      ; r13 = max bytes to read
READ r3, r1, r13  ; r8 = bytes actually read
...
ADD r8, r14       ; r8 = buffer start + bytes_read = end address
LDI r15, 0
STORE r8, r15     ; write null terminator

; ── Display file contents using TEXT opcode ──
LDI r12, 2        ; x position
LDI r7, 10       ; y position
LDI r1, 0x2000   ; pointer to string buffer
TEXT r12, r7, r1

...
mark_row_dirty:
    PUSH r7
    LDI r7, 31
    SUB r7, r13          ; r7 = 31 - row
    LDI r1, 1
    SHL r1, r7          ; r1 = 1 << (31 - row)
    LDI r10, DIRTY_ROWS
    LOAD r12, r10         ; r12 = current bitmap
    OR r12, r1           ; set the bit
    STORE r10, r12
    POP r7
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r10 = word_index, r13 = bit position within word
...
att_scan_loop:
    ...
    MOV r1, r13           ; r1 = col (will become bit_pos after div)
    LDI r10, 0            ; r10 = word_index
    LDI r4, 32           ; r4 = divisor
att_div_loop:
    CMPI r1, 32
    BLT r13, att_check_bit
    SUB r1, r4           ; r1 -= 32
    ADD r10, r7           ; word_index++
    JMP att_div_loop
...
apply_sgr:
    ...
    CMPI r4, 0x85
    JNZ r13, sgr_ext95
    CALL sgr_256_to_rgb  ; r13 = RGB from r6 (color index)
    LDI r20, FG_COLOR
    STORE r20, r13
    LDI r20, SGR_EXTENDED
    LDI r13, 0
...
; Clobbers: r13, r7, r12, r8
; Preserves: r6, r4, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r13, 232
    SUB r6, r13        ; r6 = index - 232
    LDI r13, 10
    MUL r6, r13        ; r6 = (index - 232) * 10
    LDI r13, 8
    ADD r6, r13        ; r6 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r6           ; save v
...
sgr256_cube:
    ...
    LDI r7, 8
    SHL r6, r7        ; r6 = G << 8
    OR r13, r6         ; r13 = (R << 16) | (G << 8)
    OR r13, r8         ; r13 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    ; mode 1: 0..col
    ; mode 2: 0..COLS-1
    LDI r0, 0         ; r0 = start_col
    MOV r14, r5        ; r14 = end_col (exclusive)
    CMPI r6, 0
    JZ r13, cel_clear
    CMPI r6, 1
    JNZ r13, cel_mode2
...
cil_count:
    ...
    LDI r0, ROWS
    SUB r0, r7        ; r0 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r0, r4
    BLT r13, cil_clear
...
cdl_clear:
    ...
    MOV r0, r9
    SUB r0, r6
    ADD r0, r7        ; r0 = first row to clear (ROWS - count)
    CMP r0, r4
...
cdc_count:
    ...
    MOV r13, r4
    MUL r13, r5
    MOV r4, r13         ; r4 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r0, r9        ; r0 = dst col (starting at cursor)
cdc_shift:
    ADD r13, r0
    ADD r13, r6         ; r0 + count = src col
    LDI r5, COLS
    CMP r13, r5
...
cdc_clear:
    ...
    LDI r0, COLS
    SUB r0, r6        ; r0 = first col to clear
    LDI r5, COLS
    CMP r0, r9
    BGE r13, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r0 < r9 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r0, COLS
    SUB r0, r7        ; r0 = COLS-1 (start from rightmost)
    SUB r0, r6        ; r0 = COLS-1-count (first dst position)
cic_shift:
    CMP r0, r9
    BLT r13, cic_clear

    ; dst = BUF + base + r0 + count
    LDI r21, BUF
    MOV r13, r4
    ADD r21, r13
    ADD r21, r0
...
render_row:
    ...
    LDI r7, 1           ; restore r7 (clobbered by mark_row_dirty_check)
    ...
    LDI r11, 0          ; x (use r11 to preserve r7)
    MOV r1, r15         ; y = 10 + row*6
    LDI r10, 256         ; w
    LDI r12, 6           ; h

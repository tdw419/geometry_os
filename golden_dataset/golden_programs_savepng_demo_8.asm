; DESCRIPTION: This GeOS assembly code draws a colorful pattern on the screen using rectangles of different colors and then exports the current screen as a PNG file named "demo_savepng.png" to the virtual filesystem. The code stores the filename in memory, uses drawing commands to render the pattern, writes a text label, and finally calls the SAVEPNG opcode to perform the export, returning the number of bytes written or an error code.

; ── Phase 212 Demo: SAVEPNG Opcode ──
; Draws a colorful pattern on screen, then exports as PNG via SAVEPNG (0xAF)
;
; SAVEPNG path_addr_reg  -- saves current screen as PNG to VFS file
; Returns bytes written in r8, or 0xFFFFFFFF on error

; Store filename "demo_savepng.png" at 0x2000
LDI r1, 100
LDI r13, 0x2000
STORE r13, r1
LDI r1, 101
LDI r13, 0x2001
STORE r13, r1
LDI r1, 109
LDI r13, 0x2002
STORE r13, r1
LDI r1, 111
LDI r13, 0x2003
STORE r13, r1
LDI r1, 95
LDI r13, 0x2004
STORE r13, r1
LDI r1, 115
LDI r13, 0x2005
STORE r13, r1
LDI r1, 97
LDI r13, 0x2006
STORE r13, r1
LDI r1, 118
LDI r13, 0x2007
STORE r13, r1
LDI r1, 101
LDI r13, 0x2008
STORE r13, r1
LDI r1, 112
LDI r13, 0x2009
STORE r13, r1
LDI r1, 110
LDI r13, 0x200A
STORE r13, r1
LDI r1, 103
LDI r13, 0x200B
STORE r13, r1
LDI r1, 46
LDI r13, 0x200C
STORE r13, r1
LDI r1, 112
LDI r13, 0x200D
STORE r13, r1
LDI r1, 110
LDI r13, 0x200E
STORE r13, r1
LDI r1, 103
LDI r13, 0x200F
STORE r13, r1
LDI r1, 0
LDI r13, 0x2010
STORE r13, r1    ; null terminator

; Draw colored rectangles
LDI r1, 0x00FF0000   ; red
LDI r3, 10
LDI r11, 10
LDI r12, 60
LDI r14, 40
RECT r3, r11, r12, r14, r1

LDI r1, 0x0000FF00   ; green
LDI r3, 80
LDI r11, 10
LDI r12, 60
LDI r14, 40
RECT r3, r11, r12, r14, r1

LDI r1, 0x000000FF   ; blue
LDI r3, 10
LDI r11, 70
LDI r12, 60
LDI r14, 40
RECT r3, r11, r12, r14, r1

LDI r1, 0x00FFFF00   ; yellow
LDI r3, 80
LDI r11, 70
LDI r12, 60
LDI r14, 40
RECT r3, r11, r12, r14, r1

; Draw text label
LDI r1, 50
LDI r3, 125
LDI r11, 0x2000      ; filename address
LDI r12, 0xFFFFFF
LDI r14, 0
VWTXT r1, r3, r11, r12, r14

; Export screen as PNG
LDI r1, 0x2000
SAVEPNG r1

; r8 = bytes written (or 0xFFFFFFFF on error)
HALT

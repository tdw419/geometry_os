...
LDI r6, 0x7FFE        ; frame_counter (TICKS)
LDI r13, 0             ; player_x (will init)
LDI r7, 0             ; player_y (will init)
...
; WINSYS op=0: create window (r4=x, r2=y, r1=w, r0=h, r3=title_addr)
LDI r17, 0x94
LDI r18, 1             ; op_reg = r4, set r4=0 (op=0=create)
LDI r4, 0              ; op = 0 (create)
LDI r2, 16             ; x = 16
LDI r1, 16             ; y = 16
LDI r0, 200            ; w = 200
LDI r3, 150            ; h = 150
LDI r14, 0x2100         ; title_addr = "Linux Terminal"
STORE r14, r14           ; ensure title exists
...
do_winsys_create:
  ...
  LDI r0, 200           ; w
  LDI r3, 150           ; h
  LDI r14, 0x2100        ; title addr

  ; Use WINSYS instruction directly
  WINSYS r4
  ; r12 = window_id (returned by create)
  LDI r10, 0x7810
  STORE r10, r12         ; save window_id
  RET
...
LDI r8, 0x2000        ; config string addr
HYPERVISOR r8, r10    ; boot hypervisor, target window
; r12 = 0 on success

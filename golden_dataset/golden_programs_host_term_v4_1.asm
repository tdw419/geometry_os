...
po_copy_path:
    ; r9 points to the 3rd '/' -- copy from there into STATUS_CWD
    LDI r2, 0     ; cwd offset
po_cp_loop:
    LDI r20, OSC_BUF
    ADD r20, r9
...
copy_col_after_skip:
    ...
    LDI r14, 0         ; no background (already cleared)
    SMALLTEXT r6, r2, r13, r10, r14
...
draw_cursor:
    ...
    LOAD r0, r20
    LDI r15, 3
    MUL r0, r15          ; x = col * 3 (3px per char)
    LDI r20, CUR_ROW
    LOAD r12, r20
    LDI r15, 6
    MUL r12, r15          ; row * 6
...
; Uses: r0-r15, r9-r2 (saved/restored around CALL)
; =========================================
check_resize:
    PUSH r31
    ...
    ; WINSYS op=6 (WINFO): r0=win_id, r6=winfo_buf_addr
    LDI r20, WIN_ID
    LOAD r0, r20
    LDI r6, WINFO_BUF
    ; WINSYS op_reg needs to hold the operation number (6)
    LDI r9, 6
    ; Call WINSYS with r9 containing op=6
    ; WINSYS reads op from register at op_reg position
    ; Set r0=win_id, r6=WINFO_BUF, r9=6 for WINSYS r9
    WINSYS r9

...
; _check_z: if r10==0 → pass (for non-CMP checks)
_check_z:
    JZ r10, _pass
    JMP _fail

; _check_nz: if r10!=0 → pass
_check_nz:
    JZ r10, _fail
    JMP _pass
...
; T16: malloc returns address >= HEAP_START
    ...
    LDI r11, 0x1F8F
    LDI r6, 0xC000
    ; Check r10 >= 0xC000: CMP(r10, 0xC000) should NOT be -1
    CMP r10, r6
    LDI r6, 0xFFFFFFFF
    CMP r10, r6
    ; If r10 == 0xFFFFFFFF (CMP was -1, addr < heap_start), fail
    JZ r10, _t16_fail
    LDI r10, 1
    LDI r11, 0x1F8F
    STORE r11, r10

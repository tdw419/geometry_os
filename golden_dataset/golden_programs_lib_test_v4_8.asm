...
; _check_z: if r5==0 → pass (for non-CMP checks)
_check_z:
    JZ r5, _pass
    JMP _fail

; _check_nz: if r5!=0 → pass
_check_nz:
    JZ r5, _fail
    JMP _pass
...
; T16: malloc returns address >= HEAP_START
    ...
    LDI r4, 0x1F8F
    LDI r11, 0xC000
    ; Check r5 >= 0xC000: CMP(r5, 0xC000) should NOT be -1
    CMP r5, r11
    LDI r11, 0xFFFFFFFF
    CMP r5, r11
    ; If r5 == 0xFFFFFFFF (CMP was -1, addr < heap_start), fail
    JZ r5, _t16_fail
    LDI r5, 1
    LDI r4, 0x1F8F
    STORE r4, r5

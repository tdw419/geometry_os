...
; ═══════════════════════════════════════════════════════════════
    ...
    LDI r11, 0x1201
    LDI r12, 20
    STORE r11, r12          ; prompt y starts at line 20
    LDI r11, 0x1207
    LDI r12, 0
    STORE r11, r12          ; pipe mode = 0
    LDI r11, 0x1205
    STORE r11, r12          ; redirect mode = 0
...
main_loop:
    ...
    LOAD r15, r11            ; r15 = y position
    LDI r2, 2              ; x = 2
    LDI r7, prompt_str
    TEXT r2, r15, r7

    ; Read a line of input
    LDI r12, 0x0200         ; buf addr
    LDI r15, 200            ; max len
    LDI r2, 0x1200         ; pos addr
    LDI r11, 0x1200
    LDI r7, 0
    STORE r11, r7           ; reset position

readln_loop:
    READLN r12, r15, r2
    CMP r12, r7             ; r12 == 0 means still reading
    JZ r12, readln_loop

    ; r12 > 0 means line complete (Enter pressed)
    ; Check if line is empty
    JZ r12, main_loop       ; empty line, loop
...
skip_spaces:
    LOAD r12, r9
    JZ r12, parse_done      ; empty string
    LDI r15, 32
    CMP r12, r15
    JZ r12, skip_spaces     ; skip space

    ; Copy command name until space or null
    LDI r0, 0x0400       ; dest for command name
copy_cmd:
    LOAD r12, r9
    JZ r12, parse_cmd_done
    LDI r15, 32
    CMP r12, r15
    JZ r12, parse_cmd_done
    ...
    LDI r15, 124
    CMP r12, r15
    JZ r12, parse_pipe_found
    ; Check for '>' (62)
    LDI r15, 62
    CMP r12, r15
    JZ r12, parse_redirect_out
    ; Check for '<' (60)
    LDI r15, 60
    CMP r12, r15
    JZ r12, parse_redirect_in
    STORE r0, r12
    ADD r9, r7            ; r7 is still 0 from earlier... let's use r15
...
parse_cmd_done:
    ...
    LOAD r12, r9
    JZ r12, parse_arg_done  ; no argument
skip_arg_spaces:
    LOAD r12, r9
    LDI r15, 32
    CMP r12, r15
    JZ r12, skip_arg_spaces_inc
    JMP copy_arg_start
skip_arg_spaces_inc:
    LDI r15, 1
    ADD r9, r15
...
copy_arg:
    LOAD r12, r9
    JZ r12, parse_arg_done
    ; Check for pipe
    LDI r15, 124
    CMP r12, r15
    JZ r12, parse_pipe_found_arg
    ; Check for redirect
    LDI r15, 62
    CMP r12, r15
    JZ r12, parse_redirect_out_arg
    LDI r15, 60
    CMP r12, r15
    JZ r12, parse_redirect_in_arg
    STORE r0, r12
    LDI r15, 1
    ADD r9, r15
    ADD r0, r15
...
; Each sets r12=1 if match, r12=0 if not
...
do_ls:
    ...
    LDI r15, 0x0800
    LS r15                  ; list files into buffer at 0x0800
    ; r12 = number of files
...
do_cat:
    ...
    LDI r15, 0x0600       ; filename addr
    LDI r2, 0            ; mode = read
    OPEN r15, r2           ; r12 = fd
    ; Check for error
    LDI r15, 0xFFFFFFFF
    CMP r12, r15
    JZ r12, exec_done      ; error, skip

    MOV r5, r12            ; save fd in r5
    LDI r7, 0x1000        ; read buffer
    LDI r8, 200           ; max bytes
    READ r5, r7, r8       ; r12 = bytes read
    ; Null terminate
    LDI r13, 200
    CMP r12, r13
    BLT r12, cat_term_ok
...
rm_copy_done:
    LDI r12, 0
    STORE r0, r12      ; null terminate
    ; UNLINK r15 where r15 = 0x1600
    LDI r15, 0x1600
    UNLINK r15
...
cp_find_space:
    LOAD r12, r10
    JZ r12, cp_usage    ; no space found -- need 2 args
    LDI r15, 32         ; space
    CMP r12, r15
    JZ r12, cp_found_space
    LDI r12, 1
    ADD r10, r12
    JMP cp_find_space
cp_found_space:
...
cp_src_done:
    STORE r0, r12
cp_do_copy:
    ; FCOPY r15, r2 where r15=0x1600 (src), r2=0x1800 (dst)
    LDI r15, 0x1600
    LDI r2, 0x1800
    FCOPY r15, r2
    ; Check result
...
hypervisor_copy_done:
    ...
    JZ r12, hypervisor_ok
    LDI r15, 0xFFFFFFFF
    CMP r12, r15
    JZ r12, hypervisor_err
...
do_edit:
    ...
    LDI r15, 0xFFFFFFFF
    CMP r12, r15
    JZ r12, edit_err      ; file not found

    MOV r5, r12            ; save fd in r5

    ; Read file content into buffer at 0x2000
    LDI r7, 0x2000        ; read buffer (8KB, up to row 127 * 32 cols)
    LDI r8, 4096          ; max words to read
    READ r5, r7, r8       ; r12 = bytes read
    ...
    LDI r13, 0x2000
    ADD r13, r12            ; r13 = buffer + bytes_read
    LDI r4, 0
    STORE r13, r4          ; null terminator
    ...
    LDI r1, 4            ; y position (start near top)
    LDI r14, 0            ; line buffer index

edit_line_loop:
    LOAD r12, r3
    JZ r12, edit_done      ; null = end of file
    ...
    LDI r15, 1
    ADD r3, r15           ; advance source
    ADD r14, r15           ; advance line index

    ; Check if line is 32 chars (screen width at 8px font)
    LDI r15, 32
    CMP r14, r15
    JZ r12, edit_line_full
    JMP edit_line_loop
...
edit_newline:
    ...
    LDI r2, 4             ; x = 4
    LDI r7, 0x2100        ; line buffer
    TEXT r2, r1, r7      ; render text at (4, y)

    ; Advance to next line
    LDI r15, 1
    ADD r3, r15           ; skip the newline char
    ADD r1, r15           ; next y position
    LDI r14, 0            ; reset line buffer index
    JMP edit_line_loop
...
do_save:
    ...
    LDI r15, 0xFFFFFFFF
    CMP r12, r15
    JZ r12, save_err      ; could not create file
    ...
    LDI r3, 0x2000
    LDI r1, 0            ; length counter
save_scan:
    LOAD r12, r3
    JZ r12, save_scan_done
    LDI r15, 1
    ADD r3, r15
    ADD r1, r15
    JMP save_scan
...
save_scan_done:
    ...
    LDI r15, 0x2000        ; buffer addr
    MOV r2, r15             ; fd_reg = r5 already
    ; WRITE fd_reg, buf_reg, len_reg
    MOV r15, r5             ; r15 = fd
    LDI r2, 0x2000         ; buf addr
    MOV r7, r1            ; length
    WRITE r15, r2, r7       ; r12 = bytes written
...
do_grep:
    ...
    LDI r1, 0x2200      ; pattern buffer
    LDI r14, 0           ; pattern length

grep_parse_pat:
    LOAD r12, r3
    JZ r12, grep_no_file   ; no space found, error
    LDI r15, 32            ; space
    CMP r12, r15
    JZ r12, grep_copy_file
    STORE r1, r12
    LDI r15, 1
    ADD r3, r15
    ADD r1, r15
...
grep_have_file:
    LDI r12, 0
    STORE r1, r12         ; null terminate filename
    ...
    LDI r10, 0            ; match count

grep_line_loop:
    ; Save file pointer to scratch
    MOV r6, r3           ; r6 = saved file ptr

    LOAD r12, r3
    JZ r12, grep_done       ; end of file

    ; Copy line to buffer at 0x2400, track length
    LDI r1, 0x2400
    LDI r14, 0            ; line length

grep_copy_line:
    LOAD r12, r3
    JZ r12, grep_check_line     ; end of file, check this line
    LDI r15, 10                 ; newline
    CMP r12, r15
    JZ r12, grep_advance_nl
    STORE r1, r12
    LDI r15, 1
    ADD r3, r15
    ADD r1, r15
    ...
    LDI r15, 64
    CMP r14, r15
    JZ r12, grep_check_line
    JMP grep_copy_line

grep_advance_nl:
    LDI r15, 1
    ADD r3, r15             ; skip newline
...
grep_have_pat_len:
    ; r0 = pattern length, r14 = line length
    ; Skip if line shorter than pattern
    CMP r14, r0
    BLT r12, grep_next_line

    ; Simple substring search: check each starting position
    ; r14 - r0 = max starting position
    SUB r14, r0           ; r14 = max_start (line_len - pat_len)

grep_search_pos:
    ; r1 = current position in line (0x2400 + offset)
    ; Use offset tracking instead of absolute address comparison
    LDI r11, 0              ; search offset

grep_search_pos2:
    CMP r11, r14            ; offset vs max_start
    BGE r12, grep_next_line

    ; Compare pattern with line at offset r11
    LDI r9, 0x2400
    ADD r9, r11            ; r9 = line_base + offset
    LDI r10, 0x2200        ; pattern start
    LDI r15, 0              ; chars matched

grep_cmp_loop:
    LOAD r12, r9
    LOAD r2, r10
    CMP r12, r2
    JNZ r12, grep_no_match_here
    ; Match so far
    LDI r7, 1
    ADD r9, r7
    ADD r10, r7
    ADD r15, r7             ; count matched chars
    CMP r15, r0            ; matched vs pattern_len
    BLT r12, grep_cmp_loop
    JMP grep_found_match

grep_no_match_here:
    LDI r7, 1
    ADD r11, r7             ; advance offset
...
head_parse_n:
    ...
    LOAD r12, r3
    JZ r12, head_name_done  ; no number, use default
    ...
    LDI r15, 1
    CMP r12, r15
    BLT r12, head_name_done
    LDI r15, 9
    CMP r12, r15
    BGE r12, head_name_done
    MOV r0, r12           ; use parsed N
...
head_name_done:
    ...
    LDI r3, 0x2000       ; file pointer
    LDI r1, 0x2400       ; line buffer
    LDI r10, 0            ; line count
    LDI r14, 0            ; chars in line

head_read_line:
    LOAD r12, r3
    JZ r12, head_done      ; end of file

    LDI r15, 10
    CMP r12, r15
    JZ r12, head_print_line
...
tail_parse_n:
    ...
    LDI r15, 1
    CMP r12, r15
    BLT r12, tail_name_done
    LDI r15, 9
    CMP r12, r15
    BGE r12, tail_name_done
    MOV r0, r12
...
tail_name_done:
    ...
    ; Count total lines
    LDI r3, 0x2000
    LDI r10, 1            ; line count (at least 1)

tail_count_lines:
    LOAD r12, r3
    JZ r12, tail_count_done
    LDI r15, 10
    CMP r12, r15
    JNZ r12, tail_count_next
    LDI r15, 1
    ADD r10, r15           ; increment line count
tail_count_next:
    LDI r15, 1
    ADD r3, r15
...
tail_count_done:
    ...
    MOV r1, r10           ; total_lines
    SUB r1, r0           ; total - N
    LDI r15, 1
    CMP r1, r15
    BGE r12, tail_skip_ok
    MOV r1, r15            ; at least line 1

tail_skip_ok:
    ; Skip to start line
    LDI r3, 0x2000
    LDI r10, 1             ; current line

tail_skip_loop:
    CMP r10, r1
    BGE r12, tail_display   ; reached start line

tail_skip_next:
    LOAD r12, r3
    JZ r12, tail_done       ; end of file
    LDI r15, 10
    CMP r12, r15
    JNZ r12, tail_skip_adv
    LDI r15, 1
    ADD r10, r15            ; next line on newline
tail_skip_adv:
    LDI r15, 1
    ADD r3, r15
    JMP tail_skip_loop
...
wc_utoa:
    ; Convert r3 to decimal, write to buffer at r1
    ; Modifies r3, does not change r1 (caller manages it)
    PUSH r9
    ; Handle 0 case
...
wc_do_digits:
    ...
    LDI r10, 0x2500       ; temp digit buffer
    LDI r0, 0            ; digit index
    MOV r3, r0          ; restore original? No, r3 was modified.
...
wc_utoa_done:
    MOV r1, r14          ; update output pointer
    POP r14
    POP r10
    POP r0
    POP r9
...
exec_wait:
    ...
    LOAD r15, r11
    WAITPID r15
    JZ r12, exec_wait     ; still running

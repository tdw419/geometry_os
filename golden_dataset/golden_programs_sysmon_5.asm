...
main_loop:
    ...
    LDI r8, 0           ; mode = read
    FSOPEN r13, r8
    ; r4 = handle or 0xFFFFFFFF on error
    LDI r5, 0xFFFFFFFF
    CMP r4, r5
    JZ r4, meminfo_error
...
parse_num_mt:
    ; Parse decimal number at r20 into r2
    LDI r2, 0           ; accumulated number
parse_digit_mt:
    PUSH r20
    LOAD r5, r20        ; char
    LDI r16, 48          ; '0'
    CMP r5, r16
    BLT r4, parse_done_mt
    LDI r16, 57          ; '9'
    CMP r5, r16
    BGE r4, parse_next_chk_mt
    JMP parse_done_mt
parse_next_chk_mt:
    ; digit = char - '0'
    LDI r16, 48
...
scan_done_mt:
    ...
    CMP r0, r4          ; r4 is 0 from CMP
    LDI r5, 0
    CMP r0, r5
    JZ r4, show_meminfo_err

    ; r2 has MemTotal in kB. Display as MB: divide by 1024
    LDI r5, 1024
    DIV r2, r5
    ...
    ; Fill bar proportional (assume r2 is MB, max ~16GB=16384MB)
    ; bar_width = r2 * 112 / 16384
    LDI r5, 112
    MUL r2, r5
    LDI r5, 16384
    DIV r2, r5
...
; Returns null-terminated string. r4 = start address.
; =========================================
int_to_str:
    PUSH r31
    PUSH r20
...
; Converts r2 (u32) to hex string "0x" prefix at r20
; Returns null-terminated. r4 = start address.
; =========================================
hex_to_str:
    PUSH r31
    PUSH r20

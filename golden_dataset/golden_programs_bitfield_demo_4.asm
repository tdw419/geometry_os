...
LDI r2, 0xAA        ; R channel
LDI r13, 8           ; shift amount
SHL r2, r13          ; r2 = 0x0000AA00 (R at bits 8-15)
LDI r11, 0xBB        ; G channel
OR r2, r11           ; r2 = 0x0000AABB (R+G)
LDI r13, 16
SHL r2, r13          ; r2 = 0xAABB0000 (R+G shifted up)
LDI r11, 0xCC        ; B channel
OR r2, r11           ; r2 = 0xAABBCC (packed: R=bits16-23, G=bits8-15, B=bits0-7)

; Now extract each channel using BFE
; BFE r9, r2, r6, r0 -- extract 8 bits at lsb=16 from r2 -> R
LDI r6, 8           ; width = 8
LDI r0, 16          ; lsb = 16
BFE r9, r2, r6, r0  ; r9 = 0xAA (red channel)

; BFE r5, r2, r6, r15 -- extract 8 bits at lsb=8 from r2 -> G
LDI r15, 8           ; lsb = 8
BFE r5, r2, r6, r15  ; r5 = 0xBB (green channel)

; BFE r14, r2, r7, r4 -- extract 8 bits at lsb=0 from r2 -> B
LDI r7, 8          ; width = 8
LDI r4, 0          ; lsb = 0
BFE r14, r2, r7, r4 ; r14 = 0xCC (blue channel)

; Now reassemble as BGR: B at bits 16-23, G at bits 8-15, R at bits 0-7
LDI r10, 0          ; start with 0

; Insert B (r14) at bits 16-23
LDI r1, 8          ; width = 8
LDI r3, 16         ; lsb = 16
MOV r12, r14         ; src = blue
BFI r10, r12, r1, r3 ; r10 = 0x00CC0000

; Insert G (r5) at bits 8-15
LDI r3, 8          ; lsb = 8
MOV r12, r5         ; src = green
BFI r10, r12, r1, r3 ; r10 = 0x00CCBB00

; Insert R (r9) at bits 0-7
LDI r3, 0          ; lsb = 0
MOV r12, r9         ; src = red
BFI r10, r12, r1, r3 ; r10 = 0x00CCBBAA (BGR swapped!)
...
LDI r22, 32
LDI r23, 32
RECTF r20, r21, r22, r23, r2  ; original

; Swapped color (CCBBAA) as a 32x32 rect at (60, 10)
LDI r20, 60
LDI r21, 10
RECTF r20, r21, r22, r23, r10 ; swapped
...
LDI r6, 1
LDI r0, 0
BFE r17, r16, r6, r0  ; r17 = bit 0 of 0xAA = 0

; Extract bit 1 (should be 1)
LDI r0, 1
BFE r18, r16, r6, r0  ; r18 = bit 1 of 0xAA = 1

; Extract bit 7 (should be 1)
LDI r0, 7
BFE r19, r16, r6, r0  ; r19 = bit 7 of 0xAA = 1

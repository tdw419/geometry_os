...
pb_csi_h_mode:
    ...
    CMPI r6, 104
    JNZ r7, pb_csi_l_mode
    CMPI r24, 0
    JZ r7, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r7, r20
    CMPI r7, 1049
    JNZ r7, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r7, pb_csi_s
    CMPI r24, 0
    JZ r7, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r7, r20
    CMPI r7, 1049
    JNZ r7, pb_ret        ; only handle 1049
...
pb_check_osc:
    CMPI r4, ANS_OSC
    JNZ r7, pb_check_utf8_2

    CMPI r6, 7
    JZ r7, pb_osc_end
    CMPI r6, 27
    JNZ r7, pb_osc_continue
    LDI r20, ANSI_STATE
    LDI r7, ANS_ESC
    STORE r20, r7
    JMP pb_ret
...
pb_check_utf8_2:
    CMPI r4, ANS_UTF8_2
    JNZ r7, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r6, 128
    BLT r7, pb_reset_state
    CMPI r6, 192
    BGE r7, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r7 as dirty
; Sets bit (31 - row) in DIRTY_ROWS bitmap
mark_row_dirty:
    PUSH r12
    LDI r12, 31
    SUB r12, r7          ; r12 = 31 - row
    LDI r1, 1
    SHL r1, r12          ; r1 = 1 << (31 - row)
    LDI r15, DIRTY_ROWS
    LOAD r4, r15         ; r4 = current bitmap
    OR r4, r1           ; set the bit
    STORE r15, r4
    POP r12
    RET
...
; mark_row_dirty_check -- check if row in r7 is dirty
; Returns r7 = bit value (0=clean, nonzero=dirty)
...
map_codepoint:
    ...
    CMPI r6, 9472
    BLT r7, mc_check_arrows
    CMPI r6, 9600
    BGE r7, mc_check_blocks
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r15 = word_index, r7 = bit position within word
    PUSH r7              ; save bit position (CMP will clobber r7)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r15
    LOAD r1, r20
    POP r7               ; restore bit position
    ; Set bit: r1 |= (1 << r7)
    LDI r4, 1
    LDI r6, 0
sts_shift_loop:
    CMP r6, r7
    JZ r7, sts_shift_done
    SHL r4, r12
    ADD r6, r12
    JMP sts_shift_loop
sts_shift_done:
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r7)
    PUSH r7

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r1, r7           ; r1 = col (will become bit_pos after div)
    LDI r15, 0            ; r15 = word_index
    LDI r14, 32           ; r14 = divisor
att_div_loop:
    CMPI r1, 32
    BLT r7, att_check_bit
    SUB r1, r14           ; r1 -= 32
    ADD r15, r12           ; word_index++
    JMP att_div_loop
...
att_test_bit:
    ...
    POP r7               ; restore scan column
att_scan_next:
    ADD r7, r12
    JMP att_scan_loop

att_found_pop:
    POP r7               ; restore scan column (r7 = tab stop column)
att_found:
    ; Found tab stop at column r7 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r7
    LDI r20, CUR_ROW
    LOAD r7, r20         ; r7 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r7
    POP r31
    RET
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r7 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r7               ; r7 = original count
    LDI r21, 10
    MOD r7, r21          ; r7 = count % 10
...
; Preserves all registers except r7 (CMP clobbers it).
...
apply_sgr:
    ...
    CMPI r14, 0x85
    JNZ r7, sgr_ext95
    CALL sgr_256_to_rgb  ; r7 = RGB from r9 (color index)
    LDI r20, FG_COLOR
    STORE r20, r7
    LDI r20, SGR_EXTENDED
    LDI r7, 0
...
; Output: r7 = RGB color value
; Clobbers: r7, r12, r4, r6
; Preserves: r9, r14, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r7, 232
    SUB r9, r7        ; r9 = index - 232
    LDI r7, 10
    MUL r9, r7        ; r9 = (index - 232) * 10
    LDI r7, 8
    ADD r9, r7        ; r9 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r9           ; save v
    ...
    MOV r7, r9
    SHL r7, r12        ; r7 = v << 16
    PUSH r7           ; save high bits
    LDI r12, 8
    MOV r7, r9
    SHL r7, r12        ; r7 = v << 8
    OR r7, r9         ; r7 = (v << 8) | v
    POP r12            ; r12 = v << 16
    OR r7, r12         ; r7 = (v << 16) | (v << 8) | v
    POP r9            ; restore r9
    JMP sgr256_ret
...
sgr256_cube:
    ...
    LDI r12, 8
    SHL r9, r12        ; r9 = G << 8
    OR r7, r9         ; r7 = (R << 16) | (G << 8)
    OR r7, r6         ; r7 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r13, r8        ; r13 = end_col (exclusive)
    CMPI r9, 0
    JZ r7, cel_clear
    CMPI r9, 1
    JNZ r7, cel_mode2
...
ced_check0:
    ...
    LDI r13, COLS
    MUL r10, r13
    ADD r10, r8        ; offset = row*COLS + col

    LDI r14, ROWS
    LDI r2, COLS
    MUL r14, r2         ; total cells
ced_loop0:
    CMP r10, r14
    BGE r7, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r9, 1
ccp_row_set:
    SUB r9, r12         ; convert to 0-based
    CMPI r9, 0
    BGE r7, ccp_row_clamp
    LDI r9, 0
ccp_row_clamp:
    LDI r14, ROWS
    SUB r14, r12
...
cil_count:
    ...
    LDI r10, ROWS
    SUB r10, r12        ; r10 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r10, r14
    BLT r7, cil_clear
...
cil_clear_loop:
    ...
    LDI r2, ROWS
    SUB r2, r12
    BGE r7, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r8, r12
    CMP r7, r8
    BGE r7, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    LDI r2, ROWS
    SUB r2, r12         ; r2 = ROWS-1
    MOV r10, r2
    SUB r10, r9
    ADD r10, r12        ; r10 = first row to clear (ROWS - count)
    CMP r10, r14
    BGE r7, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r9
    BGE r7, cdl_done2
...
cdc_count:
    ...
    LOAD r14, r20
    LDI r20, CUR_COL
    LOAD r2, r20       ; r2 = cursor col

    ; Base offset = row * COLS
    LDI r8, COLS
    MOV r7, r14
    MUL r7, r8
    MOV r14, r7         ; r14 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r10, r2        ; r10 = dst col (starting at cursor)
cdc_shift:
    ADD r7, r10
    ADD r7, r9         ; r10 + count = src col
    LDI r8, COLS
    CMP r7, r8
    BGE r7, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r10, COLS
    SUB r10, r9        ; r10 = first col to clear
    LDI r8, COLS
    CMP r10, r2
    BGE r7, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r10 < r2 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r10, COLS
    SUB r10, r12        ; r10 = COLS-1 (start from rightmost)
    SUB r10, r9        ; r10 = COLS-1-count (first dst position)
cic_shift:
    CMP r10, r2
    BLT r7, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r7, r20
    JZ r7, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    LDI r3, 0          ; x (use r3 to preserve r12)
    MOV r1, r0         ; y = 10 + row*6
    LDI r15, 256         ; w
    LDI r4, 6           ; h

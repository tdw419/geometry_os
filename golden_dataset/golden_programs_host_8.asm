...
STORE r20, r5
ADD r20, r11
STORE r20, r5       ; word2 (cols 64-79): only bits 0,8 used (cols 64,72)
...
pb_normal_byte:
    ...
    BLT r4, pb_ascii
    CMPI r6, 224
    BGE r4, pb_utf8_3_start

    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r6 & 0x1F) << 6
    LDI r14, 31
    AND r6, r14
    LDI r14, 6
    SHL r6, r14
...
pb_utf8_3_start:
    CMPI r6, 240
    BGE r4, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r6 & 0x0F) << 12
    LDI r14, 15
    AND r6, r14
    LDI r14, 12
    SHL r6, r14
...
pb_esc_save_cursor:
    CMPI r6, 55 ; '7' = DEC Save Cursor
    JNZ r4, pb_esc_restore_cursor
    CALL save_cursor
    LDI r20, ANSI_STATE
    LDI r4, ANS_NORMAL
    STORE r20, r4
...
pb_esc_restore_cursor:
    CMPI r6, 56 ; '8' = DEC Restore Cursor
    JNZ r4, pb_esc_index
    CALL restore_cursor
    LDI r20, ANSI_STATE
    LDI r4, ANS_NORMAL
...
pb_esc_index:
    CMPI r6, 68 ; 'D' = Index (move down, scroll if at bottom)
    JNZ r4, pb_esc_rev_index
    CALL do_newline
    LDI r20, ANSI_STATE
    LDI r4, ANS_NORMAL
    STORE r20, r4
...
pb_esc_rev_index:
    CMPI r6, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r4, pb_esc_ris
...
pb_esc_next_line:
    CMPI r6, 69 ; 'E' = Next Line (CR + Index)
    JNZ r4, pb_esc_other
    LDI r20, CUR_COL
    LDI r4, 0
    STORE r20, r4
...
pb_csi_h_mode:
    ...
    CMPI r6, 104
    JNZ r4, pb_csi_l_mode
    CMPI r24, 0
    JZ r4, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r4, r20
    CMPI r4, 1049
    JNZ r4, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r4, pb_csi_s
    CMPI r24, 0
    JZ r4, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r4, r20
    CMPI r4, 1049
    JNZ r4, pb_ret        ; only handle 1049
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r6 - 48)
    LDI r20, CSI_PARAM
    LOAD r4, r20
    LDI r5, 10
    MUL r4, r5
...
pb_check_utf8_2:
    CMPI r10, ANS_UTF8_2
    JNZ r4, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r6, 128
    BLT r4, pb_reset_state
    CMPI r6, 192
    BGE r4, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r4 as dirty
; Sets bit (31 - row) in DIRTY_ROWS bitmap
mark_row_dirty:
    PUSH r11
    LDI r11, 31
    SUB r11, r4          ; r11 = 31 - row
    LDI r9, 1
    SHL r9, r11          ; r9 = 1 << (31 - row)
    LDI r0, DIRTY_ROWS
    LOAD r10, r0         ; r10 = current bitmap
    OR r10, r9           ; set the bit
    STORE r0, r10
    POP r11
    RET
...
; mark_row_dirty_check -- check if row in r4 is dirty
; Returns r4 = bit value (0=clean, nonzero=dirty)
mark_row_dirty_check:
    ...
    LOAD r10, r0         ; r10 = current bitmap
    MOV r4, r10
    AND r4, r9          ; r4 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r6 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r11, 1
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r0 = word_index, r4 = bit position within word
    PUSH r4              ; save bit position (CMP will clobber r4)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r0
    LOAD r9, r20
    POP r4               ; restore bit position
    ; Set bit: r9 |= (1 << r4)
    LDI r10, 1
    LDI r6, 0
sts_shift_loop:
    CMP r6, r4
    JZ r4, sts_shift_done
    SHL r10, r11
    ADD r6, r11
    JMP sts_shift_loop
sts_shift_done:
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r4)
    PUSH r4

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r9, r4           ; r9 = col (will become bit_pos after div)
    LDI r0, 0            ; r0 = word_index
    LDI r14, 32           ; r14 = divisor
att_div_loop:
    CMPI r9, 32
    BLT r4, att_check_bit
    SUB r9, r14           ; r9 -= 32
    ADD r0, r11           ; word_index++
    JMP att_div_loop
...
att_check_bit:
    ...
    ADD r20, r0
    LOAD r10, r20
    ; Check if bit r9 is set: shift 1 left by r9, AND
    LDI r6, 1
    LDI r5, 0
att_shift_loop:
    CMP r5, r9
...
att_test_bit:
    ...
    POP r4               ; restore scan column
att_scan_next:
    ADD r4, r11
    JMP att_scan_loop

att_found_pop:
    POP r4               ; restore scan column (r4 = tab stop column)
att_found:
    ; Found tab stop at column r4 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r4
    LDI r20, CUR_ROW
    LOAD r4, r20         ; r4 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r4
    POP r31
    RET
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r4 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r4               ; r4 = original count
    LDI r21, 10
    MOD r4, r21          ; r4 = count % 10
...
; Preserves all registers except r4 (CMP clobbers it).
...
apply_sgr:
    ...
    LDI r20, CSI_PARAM
    LOAD r5, r20       ; r5 = SGR code

    ; ── Extended SGR state machine ──
    LDI r20, SGR_EXTENDED
    LOAD r14, r20       ; r14 = extended state

    ; State 0x85: waiting for 256-color FG index
    CMPI r14, 0x85
    JNZ r4, sgr_ext95
    CALL sgr_256_to_rgb  ; r4 = RGB from r5 (color index)
    LDI r20, FG_COLOR
    STORE r20, r4
    LDI r20, SGR_EXTENDED
    LDI r4, 0
...
sgr_existing:
    ...
    ; r5 already loaded with SGR code from above
...
; Input:  r5 = color index (0-255)
; Output: r4 = RGB color value
; Clobbers: r4, r11, r10, r6
; Preserves: r5, r14, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r4, 232
    SUB r5, r4        ; r5 = index - 232
    LDI r4, 10
    MUL r5, r4        ; r5 = (index - 232) * 10
    LDI r4, 8
    ADD r5, r4        ; r5 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r5           ; save v
    ...
    MOV r4, r5
    SHL r4, r11        ; r4 = v << 16
    PUSH r4           ; save high bits
    LDI r11, 8
    MOV r4, r5
    SHL r4, r11        ; r4 = v << 8
    OR r4, r5         ; r4 = (v << 8) | v
    POP r11            ; r11 = v << 16
    OR r4, r11         ; r4 = (v << 16) | (v << 8) | v
    POP r5            ; restore r5
    JMP sgr256_ret
...
sgr256_cube:
    ...
    LDI r11, 8
    SHL r5, r11        ; r5 = G << 8
    OR r4, r5         ; r4 = (R << 16) | (G << 8)
    OR r4, r6         ; r4 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    LDI r20, CSI_PARAM
    LOAD r5, r20       ; r5 = mode (0, 1, or 2)

    ; Get current row
    LDI r20, CUR_ROW
    LOAD r14, r20       ; r14 = row
    LDI r13, COLS
    ...
    MOV r2, r12        ; r2 = end_col (exclusive)
    CMPI r5, 0
    JZ r4, cel_clear
    CMPI r5, 1
    JNZ r4, cel_mode2
...
cel_done:
    ...
    LDI r4, 1
    STORE r20, r4
    ; Mark current row as dirty (row is in r14 / COLS)
    ; Re-derive row from saved value: r14 = row * COLS, row = r14 / COLS
    ; Actually we saved CUR_ROW into r14 early, then multiplied. Re-read CUR_ROW.
    LDI r20, CUR_ROW
    LOAD r4, r20
    CALL mark_row_dirty
...
csi_cursor_position:
    ...
    LDI r5, 1
ccp_row_set:
    SUB r5, r11         ; convert to 0-based
    CMPI r5, 0
    BGE r4, ccp_row_clamp
    LDI r5, 0
ccp_row_clamp:
    LDI r14, ROWS
    SUB r14, r11
...
cil_clear_loop:
    ...
    LDI r13, ROWS
    SUB r13, r11
    BGE r4, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r12, r11
    CMP r4, r12
    BGE r4, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    LDI r13, ROWS
    SUB r13, r11         ; r13 = ROWS-1
    MOV r7, r13
    SUB r7, r5
    ADD r7, r11        ; r7 = first row to clear (ROWS - count)
    CMP r7, r14
    BGE r4, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r5
    BGE r4, cdl_done2
...
cdc_count:
    ...
    LOAD r14, r20
    LDI r20, CUR_COL
    LOAD r13, r20       ; r13 = cursor col

    ; Base offset = row * COLS
    LDI r12, COLS
    MOV r4, r14
    MUL r4, r12
    MOV r14, r4         ; r14 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r7, r13        ; r7 = dst col (starting at cursor)
cdc_shift:
    ADD r4, r7
    ADD r4, r5         ; r7 + count = src col
    LDI r12, COLS
    CMP r4, r12
    BGE r4, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r7, COLS
    SUB r7, r5        ; r7 = first col to clear
    LDI r12, COLS
    CMP r7, r13
    BGE r4, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r7 < r13 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r7, COLS
    SUB r7, r11        ; r7 = COLS-1 (start from rightmost)
    SUB r7, r5        ; r7 = COLS-1-count (first dst position)
cic_shift:
    CMP r7, r13
    BLT r4, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r4, r20
    JZ r4, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    JZ r4, render_row_skip
    LDI r11, 1           ; restore r11 (clobbered by mark_row_dirty_check)
    ...
    LDI r8, 0          ; x (use r8 to preserve r11)
    MOV r9, r1         ; y = 10 + row*6
    LDI r0, 256         ; w
    LDI r10, 6           ; h
    LDI r6, 0x0A0A0A    ; bg color
    RECTF r8, r9, r0, r10, r6

...
; _check_z: if r15==0 → pass (for non-CMP checks)
_check_z:
    JZ r15, _pass
    JMP _fail

; _check_nz: if r15!=0 → pass
_check_nz:
    JZ r15, _fail
    JMP _pass
...
; T16: malloc returns address >= HEAP_START
    ...
    LDI r4, 0x1F8F
    LDI r12, 0xC000
    ; Check r15 >= 0xC000: CMP(r15, 0xC000) should NOT be -1
    CMP r15, r12
    LDI r12, 0xFFFFFFFF
    CMP r15, r12
    ; If r15 == 0xFFFFFFFF (CMP was -1, addr < heap_start), fail
    JZ r15, _t16_fail
    LDI r15, 1
    LDI r4, 0x1F8F
    STORE r4, r15

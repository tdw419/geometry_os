...
; ═══════════════════════════════════════════════════════════════
    ...
    LDI r1, 0x1201
    LDI r10, 20
    STORE r1, r10          ; prompt y starts at line 20
    LDI r1, 0x1207
    LDI r10, 0
    STORE r1, r10          ; pipe mode = 0
    LDI r1, 0x1205
    STORE r1, r10          ; redirect mode = 0
...
main_loop:
    ...
    LDI r1, 0x1201         ; r1 = prompt y addr
    LOAD r5, r1            ; r5 = y position
    LDI r6, 2              ; x = 2
    LDI r7, prompt_str
    TEXT r6, r5, r7
    ...
    LDI r5, 200            ; max len
    LDI r6, 0x1200         ; pos addr
    LDI r1, 0x1200
    LDI r7, 0
    STORE r1, r7           ; reset position

readln_loop:
    READLN r10, r5, r6
    CMP r10, r7             ; r10 == 0 means still reading
    JZ r10, readln_loop
...
copy_cmd:
    ...
    STORE r2, r10
    ADD r12, r7            ; r7 is still 0 from earlier... let's use r5
...
do_cat:
    ...
    MOV r8, r10            ; save fd in r8
    LDI r7, 0x1000        ; read buffer
    LDI r4, 200           ; max bytes
    READ r8, r7, r4       ; r10 = bytes read
    ; Null terminate
    LDI r15, 200
    CMP r10, r15
    BLT r10, cat_term_ok
...
cp_src_done:
    STORE r2, r10
cp_do_copy:
    ; FCOPY r5, r6 where r5=0x1600 (src), r6=0x1800 (dst)
    LDI r5, 0x1600
    LDI r6, 0x1800
    FCOPY r5, r6
    ; Check result
...
do_edit:
    ...
    LDI r15, 0x2000
    ADD r15, r10            ; r15 = buffer + bytes_read
    LDI r0, 0
...
edit_line_loop:
    ...
    LDI r5, 1
    ADD r3, r5           ; advance source
    ADD r14, r5           ; advance line index
...
edit_newline:
    ...
    TEXT r6, r9, r7      ; render text at (4, y)

    ; Advance to next line
    LDI r5, 1
    ADD r3, r5           ; skip the newline char
    ADD r9, r5           ; next y position
...
save_scan_done:
    ...
    MOV r6, r5             ; fd_reg = r8 already
    ; WRITE fd_reg, buf_reg, len_reg
    MOV r5, r8             ; r5 = fd
    LDI r6, 0x2000         ; buf addr
    MOV r7, r9            ; length
    WRITE r5, r6, r7       ; r10 = bytes written
...
grep_have_file:
    LDI r10, 0
    STORE r9, r10         ; null terminate filename
...
grep_cmp_loop:
    ...
    ADD r12, r7
    ADD r11, r7
    ADD r5, r7             ; count matched chars
    CMP r5, r2            ; matched vs pattern_len
    BLT r10, grep_cmp_loop
    JMP grep_found_match

grep_no_match_here:
    LDI r7, 1
    ADD r1, r7             ; advance offset
...
tail_count_lines:
    ...
    LDI r5, 1
    ADD r11, r5           ; increment line count
tail_count_next:
    LDI r5, 1
    ADD r3, r5
...
tail_count_done:
    ...
    MOV r9, r11           ; total_lines
    SUB r9, r2           ; total - N
    LDI r5, 1
    CMP r9, r5
    BGE r10, tail_skip_ok
    MOV r9, r5            ; at least line 1
...
tail_skip_next:
    ...
    LDI r5, 1
    ADD r11, r5            ; next line on newline
tail_skip_adv:
    LDI r5, 1
    ADD r3, r5
    JMP tail_skip_loop
...
wc_utoa:
    ; Convert r3 to decimal, write to buffer at r9
    ; Modifies r3, does not change r9 (caller manages it)
    PUSH r12
    ; Handle 0 case
...
wc_do_digits:
    ...
    MOV r3, r2          ; restore original? No, r3 was modified.
...
wc_utoa_done:
    MOV r9, r14          ; update output pointer
    POP r14
    POP r11
    POP r2
    POP r12

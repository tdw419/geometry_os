...
LDI r7, 0x2000
STRO r7, "/etc/boot.cfg"

; Open the config file
LDI r6, 0x2000   ; path addr
LDI r2, 0        ; flags = O_RDONLY
LDI r13, 0x54     ; OPEN opcode
OPEN r6, r2
; r14 = fd (or 0xFFFFFFFF on error)
...
LDI r15, 512      ; max bytes
LDI r13, 0x55     ; READ opcode
READ r6, r2, r15
; r14 = bytes read

; Close the file
MOV r6, r3
LDI r13, 0x57     ; CLOSE opcode
CLOSE r6
...
LDI r4, 0x1800  ; cfg read pointer
LDI r8, 0x1000  ; service table base
LDI r11, 0       ; service count
LDI r1, 256     ; bytes per service entry
...
parse_line:
  ...
  ; Compute dest addr: r8 + r11 * r1
  MOV r16, r11
  MUL r16, r1
  ADD r16, r8    ; r16 = current service entry base
...
parse_path:
  ...
  STORE r18, r0
  ADD r17, r9     ; offset++
  ADD r4, r9     ; advance read pointer
  JMP parse_path
...
path_done:
  ...
  LDI r25, 32
  CMP r0, r25
  JNZ r14, parse_policy
  ADD r4, r9     ; skip space
...
parse_policy:
  ...
  LDI r25, 32
  ADD r18, r25
  SUB r0, r9     ; assuming digit '0'-'2', but actually we want the numeric value
  ...
  LDI r25, 48
  SUB r0, r25     ; r0 = policy value (0, 1, or 2)
  MOV r18, r16
  LDI r25, 33     ; offset 33
  ADD r18, r25
  STORE r18, r0  ; store restart_policy

  ADD r4, r9     ; skip policy char
  ; Skip space
  MOV r0, r4
  LOAD r0, r0
  LDI r25, 32
...
parse_dep:
  ...
  CMP r18, r25
  JNZ r14, store_dep
  LDI r0, 255   ; no dependency
store_dep:
  MOV r18, r16
  LDI r25, 34    ; offset 34
  ADD r18, r25
  STORE r18, r0 ; store dep_idx
...
parse_done:
  ; r11 = number of services parsed
  ; If no services, spawn shell manually
  JNZ r11, start_services
...
svc_loop:
  ...
  LDI r26, 1
  CMP r25, r26
  JZ r14, svc_next   ; already running, skip
...
supervisor:
  LDI r4, 60
  SLEEP r4        ; sleep ~1 second between checks

  LDI r23, 0       ; service index

check_loop:
  CMP r23, r11
  BGE r14, supervisor  ; all checked, loop back
  ...
  LDI r26, 1
  CMP r25, r26
  JNZ r14, check_next  ; not running, skip
  ...
  ; r14 = 0 means still running, nonzero means exited
  JZ r14, check_next
  ...
  LDI r26, 0xFFFFFFFF
  CMP r25, r26
  JZ r14, check_next  ; spawn failed

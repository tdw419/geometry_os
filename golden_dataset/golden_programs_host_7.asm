...
pb_csi_h_mode:
    ...
    CMPI r11, 104
    JNZ r1, pb_csi_l_mode
    CMPI r24, 0
    JZ r1, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r1, r20
    CMPI r1, 1049
    JNZ r1, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r1, pb_csi_s
    CMPI r24, 0
    JZ r1, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r1, r20
    CMPI r1, 1049
    JNZ r1, pb_ret        ; only handle 1049
...
pb_check_osc:
    CMPI r5, ANS_OSC
    JNZ r1, pb_check_utf8_2

    CMPI r11, 7
    JZ r1, pb_osc_end
    CMPI r11, 27
    JNZ r1, pb_osc_continue
    LDI r20, ANSI_STATE
    LDI r1, ANS_ESC
    STORE r20, r1
    JMP pb_ret
...
pb_check_utf8_2:
    CMPI r5, ANS_UTF8_2
    JNZ r1, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r11, 128
    BLT r1, pb_reset_state
    CMPI r11, 192
    BGE r1, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r1 as dirty
; Sets bit (31 - row) in DIRTY_ROWS bitmap
mark_row_dirty:
    PUSH r12
    LDI r12, 31
    SUB r12, r1          ; r12 = 31 - row
    LDI r2, 1
    SHL r2, r12          ; r2 = 1 << (31 - row)
    LDI r13, DIRTY_ROWS
    LOAD r5, r13         ; r5 = current bitmap
    OR r5, r2           ; set the bit
    STORE r13, r5
    POP r12
    RET
...
; mark_row_dirty_check -- check if row in r1 is dirty
; Returns r1 = bit value (0=clean, nonzero=dirty)
...
map_codepoint:
    ...
    CMPI r11, 9472
    BLT r1, mc_check_arrows
    CMPI r11, 9600
    BGE r1, mc_check_blocks
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r13 = word_index, r1 = bit position within word
    PUSH r1              ; save bit position (CMP will clobber r1)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r13
    LOAD r2, r20
    POP r1               ; restore bit position
    ; Set bit: r2 |= (1 << r1)
    LDI r5, 1
    LDI r11, 0
sts_shift_loop:
    CMP r11, r1
    JZ r1, sts_shift_done
    SHL r5, r12
    ADD r11, r12
    JMP sts_shift_loop
sts_shift_done:
...
ctc_shift_loop:
    CMP r11, r1
    JZ r1, ctc_shift_done
    SHL r5, r12
    ADD r11, r12
    JMP ctc_shift_loop
ctc_shift_done:
    ; r5 = mask. Invert: r5 = ~r5 (all bits except the one to clear)
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r1)
    PUSH r1

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r2, r1           ; r2 = col (will become bit_pos after div)
    LDI r13, 0            ; r13 = word_index
    LDI r15, 32           ; r15 = divisor
att_div_loop:
    CMPI r2, 32
    BLT r1, att_check_bit
    SUB r2, r15           ; r2 -= 32
    ADD r13, r12           ; word_index++
    JMP att_div_loop
...
att_test_bit:
    AND r5, r11
    JNZ r5, att_found_pop

    ; Not a tab stop -- restore column and try next
    POP r1               ; restore scan column
att_scan_next:
    ADD r1, r12
    JMP att_scan_loop

att_found_pop:
    POP r1               ; restore scan column (r1 = tab stop column)
att_found:
    ; Found tab stop at column r1 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r1
    LDI r20, CUR_ROW
    LOAD r1, r20         ; r1 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r1
    POP r31
    RET
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r1 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r1               ; r1 = original count
    LDI r21, 10
    MOD r1, r21          ; r1 = count % 10
...
; Preserves all registers except r1 (CMP clobbers it).
...
apply_sgr:
    ...
    CMPI r15, 0x85
    JNZ r1, sgr_ext95
    CALL sgr_256_to_rgb  ; r1 = RGB from r9 (color index)
    LDI r20, FG_COLOR
    STORE r20, r1
    LDI r20, SGR_EXTENDED
    LDI r1, 0
...
; Output: r1 = RGB color value
; Clobbers: r1, r12, r5, r11
; Preserves: r9, r15, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r1, 232
    SUB r9, r1        ; r9 = index - 232
    LDI r1, 10
    MUL r9, r1        ; r9 = (index - 232) * 10
    LDI r1, 8
    ADD r9, r1        ; r9 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r9           ; save v
    ...
    MOV r1, r9
    SHL r1, r12        ; r1 = v << 16
    PUSH r1           ; save high bits
    LDI r12, 8
    MOV r1, r9
    SHL r1, r12        ; r1 = v << 8
    OR r1, r9         ; r1 = (v << 8) | v
    POP r12            ; r12 = v << 16
    OR r1, r12         ; r1 = (v << 16) | (v << 8) | v
    POP r9            ; restore r9
    JMP sgr256_ret
...
sgr256_cube:
    ...
    LDI r12, 8
    SHL r9, r12        ; r9 = G << 8
    OR r1, r9         ; r1 = (R << 16) | (G << 8)
    OR r1, r11         ; r1 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r6, r3        ; r6 = end_col (exclusive)
    CMPI r9, 0
    JZ r1, cel_clear
    CMPI r9, 1
    JNZ r1, cel_mode2
...
ced_check0:
    ...
    LDI r6, COLS
    MUL r4, r6
    ADD r4, r3        ; offset = row*COLS + col

    LDI r15, ROWS
    LDI r0, COLS
    MUL r15, r0         ; total cells
ced_loop0:
    CMP r4, r15
    BGE r1, ced_done
    LDI r20, BUF
...
cil_count:
    ...
    LDI r4, ROWS
    SUB r4, r12        ; r4 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r4, r15
    BLT r1, cil_clear
...
cil_clear_loop:
    ...
    LDI r0, ROWS
    SUB r0, r12
    BGE r1, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r3, r12
    CMP r1, r3
    BGE r1, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    LDI r0, ROWS
    SUB r0, r12         ; r0 = ROWS-1
    MOV r4, r0
    SUB r4, r9
    ADD r4, r12        ; r4 = first row to clear (ROWS - count)
    CMP r4, r15
    BGE r1, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r9
    BGE r1, cdl_done2
...
cdc_count:
    ...
    LOAD r15, r20
    LDI r20, CUR_COL
    LOAD r0, r20       ; r0 = cursor col

    ; Base offset = row * COLS
    LDI r3, COLS
    MOV r1, r15
    MUL r1, r3
    MOV r15, r1         ; r15 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r4, r0        ; r4 = dst col (starting at cursor)
cdc_shift:
    ADD r1, r4
    ADD r1, r9         ; r4 + count = src col
    LDI r3, COLS
    CMP r1, r3
    BGE r1, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r4, COLS
    SUB r4, r9        ; r4 = first col to clear
    LDI r3, COLS
    CMP r4, r0
    BGE r1, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r4 < r0 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r4, COLS
    SUB r4, r12        ; r4 = COLS-1 (start from rightmost)
    SUB r4, r9        ; r4 = COLS-1-count (first dst position)
cic_shift:
    CMP r4, r0
    BLT r1, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r1, r20
    JZ r1, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    LDI r14, 0          ; x (use r14 to preserve r12)
    MOV r2, r7         ; y = 10 + row*6
    LDI r13, 256         ; w
    LDI r5, 6           ; h

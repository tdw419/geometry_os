...
after_input:
  ...
  ; check threshold
  LDI r9, 0x40D8
  LOAD r0, r9           ; soft_drop flag
  JNZ r0, soft_thresh
  
  ; normal threshold based on level: max(2, 15 - level)
  LDI r9, 0x40DA
  LOAD r11, r9           ; r11 = level
  LDI r7, 15
  SUB r7, r11            ; r7 = 15 - level
...
check_collision:
  ; load piece info
  LDI r9, 0x40D0
  LOAD r2, r9          ; current_piece index
  LDI r9, 0x40D1
  LOAD r12, r9          ; piece_x
  LDI r9, 0x40D2
  LOAD r6, r9          ; piece_y
  LDI r9, 0x40D3
  LOAD r1, r9          ; piece_rot
  ...
  LDI r7, 16
  MUL r13, r7
  ADD r3, r13            ; r3 = base address of rotation data

  LDI r8, 0            ; row counter (0-3)
cc_row:
  ; load row bitmap
  LDI r9, 0
  ADD r9, r3
...
cc_col:
  ...
  LDI r7, 0
  ADD r7, r9
  SHR r13, r7            ; r13 = bitmap >> (3-col)
  LDI r7, 1
  AND r13, r7            ; r13 = bit

  JZ r13, cc_next        ; empty cell, skip
  ...
  ADD r7, r12
  ADD r7, r16
  ADD r13, r7            ; board index
  LDI r9, 0x4000
  ADD r9, r13
  LOAD r13, r9           ; board cell
  JNZ r13, cc_collision  ; occupied = collision
...
cl_not_full:
  ...
  ; we compare with 0 as unsigned. If r8 was 0 and we subtract 1,
  ; it becomes 0xFFFFFFFF which is >= 0. So we need a different approach.
  ; Use CMP: if r8 < 0 (as signed), CMP with 0 would show BLT.
  ; But we use unsigned CMP. Let's just check if r8 > 19 or == 0xFFFFFFFF.
  ; Actually: CMP r8, r7 where r7=0 and using BLT: unsigned 0xFFFFFFFF > 0,
  ; so BLT won't trigger. We need a different sentinel.
  ; Fix: check if r8 < 0 by checking if r8 >= 0x80000000 (sign bit set)
  LDI r7, 0x80000000
  CMP r8, r7
...
cl_do_clear:
  ...
  ; row r8 is full: copy row r8-1 to r8, r8-2 to r8-1, etc.
  LDI r15, 0             ; src row = r8 - 1
  LDI r13, 1
  SUB r15, r8
  LDI r13, 1
  ; wait, r15 = r8, then subtract 1
  LDI r15, 0
  ADD r15, r8

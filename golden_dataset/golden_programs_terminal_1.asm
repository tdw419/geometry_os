...
LDI r0, 0x333355  ; bg = match title bar
DRAWTEXT r10, r6, r8, r14, r0
...
LDI r8, 36
LDI r14, 16
HITSET r10, r6, r8, r14, 99

; Restore r10 = 1 before writing prompt!
LDI r10, 1
...
copy_col:
    ...
    ; DRAWTEXT x=0, y=r4, addr=SCRATCH, fg=white, bg=0 (transparent)
    LDI r10, 0
    LDI r9, SCRATCH
    LDI r1, 0xCCCCCC  ; fg = light gray
    LDI r15, 0         ; bg = transparent
    DRAWTEXT r10, r4, r9, r1, r15
...
draw_cursor:
    ...
    MUL r6, r2           ; row * 8
    LDI r8, 16
    ADD r6, r8           ; y = 16 + row*8
    LDI r8, 6
    LDI r14, 8
    LDI r0, 0x44FF44
    RECTF r12, r6, r8, r14, r0
...
do_char:
    ...
    LOAD r6, r20
    LDI r8, COLS
    MUL r6, r8           ; r6 = row * COLS
    LDI r20, CUR_COL
    LOAD r12, r20
    ADD r6, r12           ; r6 = row*COLS + col
    LDI r20, BUF
...
extract_cmd:
    ...
    LOAD r7, r20          ; r7 = row
    LDI r13, COLS
    MUL r7, r13            ; r7 = row * COLS

    ; Source starts at col 2 (skip "$ ")
    LDI r20, BUF
    ADD r20, r7           ; r20 = BUF + row*COLS
    ADD r20, r10
    ADD r20, r10           ; r20 = BUF + row*COLS + 2
...
cmd_ls:
    ...
    LS r20                ; list files, count in r12
    CMPI r12, 0
    JZ r12, ls_empty
...
ls_copied:
    ADD r21, r10           ; skip null terminator
    CALL do_newline
    JMP ls_next
...
write_unknown:
    ...
    MUL r7, r13
    LDI r20, BUF
    ADD r20, r7           ; r20 = BUF + row*COLS (destination)
...
cmd_cat:
    ...
    ADD r21, r10
    ADD r21, r10
    ADD r21, r10           ; r21 = SCRATCH+4 (filename)
    ...
    MOV r18, r12           ; r18 = fd
    CMPI r12, 0xFFFFFFFF
    JNZ r12, cat_read      ; if not -1, file opened OK
...
cat_read:
    ...
    READ r18, r22, r23
    ; r12 = bytes read (0 = EOF, 0xFFFFFFFF = error)
    CMPI r12, 0
    JZ r12, cat_close      ; EOF or empty
    CMPI r12, 0xFFFFFFFF
    JNZ r12, cat_close     ; error
...
cat_write_loop:
    ...
    BGE r12, cat_close
    LOAD r12, r22
    JZ r12, cat_close      ; null terminator
...
cat_write_char:
    ; Write r12 (char) at current cursor position in buffer
    ...
    ADD r8, r10
    CMPI r8, COLS
    JNZ r12, cwc_done      ; branch if not equal (r12 = CMPI result)
    CALL do_newline
    JMP cwc_ret
cwc_done:
    LDI r20, CUR_COL
...
cmd_uptime:
    ...
    ADD r20, r10
    ADD r20, r10
    ADD r20, r10           ; advance past "uptime: "
    STRO r20, "running"
    CALL do_newline
    JMP dc_ret
...
cmd_run:
    ...
    ADD r21, r10       ; +2
    ADD r21, r10       ; +3
    ADD r21, r10       ; +4 = SCRATCH+4 (start of filename)
...
run_copy:
    ...
    ADD r21, r10
    ADD r22, r10
    JZ r12, run_exec   ; stop at null terminator

run_exec:
    ; EXEC takes a register pointing to the filename
    LDI r10, 0x5300
    EXEC r10            ; r12 = PID or 0xFFFFFFFF on error
    CMPI r12, 0xFFFFFFFF
    JZ r12, run_err
...
run_wait:
    WAITPID r6         ; r12 = 0 still running, 1 done
    JZ r12, run_wait
...
cmd_edit:
    ...
    ADD r21, r10       ; +3
    ADD r21, r10       ; +4
    ADD r21, r10       ; +5 = SCRATCH+5 (start of filename)
    LDI r22, 0x5300   ; dest buffer
edit_copy:
    LOAD r12, r21
    STORE r22, r12
    ADD r21, r10
    ADD r22, r10
    JZ r12, edit_fire   ; stop at null terminator
...
cmd_build:
    ...
    LDI r12, 47         ; '/'
    STORE r22, r12
    ADD r22, r10        ; r22 = 0x5309, pointing after "programs/"
    ...
    ADD r21, r10       ; +4
    ADD r21, r10       ; +5
    ADD r21, r10       ; +6 = SCRATCH+6 (start of filename)
...
build_copy:
    ...
    ADD r21, r10
    ADD r22, r10
    JZ r12, build_open   ; stop at null
...
build_open:
    ...
    LDI r21, 0x5300
    LDI r20, 0
    OPEN r21, r20, r20  ; fd in r12
    MOV r18, r12         ; r18 = fd
    CMPI r12, 0xFFFFFFFF
    JZ r12, build_fopen_err
    ...
    READ r18, r22, r23  ; r12 = bytes read
    MOV r19, r12         ; r19 = bytes read
...
build_pack:
    ...
    JZ r12, build_asm
    LOAD r12, r21
    STORE r22, r12       ; store the word (low byte has the char)
    ADD r21, r10
    ADD r22, r10
    SUB r24, r10
    JMP build_pack
...
build_asm:
    ...
    LDI r10, 0x9000      ; source address
    LDI r6, 0xC000      ; dest address for bytecode
    ASM r10, r6
...
do_newline:
    ...
    STORE r20, r12
    LDI r20, CUR_ROW
    LOAD r7, r20          ; r7 = current row
    ADD r7, r10            ; r7 = new row
    CMPI r7, ROWS
    BLT r12, dn_store      ; branch if new row < ROWS (r12 = CMPI result)

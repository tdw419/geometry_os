...
draw_status_done:
  ...
  LDI r10, BODY_BUF   ; body pointer (adjusted by scroll)

  ; Calculate start position in body based on scroll
  ; Each "line" is ~40 chars for wrapping. Approximate: skip scroll*32 chars
  LDI r8, 32
  LDI r12, 0          ; char offset in body
scroll_skip:
  JZ r15, scroll_done
  ADD r12, r8         ; offset += 32
  SUB r15, r14          ; scroll--
  JMP scroll_skip

scroll_done:
  ; r10 = BODY_BUF + r12 (body start with scroll)
  LDI r15, BODY_BUF
  ADD r15, r12
  MOV r10, r15
...
draw_line_loop:
  ...
  LOAD r12, r10        ; char at current body pos
  JZ r12, draw_content_done

...
mark_row_dirty:
    PUSH r0
    LDI r0, 31
    SUB r0, r15          ; r0 = 31 - row
    LDI r14, 1
    SHL r14, r0          ; r14 = 1 << (31 - row)
    LDI r2, DIRTY_ROWS
    LOAD r3, r2         ; r3 = current bitmap
    OR r3, r14           ; set the bit
    STORE r2, r3
    POP r0
    RET
...
att_scan_loop:
    ...
    MOV r14, r15           ; r14 = col (will become bit_pos after div)
    LDI r2, 0            ; r2 = word_index
    LDI r5, 32           ; r5 = divisor
att_div_loop:
    CMPI r14, 32
    BLT r15, att_check_bit
    SUB r14, r5           ; r14 -= 32
    ADD r2, r0           ; word_index++
    JMP att_div_loop
...
apply_sgr:
    ...
    LDI r20, SGR_EXTENDED
    LOAD r5, r20       ; r5 = extended state

    ; State 0x85: waiting for 256-color FG index
    CMPI r5, 0x85
    JNZ r15, sgr_ext95
    CALL sgr_256_to_rgb  ; r15 = RGB from r4 (color index)
    LDI r20, FG_COLOR
    STORE r20, r15
    LDI r20, SGR_EXTENDED
    LDI r15, 0
...
; Clobbers: r15, r0, r3, r8
; Preserves: r4, r5, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r15, 232
    SUB r4, r15        ; r4 = index - 232
    LDI r15, 10
    MUL r4, r15        ; r4 = (index - 232) * 10
    LDI r15, 8
    ADD r4, r15        ; r4 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r4           ; save v
...
sgr256_cube:
    ...
    LDI r0, 8
    SHL r4, r0        ; r4 = G << 8
    OR r15, r4         ; r15 = (R << 16) | (G << 8)
    OR r15, r8         ; r15 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    ; mode 1: 0..col
    ; mode 2: 0..COLS-1
    LDI r7, 0         ; r7 = start_col
    MOV r13, r12        ; r13 = end_col (exclusive)
    CMPI r4, 0
    JZ r15, cel_clear
    CMPI r4, 1
    JNZ r15, cel_mode2
...
cel_done:
    ...
    LDI r15, 1
    STORE r20, r15
    ; Mark current row as dirty (row is in r5 / COLS)
    ; Re-derive row from saved value: r5 = row * COLS, row = r5 / COLS
    ; Actually we saved CUR_ROW into r5 early, then multiplied. Re-read CUR_ROW.
    LDI r20, CUR_ROW
    LOAD r15, r20
    CALL mark_row_dirty
...
ced_check0:
    ...
    LDI r13, COLS
    MUL r7, r13
    ADD r7, r12        ; offset = row*COLS + col

    LDI r5, ROWS
    LDI r10, COLS
    MUL r5, r10         ; total cells
ced_loop0:
    CMP r7, r5
    BGE r15, ced_done
    LDI r20, BUF
...
cdl_clear:
    ...
    LDI r10, ROWS
    SUB r10, r0         ; r10 = ROWS-1
    MOV r7, r10
    SUB r7, r4
    ADD r7, r0        ; r7 = first row to clear (ROWS - count)
    CMP r7, r5
...
cdc_count:
    ...
    LOAD r5, r20
    LDI r20, CUR_COL
    LOAD r10, r20       ; r10 = cursor col

    ; Base offset = row * COLS
    LDI r12, COLS
    MOV r15, r5
    MUL r15, r12
    MOV r5, r15         ; r5 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r7, r10        ; r7 = dst col (starting at cursor)
cdc_shift:
    ADD r15, r7
    ADD r15, r4         ; r7 + count = src col
    LDI r12, COLS
    CMP r15, r12
...
cdc_clear:
    ...
    LDI r7, COLS
    SUB r7, r4        ; r7 = first col to clear
    LDI r12, COLS
    CMP r7, r10
    BGE r15, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r7 < r10 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r7, COLS
    SUB r7, r0        ; r7 = COLS-1 (start from rightmost)
    SUB r7, r4        ; r7 = COLS-1-count (first dst position)
cic_shift:
    CMP r7, r10
    BLT r15, cic_clear

    ; dst = BUF + base + r7 + count
    LDI r21, BUF
    MOV r15, r5
    ADD r21, r15
    ADD r21, r7
...
render_row:
    ...
    LDI r0, 1           ; restore r0 (clobbered by mark_row_dirty_check)
    ...
    LDI r11, 0          ; x (use r11 to preserve r0)
    MOV r14, r1         ; y = 10 + row*6
    LDI r2, 256         ; w
    LDI r3, 6           ; h

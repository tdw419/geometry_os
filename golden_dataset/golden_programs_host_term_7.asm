...
mark_row_dirty:
    ...
    SUB r8, r7          ; r8 = 31 - row
    LDI r10, 1
    SHL r10, r8          ; r10 = 1 << (31 - row)
    LDI r13, DIRTY_ROWS
    LOAD r6, r13         ; r6 = current bitmap
    OR r6, r10           ; set the bit
    STORE r13, r6
    POP r8
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r13 = word_index, r7 = bit position within word
...
att_scan_loop:
    ...
    MOV r10, r7           ; r10 = col (will become bit_pos after div)
    LDI r13, 0            ; r13 = word_index
    LDI r12, 32           ; r12 = divisor
att_div_loop:
    CMPI r10, 32
    BLT r7, att_check_bit
    SUB r10, r12           ; r10 -= 32
    ADD r13, r8           ; word_index++
    JMP att_div_loop
...
apply_sgr:
    ...
    LDI r20, SGR_EXTENDED
    LOAD r12, r20       ; r12 = extended state

    ; State 0x85: waiting for 256-color FG index
    CMPI r12, 0x85
    JNZ r7, sgr_ext95
    CALL sgr_256_to_rgb  ; r7 = RGB from r0 (color index)
    LDI r20, FG_COLOR
    STORE r20, r7
    LDI r20, SGR_EXTENDED
    LDI r7, 0
...
; Clobbers: r7, r8, r6, r11
; Preserves: r0, r12, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r7, 232
    SUB r0, r7        ; r0 = index - 232
    LDI r7, 10
    MUL r0, r7        ; r0 = (index - 232) * 10
    LDI r7, 8
    ADD r0, r7        ; r0 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r0           ; save v
...
sgr256_cube:
    ...
    LDI r8, 8
    SHL r0, r8        ; r0 = G << 8
    OR r7, r0         ; r7 = (R << 16) | (G << 8)
    OR r7, r11         ; r7 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    ; Get current row
    LDI r20, CUR_ROW
    LOAD r12, r20       ; r12 = row
    LDI r3, COLS
    MUL r12, r3         ; r12 = row * COLS
    ...
    ; mode 1: 0..col
    ; mode 2: 0..COLS-1
    LDI r1, 0         ; r1 = start_col
    MOV r5, r14        ; r5 = end_col (exclusive)
    CMPI r0, 0
    JZ r7, cel_clear
    CMPI r0, 1
    JNZ r7, cel_mode2
...
cel_done:
    ...
    LDI r7, 1
    STORE r20, r7
    ; Mark current row as dirty (row is in r12 / COLS)
    ; Re-derive row from saved value: r12 = row * COLS, row = r12 / COLS
    ; Actually we saved CUR_ROW into r12 early, then multiplied. Re-read CUR_ROW.
    LDI r20, CUR_ROW
    LOAD r7, r20
    CALL mark_row_dirty
...
ced_check0:
    ...
    LDI r5, COLS
    MUL r1, r5
    ADD r1, r14        ; offset = row*COLS + col

    LDI r12, ROWS
    LDI r3, COLS
    MUL r12, r3         ; total cells
ced_loop0:
    CMP r1, r12
    BGE r7, ced_done
    LDI r20, BUF
...
cdl_clear:
    ...
    MOV r1, r3
    SUB r1, r0
    ADD r1, r8        ; r1 = first row to clear (ROWS - count)
    CMP r1, r12
...
cdc_count:
    ...
    MOV r7, r12
    MUL r7, r14
    MOV r12, r7         ; r12 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r1, r3        ; r1 = dst col (starting at cursor)
cdc_shift:
    ADD r7, r1
    ADD r7, r0         ; r1 + count = src col
    LDI r14, COLS
    CMP r7, r14
...
cdc_clear:
    ...
    LDI r1, COLS
    SUB r1, r0        ; r1 = first col to clear
    LDI r14, COLS
    CMP r1, r3
    BGE r7, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r1 < r3 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r1, COLS
    SUB r1, r8        ; r1 = COLS-1 (start from rightmost)
    SUB r1, r0        ; r1 = COLS-1-count (first dst position)
cic_shift:
    CMP r1, r3
    BLT r7, cic_clear

    ; dst = BUF + base + r1 + count
    LDI r21, BUF
    MOV r7, r12
    ADD r21, r7
    ADD r21, r1
...
render_row:
    ...
    LDI r4, 0          ; x (use r4 to preserve r8)
    MOV r10, r2         ; y = 10 + row*6
    LDI r13, 256         ; w
    LDI r6, 6           ; h

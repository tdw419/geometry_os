...
; ═══════════════════════════════════════════════════════════════
    ...
    LDI r10, 0xF00
    LDI r3, 0
    STORE r10, r3            ; active_session = 0
    ADD r10, r15
    STORE r10, r3            ; session_0_output_len = 0
    ADD r10, r15
    STORE r10, r3            ; session_1_output_len = 0

    ; Initialize scrollback buffers to zero
    ; NOTE: r3 is CMP result register, so use r12 for zero
    LDI r12, 0
    LDI r7, 0x5000
    LDI r5, 0x7000
init_buffers:
    ...
    LDI r10, 0x4002
    LDI r3, 0
    STORE r10, r3            ; session_0_pid = 0 (not spawned yet)
...
main_loop:
    ...
    MUL r5, r8
    ADD r9, r5             ; r9 = &session[active].stdin_write_fd
...
draw_status_bar:
    ...
    LDI r1, 256
    LDI r6, 12
    LDI r11, 0x1A1A2E        ; dark blue
    RECTF r3, r2, r1, r6, r11

    ; Draw "S0" indicator (session 0)
    LDI r3, 0xF00
    LOAD r3, r3              ; active_session
    JZ r3, s0_active

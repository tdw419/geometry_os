...
mark_row_dirty:
    ...
    SUB r5, r9          ; r5 = 31 - row
    LDI r13, 1
    SHL r13, r5          ; r13 = 1 << (31 - row)
    LDI r14, DIRTY_ROWS
    LOAD r8, r14         ; r8 = current bitmap
    OR r8, r13           ; set the bit
    STORE r14, r8
    POP r5
    RET
...
att_scan_loop:
    ...
    MOV r13, r9           ; r13 = col (will become bit_pos after div)
...
; Clobbers: r9, r5, r8, r3
; Preserves: r6, r0, r20, r31
; Palette:
...
csi_erase_line:
    ...
    MOV r4, r10        ; r4 = end_col (exclusive)
    CMPI r6, 0
    JZ r9, cel_clear
    CMPI r6, 1
    JNZ r9, cel_mode2
...
ced_check0:
    ...
    LDI r4, COLS
    MUL r11, r4
    ADD r11, r10        ; offset = row*COLS + col

    LDI r0, ROWS
    LDI r1, COLS
    MUL r0, r1         ; total cells
ced_loop0:
    CMP r11, r0
    BGE r9, ced_done
    LDI r20, BUF
...
cil_count:
    ...
    LDI r11, ROWS
    SUB r11, r5        ; r11 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r11, r0
    BLT r9, cil_clear
...
cdl_clear:
    ...
    MOV r11, r1
    SUB r11, r6
    ADD r11, r5        ; r11 = first row to clear (ROWS - count)
    CMP r11, r0
...
cdc_count:
    ...
    MOV r9, r0
    MUL r9, r10
    MOV r0, r9         ; r0 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r11, r1        ; r11 = dst col (starting at cursor)
cdc_shift:
    ADD r9, r11
    ADD r9, r6         ; r11 + count = src col
    LDI r10, COLS
    CMP r9, r10
...
cdc_clear:
    ...
    LDI r11, COLS
    SUB r11, r6        ; r11 = first col to clear
    LDI r10, COLS
    CMP r11, r1
    BGE r9, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r11 < r1 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r11, COLS
    SUB r11, r5        ; r11 = COLS-1 (start from rightmost)
    SUB r11, r6        ; r11 = COLS-1-count (first dst position)
cic_shift:
    CMP r11, r1
    BLT r9, cic_clear
...
render_row:
    ...
    LDI r12, 0          ; x (use r12 to preserve r5)
    MOV r13, r2         ; y = 10 + row*6
    LDI r14, 256         ; w
    LDI r8, 6           ; h

...
; ── Register plan ──────────────────────────────────────────────
;   r13  = sprite x position (top-left of rendered area)
;   r11  = sprite y position (top-left of rendered area)
;   r8  = base sprite size (32x32)
;   r5  = current scale factor (32 = 1x, 64 = 2x, etc.)
...
;   r6 = source y
;   r4 = dest x (scaled output position)
;   r16 = dest y

; ── init ──────────────────────────────────────────────────────
LDI r13, 0        ; sprite screen x (computed each frame)
LDI r11, 0        ; sprite screen y (computed each frame)
LDI r8, 32       ; base sprite size
LDI r5, 32       ; scale = 1x (32x32 output = 32x32 source)
LDI r3, 0        ; angle = 0
LDI r7, 0xFF0000 ; red
LDI r12, 0        ; black
...
angle_inc:
  LDI r0, 64
  ADD r3, r0        ; +64 fixed-point (~14 degrees)
  JMP frame_done

angle_dec:
  LDI r0, 64
  SUB r3, r0        ; -64 fixed-point (~14 degrees)
  JMP frame_done

scale_up:
  LDI r0, 8
  ADD r5, r0        ; increase scale
  ; Clamp: max 128
  LDI r0, 128
  CMP r5, r0
  BLT r14, frame_done

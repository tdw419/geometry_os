...
;   r14  = sprite y position (top-left of rendered area)
;   r4  = base sprite size (32x32)
;   r2  = current scale factor (32 = 1x, 64 = 2x, etc.)
;   r9  = angle (fixed-point radians, increments by 64 = ~14 degrees)
;   r7  = scratch
;   r11  = key code from IKEY
;   r3  = color (red)
;   r13  = 0 (black for clearing)
;   r5 = color (green)
;   r0 = color (blue)
;   r12 = color (yellow)
;   r6 = source x (sprite drawn at fixed position)
;   r1 = source y
...
LDI r14, 0        ; sprite screen y (computed each frame)
LDI r4, 32       ; base sprite size
...
LDI r6, 0       ; source x (sprite drawn at top-left)
LDI r1, 0       ; source y
...
angle_inc:
  LDI r7, 64
  ADD r9, r7        ; +64 fixed-point (~14 degrees)
  JMP frame_done

angle_dec:
  LDI r7, 64
  SUB r9, r7        ; -64 fixed-point (~14 degrees)
  JMP frame_done

scale_up:
  LDI r7, 8
  ADD r2, r7        ; increase scale
  ; Clamp: max 128
  LDI r7, 128
  CMP r2, r7
  BLT r15, frame_done

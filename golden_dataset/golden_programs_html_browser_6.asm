; DESCRIPTION: A colored object centered at the screen with fixed size.

; html_browser.asm -- Simple HTML Browser for Geometry OS
;
; Fetches HTML pages via HTTPGET, parses with HTPARSE, renders styled text.
; Supports: headings (h1/h2/h3), paragraphs, links, bold, italic, horizontal rules.
; Navigation: Enter=fetch, W/S=scroll, B=back, H=home, M=bookmark.
;
; Memory layout:
;   0x5000  URL buffer (64 chars)
;   0x5100  HTTP host buffer
;   0x5200  HTTP path buffer
;   0x9000  HTML body buffer (8K)
;   0xA000  Parsed lines (100 lines x 33 words = 3300 words)
;   0xD000  Scroll offset (1 word)
;   0xD004  Line count (1 word)
;   0xD008  Mode (0=browse, 1=edit url)
;   0xD00C  History count
;   0xD010  History URLs (4 x 32 words)

; -- Constants --
LDI r20, 0       ; color: black
LDI r21, 0x00FFFF ; color: cyan
LDI r22, 0x00FF00 ; color: green
LDI r23, 0xFFFFFF ; color: white
LDI r24, 0x000040 ; color: dark blue (url bar bg)
LDI r25, 0x333333 ; color: dark gray (status)

; -- Init --
LDI r0, 0xD000
LDI r2, 0
STORE r0, r2     ; scroll = 0
LDI r0, 0xD004
STORE r0, r2     ; line_count = 0
LDI r0, 0xD008
STORE r0, r2     ; mode = browse
LDI r0, 0xD00C
STORE r0, r2     ; history count = 0

; -- Build a test page in HTML buffer --
; Write "<h1>Geometry OS</h1><p>Welcome!</p>" into 0x9000
LDI r5, 0x9000
LDI r9, 0

; Write chars using STRO-like STORE sequence
; "<h1>Geometry OS</h1><p>Welcome!</p>"
; Character codes: < = 60, h = 104, 1 = 49, > = 62, etc.
; We'll write the HTML string byte by byte
LDI r6, 60      ; <
STORE r5, r6
LDI r6, 104     ; h
LDI r9, 1
ADD r5, r9
STORE r5, r6
LDI r6, 49      ; 1
ADD r5, r9
STORE r5, r6
LDI r6, 62      ; >
ADD r5, r9
STORE r5, r6
LDI r6, 71      ; G
ADD r5, r9
STORE r5, r6
LDI r6, 101     ; e
ADD r5, r9
STORE r5, r6
LDI r6, 111     ; o
ADD r5, r9
STORE r5, r6
LDI r6, 109     ; m
ADD r5, r9
STORE r5, r6
LDI r6, 101     ; e
ADD r5, r9
STORE r5, r6
LDI r6, 116     ; t
ADD r5, r9
STORE r5, r6
LDI r6, 114     ; r
ADD r5, r9
STORE r5, r6
LDI r6, 121     ; y
ADD r5, r9
STORE r5, r6
LDI r6, 32      ; (space)
ADD r5, r9
STORE r5, r6
LDI r6, 79      ; O
ADD r5, r9
STORE r5, r6
LDI r6, 83      ; S
ADD r5, r9
STORE r5, r6
LDI r6, 60      ; <
ADD r5, r9
STORE r5, r6
LDI r6, 47      ; /
ADD r5, r9
STORE r5, r6
LDI r6, 104     ; h
ADD r5, r9
STORE r5, r6
LDI r6, 49      ; 1
ADD r5, r9
STORE r5, r6
LDI r6, 62      ; >
ADD r5, r9
STORE r5, r6
LDI r6, 60      ; <
ADD r5, r9
STORE r5, r6
LDI r6, 112     ; p
ADD r5, r9
STORE r5, r6
LDI r6, 62      ; >
ADD r5, r9
STORE r5, r6
LDI r6, 87      ; W
ADD r5, r9
STORE r5, r6
LDI r6, 101     ; e
ADD r5, r9
STORE r5, r6
LDI r6, 108     ; l
ADD r5, r9
STORE r5, r6
LDI r6, 99      ; c
ADD r5, r9
STORE r5, r6
LDI r6, 111     ; o
ADD r5, r9
STORE r5, r6
LDI r6, 109     ; m
ADD r5, r9
STORE r5, r6
LDI r6, 101     ; e
ADD r5, r9
STORE r5, r6
LDI r6, 33      ; !
ADD r5, r9
STORE r5, r6
LDI r6, 60      ; <
ADD r5, r9
STORE r5, r6
LDI r6, 47      ; /
ADD r5, r9
STORE r5, r6
LDI r6, 112     ; p
ADD r5, r9
STORE r5, r6
LDI r6, 62      ; >
ADD r5, r9
STORE r5, r6
LDI r6, 0       ; null terminator
ADD r5, r9
STORE r5, r6

; -- Parse HTML --
HITCLR
LDI r5, 0x9000  ; src = HTML buffer
LDI r9, 0xA000  ; dest = parsed lines buffer
LDI r6, 100     ; max_lines = 100
HTPARSE r5, r9, r6
; r7 = line count

; Store line count
LDI r0, 0xD004
STORE r0, r7

; -- Render URL bar --
; Draw dark blue bar at top
LDI r5, 0
LDI r9, 0
LDI r6, 256
LDI r15, 16
RECTF r5, r9, r6, r15, r24

; Draw "geo://" in URL bar using TEXT opcode
LDI r5, 0x5000
LDI r6, 103     ; g
STORE r5, r6
LDI r9, 1
ADD r5, r9
LDI r6, 101     ; e
STORE r5, r6
ADD r5, r9
LDI r6, 111     ; o
STORE r5, r6
ADD r5, r9
LDI r6, 58      ; :
STORE r5, r6
ADD r5, r9
LDI r6, 47      ; /
STORE r5, r6
ADD r5, r9
LDI r6, 47      ; /
STORE r5, r6
ADD r5, r9
LDI r6, 104     ; h
STORE r5, r6
ADD r5, r9
LDI r6, 111     ; o
STORE r5, r6
ADD r5, r9
LDI r6, 109     ; m
STORE r5, r6
ADD r5, r9
LDI r6, 101     ; e
STORE r5, r6
ADD r5, r9
LDI r6, 0       ; null
STORE r5, r6

; Render URL text
LDI r5, 2       ; x
LDI r9, 2       ; y
LDI r6, 0x5000  ; addr
LDI r15, 0x00FFFF ; cyan color
TEXT r5, r9, r6

; -- Render status bar --
LDI r5, 0
LDI r9, 240
LDI r6, 256
LDI r15, 16
RECTF r5, r9, r6, r15, r25

; -- Render parsed content --
; Read scroll offset
LDI r1, 0xD000
LOAD r1, r1    ; r1 = scroll

; Read line count
LDI r8, 0xD004
LOAD r8, r8    ; r8 = line_count

; Render up to 24 visible lines (screen rows 20-236, 8px per line)
LDI r0, 0        ; visible line counter
LDI r2, 1        ; increment
LDI r3, 24       ; max visible lines
LDI r5, 0       ; base Y = 20

render_loop:
  CMP r0, r3
  BGE r7, render_done
  CMP r0, r8
  BGE r7, render_done

  ; Calculate line address: 0xA000 + (scroll + vis_idx) * 33
  MOV r9, r1     ; scroll
  ADD r9, r0      ; + vis_idx
  LDI r6, 33
  MUL r9, r6     ; * 33
  LDI r6, 0xA000
  ADD r9, r6     ; + base

  ; Check if we have valid data (color != 0 or has chars)
  LOAD r6, r9    ; fg_color
  CMP r6, r20     ; compare with 0 (black = empty)
  JZ r7, next_line

  ; Draw the line using DRAWTEXT-style rendering
  ; For simplicity, draw first char as a colored pixel
  ; Real rendering would use DRAWTEXT opcode with per-line colors
  LDI r15, 4       ; x offset
  MOV r15, r5     ; y = base_y + line * 8
  LDI r16, 8
  MUL r15, r16
  LDI r16, 20
  ADD r15, r16

  ; Draw each char as a single pixel (simplified)
  LDI r16, 0       ; char index
  draw_chars:
    CMP r16, r2
    BGE r7, next_line
    LDI r17, 1
    CMP r16, r17
    BGE r7, skip_first
    ; Draw char pixel
    MOV r18, r9
    ADD r18, r16
    ADD r18, r2     ; skip color word
    LOAD r18, r18   ; char code
    CMP r18, r20
    JZ r7, next_line

    ; x = 4 + char_idx * 8, y = line_y
    MOV r19, r16
    LDI r17, 8
    MUL r19, r17
    LDI r17, 4
    ADD r19, r17
    PSET r19, r15, r6  ; colored pixel

  skip_first:
    ADD r16, r2
    JMP draw_chars

next_line:
  ADD r0, r2
  ADD r5, r2
  JMP render_loop

render_done:
  ; Wait for user input
  FILL r20         ; clear screen (black)
  FRAME
  JMP render_done  ; infinite display loop

HALT

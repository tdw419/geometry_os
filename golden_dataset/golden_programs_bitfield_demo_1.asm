...
LDI r11, 0xAA        ; R channel
LDI r2, 8           ; shift amount
SHL r11, r2          ; r11 = 0x0000AA00 (R at bits 8-15)
LDI r15, 0xBB        ; G channel
OR r11, r15           ; r11 = 0x0000AABB (R+G)
LDI r2, 16
SHL r11, r2          ; r11 = 0xAABB0000 (R+G shifted up)
LDI r15, 0xCC        ; B channel
OR r11, r15           ; r11 = 0xAABBCC (packed: R=bits16-23, G=bits8-15, B=bits0-7)

; Now extract each channel using BFE
; BFE r9, r11, r13, r12 -- extract 8 bits at lsb=16 from r11 -> R
LDI r13, 8           ; width = 8
LDI r12, 16          ; lsb = 16
BFE r9, r11, r13, r12  ; r9 = 0xAA (red channel)

; BFE r14, r11, r13, r4 -- extract 8 bits at lsb=8 from r11 -> G
LDI r4, 8           ; lsb = 8
BFE r14, r11, r13, r4  ; r14 = 0xBB (green channel)

; BFE r3, r11, r8, r5 -- extract 8 bits at lsb=0 from r11 -> B
LDI r8, 8          ; width = 8
LDI r5, 0          ; lsb = 0
BFE r3, r11, r8, r5 ; r3 = 0xCC (blue channel)

; Now reassemble as BGR: B at bits 16-23, G at bits 8-15, R at bits 0-7
LDI r7, 0          ; start with 0

; Insert B (r3) at bits 16-23
LDI r10, 8          ; width = 8
LDI r6, 16         ; lsb = 16
MOV r1, r3         ; src = blue
BFI r7, r1, r10, r6 ; r7 = 0x00CC0000

; Insert G (r14) at bits 8-15
LDI r6, 8          ; lsb = 8
MOV r1, r14         ; src = green
BFI r7, r1, r10, r6 ; r7 = 0x00CCBB00

; Insert R (r9) at bits 0-7
LDI r6, 0          ; lsb = 0
MOV r1, r9         ; src = red
BFI r7, r1, r10, r6 ; r7 = 0x00CCBBAA (BGR swapped!)
...
LDI r22, 32
LDI r23, 32
RECTF r20, r21, r22, r23, r11  ; original
...
LDI r13, 1
LDI r12, 0
BFE r17, r16, r13, r12  ; r17 = bit 0 of 0xAA = 0

; Extract bit 1 (should be 1)
LDI r12, 1
BFE r18, r16, r13, r12  ; r18 = bit 1 of 0xAA = 1

; Extract bit 7 (should be 1)
LDI r12, 7
BFE r19, r16, r13, r12  ; r19 = bit 7 of 0xAA = 1

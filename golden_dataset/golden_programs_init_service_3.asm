...
LDI r4, 0x2000
STRO r4, "/etc/boot.cfg"

; Open the config file
LDI r15, 0x2000   ; path addr
LDI r14, 0        ; flags = O_RDONLY
LDI r0, 0x54     ; OPEN opcode
OPEN r15, r14
; r5 = fd (or 0xFFFFFFFF on error)
...
parse_path:
  ...
  STORE r18, r11
  ADD r17, r9     ; offset++
  ADD r10, r9     ; advance read pointer
  JMP parse_path
...
parse_done:
  ; r8 = number of services parsed
  ; If no services, spawn shell manually
  JNZ r8, start_services
...
svc_loop:
  ...
  LDI r26, 1
  CMP r25, r26
  JZ r5, svc_next   ; already running, skip
  ...
  LDI r26, 255
  CMP r25, r26
  JZ r5, can_start  ; no dep, can start
  ...
  LDI r27, 1
  CMP r26, r27
  JZ r5, can_start  ; dep is running, can start
  JMP svc_next       ; dep not running, skip for now

can_start:
  ; Spawn the service
  ; path is at service base, use EXEC
  MOV r5, r16       ; path addr = service base
  EXEC r5
...
check_loop:
  CMP r23, r8
  BGE r5, supervisor  ; all checked, loop back
  ...
  LDI r26, 1
  CMP r25, r26
  JNZ r5, check_next  ; not running, skip
  ...
  ; r5 = 0 means still running, nonzero means exited
  JZ r5, check_next

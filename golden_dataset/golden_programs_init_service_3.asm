...
LDI r13, 0x2000
STRO r13, "/etc/boot.cfg"

; Open the config file
LDI r9, 0x2000   ; path addr
LDI r8, 0        ; flags = O_RDONLY
LDI r0, 0x54     ; OPEN opcode
OPEN r9, r8
; r1 = fd (or 0xFFFFFFFF on error)
...
MOV r9, r10       ; fd
LDI r8, 0x1800   ; buffer addr
LDI r2, 512      ; max bytes
LDI r0, 0x55     ; READ opcode
READ r9, r8, r2
; r1 = bytes read

; Close the file
MOV r9, r10
LDI r0, 0x57     ; CLOSE opcode
CLOSE r9
...
parse_path:
  ...
  STORE r18, r14
  ADD r17, r12     ; offset++
  ADD r11, r12     ; advance read pointer
  JMP parse_path
...
parse_policy:
  ...
  LDI r25, 32
  ADD r18, r25
  SUB r14, r12     ; assuming digit '0'-'2', but actually we want the numeric value
  ...
  LDI r25, 48
  SUB r14, r25     ; r14 = policy value (0, 1, or 2)
  MOV r18, r16
  LDI r25, 33     ; offset 33
  ADD r18, r25
  STORE r18, r14  ; store restart_policy

  ADD r11, r12     ; skip policy char
  ; Skip space
  MOV r14, r11
  LOAD r14, r14
  LDI r25, 32
...
parse_dep:
  ...
  CMP r18, r25
  JNZ r1, store_dep
  LDI r14, 255   ; no dependency
store_dep:
  MOV r18, r16
  LDI r25, 34    ; offset 34
  ADD r18, r25
  STORE r18, r14 ; store dep_idx
...
svc_loop:
  ...
  LDI r26, 1
  CMP r25, r26
  JZ r1, svc_next   ; already running, skip
  ...
  LDI r26, 255
  CMP r25, r26
  JZ r1, can_start  ; no dep, can start
  ...
  LDI r27, 1
  CMP r26, r27
  JZ r1, can_start  ; dep is running, can start
  JMP svc_next       ; dep not running, skip for now

can_start:
  ; Spawn the service
  ; path is at service base, use EXEC
  MOV r1, r16       ; path addr = service base
  EXEC r1
...
supervisor:
  LDI r11, 60
  SLEEP r11        ; sleep ~1 second between checks

  LDI r23, 0       ; service index

check_loop:
  CMP r23, r7
  BGE r1, supervisor  ; all checked, loop back
  ...
  LDI r26, 1
  CMP r25, r26
  JNZ r1, check_next  ; not running, skip
  ...
  ; r1 = 0 means still running, nonzero means exited
  JZ r1, check_next

...
LDI r12, 0xAA        ; R channel
LDI r3, 8           ; shift amount
SHL r12, r3          ; r12 = 0x0000AA00 (R at bits 8-15)
LDI r10, 0xBB        ; G channel
OR r12, r10           ; r12 = 0x0000AABB (R+G)
LDI r3, 16
SHL r12, r3          ; r12 = 0xAABB0000 (R+G shifted up)
LDI r10, 0xCC        ; B channel
OR r12, r10           ; r12 = 0xAABBCC (packed: R=bits16-23, G=bits8-15, B=bits0-7)

; Now extract each channel using BFE
; BFE r2, r12, r13, r1 -- extract 8 bits at lsb=16 from r12 -> R
LDI r13, 8           ; width = 8
LDI r1, 16          ; lsb = 16
BFE r2, r12, r13, r1  ; r2 = 0xAA (red channel)

; BFE r0, r12, r13, r9 -- extract 8 bits at lsb=8 from r12 -> G
LDI r9, 8           ; lsb = 8
BFE r0, r12, r13, r9  ; r0 = 0xBB (green channel)

; BFE r8, r12, r5, r15 -- extract 8 bits at lsb=0 from r12 -> B
LDI r5, 8          ; width = 8
LDI r15, 0          ; lsb = 0
BFE r8, r12, r5, r15 ; r8 = 0xCC (blue channel)

; Now reassemble as BGR: B at bits 16-23, G at bits 8-15, R at bits 0-7
LDI r11, 0          ; start with 0

; Insert B (r8) at bits 16-23
LDI r4, 8          ; width = 8
LDI r7, 16         ; lsb = 16
MOV r6, r8         ; src = blue
BFI r11, r6, r4, r7 ; r11 = 0x00CC0000

; Insert G (r0) at bits 8-15
LDI r7, 8          ; lsb = 8
MOV r6, r0         ; src = green
BFI r11, r6, r4, r7 ; r11 = 0x00CCBB00

; Insert R (r2) at bits 0-7
LDI r7, 0          ; lsb = 0
MOV r6, r2         ; src = red
BFI r11, r6, r4, r7 ; r11 = 0x00CCBBAA (BGR swapped!)
...
LDI r22, 32
LDI r23, 32
RECTF r20, r21, r22, r23, r12  ; original

; Swapped color (CCBBAA) as a 32x32 rect at (60, 10)
LDI r20, 60
LDI r21, 10
RECTF r20, r21, r22, r23, r11 ; swapped
...
LDI r13, 1
LDI r1, 0
BFE r17, r16, r13, r1  ; r17 = bit 0 of 0xAA = 0

; Extract bit 1 (should be 1)
LDI r1, 1
BFE r18, r16, r13, r1  ; r18 = bit 1 of 0xAA = 1

; Extract bit 7 (should be 1)
LDI r1, 7
BFE r19, r16, r13, r1  ; r19 = bit 7 of 0xAA = 1

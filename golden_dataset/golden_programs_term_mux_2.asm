...
; ═══════════════════════════════════════════════════════════════
    ...
    LDI r6, 0xF00
    LDI r2, 0
    STORE r6, r2            ; active_session = 0
    ADD r6, r4
    STORE r6, r2            ; session_0_output_len = 0
    ADD r6, r4
    STORE r6, r2            ; session_1_output_len = 0

    ; Initialize scrollback buffers to zero
    ; NOTE: r2 is CMP result register, so use r9 for zero
    LDI r9, 0
    LDI r11, 0x5000
    LDI r15, 0x7000
init_buffers:
    ...
    LDI r6, 0x4000
    STORE r6, r20           ; RAM[0x4000] = session_0_stdout_read_fd
    ADD r6, r4
    STORE r6, r21           ; RAM[0x4001] = session_0_stdin_write_fd

    ; Store child PID
    LDI r6, 0x4002
    LDI r2, 0
    STORE r6, r2            ; session_0_pid = 0 (not spawned yet)
    ...
    LDI r6, 0x4010
    STORE r6, r22           ; RAM[0x4010] = session_1_stdout_read_fd
    ADD r6, r4
    STORE r6, r23           ; RAM[0x4011] = session_1_stdin_write_fd

    LDI r6, 0x4012
    LDI r2, 0
    STORE r6, r2            ; session_1_pid = 0
    ...
    ; ═══════════════════════════════════════════════════════════════
main_loop:
    IKEY r6                 ; read key from keyboard port
    ...
    ; Get active session
    LDI r15, 0xF00
    LOAD r15, r15            ; r15 = active_session

    ; Get stdin_write_fd for active session
    LDI r10, 0x4001          ; base + 1 for stdin_write_fd
    LDI r3, 16              ; offset between sessions (0x4011 - 0x4001)
    MUL r15, r3
    ADD r10, r15             ; r10 = &session[active].stdin_write_fd
    LOAD r10, r10            ; r10 = stdin_write_fd
...
draw_status_bar:
    ...
    RECTF r2, r8, r12, r5, r0

    ; Draw "S0" indicator (session 0)
    LDI r2, 0xF00
    LOAD r2, r2              ; active_session
    JZ r2, s0_active
...
read_session_output:
    ...
    LDI r6, 0x4000
    LOAD r6, r6            ; r6 = session_0_stdout_read_fd
    LDI r11, 0x4090          ; read buffer
    LDI r15, 1               ; read 1 word
    READ r6, r11, r15

    ; Read from session 1 stdout pipe
    LDI r6, 0x4010
    LOAD r6, r6            ; r6 = session_1_stdout_read_fd

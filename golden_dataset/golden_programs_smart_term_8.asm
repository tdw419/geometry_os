; DESCRIPTION: Render a colored object at the screen.

; smart_term.asm -- Terminal with AI assist
; Type text, press Tab to ask the LLM, response appears on screen.
; Uses LLM opcode (0x9C) to send prompts to external AI.
;
; Memory layout:
;   0x2000-0x20FF: Input buffer (prompt typed by user)
;   0x2100-0x24FF: Response buffer (LLM response, up to 1K chars)
;   0x2500-0x25FF: Status line text
;
; Registers:
;   r12: CMP result (reserved)
;   r4: Key input from IKEY
;   r7: Cursor position (offset into input buffer)
;   r2: Input buffer base (0x2000)
;   r3: Response buffer base (0x2100)
;   r8: Max response length (256)
;   r0: Response length (from LLM opcode)
;   r5: Constant 1
;   r9: Constant 9 (Tab ASCII)
;   r13: Constant 10 (Enter/LF)
;   r10: Constant 8 (Backspace)
;   r15: Constant 27 (Escape)
;   r1: Loop temp
;   r14: Screen Y for text rendering
;   r6: Constant 32 (space)
;   r11: Tick counter for blinking cursor
;   r16: Color for prompt text

; Initialize constants
LDI r2, 0x2000          ; input buffer base
LDI r3, 0x2100          ; response buffer base
LDI r8, 256             ; max response length
LDI r5, 1
LDI r9, 9               ; Tab
LDI r13, 10              ; LF
LDI r10, 8              ; Backspace
LDI r15, 27             ; Escape
LDI r6, 32             ; Space
LDI r11, 0              ; tick counter
LDI r16, 0x00FF00       ; green color for prompt

; Write title bar
LDI r1, 0
CALL draw_title

; Write prompt indicator "> " at top
LDI r1, 0x2000
LDI r14, 0
CALL clear_input

; Write status line
LDI r1, 0x2500
LDI r14, 0x00FFFF       ; cyan
LDI r6, 28             ; screen Y position
CALL write_status

; Main loop
main_loop:
    FILL r12             ; clear screen (black)
    ; Redraw on every frame

    ; Draw title
    CALL draw_title

    ; Draw input prompt "> " in green
    LDI r1, 0x00FF00   ; green
    LDI r14, 2           ; x
    LDI r6, 4           ; y (below title)
    LDI r11, 0x2000      ; input buffer
    CALL draw_text_line

    ; Draw response in white below input
    LDI r1, 0xFFFFFF   ; white
    LDI r14, 2           ; x
    LDI r6, 6           ; y (below prompt)
    LDI r11, 0x2100      ; response buffer
    CALL draw_text_line

    ; Draw status bar
    LDI r1, 0x00FFFF   ; cyan
    LDI r14, 2           ; x
    LDI r6, 28          ; y (bottom area)
    LDI r11, 0x2500      ; status buffer
    CALL draw_text_line

    ; Read keyboard
    IKEY r4
    LDI r1, 0
    ADD r1, r4
    JZ r1, main_loop_continue

    ; Check Escape
    CMP r4, r15
    JZ r12, done

    ; Check Tab (send to LLM)
    CMP r4, r9
    JZ r12, send_to_llm

    ; Check Enter (clear input)
    CMP r4, r13
    JZ r12, handle_enter

    ; Check Backspace
    CMP r4, r10
    JZ r12, handle_backspace

    ; Regular character - store in input buffer
    ; Find end of current input (null terminator)
    LDI r1, 0x2000
    CALL find_end
    ; r1 = address of null terminator
    ; Check buffer not full
    LDI r14, 0x20FF
    CMP r1, r14
    BGE r12, main_loop_continue
    ; Store character
    STORE r1, r4

main_loop_continue:
    FRAME
    JMP main_loop

; --- Send prompt to LLM ---
send_to_llm:
    ; Update status to "Thinking..."
    LDI r1, 0x2500
    LDI r14, 84           ; 'T'
    STORE r1, r14
    LDI r14, 104          ; 'h'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 105          ; 'i'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 110          ; 'n'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 107          ; 'k'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 105          ; 'i'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 110          ; 'n'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 103          ; 'g'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 46           ; '.'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 46           ; '.'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 46           ; '.'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 0            ; null terminate
    ADD r1, r5
    STORE r1, r14

    ; Call LLM opcode: LLM prompt_addr, response_addr, max_len
    LLM r2, r3, r8

    ; r12 = response length
    ; Clear input buffer for next prompt
    LDI r1, 0x2000
    CALL clear_input

    ; Update status
    LDI r1, 0x2500
    LDI r14, 68            ; 'D'
    STORE r1, r14
    LDI r14, 111           ; 'o'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 110           ; 'n'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 101           ; 'e'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 46            ; '.'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 0
    ADD r1, r5
    STORE r1, r14

    JMP main_loop_continue

; --- Handle Enter: clear input and response ---
handle_enter:
    LDI r1, 0x2000
    CALL clear_input
    LDI r1, 0x2100
    CALL clear_input
    JMP main_loop_continue

; --- Handle Backspace ---
handle_backspace:
    LDI r1, 0x2000
    CALL find_end
    ; If at buffer start, ignore
    LDI r14, 0x2000
    CMP r1, r14
    JZ r12, main_loop_continue
    ; Go back one and null-terminate
    SUB r1, r5
    LDI r14, 0
    STORE r1, r14
    JMP main_loop_continue

; --- Draw title bar ---
draw_title:
    PUSH r31
    ; Draw title background
    LDI r1, 0x222244     ; dark purple
    LDI r14, 0
    LDI r6, 0
    LDI r11, 256
    LDI r16, 12
    RECTF r14, r6, r11, r16, r1

    ; Draw "Smart Terminal - Press Tab for AI, Esc to quit"
    ; Use TEXT opcode to render title
    ; Title is stored inline -- write it to RAM first
    LDI r1, 0x2600
    LDI r14, 83           ; 'S'
    STORE r1, r14
    LDI r14, 109          ; 'm'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 97           ; 'a'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 114          ; 'r'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 116          ; 't'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 32           ; ' '
    ADD r1, r5
    STORE r1, r14
    LDI r14, 84           ; 'T'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 101          ; 'e'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 114          ; 'r'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 109          ; 'm'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 0            ; null terminate
    ADD r1, r5
    STORE r1, r14

    ; Render with TEXT opcode
    LDI r1, 0x2600       ; source addr
    LDI r14, 4            ; x
    LDI r6, 2            ; y
    LDI r11, 0xFFFF00     ; yellow
    TEXT r14, r6, r1

    POP r31
    RET

; --- Write status line ---
write_status:
    ; "Tab=AI  Esc=Quit  Enter=Clear"
    PUSH r31
    LDI r1, 0x2500
    LDI r14, 84           ; 'T'
    STORE r1, r14
    LDI r14, 97           ; 'a'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 98           ; 'b'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 61           ; '='
    ADD r1, r5
    STORE r1, r14
    LDI r14, 65           ; 'A'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 73           ; 'I'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 32           ; ' '
    ADD r1, r5
    STORE r1, r14
    LDI r14, 32           ; ' '
    ADD r1, r5
    STORE r1, r14
    LDI r14, 69           ; 'E'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 115          ; 's'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 99           ; 'c'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 61           ; '='
    ADD r1, r5
    STORE r1, r14
    LDI r14, 81           ; 'Q'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 117          ; 'u'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 105          ; 'i'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 116          ; 't'
    ADD r1, r5
    STORE r1, r14
    LDI r14, 0            ; null terminate
    ADD r1, r5
    STORE r1, r14
    POP r31
    RET

; --- Draw text line from RAM buffer ---
; r1 = color, r14 = screen x, r6 = screen y, r11 = RAM addr
draw_text_line:
    PUSH r31
    PUSH r11
    PUSH r1
draw_text_loop:
    LOAD r16, r11
    LDI r1, 0
    CMP r16, r1
    JZ r12, draw_text_done
    PSET r14, r6, r1
    ADD r14, r5
    ADD r11, r5
    JMP draw_text_loop
draw_text_done:
    POP r1
    POP r11
    POP r31
    RET

; --- Find null terminator in buffer ---
; r1 = start address. Returns r1 = address of null.
find_end:
    PUSH r31
    PUSH r5
find_end_loop:
    LOAD r16, r1
    LDI r14, 0
    CMP r16, r14
    JZ r12, find_end_done
    ADD r1, r5
    JMP find_end_loop
find_end_done:
    POP r5
    POP r31
    RET

; --- Clear input buffer (fill with 0) ---
; r1 = buffer start address
clear_input:
    PUSH r31
    PUSH r5
    PUSH r1
    LDI r14, 0
    LDI r16, 256           ; max clear length
clear_loop:
    STORE r1, r14
    ADD r1, r5
    LDI r6, 0
    ADD r6, r1
    LDI r11, 0x2100
    ; Only clear up to response buffer size
    CMP r6, r11
    BGE r12, clear_done
    JMP clear_loop
clear_done:
    POP r1
    POP r5
    POP r31
    RET

done:
    HALT

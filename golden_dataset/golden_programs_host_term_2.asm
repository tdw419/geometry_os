...
mark_row_dirty:
    PUSH r13
    LDI r13, 31
    SUB r13, r9          ; r13 = 31 - row
    LDI r7, 1
    SHL r7, r13          ; r7 = 1 << (31 - row)
    LDI r12, DIRTY_ROWS
    LOAD r10, r12         ; r10 = current bitmap
    OR r10, r7           ; set the bit
    STORE r12, r10
    POP r13
    RET
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r12 = word_index, r9 = bit position within word
...
att_scan_loop:
    ...
    MOV r7, r9           ; r7 = col (will become bit_pos after div)
...
att_check_bit:
    ...
    ADD r20, r12
    LOAD r10, r20
    ; Check if bit r7 is set: shift 1 left by r7, AND
    LDI r8, 1
    LDI r14, 0
att_shift_loop:
    CMP r14, r7
...
apply_sgr:
    ...
    CMPI r6, 0x85
    JNZ r9, sgr_ext95
    CALL sgr_256_to_rgb  ; r9 = RGB from r14 (color index)
    LDI r20, FG_COLOR
    STORE r20, r9
    LDI r20, SGR_EXTENDED
    LDI r9, 0
...
; Clobbers: r9, r13, r10, r8
; Preserves: r14, r6, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r9, 232
    SUB r14, r9        ; r14 = index - 232
    LDI r9, 10
    MUL r14, r9        ; r14 = (index - 232) * 10
    LDI r9, 8
    ADD r14, r9        ; r14 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r14           ; save v
...
sgr256_cube:
    ...
    LDI r13, 8
    SHL r14, r13        ; r14 = G << 8
    OR r9, r14         ; r9 = (R << 16) | (G << 8)
    OR r9, r8         ; r9 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r0, r1        ; r0 = end_col (exclusive)
    CMPI r14, 0
    JZ r9, cel_clear
    CMPI r14, 1
    JNZ r9, cel_mode2
...
ced_check0:
    ...
    LDI r0, COLS
    MUL r11, r0
    ADD r11, r1        ; offset = row*COLS + col

    LDI r6, ROWS
    LDI r3, COLS
    MUL r6, r3         ; total cells
ced_loop0:
    CMP r11, r6
    BGE r9, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r14, 1
ccp_row_set:
    SUB r14, r13         ; convert to 0-based
    CMPI r14, 0
...
cil_count:
    ...
    LDI r11, ROWS
    SUB r11, r13        ; r11 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r11, r6
    BLT r9, cil_clear
...
cdl_clear:
    ...
    LDI r3, ROWS
    SUB r3, r13         ; r3 = ROWS-1
    MOV r11, r3
    SUB r11, r14
    ADD r11, r13        ; r11 = first row to clear (ROWS - count)
    CMP r11, r6
...
cdc_count:
    ...
    MOV r9, r6
    MUL r9, r1
    MOV r6, r9         ; r6 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r11, r3        ; r11 = dst col (starting at cursor)
cdc_shift:
    ADD r9, r11
    ADD r9, r14         ; r11 + count = src col
    LDI r1, COLS
    CMP r9, r1
...
cdc_clear:
    ...
    LDI r11, COLS
    SUB r11, r14        ; r11 = first col to clear
    LDI r1, COLS
    CMP r11, r3
    BGE r9, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r11 < r3 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r11, COLS
    SUB r11, r13        ; r11 = COLS-1 (start from rightmost)
    SUB r11, r14        ; r11 = COLS-1-count (first dst position)
cic_shift:
    CMP r11, r3
    BLT r9, cic_clear
...
render_row:
    ...
    LDI r5, 0          ; x (use r5 to preserve r13)
    MOV r7, r15         ; y = 10 + row*6
    LDI r12, 256         ; w
    LDI r10, 6           ; h

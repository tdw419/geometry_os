...
pb_normal_byte:
    ...
    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r9 & 0x1F) << 6
    LDI r4, 31
    AND r9, r4
    LDI r4, 6
    SHL r9, r4
...
pb_utf8_3_start:
    CMPI r9, 240
    BGE r3, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r9 & 0x0F) << 12
    LDI r4, 15
    AND r9, r4
    LDI r4, 12
    SHL r9, r4
...
pb_esc_restore_cursor:
    CMPI r9, 56 ; '8' = DEC Restore Cursor
    JNZ r3, pb_esc_index
    CALL restore_cursor
    LDI r20, ANSI_STATE
    LDI r3, ANS_NORMAL
...
pb_esc_index:
    CMPI r9, 68 ; 'D' = Index (move down, scroll if at bottom)
...
pb_esc_rev_index:
    CMPI r9, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r3, pb_esc_ris
...
pb_esc_next_line:
    CMPI r9, 69 ; 'E' = Next Line (CR + Index)
    JNZ r3, pb_esc_other
    LDI r20, CUR_COL
    LDI r3, 0
    STORE r20, r3
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r9 - 48)
    LDI r20, CSI_PARAM
    LOAD r3, r20
    LDI r7, 10
    MUL r3, r7
...
mark_row_dirty:
    ...
    SUB r6, r3          ; r6 = 31 - row
    LDI r15, 1
    SHL r15, r6          ; r15 = 1 << (31 - row)
    LDI r8, DIRTY_ROWS
    LOAD r0, r8         ; r0 = current bitmap
    OR r0, r15           ; set the bit
    STORE r8, r0
    POP r6
    RET
...
mark_row_dirty_check:
    ...
    LOAD r0, r8         ; r0 = current bitmap
    MOV r3, r0
    AND r3, r15          ; r3 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r9 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r6, 1
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r8 = word_index, r3 = bit position within word
...
att_scan_loop:
    ...
    MOV r15, r3           ; r15 = col (will become bit_pos after div)
...
apply_sgr:
    ...
    CMPI r4, 0x85
    JNZ r3, sgr_ext95
    CALL sgr_256_to_rgb  ; r3 = RGB from r7 (color index)
    LDI r20, FG_COLOR
    STORE r20, r3
    LDI r20, SGR_EXTENDED
    LDI r3, 0
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r3, 232
    SUB r7, r3        ; r7 = index - 232
    LDI r3, 10
    MUL r7, r3        ; r7 = (index - 232) * 10
    LDI r3, 8
    ADD r7, r3        ; r7 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r7           ; save v
...
sgr256_cube:
    ...
    LDI r6, 8
    SHL r7, r6        ; r7 = G << 8
    OR r3, r7         ; r3 = (R << 16) | (G << 8)
    OR r3, r9         ; r3 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    ; mode 1: 0..col
    ; mode 2: 0..COLS-1
    LDI r13, 0         ; r13 = start_col
    MOV r11, r5        ; r11 = end_col (exclusive)
    CMPI r7, 0
    JZ r3, cel_clear
    CMPI r7, 1
    JNZ r3, cel_mode2
...
ced_check0:
    ...
    LDI r11, COLS
    MUL r13, r11
    ADD r13, r5        ; offset = row*COLS + col

    LDI r4, ROWS
    LDI r14, COLS
    MUL r4, r14         ; total cells
ced_loop0:
    CMP r13, r4
    BGE r3, ced_done
    LDI r20, BUF
...
cil_count:
    ...
    LDI r13, ROWS
    SUB r13, r6        ; r13 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r13, r4
    BLT r3, cil_clear
...
cdl_clear:
    ...
    LDI r14, ROWS
    SUB r14, r6         ; r14 = ROWS-1
    MOV r13, r14
    SUB r13, r7
    ADD r13, r6        ; r13 = first row to clear (ROWS - count)
    CMP r13, r4
...
cdc_count:
    ...
    MOV r3, r4
    MUL r3, r5
    MOV r4, r3         ; r4 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r13, r14        ; r13 = dst col (starting at cursor)
cdc_shift:
    ADD r3, r13
    ADD r3, r7         ; r13 + count = src col
    LDI r5, COLS
    CMP r3, r5
...
cdc_clear:
    ...
    LDI r13, COLS
    SUB r13, r7        ; r13 = first col to clear
    LDI r5, COLS
    CMP r13, r14
    BGE r3, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r13 < r14 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r13, COLS
    SUB r13, r6        ; r13 = COLS-1 (start from rightmost)
    SUB r13, r7        ; r13 = COLS-1-count (first dst position)
cic_shift:
    CMP r13, r14
    BLT r3, cic_clear

    ; dst = BUF + base + r13 + count
    LDI r21, BUF
    MOV r3, r4
    ADD r21, r3
    ADD r21, r13
...
render_row:
    ...
    LDI r6, 1           ; restore r6 (clobbered by mark_row_dirty_check)
    ...
    LDI r2, 0          ; x (use r2 to preserve r6)
    MOV r15, r12         ; y = 10 + row*6
    LDI r8, 256         ; w
    LDI r0, 6           ; h
    LDI r9, 0x0A0A0A    ; bg color
    RECTF r2, r15, r8, r0, r9

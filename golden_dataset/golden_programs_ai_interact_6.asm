; DESCRIPTION: This GeOS assembly code demonstrates programmatic input injection via the AI_INJECT opcode. It injects keystrokes and mouse inputs into the system, reads them back, and visually confirms the operations by drawing colored borders on the screen.

; ai_interact.asm -- AI Agent Input Demo (Phase 89)
; Demonstrates programmatic input injection via AI_INJECT opcode.
; The program injects keystrokes into itself, reads them back, and
; displays the result on screen -- proving AI can drive the GUI.

  ; Set up constants
  LDI r14, 1             ; increment constant

  ; === Phase 1: Inject a key and read it back ===
  LDI r9, 0             ; op=0 (inject key)
  LDI r1, 65            ; keycode = 65 ('A')
  LDI r6, 0             ; shift = 0
  AI_INJECT r9           ; inject 'A' into key buffer

  ; Read the injected key back
  IKEY r10               ; r10 = 65 ('A') from the key buffer

  ; Verify: draw green if correct, red if wrong
  LDI r3, 0xFF00       ; green
  LDI r5, 0xFF0000     ; red
  LDI r13, 65           ; expected value
  CMP r10, r13
  JZ r2, key_ok

  ; Wrong key -- draw red border
  LDI r20, 0
  LDI r21, 0
  LDI r22, 256
  LDI r23, 4
  RECTF r20, r21, r22, r23, r5
  JMP do_mouse

key_ok:
  ; Correct key -- draw green border
  LDI r20, 0
  LDI r21, 0
  LDI r22, 256
  LDI r23, 4
  RECTF r20, r21, r22, r23, r3

do_mouse:
  ; === Phase 2: Inject mouse move ===
  LDI r9, 1             ; op=1 (inject mouse move)
  LDI r1, 128           ; x = 128
  LDI r6, 128           ; y = 128
  AI_INJECT r9           ; inject mouse move

  ; === Phase 3: Inject mouse click ===
  LDI r9, 2             ; op=2 (inject mouse click)
  LDI r1, 100           ; x = 100
  LDI r6, 200           ; y = 200
  LDI r10, 2             ; button = 2 (click)
  AI_INJECT r9           ; inject mouse click

  ; === Phase 4: Inject text string ===
  ; Write "HI" to RAM at 0x2000
  LDI r3, 0x2000
  LDI r5, 72           ; 'H'
  STORE r3, r5
  MOV r13, r3
  ADD r13, r14           ; r13 = 0x2001
  LDI r5, 73           ; 'I'
  STORE r13, r5
  MOV r13, r3
  ADD r13, r14
  ADD r13, r14           ; r13 = 0x2002
  LDI r5, 0
  STORE r13, r5        ; null terminator

  ; Inject the text string
  LDI r9, 3             ; op=3 (inject text string)
  LDI r1, 0x2000        ; addr of null-terminated string
  AI_INJECT r9           ; inject "HI" -- r2 = count (2)

  ; Draw colored bar for each injected char
  LDI r0, 0x0000FF     ; blue
  LDI r3, 10           ; start x
  LDI r5, 10           ; y position
  LDI r13, 10           ; width
  LDI r7, 10           ; height
  LDI r11, 0            ; counter

draw_loop:
  CMP r11, r2            ; compare counter with injected count
  BGE r2, done
  RECTF r3, r5, r13, r7, r0
  LDI r16, 12
  ADD r3, r16           ; move x right
  ADD r11, r14            ; counter++
  JMP draw_loop

done:
  HALT

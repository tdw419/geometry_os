...
STORE r20, r12
ADD r20, r14
STORE r20, r12       ; word2 (cols 64-79): only bits 0,8 used (cols 64,72)
...
pb_normal_byte:
    ...
    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r10 & 0x1F) << 6
    LDI r2, 31
    AND r10, r2
    LDI r2, 6
    SHL r10, r2
...
pb_utf8_3_start:
    CMPI r10, 240
    BGE r11, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r10 & 0x0F) << 12
    LDI r2, 15
    AND r10, r2
    LDI r2, 12
    SHL r10, r2
...
pb_esc_restore_cursor:
    CMPI r10, 56 ; '8' = DEC Restore Cursor
...
pb_esc_index:
    CMPI r10, 68 ; 'D' = Index (move down, scroll if at bottom)
...
pb_esc_rev_index:
    CMPI r10, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r11, pb_esc_ris
...
pb_esc_next_line:
    CMPI r10, 69 ; 'E' = Next Line (CR + Index)
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r10 - 48)
    LDI r20, CSI_PARAM
    LOAD r11, r20
    LDI r12, 10
    MUL r11, r12
...
mark_row_dirty:
    PUSH r14
    LDI r14, 31
    SUB r14, r11          ; r14 = 31 - row
    LDI r9, 1
    SHL r9, r14          ; r9 = 1 << (31 - row)
    LDI r6, DIRTY_ROWS
    LOAD r3, r6         ; r3 = current bitmap
    OR r3, r9           ; set the bit
    STORE r6, r3
    POP r14
    RET
...
mark_row_dirty_check:
    ...
    LOAD r3, r6         ; r3 = current bitmap
    MOV r11, r3
    AND r11, r9          ; r11 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r10 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r14, 1
...
att_scan_loop:
    ...
    MOV r9, r11           ; r9 = col (will become bit_pos after div)
...
apply_sgr:
    ...
    CMPI r2, 0x85
    JNZ r11, sgr_ext95
    CALL sgr_256_to_rgb  ; r11 = RGB from r12 (color index)
    LDI r20, FG_COLOR
    STORE r20, r11
    LDI r20, SGR_EXTENDED
    LDI r11, 0
...
sgr_existing:
    ...
    ; r12 already loaded with SGR code from above
...
; Input:  r12 = color index (0-255)
; Output: r11 = RGB color value
; Clobbers: r11, r14, r3, r10
; Preserves: r12, r2, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r11, 232
    SUB r12, r11        ; r12 = index - 232
    LDI r11, 10
    MUL r12, r11        ; r12 = (index - 232) * 10
    LDI r11, 8
    ADD r12, r11        ; r12 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r12           ; save v
    LDI r14, 16
    MOV r11, r12
    SHL r11, r14        ; r11 = v << 16
    PUSH r11           ; save high bits
    LDI r14, 8
    MOV r11, r12
    SHL r11, r14        ; r11 = v << 8
    OR r11, r12         ; r11 = (v << 8) | v
    POP r14            ; r14 = v << 16
    OR r11, r14         ; r11 = (v << 16) | (v << 8) | v
    POP r12            ; restore r12
    JMP sgr256_ret
...
sgr256_cube:
    ...
    MOV r11, r14
    LDI r12, 16
    SHL r11, r12        ; r11 = R << 16
    MOV r12, r3
    LDI r14, 8
    SHL r12, r14        ; r12 = G << 8
    OR r11, r12         ; r11 = (R << 16) | (G << 8)
    OR r11, r10         ; r11 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    LDI r20, CSI_PARAM
    LOAD r12, r20       ; r12 = mode (0, 1, or 2)

    ; Get current row
    LDI r20, CUR_ROW
    LOAD r2, r20       ; r2 = row
    LDI r1, COLS
    MUL r2, r1         ; r2 = row * COLS
    ...
    MOV r7, r13        ; r7 = end_col (exclusive)
    CMPI r12, 0
    JZ r11, cel_clear
    CMPI r12, 1
    JNZ r11, cel_mode2
...
ced_check0:
    ...
    LDI r7, COLS
    MUL r0, r7
    ADD r0, r13        ; offset = row*COLS + col

    LDI r2, ROWS
    LDI r1, COLS
    MUL r2, r1         ; total cells
ced_loop0:
    CMP r0, r2
    BGE r11, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r12, 1
ccp_row_set:
    SUB r12, r14         ; convert to 0-based
    CMPI r12, 0
...
cil_count:
    ...
    LDI r0, ROWS
    SUB r0, r14        ; r0 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r0, r2
    BLT r11, cil_clear
...
cdl_clear:
    ...
    LDI r1, ROWS
    SUB r1, r14         ; r1 = ROWS-1
    MOV r0, r1
    SUB r0, r12
    ADD r0, r14        ; r0 = first row to clear (ROWS - count)
    CMP r0, r2
...
cdc_count:
    ...
    MOV r0, r1        ; r0 = dst col (starting at cursor)
cdc_shift:
    ADD r11, r0
    ADD r11, r12         ; r0 + count = src col
    LDI r13, COLS
    CMP r11, r13
...
cdc_clear:
    ...
    LDI r0, COLS
    SUB r0, r12        ; r0 = first col to clear
    LDI r13, COLS
    CMP r0, r1
    BGE r11, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r0 < r1 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r0, COLS
    SUB r0, r14        ; r0 = COLS-1 (start from rightmost)
    SUB r0, r12        ; r0 = COLS-1-count (first dst position)
cic_shift:
    CMP r0, r1
    BLT r11, cic_clear
...
render_row:
    ...
    LDI r14, 1           ; restore r14 (clobbered by mark_row_dirty_check)
    ...
    LDI r15, 0          ; x (use r15 to preserve r14)
    MOV r9, r5         ; y = 10 + row*6
    LDI r6, 256         ; w
    LDI r3, 6           ; h
    LDI r10, 0x0A0A0A    ; bg color
    RECTF r15, r9, r6, r3, r10

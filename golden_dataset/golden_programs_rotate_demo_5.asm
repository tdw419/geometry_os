...
; ── Register plan ──────────────────────────────────────────────
;   r12  = sprite x position (top-left of rendered area)
;   r5  = sprite y position (top-left of rendered area)
;   r14  = base sprite size (32x32)
;   r2  = current scale factor (32 = 1x, 64 = 2x, etc.)
...
;   r13 = color (blue)
;   r3 = color (yellow)
;   r9 = source x (sprite drawn at fixed position)
;   r6 = source y
;   r10 = dest x (scaled output position)
;   r16 = dest y

; ── init ──────────────────────────────────────────────────────
LDI r12, 0        ; sprite screen x (computed each frame)
LDI r5, 0        ; sprite screen y (computed each frame)
LDI r14, 32       ; base sprite size
LDI r2, 32       ; scale = 1x (32x32 output = 32x32 source)
LDI r1, 0        ; angle = 0
LDI r8, 0xFF0000 ; red
LDI r4, 0        ; black
...
LDI r9, 0       ; source x (sprite drawn at top-left)
LDI r6, 0       ; source y
...
angle_inc:
  LDI r0, 64
  ADD r1, r0        ; +64 fixed-point (~14 degrees)
  JMP frame_done

angle_dec:
  LDI r0, 64
  SUB r1, r0        ; -64 fixed-point (~14 degrees)
  JMP frame_done

scale_up:
  LDI r0, 8
  ADD r2, r0        ; increase scale
  ; Clamp: max 128
  LDI r0, 128
  CMP r2, r0
  BLT r15, frame_done

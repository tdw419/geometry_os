...
1. **Main Loop (`main_loop`)**:
   - Handles frame updates, input processing, rendering, and command execution.
   - Updates the player's position based on keyboard input (arrow keys).
   - Calls entity update functions to move entities around.
   - Renders various game elements including terrain, buildings, entities, and UI components.

2. **Rendering**:
   - Terrain is generated using a deterministic random number generator (`rand()`) to create a pseudo-random world.
   - Entities are drawn on the screen with different visual representations based on their type (e.g., program nodes, agents).
   - A minimap is displayed in the top-left corner showing building locations and entity markers.
   - UI elements like command bars and tooltips are rendered at the bottom of the screen.

3. **Command Parsing**:
   - The program allows users to input commands via a buffer (`CMD_BUF`).
   - Commands can be executed using the `cmd_execute` function, which parses and processes commands.
   - Supported commands include teleporting the player (`/tp X Y`) and building new structures (`/build NAME`).

4. **Entity Management**:
   - Entities are stored in a table at `0x7900`, with each entity having fields for position, type, direction seed, and animation frame.
   - The program includes functions to update entity positions and handle interactions (e.g., entities moving towards their targets or fleeing).

5. **Biome Check**:
   - A subroutine (`check_biome_walkable`) determines if a given world position is walkable by checking the biome type against predefined rules.

### Detailed Breakdown

- **Terrain Rendering**:
  - The terrain is generated using a random number generator and a deterministic algorithm to create a consistent world map.
  - Each tile on the map has a biome type, which affects rendering and entity behavior.

- **Entity Movement**:
  - Entities move towards their targets or randomly within a range (`ent_update_program`, `ent_update_agent`).
  - Agents have a direction indicator shown above them to indicate movement direction.

- **Command Execution**:
  - Commands are parsed and executed based on the first character of the input.
  - Supported commands include teleportation (`/tp`) and building creation (`/build`).

### Example Usage

- **Teleport Command**: `/tp 32 32` would move the player to world coordinates (32, 32).
- **Build Command**: `/build MyBuilding` would create a new building at the player's current position with the name "MyBuilding".

### Conclusion

This assembly program demonstrates a basic game loop structure, entity management, rendering, and command parsing. It provides a foundation for more complex simulations or games, showcasing how to implement these features in an assembly language environment.
...
LDI r8, 0xFFB          ; key bitmask port
LDI r0, 0x7800         ; camera_x address
LDI r14, 0x7801         ; camera_y address
...
zoom_0:
LDI r9, 256              ; TILES per axis (zoomed out: see 256 tiles)
LDI r3, 1                ; TILE_SIZE = 1 pixel
LDI r30, 0               ; detail_level = 0 (minimal: flat tiles only)
JMP zoom_set

zoom_1:
LDI r9, 128              ; TILES per axis (medium: 128 tiles)
LDI r3, 2                ; TILE_SIZE = 2 pixels
LDI r30, 1               ; detail_level = 1 (reduced: patterns + shimmer, no trees/contours)
JMP zoom_set

zoom_default:
LDI r9, 64               ; TILES per axis (standard: 64 tiles)
LDI r3, 4                ; TILE_SIZE = 4 pixels
...
LDI r18, 0x7808
LOAD r2, r18           ; r2 = player_x
LDI r18, 0x7809
LOAD r11, r18           ; r11 = player_y

; --- Read key bitmask ---
LOAD r16, r8           ; r16 = key bitmask
...
LOAD r18, r17                 ; r18 = CMD_LEN
CMPI r18, 63
BGE r15, cmd_type_no_key       ; buffer full
LDI r19, 0x7832
...
MOV r17, r11
SUB r17, r10             ; target_y = player_y - 1
LDI r18, 0x7809
STORE r18, r17          ; player_y = target_y (move will be reverted if blocked)
; Check biome at (player_x, target_y)
MOV r4, r2             ; world_x = player_x
MOV r6, r17             ; world_y = target_y
CALL check_biome_walkable
JZ r15, up_blocked       ; r15 == 0 means not walkable
JMP no_up
up_blocked:
LDI r18, 0x7809
STORE r18, r11          ; revert player_y to original
no_up:
...
STORE r17, r18          ; facing = down
MOV r17, r11
ADD r17, r10             ; target_y = player_y + 1
LDI r18, 0x7809
STORE r18, r17
MOV r4, r2
MOV r6, r17
...
MOV r17, r2
SUB r17, r10             ; target_x = player_x - 1
LDI r18, 0x7808
STORE r18, r17
MOV r4, r17             ; world_x = target_x
MOV r6, r11             ; world_y = player_y
CALL check_biome_walkable
JZ r15, left_blocked
JMP no_left
left_blocked:
LDI r18, 0x7808
STORE r18, r2          ; revert player_x to original
no_left:
...
MOV r17, r2
ADD r17, r10             ; target_x = player_x + 1
LDI r18, 0x7808
STORE r18, r17
MOV r4, r17
MOV r6, r11
CALL check_biome_walkable
JZ r15, right_blocked
JMP no_right
right_blocked:
LDI r18, 0x7808
MOV r17, r2
...
ent_update_loop:
  CMP r26, r21
  BGE r15, ent_update_done

  ; Load entity fields
  MOV r22, r20
  LOAD r4, r22             ; world_x (r4 for biome check)
  ADDI r22, 1
  LOAD r6, r22             ; world_y (r6 for biome check)
  ADDI r22, 1
  LOAD r17, r22            ; type
  ADDI r22, 1
...
ent_not_program:
  ...
  LDI r22, 1
  CMP r17, r22
  JZ r15, ent_ai_wander     ; wanderer
  LDI r22, 2
  CMP r17, r22
  JZ r15, ent_ai_guard      ; guard
  LDI r22, 3
  CMP r17, r22
  JZ r15, ent_ai_animal     ; animal (flee)
  LDI r22, 4
  CMP r17, r22
  JZ r15, ent_ai_ghost      ; ghost
  LDI r22, 5
  CMP r17, r22
  JZ r15, ent_ai_area_agent ; area_agent
...
ent_agent_not_idle:
  LDI r22, 1
  CMP r17, r22
  JZ r15, ent_agent_state_moving
  LDI r22, 3
  CMP r17, r22
  JZ r15, ent_agent_state_inside
  LDI r22, 5
  CMP r17, r22
  JZ r15, ent_agent_state_returning
...
ent_agent_state_moving:
  ...
  MOV r22, r23
  SUB r22, r4               ; dx = target_x - entity_x
  MOV r17, r24
  SUB r17, r6               ; dy = target_y - entity_y
  ...
  LDI r23, 2
  CMP r22, r23
  BLT r15, ent_agent_check_dy  ; dx < 2, check dy
  JMP ent_agent_move_dx       ; dx >= 2, move
ent_agent_dx_neg:
  NEG r22                   ; |dx|
  LDI r23, 2
  CMP r22, r23
  BLT r15, ent_agent_check_dy  ; |dx| < 2
...
ent_agent_check_dy:
  ...
  LDI r23, 2
  CMP r17, r23
  BLT r15, ent_agent_arrived   ; dy < 2, arrived!
...
ent_agent_arrived:
  ...
  LDI r23, 4
  CMP r17, r23
  BGE r15, ent_agent_arrived_done  ; mailbox full
  ADDI r17, 1
...
ent_agent_move_dy:
  ...
  SUB r22, r6               ; dy = target_y - entity_y
  LDI r23, 31
  MOV r25, r22
  SAR r25, r23
...
ent_agent_state_returning:
  ...
  MOV r22, r23
  SUB r22, r4               ; dx = home_x - entity_x
  MOV r17, r24
  SUB r17, r6               ; dy = home_y - entity_y
  ...
  LDI r24, 2
  CMP r22, r24
  BLT r15, ent_agent_home_check_dy
...
ent_agent_home_check_dy:
  ...
  LDI r24, 2
  CMP r17, r24
  BLT r15, ent_agent_home_arrived
...
ent_compute_target:
  ...
  MOV r22, r4              ; target_x = world_x
  MOV r23, r6              ; target_y = world_y
  JNZ r25, ent_not_up
  SUB r23, r10              ; target_y -= 1
  JMP ent_do_biome_check
ent_not_up:
  LDI r24, 1
  CMP r25, r24
  JNZ r15, ent_not_down
  ADD r23, r10              ; target_y += 1
  JMP ent_do_biome_check
ent_not_down:
  LDI r24, 2
  CMP r25, r24
  JNZ r15, ent_not_left
  SUB r22, r10              ; target_x -= 1
  JMP ent_do_biome_check
ent_not_left:
  ADD r22, r10              ; target_x += 1 (right)
ent_do_biome_check:

  ; Check biome walkability (all types except ghost)
  MOV r4, r22              ; target coords for biome check
  MOV r6, r23
  CALL check_biome_walkable
  JZ r15, ent_move_blocked  ; not walkable, skip move
...
ent_nt_up2:
  LDI r24, 1
  CMP r25, r24
  JNZ r15, ent_nt_down2
  ADD r23, r10
  JMP ent_do_move_no_check
ent_nt_down2:
  LDI r24, 2
  CMP r25, r24
  JNZ r15, ent_nt_left2
  SUB r22, r10
  JMP ent_do_move_no_check
ent_nt_left2:
  ADD r22, r10
...
; r9 = tiles per axis, so center offset = r9 / 2
MOV r17, r9
LDI r18, 1
...
MOV r18, r11
SUB r18, r17
STORE r14, r18            ; camera_y = player_y - tiles/2
...
ypre_done:
  ...
  render_x:
    ; World coordinates
    MOV r4, r2
    ADD r4, r12           ; r4 = world_x
    MOV r6, r11
    ADD r6, r1           ; r6 = world_y
...
no_claim:
    ...
    MOV r5, r6
    LDI r18, 3
    SHR r7, r18          ; r7 = world_x >> 3
    SHR r5, r18          ; r5 = world_y >> 3
    LDI r18, 99001
    MUL r7, r18          ; r7 = x_hash
    LDI r18, 79007
    MUL r5, r18          ; r5 = y_hash
    XOR r7, r5           ; r7 = coarse_hash
    LDI r18, 1103515245
    MUL r7, r18          ; r7 = mixed_hash
...
no_yblend:
    ...
    ; r5 = world_x * 374761393 XOR world_y * 668265263
    ...
    LDI r18, 668265263
    MUL r21, r18
    XOR r5, r21           ; r5 = fine_hash (THE SEED, 32 bits of goodness)
...
water_checked:
    ...
    ADD r20, r4              ; fc + world_x (cheap position variation)
...
no_shimmer:
    ...
    MOV r18, r4
    SUB r18, r10              ; r18 = world_x - 1 (left neighbor)
    MOV r21, r18
    LDI r18, 3
...
pat_horiz:
    ...
    RECTF r28, r27, r3, r20, r17
    MOV r21, r27
    ADD r21, r20           ; r21 = y + 2
    RECTF r28, r21, r3, r20, r19
    JMP tile_done
...
tile_done:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r15, contour_clr_skip   ; detail < 2 → skip
    ...
    LDI r18, 28
    SHR r7, r18
    ANDI r7, 7              ; r7 = current_elevation (0-7)
...
contour_r_draw:
    ...
    MOV r18, r28
    ADD r18, r10
    ADD r18, r10               ; x = screen_x + 2 = sx + 2
    RECTF r18, r27, r10, r3, r17
...
contour_b_draw:
    ...
    MOV r18, r27
    ADD r18, r10
    ADD r18, r10               ; y = screen_y + 2 = sy + 2
    RECTF r28, r18, r3, r10, r17

contour_done:
    ; Store current elevation for next tile's right-edge contour check
    LDI r18, 0x7807
    STORE r18, r7             ; RAM[0x7807] = current_elevation
    JMP contour_after
...
contour_after:
    ...
    LDI r17, 2
    CMP r18, r17
    BLT r15, no_tree        ; detail < 2 → skip trees
...
sky_night:
  LDI r7, 0x050510       ; top: near-black with hint of blue
  LDI r5, 0x0A0A30       ; horizon: dark navy
  JMP sky_draw

sky_dawn:
  LDI r7, 0x101040       ; top: deep blue-purple
  LDI r5, 0xCC6600       ; horizon: warm orange
  JMP sky_draw

sky_day:
  LDI r7, 0x1844AA       ; top: medium blue
  LDI r5, 0x5599DD       ; horizon: light sky blue
  JMP sky_draw

sky_dusk:
  LDI r7, 0x0C0820       ; top: dark purple
  LDI r5, 0xDD4400       ; horizon: deep orange-red
  JMP sky_draw
...
LDI r18, 256
LDI r19, 4
RECTF r4, r6, r18, r19, r7

; Band 1 (rows 4-7): top + (horizon >> 2) = 75% top + 25% horizon
MOV r17, r5
LDI r18, 2
SHR r17, r18            ; horizon >> 2
ADD r17, r7             ; top + (horizon >> 2)
LDI r18, 256            ; width
LDI r6, 4
RECTF r4, r6, r18, r19, r17
...
LDI r17, 2
CMP r18, r17
BLT r15, mm_skip          ; detail < 2 → skip minimap
...
mm_y:
  ...
  mm_x:
    ...
    LDI r18, 2
    MUL r4, r18
    ADD r4, r2          ; r4 = camera_x + mx*2

    MOV r6, r1
    LDI r18, 2
    MUL r6, r18
    ADD r6, r11          ; r6 = camera_y + my*2

    ; Coarse hash for biome (same hash as main terrain)
    MOV r7, r4
    LDI r18, 3
    SHR r7, r18          ; world_x >> 3
    LDI r18, 99001
    MUL r7, r18          ; x_hash

    MOV r5, r6
    LDI r18, 3
    SHR r5, r18          ; world_y >> 3
    LDI r18, 79007
    MUL r5, r18          ; y_hash

    XOR r7, r5           ; coarse_hash
    LDI r18, 1103515245
    MUL r7, r18          ; mixed_hash
    LDI r18, 27
    SHR r7, r18          ; biome index (0..31)
...
LDI r4, 224
LDI r6, 0
RECTF r4, r6, r19, r18, r17    ; top edge
LDI r6, 31
RECTF r4, r6, r19, r18, r17    ; bottom edge
LDI r6, 0
RECTF r4, r6, r18, r19, r17    ; left edge
LDI r4, 255
RECTF r4, r6, r18, r19, r17    ; right edge
...
LDI r17, 2
CMP r18, r17
BLT r15, player_simple   ; detail < 2 → simple marker
...
LDI r17, 2
CMP r18, r17
BLT r15, bldg_simple_dots   ; detail < 2 → simple dots
...
bldg_dot_loop:
  MOV r22, r20
  LOAD r4, r22          ; bldg world_x
  ADDI r22, 1
  LOAD r6, r22          ; bldg world_y
  ADDI r22, 1
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r15, bldg_dot_next
  MOV r29, r9
  SUB r29, r10           ; tiles - 1
  CMP r28, r29
  BGE r15, bldg_dot_next
  ...
  LOAD r27, r18
  SUB r28, r27
  MUL r28, r3           ; screen_x = dx * tile_size

  MOV r29, r6
  LDI r18, 0x7801
  LOAD r27, r18
  SUB r29, r27
  MUL r29, r3           ; screen_y = dy * tile_size
...
bldg_loop:
  ...
  LDI r29, 0
  CMP r28, r29
  BLT r15, bldg_next     ; off-screen left
  LDI r29, 62
  CMP r28, r29
  BGE r15, bldg_next     ; off-screen right
  ...
  LDI r18, 0
  CMP r29, r18
  BLT r15, bldg_next     ; off-screen top
  LDI r18, 58
  CMP r29, r18
  BGE r15, bldg_next     ; off-screen bottom
...
LDI r18, 0x7800
LOAD r4, r18             ; camera_x
LDI r19, 0x7801
LOAD r6, r19             ; camera_y

; Compute screen pixel for world (32,32)
LDI r17, 32
SUB r17, r4
MUL r17, r3              ; px_x = (32 - cam_x) * tile_size
MOV r18, r17             ; save px_x

LDI r17, 32
SUB r17, r6
MUL r17, r3              ; px_y = (32 - cam_y) * tile_size
...
ent_render_loop:
  ...
  LDI r25, 0
  CMP r27, r25
  BLT r15, ent_render_next
  MOV r25, r9
  SUB r25, r10              ; tiles - 1
  CMP r27, r25
  BGE r15, ent_render_next
  ...
  MUL r27, r3              ; screen_x = dx * tile_size
  MUL r28, r3              ; screen_y = dy * tile_size
...
ent_render_not_program:
  ...
  LDI r25, 1
  CMP r17, r25
  JZ r15, ent_render_agent   ; wanderer

  LDI r25, 2
  CMP r17, r25
  JZ r15, ent_render_guard   ; guard

  LDI r25, 3
  CMP r17, r25
  JZ r15, ent_render_animal  ; animal

  LDI r25, 4
  CMP r17, r25
  JZ r15, ent_render_ghost   ; ghost

  LDI r25, 5
  CMP r17, r25
  JZ r15, ent_render_area_agent ; area_agent
...
ent_agent_color:
  ...
  MOV r25, r18
  LDI r5, 16
  SHR r25, r5         ; direction
  LDI r5, 0
  CMP r25, r5
  JNZ r15, ent_agent_not_up2
  LDI r25, 0
  ADD r25, r27             ; dx=0
  SUB r28, r10              ; dy=-1
  LDI r5, 0xFFFFFF
  PSET r25, r28, r5   ; white dot above
  ADDI r28, 1              ; restore y
  JMP ent_agent_dir_done
ent_agent_not_up2:
  LDI r5, 1
  CMP r25, r5
  JNZ r15, ent_agent_not_down2
  LDI r25, 0
  ADD r25, r27
  ADD r28, r10              ; dy=+1
  LDI r5, 0xFFFFFF
...
ent_animal_color:
  ...
  PSET r27, r28, r5        ; left ear
  ADDI r27, 1
  PSET r27, r28, r5        ; right ear
  JMP ent_render_next
...
ent_ghost_visible:
  ...
  MOV r25, r19
  LDI r5, 3
  AND r25, r5              ; 0-3 phase
  LDI r17, 0x9933FF        ; purple base
  LDI r5, 1
  CMP r25, r5
  JNZ r15, ent_ghost_not_p2
  LDI r17, 0xBB66FF        ; light purple
  JMP ent_ghost_draw
ent_ghost_not_p2:
  LDI r5, 2
  CMP r25, r5
  JNZ r15, ent_ghost_not_p3
  LDI r17, 0x7722CC        ; dark purple
  JMP ent_ghost_draw
ent_ghost_not_p3:
  LDI r5, 3
  CMP r25, r5
  JNZ r15, ent_ghost_draw
...
ent_ghost_draw:
  ...
  SUBI r28, 2              ; eye row
  LDI r5, 0xFFFFFF
  PSET r27, r28, r5        ; center eye
  SUBI r27, 1
  PSET r27, r28, r5        ; left eye
  ADDI r27, 2
  PSET r27, r28, r5        ; right eye
  JMP ent_render_next
...
mm_ent_not_program:
  LDI r25, 1
  CMP r17, r25
  JNZ r15, mm_ent_not_wanderer
  LDI r18, 0xFF8800        ; orange (wanderer)
  JMP mm_ent_dot
mm_ent_not_wanderer:
  LDI r25, 2
  CMP r17, r25
  JNZ r15, mm_ent_not_guard
  LDI r18, 0xFF2020        ; red (guard)
  JMP mm_ent_dot
mm_ent_not_guard:
  LDI r25, 3
  CMP r17, r25
  JNZ r15, mm_ent_not_animal
...
LDI r18, 0xFFFFFFFF
CMP r17, r18
JZ r15, no_entity_tooltip   ; -1 = no entity nearby
...
LDI r18, 1
CMP r17, r18
JZ r15, ent_tooltip_wanderer
LDI r18, 2
CMP r17, r18
JZ r15, ent_tooltip_guard
LDI r18, 3
CMP r17, r18
JZ r15, ent_tooltip_animal
...
LDI r17, 2
CMP r18, r17
BGE r15, player_marker_done   ; detail >= 2 → skip (full sprite already drawn)
...
LDI r4, 128
LDI r6, 128
PSET r4, r6, r17            ; center dot
LDI r4, 127
PSET r4, r6, r17            ; left
LDI r4, 129
LDI r6, 128
PSET r4, r6, r17            ; right
LDI r4, 128
LDI r6, 127
PSET r4, r6, r17            ; up
LDI r6, 129
...
LOAD r18, r17
CMPI r18, 47               ; '/'
JNZ r15, cmd_oracle          ; not a / command → send to Oracle
...
CMPI r18, 116               ; 't'
JNZ r15, cmd_try_build
ADDI r17, 1
LOAD r18, r17
CMPI r18, 112               ; 'p'
JNZ r15, cmd_oracle          ; not /tp, fall through to oracle

; Parse first number (x) after /tp
ADDI r17, 1                  ; skip 'p'
CALL parse_next_number       ; r15 = number, r17 advanced past it
LDI r19, 0x7808
STORE r19, r15                ; player_x = parsed x

; Parse second number (y)
CALL parse_next_number
LDI r19, 0x7809
STORE r19, r15                ; player_y = parsed y
...
CMPI r18, 32
BGE r15, cmd_build_full       ; max 32 buildings
...
LDI r4, 0x7832              ; prompt = CMD_BUF text
LDI r6, 0x7873              ; response buffer
LDI r7, 895                 ; max response length
LLM r4, r6, r7
...
; Output: r15 = parsed number, r17 = advanced past number + space
; Uses r12 as accumulator (r15 is clobbered by CMPI!)
; =========================================
parse_next_number:
PUSH r31
LDI r1, 1
...
LOAD r18, r17
CMPI r18, 32                ; space
JNZ r15, pnn_digit_start
ADDI r17, 1
JMP pnn_skip

pnn_digit_start:
LDI r12, 0                   ; accumulator in r12 (NOT r15, CMPI clobbers r15)
...
pnn_done:
MOV r15, r12                  ; move result to r15 for caller
POP r31
RET

; ===== check_biome_walkable subroutine =====
; Input: r4 = world_x, r6 = world_y
; Output: r15 = 1 (walkable) or 0 (blocked)
; Preserves: r1-r5, r2-r11
; Uses: r17-r22 as temporaries
; Clobbers: r15 (CMP result), r31 (saved/restored via push/pop)
...
LDI r17, 2
CMP r21, r17
BLT r15, biome_blocked

; Mountain: biome 13-14 (13 <= biome < 15)
LDI r17, 13
CMP r21, r17
BLT r15, biome_not_mt
LDI r17, 15
CMP r21, r17
BGE r15, biome_not_mt
JMP biome_blocked
...
LDI r17, 16
CMP r21, r17
BLT r15, biome_not_lava
LDI r17, 19
CMP r21, r17
BGE r15, biome_not_lava
JMP biome_blocked

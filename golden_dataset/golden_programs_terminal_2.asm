...
LDI r6, 0x333355  ; bg = match title bar
DRAWTEXT r2, r7, r9, r1, r6
...
LDI r9, 36
LDI r1, 16
HITSET r2, r7, r9, r1, 99

; Restore r2 = 1 before writing prompt!
LDI r2, 1
...
copy_col:
    ...
    ; DRAWTEXT x=0, y=r10, addr=SCRATCH, fg=white, bg=0 (transparent)
    LDI r2, 0
    LDI r13, SCRATCH
    LDI r8, 0xCCCCCC  ; fg = light gray
    LDI r14, 0         ; bg = transparent
    DRAWTEXT r2, r10, r13, r8, r14
...
draw_cursor:
    ...
    MUL r7, r15           ; row * 8
    LDI r9, 16
    ADD r7, r9           ; y = 16 + row*8
    LDI r9, 6
    LDI r1, 8
    LDI r6, 0x44FF44
    RECTF r4, r7, r9, r1, r6
...
do_char:
    ...
    LOAD r7, r20
    LDI r9, COLS
    MUL r7, r9           ; r7 = row * COLS
    LDI r20, CUR_COL
    LOAD r4, r20
    ADD r7, r4           ; r7 = row*COLS + col
    LDI r20, BUF
...
extract_cmd:
    ...
    LOAD r3, r20          ; r3 = row
    LDI r5, COLS
    MUL r3, r5            ; r3 = row * COLS

    ; Source starts at col 2 (skip "$ ")
    LDI r20, BUF
    ADD r20, r3           ; r20 = BUF + row*COLS
    ADD r20, r2
    ADD r20, r2           ; r20 = BUF + row*COLS + 2
...
ec_loop:
    ...
    ADD r20, r2           ; advance source
    ADD r21, r2           ; advance dest
    ADD r22, r2           ; col++
    JMP ec_loop
...
ls_copied:
    ADD r21, r2           ; skip null terminator
    CALL do_newline
    JMP ls_next
...
write_unknown:
    ...
    MUL r3, r5
    LDI r20, BUF
    ADD r20, r3           ; r20 = BUF + row*COLS (destination)
...
cmd_cat:
    ...
    ADD r21, r2
    ADD r21, r2
    ADD r21, r2           ; r21 = SCRATCH+4 (filename)
    ...
    MOV r18, r4           ; r18 = fd
    CMPI r4, 0xFFFFFFFF
    JNZ r4, cat_read      ; if not -1, file opened OK
...
cat_read:
    ...
    READ r18, r22, r23
    ; r4 = bytes read (0 = EOF, 0xFFFFFFFF = error)
    CMPI r4, 0
    JZ r4, cat_close      ; EOF or empty
    CMPI r4, 0xFFFFFFFF
    JNZ r4, cat_close     ; error
...
cat_write_loop:
    ...
    BGE r4, cat_close
    LOAD r4, r22
    JZ r4, cat_close      ; null terminator
...
cat_write_char:
    ; Write r4 (char) at current cursor position in buffer
    ...
    ADD r9, r2
    CMPI r9, COLS
    JNZ r4, cwc_done      ; branch if not equal (r4 = CMPI result)
    CALL do_newline
    JMP cwc_ret
cwc_done:
    LDI r20, CUR_COL
...
cmd_uptime:
    ...
    ADD r20, r2
    ADD r20, r2
    ADD r20, r2           ; advance past "uptime: "
    STRO r20, "running"
    CALL do_newline
    JMP dc_ret
...
cmd_run:
    ...
    ADD r21, r2       ; +2
    ADD r21, r2       ; +3
    ADD r21, r2       ; +4 = SCRATCH+4 (start of filename)
...
run_copy:
    ...
    ADD r21, r2
    ADD r22, r2
    JZ r4, run_exec   ; stop at null terminator

run_exec:
    ; EXEC takes a register pointing to the filename
    LDI r2, 0x5300
    EXEC r2            ; r4 = PID or 0xFFFFFFFF on error
    CMPI r4, 0xFFFFFFFF
    JZ r4, run_err
...
run_wait:
    WAITPID r7         ; r4 = 0 still running, 1 done
    JZ r4, run_wait
...
cmd_edit:
    ...
    ADD r21, r2       ; +3
    ADD r21, r2       ; +4
    ADD r21, r2       ; +5 = SCRATCH+5 (start of filename)
    LDI r22, 0x5300   ; dest buffer
edit_copy:
    LOAD r4, r21
    STORE r22, r4
    ADD r21, r2
    ADD r22, r2
    JZ r4, edit_fire   ; stop at null terminator
...
cmd_build:
    ...
    LDI r4, 47         ; '/'
    STORE r22, r4
    ADD r22, r2        ; r22 = 0x5309, pointing after "programs/"
    ...
    ADD r21, r2       ; +4
    ADD r21, r2       ; +5
    ADD r21, r2       ; +6 = SCRATCH+6 (start of filename)
...
build_open:
    ...
    LDI r21, 0x5300
    LDI r20, 0
    OPEN r21, r20, r20  ; fd in r4
    MOV r18, r4         ; r18 = fd
    CMPI r4, 0xFFFFFFFF
    JZ r4, build_fopen_err
    ...
    READ r18, r22, r23  ; r4 = bytes read
    MOV r19, r4         ; r19 = bytes read
...
build_pack:
    ...
    JZ r4, build_asm
    LOAD r4, r21
    STORE r22, r4       ; store the word (low byte has the char)
    ADD r21, r2
    ADD r22, r2
    SUB r24, r2
    JMP build_pack
...
do_newline:
    ...
    STORE r20, r4
    LDI r20, CUR_ROW
    LOAD r3, r20          ; r3 = current row
    ADD r3, r2            ; r3 = new row
    CMPI r3, ROWS
    BLT r4, dn_store      ; branch if new row < ROWS (r4 = CMPI result)

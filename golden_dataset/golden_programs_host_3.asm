...
STORE r20, r8
ADD r20, r5
STORE r20, r8       ; word2 (cols 64-79): only bits 0,8 used (cols 64,72)
...
pb_normal_byte:
    ...
    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r14 & 0x1F) << 6
    LDI r13, 31
    AND r14, r13
    LDI r13, 6
    SHL r14, r13
...
pb_utf8_3_start:
    CMPI r14, 240
    BGE r11, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r14 & 0x0F) << 12
    LDI r13, 15
    AND r14, r13
    LDI r13, 12
    SHL r14, r13
...
pb_esc_restore_cursor:
    CMPI r14, 56 ; '8' = DEC Restore Cursor
...
pb_esc_index:
    CMPI r14, 68 ; 'D' = Index (move down, scroll if at bottom)
...
pb_esc_rev_index:
    CMPI r14, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r11, pb_esc_ris
...
pb_esc_next_line:
    CMPI r14, 69 ; 'E' = Next Line (CR + Index)
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r14 - 48)
    LDI r20, CSI_PARAM
    LOAD r11, r20
    LDI r8, 10
    MUL r11, r8
...
mark_row_dirty:
    ...
    LDI r6, DIRTY_ROWS
    LOAD r15, r6         ; r15 = current bitmap
    OR r15, r10           ; set the bit
    STORE r6, r15
    POP r5
    RET
...
mark_row_dirty_check:
    ...
    LOAD r15, r6         ; r15 = current bitmap
    MOV r11, r15
    AND r11, r10          ; r11 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r14 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r5, 1
...
sts_div_loop:
    ...
    JMP sts_div_loop
sts_div_done:
    ; r6 = word_index, r11 = bit position within word
...
ctc_shift_loop:
    ...
    JMP ctc_shift_loop
ctc_shift_done:
    ; r15 = mask. Invert: r15 = ~r15 (all bits except the one to clear)
...
; Clobbers: r11, r5, r10, r6, r15, r14
advance_to_tab:
    PUSH r31
    PUSH r11
    LDI r5, 1
...
att_scan_loop:
    ...
    MOV r10, r11           ; r10 = col (will become bit_pos after div)
...
apply_sgr:
    ...
    CMPI r13, 0x85
    JNZ r11, sgr_ext95
    CALL sgr_256_to_rgb  ; r11 = RGB from r8 (color index)
    LDI r20, FG_COLOR
    STORE r20, r11
    LDI r20, SGR_EXTENDED
    LDI r11, 0
...
sgr_existing:
    ...
    ; r8 already loaded with SGR code from above
...
; Input:  r8 = color index (0-255)
; Output: r11 = RGB color value
; Clobbers: r11, r5, r15, r14
; Preserves: r8, r13, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r11, 232
    SUB r8, r11        ; r8 = index - 232
    LDI r11, 10
    MUL r8, r11        ; r8 = (index - 232) * 10
    LDI r11, 8
    ADD r8, r11        ; r8 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r8           ; save v
...
sgr256_cube:
    ...
    LDI r5, 8
    SHL r8, r5        ; r8 = G << 8
    OR r11, r8         ; r11 = (R << 16) | (G << 8)
    OR r11, r14         ; r11 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    LDI r20, CSI_PARAM
    LOAD r8, r20       ; r8 = mode (0, 1, or 2)

    ; Get current row
    LDI r20, CUR_ROW
    LOAD r13, r20       ; r13 = row
    LDI r1, COLS
    MUL r13, r1         ; r13 = row * COLS
    ...
    MOV r12, r3        ; r12 = end_col (exclusive)
    CMPI r8, 0
    JZ r11, cel_clear
    CMPI r8, 1
    JNZ r11, cel_mode2
...
cil_count:
    ...
    LDI r4, ROWS
    SUB r4, r5        ; r4 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r4, r13
    BLT r11, cil_clear
...
cdl_clear:
    ...
    MOV r4, r1
    SUB r4, r8
    ADD r4, r5        ; r4 = first row to clear (ROWS - count)
    CMP r4, r13
...
cdc_count:
    ...
    MOV r11, r13
    MUL r11, r3
    MOV r13, r11         ; r13 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r4, r1        ; r4 = dst col (starting at cursor)
cdc_shift:
    ADD r11, r4
    ADD r11, r8         ; r4 + count = src col
    LDI r3, COLS
    CMP r11, r3
...
cdc_clear:
    ...
    LDI r4, COLS
    SUB r4, r8        ; r4 = first col to clear
    LDI r3, COLS
    CMP r4, r1
    BGE r11, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r4 < r1 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r4, COLS
    SUB r4, r5        ; r4 = COLS-1 (start from rightmost)
    SUB r4, r8        ; r4 = COLS-1-count (first dst position)
cic_shift:
    CMP r4, r1
    BLT r11, cic_clear
...
render_row:
    ...
    LDI r2, 0          ; x (use r2 to preserve r5)
    MOV r10, r0         ; y = 10 + row*6
    LDI r6, 256         ; w
    LDI r15, 6           ; h
    LDI r14, 0x0A0A0A    ; bg color
    RECTF r2, r10, r6, r15, r14

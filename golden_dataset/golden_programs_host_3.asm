...
pb_normal_byte:
    ...
    BLT r13, pb_ascii
    CMPI r12, 224
    BGE r13, pb_utf8_3_start

    ; 2-byte UTF-8 (0xC0-0xDF): cp = (r12 & 0x1F) << 6
    LDI r1, 31
    AND r12, r1
    LDI r1, 6
    SHL r12, r1
...
pb_utf8_3_start:
    CMPI r12, 240
    BGE r13, pb_ret
    ; 3-byte UTF-8 (0xE0-0xEF): cp = (r12 & 0x0F) << 12
    LDI r1, 15
    AND r12, r1
    LDI r1, 12
    SHL r12, r1
...
pb_esc_save_cursor:
    CMPI r12, 55 ; '7' = DEC Save Cursor
    JNZ r13, pb_esc_restore_cursor
    CALL save_cursor
    LDI r20, ANSI_STATE
    LDI r13, ANS_NORMAL
    STORE r20, r13
...
pb_esc_restore_cursor:
    CMPI r12, 56 ; '8' = DEC Restore Cursor
    JNZ r13, pb_esc_index
    CALL restore_cursor
    LDI r20, ANSI_STATE
    LDI r13, ANS_NORMAL
    STORE r20, r13
...
pb_esc_index:
    CMPI r12, 68 ; 'D' = Index (move down, scroll if at bottom)
    JNZ r13, pb_esc_rev_index
    CALL do_newline
    LDI r20, ANSI_STATE
    LDI r13, ANS_NORMAL
    STORE r20, r13
...
pb_esc_rev_index:
    CMPI r12, 77 ; 'M' = Reverse Index (move up, scroll if at top)
    JNZ r13, pb_esc_ris
...
pb_esc_next_line:
    CMPI r12, 69 ; 'E' = Next Line (CR + Index)
...
pb_csi_g_tab:
    ...
    CMPI r12, 103
    JNZ r13, pb_csi_h_mode
    ; TBC: CSI_PARAM determines mode (default 0)
    LDI r20, CSI_PARAM
    LOAD r13, r20
    CMPI r13, 3
    JZ r13, tbc_clear_all
    CMPI r13, 0
    JZ r13, tbc_clear_one
...
pb_csi_h_mode:
    ...
    CMPI r12, 104
    JNZ r13, pb_csi_l_mode
    CMPI r24, 0
    JZ r13, pb_ret         ; ignore non-private 'h'
    LDI r20, CSI_PARAM
    LOAD r13, r20
    CMPI r13, 1049
    JNZ r13, pb_ret        ; only handle 1049
    CALL alt_screen_on
    JMP pb_ret
...
pb_csi_l_mode:
    ...
    JNZ r13, pb_csi_s
    CMPI r24, 0
    JZ r13, pb_ret         ; ignore non-private 'l'
    LDI r20, CSI_PARAM
    LOAD r13, r20
    CMPI r13, 1049
    JNZ r13, pb_ret        ; only handle 1049
...
pb_csi_digit:
    ...
    ; Accumulate digit: CSI_PARAM = CSI_PARAM * 10 + (r12 - 48)
    LDI r20, CSI_PARAM
    LOAD r13, r20
    LDI r10, 10
    MUL r13, r10
...
pb_check_utf8_2:
    CMPI r4, ANS_UTF8_2
    JNZ r13, pb_check_utf8_3a

    ; Validate continuation byte (0x80-0xBF)
    CMPI r12, 128
    BLT r13, pb_reset_state
    CMPI r12, 192
    BGE r13, pb_reset_state
...
; DIRTY ROW HELPERS
; =========================================
; mark_row_dirty -- mark row in r13 as dirty
; Sets bit (31 - row) in DIRTY_ROWS bitmap
mark_row_dirty:
    PUSH r5
    LDI r5, 31
    SUB r5, r13          ; r5 = 31 - row
    LDI r15, 1
    SHL r15, r5          ; r15 = 1 << (31 - row)
    LDI r11, DIRTY_ROWS
    LOAD r4, r11         ; r4 = current bitmap
    OR r4, r15           ; set the bit
    STORE r11, r4
    POP r5
    RET
...
; mark_row_dirty_check -- check if row in r13 is dirty
; Returns r13 = bit value (0=clean, nonzero=dirty)
mark_row_dirty_check:
    ...
    LOAD r4, r11         ; r4 = current bitmap
    MOV r13, r4
    AND r13, r15          ; r13 = bitmap & mask
    POP r31
    RET

; =========================================
; APPEND_BYTE -- append r12 to text buffer
; =========================================
append_byte:
    PUSH r31
    LDI r5, 1
...
sts_div_loop:
    CMPI r13, 32
    BLT r13, sts_div_done
    SUB r13, r15
    ADD r11, r5
    JMP sts_div_loop
sts_div_done:
    ; r11 = word_index, r13 = bit position within word
    PUSH r13              ; save bit position (CMP will clobber r13)
    ; Load the word
    LDI r20, TAB_STOPS
    ADD r20, r11
    LOAD r15, r20
    POP r13               ; restore bit position
    ; Set bit: r15 |= (1 << r13)
    LDI r4, 1
    LDI r12, 0
sts_shift_loop:
    CMP r12, r13
    JZ r13, sts_shift_done
    SHL r4, r5
    ADD r12, r5
    JMP sts_shift_loop
sts_shift_done:
...
; Clobbers: r13, r5, r15, r11, r4, r12
advance_to_tab:
    PUSH r31
    PUSH r13
    LDI r5, 1
...
att_scan_loop:
    ...
    ; Save scan column (CMP in div/shift loops clobbers r13)
    PUSH r13

    ; Compute word_index = col / 32 and bit_pos = col % 32
    ; using repeated subtraction on a copy
    MOV r15, r13           ; r15 = col (will become bit_pos after div)
    LDI r11, 0            ; r11 = word_index
    LDI r1, 32           ; r1 = divisor
att_div_loop:
    CMPI r15, 32
    BLT r13, att_check_bit
    SUB r15, r1           ; r15 -= 32
    ADD r11, r5           ; word_index++
    JMP att_div_loop
...
att_check_bit:
    ...
    ADD r20, r11
    LOAD r4, r20
    ; Check if bit r15 is set: shift 1 left by r15, AND
    LDI r12, 1
    LDI r10, 0
att_shift_loop:
    CMP r10, r15
...
att_test_bit:
    ...
    POP r13               ; restore scan column
att_scan_next:
    ADD r13, r5
    JMP att_scan_loop

att_found_pop:
    POP r13               ; restore scan column (r13 = tab stop column)
att_found:
    ; Found tab stop at column r13 -- move cursor there
    LDI r20, CUR_COL
    STORE r20, r13
    LDI r20, CUR_ROW
    LOAD r13, r20         ; r13 = current row for mark_row_dirty
    CALL mark_row_dirty
    POP r13
    POP r31
    RET
...
dsb_ret:
    ...
    ; Divide by 10: r20 = tens, r13 = ones
    PUSH r20
    LDI r21, 10
    DIV r20, r21         ; r20 = count / 10
    POP r13               ; r13 = original count
    LDI r21, 10
    MOD r13, r21          ; r13 = count % 10
...
; Preserves all registers except r13 (CMP clobbers it).
...
apply_sgr:
    ...
    CMPI r1, 0x85
    JNZ r13, sgr_ext95
    CALL sgr_256_to_rgb  ; r13 = RGB from r10 (color index)
    LDI r20, FG_COLOR
    STORE r20, r13
    LDI r20, SGR_EXTENDED
    LDI r13, 0
...
; Output: r13 = RGB color value
; Clobbers: r13, r5, r4, r12
; Preserves: r10, r1, r20, r31
; Palette:
...
sgr256_cube_check:
    ...
    ; v = (index - 232) * 10 + 8
    LDI r13, 232
    SUB r10, r13        ; r10 = index - 232
    LDI r13, 10
    MUL r10, r13        ; r10 = (index - 232) * 10
    LDI r13, 8
    ADD r10, r13        ; r10 = v
    ; color = (v << 16) | (v << 8) | v
    PUSH r10           ; save v
    ...
    MOV r13, r10
    SHL r13, r5        ; r13 = v << 16
    PUSH r13           ; save high bits
    LDI r5, 8
    MOV r13, r10
    SHL r13, r5        ; r13 = v << 8
    OR r13, r10         ; r13 = (v << 8) | v
    POP r5            ; r5 = v << 16
    OR r13, r5         ; r13 = (v << 16) | (v << 8) | v
    POP r10            ; restore r10
    JMP sgr256_ret
...
sgr256_cube:
    ...
    MOV r13, r5
    LDI r10, 16
    SHL r13, r10        ; r13 = R << 16
    MOV r10, r4
    LDI r5, 8
    SHL r10, r5        ; r10 = G << 8
    OR r13, r10         ; r13 = (R << 16) | (G << 8)
    OR r13, r12         ; r13 = (R << 16) | (G << 8) | B
...
csi_erase_line:
    ...
    MOV r2, r3        ; r2 = end_col (exclusive)
    CMPI r10, 0
    JZ r13, cel_clear
    CMPI r10, 1
    JNZ r13, cel_mode2
...
ced_check0:
    ...
    LDI r2, COLS
    MUL r7, r2
    ADD r7, r3        ; offset = row*COLS + col

    LDI r1, ROWS
    LDI r6, COLS
    MUL r1, r6         ; total cells
ced_loop0:
    CMP r7, r1
    BGE r13, ced_done
    LDI r20, BUF
...
csi_cursor_position:
    ...
    LDI r10, 1
ccp_row_set:
    SUB r10, r5         ; convert to 0-based
    CMPI r10, 0
    BGE r13, ccp_row_clamp
    LDI r10, 0
ccp_row_clamp:
    LDI r1, ROWS
    SUB r1, r5
...
cil_count:
    ...
    LDI r7, ROWS
    SUB r7, r5        ; r7 = ROWS-1 (source row, start from bottom-1)
cil_shift:
    CMP r7, r1
    BLT r13, cil_clear
...
cil_clear_loop:
    ...
    LDI r6, ROWS
    SUB r6, r5
    BGE r13, cil_skip   ; skip if past last row
...
cdl_shift:
    ...
    SUB r3, r5
    CMP r13, r3
    BGE r13, cdl_clear_src  ; if src past last row, clear instead
...
cdl_clear:
    ...
    MOV r7, r6
    SUB r7, r10
    ADD r7, r5        ; r7 = first row to clear (ROWS - count)
    CMP r7, r1
    BGE r13, cdl_done2  ; if nothing to clear, skip
    LDI r22, 0
cdl_clr_loop:
    CMP r22, r10
    BGE r13, cdl_done2
...
cdc_count:
    ...
    LOAD r1, r20
    LDI r20, CUR_COL
    LOAD r6, r20       ; r6 = cursor col

    ; Base offset = row * COLS
    LDI r3, COLS
    MOV r13, r1
    MUL r13, r3
    MOV r1, r13         ; r1 = row * COLS (base offset)

    ; Shift chars left: col+N -> col, col+N+1 -> col+1, etc.
    MOV r7, r6        ; r7 = dst col (starting at cursor)
cdc_shift:
    ADD r13, r7
    ADD r13, r10         ; r7 + count = src col
    LDI r3, COLS
    CMP r13, r3
    BGE r13, cdc_clear  ; if src >= COLS, clear instead
...
cdc_clear:
    ...
    LDI r7, COLS
    SUB r7, r10        ; r7 = first col to clear
    LDI r3, COLS
    CMP r7, r6
    BGE r13, cdc_skip   ; if clear area starts after cursor, nothing to clear
    ; r7 < r6 is guaranteed here (BGE above would have branched)
    JMP cdc_do_clear
cdc_do_clear:
...
cic_count:
    ...
    LDI r7, COLS
    SUB r7, r5        ; r7 = COLS-1 (start from rightmost)
    SUB r7, r10        ; r7 = COLS-1-count (first dst position)
cic_shift:
    CMP r7, r6
    BLT r13, cic_clear
...
render_skip_status:
    ...
    LDI r20, DIRTY_ROWS
    LOAD r13, r20
    JZ r13, render_cursor_blink  ; no dirty rows, skip to cursor
...
render_row:
    ...
    LDI r8, 0          ; x (use r8 to preserve r5)
    MOV r15, r0         ; y = 10 + row*6
    LDI r11, 256         ; w
    LDI r4, 6           ; h
    LDI r12, 0x0A0A0A    ; bg color
    RECTF r8, r15, r11, r4, r12

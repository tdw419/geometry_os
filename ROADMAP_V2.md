...
## [x] phase-142: EPL Journal Compaction (COMPLETE)

**Goal:** Reclaim disk space by keeping only the latest delta for each chunk in the append-only journal.

The EPL journal is append-only, causing it to grow indefinitely if the same chunks are modified frequently. This phase adds a background or manual compaction pass that rewrites the journal file.

### Deliverables
- [x] **Compaction Mechanism in EplManager** -- Implement `compact()` which rewrites the journal to a temporary file, skipping old versions of chunks, then replaces the original.
- [x] **Automatic Compaction Threshold** -- Trigger compaction when the journal size exceeds a certain ratio of valid data or a raw size limit (e.g., >10MB and >50% redundant).
- [x] **Verification** -- Test that compaction preserves chunk data and correctly updates the exception set.

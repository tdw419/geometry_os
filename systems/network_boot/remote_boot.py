...
class RemoteBootClient:
    ...
    async def boot(
        self,
        rts_file: str,
        progress_callback: Optional[Callable[[RemoteBootProgress], None]] = None,
                ...
                async with session.post(url, data=form) as response:
                    ...
                    async for chunk in response.content:
                        if self._cancel_requested:
                            session = await self._get_session()
                            await self._cancel_boot(session)
                            return RemoteBootResult(
                                success=False,
    ...
    async def _cancel_boot(self, session: aiohttp.ClientSession):
    ...
    async def status(self, container_id: str) -> Optional[Dict[str, Any]]:
        """

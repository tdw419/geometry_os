use crate::backends::{ExecutionBackend, InputEvent, InputBusState};
...
impl<B: ExecutionBackend> AppCoordinator<B> {
    ...
    /// Write an input event to the Visual Interaction Bus.
    /// This is the zero-CPU input path - events go directly to VRAM.
    pub fn write_input_event(&mut self, event: InputEvent) -> Result<(), String> {
        self.backend.write_input_event(event)
    }

    /// Read the current state of the Visual Interaction Bus.
    /// Useful for debugging and verifying input flow.
    pub fn get_input_bus_state(&mut self) -> Result<InputBusState, String> {
        self.backend.get_input_bus_state()
    }
}

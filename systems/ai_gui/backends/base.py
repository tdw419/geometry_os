...
class InputEvent:
    ...
    type: InputEventType
    keys: str | None = None  # e.g., "Ctrl+S" for key events
    text: str | None = None  # Text to type for text events
    x: int | None = None     # Mouse X coordinate
    y: int | None = None     # Mouse Y coordinate
    button: int | None = None  # Mouse button (1=left, 2=middle, 3=right)
    modifiers: list[str] | None = field(default_factory=list)  # ["ctrl", "shift", "alt"]
...
class AppConfig:
    ...
    location: tuple[int, int] | None = None  # (x, y) position on map
    width: int = 800
    height: int = 600

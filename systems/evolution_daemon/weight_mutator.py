...
class WeightMutator:
    ...
    def _apply_radiation(self, start: int, end: int, intensity: float):
        ...
        Good for exploration and random search.
        """
    ...
    def _apply_crossover(self, start: int, end: int, source_sector: int):
        ...
        for i, d in enumerate(range(start, end)):
            ...
            # Copy all channels
            for c in range(self.atlas.shape[2]):
                self.atlas[target_row, target_col, c] = \
                    self.atlas[source_row, source_col, c]

    def _apply_noise(self, start: int, end: int, intensity: float):
        """
        Apply noise mutation: weight += randn() × intensity × 0.1.

        Good for fine-tuning and subtle adjustments.
        """
        # Noise is 10x smaller than radiation
        scaled_intensity = intensity * 0.1

        for d in range(start, end):
            row, col = self.hilbert_lut[d]
            # Generate Gaussian noise
            noise = np.random.normal(0, scaled_intensity)
            # Apply to all channels
            for c in range(self.atlas.shape[2]):
                self.atlas[row, col, c] += noise

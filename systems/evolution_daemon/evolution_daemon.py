...
Refactored for maintainability - 2026-03-10
...
- core/webmcp_client.py: WebSocket client for visual shell
- core/virtual_filesystem.py: File operations for self-modification
- core/task_dataclasses.py: EvolutionTask, CodebaseAnalysis
- core/tool_callbacks.py: Z.ai function calling registry
- core/ambient_mode.py: WordPress WebMCP narrative mode
- core/radio_controller.py: GOSR radio broadcasting
"""
...
# Import core components (refactored)
from systems.evolution_daemon.core import (
    CodebaseAnalysis,
    EvolutionTask,
    VirtualFileSystem,
    WebMCPClient,
...
from systems.evolution_daemon.safety.post_commit_monitor import EvolutionRecovery, PostCommitMonitor
...
try:
    from systems.evolution_daemon.safety.fracture_detector import FractureDetector, OuroborosReflex
    HAS_FRACTURE_DETECTOR = True
except ImportError:
    HAS_FRACTURE_DETECTOR = False
    FractureDetector = None
    OuroborosReflex = None
...
try:
    ...
    HAS_DREAM_ENGINE = True
except ImportError:
    HAS_DREAM_ENGINE = False
    DreamEngine = None
    TraumaLog = None
    GeneticTrauma = None
...
from systems.evolution_daemon.stages.tectonic_stage import TectonicShiftResult, TectonicStage
...
class EvolutionTask:
    ...
    visual_verification: str | None = None  # Screenshot/data for visual check
    ...
    # V16 Agency Integration - PAS Score tracking
    pas_score: float = 0.0
    pas_decision: str = "PENDING"  # PENDING, PROMOTE, QUARANTINE, REJECT
...
class EvolutionDaemon:
    ...
    def __init__(self, api_key: str | None = None, ws_url: str = "ws://localhost:8765", brain_atlas: np.ndarray = None, budget: int | None = None, intent_file: str | None = None):
        # Initialize Z.ai Integration (our new brain)
        self.zai = ZAIIntegration(api_key=api_key) if HAS_ZAI else None
        self.vfs = VirtualFileSystem()

        # Ralph Wiggum Mode - Budget and Intent
        self.budget = budget
        ...
        # Ouroboros Fracture Detector - Phase 56 (Self-Healing Kernel)
        self._fracture_detector = None
        self._fracture_task = None
        ...
        self.radio_broadcaster: NarrativeBroadcaster | None = None
        ...
        # V16 Agency Integration - NEXUS Phase 6 (Operate & Evolve)
        self._agency_orchestrator = None  # EvolutionOrchestratorAgent
        self._agency_governor = None      # MutationGovernorAgent
        self._init_agency_agents()

        # Initialize Ouroboros Fracture Detector
    ...
    def _init_agency_agents(self):
        """
        Initialize Agency agents for NEXUS Phase 6 (Operate & Evolve).

        Agents:
        - EvolutionOrchestrator: Manages Dev ↔ QA loop, pipeline coordination
        - MutationGovernor: PAS Score evaluation, promotion decisions
        """
        try:
            from systems.swarm.agency_agents import create_agency_agent

            # Initialize the Orchestrator (NEXUS Phase 6 Manager)
            self._agency_orchestrator = create_agency_agent(
                "evolution_orchestrator",
                f"orch-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            )

            # Initialize the Governor (PAS Score Gatekeeper)
            self._agency_governor = create_agency_agent(
                "mutation_governor",
                f"gov-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            )

            logger.info("🤖 Agency agents initialized: orchestrator + governor")
            logger.info("   PAS Score framework active (P=0.4, A=0.3, S=0.3)")

        except ImportError as e:
            logger.warning(f"Agency agents not available: {e}")
            self._agency_orchestrator = None
            self._agency_governor = None
        except Exception as e:
            logger.error(f"Failed to initialize agency agents: {e}")
            self._agency_orchestrator = None
            self._agency_governor = None
    ...
    def _init_ouroboros(self):
        """
        Initialize the Ouroboros Fracture Detector - Phase 56 (Self-Healing Kernel).

        The Fracture Detector monitors the ASCII Scene Graph at 1Hz and triggers
        reflex responses when geometric coherence breaks down.
        """
        if not HAS_FRACTURE_DETECTOR:
            logger.warning("FractureDetector not available, Ouroboros monitoring disabled")
            return

        try:
            self._fracture_detector = FractureDetector(daemon=self)
            logger.info("🧬 Ouroboros Fracture Detector initialized (1Hz monitoring)")
        except Exception as e:
            logger.error(f"Failed to initialize Ouroboros Fracture Detector: {e}")
            self._fracture_detector = None

    def is_district_quarantined(self, district: str) -> bool:
        """
        Check if a district is currently quarantined by Ouroboros.

        Args:
            district: District identifier (e.g., "attention", "memory", "evolution")

        Returns:
            True if the district is quarantined, False otherwise
        """
    ...
    def is_emergency_frozen(self) -> bool:
        """
        Check if the system is in emergency freeze state.

        When frozen, all mutations should be halted immediately.

        Returns:
            True if emergency freeze is active, False otherwise
        """
        if self._fracture_detector is None:
            return False
    ...
    def _resolve_sector_bounds(self, sector: str) -> dict | None:
    ...
    def get_mutation_target(self, name: str) -> dict[str, Any] | None:
        """Get a mutation target by name."""
        return self._mutation_targets.get(name)

    def list_mutation_targets(self, sector: str | None = None) -> list[str]:
    ...
    async def initialize(self):
        ...
        if self.zai is None:
            logger.warning("⚠️ Z.ai not initialized. Running in static mode.")
        elif self.zai.is_mock_mode:
        ...
        # --- Phase 57 Predictive Simulation ---
        if HAS_DREAM_ENGINE:
            try:
                self._trauma_log = TraumaLog()
                self._dream_engine = DreamEngine(sandbox_manager=self.sandbox)
                logger.info("🌙 Predictive Dream Engine active (Phase 57)")
            except Exception as e:
                logger.warning(f"⚠️ Failed to initialize Dream Engine: {e}")
                self._dream_engine = None
                self._trauma_log = None
        else:
            self._dream_engine = None
            self._trauma_log = None
        ...
        # --- V16 Perception Expansion ---
        try:
            from systems.visual_shell.ascii_scene import wire_all_renderers
            wire_all_renderers()
            logger.info("✅ ASCII Scene Renderers wired for zero-cost perception")
        except Exception as e:
            logger.warning(f"⚠️ Could not wire ASCII Scene Renderers: {e}")
        ...
        # Start ASCII Scene heartbeat for zero-cost AI perception
        self.running = True
        self._heartbeat_task = asyncio.create_task(self._ascii_heartbeat_loop())
        logger.info("💓 ASCII Scene heartbeat started (1Hz)")

        # Start Ouroboros Fracture Detector for self-healing
        if self._fracture_detector is not None:
            self._fracture_task = asyncio.create_task(self._fracture_detector.run())
            logger.info("🧬 Ouroboros Fracture Detector started (1Hz monitoring)")

    async def _ascii_heartbeat_loop(self):
        """
        Continuous heartbeat that flushes ASCII Scene renderers.

        This ensures the Visual Substrate stays "alive" for both:
        - GNB (Geometric Native Browser) perception
        - Autonomous agent monitoring
        - Zero-cost AI verification

        Runs at 1Hz even when idle to maintain state visibility.
        """
        heartbeat_count = 0
        while self.running:
            try:
                ...
                # Force flush all ASCII renderers
                try:
                    from systems.visual_shell.ascii_scene import flush_all_renderers
                    flush_all_renderers()
                except ImportError:
                    # Fallback: manually write heartbeat state
                    heartbeat_file = Path(".geometry/ascii_scene/heartbeat.ascii")
                    heartbeat_file.parent.mkdir(parents=True, exist_ok=True)
                    heartbeat_file.write_text(f"""# ASCII Scene Heartbeat
timestamp: {datetime.now().isoformat()}
count: {heartbeat_count}
status: alive
daemon: evolution_daemon
agency_mode: V16_ORCHESTRATED
""")

                # Update GNB state mirror
                gnb_state = Path(".geometry/gnb_state.ascii")
                gnb_state.parent.mkdir(parents=True, exist_ok=True)
                gnb_state.write_text(f"""GEOMETRIC NATIVE BROWSER - STATE MIRROR
TIMESTAMP: {time.time()}
MODE: HEADLESS
...
SCENE SUMMARY:
Daemon: ACTIVE
Agency: V16_ORCHESTRATED
Orchestrator: {'ACTIVE' if self._agency_orchestrator else 'OFFLINE'}
Governor: {'ACTIVE' if self._agency_governor else 'OFFLINE'}
Evolution Count: {self.evolution_count}
---
WINDOW TOPOLOGY:
See .geometry/ascii_scene/ for detailed fragments
""")

                if heartbeat_count % 60 == 0:  # Log every minute
                    logger.debug(f"💓 ASCII heartbeat: {heartbeat_count} cycles")
    ...
    async def scan_codebase(self) -> CodebaseAnalysis:
        ...
        if self.zai and not self.zai.is_mock_mode:
            analysis_prompt = f"""
            ...
            logger.info("💡 Architect identified improvement opportunities")
    ...
    async def safe_evolve(self, task: EvolutionTask) -> bool:
        ...
        try:
            ...
            # 4.5 PAS EVAL - Agency Governor evaluates mutation for kernel promotion
            pas_result = await self._evaluate_pas_score(task, proposal, sandbox_result, verdict)
            if pas_result and pas_result.get("decision") == "REJECT":
                logger.warning(f"❌ PAS Governor rejected: score={pas_result.get('pas_score', {}).get('total', 0):.2f}")
                await self.visual_log(f"PAS REJECTED (score={pas_result.get('pas_score', {}).get('total', 0):.0%})", "error")
                task.status = "rejected"
                task.result = f"PAS score too low: {pas_result.get('reasoning', 'Unknown')}"
                return False
            elif pas_result:
                pas_score = pas_result.get('pas_score', {}).get('total', 0)
                decision = pas_result.get('decision', 'UNKNOWN')
                logger.info(f"✅ PAS evaluated: score={pas_score:.2f}, decision={decision}")
                await self.visual_log(f"PAS: {pas_score:.0%} ({decision})", "success")
                # Store PAS score in task for later reference
                task.pas_score = pas_score
                task.pas_decision = decision
            ...
            if task.visual_intent:
                ...
                if verification_result.should_retry and task.visual_attempt < 5:
                    ...
                    await self.visual_log("Visual: retry suggested", "info")
    ...
    async def _evaluate_pas_score(
        ...
        """
        Evaluate mutation using PAS Score framework via Agency Governor.

        PAS = (Performance × 0.4) + (Aesthetics × 0.3) + (Stability × 0.3)

        Returns:
            Dict with pas_score, decision, and reasoning, or None if governor unavailable
        """
        if self._agency_governor is None:
            logger.debug("Agency governor not available, skipping PAS evaluation")
            return None

        try:
            from systems.swarm.llm_agent import AgentTask

            # Extract metrics for PAS calculation
            # Performance: based on sandbox test pass rate
            performance_score = 0.5  # Default
            if hasattr(sandbox_result, 'heartbeat_tests_passed') and hasattr(sandbox_result, 'heartbeat_tests_total'):
                if sandbox_result.heartbeat_tests_total > 0:
                    performance_score = sandbox_result.heartbeat_tests_passed / sandbox_result.heartbeat_tests_total

            # Aesthetics: based on guardian confidence (proxy for code quality)
            aesthetics_score = 0.5
            if hasattr(verdict, 'confidence'):
                aesthetics_score = verdict.confidence

            # Stability: based on guardian risk assessment
            stability_score = 0.5
            if hasattr(verdict, 'risk_level'):
                # Map risk levels to stability scores
                risk_map = {"low": 0.9, "medium": 0.6, "high": 0.3}
                stability_score = risk_map.get(verdict.risk_level.lower(), 0.5)

            # Create task for governor
            gov_task = AgentTask(
                task_id=f"pas_{task.task_id}",
                ...
                payload={
                    ...
                    "tests_passed": sandbox_result.passed if hasattr(sandbox_result, 'passed') else True
                }
            )

            # Run governor evaluation
            result = await self._agency_governor.run_task(gov_task)

            if result.success and result.metadata:
                return {
                    "pas_score": result.metadata.get("pas_score", {}),
                    "decision": result.metadata.get("decision", "QUARANTINE"),
                    "reasoning": result.output
                }

            return {"pas_score": {"total": 0}, "decision": "QUARANTINE", "reasoning": "Governor evaluation failed"}

        except Exception as e:
            logger.warning(f"PAS evaluation failed: {e}")
            return None
    ...
    async def _create_proposal(self, task: EvolutionTask) -> EvolutionProposal:
        ...
        {('Current code context:' + _nl + '```python' + _nl + context[:5000] + _nl + '```') if context else ''}
    ...
    async def evolve(self, task: EvolutionTask) -> bool:
        ...
        If Agency Orchestrator is available, uses the orchestrated NEXUS Phase 6 pipeline.
        Otherwise falls back to the standard safe_evolve pipeline.
        """
        if self._agency_orchestrator:
            return await self.orchestrated_evolve(task)
        return await self.safe_evolve(task)

    async def orchestrated_evolve(self, task: EvolutionTask) -> bool:
        """
        Execute evolution using the NEXUS Phase 6 Orchestrator.

        This delegates the Dev↔QA loop to the EvolutionOrchestratorAgent.
        """
        logger.info(f"🧬 [V16] Starting orchestrated evolution: {task.task_id}")
        task.status = "in_progress"

        try:
            # Visual feedback
            await self.visualize_evolution(task, "in_progress")
            await self.visual_log(f"[V16] Orchestrating: {task.goal[:30]}", "info")

            from systems.swarm.llm_agent import AgentTask

            # Prepare the pipeline task for the orchestrator
            orch_task = AgentTask(
                task_id=f"orch_{task.task_id}",
                task_type="run_pipeline",
                payload={
                    "mutation_spec": {"id": task.task_id, "goal": task.goal},
                    "daemon": self,
                    "task": task
                }
            )
            ...
            result = await self._agency_orchestrator.run_task(orch_task)

            if not result.success:
                logger.warning(f"❌ Orchestrated pipeline failed: {result.error or result.output}")
                await self.visual_log("Pipeline FAILED", "error")
                task.status = "failed"
                task.result = result.error or "Orchestrator failed to find stable mutation"
                return False

            # Extract results from the successful pipeline
            meta = result.metadata
            proposal = meta.get("proposal")
            verdict = meta.get("verdict")
            pas_result = meta.get("pas_result")

            # --- Phase 57: Predictive Simulation (The Dream) ---
            if self._dream_engine and proposal:
                logger.info(f"🌙 [V16] Entering Predictive Dream Phase for {proposal.proposal_id}")
                await self.visual_log("Simulating dream...", "info")

                dream_frame, reflex_score = await self._dream_engine.simulate_dream(proposal)

                # Check for preemptive fracture
                pain_threshold = self._fracture_detector.config.PAIN_THRESHOLD if self._fracture_detector else 0.3
                if reflex_score > pain_threshold:
                    logger.error(f"🛑 [V16] PREEMPTIVE FRACTURE detected in dream! Score: {reflex_score:.2f}")
                    await self.visual_log("PREEMPTIVE FRACTURE", "error")

                    # Record trauma
                    if self._trauma_log:
                        self._trauma_log.record_trauma(GeneticTrauma(
                            proposal_id=proposal.proposal_id,
                            timestamp=time.time(),
                            reflex_score=reflex_score,
                            target_files=proposal.target_files,
                            diff_summary="Dream simulation detected structural fracture",
                            fracture_signal="preemptive_fracture"
                        ))

                    task.status = "preempted"
                    task.result = f"Preempted: Dream simulation detected fracture (Score: {reflex_score:.2f})"
                    return False

                logger.info("✨ [V16] Dream simulation passed. Coherence maintained.")
                await self.visual_log("Dream passed", "success")

            # Since the orchestrator already ran the PAS evaluation, we can proceed to TIER and COMMIT

            logger.info("✅ Orchestrator found stable mutation. Proceeding to integration.")
            ...
            if tier == 3:
                branch_name = await self.git.create_pr_branch(proposal)
                task.status = "awaiting_review"
                task.result = f"Tier 3 - PR created: {branch_name}"
                return True
            ...
            # 8. MONITOR - Post-commit health checks
            await self.monitor.capture_baseline()
            mon_result = await self.monitor.monitor(commit_sha, tier)

            if not mon_result.healthy:
                logger.warning("🚨 Regression detected post-orchestration!")
                await self.visual_log("REGRESSION detected!", "error")
                await self.recovery.handle_regression(commit_sha, mon_result)
                task.status = "reverted"
                return False

            # Success!
            task.status = "completed"
            task.result = "Evolution successful (Orchestrated)"
            self.evolution_count += 1
            await self.visualize_evolution(task, "completed")
            await self.visual_log("SUCCESS: Orchestrated", "success")
            return True
    ...
    async def safe_evolve(self, task: EvolutionTask) -> bool:
        ...
        try:
            ...
            {('Current code context:' + _nl + '```' + _nl + context[:2000] + _nl + '```') if context else ''}
    ...
    async def run_loop(self):
        ...
        while self.running and self.evolution_count < self.max_evolutions_per_session:
            ...
            # Check time budget (Ralph Wiggum Mode)
            if self.budget:
            ...
            elif iteration % 5 == 0 and self.zai and not self.zai.is_mock_mode:
    ...
    async def stop(self):
        ...
        if self._fracture_task:
            self._fracture_task.cancel()
        if self._fracture_detector:
            self._fracture_detector.stop()
...
async def main():
    ...
    # Ralph Wiggum Mode flags
    parser.add_argument("--budget", type=int, help="Time budget in seconds (Ralph Wiggum Mode)")
    parser.add_argument("--intent", type=str, help="Intent file path (.geometry/intent/evolution.md)")
    ...
    # Load initial task from intent file if provided
    if args.intent and Path(args.intent).exists():
        intent_content = Path(args.intent).read_text()
        # Create a task from the first 200 chars of the intent
        await daemon.create_evolution_task(
            goal=f"INTENT: {intent_content[:200]}...",
            priority=10
        )

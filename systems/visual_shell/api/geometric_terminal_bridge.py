...
class GeometricTerminalBridge:
    ...
    def render_to_texture(self, tile_id: int, output_path: str | None = None) -> Path | None:
        """
    ...
    def get_cell(self, tile_id: int, row: int, col: int) -> TerminalCell | None:
        """Get cell at position."""
        if tile_id not in self._tiles:
            return None
        return self._tiles[tile_id].terminal.get_cell_at(row, col)

    def get_cell_rgba(self, tile_id: int, row: int, col: int) -> tuple[int, int, int, int] | None:

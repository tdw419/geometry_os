...
impl PixelRTSBridge {
    ...
    ///
    /// Uses separable box blur for O(n * blur_radius) instead of O(n * blur_radius²)
    fn apply_glassmorphism(
        ...
        let blur_diameter = (2 * blur_radius + 1) as u32;
        let total_samples = blur_diameter * blur_diameter;
        
        // Allocate temp buffer once for separable blur
        let mut temp = vec![RgbaPixel::new(0, 0, 0, 0); texture.len()];
        
        // Pass 1: Horizontal blur (texture -> temp)
        for y in 0..resolution {
            let row_start = (y * resolution) as usize;
            ...
            for x in 0..resolution {
                ...
                // Sum horizontal strip
                for dx in -blur_radius..=blur_radius {
                    let px = (x as i32 + dx).clamp(0, (resolution - 1) as i32) as u32;
                ...
                temp[idx] = RgbaPixel::new(
                    (sum_r / blur_diameter) as u8,
                    (sum_g / blur_diameter) as u8,
                    (sum_b / blur_diameter) as u8,
                    (sum_a / blur_diameter) as u8,
                );
            }
        }
        
        // Pass 2: Vertical blur (temp -> texture) with glass blend
        ...
        for y in 0..resolution {
            for x in 0..resolution {
                ...
                for dy in -blur_radius..=blur_radius {
                    let py = (y as i32 + dy).clamp(0, (resolution - 1) as i32) as u32;
                    let idx = (py * resolution + x) as usize;
                    sum_r += temp[idx].r as u32;
                    sum_g += temp[idx].g as u32;
                    sum_b += temp[idx].b as u32;
                    sum_a += temp[idx].a as u32;
                }
                
                let blurred = RgbaPixel::new(
                    (sum_r / blur_diameter) as u8,
                    (sum_g / blur_diameter) as u8,
                    (sum_b / blur_diameter) as u8,
                    (sum_a / blur_diameter) as u8,
                ...
                let original = texture[idx];
                
                texture[idx] = RgbaPixel::new(
                    (original.r as f32 * (1.0 - glass_alpha) + blurred.r as f32 * glass_alpha) as u8,
                    (original.g as f32 * (1.0 - glass_alpha) + blurred.g as f32 * glass_alpha) as u8,
                    (original.b as f32 * (1.0 - glass_alpha) + blurred.b as f32 * glass_alpha) as u8,
                    original.a,
                );
            }
        }

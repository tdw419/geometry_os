...
class ResearcherAgent(WorkerAgent):
    ...
    async def handle_message(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        if msg.get("type") == "scan_patterns":
            logger.info(
                f"Researcher {self.agent_id} scanning patterns for: {msg.get('prompt')}"
            )

            # Check if visual artifacts are present - delegate to perception if needed
            ...
            if any(kw in prompt.lower() for kw in visual_keywords):
                logger.info(
                    f"Researcher {self.agent_id} detected visual context - delegating to perception-specialist"
                )
                return {
                    "status": "delegated",
                    "delegate_to": "perception-specialist",
                    "original_request": msg,
                    "reason": "Visual artifact detection - requires perception capability",
                }
...
class ArchitectAgent(WorkerAgent):
    ...
    async def handle_message(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        if msg.get("type") == "design_structure":
            logger.info(
                f"Architect {self.agent_id} designing structure for: {msg.get('research')}"
            )
...
class ImplementerAgent(WorkerAgent):
    ...
    async def handle_message(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        if msg.get("type") == "generate_code":
            logger.info(
                f"Implementer {self.agent_id} writing code for: {msg.get('design')}"
            )
...
class ValidatorAgent(WorkerAgent):
    ...
    async def handle_message(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        if msg.get("type") == "verify_artifact":
            logger.info(
                f"Validator {self.agent_id} testing artifact: {msg.get('file')}"
            )

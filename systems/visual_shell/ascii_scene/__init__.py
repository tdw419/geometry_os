...
def wire_neural_city_renderer(broadcaster=None, output_dir: str = ".geometry/ascii_scene"):
    ...
    logger.info("Wired NeuralCityAsciiRenderer to broadcaster")
...
def wire_visual_shell_renderer(broadcaster=None, output_dir: str = ".geometry/ascii_scene"):
    ...
    logger.info("Wired VisualShellAsciiRenderer to broadcaster")
    return broadcaster, renderer
...
def wire_evolution_renderer(broadcaster=None, output_dir: str = ".geometry/ascii_scene"):
    ...
    logger.info("Wired EvolutionAsciiRenderer to broadcaster")
    return broadcaster, renderer
...
def flush_all_renderers():
    """
    Force flush all ASCII renderers to their output files.

    Called by the EvolutionDaemon heartbeat loop to ensure the ASCII Scene
    stays "alive" even when idle. This enables zero-cost AI perception
    without requiring browser interaction.

    This is a no-op if no renderers are wired.
    """
    flushed = []

    if _neural_city_renderer is not None:
        try:
            _neural_city_renderer.force_flush()
            flushed.append('neural_city')
        except Exception as e:
            logger.warning(f"Failed to flush neural_city renderer: {e}")

    if _visual_shell_renderer is not None:
        try:
            _visual_shell_renderer.force_flush()
            flushed.append('visual_shell')
        except Exception as e:
            logger.warning(f"Failed to flush visual_shell renderer: {e}")

    if _evolution_renderer is not None:
        try:
            _evolution_renderer.force_flush()
            flushed.append('evolution')
        except Exception as e:
            logger.warning(f"Failed to flush evolution renderer: {e}")

    return flushed


# Convenience exports
from .hooks import LoggingHook as VisualShellLoggingHook
from .hooks import NoOpHook as VisualShellNoOpHook
from .hooks import VisualShellHook, VisualShellHookBroadcaster
from .renderers import EvolutionAsciiRenderer, NeuralCityAsciiRenderer, VisualShellAsciiRenderer

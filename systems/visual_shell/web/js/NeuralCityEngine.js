...
import { AgencyOverlay } from '../agency_overlay.js';
...
export class NeuralCityEngine {
    constructor(config = {}) {
        ...
        // Agency Overlay for spatial personas (Task 7.3)
        this.agencyOverlay = null;
    ...
    async start() {
        ...
        // Initialize Agency Overlay (Task 7.3)
        if (this.config.app) {
            this.agencyOverlay = new AgencyOverlay(this.config.app, null);
            this.world.addChild(this.agencyOverlay.container);
            console.log('[Engine] AgencyOverlay initialized');
        }
    ...
    handleFFIResponse(response) {
        if (response.type === 'functions') {
            ...
            // Route generic events (like PERSONA_ACTIVE)
            if (response.data && response.data.type === 'PERSONA_ACTIVE') {
                this._handlePersonaActive(response.data);
            }
        }
    }

    /**
     * Handle persona activation from backend.
     */
    _handlePersonaActive(data) {
        console.log('[Engine] Persona Active:', data.name);
        if (this.agencyOverlay) {
            this.agencyOverlay.handlePersonaActive(data);
        }
        
        // Dispatch global event for other components
        window.dispatchEvent(new CustomEvent('PERSONA_ACTIVE', { detail: data }));
    }

...
import aiohttp
import websockets
from vm_linux_bridge import HostBridge, QEMUBridge, VMLinuxBridge, WGPUBridge
...
class FileBrowser:
    ...
    async def send_js(self, js_code: str) -> dict | None:
...
async def create_bridge(backend: str, cdp_ws=None) -> VMLinuxBridge:
    ...
    if backend == 'host':
        print("Using HostBridge (subprocess execution)")
        return HostBridge()
    elif backend == 'qemu':
        print("Using QEMUBridge")
        bridge = QEMUBridge()

...
try:
    from .async_api import AsyncLMStudioClient, BatchProcessor, FeatureCache
...
class HybridPixelLLM:
    ...
    async def _call_lm_studio_async_with_retry(
        ...
        for attempt in range(max_attempts):
            ...
            except (TimeoutError, PixelLMConnectionError, PixelLMTimeoutError) as e:
                last_exception = e
                self._track_error(type(e).__name__)
    ...
    async def generate_batch_async(
        ...
        async def process_single_image(img_tensor: torch.Tensor) -> str | None:

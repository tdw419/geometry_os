...
def __getattr__(name):
    ...
    if name in ("PixelLLM", "VisionEncoder", "LanguageDecoder", "create_pixel_llm"):
        try:
            from .models import LanguageDecoder, PixelLLM, VisionEncoder, create_pixel_llm
            return locals()[name]
        except ImportError:
    ...
    if name in ("PixelLLMTrainer", "PixelLLMConfig"):
        try:
            from .trainer import PixelLLMConfig, PixelLLMTrainer
            return locals()[name]
        except ImportError:
    ...
    if name in ("generate_caption", "beam_search_decode"):
        try:
            from .inference import beam_search_decode, generate_caption
            return locals()[name]
        except ImportError:

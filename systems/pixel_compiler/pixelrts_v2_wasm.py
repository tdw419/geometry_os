...
class WASMCodeVisualizer:
    ...
    def get_opcode_category(self, opcode: int) -> WASMOpcodeCategory | None:
        """
        ...
        for i in range(len(data_array)):
            ...
            if opcode in self.CONTROL_FLOW_OPCODES:
                ...
                if opcode == 0x00:  # unreachable - high entropy
                    ...
                    entropy[i] = 200
                elif opcode in [0x0c, 0x0d] or opcode == 0x0f:  # br, br_if - medium
                    entropy[i] = 150
    ...
    def _get_default_entropy(self, opcode: int) -> int:
        ...
        if opcode == 0x00:  # unreachable - maximum entropy
            ...
            return 200
        elif opcode in [0x0c, 0x0d] or opcode in [0x0f, 0x10]:  # br, br_if - medium-high
            return 150
        elif opcode == 0x01:  # nop - zero entropy
            return 0
    ...
    def color_opcode(self, opcode: int, operand1: int = 0, operand2: int = 0,
                    entropy: int | None = None) -> tuple[int, int, int, int]:
        """
    ...
    def get_opcode_name(self, opcode: int) -> str | None:
        """

...
class ACLEntry:
    ...
    """
    tag: ACLTag
    qualifier: str | None  # User/group name (None for user_obj, group_obj, other, mask)
    permissions: int
    ...
    def matches(self, tag: ACLTag, qualifier: str | None = None) -> bool:
...
class ACL:
    ...
    def remove_entry(self, tag: ACLTag, qualifier: str | None = None) -> bool:
    ...
    def get_entry(self, tag: ACLTag, qualifier: str | None = None) -> ACLEntry | None:
...
class PermissionStore:
    ...
    def get_security_info(self, path: str) -> dict[str, Any] | None:
...
class PermissionManager:
    ...
    def get_security_info(self, path: str) -> FileSecurityInfo | None:
    ...
    def get_permissions(self, path: str) -> int | None:
    ...
    def chown(self, path: str, owner: str | None, group: str | None) -> tuple[bool, str]:
    ...
    def add_acl_entry(
        ...
        with self.lock:
            ...
            if not acl.add_entry(entry):
                return False, "ACL entry already exists"

            info["acl"] = acl.to_dict()
            self.store.set_security_info(path, info)
            return True, "ACL entry added"
    ...
    def remove_acl_entry(
        ...
        with self.lock:
            ...
            if not acl.remove_entry(tag, qualifier):
                return False, "ACL entry not found"

            info["acl"] = acl.to_dict() if acl.entries else None
            self.store.set_security_info(path, info)
            return True, "ACL entry removed"

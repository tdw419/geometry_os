...
from systems.pixel_compiler.vat_parser import VATInspector, VATNotFoundError, VATParser
from systems.rts_fuse.container import RTSContainer
from systems.rts_fuse.filesystem import FUSE, FuseOSError, RTSFilesystem
...
class FileInfo:
    ...
    mode: int
    mtime: int
    first_cluster: ClusterLocation | None = None
...
class InfiniteMapContainer(RTSContainer):
    ...
    def __init__(self, container_path: str, use_vat_parser: bool = True):
        ...
        # VAT storage
        self.vat: VisualAllocationTable | None = None
    ...
    def _try_parse_vat(self):
        ...
        try:
            ...
            print("    VAT v2 loaded successfully!")
            ...
            print("    VAT v2 not found, falling back to legacy manifest")
            print(f"    Reason: {e}")
            self._parse_embedded_manifest()
        except Exception as e:
            print(f"    VAT v2 parsing failed: {e}")
            print("    Falling back to legacy manifest")
            self._parse_embedded_manifest()
    ...
    def get_file_info_extended(self, filename: str) -> FileInfo | None:
        """
...
class InfiniteMapFilesystem(RTSFilesystem):
    ...
    def __init__(self, container_path: str, max_workers: int = 4, enable_writes: bool = False, cache_size: int = 10 * 1024 * 1024):
        ...
        print("[*] Infinite Map Filesystem initialized")

        if self.container.vat:
            print(f"    VAT: {len(self.container.vat.entries)} files")
        else:
            print("    VAT: Not available (using v1 mode)")
    ...
    def destroy(self, path):
        ...
        if self.wal is not None:
            ...
            try:
                self.wal.checkpoint()
                self.wal.close()
                print("    WAL checkpoint created and closed")
            except Exception as e:
    ...
    def get_streaming_reader(
        ...
        """
        from systems.pixel_compiler.infinite_map_streaming import StreamingConfig, StreamingReader
    ...
    def get_streaming_writer(
        ...
        """
        from systems.pixel_compiler.infinite_map_streaming import StreamingConfig, StreamingWriter
        config = config or StreamingConfig()
    ...
    def get_streaming_copier(
        ...
        """
        from systems.pixel_compiler.infinite_map_streaming import StreamingConfig, StreamingCopier
        config = config or StreamingConfig()
    ...
    def get_parallel_reader(
        ...
        """
        from systems.pixel_compiler.infinite_map_parallel import ParallelConfig, ParallelReader
    ...
    def get_parallel_writer(
        ...
        """
        from systems.pixel_compiler.infinite_map_parallel import ParallelConfig, ParallelWriter
        config = config or ParallelConfig()
...
def mount_infinite_map(
    ...
    print("*** Infinite Map OS FUSE v2 (VAT Integration) ***")

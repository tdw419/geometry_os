...
class LinkEntry:
    ...
    link_type: LinkType
    created_at: datetime
    inode: int | None = None  # For hard links, shared inode with target
...
class LinkTable:
    ...
    def get_link(self, link_path: str) -> LinkEntry | None:
        """Get link by path."""
        with self.lock:
    ...
    def get_inode_for_path(self, path: str) -> int | None:
        """Get inode for a path if it has one."""
        with self.lock:
            return self.path_to_inode.get(path)

    def get_inode(self, inode: int) -> InodeEntry | None:
...
class LinkResolver:
    ...
    def resolve(
        ...
        max_depth: int = None
    ) -> tuple[LinkState, str | None, list[str]]:
        """
    ...
    def get_link_info(self, path: str) -> LinkInfo | None:
...
class LinkManager:
    ...
    def readlink(self, path: str) -> tuple[bool, str | None]:
        """
...
def resolve_link(path: str, table_path: str = "links.json") -> str | None:

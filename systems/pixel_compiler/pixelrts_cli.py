...
def cmd_transpile(args):
    ...
    try:
        ...
        if HAS_CLICK:
            click.secho("[✓] Transpilation Complete!", fg='green', bold=True)
...
def _boot_multiple(args, input_paths):
    ...
    if args.verbose:
        ...
        if args.primary:
            ...
            print("Shutdown order: helpers stop first, primary stops last")
    ...
    try:
        ...
        if args.background:
            if not args.quiet:
                ...
                print(f"Use 'kill {result.pid}' or Ctrl+C to stop.")
...
def cmd_patch(args):
    ...
    from systems.pixel_compiler.delta_patch import PatchError, apply_delta_patch
...
def cmd_commit(args):
    ...
    from systems.pixel_compiler.boot import MultiBootManager
    from systems.pixel_compiler.boot.snapshot_exporter import ExportStage, SnapshotExporter
    ...
    if not str(output_path).endswith('.rts.png'):
        print("Error: Output path must end with .rts.png", file=sys.stderr)
        return 1
    ...
    try:
        ...
        if not args.quiet:
            print("Starting commit operation...")
        ...
        if result.success:
            ...
            if args.verbose:
                ...
                if result.identity_preserved:
                    ...
                    print("  Identity preserved: No (WARNING: name or ports changed)")

                # Network reconnection status
                if result.network_reconnected is None:
                    print("  Network reconnected: Using fallback mode")
                elif result.network_reconnected:
                    print("  Network reconnected: Yes")
                else:
                    print("  Network reconnected: No")
            ...
            if HAS_CLICK:
                click.secho("[FAIL] Failed to restore snapshot", fg='red', file=sys.stderr)
            else:
                print("[FAIL] Failed to restore snapshot", file=sys.stderr)
...
def cmd_mesh_discover(args):
    ...
    async def run_discovery():
        ...
        print("Broadcasting discovery beacon...")

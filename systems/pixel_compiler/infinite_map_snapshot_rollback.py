...
class RestorePoint:
    ...
    partial_snapshot_id: str | None = None
...
class SnapshotStore:
    ...
    def get_by_name(self, name: str) -> SnapshotMetadata | None:
    ...
    def list(self, state: SnapshotState | None = None) -> list[SnapshotMetadata]:
...
class SnapshotManager:
    ...
    def get(self, name_or_id: str) -> SnapshotInfo | None:
        ...
        if metadata:
            ...
            try:
                with open(metadata.vat_path) as f:
                    vat_data = json.load(f)
                    info.vat_entries = len(vat_data.get("entries", {}))
                    info.clusters_captured = len(vat_data.get("clusters", {}))
                    info.total_data_size = vat_data.get("total_size", 0)
    ...
    def verify(self, name_or_id: str) -> tuple[bool, builtins.list[str]]:
        """
    ...
    def _restore_vat(self, snapshot_vat_path: str) -> bool:
        ...
        try:
            if hasattr(self.vat, "load_from_file"):
                ...
                with open(snapshot_vat_path) as f:
                    data = json.load(f)

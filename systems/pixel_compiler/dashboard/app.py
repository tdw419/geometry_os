...
from pattern_detector import EdgeDetectionResult, PatternDetector
...
def _register_callbacks(app: Dash):
    ...
    def update_quick_stats(image_data: dict[str, Any] | None):
    ...
    def run_analysis(n_clicks: int, image_data: dict | None, methods: list[str],
    ...
    def update_frequency_graph(results: dict | None):
...
def run_dashboard(host: str = "127.0.0.1", port: int = 8050, debug: bool = True):
    ...
    print(f"\n{'='*60}")
    print("PixelRTS Pattern Analysis Dashboard")
    print(f"{'='*60}")

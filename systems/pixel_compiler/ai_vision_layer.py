...
from systems.pixel_compiler.pixel_native_types import GUIState, WidgetInfo, WidgetType
...
class BaseVLM(ABC):
    ...
    @abstractmethod
    def ground_intent(self, intent: str, pixels: np.ndarray) -> tuple[int, int] | None:
...
class AIVisionLayer:
    ...
    def find_widget(self, pixels: np.ndarray, widget_type: WidgetType,
                    label: str | None = None) -> WidgetInfo | None:

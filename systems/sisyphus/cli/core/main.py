...
def status(ctx: click.Context):
    ...
    if ctx.obj.get('json'):
        ...
        click.echo(f"=" * 40)
        click.echo(f"Tasks: {status_data['tasks']['pending']} pending, "
...
@click.option('--state', '-s', type=click.Choice(['pending', 'complete', 'failed', 'all']),
              default='pending', help='Filter by state')
@click.pass_context
def tasks(ctx: click.Context, limit: int, state: str):
    """List tasks by state."""
    ...
    if ctx.obj.get('json'):
        click.echo(json_output({
            "state_filter": state,
            "count": len(filtered),
            "total_matching": len([t for t in all_tasks if state == 'all' or t.state == state_map.get(state)]),
        ...
        click.echo("-" * 60)
        for t in filtered:
            state_char = {'pending': '[ ]', 'complete': '[x]', 'failed': '[!]', 'in_progress': '[→]'}.get(t.state.value, '[?]')
...
def run(ctx: click.Context, timeout: int):
    ...
    if ctx.obj.get('json'):
        click.echo(json_output({
            "status": "completed",
            "task": task_to_dict(updated) if updated else None
        }))
    else:
        if updated:
...
def repl(ctx: click.Context):
    ...
    while True:
        ...
        try:
            if cmd == 'status':
                status_data = daemon_status_dict(daemon)
                click.echo(f"Pending: {status_data['tasks']['pending']}, "

...
COGNITIVE_GOAL_TEMPLATES = [
    ...
    },
    {
        ...
        "priority_base": 0.95,
        "rationale_template": "Critical brain fitness collapse (Fitness: {fitness:.2f}). Emergency weight sector restoration required."
    }
]

# Geometric/Native Substrate goal templates - focus on core GPU optimizations
GEOMETRIC_GOAL_TEMPLATES = [
    {
        "goal_type": "wgsl_optimization",
        "priority_base": 0.9,
        "rationale_template": "WGSL shader {shader_name} shows performance drift. Optimize compute dispatch or memory access patterns."
    },
    {
        "goal_type": "hilbert_remap",
        "priority_base": 0.85,
        "rationale_template": "Hilbert curve locality degraded in sector {sector}. Re-map weight atlas for better GPU cache coherence."
    },
    {
        "goal_type": "atlas_compression",
        "priority_base": 0.75,
        "rationale_template": "Brain atlas has sparse regions. Compact weight storage to reduce texture memory bandwidth."
    },
    {
        "goal_type": "attention_parallelization",
        "priority_base": 0.88,
        "rationale_template": "Self-attention layer {layer} could benefit from FlashAttention-style tiling for better GPU utilization."
    },
    {
        "goal_type": "embedding_quantization",
        "priority_base": 0.7,
        "rationale_template": "Token embeddings could be quantized to int8 without quality loss, reducing memory footprint by 50%."
    },
    {
        "goal_type": "kv_cache_optimization",
        "priority_base": 0.82,
...
class GoalSynthesizer:
    ...
    def synthesize_from_health_score(
        ...
        # Inject geometric goals periodically (20% chance)
        import random
        if random.random() < 0.2:
            template = random.choice(GEOMETRIC_GOAL_TEMPLATES)
            goal = AutonomousGoal(
                goal_id=self._generate_goal_id(),
                goal_type=template["goal_type"],
                target="systems/visual_shell/wgsl/",
                rationale=template["rationale_template"],
                priority=template["priority_base"],
                entropy_score=0.5,
                created_at=created_at,
                metrics={"source": "geometric_injection"}
            )
            # Convert to EntropySpot-like for batch synthesis
            from dataclasses import dataclass
            @dataclass
            class GeometricSpot:
                score: float
                entropy_type: str = "geometric"
                file_path: str = "systems/visual_shell/wgsl/"
                function_name: str = ""
                metrics: dict = field(default_factory=dict)
            selected_spots.append(GeometricSpot(
                score=template["priority_base"],
                metrics={"goal_template": template}
            ))

...
impl SynapticDaemonBridge {
    ...
    pub async fn interpret_intent(
        ...
        if !response.status().is_success() {
            return Err(format!(
                "Synaptic daemon returned error: {}",
                response.status()
            ));
        }
        ...
        log::info!(
            "🧠 Daemon response: status={}, confidence={}, fallback={}",
            daemon_response.status,
            daemon_response.confidence,
            daemon_response.fallback_used
        );
    ...
    fn parse_commands(
        ...
        for cmd in commands {
            ...
            let params = cmd.get("params").ok_or("Command missing 'params' field")?;
            ...
            let morphology_cmd = match cmd_type {
                ...
                "Highlight" => {
                    ...
                    let radius = params
                        .get("radius")
                        .and_then(|v| v.as_f64())
                        .ok_or("Highlight missing 'radius'")?
                        as f32;
                    let color = params
                        .get("color")
                        .and_then(|v| v.as_array())
                ...
                "SpawnEffect" => {
                    ...
                    let scale = params
                        .get("scale")
                        .and_then(|v| v.as_f64())
                        .ok_or("SpawnEffect missing 'scale'")?
                        as f32;
    ...
    pub async fn test_connection(&self) -> Result<(), String> {
        log::info!(
            "🧠 Testing connection to synaptic daemon at {}",
            self.daemon_url
        );

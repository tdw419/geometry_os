...
impl EntropyAnalyzer {
    ...
    pub fn estimate_entropy_from_compression(
        &self,
        original_size: usize,
    ...
    pub fn update(&mut self, entropy: f32) -> bool {
        ...
        if entropy > self.realignment_threshold {
            log::warn!(
                "🎨 Aesthetic Degradation Detected: Entropy={:.2} (threshold={:.2})",
                entropy,
                self.realignment_threshold
            );
...
impl AestheticParams {
    ...
    pub fn to_llm_prompt(&self, current_entropy: f32, target_entropy: f32) -> String {
        format!(
            ...
            current_entropy, target_entropy, self.chaos, self.temperature, self.saturation
        )
    }
}

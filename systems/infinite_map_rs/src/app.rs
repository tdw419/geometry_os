...
use smithay::{
    backend::input::{
        AbsolutePositionEvent, Axis, ButtonState, InputBackend, InputEvent as SmithayInputEvent,
        KeyboardKeyEvent, MouseButton, PointerAxisEvent, PointerButtonEvent,
    },
    backend::renderer::utils::with_renderer_surface_state,
    reexports::wayland_server::{protocol::wl_surface::WlSurface, Resource},
    utils::Size,
    wayland::compositor::{with_states, Damage, SurfaceAttributes},
...
use crate::visual_shell::{DaemonId, FrequencyBand, VisualShell};
...
pub struct InfiniteMapApp<'a> {
    ...
    pub visual_shell: Option<VisualShell>,
    ...
    pub riscv_executor:
        Option<std::sync::Arc<std::sync::Mutex<crate::riscv_executor::RiscvExecutor>>>,
    ...
    pub cognitive_manager:
        Option<std::sync::Arc<crate::cognitive::entity_manager::CognitiveEntityManager>>,
    pub cognitive_runtimes:
        std::collections::HashMap<String, crate::cognitive::ace_runtime::ACERuntime>,
    ...
    pub evolution_terrain_bridge:
        Option<crate::evolution_terrain_bridge::SharedEvolutionTerrainBridge>,
...
impl<'a> InfiniteMapApp<'a> {
    pub fn new(
        ...
        let mut app = InfiniteMapApp {
            ...
            // Phase 42: Map Loader
            map_loader: Some(crate::map_loader::MapLoader::new(std::path::PathBuf::from(
                "systems/builder/map/manifest.json",
            ))),
            ...
            visual_cortex: crate::visual_cortex::VisualCortex::new(
                "/home/jericho/zion/projects/geometry_os/geometry_os/systems/vision/retina.rts.png",
            ),
            ...
            filesystem_hilbert_manager: Some(
                crate::filesystem_hilbert::FilesystemHilbertManager::new(std::path::PathBuf::from(
                    "/home/jericho/zion/projects/geometry_os/geometry_os",
                )),
            ),
    ...
    pub fn set_visual_command_rx(
        &mut self,
        rx: tokio::sync::mpsc::Receiver<crate::glass_ram::bridge::VisualCommand>,
    ) {
    ...
    pub fn import_source_crystal(&mut self) {
        ...
        match importer.import_crate((1200, 3400)) {
            Ok(project) => {
                log::info!(
                    "✅ Successfully crystallized project: {} tiles",
                    project.tiles.len()
                );
    ...
    pub fn initialize_memory_mapper(&mut self) {
        ...
        if let Err(e) = mapper.map_memory_regions() {
            ...
            log::info!(
                "✅ Mapped memory regions (Heap found: {})",
                mapper.heap_region.is_some()
            );
        }
    ...
    pub fn initialize_module_manager(&mut self) {
        ...
        let vat_registry =
            std::sync::Arc::new(std::sync::Mutex::new(crate::hot_swap::VatRegistry::new(
                std::path::PathBuf::from("systems/health/state_vats"),
            )));
        ...
        if test_module_path.exists() {
            ...
            match manager.load_module(&test_module_path) {
                Ok(vat_id) => log::info!(
                    "🔋 Phase 40.5: Test module loaded successfully ({})",
                    vat_id.as_str()
                ),
                ...
                for cmd in commands {
                    match cmd {
                        ...
                        139 => {
                            ...
                            let new_pos = (text_engine.local_stats.cursor_pos + 80)
                                .min(text_engine.local_stats.capacity - 1);
                            text_engine.extend_selection(&queue, new_pos);
                        },
                        140 => {
                            // Shift+Left
                        ...
                        141 => {
                            ...
                            let new_pos = (text_engine.local_stats.cursor_pos + 1)
                                .min(text_engine.local_stats.capacity - 1);
                            text_engine.extend_selection(&queue, new_pos);
                        },
                        _ => {
                    ...
                    match cmd {
                        142 => {
                            ...
                            if !selected_text.is_empty() {
                                ...
                                log::info!(
                                    "📋 Copied {} chars to clipboard (internal)",
                                    selected_text.len()
                                );
                        ...
                        144 => {
                            ...
                            if !selected_text.is_empty() {
                                log::info!(
                                    "📋 Cut {} chars to clipboard (internal)",
                                    selected_text.len()
                                );
            ...
            if let Some(commands) = self.input_manager.get_undo_commands() {
                for cmd in commands {
                    match cmd {
                        ...
                        146 => {
                            // Ctrl+Y = Redo (Ctrl+Shift+Z also maps here)
    ...
    pub fn world_to_grid_index(&self, world_x: f32, world_y: f32) -> Option<u32> {
        ...
        if local_x >= 0.0
            && local_x < grid_columns as f32
            && local_y >= 0.0
            && local_y < grid_rows as f32
        {
                    ...
                    149 => {
                        ...
                        if path.exists() {
                            ...
                            if path2.exists() {
                                if let Some(text_engine) = &mut self.renderer.text_engine {
                                    if let Err(e) =
                                        text_engine.enter_hex_mode(&device, &queue, &path2)
                                    {
                                        log::error!("Failed to enter hex mode: {}", e);
                                    } else {
                                        eprintln!(
                                            "🧬 GOD MODE ENGAGED: Editing alpine_bootable.rts"
                                        );
                                    }
                                }
                            }
                        ...
                        let prompt_request =
                            if let Some(text_engine) = &mut self.renderer.text_engine {
                                if let Some(prompt) = text_engine.get_hex_neural_prompt() {
                                    ...
                                    let offset = text_engine
                                        .hex_editor
                                        .as_ref()
                                        .map(|h| h.cursor_byte_pos as usize)
                                        .unwrap_or(0);
                                    Some((prompt, offset))
                                } else {
                                    None
                                ...
                                rt.spawn(async move {
                                    ...
                                    if let Some(client) = zai_client {
                                            ...
                                            .await
                                        {
                                            Ok(analysis) => {
                                                log::info!(
                                                    "🧠 Neural Analysis Received ({} chars)",
                                                    analysis.len()
                                                );
                                                if let Err(e) = tx.send((offset, analysis)).await {
                                                    log::error!(
                                                        "Failed to send introspection result: {}",
                                                        e
                                                    );
                                                }
                                            },
                                            Err(e) => {
                                                log::error!("🧠 Neural Consult Failed: {}", e)
                                            },
                                        }
                                    } else {
                                        log::error!(
                                            "EvolutionManager lock failed or ZAiClient missing"
                                        );
                                    }
                                });
                            } else {
                ...
                match cmd {
                    154 => {
                        ...
                        let source_path = if let Some(text_engine) = &self.renderer.text_engine {
                            if text_engine.is_hex_mode() {
                                text_engine.hex_editor.as_ref().and_then(|h| {
                        ...
                        if let Some(path) = source_path {
                                ...
                                || path.to_string_lossy().ends_with(".rts.png")
                            {
                                ...
                                eprintln!(
                                    "⚠️ Phase 42: Not a compilable tile (need .rts.png or .wgsl)"
                                );
                            }
                        } else {
                ...
                match cmd {
                    155 => {
                        ...
                        if self.profiler_enabled {
                            log::info!(
                                "🔍 Phase 44: Profiler ENABLED - polling every {:?}",
    ...
    fn compile_tile_at_path(&mut self, path: &std::path::Path) {
        ...
        eprintln!(
            "🔧 Phase 43: Starting compilation of {:?}...",
    ...
    fn ensure_compiler_dispatcher(&mut self) {
        if self.compiler_dispatcher.is_none() {
            ...
            if let Err(e) = dispatcher_mut.initialize() {
                log::error!(
                    "❌ Phase 42: Failed to initialize compiler dispatcher: {}",
                    e
                );
    ...
    pub fn log_gpu_info(&self) {
        log::info!(
            "GPU: {} {}",
            self.gpu_caps.vendor_name,
            self.gpu_caps.device_name
        );
    ...
    fn execute_code(&mut self, code: &str) {
        ...
        else if code.contains("@fragment")
    ...
    fn crystallize_text_to_rts(&mut self) {
        ...
        if !converter_path.exists() {
            log::error!(
                "pixelrts_v2_converter.py not found at: {}",
    ...
    fn spawn_crystallized_tile(&mut self, rts_path: &std::path::Path, timestamp: u64) {
        ...
        log::info!(
            "✅ Crystallized tile spawned at ({}, {}): {}",
            tile_x,
            tile_y,
            tile_name
        );
        eprintln!(
            "✅ Spawned crystallized tile at ({}, {}): {}",
            tile_x, tile_y, tile_name
        );
    }
            ...
            if crate::text_engine::TextEngine::has_spatial_snapshot(tile_x, tile_y) {
                match text_engine.load_spatial_snapshot(&queue, tile_x, tile_y) {
                    Ok(snapshot) => {
                        ...
                        log::info!(
                            "📍 Restored spatial snapshot: tile ({}, {})",
                            tile_x,
                            tile_y
                        );
                    },
    ...
    pub fn update_camera_for_text_cursor(&mut self) {
        if let Some(text_engine) = &self.renderer.text_engine {
            ...
            log::debug!(
                "🎥 Camera following Crystallized Cursor at ({}, {})",
                world_x,
                world_y
            );
        }
        ...
        match proc.start() {
            Ok(_) => {
                ...
                if let Some(win) = self.window_manager.get_window_mut(window_id) {
                    ...
                    log::info!(
                        "✅ Created QEMU Window ID {} at (50, 50) with size 640x480",
                        window_id
                    );
    ...
    pub fn load_module(
        &mut self,
        path: &std::path::Path,
    ) -> Result<crate::hot_swap::VatId, String> {
        if let Some(ref manager) = self.module_manager {
    ...
    pub fn unload_module(&mut self, vat_id: &crate::hot_swap::VatId) -> Result<(), String> {
        if let Some(ref manager) = self.module_manager {
            let mut mgr = manager.lock().map_err(|e| format!("Lock error: {}", e))?;
            mgr.unload_module(vat_id)
                .map_err(|e| format!("Unload error: {}", e))
        } else {
            Err("ModuleManager not initialized".to_string())
        }
    }
    ...
    pub fn initialize_cartridge_texture_manager(&mut self) {
        ...
        let cartridge_texture_manager =
            CartridgeTextureManager::new(device, queue, bind_group_layout, sampler);
    ...
    pub fn initialize_riscv_executor(
        &mut self,
        rts_path: Option<String>,
        ...
        if let Some(path) = rts_path {
            ...
            if let Err(e) = res {
                ...
                if let Some(dtb) = dtb_path {
                    ...
                    log::info!(
                        "💾 Loading RISC-V DTB from: {} to offset 0x{:x}",
                        dtb,
                        dtb_offset
                    );
    ...
    pub fn initialize_riscv_linux_vm(
        &mut self,
        ...
        if let Some(path) = kernel_path {
            ...
            if let Some(dtb_file) = dtb_path {
                match std::fs::read(&dtb_file) {
                    ...
                    Err(e) => {
                        log::warn!(
                            "⚠️  Failed to read DTB file {}, generating simple DTB instead: {}",
                            dtb_file,
                            e
                        );
                        let dtb = crate::riscv_linux_vm::RiscVLinuxVM::generate_simple_dtb(
                            memory_size as u64,
                            "console=ttyS0 earlycon=sbi root=/dev/ram0",
                        );
        ...
        if let Some(ref mut vm) = self.riscv_linux_vm {
            ...
            unsafe {
                ...
                if FRAME_COUNTER % 60 == 0 {
                    ...
                    log::info!(
                        "📟 RISC-V VM: PC=0x{:08x}, Instructions={}",
                        vm.read_pc(),
            ...
            if !vm.console_output.is_empty() {
                ...
                if let Some(window) = self
                    ...
                    .get_windows_mut()
                    .iter_mut()
                    .find(|w| w.title == "RISC-V Linux Console")
                {
    ...
    pub fn launch_multiple_vms(&mut self) {
        ...
        let configs = vec![
            crate::multi_vm_manager::VmInstanceConfig::with_test_kernel(
                0,
                "VM-0-Alpine".to_string(),
            ),
            crate::multi_vm_manager::VmInstanceConfig::with_test_kernel(
                1,
                "VM-1-Alpine".to_string(),
            ),
            crate::multi_vm_manager::VmInstanceConfig::with_test_kernel(
                2,
                "VM-2-Alpine".to_string(),
            ),
            crate::multi_vm_manager::VmInstanceConfig::with_test_kernel(
                3,
                "VM-3-Alpine".to_string(),
            ),
        ];
    ...
    fn initialize_live_memory_textures(&mut self) {
        if let (Some(ref mapper), Some(ref mut manager)) = (
            &self.memory_texture_mapper,
            &mut self.memory_texture_manager,
        ) {
            ...
            if let Some(ref heap_region) = mapper.heap_region {
                ...
                match manager.create_live_texture(
                    ...
                    Ok(_) => {
                        ...
                        if let Some(window) =
                            self.window_manager.get_windows_mut().get_mut(window_id)
                        {
    ...
    pub fn initialize_memory_raycaster(&mut self) {
        ...
        let raycaster =
            crate::memory_raycaster::MemoryRaycaster::new(self.camera.clone(), width, height);
        ...
        if let Some(bridge_arc) = &self.evolution_terrain_bridge {
            if let Ok(bridge) = bridge_arc.lock() {
                if let Some(ref gpu) = bridge.gpu_resources {
                    ...
                    {
                        self.evolution_terrain_bind_group_1 =
                    ...
                    if let Some(ref layout_2) = self.renderer.evolution_terrain_bind_group_layout_2
                    {
                        if let Some(ref lighting_buffer) = self.renderer.evolution_lighting_buffer {
                            self.evolution_terrain_bind_group_2 =
                                Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
    ...
    pub fn render_cartridges(&mut self) {
        ...
        for entry in new_cartridges {
            log::info!(
                "🎨 Rendering new cartridge: {} (Gen {}) at ({}, {})",
                entry.id,
                entry.generation,
                entry.spawn_x,
                entry.spawn_y
            ...
            self.window_manager
                .set_window_type(window_id, crate::window::WindowType::EvolutionZone);
            ...
            self.window_manager
                .set_window_decorations(window_id, tile_decorations);

            // Phase 35.9.1: Mark window as having cartridge texture
            if texture_loaded {
                self.window_manager
                    .set_window_has_cartridge_texture(window_id, &entry.id);
            }
    ...
    fn emit_cartridge_boot_resonance(
        &self,
        entry: &crate::cartridge_registry::CartridgeEntry,
        size: usize,
    ) -> Result<(), String> {
    ...
    fn launch_cartridge_vm(
        ...
        let console_window_id = self.window_manager.create_window(
            ...
            100.0,
            100.0, // Position (will be adjusted)
            400.0,
            300.0, // Size
        );
    ...
    pub fn submit_evolution_genome(
        &mut self,
        genome: crate::evolution_terrain_bridge::EvolutionGenome,
    ) {
        if let Some(bridge_arc) = &self.evolution_terrain_bridge {
            if let Ok(mut bridge) = bridge_arc.lock() {
                bridge.submit_genome(genome);
    ...
    pub fn initialize_vm(&mut self) {
        ...
        match VirtualMachine::new() {
            Ok(vm) => {
                ...
                if let Some(clipboard_manager) = self.get_clipboard_manager() {
                    let terminal_clipboard =
                        crate::clipboard_manager::TerminalClipboard::new(clipboard_manager.clone());
                        ...
                        Ok((framebuffer_data, width, height)) => {
                            ...
                            if let Some(vm_texture_manager) = &mut self.vm_texture_manager {
                                if let Some(window_id) = vm.window_id() {
                                    ...
                                    if let Err(e) = vm_texture_manager
                                        .update_vm_texture(window_id, &rgba_data, width, height)
                                    {
    ...
    pub fn try_connect_qemu_shared_memory(&mut self) {
        ...
        let vm_id_to_connect = if available_vms.contains(&target_vm.to_string()) {
            ...
            log::info!(
                "ℹ️ 'default' VM not found, auto-selecting: {}",
                available_vms[0]
            );
        ...
        if let Some(vm_id) = vm_id_to_connect {
            match crate::qemu::SharedMemoryBridge::new(&vm_id) {
                Ok(bridge) => {
                    log::info!(
                        "✅ QEMU Shared Memory Bridge connected (VM: {}, Size: {} bytes)",
                        vm_id,
                        bridge.size()
                    );
                    ...
                    tokio::spawn(async move {
                        ...
                        match crate::qemu::QmpClient::connect(&qmp_id).await {
                            ...
                            Err(e) => {
                                log::warn!(
                                    "⚠️ QMP Connection failed: {} (Is the VM running with -qmp?)",
                                    e
                                );
                            },
                        }
    ...
    pub fn set_evolution_manager(
        &mut self,
        evolution_manager: std::sync::Arc<std::sync::Mutex<EvolutionManager>>,
    ) {
        ...
        if let Some(text_engine) = &mut self.renderer.text_engine {
            if let Some(hex_editor) = &mut text_engine.hex_editor {
                ...
                log::info!(
                    "🧠 Phase 40.3: Hex Tensor Editor neural link established via EvolutionManager"
                );
            }
        }
    }
    ...
    pub fn initialize_tool_manager(&mut self) {
        ...
        tool_manager.register_adapter(std::sync::Arc::new(
            crate::tool_adapter::BpftraceAdapter::new(),
        ));
        ...
        if let Some(ref mut manager) = self.terminal_clone_manager {
            match manager.create_terminal(req.rows as u16, req.cols as u16, &req.shell) {
                Ok(pty_id) => {
                    ...
                    let mut tile = crate::terminal_tile::TerminalTile::new(
                        tile_idx,
                        format!("Terminal {}", pty_id),
                        req.cols,
                        req.rows,
                    );
    ...
    pub fn update_terminal_tiles(&mut self) {
        ...
        #[cfg(feature = "hypervisor")]
        {
            ...
            for req in resizes {
                ...
                if let Some(ref mut manager) = self.terminal_clone_manager {
                    ...
                    let _ = manager.resize_terminal(
                        req.tile_id as usize,
                        req.rows as u16,
                        req.cols as u16,
                    );
                }
        ...
        for tile in &mut self.terminal_tiles {
            if tile.needs_render {
                ...
                if let (Some(window_id), Some(ref mut vtm), Some(ref geo_vm)) = (
                    tile.window_id,
                    &mut self.vm_texture_manager,
                    &self.geometric_vm,
                ) {
    ...
    fn handle_source_city_interaction(
        &mut self,
        world_x: f32,
        world_y: f32,
        is_press: bool,
        button: Option<smithay::backend::input::MouseButton>,
    ) -> bool {
            ...
            if button == Some(smithay::backend::input::MouseButton::Right) {
                if let Some(loader) = &self.source_city_loader {
                    if let Some(tile) = loader.find_tile_at(world_x, world_y, 0.5) {
                        ...
                        if tile.path.ends_with(".iso")
                            || tile.path.ends_with(".kernel")
                            || tile.path.ends_with(".bin")
                            || tile.path.ends_with(".img")
                        {
                            ...
                            if self.alpine_vm.is_none() {
                                ...
                                self.alpine_vm =
                                    Some(std::sync::Arc::new(std::sync::Mutex::new(manager)));

                                // Create window logic embedded here for now
                                self.alpine_vm_window_id =
                                    Some(self.window_manager.create_demo_window(
                ...
                if let Some(district) = target_district {
                    ...
                    if let Ok(mut rs) = self.runtime_state.lock() {
                        rs.pending_synaptic_actions
                            .push(format!("fs_move {} {}", path, district));
                    }
                }
    ...
    pub fn handle_diagnostic_click(&mut self, world_x: f32, world_y: f32) -> bool {
        if let Some(id) = self.diagnostic_window_id {
            if let Some(window) = self.window_manager.get_windows().get(id) {
                ...
                if world_x >= window.x
                    && world_x <= window.x + window.width
                    && world_y >= window.y - 30.0
                    && world_y <= window.y + window.height
                {
                    self.diagnostic_overlay.toggle_expansion();
                    log::info!(
                        "🔧 Diagnostic Overlay toggled: expanded={}",
                        self.diagnostic_overlay.expanded
                    );
                    return true;
                }
            }
    ...
    pub fn handle_process_tile_interaction(
        &mut self,
        screen_x: f32,
        screen_y: f32,
        world_x: f32,
    ...
    fn initialize_visual_shell(&mut self) {
        ...
        match VisualShell::new() {
            ...
            Err(e) => {
                log::warn!(
                    "Failed to initialize Visual Shell (tokens not found?): {}",
                    e
                );
            },
        }
    ...
    pub fn initialize_antigravity_bridge(&mut self) {
        ...
        if let Some((data, width, height, metadata_str)) = watcher.check_and_load() {
            ...
            let display_text = if let Ok(json) =
            ...
            self.renderer
                .set_background_texture(simulator.get_current_view());
        ...
        if let Some(manager_arc) = &self.evolution_manager {
            if let Ok(manager) = manager_arc.lock() {
                if let Some(neural_texture) = manager.get_neural_texture_view() {
                    ...
                    log::info!(
                        "✅ Thought Renderer initialized with Neural Texture and Synaptic Layer"
                    );
                }
            }
        }
    ...
    fn initialize_audio_manager(&mut self) {
        ...
        match crate::audio_manager::AudioManager::new(config) {
            Ok(mut audio_manager) => match audio_manager.initialize() {
                Ok(_) => {
    ...
    pub fn update_evolution_display(&mut self) {
        if let (Some(manager_arc), Some(window_id)) =
            (&self.evolution_manager, self.evolution_window_id)
        {
    ...
    pub fn get_memory_texture_manager(
        &self,
    ) -> Option<&crate::memory_texture_manager::MemoryTextureManager> {
    ...
    pub fn handle_memory_artifact_click(
        &mut self,
        screen_x: f32,
        screen_y: f32,
    ) -> (Option<usize>, Option<usize>) {
        if let (Some(raycaster), Some(artifact_manager)) =
            (&self.memory_raycaster, &self.memory_artifact_manager)
        {
            let artifacts = artifact_manager.get_artifacts();

            if let (Some(artifact_id), Some(address)) =
                raycaster.handle_click(screen_x, screen_y, artifacts)
            {
                log::info!(
                    "🧠 Clicked on memory artifact {} at address 0x{:x}",
                    artifact_id,
                    address
                );
    ...
    fn show_hex_dump(&mut self, address: usize) {
        ...
        let hex_dump = if let Some(ref mapper) = self.memory_texture_mapper {
            match mapper.read_memory(address, 256) {
                Ok(data) => self.format_hex_dump(&data, address),
                Err(e) => {
    ...
    pub fn create_memory_window<P: AsRef<std::path::Path>>(
        ...
        if let Some(manager) = &mut self.memory_texture_manager {
            ...
            log::info!(
                "Created memory window {} for brick {}",
                window_id,
                brick_path.display()
            );
            Some(window_id)
    ...
    fn load_map_textures(&mut self) {
        ...
        let entries: Vec<((i32, i32), crate::map_loader::BrickEntry)> = self
            .map_loader
            .as_ref()
            .unwrap()
            .entries
                ...
                if let Some(cognitive_manager) = &self.cognitive_manager {
                    ...
                    if meta_path.exists() {
                        if let Ok(file) = std::fs::File::open(&meta_path) {
                            if let Ok(metadata) = serde_json::from_reader::<
                                _,
        ...
        if let Some(ref mut artifact_manager) = self.memory_artifact_manager {
            ...
            if new_artifacts_count > 0 {
                ...
                log::info!(
                    "✨ Created {} new 3D artifacts for map",
                    new_artifacts_count
                );
            }
        }
    ...
    pub fn update(&mut self, state: &mut GeometryCompositorState) {
        ...
        */
        ...
        if let Some((data, width, height, metadata_str)) = update_data {
            log::info!(
                "↺ Antigravity Bridge: Reloading Shared Memory Texture ({}x{})",
                width,
                height
            );
                        ...
                        if let Some(filename) = json["Attention_Tile"].as_str() {
                            ...
                            if let Some(id) = target_id {
                                if let Some(ref mut am) = self.memory_artifact_manager {
                                    log::info!(
                                        "⚡ Proprioception: Pulsing Attention Tile '{}' (ID: {})",
                                        filename,
                                        id
                                    );
        ...
        if let Some(ref mtm) = self.memory_texture_manager {
            self.diagnostic_overlay
                .update_system_health(mtm.get_total_vram_usage());
        }
                    ...
                    if let Some(sync) = manager.fetch_daemon_sync() {
                        ...
                        for reg in sync.registered {
                            ...
                            let band = match reg.preferred_band {
                                ...
                                DaemonFrequencyBand::Custom(millihz) => {
                                    FrequencyBand::Custom(millihz as f32 / 1000.0)
                                },
                            };
                        ...
                        for update in sync.updates {
                            if let Ok(data) =
                                serde_json::from_value::<Vec<f32>>(update.data.clone())
                            {
                                let _ = shell
                                    .update_daemon_data(DaemonId::from_name(&update.name), data);
                            }
                        }

                        // Update Amplitudes
                        for amp in sync.amplitude_updates {
                            let _ = shell.set_daemon_amplitude(
                                DaemonId::from_name(&amp.name),
                                amp.amplitude,
                            );
                        }
                    }
        ...
        if let Some(ref mut renderer) = self.thought_renderer {
            ...
            if let Some(token_data) = token_data_clone {
                ...
                renderer.update_with_token_visualization(
        ...
        if let (Some(ref mut feedback), Some(output_texture)) = (
            &mut self.visual_feedback,
        ...
        if let Some(ref watcher) = self.antigravity_watcher {
            if let Some((data, width, height, _)) = watcher.check_and_load() {
                log::info!(
                    "⚡ Tectonic Switch: Updating Ground Truth Substrate ({}x{})",
                    width,
                    height
                );
            ...
            while let Ok((address, analysis)) = rx.try_recv() {
                ...
                if let Some(text_engine) = &mut self.renderer.text_engine {
                    if text_engine.is_hex_mode() {
                        ...
                        log::info!(
                            "✨ Applied Neural Annotation to Hex Editor at 0x{:04X}",
                            address
                        );
                    }
                }
        ...
        if let Some(rx) = &mut self.visual_command_rx {
            while let Ok(cmd) = rx.try_recv() {
                match cmd {
                    crate::glass_ram::bridge::VisualCommand::AllocTower {
                        id,
                        hilbert_index,
                        height,
                        color: _,
                    ...
                    crate::glass_ram::bridge::VisualCommand::Pulse {
                        id,
                        intensity,
                        duration_ms,
                    } => {
                        log::info!(
                            "Bridge Command: Pulse id={} intensity={} duration={}",
                            id,
                            intensity,
                            duration_ms
                        );
                    ...
                    crate::glass_ram::bridge::VisualCommand::MoveMouse { x, y } => {
                        ...
                        let screen_pos =
                            self.camera
                                .world_to_screen(x, y, screen_width, screen_height);
                    ...
                    crate::glass_ram::bridge::VisualCommand::MouseClick { button, down } => {
                        ...
                        self.input_manager
                            .handle_pointer_button(state, btn, state_input, 0, 0);
                    },
                }
            }
        }
        ...
        if let (Some(console), Some(window_id)) =
            (&mut self.neural_console, self.neural_console_window_id)
        {
            if let Some(manager) = &mut self.vm_texture_manager {
                // Reuse VM texture manager for terminal rendering
        ...
        #[cfg(feature = "hypervisor")]
        if let (Some(vm), Some(window_id), Some(vm_texture_manager)) =
            (&self.vm, self.vm_window_id, &mut self.vm_texture_manager)
        {
                    ...
                    if running {
                        ...
                        // Phase 32: Handle window resize (inline to avoid borrow conflict)
                        {
                            ...
                            if new_rows != current_rows || new_cols != current_cols {
                                log::info!(
                                    "📐 VM window resized: {}x{} -> {}x{} terminal",
                                    current_cols,
                                    current_rows,
                                    new_cols,
                                    new_rows
                        ...
                        if let Some(terminal_buffer) = locked_vm.get_terminal_buffer() {
                            let selection_range = locked_vm
                                .get_terminal_clipboard()
                        ...
                        if let Err(e) =
                            vm_texture_manager.update_vm_texture(window_id, &fb, width, height)
                        {
    ...
    fn update_tectonic_bridge(&mut self) {
        ...
        if let (Some(ref bridge), Some(ref mut simulator)) =
            (&self.qemu_bridge, &mut self.tectonic_simulator)
        {
    ...
    pub fn handle_surface_commit(&mut self, surface: &WlSurface) {
        ...
        log::debug!(
            "Surface Commit {:?}: Damage rects: {:?}, Full Update: {}",
            surface.id(),
            damage_rects,
            full_update_needed
        );
        ...
        with_renderer_surface_state(surface, |state| {
            if let Some(buffer) = state.buffer() {
                ...
                if let Ok((data, width, height, format)) = result {
                    ...
                    if let Some(wgpu_format) =
                        crate::surface_manager::wayland_format_to_wgpu(format_u32)
                    {
                    ...
                    log::warn!(
                        "Failed to read buffer contents for surface {:?}",
                        surface.id()
                    );
                }
            }
        ...
        if let Some((data, width, height, format)) = texture_update {
            if let Err(e) = self.renderer.get_surface_manager().update_surface(
                ...
                height,
                format,
                if full_update_needed || damage_rects.is_empty() {
                    None
                } else {
                    Some(&damage_rects)
                },
            ...
            SmithayInputEvent::Keyboard { event, .. } => {
                        ...
                        47 => {
                            ...
                            if let Some(clipboard) = &self.clipboard_manager {
                                if let Some(contents) = clipboard.get_text() {
                                    if let Some(text_engine) = &mut self.renderer.text_engine {
                                        text_engine
                                            .replace_selection(&self.renderer.queue, &contents);
                                        log::info!(
                        ...
                        44 => {
                            ...
                            if is_shift {
                                ...
                                if let Some(_client) = &self.zai_client {
                                    ...
                                    log::warn!(
                                        "⚠️ Z.ai Client not initialized (Check ZAI_API_KEY)"
                                    );
                    ...
                    && !self.input_manager.is_window_possessed()
                {
                    if key_code == 16 || key_code == 24 || key_code == 113 {
                        ...
                        if iso.is_empty() {
                            log::error!(
                                "❌ alpine.iso not found in CWD or parents. Cannot spawn VM."
                            );
                    ...
                    && !self.input_manager.is_window_possessed()
                {
                    ...
                    if let Some((mx, my)) = self.last_mouse_pos {
                        ...
                        let target_id = if let Some(window) = self
                            .window_manager
                            .find_window_at_position(world_pos.x, world_pos.y)
                        {
                            // Check validity (Antigravity Bridge or VM)
                            if Some(window.id) == self.antigravity_window_id
                                || window.has_vm_texture
                                || window.has_memory_texture
                            {
                                Some(window.id)
                            } else {
                                None
                ...
                if let Some(possessed_id) = self.input_manager.get_possessed_window_id() {
                    ...
                    if self.neural_console_window_id == Some(possessed_id) {
                        ...
                        if let Some(console) = &mut self.neural_console {
                            if key_state == smithay::backend::input::KeyState::Pressed {
                                if let Some(byte) =
                                    self.input_manager.map_scancode_to_ascii(key_code)
                                {
                ...
                if let Some(focused_window_id) = self.input_manager.get_focused_window_id() {
                    ...
                    if let (Some(console_id), Some(_)) =
                        (self.neural_console_window_id, &mut self.neural_console)
                    {
                                ...
                                && event.state() == smithay::backend::input::KeyState::Pressed
                            {
                                match key_code.raw() {
                                    ...
                                    47 => {
                                        // Ctrl+V - Paste
                                        if let Some(text) =
                                            self.input_manager.handle_clipboard_paste()
                                        {
                                            self.input_manager.paste_to_console(&text);
                                            log::info!(
                                                "📋 Pasted from clipboard: {} bytes",
                                                text.len()
                                            );
                                            handled = true;
                                        }
                ...
                if let Some(possessed_id) = self.input_manager.get_possessed_window_id() {
                    ...
                    if Some(possessed_id) == self.alpine_vm_window_id {
                        if let Some(alpine_vm) = &self.alpine_vm {
                            ...
                            if let Some(qcode) =
                                crate::alpine_vm::AlpineVmManager::scancode_to_qcode(key_code)
                            {
                ...
                self.input_manager
                    .handle_keyboard_event(state, event, serial, time);
            },
            SmithayInputEvent::PointerMotionAbsolute { event, .. } => {
                        ...
                        if let Some(text_engine) = &mut self.renderer.text_engine {
                            ...
                            if self.text_selection_last_update != Some(grid_index) {
                                if let Some(drag_start) = self.text_selection_drag_start {
                                    ...
                                    self.text_selection_drag_start =
                                        Some(text_engine.local_stats.cursor_pos);
                                }
                ...
                if let (Some(vm_window_id), Some(vm)) = (self.vm_window_id, &self.vm) {
                    ...
                    if let Some(window) = self.window_manager.get_windows().get(vm_window_id) {
                        ...
                        if surface_x >= 0.0
                            && surface_x <= window.width
                            && surface_y >= 0.0
                            && surface_y <= window.height
                        {
                            if let Ok(mut locked_vm) = vm.try_lock() {
            ...
            SmithayInputEvent::PointerButton { event, .. } => {
                ...
                if event.state() == ButtonState::Released {
                    ...
                    if let Some((mx, my)) = self.last_mouse_pos {
                        ...
                        self.handle_source_city_interaction(
                            world_pos.x,
                            world_pos.y,
                            false,
                            event.button(),
                ...
                if event.state() == ButtonState::Pressed {
                    if let Some((mx, my)) = self.last_mouse_pos {
                        ...
                        let boot_target = if let Some(target_window) = self
                            .window_manager
                            .find_window_at_position(world_pos.x, world_pos.y)
                        {
                            if target_window.window_type == crate::window::WindowType::EvolutionZone
                                && target_window.has_cartridge_texture
                            {
                                target_window
                                    .cartridge_texture_id
                                    .as_ref()
                                    .map(|id| (id.clone(), target_window.id))
                            } else {
                                None
                            }
                        } else {
                        ...
                        } else if let Some(target_window) = self
                            .window_manager
                            .find_window_at_position(world_pos.x, world_pos.y)
                        {
                            if target_window.window_type == crate::window::WindowType::EvolutionZone
                            {
                                ...
                                log::info!(
                                    "🧬 Clicked Evolution Zone! Initiating Autonomous Execution..."
                                );
                                    ...
                                    match click_type {
                                        ...
                                        2 => {
                                            ...
                                            if let Some((start, end)) = word_bounds {
                                                ...
                                                log::info!(
                                                    "🖱️🖱️ Double click: selected word {}..{}",
                                                    start,
                                                    end
                                                );
                                            }
                                        ...
                                        3 => {
                                            ...
                                            if let Some((start, end)) = line_bounds {
                                                ...
                                                log::info!(
                                                    "🖱️🖱️🖱️ Triple click: selected line {}..{}",
                                                    start,
                                                    end
                                                );
                                            }
                        ...
                        if self.handle_source_city_interaction(
                            world_pos.x,
                            world_pos.y,
                            true,
                            event.button(),
                        ...
                        if let Some(ref mut raycaster) = self.terrain_raycaster {
                            if let Some(ref terrain) = &self.renderer.neural_terrain {
                                ...
                                if let Some(result) = raycaster.cast_ray(
                                    ...
                                    self.hovered_memory_address =
                                        Some(result.memory_address as usize);
                        ...
                        if let (Some(vm_window_id), Some(vm)) = (self.vm_window_id, &self.vm) {
                            if let Some(window) =
                                self.window_manager.get_windows().get(vm_window_id)
                            {
                                let surface_x = world_pos.x - window.x;
                                let surface_y = world_pos.y - window.y;
                                    ...
                                    && surface_y <= window.height
                                {
                                    ...
                                    if let Some(ref bridge) = self.qemu_bridge {
                                        if let Some(addr) = bridge.xy2h(tex_x, tex_y, 1024) {
                                            ...
                                            log::info!(
                                                "🔍 QEMU RAM: Hovered at 0x{:x} (pixel: {},{})",
                                                addr,
                                                tex_x,
                                                tex_y
                                            );
                        ...
                        if let Some(id) = found_id {
                            ...
                            if let Some(surf) = found_surface {
                                ...
                                self.input_manager
                                    .set_focus(state, Some(surf_clone), Some(id));
                            } else {
            ...
            SmithayInputEvent::PointerAxis { event, .. } => {
                for axis in [Axis::Horizontal, Axis::Vertical] {
                    if let Some(amount) = event.amount(axis) {
                        self.input_manager
                            .handle_pointer_axis(state, axis, amount, time);
                    }
                }
            },
            _ => {},
    ...
    pub fn update_cognitive_system(&mut self) {
        ...
        if let Some(manager) = &self.cognitive_manager {
            ...
            for entity in entities {
                match entity.state {
                    crate::cognitive::entity_manager::EntityState::Active
                    | crate::cognitive::entity_manager::EntityState::Booting => {
                        ...
                        if !self.cognitive_runtimes.contains_key(&entity.id) {
                            ...
                            match crate::cognitive::ace_runtime::ACERuntime::boot_from_texture(
                                ...
                                Err(e) => {
                                    ...
                                    manager.set_state(
                                        &entity.id,
                                        crate::cognitive::entity_manager::EntityState::Error(
                                            e.to_string(),
                                        ),
                                    );
                                },
                            ...
                            if let Some(runtime) = self.cognitive_runtimes.get_mut(&entity.id) {
                                if let Err(e) = runtime.think() {
                                    log::error!(
                                        "❌ Error in entity {} during thought process: {}",
                                        entity.id,
                                        e
                                    );
        ...
        if let Some(ref mut agent_mgr) = self.agent_manager {
            ...
            while let Some(request) = agent_mgr.requests.pop_front() {
                match request {
                    ...
                    crate::cognitive::agents::AgentRequest::Log {
                        message,
                        agent_id: _,
                    } => {
        ...
        self.recent_agent_actions
            .retain(|_, (_, ts)| now - *ts < 10.0); // Keep last 10 seconds
    ...
    pub fn spawn_city_agents_batch(&mut self, count: usize) {
        if let Some(ref mut mgr) = self.agent_manager {
            ...
            log::info!(
                "🤖 Phase 46.5: Spawned {} additional agents (Total: {})",
                count,
                mgr.list_agents().len()
            );
        }
    ...
    pub fn trigger_memory_analysis(&mut self, address: usize) {
        log::info!(
            "🧠 Triggering semantic analysis for address 0x{:x}",
            address
        );
    ...
    pub fn handle_overlay_input(
        ...
        match key_code {
            ...
            30 => {
                self.synaptic_overlay.insert_char('a');
            },
            48 => {
                self.synaptic_overlay.insert_char('b');
            },
            46 => {
                self.synaptic_overlay.insert_char('c');
            },
            32 => {
                self.synaptic_overlay.insert_char('d');
            },
            18 => {
                self.synaptic_overlay.insert_char('e');
            },
            33 => {
                self.synaptic_overlay.insert_char('f');
            },
            34 => {
                self.synaptic_overlay.insert_char('g');
            },
            35 => {
                self.synaptic_overlay.insert_char('h');
            },
            23 => {
                self.synaptic_overlay.insert_char('i');
            },
            36 => {
                self.synaptic_overlay.insert_char('j');
            },
            37 => {
                self.synaptic_overlay.insert_char('k');
            },
            38 => {
                self.synaptic_overlay.insert_char('l');
            },
            50 => {
                self.synaptic_overlay.insert_char('m');
            },
            49 => {
                self.synaptic_overlay.insert_char('n');
            },
            24 => {
                self.synaptic_overlay.insert_char('o');
            },
            25 => {
                self.synaptic_overlay.insert_char('p');
            },
            16 => {
                self.synaptic_overlay.insert_char('q');
            },
            19 => {
                self.synaptic_overlay.insert_char('r');
            },
            31 => {
                self.synaptic_overlay.insert_char('s');
            },
            20 => {
                self.synaptic_overlay.insert_char('t');
            },
            22 => {
                self.synaptic_overlay.insert_char('u');
            },
            47 => {
                self.synaptic_overlay.insert_char('v');
            },
            17 => {
                self.synaptic_overlay.insert_char('w');
            },
            45 => {
                self.synaptic_overlay.insert_char('x');
            },
            21 => {
                self.synaptic_overlay.insert_char('y');
            },
            44 => {
                self.synaptic_overlay.insert_char('z');
            },
            // 0-9
            2 => {
                self.synaptic_overlay.insert_char('1');
            },
            3 => {
                self.synaptic_overlay.insert_char('2');
            },
            4 => {
                self.synaptic_overlay.insert_char('3');
            },
            5 => {
                self.synaptic_overlay.insert_char('4');
            },
            6 => {
                self.synaptic_overlay.insert_char('5');
            },
            7 => {
                self.synaptic_overlay.insert_char('6');
            },
            8 => {
                self.synaptic_overlay.insert_char('7');
            },
            9 => {
                self.synaptic_overlay.insert_char('8');
            },
            10 => {
                self.synaptic_overlay.insert_char('9');
            },
            11 => {
                self.synaptic_overlay.insert_char('0');
            },
            // Special characters
            12 => {
                self.synaptic_overlay.insert_char('-');
    ...
    pub fn initialize_memory_artifact_manager(&mut self) {
        ...
        self.memory_artifact_manager = Some(crate::memory_artifacts::MemoryArtifactManager::new(
            device, queue,
        ));
    ...
    pub fn hydrate_source_city_artifacts(&mut self) {
        ...
        let tiles_data: Vec<(usize, std::path::PathBuf, f32, f32, f32, f32, f32, f32)> =
            if let Some(loader) = &self.source_city_loader {
                if let Some(layout) = loader.layout() {
                    eprintln!(
                        "🏛️  Hydrating Source City Artifacts: {} tiles",
                        layout.tiles.len()
                    );
                    layout
                        .tiles
        ...
        if let Some(manager) = &mut self.memory_artifact_manager {
            ...
            manager.update_buffers();
            log::info!(
                "✅ Source City Artifacts Hydrated and Uploaded to GPU (Mapped {} paths)",
                self.path_to_artifact.len()
            );
        }
    }

...
use std::sync::Arc;
use wgpu::{
    BindGroup, BindGroupLayout, Device, Extent3d, ImageCopyTexture, ImageDataLayout, Queue,
    Sampler, Texture, TextureDescriptor, TextureDimension, TextureFormat, TextureUsages,
    TextureView,
};
...
impl VmTextureManager {
    ...
    pub fn register_external_texture(
        ...
        let bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some(&format!(
                "External VM Texture BindGroup Window {}",
                window_id
            )),
            layout: &self.bind_group_layout,
            entries: &[
        ...
        for row in 0..rows {
            for col in 0..cols {
                ...
                if let Some(cell) = terminal_buffer.get_render_cell(row, col) {
                    ...
                    let is_selected = if let Some(((start_row, start_col), (end_row, end_col))) =
                        selection_range
                    {
                    ...
                    // Cursor check
                    let is_cursor = view_offset == 0
                        && cursor_visible
                        && cursor_blink_state > 0.5
                        && row == cursor_row
                        && col == cursor_col;

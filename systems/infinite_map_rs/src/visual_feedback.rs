...
use crate::evolution_protocol::{EvolutionClient, MessageType, ProtocolMessage};
use serde::{Deserialize, Serialize};
use std::io;
use std::sync::{Arc, Mutex};
use std::time::{Duration, Instant};
use wgpu::{
    self, Device, Extent3d, ImageCopyTexture, ImageDataLayout, Queue, Texture, TextureFormat,
...
impl VisualFeedbackEncoder {
    ...
    pub fn capture_feedback(&mut self, source_texture: &Texture) -> io::Result<Vec<u8>> {
        let feedback_texture = self.feedback_texture.as_ref().ok_or_else(|| {
            io::Error::new(
                ...
                "Feedback texture not initialized",
            )
        })?;
    ...
    pub fn send_feedback(
        &mut self,
        feedback_data: &VisualFeedbackData,
        encoded_data: &[u8],
    ) -> io::Result<()> {
    ...
    pub fn update(&mut self, source_texture: &Texture) -> io::Result<()> {
        ...
        if self.feedback_texture.is_none()
            || self.feedback_texture.as_ref().unwrap().width()
                != size.width / self.config.downsample_factor
            || self.feedback_texture.as_ref().unwrap().height()
                != size.height / self.config.downsample_factor
        {
...
mod base64 {
    ...
    pub fn decode(encoded: &str) -> Result<Vec<u8>, String> {
        ...
        let chars: Vec<u8> = encoded
            .chars()
            .filter_map(|c| TABLE.iter().position(|&b| b as char == c).map(|i| i as u8))
            .collect();

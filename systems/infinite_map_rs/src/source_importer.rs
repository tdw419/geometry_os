...
fn derive_module_path(path: &Path) -> String {
    ...
    let components: Vec<_> = path
        .components()
        .skip_while(
            |c| matches!(c, std::path::Component::Normal(s) if s.as_encoded_bytes() == b"src"),
        )
        .collect();
...
impl SourceImporter {
    ...
    pub fn import_crate(&mut self, origin: (i32, i32)) -> Result<CrystallizedProject, ImportError> {
        ...
        log::info!(
            "✅ Source crystallization complete: {} tiles",
            self.tiles.len()
        );
    ...
    fn parse_cargo_toml(&self) -> Result<CrateManifest, ImportError> {
            ...
            for (name, value) in deps {
                let (version, optional, features): (String, bool, Vec<String>) = match value {
                    ...
                    toml::Value::Table(t) => {
                        ...
                        let opt = t.get("optional").and_then(|v| v.as_bool()).unwrap_or(false);
                        let feats: Vec<String> = t
                            .get("features")
                            .and_then(|v| v.as_array())
                            .map(|arr| {
                                arr.iter()
        ...
        // Parse features
        let features: HashMap<String, Vec<String>> = parsed
            .get("features")
    ...
    fn district_for_path(&self, path: &Path) -> String {
        ...
        let parent = relative.parent().and_then(|p| p.to_str()).unwrap_or("src");
...
impl SourceSync {
    ...
    pub fn import_changes(
        &self,
        tiles: &mut [CrystallizedSourceTile],

...
use infinite_map_rs::terminal_clone::{TerminalCloneManager, PtyEngine};
#[cfg(feature = "hypervisor")]
...
fn test_terminal_clone_full_pipeline() {
    ...
    let id = manager.create_terminal(24, 80, "/bin/sh")
        .expect("Failed to create terminal");
    
    // 2. Write a command to the PTY
    let cmd = b"echo 'GEOMETRY_OS_TEST'
";
    manager.write_to_terminal(id, cmd).expect("Failed to write to terminal");
    ...
    assert!(found, "Terminal output did not contain expected test string within timeout");
    
    // 4. Test Resize propagation
    manager.resize_terminal(id, 30, 120).expect("Failed to resize terminal");

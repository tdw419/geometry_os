...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    switch (glyph.opcode) {
        ...
        case 215u: { // DRAW (Spatial Blit)
            ...
            if (mem_len > 61443u) {
                // Check if bounds are set/non-zero
                let wx = memory[61440u];
                let wy = memory[61441u];
                let ww = memory[61442u];
                let wh = memory[61443u];
                if (ww > 0.0 && wh > 0.0) {
                    win_x = wx;
                    win_y = wy;
                    win_w = ww;
            ...
            for (var row = 0u; row < 64u; row++) {
                for (var col = 0u; col < 64u; col++) {
                    let pixel_pos = dst_pos + vec2<u32>(col, row);
                    if (f32(pixel_pos.x) >= win_x && f32(pixel_pos.x) < (win_x + win_w) &&
                        f32(pixel_pos.y) >= win_y && f32(pixel_pos.y) < (win_y + win_h)) {
                        let texel = textureLoad(atlas, src_cell * 64u + vec2<u32>(col, row), 0);
                        textureStore(screen, pixel_pos, texel);
                    }
                }
            }
        }

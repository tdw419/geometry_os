...
async def _capture_framebuffer(region: dict | None) -> ToolResult:
...
async def _navigate_map(x: float, y: float, zoom: float | None) -> ToolResult:
...
async def _run_pixel_program(program: str, region: dict | None) -> ToolResult:
...
async def _trigger_evolution(region: dict | None, generations: int) -> ToolResult:

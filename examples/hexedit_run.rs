...
fn run_hexedit(elf_data: &[u8], input: &[u8], max_steps: u64) -> (Vec<u32>, u32, String) {
    ...
    // Load ELF directly — hexedit.elf is linked at absolute 0x80000000,
    // boot_guest adds offset which doubles the address.
    let load_info =
        geometry_os::riscv::loader::load_elf(&mut vm.bus, elf_data).expect("ELF load failed");
    vm.cpu.pc = load_info.entry;
    vm.cpu.privilege = geometry_os::riscv::cpu::Privilege::Machine;
    vm.cpu.x[10] = 0; // a0 = hartid
    vm.cpu.x[11] = 0; // a1 = 0 (no DTB)

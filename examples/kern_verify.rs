...
fn main() {
    ...
    eprintln!("[kern_verify] entry=0x{:08X} ran {} instructions", r.entry, r.instructions);
    eprintln!("[kern_verify] final pc=0x{:08X} mcause=0x{:08X} mepc=0x{:08X} mtval=0x{:08X}",
        vm.cpu.pc,
        vm.cpu.csr.read(0x342),
        vm.cpu.csr.read(0x341),
        vm.cpu.csr.read(0x343));
    eprintln!("[kern_verify] mscratch=0x{:08X} mtvec=0x{:08X}",
        vm.cpu.csr.read(0x340),
        vm.cpu.csr.read(0x305));
    eprintln!("[kern_verify] sp=0x{:08X} ra=0x{:08X} tp=0x{:08X}",
    ...
    for &b in raw { *hist.entry(b).or_insert(0u32) += 1; }
    for (b, c) in &hist {

...
fn main() {
    ...
    eprintln!("[verify] left half (painter): {} non-black pixels, {} unique colors", 
              left_nonblack, left_colors.len());
    eprintln!("[verify] right half (life32): {} non-black pixels, {} unique colors", 
              right_nonblack, right_colors.len());
    
    // Sample some specific pixels
    eprintln!("[verify] sample pixels:");
    for (x, y) in [(10, 10), (64, 128), (127, 200), (140, 64), (200, 128), (250, 200)] {
        let idx = (y * 256 + x) as usize;
        if idx < pixels.len() {
            let px = pixels[idx];
            eprintln!("  ({:3},{:3}) = 0x{:08X} (R={} G={} B={} A={})",
                x, y, px, (px>>24)&0xFF, (px>>16)&0xFF, (px>>8)&0xFF, px&0xFF);
        }
    }
    ...
    if left_nonblack < 100 {
        eprintln!("[FAIL] Left half has only {} non-black pixels (expected 100+)", left_nonblack);
        pass = false;
    }
    if right_nonblack < 50 {
        eprintln!("[FAIL] Right half has only {} non-black pixels (expected 50+)", right_nonblack);
        pass = false;
    }
    if left_colors.len() < 10 {
        eprintln!("[FAIL] Left half has only {} unique colors (expected 10+)", left_colors.len());
        pass = false;
    }

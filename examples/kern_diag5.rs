...
fn main() {
    ...
    for i in 0..max_instr {
        ...
        if vm.cpu.csr.mepc != last_mepc {
            ...
            if mepc != 0x80000340 {
                ...
                if ctx_switches <= 20 {
                    println!(
                        "[CTX-SWITCH #{}] instr={}: mepc=0x{:08X} pc=0x{:08X} tp=0x{:08X}",
        ...
        if i > 0 && i % check_interval == 0 {
            ...
            println!(
                "[DIAG] {} instr: pc=0x{:08X} timer_fires={} ctx_switches={} mip=0x{:08X}",
    ...
    println!(
        "[FINAL] sscratch=0x{:08X} mtvec=0x{:08X} mepc=0x{:08X}",
        vm.cpu.csr.sscratch, vm.cpu.csr.mtvec, vm.cpu.csr.mepc
    );
    println!(
        "[FINAL] sp=0x{:08X} ra=0x{:08X} tp=0x{:08X}",

...
build_all() {
    ...
    for src in *.c; do
        ...
        # Skip geos_kern.c itself (built via build_kern.sh)
        [ "$src" = "geos_kern.c" ] && continue
        # Skip libc-related test programs (no rv32 glibc headers on system)
        [[ "$src" == libc_* ]] && continue
        # Skip linux userspace test programs
        [[ "$src" == linux_* ]] && continue
        # Skip multiproc programs and custom guests
        [[ "$src" == multiproc_* ]] && continue
        [ "$src" = "painter_left.c" ] && continue
        # Skip net syscall test programs (network feature disabled)
        [[ "$src" == net_* ]] && continue
        # Skip shm test programs
        [[ "$src" == shm_* ]] && continue
        # Skip viewer.c due to header conflicts with vfs_pixel.h
        [ "$src" = "viewer.c" ] && continue
        # Derive output name: foo.c -> foo.elf
        local base="${src%.c}"
        build_one "$src" "${base}.elf"
    done
    # Build geos_kern.elf using the specialized build script
    bash build_kern.sh
}

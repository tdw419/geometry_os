...
class SystemObserverBlogger:
    ...
    def _publish_report(self, new_files, modified_files):
        print(f"🧐 Detected changes in {self.watch_dir}")
        ...
        if new_files:
            content += "<h3>🆕 New Components</h3><ul>"
            for f in new_files:
                content += f"<li><code>{f}</code></li>"
            content += "</ul>"
            
        if modified_files:
            content += "<h3>⚡ Modified Modules</h3><ul>"
    ...
    def run(self):
        print(f"🕵️ System Observer Blogger active. Watching '{self.watch_dir}'...")

...
// Uses Sv32: 2-level page table with 10-bit indices.
pagetable_t
kvmmake(void)
{
  pagetable_t kpgtbl;
  ...
  // map the framebuffer MMIO region (256x256 RGBA at 0x6000_0000, 5MB covers pixel buffer + control regs).
...
// given a process and a virtual address, ensure that the page
// table has a mapping for that address; if not, allocate a page
// (from the kernel) and map it. used for lazy allocation and
// for copy-on-write.
int
vmfault(pagetable_t pagetable, uint32 va, int is_write)
{
  uint32 pa;
  pte_t *pte;
...
// Copy len bytes from virtual address srcva in a given page table to dst.
// Return 0 on success, -1 on error.
int

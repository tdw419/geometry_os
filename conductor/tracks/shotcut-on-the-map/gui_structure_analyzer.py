...
class GUIAnalyzer:
    ...
    def generate_ascii_view(self, elements: List[UIElement], width: int = 80,
                            height: int = 24) -> str:
        """
        ...
        for e in elements:
            ...
            bbox = f"{e.bbox[0]},{e.bbox[1]},{e.bbox[2]},{e.bbox[3]}"
            action = f"action:click_{e.text.lower()}"
            clickables.append(f"# [CLICKABLE: {bbox} -> {action}]")
    ...
    def analyze_with_clustering(self, image_path: str, width: int = 80,
                                height: int = 24) -> AnalysisResult:
        """
...
def analyze_screenshot(image_path: str, width: int = 80,
                       height: int = 24) -> AnalysisResult:
    """

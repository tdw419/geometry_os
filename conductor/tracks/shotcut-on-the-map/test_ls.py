...
async def test():
    try:
        async with websockets.connect('ws://127.0.0.1:8768') as ws:
            ...
            print("[*] Typing 'ls -la'...")
            await ws.send(json.dumps({'command': 'type', 'params': {'text': 'ls -la\n'}}))
            print(await ws.recv())
            ...
            with open('conductor/tracks/shotcut-on-the-map/screenshots/ls_test.png', 'wb') as f:
                f.write(base64.b64decode(res['image']))
            print("[+] Saved screenshot: ls_test.png")

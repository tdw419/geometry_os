...
| 1.7 Player Profile & Dashboard | Complete | 3m | WOR_Player_API with 4 REST endpoints + dashboard shortcode |
| 1.8 Easy/Hard Mode Toggle | Complete | 3m | quest-single.php template + wor-frontend.css + enqueue hooks |
| 2.1 Truth Engine | Complete | 3m | WOR_Truth_Engine + WOR_Verify_API with peer/algorithmic/oracle verification |
...
- WOR_Player_API uses singleton pattern, same API_NAMESPACE 'wor/v1'
- REST endpoints: GET /player/me, POST /player/sefirah, GET /player/stats, GET /player/history
- All endpoints require authentication (check_auth method)
- Profile includes: sefirah, credits_balance, ctrm_score, tms_level, current_role, quest counts
- Role definitions: Visitor (0), Player (1), Contributor (2), TMS Member (3), GEMS Manager (4), Oracle (5)
- Sefirah selection stores in wor_player_sefirah user meta
- Quest history queried from wor_completion posts with 'completed' status
- Dashboard shortcode [wor_dashboard] with show_history and history_limit attributes
- Dashboard template uses inline CSS with responsive grid layout
- Progress bar calculates level progress percentage from CTRM score
- Quest single template uses [wor_quest] shortcode with optional id attribute
- Template redirect hook handles single wor_quest posts automatically
- Easy mode: obvious choices, standard credits
- Hard mode: contributes to Geometry OS, 2.5x credits, higher CTRM
- Sefirah bonus (1.5x) applies on top of mode multiplier
- Frontend CSS uses CSS variables for theming (--wor-primary, --wor-warning, etc.)
- Mode selection uses radio inputs with styled labels for accessibility
- WOR_Truth_Engine registers wor_completion post type with meta fields for verification
- Verification status: pending, verified, rejected
- Verification methods: peer, algorithmic, oracle
- Meta fields: wor_quest_id, wor_status, wor_evidence, wor_verified_by, wor_verified_at
- submit_for_verification() stores evidence and sets status to pending
- verify_completion() handles peer verification with self-verification prevention
- auto_verify() runs algorithmic checks with scoring (60+ threshold)
- Algorithmic scoring: photo(40), reasoning(30), time(15), mode(15)
- Verification triggers award_verification_rewards() for credits and CTRM
- WOR_Verify_API provides REST endpoints: POST /completions, GET /completions/pending, POST /completions/{id}/verify, POST /completions/{id}/reject
- GET /completions/me returns current user's completions with stats

## Next
Task 2.2: Node Sync Manager

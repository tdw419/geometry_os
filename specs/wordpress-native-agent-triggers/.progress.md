...
- [x] 4.1 Create Integration Tests - (pending commit)
...
- Integration tests use `@pytest.mark.usefixtures("require_visual_bridge")` for HTTP endpoint tests
- Handler tests run without Visual Bridge dependency (5 pass, 8 skip when bridge not running)
- `pytest.fixture(autouse=True)` applies to all tests; use explicit `usefixtures` for selective skip
- Visual Bridge HTTP port 8769, not 8768 (WebSocket is 8768)

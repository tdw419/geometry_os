...
/* Get file count */
static inline uint32_t vfs_file_count(void) {
    return VFS_PIXEL(0, 1);
}

/* Find a file by name. Returns start_row or 0 if not found. */
...
/* Create a new file with initial data. Returns start_row or 0 on failure. */
...
/* Read file data into buffer. Returns bytes read or -1. */
...
static inline int vfs_append(const char *name, uint32_t len, const void *data) {
    uint16_t sr = vfs_find(name);
    if (sr == 0) return -1;
    uint32_t header = VFS_PIXEL(sr, 0);
    uint32_t bc = header >> 16;
    ...
    uint32_t dc = guest ? 64 : 1;
    const uint8_t *bytes = (const uint8_t *)data;
    uint32_t new_end = bc + len;
    uint32_t pc = (new_end + 3) / 4;
    for (uint32_t i = 0; i < pc; i++) {
        uint32_t pixel = 0;
        for (int j = 0; j < 4; j++) {
            uint32_t idx = i * 4 + j;
            if (idx >= bc && idx < new_end) {
                pixel |= (uint32_t)bytes[idx - bc] << (j * 8);
            } else if (idx < bc) {
                uint16_t col = dc + i;
                uint16_t row = sr + col / 256;
                uint32_t old = VFS_PIXEL(row, col % 256);
                pixel |= (old >> (j * 8)) & 0xFF;
            }
        }
        uint16_t col = dc + i;
    ...
    uint32_t hash = (header >> 8) & 0xFF;
    VFS_PIXEL(sr, 0) = ((new_end & 0xFFFF) << 16) | (hash << 8) | (header & 0xFF);
    VFS_CONTROL = VFS_CTRL_FLUSH;
    return (int)len;
}
...
static inline int vfs_unlink(const char *name) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        uint16_t sr = (uint16_t)(entry >> 16);
        if (sr == 0) continue;
        if ((entry & 0xFFFF) != (hash & 0xFFFF)) continue;
        uint32_t header = VFS_PIXEL(sr, 0);
...
/* Trigger flush of dirty data to host filesystem */

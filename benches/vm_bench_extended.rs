...
fn bench_icache(c: &mut Criterion) {
    ...
    // Phase 382: Threshold tuning benchmarks
    // Measure how different hot thresholds affect throughput for the same tight loop.
    // Lower thresholds cache sooner but may cache one-off code. Higher thresholds
    // delay caching but avoid polluting the cache with init code.
    for threshold in [1u32, 2, 4, 8] {
        let name = format!("tight_loop_threshold_{}", threshold);
        group.bench_function(&name, |b| {
            b.iter_batched(
                || {
                    let mut vm = vm_with_source(tight_loop);
                    vm.icache_hot_threshold = threshold as usize;
                    vm
                },
                |mut vm| {
                    let s = run_steps(&mut vm, 200_000);
    ...
    // Phase 382: Nested loop pattern (icache behavior differs from flat loops)
    let nested_loop = r#"
        LDI r1, 0
        LDI r2, 100
        LDI r3, 0
    ...
    group.throughput(Throughput::Elements(10000));
    group.bench_function("nested_loop_10k", |b| {
        b.iter_batched(
            || vm_with_source(nested_loop),
            |mut vm| {
                let s = run_steps(&mut vm, 500_000);
                black_box(s);
            },
            BatchSize::SmallInput,
        );
    ...
    // Phase 382: Game loop pattern (init + tight hot loop)
    let game_loop = r#"
        LDI r1, 0
        LDI r2, 100000
        LDI r7, 1
    ...
    group.bench_function("game_loop_100k", |b| {
        b.iter_batched(
            || vm_with_source(game_loop),
            |mut vm| {
                let s = run_steps(&mut vm, 500_000);
                black_box(s);
            },

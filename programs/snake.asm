...
;   0x2207          tick_mask (speed, 7=slowest, 1=fastest)
;   0x2208          high_score (persistent across games)
;
...
restart:
  ...
  LDI r1, 7           ; initial tick mask (move every 8 frames)
  LDI r4, 0x2207
  STORE r4, r1

  ; Initialize high score to 0 if not already set (first run)
  LDI r4, 0x2208
  LOAD r1, r4
  JZ r1, high_score_ok ; if high_score != 0, keep it
  LDI r1, 0
  STORE r4, r1        ; else set to 0
high_score_ok:
...
after_input:
  ; throttle: only move snake every (tick_mask+1) frames
  ; tick mask decreases as snake grows, making it faster
  LDI r4, 0xFFE
  LOAD r8, r4           ; r8 = TICKS
  LDI r4, 0x2207
  LOAD r9, r4           ; r9 = tick_mask
  AND r8, r9            ; r8 = TICKS & tick_mask
  JZ r8, do_move        ; move on frame where TICKS & mask == 0
  CALL draw_frame
  FRAME
  JMP game_loop
...
; ── game over screen ─────────────────────────────────────────────
game_over_screen:
  ; Calculate final score (length - 3, since we start with 3 segments)
  LDI r4, 0x2202
  LOAD r10, r4           ; r10 = length
  LDI r9, 3
  SUB r10, r9            ; r10 = score = length - 3
  
  ; Update high score if current score is higher
  LDI r4, 0x2208
  LOAD r11, r4           ; r11 = high_score
  CMP r10, r11
  BLT r0, skip_high_score ; if score < high_score, skip update
  STORE r4, r10          ; else update high_score
skip_high_score:
  ...
  ; Draw high score in cyan below score
  LDI r2, 20
  LDI r3, 60
  LDI r4, 100
  LDI r5, 30
...
wall_check:
  ...
  ; Iterate body ring buffer from tail, length times.
  ; If any cell matches new head (r11), trigger game over.
  LDI r4, 0x2201
  LOAD r26, r4           ; r26 = current index = tail
  LDI r4, 0x2202
  LOAD r27, r4           ; r27 = remaining cells to check = length

self_coll_loop:
  JZ r27, self_coll_done ; if no cells left, pass
  LDI r4, 0x2000
  ADD r4, r26
  LOAD r28, r4           ; r28 = body cell at current index
  CMP r28, r11
  JZ r0, do_wall_hit     ; collision! new head hits body
...
do_eat_apple:
  ...
  ; update tick mask: decrease by 1 every 10 apples (down to minimum 1)
  LDI r4, 0x2202
  LOAD r9, r4           ; r9 = length
  LDI r20, 10
  SUB r9, r20           ; r9 = length - 10
  LDI r20, 0
  CMP r9, r20           ; if length < 10, no speed increase yet
  BLT r0, skip_speed_update
  LDI r4, 0x2207
  LOAD r10, r4          ; r10 = current tick mask
  LDI r9, 7
  SUB r10, r9           ; decrease mask (increase speed)
  LDI r9, 1
  CMP r10, r9           ; minimum mask is 1 (fastest speed)
  BGE r0, save_tick_mask
  LDI r10, 1            ; clamp to minimum
save_tick_mask:
  STORE r4, r10
skip_speed_update:

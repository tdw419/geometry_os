# Visual File Browser Implementation Plan
...
## Task 1: FileInfo Dataclass and Color Coding

**Files:**
- Create: `file_browser.py:1-80`

**Step 1: Create the file with imports and FileInfo dataclass**
...
git commit -m "feat(browser): add FileInfo dataclass and color coding"
```

---

## Task 2: LS Output Parser

**Files:**
- Modify: `file_browser.py` (add after FileInfo class)

**Step 1: Add the parse_ls_output function**
...
def parse_ls_output(output: str, parent_path: str) -> List[FileInfo]:
    ...
    for line in lines[1:]:  # Skip "total X" line
        ...
        # Example: drwxr-xr-x  5 user group 4096 Feb 15 10:30 directory_name
...
```bash
git add file_browser.py
git commit -m "feat(browser): add ls output parser"
```

---

## Task 3: Spatial Layout Engine

**Files:**
- Modify: `file_browser.py` (add after parser)

**Step 1: Add the SpatialLayout class**
...
```bash
git add file_browser.py
git commit -m "feat(browser): add spatial layout engine with grid and radial modes"
```

---

## Task 4: FileBrowser Class

**Files:**
- Modify: `file_browser.py` (add after SpatialLayout)

**Step 1: Add the FileBrowser class**
...
```bash
git add file_browser.py
git commit -m "feat(browser): add FileBrowser class with navigation"
```

---

## Task 5: Tile Renderer

**Files:**
- Modify: `file_browser.py` (add render method to FileBrowser)

**Step 1: Add the render method to FileBrowser class**
    ...
    async def draw_tile(self, f: FileInfo, id: str):
        ...
        js = f"""
            (function() {{
                ...
                const isDir = {str(f.file_type == 'directory').lower()};
...
```bash
git add file_browser.py
git commit -m "feat(browser): add tile renderer with color-coded backgrounds"
```

---

## Task 6: ClickServer for Tile Interaction

**Files:**
- Modify: `file_browser.py` (add after FileBrowser)

**Step 1: Add the ClickServer class**
...
```bash
git add file_browser.py
git commit -m "feat(browser): add ClickServer for tile interaction"
```

---

## Task 7: Browser-Side Click Handler

**Files:**
- Modify: `file_browser.py` (add init_display method to FileBrowser)

**Step 1: Add init_display method to FileBrowser class**
...
**Step 2: Update render method to register tiles in browser**

Modify the render method to register tile positions:
...
```bash
git add file_browser.py
git commit -m "feat(browser): add browser-side click handler with tile registry"
```
...
**Files:**
- Modify: `file_browser.py` (add main function and entry point)

**Step 1: Add argument parsing and main function**
...
```bash
git add file_browser.py
git commit -m "feat(browser): add CLI and main loop"
```
...
**Step 2: Start the file browser**

```bash
python3 file_browser.py --path /home/$USER
```

**Step 3: Verify the following**

1. Files appear as color-coded tiles on the map
2. Directories are blue with borders
3. Different file types have different colors
4. Clicking a directory expands it radially
5. Clicking an expanded directory collapses it
6. Clicking a file logs to console

**Step 4: Document verification**

```bash
echo "Phase 50.6: Visual File Browser - VERIFIED" >> VERIFICATION.md
```
...
|------|-------------|-------|
| 1 | FileInfo dataclass and color coding | `file_browser.py` |
| 2 | LS output parser | `file_browser.py` |
| 3 | Spatial layout engine | `file_browser.py` |
| 4 | FileBrowser class | `file_browser.py` |
| 5 | Tile renderer | `file_browser.py` |
| 6 | ClickServer for interaction | `file_browser.py` |
| 7 | Browser-side click handler | `file_browser.py` |
| 8 | CLI and main loop | `file_browser.py` |
| 9 | Manual verification | Manual |
